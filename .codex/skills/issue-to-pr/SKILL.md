...
8. PR 作成前のローカルセルフレビューとして `$local-review` を必ず使う。
   - 対象は現在のローカル差分、staged / unstaged diff、必要に応じて `main...HEAD` とする。
   - `$local-review` は対象分野をひとつに絞り、4観点の multi-agent review をデフォルトで行う。
   - 指摘が出た場合は、Issue scope と照合して必要な修正を行い、関連する検証を再実行する。
   - 対応しない指摘がある場合は、理由を PR 本文の残リスクまたは計画変更に明記する。
   - PR 番号起点のレビューは `$review-pr` の責務であり、この段階では `$local-review` を使う。
9. 日本語でコミットする。
10. ブランチを push する。
11. 日本語で PR を作成する。
    ...
    - `$local-review` の結果を、実施したテストまたは残リスク欄に簡潔に反映する。
