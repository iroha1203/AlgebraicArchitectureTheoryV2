...
8. PR 作成前のローカルセルフレビューとして、**分野別の敵対レビュー SKILL**
   を必ず使う。
   - 分野は責務境界で選ぶ: Lean 実装(`Formal/`、差分1行でも)と
     `docs/aat/` 台帳は `$math-lean-review`、`tools/` と `docs/tool/` は
     `$tool-review`、`website/` と `docs/website/` は `$website-review`、
     docs-only は `$docs-review`(レビューモード)。
     真の分野横断だけ複数 SKILL を使う。
   - 対象は現在のローカル差分、staged / unstaged diff、必要に応じて `main...HEAD` とする。
   - 各 SKILL の multi-agent review(Lean は4並列査読)を省略しない。
     サブエージェントが起動できない場合、セルフレビュー済みと主張せず、
     PR 本文の残リスクに「セルフレビュー未実施(環境制約)」と明記する。
   - 指摘が出た場合は、Issue scope と照合して必要な修正を行い、関連する検証を再実行する。
   - 対応しない指摘がある場合は、理由を PR 本文の残リスクまたは計画変更に明記する。
   - PR 番号起点のレビューは `$review-pr` の責務であり、この段階では分野別 SKILL を直接使う。
9. 日本語でコミットする。
10. ブランチを push する。
11. 日本語で PR を作成する。
    ...
    - 分野別セルフレビューの結果を、実施したテストまたは残リスク欄に簡潔に反映する。
