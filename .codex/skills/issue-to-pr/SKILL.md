...
2. Issue を確認し、**タスク型を確定する**。
   ...
   - **移植型の規律**(通常の修正と規律が反転する):
     ...
     - 実装前に移植元 statement(Issue の `移植元 theorem` /
       `rg` での `research/lean/ResearchLean/` 検索)を
...
12. `gh pr checks` で CI の起動と状態を確認する。単独実行時は `--watch` で
    完走まで確認してよい。prd-loop 文脈では完走を待たない(CI は内容レビューと
    並行して進み、マージ直前に prd-loop 本体が確認する)。
13. 最後に PR URL、主な変更、確認結果を日本語で報告する。

レビューと修正後確認の起動規則は、`.codex/skills/_shared/review-protocol.md`の
「レビューバッチと修正後確認」を正とする。このskillは実装とPR作成を担当し、正式な内容レビューを
実装途中で起動しない。
