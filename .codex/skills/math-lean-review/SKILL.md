...
重い Lean 検証には `docs/aat/guideline.md` の「Lean build 運用(hard rule)」を適用する。
4本の独立査読では CI の `lake build` 結果を共有し、各査読者は `lake build` を実行しない。
追加検証は、親が明示した単一の非aggregate fileへの focused check に限定する。
