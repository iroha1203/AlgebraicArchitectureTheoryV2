...
- 差分が PRD の完了条件、台帳 status、処置ラベル(昇格 / 実質化 / 宣言 / 削除 等)を更新する場合、記載の変更が実体(コード・Lean 宣言・テスト)と一致しているかを確認する。実装・証明・発火を要求する条件を docs 変更だけで完了側へ動かす差分は、PRD がその処置を許可していない限り finding にする。
- Lean 差分では、新規の `Prop + holds` / `_cert` 対、`:= True` 充填フィールド、instance の無い仮定パッケージ定理の導入を finding にする(`docs/aat/guideline.md` の恒久規律)。周辺コードが同じパターンを既に持つことを見逃しの理由にしない。
