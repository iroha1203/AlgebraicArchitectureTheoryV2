...
数学本文・研究GOAL・一次仕様・数学claimの変更は、Lean差分の有無に
...
- 大きな rewrite より、小さな wording update、status table、索引追加を
  優先する。
- 修正後は checklist §6 の横断機械 scan を実行する。Lean 検証は
  `docs/aat/guideline.md`「Lean build 運用(hard rule)」に従う(本体のフル
  `lake build` はPR作成後のCIで確認、subagentの`lake build`は全面禁止、
  focused checkは単一file制限)。該当cargo testは変更範囲に応じて実行する。
