...
1. `git status --short --branch` と `git ls-files --others --exclude-standard` を確認する。ユーザーの未コミット変更は戻さない。対象target Lean fileの差分・未追跡ファイル・最初の変更commitを実装開始の証拠として記録し、contract受理前の実装差分を後からauditで正当化しない。
2. `main` を最新化する。
