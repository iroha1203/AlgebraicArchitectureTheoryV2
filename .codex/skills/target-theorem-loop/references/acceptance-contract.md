...
カード固有の要求と本契約を併せて判定し、旧カード固有の構成経路やwitnessを別GOALへ追加しない。
...
- 証明・反証のどちらも達成とする条項では、カードが事前に指定した各枝の具体的証拠を要求する。
  `Classical.em`、`not_forall`、choiceによる枝や存在の提示だけでは、指定データでの決定・評価・
  構成を放電しない。通常の非計算的存在証明は、targetがその証拠強度を許す場合に使える。
  許容されていない固定主張への反例は、別の成功枝を後から追加せずループの反証停止規則で扱う。
