...
- `target theorem completion criteria`: sorry なし Lean proof、対象 declaration の axiom audit、material premise / hypothesis discharge audit、T3.5 同期、T4 監査、T5 レビュー、T6 `$math-lean-review` を含む完了条件。
...
- `target material premise ledger`: 各 premise について、名前、支える結論、role (`direction-hypothesis` / `ambient-boundary` / `discharge-required`)、必要 discharge artifact、結論相当 premise ではない理由。
...
- completion criteria が SCORE threshold 到達や PR merge だけになっている。
- completion criteria に `$math-lean-review` の正式 4 並列査読 gate が含まれていない、またはこの skill 側で gate を実行できない。
