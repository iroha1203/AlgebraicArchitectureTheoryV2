...
3. 既存 Issue と照合する。
   - `gh api --paginate 'repos/{owner}/{repo}/issues?state=open&per_page=100' --jq '.[] | select(.pull_request == null) | {number,title,state,labels:[.labels[].name],milestone:(.milestone.title // null),url:.html_url}'`
   - closed Issue は検索せず、open Issue の pagination を最後まで完走して照合範囲を報告する。取得不能なら
     重複なしと判定せず停止する。
   - 例外: prd-loop の tracking Issue 配下への sub-issue 起票では、照合範囲を
     tracking Issue の既存 sub-issue と対象 PRD に関連する open Issue に
     限定してよい(全 open Issue の走査を省略できる)。
