...
- requirement matrix(各 AC がどの R 番号 / Implementation Plan 区分に
  依存するか)と must-not-remain リストは台帳へ複製しない。手順0で PRD から
  1回抽出し、この実行の作業メモとして保持する。PRD は不変入力なので、
  中断後の再開時も PRD から同じものを再抽出できる。
...
0. 前提を確認する。
   ...
   - PRD ファイルを読み、冒頭の「## 問い」と Acceptance Criteria / 完了条件の節に加えて、
     ...
     `Changed / Removed Fields` を抽出する。requirement matrix(各 AC が
     どの R / PR 区分に依存するか)と must-not-remain リストはここで1回作り、
     この実行の作業メモとして保持する(台帳へは複製しない)。
     matrix は周回の対象選定と、候補状態検査での「実装計画の丸ごと欠落」の
     検出に使う。AC を置き換える合否条件ではない。
   - PRD が Lean 実装(`Formal/`)を要求する場合、PRDまたはtracking Issueが
     statement contractの正本を一意に指定し、実装中に参照できることを確認する
     (`docs/aat/lean_quality_standard.md` §5: target theorem と新規 def の
     signature の Lean コードブロック固定)。正本を特定できなければ
     ループを開始せず `PRD 欠陥` として停止・報告する。
   - ループ台帳(tracking Issue)を確認または作成する。

1. ギャップ分析(プランニング)。
   - **差分駆動で行う。** この実行の初回周回では、PRD の完了条件を1項目ずつ
     現状の実装(対象コード、schema、テスト、fixtures、docs)と突き合わせ、
     `満たした / 未達 / blocked / stalled` に分類する。2周目以降は、再判定の
     対象を次に限定する: (a) 直前の周回が対象とした完了条件、(b) 前回
     `未達 / blocked / stalled` に分類した項目、(c) 直前のマージ差分が触れた
     実装に依存する項目。それ以外の `満たした` 項目は再突合しない
     (全数の再検証は最終達成条件レビューの責務)。
   - 照合は文言ではなく実体で行う: 完了条件が要求するテスト・fixture・検証が
     実際に存在し、通ることを確認する(必要なら `cargo test` 等を実行する)。
   - Lean 実装系の完了条件では、指定されたstatement contractの固定signatureと
     実装 declaration を突合する(`docs/aat/lean_quality_standard.md`
   ...
   - すべて `満たした` と分類できた周回では、手順6へ進む前に**候補状態検査**を行う。
     1. 手順0の requirement matrix で、どの周回にも接続していない R / PR 区分が
        依存先 AC の充足を空洞化していないかを確認する。空洞化していれば
        該当 AC を `未達` に戻す。
     2. 手順0の must-not-remain リストの各項目を `rg` と実読で検査し、
        PRD が解消を宣言した旧構造・旧挙動(旧挙動を「正」として固定し
        続けるテストを含む)が残存していれば、対応する完了条件を `未達` に戻す。
     3. PRD 冒頭の「## 問い」に現状が答えているかを AC scoreboard から独立に
        判定する。AC 全充足でも問いが未回答なら、完了扱いにせずその乖離を
        ユーザーへエスカレートする(ループ自身で問いを再解釈したり
        スコープを拡大したりしない)。
   - 候補状態検査を通過したら: tracking Issue を close せず、手順6の最終達成条件レビューへ進む。
