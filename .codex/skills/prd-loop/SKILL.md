...
- **再レビュー(手順4)**: Needs changes後の修正と確認は、共有review protocolの
  「レビューバッチと修正後確認」に従う。修正はfindingをまとめて行い、直接対応の資格を満たす
  場合はfinding限定の単一subagent確認で足りる。正式ゲートの再実行は資格喪失時に限る。
...
4. レビューし、マージする。
   ...
   - **Mergeable**: 次の3条件が揃った場合だけ
     ...
     1. `$review-pr` の Mergeable 判定。修正が直接対応だけの場合は、初回正式レビューの
        finding全解消と、共有review protocolに従う親の修正後確認・PR監査記録を代用証拠にできる。
     2. CI が green
     3. **レビュー監査コメントが PR に投稿済み**(`$review-pr` の手順6。
        投稿が無い PR はマージしない)
   - **Needs changes**: 共有review protocolに従って指摘をまとめて修正して再検証する。直接対応の
     資格を満たす修正はfinding限定の単一subagent確認で再判定し、資格喪失時だけ実装完了と
     最終スナップショット固定後に正式ゲートを再実行する。修正は同一周回で最大2回まで。2回目の
