...
- **再レビュー(手順4)**: Needs changes後の修正とレビュー再起動は、共有review protocolの
  「レビューバッチと修正後確認」に従う。実装中に正式レビューを起動せず、修正はfindingをまとめて
  行い、実装完了後の最終スナップショットで正式ゲートを再判定する。
...
4. レビューし、マージする。
   - `$review-pr` を実行し、PR を判定する(「サブエージェント活用」参照)。
   - **Mergeable**: 次の3条件が揃った場合だけ
     `gh pr merge <PR> --merge --delete-branch` でマージする
     (このリポジトリは merge commit 方式)。
     1. `$review-pr` の Mergeable 判定。Tool / Docs / Websiteの直接対応だけの場合は、初回正式レビューの
        finding全解消と、共有review protocolに従う親の修正後確認・PR監査記録を代用証拠にできる。
     2. CI が green
     3. **レビュー監査コメントが PR に投稿済み**(`$review-pr` の手順6。
        投稿が無い PR はマージしない)
   - **Needs changes**: 共有review protocolに従って指摘をまとめて修正して再検証する。実装完了と
     最終スナップショット固定後にだけ、必要な正式ゲートを再実行する。修正は同一周回で最大2回まで。2回目の
...
- マージは `$review-pr` の Mergeable 判定、または共有review protocolに従うTool / Docs / Websiteの
  直接対応確認、CI green、修正後確認を含むレビュー監査記録のPR投稿済み、の3条件が揃った場合だけ行う。
