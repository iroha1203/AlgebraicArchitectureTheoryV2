---
name: prd-loop
description: PRD の完了条件を満たすまで、ギャップ分析・Issue 化・実装 PR・レビュー・マージ・最終確認を反復する。"$prd-loop path/to/prd.md"、"PRD を完了まで回して"、PRD 駆動実装の依頼で使う。
---
...
明示的にレビュー gate へ渡す。レビュー gate が実行不能、または `Blocked` を
...
- **再レビュー(手順4)**: Needs changes 後の修正は本体(実装文脈)が行い、
  再レビューは毎回 `$review-pr` にかける。修正とレビューを同じ文脈で完結させない。
