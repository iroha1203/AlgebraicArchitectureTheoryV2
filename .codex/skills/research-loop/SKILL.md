...
- `target anti-weakening rule`: 結論相当の仮定を theorem argument、typeclass、structure field、certificate field、opaque class membership に移して成功扱いすることを禁じる規則。`ambient-boundary` に残せるのは対象 site、finite vocabulary、cover、coefficient object、supplied observation contract などの入力幾何だけであり、target conclusion、faithfulness、effectivity、triviality、global coherence、obstruction vanishing と同値または片方向に近い条件を隠してはならない。
...
G6 では、通常の phase boundary 判定に加えて target completion 判定を行う。大定理証明モードの G6 は事務的な完了確認ではなく、論文の主定理を査読する数学者レベルの proof audit として行う。Lean declaration、自然言語 target、GOAL の claim boundary、support map、依存補題、定義展開後の命題を照合し、主張が精密で、非空で、非自明で、全方向を正しく証明し、必要な前提を隠さず、循環や型による自明化を含まないことを確認する。`target-proof` が完了し、この査読レベルの completion criteria を満たした場合だけ `target-theorem-proved` として止まる。support node が進んだだけなら、SCORE threshold に到達していても stop state は `continue` または、人間に整理判断を返す `target-proof-checkpoint` に留める。theorem package が結論に必要な実質的前提を未 discharge の引数として残している場合は `target-theorem-proved` にしない。GOAL が premise discharge を completion criteria、premise discharge policy、または material premise ledger に含める場合、その前提を Lean theorem、finite witness、または concrete certificate で支えるまでは `target-proof-checkpoint` とする。target theorem が反例で壊れた場合は `target-refuted`、support map が詰まった場合は `target-blocked` として止め、GOAL 改訂または新 target 設計を人間に返す。
...
- Lean で狙う命題が、結論相当の premise を新しい theorem argument、typeclass、structure field、certificate field、opaque class membership に移していないか。
- target material premise ledger にない material premise を導入していないか。導入する場合は target-refinement / target-obstruction として扱い、completion ではなく GOAL 改訂提案または checkpoint に戻すべきか。
...
- `target-theorem` では、target proof state、proof_obligation_delta、material premise ledger、premise discharge status、new / hidden material premise、anti-weakening verdict が PR、report、tracking Issue に忠実に反映されている。
- `git diff --check` が通る。
...
続いて独立サブエージェントに研究フェーズ区切り判定を行わせる。渡すのは GOAL、tracking Issue の active threshold、tracking Issue の SCORE 台帳、レポート、カテゴリ別 SCORE、証拠段階、phase boundary criteria だけにする。本体の期待は渡さない。`target-theorem` では、target theorem、completion criteria、premise discharge policy、material premise ledger、anti-weakening rule、target proof artifacts、support map、failure policy、tracking Issue の proof state、今回の proof_obligation_delta、G3/G3.5/G4/G5 の target 判定を渡し、target completion 判定を行わせる。
...
`target-theorem` では、G6 は先に target completion を判定する。GOAL カードの target theorem completion criteria を満たし、target theorem artifact が G3/G3.5/G4/G5 を通り、G4 が `target_progress: target-proved` 相当を返し、G5 が target completion を妨げる finding を残さず、独立審判が theorem statement の弱体化や claim boundary 越えを認めなければ、`target-theorem-proved` として止める。G6 は theorem statement の `#check` 相当を読み、自然言語 target と Lean statement の対応、量化順序、宇宙 / index / site / cover / coefficient の範囲、全ての implication / iff direction、non-vacuity、反例可能性、依存補題の proof DAG、定義展開後に主張が弱化または自明化していないかを監査する。G6 は結論を支える実質的前提、theorem / typeclass / class / structure / hypothesis / certificate 引数、opaque membership、instance 経由で入る inherited field を列挙し、GOAL の material premise ledger と照合する。completion criteria、premise discharge policy、または material premise ledger が discharge を要求する場合、各前提が Lean theorem、finite witness、または concrete certificate で discharge されていることを確認する。未 discharge の実質的前提を引数として残した conditional theorem package、ledger にない新規 material premise を含む theorem package、結論相当 premise を theorem argument / typeclass / class / structure / certificate field / opaque membership に移した theorem package、proof DAG に循環がある theorem package、片方向しか示していない theorem package、定義で結論を押し込んだ theorem package、反例探索または非自明性監査が未実施の theorem package、または `#check` / axiom audit / premise list / discharge artifact / proof dependency graph のどれかを G6 が確認できない theorem package は `target-theorem-proved` ではなく `target-proof-checkpoint` とする。G6 は fail-closed であり、`cannot-determine`、unchecked item、未同期 report / candidate card / tracking Issue、未解決 G4/G5 finding、査読者として説明できない proof step が残る場合も `target-proof-checkpoint` に落とす。証明が未完なら、SCORE threshold 到達だけでは phase boundary にしない。ただし人間に整理判断を返す価値がある場合は `phase boundary` ではなく `target-proof-checkpoint` として、完了 support node、未完 support node、blocker、次サイクル候補、material premise ledger の未放電行、査読レベル proof audit の未解決項目を tracking Issue にまとめて止めてよい。target theorem が反例により成立しないと判明した場合は `target-refuted` として止め、target 改訂または GOAL 改訂を人間に返す。
