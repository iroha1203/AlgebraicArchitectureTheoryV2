---
name: research-loop
description: 探索型の研究 GOAL に対して、定理候補・反例・構成・不変量を探索し、Lean 検証、ライバル比較、独立審判で研究貢献 SCORE を確定しながら反復する。Use when the user says "$research-loop <goal-id>", "探索研究ループ", "研究ループを回して", or wants Codex to autonomously search, score, prove, compare against rivals, report, and PR research contributions toward a non-target-theorem standing research goal.
---
...
`research mode: target-theorem` の GOAL はこの skill では実行しない。GOAL カードで大定理証明モードを検出したら、作業を開始せず `$target-theorem-loop <goal-id>` を使うよう返す。
...
ひとつの picked の成果をひとつの PR にまとめる。PR には候補カード、Lean ファイルまたは証拠、レポート、tracking Issue の SCORE 更新を含める。**Lean 検証済み(G3 で `lean-verified`)の theorem を受理する PR では、`docs/aat/research_evidence_index.md` への1行登録(theorem 名、ファイル、本文ラベル、conjuncts 要旨、受理点、移植状況 `unported`)を出力義務とする**(Research 下限原則の検索基盤。scaffolding・探索残骸は `not-for-porting` として登録するか、スパイン宣言でないものは登録対象外としてよい)。tracking Issue は研究状態の正本であり、PR 本文では原則 `Refs #<tracking-issue>` を使う。

PR 前に `git diff --check`、`git diff --cached --check`、未追跡ファイル、不可視 Unicode、ローカル絶対パス、保護対象 docs の編集有無を確認する。PR レビューは `$review-pr <PR番号>` を独立サブエージェントで実行する。マージは独立レビューが mergeable で、CI または必要なローカル検証が通った場合だけ行う。
...
候補生成、G2 審判 A/B/C/D、G3 公理検査、G3 Lean 形式化品質監査、G4 SCORE 監査、G5 PR レビュー、G6 フェーズ区切り判定は、独立したサブエージェントに行わせる。サブエージェントを使えない環境では止まり、人間のレビューに委ねる。標準プロンプトは [references/subagent-prompts.md](references/subagent-prompts.md) を使う。
