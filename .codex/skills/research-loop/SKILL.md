...
`research/GOALS.md` は research-loop の不変入力として扱う。この skill は GOAL 本文、`status`、`score threshold`、`portfolio constraint`、`reward rubric`、`spine`、`frontier` を直接編集しない。欠陥を見つけた場合は `goal defect` として止まり、tracking Issue コメントまたは別 Issue に改訂案を書く。
