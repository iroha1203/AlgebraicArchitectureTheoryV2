...
- 指定された一次仕様に完全Lean signatureがある場合、signatureのpremise追加・
  結論弱化・対象縮小を`docs/aat/lean_quality_standard.md` §5で突合する。
  Lean実装を要求するPRD・picked候補で、target claim、必要な結論、明示された
  仮定・受け入れ条件を判定できる一次仕様が参照可能でないことをfindingとする。
  完全Lean signatureの不在自体はfindingとしない。
