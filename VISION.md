...
このプロジェクトの標語は、次の一文である。

```text
Making software evolution computable.
```

このプロジェクトは、ソフトウェアアーキテクチャとソフトウェア進化について、責任をもって語るための数学、形式化、道具、公開面を作る。
...
どの claim は証明であり、どの claim は観測であり、どの claim は予測であり、どの claim はまだ語られていないのか。

AAT / SFT は、この問いに答えるための研究である。

**Goal:** ソフトウェア進化を、経験と直感だけで扱う対象から、定義・観測・計算・証明・統治の対象へ移す。

**What changes:** アーキテクチャレビュー、設計判断、CI、AI proposal、運用 feedback が、同じ語彙の上で接続される。変更は単なる diff ではなく、未来の可能性を変える構造として読まれる。
...
そのために、語彙、使用条件、推論規則、観測境界、証明境界を明らかにする。

**Goal:** ソフトウェアアーキテクチャの言葉に、使用条件と責任境界を与える。

**What changes:** 「なんとなく良い設計」「危ない気がする変更」という会話が、どの語彙、どの law、どの evidence、どの claim boundary の中で語られているかを共有できる会話へ変わる。

---

## AAT: architecture as algebraic geometry

AAT, Algebraic Architecture Theory は、ソフトウェアアーキテクチャを代数幾何の言葉で語るための理論である。
...
VISION では詳細な定義を繰り返さない。
重要なのは、AAT がアーキテクチャを「図」から「幾何」へ移すことである。

依存グラフだけを見るのではなく、contract、effect、state、authority、semantic meaning、runtime interaction を含む構造を読む。
局所的に正しいものが、大域的にどのように貼り合い、どこで貼り合わないのかを読む。
law がどの locus を切り出し、obstruction がどの失敗を witness し、repair がどの geometry を変形するのかを読む。

AAT は、アーキテクチャを代数幾何的対象として語るための文法である。

**Goal:** ソフトウェアアーキテクチャを、代数幾何の言葉で語れるようにする。

**What changes:** 設計原則は slogan ではなく、law、locus、obstruction、cover、cohomology、repair geometry として扱われる。アーキテクチャレビューは、taste の衝突から、どの幾何を見ているのかを共有する診断へ変わる。
...
SFT は、未来の一点予測ではなく、選ばれた model、horizon、policy、observation boundary、governance の中で、どの future architecture が reachable になり、どの trajectory が開かれ、どの shortcut が減衰され、どの feedback が次の field を変えるのかを扱う。
...
**What changes:** PRD、Issue、PR、review、CI、運用 feedback が、ばらばらの artifact ではなく、future architecture を形づくる field action として読めるようになる。開発プロセスは、単なる変更管理から、trajectory を設計し、狭め、開き、統治する営みへ変わる。

---

## Proof: grammar made checkable

Lean 形式化は、このプロジェクトにおける証明の層である。

証明は、語り方の文法を検査可能にする。

どの定義から、どの仮定のもとで、どの命題が従うのか。
どの claim が theorem なのか。
どの claim が construction なのか。
どの claim が analytic reading なのか。
どの claim が empirical hypothesis なのか。

Lean は、この境界を明らかにする。

AAT / SFT において、証明とは、選ばれた universe、選ばれた law、選ばれた witness、選ばれた coverage、選ばれた exactness の中で、何が本当に従うのかを示すことである。

**Goal:** アーキテクチャと進化についての主張を、検査可能な文法へ落とす。

**What changes:** もっともらしい説明と証明された主張が分離される。研究上の claim、tooling claim、empirical hypothesis、non-conclusion が同じ文書空間の中で混線せずに扱われる。
...
tool は、選ばれた evidence language の中で、語れることを語れる形にする。
...
コード、PR、policy、review、CI、運用記録、AI proposal を、AAT / SFT の語彙の中で扱える evidence に変換する。
...
**What changes:** review や CI は、単に test が通るかを見る場から、変更がどの invariant、obstruction、signature、future trajectory に作用しているかを読む場へ変わる。tool は判定器ではなく、開発者が責任ある判断をするための language surface になる。
...
AAT / SFT は、内部文書だけで閉じる理論ではない。
アーキテクチャを考える人、形式手法に関心を持つ人、開発プロセスを設計する人、review や CI を作る人、ソフトウェア進化を考える人が読める形で開かれる必要がある。
...
**What changes:** アーキテクチャ研究、形式手法、開発者 tooling、AI-assisted development の間に共通の言語が生まれる。研究は内部台帳から外へ出て、読まれ、批判され、応用され、拡張される対象になる。
...
AI は、コードを書く速度を上げる。
変更案を増やす。
局所的な pattern を素早く再利用する。
PR、修正案、移行案、回避策、設計案を高速に生成する。

その時代に必要になるのは、生成された変更をどの文法で受け止めるかである。

AI が proposal を出す。
AAT は、その proposal がどの architecture geometry に作用するのかを読む。
SFT は、その proposal がどの future trajectory を開くのかを読む。
Lean は、その proposal について語れる theorem boundary を固定する。
tooling は、その proposal を evidence language の中で診断する。
governance は、その proposal を受け入れ、拒み、修正し、減衰し、field に戻す。

AI 時代において、このプロジェクトが目指すのは、AI を中心に置くことではない。
AI によって増幅されるソフトウェア進化を、計算可能で、診断可能で、責任ある対象として扱うことである。
