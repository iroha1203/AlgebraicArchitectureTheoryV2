...
The G-122 branch presently records only the genuinely finite primitive source
indices of the fixed finite-axis-fold example.  It does not yet give the
general G-122 input declaration, an interpretation, `D_Theta`, or the final
finite presentation `Sigma`.
-/
...
inductive PrimitiveContext :
  ...
  | finiteAxisFold (cell : FiniteAxisFoldCell) :
      PrimitiveContext .finiteAxisFold (ULift.up ()) (.finiteContext cell)
  | lensState {input : LensFamilyInput.{u}}
      {X : LensRealization input.View input.reference} :
