...
An authored reading may itself define `Op A B` to be `ConfigurationHom` (the
required finite-axis-fold input does after transport); its authored evaluator
is the canonical transported/cast map descending from the finite model's
identity reading.  The provenance claim is only that syntax adds no completed
map beyond the original `Op`.  Retaining that primitive cannot by itself
discharge the later span/fullness obligation for all admissible morphisms.
The primitive-action construction is intentionally limited to operations in
one support cell.  The free identity/composite path closure constructed below
stays in that same scope; quotienting by source equations and comparison with
all admissible morphisms remain separate obligations.  The chosen target
occurrence is unique only at the evaluated Atom-value level: duplicate target
occurrences can make its raw
...
/-- Endpoint-typed paths freely generated by identities, original authored
operations, and composition between finite displays over one fixed G-122
cell.  An operation constructor stores exactly the source `Op` and adds no
separate evaluated map.  If `Op` itself is a `ConfigurationHom`, that
source-specified map is retained as primitive data rather than reconstructed
by this path layer. -/
inductive G122OperationPath
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input} :
    (source target : G122FiniteObjectFormationDisplay input X) →
      Fin source.objectCard → Fin target.objectCard → Type (max u v)
  | nil (display : G122FiniteObjectFormationDisplay input X)
      (objectIndex : Fin display.objectCard) :
      G122OperationPath display display objectIndex objectIndex
  | operation {source target : G122FiniteObjectFormationDisplay input X}
      (sourceObjectIndex : Fin source.objectCard)
      (targetObjectIndex : Fin target.objectCard)
      (op :
        let _ := input.atomDecidableEq
        (input.authored.context.supportPackage X.cell.as).reading.operationReading.Op
          (source.objectValue sourceObjectIndex)
          (target.objectValue targetObjectIndex)) :
      G122OperationPath source target sourceObjectIndex targetObjectIndex
  | seq {source middle target : G122FiniteObjectFormationDisplay input X}
      {sourceObjectIndex : Fin source.objectCard}
      {middleObjectIndex : Fin middle.objectCard}
      {targetObjectIndex : Fin target.objectCard}
      (first : G122OperationPath source middle sourceObjectIndex middleObjectIndex)
      (second : G122OperationPath middle target middleObjectIndex targetObjectIndex) :
      G122OperationPath source target sourceObjectIndex targetObjectIndex

namespace G122OperationPath

/-- Evaluate an authored-operation path to its configuration homomorphism.
Identity and composition are evaluator clauses rather than path certificates. -/
noncomputable def configurationMap
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    {sourceObjectIndex : Fin source.objectCard}
    {targetObjectIndex : Fin target.objectCard}
    (path : G122OperationPath source target sourceObjectIndex targetObjectIndex) :
    ConfigurationHom
      (source.objectValue sourceObjectIndex).configuration
      (target.objectValue targetObjectIndex).configuration := by
  induction path with
  | nil display objectIndex =>
      exact ConfigurationHom.id (display.objectValue objectIndex).configuration
  | operation sourceObjectIndex targetObjectIndex op =>
      letI := input.atomDecidableEq
      exact
        (input.authored.context.supportPackage X.cell.as).reading.operationReading.configurationMap
          op
  | seq first second firstMap secondMap =>
      exact ConfigurationHom.comp secondMap firstMap

/-- Evaluating the identity path gives the identity configuration
homomorphism. -/
@[simp] theorem configurationMap_nil
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    (display : G122FiniteObjectFormationDisplay input X)
    (objectIndex : Fin display.objectCard) :
    (G122OperationPath.nil display objectIndex).configurationMap =
      ConfigurationHom.id (display.objectValue objectIndex).configuration :=
  rfl

/-- Evaluation sends syntactic path sequencing to configuration-homomorphism
composition in the same order. -/
@[simp] theorem configurationMap_seq
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source middle target : G122FiniteObjectFormationDisplay input X}
    {sourceObjectIndex : Fin source.objectCard}
    {middleObjectIndex : Fin middle.objectCard}
    ...
    (first : G122OperationPath source middle sourceObjectIndex middleObjectIndex)
    (second : G122OperationPath middle target middleObjectIndex targetObjectIndex) :
    (G122OperationPath.seq first second).configurationMap =
      ConfigurationHom.comp second.configurationMap first.configurationMap :=
  rfl

end G122OperationPath

/-- A simultaneous authored-operation path for every source object term.
Only a finite object-index map and source-generated typed paths are stored. -/
structure G122OperationPathActionSyntax
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    (source target : G122FiniteObjectFormationDisplay input X) where
  /-- Target object term selected for each source object term. -/
  objectIndexMap : Fin source.objectCard → Fin target.objectCard
  /-- Authored-operation path at the exact evaluated endpoints. -/
  path : ∀ objectIndex,
    G122OperationPath source target objectIndex (objectIndexMap objectIndex)

namespace G122OperationPathActionSyntax

/-- Identity simultaneous path action. -/
def id
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    (display : G122FiniteObjectFormationDisplay input X) :
    G122OperationPathActionSyntax display display where
  objectIndexMap := _root_.id
  path objectIndex := G122OperationPath.nil display objectIndex

/-- Compose simultaneous path actions by sequencing their endpoint-typed
paths through the selected middle object term. -/
def comp
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source middle target : G122FiniteObjectFormationDisplay input X}
    (first : G122OperationPathActionSyntax source middle)
    (second : G122OperationPathActionSyntax middle target) :
    G122OperationPathActionSyntax source target where
  objectIndexMap := second.objectIndexMap ∘ first.objectIndexMap
  path objectIndex := G122OperationPath.seq (first.path objectIndex)
    (second.path (first.objectIndexMap objectIndex))

/-- Evaluate one component of a simultaneous path action. -/
noncomputable def configurationMap
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122OperationPathActionSyntax source target)
    (objectIndex : Fin source.objectCard) :
    ConfigurationHom (source.objectValue objectIndex).configuration
      (target.objectValue (transform.objectIndexMap objectIndex)).configuration :=
  (transform.path objectIndex).configurationMap

/-- Evaluate a simultaneous path action on the choice-independent
generated-family Atom subtype. -/
noncomputable def generatedFamilyMap
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122OperationPathActionSyntax source target)
    (objectIndex : Fin source.objectCard) :
...
/-- The identity simultaneous path action evaluates to the identity on every
generated family. -/
theorem generatedFamilyMap_id
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    (display : G122FiniteObjectFormationDisplay input X)
    (objectIndex : Fin display.objectCard) :
    (id display).generatedFamilyMap objectIndex = _root_.id :=
  generatedFamilyMapOfConfigurationHom_id objectIndex

/-- Composition of simultaneous authored-operation paths evaluates to
composition of their choice-independent generated-family maps. -/
theorem generatedFamilyMap_comp
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source middle target : G122FiniteObjectFormationDisplay input X}
    (first : G122OperationPathActionSyntax source middle)
    (second : G122OperationPathActionSyntax middle target)
    (objectIndex : Fin source.objectCard) :
    (comp first second).generatedFamilyMap objectIndex =
      second.generatedFamilyMap (first.objectIndexMap objectIndex) ∘
        first.generatedFamilyMap objectIndex :=
  generatedFamilyMapOfConfigurationHom_comp
    (first.configurationMap objectIndex)
    (second.configurationMap (first.objectIndexMap objectIndex))

/-- Left identity holds after evaluating simultaneous paths on each generated
family, even though raw syntax trees are not identified. -/
theorem generatedFamilyMap_id_comp
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122OperationPathActionSyntax source target)
    (objectIndex : Fin source.objectCard) :
    (comp (id source) transform).generatedFamilyMap objectIndex =
      transform.generatedFamilyMap objectIndex := by
  rw [generatedFamilyMap_comp, generatedFamilyMap_id]
  rfl

/-- Right identity holds after evaluating simultaneous paths on each generated
family. -/
theorem generatedFamilyMap_comp_id
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122OperationPathActionSyntax source target)
    (objectIndex : Fin source.objectCard) :
    (comp transform (id target)).generatedFamilyMap objectIndex =
      transform.generatedFamilyMap objectIndex := by
  rw [generatedFamilyMap_comp, generatedFamilyMap_id]
  rfl

/-- Associativity holds for the evaluated generated-family maps of three
simultaneous authored-operation path actions. -/
theorem generatedFamilyMap_comp_assoc
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {firstDisplay secondDisplay thirdDisplay fourthDisplay :
      G122FiniteObjectFormationDisplay input X}
    (first : G122OperationPathActionSyntax firstDisplay secondDisplay)
    (second : G122OperationPathActionSyntax secondDisplay thirdDisplay)
    (third : G122OperationPathActionSyntax thirdDisplay fourthDisplay)
    (objectIndex : Fin firstDisplay.objectCard) :
    (comp (comp first second) third).generatedFamilyMap objectIndex =
      (comp first (comp second third)).generatedFamilyMap objectIndex := by
  rw [generatedFamilyMap_comp, generatedFamilyMap_comp,
    generatedFamilyMap_comp, generatedFamilyMap_comp]
  rfl

/-- Embed the Cycle 39 primitive-operation table as length-one authored paths. -/
def ofPrimitive
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122PrimitiveOperationActionSyntax source target) :
    G122OperationPathActionSyntax source target where
  objectIndexMap := transform.objectIndexMap
  path objectIndex := G122OperationPath.operation objectIndex
    (transform.objectIndexMap objectIndex) (transform.operation objectIndex)

/-- The path evaluator on an embedded primitive operation is the original
Cycle 39 operation-reader evaluation. -/
@[simp] theorem configurationMap_ofPrimitive
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122PrimitiveOperationActionSyntax source target)
    (objectIndex : Fin source.objectCard) :
    (ofPrimitive transform).configurationMap objectIndex =
      transform.configurationMap objectIndex :=
  rfl

/-- The value-level action of an embedded primitive path is exactly the
canonical primitive-operation action constructed in Cycle 40. -/
@[simp] theorem generatedFamilyMap_ofPrimitive
    {input : G122FamilyInput.{u, v}} {X : G122CellInput input}
    {source target : G122FiniteObjectFormationDisplay input X}
    (transform : G122PrimitiveOperationActionSyntax source target)
    (objectIndex : Fin source.objectCard) :
    (ofPrimitive transform).generatedFamilyMap objectIndex =
      transform.generatedFamilyMap objectIndex :=
  rfl

end G122OperationPathActionSyntax
