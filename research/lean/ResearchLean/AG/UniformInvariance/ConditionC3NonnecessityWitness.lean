...
/-- The unique coarse chart selected by target zero.  This public Cycle 15
fixture-local datum is computed from raw selection and anchors the fiber constraint;
it is not a supplied C3 witness. -/
def coarseChartZero : presentation.CoarseChartIn targetZero :=
  ⟨(0 : Fin 3), by decide⟩

/-- The unique fine self-loop selected over target zero.  This public Cycle
15 fixture-local datum is computed from raw selection and names the cycle support. -/
...
/-- Every fine chart selected over target zero is chart zero.  This public
Cycle 15 fixture-normalization API derives from the raw-selection
...
/-- Every fine edge selected over target zero is edge zero.  This public
Cycle 15 fixture-normalization API derives from raw support and supports both cycle
...
/-- The unique selected raw coarse self-loop at target zero.  This public
Cycle 15 fixture-local datum is generated by raw selection and feeds the canonical
...
/-- No coarse face is selected by target zero.  This public Cycle 15
fixture API transports the already proved fine-face absence through the selected-face
...
/-- The displayed target-zero cocycle has unit period.  This public Cycle 15
fixture-normalization lemma evaluates the explicit chain and is used only to prove the
