...
The per-universe summand-carrier fixtures and the two newly authored
nonidentity qualification arrows transcribe the third raw-data checkpoint in
`research/fixtures/G-112-k1-o6-raw-data-v3.md`.  Earlier cycles were rejected
for missing chronology and for preserving the known refutation/family route.
V3 changes the candidate assignment and derives positive-family properties
without a predecessor selective arrow.  Its raw structures contain only
carriers, endpoints, and arrows, while firing and all negative/nondegeneracy
claims are separate theorem outputs.
-/
...
/--
Projection API for the doctrine of the canonical object code.  K1 presentation
proofs use it instead of unfolding the code constructor; `hall` is the selected
term's target extraction premise (or its pullback along the semantic hom).
-/
@[simp]
theorem finiteCofiniteInstanceCodeOf_doctrine {U : AtomCarrier.{u}}
    [DecidableEq U.Atom] (object : ExtractionInstance U)
    [Finite object.doctrine.Source]
    (hall : AllExtractionsFiniteOrCofinite object)
    (atomEquiv : Equiv.Perm U.Atom) :
    (finiteCofiniteInstanceCodeOf object hall atomEquiv).doctrine =
      finiteCofiniteDoctrineCodeOf object.doctrine hall atomEquiv :=
  rfl

/--
Projection API for the selected point of the canonical object code.  This is an
API lemma for the K1 arrow presentation, with finiteness supplied by candidate
membership rather than stored in the semantic condition.
-/
@[simp]
theorem finiteCofiniteInstanceCodeOf_point {U : AtomCarrier.{u}}
    [DecidableEq U.Atom] (object : ExtractionInstance U)
    [Finite object.doctrine.Source]
    (hall : AllExtractionsFiniteOrCofinite object)
    (atomEquiv : Equiv.Perm U.Atom) :
    (finiteCofiniteInstanceCodeOf object hall atomEquiv).point =
...
noncomputable def endpointFiniteTargetCofinitePresentation
  ...
  extraction_eq := by
    ...
    let sourceHall :=
      allExtractionsFiniteOrCofinite_source_of_hom input.hom htarget
    simp only [finiteCofiniteInstanceCodeOf_doctrine,
  ...
  source_eq := by
    apply (finiteSourceEquiv input.target.doctrine.Source).injective
    simp [finiteCofiniteInstanceCodeOf_point, input.hom.source_eq]
...
/--
G-112 K1 sufficiency theorem.  From the selected candidate's endpoint
finiteness and target finite/cofinite extraction premises, it constructs both
finite-code anchors and the typed arrow square; it is the main coverage theorem,
not a wrapper around a supplied coverage certificate.
-/
...
/-- The summand Atom carrier fixed by the third O6 raw-data checkpoint. -/
def ExactBottomSumCarrier : AtomCarrier.{u} where
  AtomKind := PUnit
  Axis := PUnit
  Subject := PUnit
  Predicate := PUnit
  Payload := PUnit
  Atom := ULift.{u} (Nat ⊕ (Nat ⊕ (Nat → Bool)))
...
noncomputable local instance exactBottomSumCarrierDecidableEq :
    DecidableEq ExactBottomSumCarrier.{u}.Atom :=
  Classical.decEq _

/-- The authored bad predicate selects the outer first summand. -/
def exactBottomFirstSummand (atom : ExactBottomSumCarrier.{u}.Atom) : Prop :=
  match atom.down with
  | Sum.inl _ => True
  | Sum.inr _ => False

/-- The first summand contains an injective copy of `Nat`. -/
theorem exactBottomFirstSummand_not_finite :
    ¬ Set.Finite
      {atom : ExactBottomSumCarrier.{u}.Atom | exactBottomFirstSummand atom} := by
  intro hfinite
  letI : Finite
      {atom : ExactBottomSumCarrier.{u}.Atom // exactBottomFirstSummand atom} :=
    hfinite.to_subtype
  have hnat : Finite Nat := by
    apply Finite.of_injective
      (fun index : Nat =>
        (⟨ULift.up (Sum.inl index), by simp [exactBottomFirstSummand]⟩ :
          {atom : ExactBottomSumCarrier.{u}.Atom // exactBottomFirstSummand atom}))
    intro first second h
    simpa using congrArg (fun atom => atom.1.down) h
  letI : Finite Nat := hnat
  letI : Fintype Nat := Fintype.ofFinite Nat
  exact Fintype.false (α := Nat) inferInstance

/-- The nested second summand gives an injective copy of `Nat` in the complement. -/
theorem exactBottomFirstSummand_compl_not_finite :
    ¬ Set.Finite
      {atom : ExactBottomSumCarrier.{u}.Atom | exactBottomFirstSummand atom}ᶜ := by
  intro hfinite
  letI : Finite
      {atom : ExactBottomSumCarrier.{u}.Atom // ¬ exactBottomFirstSummand atom} :=
    hfinite.to_subtype
  have hnat : Finite Nat := by
    apply Finite.of_injective
      (fun index : Nat =>
        (⟨ULift.up (Sum.inr (Sum.inl index)), by
          simp [exactBottomFirstSummand]⟩ :
          {atom : ExactBottomSumCarrier.{u}.Atom // ¬ exactBottomFirstSummand atom}))
    intro first second h
    simpa using congrArg (fun atom => atom.1.down) h
  letI : Finite Nat := hnat
...
theorem exactBottomULiftNatPlane_not_finite :
  ...
  intro hfinite
  letI : Finite (ULift.{u} (Nat × Nat)) := hfinite
  have hnat : Finite Nat := by
    apply Finite.of_injective (fun index : Nat => ULift.up (index, index + 1))
    intro first second h
    exact congrArg (fun point => point.down.1) h
  exact exactBottomNat_not_finite hnat

/-- One-source doctrine whose unique extraction is the fixed first summand. -/
def exactBottomSummandOneDoctrine : ExtractionDoctrine ExactBottomSumCarrier.{u} where
  Source := PUnit.{u + 1}
  ...
  semanticAllows := fun _ _ atom => exactBottomFirstSummand atom
  resolutionAllows := fun _ _ _ => True
  sourceSemantics := fun _ _ => True
  normalize := id

/-- Selected left-slice one-source object. -/
def exactBottomSummandOneInstance : ExtractionInstance ExactBottomSumCarrier.{u} :=
  ⟨exactBottomSummandOneDoctrine.{u}, PUnit.unit⟩

/-- Identity on the left-slice one-source object. -/
def exactBottomSummandIdentityInput : CartSemanticInput ExactBottomSumCarrier.{u} :=
  ⟨exactBottomSummandOneInstance, exactBottomSummandOneInstance,
    𝟙 exactBottomSummandOneInstance⟩

/-- All-admitting one-source doctrine. -/
def exactBottomAllOneDoctrine : ExtractionDoctrine ExactBottomSumCarrier.{u} where
  Source := PUnit.{u + 1}
  Vocabulary := PUnit.{u + 1}
  SemanticReading := PUnit.{u + 1}
  Resolution := PUnit.{u + 1}
...
/-- All-admitting doctrine on the fixed infinite plane source. -/
def exactBottomAllPlaneDoctrine : ExtractionDoctrine ExactBottomSumCarrier.{u} where
  Source := ULift.{u} (Nat × Nat)
  Vocabulary := PUnit.{u + 1}
  SemanticReading := PUnit.{u + 1}
  Resolution := PUnit.{u + 1}
  vocabulary := PUnit.unit
...
def exactBottomAllOneInstance : ExtractionInstance ExactBottomSumCarrier.{u} :=
  ⟨exactBottomAllOneDoctrine.{u}, PUnit.unit⟩

/-- The selected `(0, 1)` cell of the infinite all-extracting plane endpoint. -/
def exactBottomAllPlaneInstance : ExtractionInstance ExactBottomSumCarrier.{u} :=
  ⟨exactBottomAllPlaneDoctrine.{u}, ULift.up (0, 1)⟩

/-- Exact map from the finite all-admitting source to the infinite one. -/
def exactBottomFiniteToPlaneHom :
    exactBottomAllOneInstance.{u} ⟶ exactBottomAllPlaneInstance.{u} where
  doctrineHom :=
    { sourceMap := fun _ => ULift.up (0, 1)
      atomEquiv := Equiv.refl _
      normalize_eq := by intro source; cases source; rfl
      extraction_iff := by simp [ExtractionDoctrine.extracts,
        exactBottomAllOneInstance, exactBottomAllPlaneInstance,
        exactBottomAllOneDoctrine, exactBottomAllPlaneDoctrine] }
  source_eq := rfl

/-- Exact map from the infinite all-admitting source to the finite one. -/
def exactBottomPlaneToFiniteHom :
    exactBottomAllPlaneInstance.{u} ⟶ exactBottomAllOneInstance.{u} where
  doctrineHom :=
...
def exactBottomFiniteToPlaneInput : CartSemanticInput ExactBottomSumCarrier.{u} :=
  ⟨exactBottomAllOneInstance, exactBottomAllPlaneInstance,
    exactBottomFiniteToPlaneHom⟩

/-- Fixed O6 semantic input from the infinite endpoint to the one-cell endpoint. -/
def exactBottomPlaneToFiniteInput : CartSemanticInput ExactBottomSumCarrier.{u} :=
  ⟨exactBottomAllPlaneInstance, exactBottomAllOneInstance,
    exactBottomPlaneToFiniteHom⟩

/-- Fixed O6 infinite identity and final characterized-branch counterexample. -/
def exactBottomPlaneIdentityInput : CartSemanticInput ExactBottomSumCarrier.{u} :=
  ⟨exactBottomAllPlaneInstance, exactBottomAllPlaneInstance,
    𝟙 exactBottomAllPlaneInstance⟩

/-- Four-source target with one all-admitting cell and three summand cells. -/
def exactBottomMixedFourDoctrine : ExtractionDoctrine ExactBottomSumCarrier.{u} where
  Source := ULift.{u} (Fin 4)
  Vocabulary := PUnit.{u + 1}
  SemanticReading := PUnit.{u + 1}
  Resolution := PUnit.{u + 1}
  vocabulary := PUnit.unit
  ...
  resolution := PUnit.unit
  vocabularyAllows := fun _ _ => True
  semanticAllows := fun _ source atom => source.down = 0 ∨ exactBottomFirstSummand atom
  resolutionAllows := fun _ _ _ => True
  sourceSemantics := fun _ _ => True
  normalize := id

/-- The selected all-extracting cell of the fixed mixed target endpoint. -/
def exactBottomMixedFourInstance : ExtractionInstance ExactBottomSumCarrier.{u} :=
  ⟨exactBottomMixedFourDoctrine.{u}, ULift.up 0⟩

/-- The source map lands only in the target's all-admitting cell. -/
def exactBottomFiniteToMixedFourHom :
    exactBottomAllOneInstance.{u} ⟶ exactBottomMixedFourInstance.{u} where
  doctrineHom :=
    { sourceMap := fun _ => ULift.up 0
      atomEquiv := Equiv.refl _
      normalize_eq := by intro source; cases source; rfl
      extraction_iff := by simp [ExtractionDoctrine.extracts,
        exactBottomAllOneInstance, exactBottomMixedFourInstance,
        exactBottomAllOneDoctrine, exactBottomMixedFourDoctrine] }
  source_eq := rfl

/-- Fixed O6 input whose map avoids the mixed target's bad cell. -/
def exactBottomFiniteToMixedFourInput : CartSemanticInput ExactBottomSumCarrier.{u} :=
  ⟨exactBottomAllOneInstance, exactBottomMixedFourInstance,
    exactBottomFiniteToMixedFourHom⟩
...
theorem exactBottomAllPlane_allExtractions :
    AllExtractionsFiniteOrCofinite exactBottomAllPlaneInstance := by
  intro source
  right
  simp [extractedAtomSet, ExtractionDoctrine.extracts,
    exactBottomAllPlaneInstance, exactBottomAllPlaneDoctrine]

/-- The summand object fails the finite/cofinite extraction condition. -/
theorem exactBottomSummandOne_not_allExtractions :
    ¬ AllExtractionsFiniteOrCofinite exactBottomSummandOneInstance := by
  intro hall
  ...
  · apply exactBottomFirstSummand_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomSummandOneInstance, exactBottomSummandOneDoctrine] using hfinite
  · apply exactBottomFirstSummand_compl_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomSummandOneInstance, exactBottomSummandOneDoctrine] using hcofinite

/-- The unused cell `1` makes the mixed-four target fail the condition. -/
theorem exactBottomMixedFour_not_allExtractions :
    ¬ AllExtractionsFiniteOrCofinite exactBottomMixedFourInstance := by
  intro hall
  rcases hall (ULift.up (1 : Fin 4)) with hfinite | hcofinite
  · apply exactBottomFirstSummand_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomMixedFourInstance, exactBottomMixedFourDoctrine] using hfinite
  · apply exactBottomFirstSummand_compl_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomMixedFourInstance, exactBottomMixedFourDoctrine] using hcofinite
...
theorem exactBottom_not_covered_of_source_infinite
    (input : CartSemanticInput ExactBottomSumCarrier.{u})
...
/--
G-112 K1 selected term: the twelfth F0 candidate, stated on `FiniteModel` only
as closed syntax.  It is the public syntax constant rebased to every carrier by
the evaluator; it stores no coverage witness.
-/
...
/--
G-112 K1 carrier-uniform sufficiency API.  Its sole material hypothesis is
membership in the selected direction; the proof consumes all three conjuncts
to call the constructive anchor-and-square theorem.
-/
...
/-- The finite summand identity is outside the anchored realization locus. -/
theorem exactBottomSummandIdentityInput_not_covered :
    ¬ Nonempty (AnchoredCoverageWitness exactBottomSummandIdentityInput) :=
...
theorem exactBottomFiniteToMixedFourInput_not_covered :
  ...
  exactBottom_not_covered_of_target_bad exactBottomFiniteToMixedFourInput
    exactBottomMixedFour_not_allExtractions

/-- Candidate 0 fires on `summand-id`, whose target extraction cannot be encoded. -/
noncomputable def exactBottomCandidateRefutation0 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨0, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomSummandIdentityInput.{u}
  · change Finite PUnit ∧ Finite PUnit
    exact ⟨inferInstance, inferInstance⟩
  · exact exactBottomSummandIdentityInput_not_covered

/-- Candidate 1 fires on `finite-to-mixed-four`; an unused target cell is bad. -/
noncomputable def exactBottomCandidateRefutation1 :
  ...
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomFiniteToMixedFourInput.{u}
  · change Finite PUnit.{u + 1} ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨inferInstance, exactBottomAllOne_allExtractions⟩
  · exact exactBottomFiniteToMixedFourInput_not_covered

/-- Candidate 2 fires on `plane-to-finite`, whose source Source is infinite. -/
noncomputable def exactBottomCandidateRefutation2 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨2, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomPlaneToFiniteInput.{u}
  · change Finite PUnit.{u + 1} ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨inferInstance, exactBottomAllOne_allExtractions⟩
  · exact exactBottomPlaneToFiniteInput_not_covered

/-- Candidate 3 fires on `plane-id`, whose source Source is infinite. -/
noncomputable def exactBottomCandidateRefutation3 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨3, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomPlaneIdentityInput.{u}
  · change AllExtractionsFiniteOrCofinite exactBottomAllPlaneInstance ∧
      AllExtractionsFiniteOrCofinite exactBottomAllPlaneInstance
    exact ⟨exactBottomAllPlane_allExtractions,
      exactBottomAllPlane_allExtractions⟩
  · exact exactBottomPlaneIdentityInput_not_covered

/-- Candidate 4 fires on `finite-to-plane`, whose target Source is infinite. -/
noncomputable def exactBottomCandidateRefutation4 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨4, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomFiniteToPlaneInput.{u}
  · change Finite PUnit ∧
      AllExtractionsFiniteOrCofinite exactBottomAllPlaneInstance
    exact ⟨inferInstance, exactBottomAllPlane_allExtractions⟩
  · exact exactBottomFiniteToPlaneInput_not_covered

/-- Candidate 5 fires on `finite-to-mixed-four`; an unused target cell is bad. -/
noncomputable def exactBottomCandidateRefutation5 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨5, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomFiniteToMixedFourInput.{u}
  · change Finite (ULift.{u} (Fin 4)) ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨inferInstance, exactBottomAllOne_allExtractions⟩
  · exact exactBottomFiniteToMixedFourInput_not_covered

/-- Candidate 6 fires on the finite `summand-id` bad endpoint. -/
noncomputable def exactBottomCandidateRefutation6 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨6, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomSummandIdentityInput.{u}
  · change Finite PUnit
    infer_instance
  · exact exactBottomSummandIdentityInput_not_covered

/-- Candidate 7 fires on the finite `summand-id` bad endpoint. -/
...
/-- Candidate 8 fires on `finite-to-mixed-four`; the source extractions are good. -/
noncomputable def exactBottomCandidateRefutation8 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨8, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomFiniteToMixedFourInput.{u}
  · change AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact exactBottomAllOne_allExtractions
  · exact exactBottomFiniteToMixedFourInput_not_covered

/-- Candidate 9 fires on `plane-id`, whose source Source is infinite. -/
...
/-- Candidate 10 fires on `finite-to-mixed-four`, whose unused cells are bad. -/
noncomputable def exactBottomCandidateRefutation10 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨10, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomFiniteToMixedFourInput.{u}
  · change (Finite PUnit.{u + 1} ∧ Finite (ULift.{u} (Fin 4))) ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨⟨inferInstance, inferInstance⟩, exactBottomAllOne_allExtractions⟩
  · exact exactBottomFiniteToMixedFourInput_not_covered

/--
G-112 K1 fixed-candidate selection artifact.  It selects index eleven from the
F0 list and consumes typed firing/noncoverage proofs for every earlier index;
the V3 raw inputs supply data, not these refutation propositions.
-/
...
/--
No-unfold API connecting the dependent selection payload to the public K1
term.  The branch constructor uses this equality instead of expanding the
selection record or its pre-registered candidate lookup.
-/
@[simp]
theorem endpointFiniteTargetCofiniteSelection_template :
    endpointFiniteTargetCofiniteSelection.{u}.template =
      rebaseExactBottomCondition endpointFiniteTargetCofiniteTerm :=
  rfl

/-! ## Qualification and the carrier-global branch -/

/-- V3 member-zero source doctrine: two cells and an authored `Fin 5` vocabulary. -/
def exactBottomV3ZeroSourceDoctrine : ExtractionDoctrine FiniteModel.carrier where
  Source := Fin 2
  Vocabulary := Fin 5
...
/-- V3 member-zero target doctrine: one cell and the same authored vocabulary. -/
def exactBottomV3ZeroTargetDoctrine : ExtractionDoctrine FiniteModel.carrier where
  Source := PUnit
  Vocabulary := Fin 5
  SemanticReading := PUnit
  Resolution := PUnit
...
/-- Selected source endpoint of V3 family member zero. -/
def exactBottomV3ZeroSourceInstance : ExtractionInstance FiniteModel.carrier :=
  ⟨exactBottomV3ZeroSourceDoctrine,
    show exactBottomV3ZeroSourceDoctrine.Source from (0 : Fin 2)⟩

/-- Selected target endpoint of V3 family member zero. -/
def exactBottomV3ZeroTargetInstance : ExtractionInstance FiniteModel.carrier :=
  ⟨exactBottomV3ZeroTargetDoctrine, PUnit.unit⟩

/-- Newly authored constant exact arrow for V3 family member zero. -/
def exactBottomV3ZeroHom :
    exactBottomV3ZeroSourceInstance ⟶ exactBottomV3ZeroTargetInstance where
  doctrineHom :=
    { sourceMap := fun _ => PUnit.unit
      atomEquiv := Equiv.refl _
      normalize_eq := by intro source; rfl
      extraction_iff := by simp [ExtractionDoctrine.extracts,
        exactBottomV3ZeroSourceInstance, exactBottomV3ZeroTargetInstance,
        exactBottomV3ZeroSourceDoctrine, exactBottomV3ZeroTargetDoctrine] }
  source_eq := rfl

/-- V3 member-one source doctrine: three cells and an authored `Fin 7` vocabulary. -/
def exactBottomV3OneSourceDoctrine : ExtractionDoctrine FiniteModel.carrier where
  Source := Fin 3
  Vocabulary := Fin 7
  SemanticReading := PUnit
  Resolution := PUnit
...
/-- V3 member-one target doctrine: one cell and the same authored vocabulary. -/
def exactBottomV3OneTargetDoctrine : ExtractionDoctrine FiniteModel.carrier where
  Source := PUnit
  Vocabulary := Fin 7
  SemanticReading := PUnit
  Resolution := PUnit
...
/-- Selected source endpoint of V3 family member one. -/
def exactBottomV3OneSourceInstance : ExtractionInstance FiniteModel.carrier :=
  ⟨exactBottomV3OneSourceDoctrine,
    show exactBottomV3OneSourceDoctrine.Source from (0 : Fin 3)⟩

/-- Selected target endpoint of V3 family member one. -/
def exactBottomV3OneTargetInstance : ExtractionInstance FiniteModel.carrier :=
  ⟨exactBottomV3OneTargetDoctrine, PUnit.unit⟩

/-- Newly authored constant exact arrow for V3 family member one. -/
def exactBottomV3OneHom :
    exactBottomV3OneSourceInstance ⟶ exactBottomV3OneTargetInstance where
  doctrineHom :=
    { sourceMap := fun _ => PUnit.unit
      atomEquiv := Equiv.refl _
      normalize_eq := by intro source; rfl
      extraction_iff := by simp [ExtractionDoctrine.extracts,
        exactBottomV3OneSourceInstance, exactBottomV3OneTargetInstance,
        exactBottomV3OneSourceDoctrine, exactBottomV3OneTargetDoctrine] }
  source_eq := rfl

/-- The V3 raw family consists of the two new nonidentity arrows. -/
def exactBottomV3PositiveFamily : ExactBottomPositiveFamilyRaw where
  Parameter := Fin 2
  distinguished := 0
  source := Fin.cases exactBottomV3ZeroSourceInstance
    (fun _ : Fin 1 => exactBottomV3OneSourceInstance)
  target := Fin.cases exactBottomV3ZeroTargetInstance
    (fun _ : Fin 1 => exactBottomV3OneTargetInstance)
  hom := Fin.cases exactBottomV3ZeroHom
    (fun _ : Fin 1 => exactBottomV3OneHom)

/-- V3 member zero lies outside the literal finite-code realization image. -/
theorem exactBottomV3Zero_not_strict :
    ¬ StrictCartRealizationImage (cartSemanticInputOfHom exactBottomV3ZeroHom) := by
  ...
  change PUnit = Fin 5 at hvocabulary
  have hcard : Fintype.card PUnit = Fintype.card (Fin 5) :=
    Fintype.card_congr (Equiv.cast hvocabulary)
  norm_num at hcard

/-- V3 members zero and one cannot be isomorphic as semantic arrows. -/
theorem exactBottomV3PositiveFamily_not_isomorphic :
    ¬ Nonempty (CartSemanticInputIso
      (cartSemanticInputOfHom exactBottomV3ZeroHom)
      (cartSemanticInputOfHom exactBottomV3OneHom)) := by
  rintro ⟨iso⟩
  let sourceEquiv := extractionInstanceIsoSourceEquiv iso.sourceIso
  change Fin 2 ≃ Fin 3 at sourceEquiv
  have hcard := Fintype.card_congr sourceEquiv
  norm_num at hcard

/-- V3 member zero is noninvertible because its source map identifies two cells. -/
theorem exactBottomV3ZeroHom_not_isIso : ¬ IsIso exactBottomV3ZeroHom := by
  intro hiso
  letI : IsIso exactBottomV3ZeroHom := hiso
  have hinjective := extInstHom_sourceMap_injective_of_isIso exactBottomV3ZeroHom
  have heq : (0 : Fin 2) = 1 := hinjective rfl
  norm_num at heq

/--
G-112 K1 qualification certificate for the selected F0 candidate.  Its fields
derive isomorphism invariance and anchor-relative identity/composition/pullback
closure from semantic inputs, strict-image inclusion from presentations, and
nondegeneracy from the V3 raw family.  No qualification proposition is a field
of that raw family; endpoint finiteness comes from candidate membership or
anchors, and extraction finiteness comes from the target condition.
-/
noncomputable def endpointFiniteTargetCofiniteQualification :
  ...
  positiveFamily := exactBottomV3PositiveFamily
  positive_fires := by
    intro parameter
    change Fin 2 at parameter
    fin_cases parameter
    · change (Finite (Fin 2) ∧ Finite PUnit) ∧
        AllExtractionsFiniteOrCofinite exactBottomV3ZeroTargetInstance
      refine ⟨⟨inferInstance, inferInstance⟩, ?_⟩
      intro source
      right
      simp [extractedAtomSet, ExtractionDoctrine.extracts,
        exactBottomV3ZeroTargetInstance, exactBottomV3ZeroTargetDoctrine]
    · change (Finite (Fin 3) ∧ Finite PUnit) ∧
        AllExtractionsFiniteOrCofinite exactBottomV3OneTargetInstance
      refine ⟨⟨inferInstance, inferInstance⟩, ?_⟩
      intro source
      right
      simp [extractedAtomSet, ExtractionDoctrine.extracts,
        exactBottomV3OneTargetInstance, exactBottomV3OneTargetDoctrine]
  positive_strictly_outside := by
    refine ⟨(show exactBottomV3PositiveFamily.Parameter from (0 : Fin 2)), ?_⟩
    simpa [ExactBottomPositiveFamilyRaw.input, exactBottomV3PositiveFamily] using
      exactBottomV3Zero_not_strict
  positive_nonisomorphic_pair := by
    refine ⟨(show exactBottomV3PositiveFamily.Parameter from (0 : Fin 2)),
      (show exactBottomV3PositiveFamily.Parameter from (1 : Fin 2)), ?_, ?_⟩
    · change (0 : Fin 2) ≠ 1
      decide
    · simpa [ExactBottomPositiveFamilyRaw.input, exactBottomV3PositiveFamily] using
        exactBottomV3PositiveFamily_not_isomorphic
  positive_noninvertible := by
    refine ⟨(show exactBottomV3PositiveFamily.Parameter from (0 : Fin 2)), ?_⟩
    simpa [exactBottomV3PositiveFamily] using exactBottomV3ZeroHom_not_isIso

/--
G-112 K1 carrier-global characterized branch.  It packages the independently
constructed selection, qualification and sufficiency proof with the V3
per-universe uncovered plane identity; no branch conclusion is assumed.
-/
noncomputable def characterizedExactBottomCoverage :
  ...
  qualification := by
    simpa only [endpointFiniteTargetCofiniteSelection_template] using
  ...
  sufficient := by
    ...
    simpa only [endpointFiniteTargetCofiniteSelection_template] using membership
  counterexampleCarrier := ExactBottomSumCarrier.{u}
  counterexampleDecidableEq := exactBottomSumCarrierDecidableEq
  counterexample := exactBottomPlaneIdentityInput.{u}
  counterexample_not_covered :=
    exactBottomPlaneIdentityInput_not_covered.{u}
