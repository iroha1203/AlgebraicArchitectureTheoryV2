...
The per-universe product-carrier fixtures and the three-member qualification
family are the Lean transcription of the second raw-data checkpoint in
`research/fixtures/G-112-k1-o6-raw-data-v2.md`.  The first Nat/even attempt was
rejected because its proof preceded its raw-data record.  V2 therefore changes
the carrier, predicate, endpoint sources, arrows, and positive family; its raw
structures contain only carriers, endpoints, and arrows, while firing and all
negative/nondegeneracy claims are separate theorem outputs.
-/
...
/-- The product Atom carrier fixed by the second O6 raw-data checkpoint. -/
def ExactBottomGridCarrier : AtomCarrier.{u} where
  AtomKind := PUnit
  Axis := PUnit
  Subject := PUnit
  Predicate := PUnit
  Payload := PUnit
  Atom := ULift.{u} (Nat × Bool)
...
local instance exactBottomGridCarrierDecidableEq :
    DecidableEq ExactBottomGridCarrier.{u}.Atom := by
  change DecidableEq (ULift.{u} (Nat × Bool))
  infer_instance

/-- The authored left-slice predicate reads the Boolean product coordinate. -/
def exactBottomLeftSlice (atom : ExactBottomGridCarrier.{u}.Atom) : Prop :=
  atom.down.2 = false

/-- The left slice contains an injective copy of `Nat`. -/
theorem exactBottomLeftSlice_not_finite :
    ¬ Set.Finite
      {atom : ExactBottomGridCarrier.{u}.Atom | exactBottomLeftSlice atom} := by
  intro hfinite
  letI : Finite
      {atom : ExactBottomGridCarrier.{u}.Atom // exactBottomLeftSlice atom} :=
    hfinite.to_subtype
  have hnat : Finite Nat := by
    apply Finite.of_injective
      (fun index : Nat =>
        (⟨ULift.up (index, false), rfl⟩ :
          {atom : ExactBottomGridCarrier.{u}.Atom // exactBottomLeftSlice atom}))
    intro first second h
    exact congrArg (fun atom => atom.1.down.1) h
  letI : Finite Nat := hnat
  letI : Fintype Nat := Fintype.ofFinite Nat
  exact Fintype.false (α := Nat) inferInstance

/-- The right slice gives an injective copy of `Nat` in the complement. -/
theorem exactBottomLeftSlice_compl_not_finite :
    ¬ Set.Finite
      {atom : ExactBottomGridCarrier.{u}.Atom | exactBottomLeftSlice atom}ᶜ := by
  intro hfinite
  letI : Finite
      {atom : ExactBottomGridCarrier.{u}.Atom // ¬ exactBottomLeftSlice atom} :=
    hfinite.to_subtype
  have hnat : Finite Nat := by
    apply Finite.of_injective
      (fun index : Nat =>
        (⟨ULift.up (index, true), by simp [exactBottomLeftSlice]⟩ :
          {atom : ExactBottomGridCarrier.{u}.Atom // ¬ exactBottomLeftSlice atom}))
    intro first second h
    exact congrArg (fun atom => atom.1.down.1) h
  letI : Finite Nat := hnat
...
theorem exactBottomULiftNatFinTwo_not_finite :
    ¬ Finite (ULift.{u} (Nat × Fin 2)) := by
  intro hfinite
  letI : Finite (ULift.{u} (Nat × Fin 2)) := hfinite
  have hnat : Finite Nat := by
    apply Finite.of_injective (fun index : Nat => ULift.up (index, (0 : Fin 2)))
    intro first second h
    exact congrArg (fun point => point.down.1) h
  exact exactBottomNat_not_finite hnat

/-- One-source doctrine whose unique extraction is the fixed left slice. -/
def exactBottomLeftOneDoctrine : ExtractionDoctrine ExactBottomGridCarrier.{u} where
  Source := PUnit.{u + 1}
  ...
  semanticAllows := fun _ _ atom => exactBottomLeftSlice atom
  resolutionAllows := fun _ _ _ => True
  sourceSemantics := fun _ _ => True
  normalize := id

/-- Selected left-slice one-source object. -/
def exactBottomLeftOneInstance : ExtractionInstance ExactBottomGridCarrier.{u} :=
  ⟨exactBottomLeftOneDoctrine.{u}, PUnit.unit⟩

/-- Identity on the left-slice one-source object. -/
def exactBottomLeftIdentityInput : CartSemanticInput ExactBottomGridCarrier.{u} :=
  ⟨exactBottomLeftOneInstance, exactBottomLeftOneInstance,
    𝟙 exactBottomLeftOneInstance⟩

/-- All-admitting one-source doctrine. -/
def exactBottomAllOneDoctrine : ExtractionDoctrine ExactBottomGridCarrier.{u} where
  Source := PUnit.{u + 1}
  Vocabulary := PUnit.{u + 1}
  SemanticReading := PUnit.{u + 1}
  Resolution := PUnit.{u + 1}
...
/-- All-admitting doctrine on the fixed infinite grid source. -/
def exactBottomAllGridDoctrine : ExtractionDoctrine ExactBottomGridCarrier.{u} where
  Source := ULift.{u} (Nat × Fin 2)
  Vocabulary := PUnit.{u + 1}
  SemanticReading := PUnit.{u + 1}
  Resolution := PUnit.{u + 1}
  vocabulary := PUnit.unit
...
def exactBottomAllOneInstance : ExtractionInstance ExactBottomGridCarrier.{u} :=
  ⟨exactBottomAllOneDoctrine.{u}, PUnit.unit⟩

/-- The selected `(0, 1)` cell of the infinite all-extracting grid endpoint. -/
def exactBottomAllGridInstance : ExtractionInstance ExactBottomGridCarrier.{u} :=
  ⟨exactBottomAllGridDoctrine.{u}, ULift.up (0, 1)⟩

/-- Exact map from the finite all-admitting source to the infinite one. -/
def exactBottomFiniteToGridHom :
    exactBottomAllOneInstance.{u} ⟶ exactBottomAllGridInstance.{u} where
  doctrineHom :=
    { sourceMap := fun _ => ULift.up (0, 1)
      ...
      extraction_iff := by simp [ExtractionDoctrine.extracts,
        exactBottomAllOneInstance, exactBottomAllGridInstance,
        exactBottomAllOneDoctrine, exactBottomAllGridDoctrine] }
  source_eq := rfl

/-- Exact map from the infinite all-admitting source to the finite one. -/
def exactBottomGridToFiniteHom :
    exactBottomAllGridInstance.{u} ⟶ exactBottomAllOneInstance.{u} where
  doctrineHom :=
...
def exactBottomFiniteToGridInput : CartSemanticInput ExactBottomGridCarrier.{u} :=
  ⟨exactBottomAllOneInstance, exactBottomAllGridInstance,
    exactBottomFiniteToGridHom⟩

/-- Fixed O6 semantic input from the infinite endpoint to the one-cell endpoint. -/
def exactBottomGridToFiniteInput : CartSemanticInput ExactBottomGridCarrier.{u} :=
  ⟨exactBottomAllGridInstance, exactBottomAllOneInstance,
    exactBottomGridToFiniteHom⟩

/-- Fixed O6 infinite identity and final characterized-branch counterexample. -/
def exactBottomGridIdentityInput : CartSemanticInput ExactBottomGridCarrier.{u} :=
  ⟨exactBottomAllGridInstance, exactBottomAllGridInstance,
    𝟙 exactBottomAllGridInstance⟩

/-- Three-source target with one all-admitting cell and two left-slice cells. -/
def exactBottomMixedThreeDoctrine : ExtractionDoctrine ExactBottomGridCarrier.{u} where
  Source := ULift.{u} (Fin 3)
  Vocabulary := PUnit.{u + 1}
  SemanticReading := PUnit.{u + 1}
  Resolution := PUnit.{u + 1}
  vocabulary := PUnit.unit
  ...
  resolution := PUnit.unit
  vocabularyAllows := fun _ _ => True
  semanticAllows := fun _ source atom => source.down = 0 ∨ exactBottomLeftSlice atom
  resolutionAllows := fun _ _ _ => True
  sourceSemantics := fun _ _ => True
  normalize := id

/-- The selected all-extracting cell of the fixed mixed target endpoint. -/
def exactBottomMixedThreeInstance : ExtractionInstance ExactBottomGridCarrier.{u} :=
  ⟨exactBottomMixedThreeDoctrine.{u}, ULift.up 0⟩

/-- The source map lands only in the target's all-admitting cell. -/
def exactBottomFiniteToMixedThreeHom :
    exactBottomAllOneInstance.{u} ⟶ exactBottomMixedThreeInstance.{u} where
  doctrineHom :=
    { sourceMap := fun _ => ULift.up 0
      atomEquiv := Equiv.refl _
      normalize_eq := by intro source; cases source; rfl
      extraction_iff := by simp [ExtractionDoctrine.extracts,
        exactBottomAllOneInstance, exactBottomMixedThreeInstance,
        exactBottomAllOneDoctrine, exactBottomMixedThreeDoctrine] }
  source_eq := rfl

/-- Fixed O6 input whose map avoids the mixed target's bad cell. -/
def exactBottomFiniteToMixedThreeInput : CartSemanticInput ExactBottomGridCarrier.{u} :=
  ⟨exactBottomAllOneInstance, exactBottomMixedThreeInstance,
    exactBottomFiniteToMixedThreeHom⟩
...
theorem exactBottomAllGrid_allExtractions :
    AllExtractionsFiniteOrCofinite exactBottomAllGridInstance := by
  intro source
  right
  simp [extractedAtomSet, ExtractionDoctrine.extracts,
    exactBottomAllGridInstance, exactBottomAllGridDoctrine]

/-- The even-set object fails the finite/cofinite extraction condition. -/
theorem exactBottomLeftOne_not_allExtractions :
    ¬ AllExtractionsFiniteOrCofinite exactBottomLeftOneInstance := by
  intro hall
  rcases hall PUnit.unit with hfinite | hcofinite
  · apply exactBottomLeftSlice_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomLeftOneInstance, exactBottomLeftOneDoctrine] using hfinite
  · apply exactBottomLeftSlice_compl_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomLeftOneInstance, exactBottomLeftOneDoctrine] using hcofinite

/-- The unused `true` cell makes the mixed target fail the condition. -/
theorem exactBottomMixedThree_not_allExtractions :
    ¬ AllExtractionsFiniteOrCofinite exactBottomMixedThreeInstance := by
  intro hall
  rcases hall (ULift.up (1 : Fin 3)) with hfinite | hcofinite
  · apply exactBottomLeftSlice_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomMixedThreeInstance, exactBottomMixedThreeDoctrine] using hfinite
  · apply exactBottomLeftSlice_compl_not_finite
    simpa [extractedAtomSet, ExtractionDoctrine.extracts,
      exactBottomMixedThreeInstance, exactBottomMixedThreeDoctrine] using hcofinite
...
theorem exactBottom_not_covered_of_source_infinite
    (input : CartSemanticInput ExactBottomGridCarrier.{u})
...
/-- The finite left-slice identity is outside the anchored realization locus. -/
theorem exactBottomLeftIdentityInput_not_covered :
    ¬ Nonempty (AnchoredCoverageWitness exactBottomLeftIdentityInput) :=
  exactBottom_not_covered_of_target_bad exactBottomLeftIdentityInput
    exactBottomLeftOne_not_allExtractions

/-- The finite-to-grid fixture is not covered. -/
theorem exactBottomFiniteToGridInput_not_covered :
    ¬ Nonempty (AnchoredCoverageWitness exactBottomFiniteToGridInput) := by
  apply exactBottom_not_covered_of_target_infinite
  exact exactBottomULiftNatFinTwo_not_finite

/-- The grid-to-finite fixture is not covered. -/
theorem exactBottomGridToFiniteInput_not_covered :
    ¬ Nonempty (AnchoredCoverageWitness exactBottomGridToFiniteInput) := by
  apply exactBottom_not_covered_of_source_infinite
  exact exactBottomULiftNatFinTwo_not_finite

/-- The grid identity fixture is not covered. -/
theorem exactBottomGridIdentityInput_not_covered :
    ¬ Nonempty (AnchoredCoverageWitness exactBottomGridIdentityInput) := by
  apply exactBottom_not_covered_of_source_infinite
  exact exactBottomULiftNatFinTwo_not_finite

/-- The finite-to-mixed-three fixture is not covered. -/
theorem exactBottomFiniteToMixedThreeInput_not_covered :
    ¬ Nonempty (AnchoredCoverageWitness exactBottomFiniteToMixedThreeInput) :=
  exactBottom_not_covered_of_target_bad exactBottomFiniteToMixedThreeInput
    exactBottomMixedThree_not_allExtractions

/-- Candidate 0 fires on `left-id`, whose target extraction cannot be encoded. -/
noncomputable def exactBottomCandidateRefutation0 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨0, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomLeftIdentityInput.{u}
  · change Finite PUnit ∧ Finite PUnit
    exact ⟨inferInstance, inferInstance⟩
  · exact exactBottomLeftIdentityInput_not_covered

/-- Candidate 1 fires on `finite-to-grid`, whose target Source is infinite. -/
noncomputable def exactBottomCandidateRefutation1 :
  ...
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomFiniteToGridInput.{u}
  · change Finite PUnit ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨inferInstance, exactBottomAllOne_allExtractions⟩
  · exact exactBottomFiniteToGridInput_not_covered

/-- Candidate 2 fires on `grid-to-finite`, whose source Source is infinite. -/
noncomputable def exactBottomCandidateRefutation2 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨2, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient
    exactBottomGridToFiniteInput.{u}
  · change Finite PUnit ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨inferInstance, exactBottomAllOne_allExtractions⟩
  · exact exactBottomGridToFiniteInput_not_covered

/-- Candidate 3 fires on `grid-id`, whose source Source is infinite. -/
noncomputable def exactBottomCandidateRefutation3 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨3, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomGridIdentityInput.{u}
  · change AllExtractionsFiniteOrCofinite exactBottomAllGridInstance ∧
      AllExtractionsFiniteOrCofinite exactBottomAllGridInstance
    exact ⟨exactBottomAllGrid_allExtractions,
      exactBottomAllGrid_allExtractions⟩
  · exact exactBottomGridIdentityInput_not_covered

/-- Candidate 4 fires on `finite-to-grid`, whose target Source is infinite. -/
noncomputable def exactBottomCandidateRefutation4 :
  ...
  · change Finite PUnit ∧
      AllExtractionsFiniteOrCofinite exactBottomAllGridInstance
    exact ⟨inferInstance, exactBottomAllGrid_allExtractions⟩
  · exact exactBottomFiniteToGridInput_not_covered

/-- Candidate 5 fires on `grid-to-finite`, whose source Source is infinite. -/
...
/-- Candidate 6 fires on `finite-to-grid`, whose target Source is infinite. -/
...
/-- Candidate 7 fires on `grid-to-finite`, whose source Source is infinite. -/
...
/-- Candidate 8 fires on `grid-id`, whose source Source is infinite. -/
...
/-- Candidate 9 fires on `grid-id`, whose source Source is infinite. -/
...
/-- Candidate 10 fires on `finite-to-mixed-three`, whose unused cell is bad. -/
noncomputable def exactBottomCandidateRefutation10 :
    ExactBottomCandidateRefutation.{u}
      (exactBottomCandidateTerm ⟨10, by decide⟩) := by
  apply ExactBottomCandidateRefutation.insufficient exactBottomFiniteToMixedThreeInput.{u}
  · change (Finite PUnit.{u + 1} ∧ Finite (ULift.{u} (Fin 3))) ∧
      AllExtractionsFiniteOrCofinite exactBottomAllOneInstance
    exact ⟨⟨inferInstance, inferInstance⟩, exactBottomAllOne_allExtractions⟩
  · exact exactBottomFiniteToMixedThreeInput_not_covered
...
the V2 raw inputs supply data, not these refutation propositions.
-/
...
/-- A V2 finite object whose selected `Fin 3` vocabulary is raw family data. -/
def exactBottomV2DecoratedDoctrine (selected : Fin 3) :
...
/-- A selected V2 decorated object. -/
def exactBottomV2DecoratedInstance (selected : Fin 3) :
    ExtractionInstance FiniteModel.carrier :=
  ⟨exactBottomV2DecoratedDoctrine selected, PUnit.unit⟩

/-- Every decorated extraction is cofinite. -/
theorem exactBottomV2Decorated_allExtractions (selected : Fin 3) :
    AllExtractionsFiniteOrCofinite (exactBottomV2DecoratedInstance selected) := by
  intro source
  right
  simp [extractedAtomSet, ExtractionDoctrine.extracts,
    exactBottomV2DecoratedInstance, exactBottomV2DecoratedDoctrine]

/-- Literal finite-code realization cannot manufacture a `Fin 3` vocabulary. -/
theorem exactBottomV2Decorated_not_strict (selected : Fin 3) :
    ¬ StrictCartRealizationImage
      (cartSemanticInputOfHom (𝟙 exactBottomV2DecoratedInstance selected)) := by
  ...
  change PUnit = Fin 3 at hvocabulary
  have hcard : Fintype.card PUnit = Fintype.card (Fin 3) :=
    Fintype.card_congr (Equiv.cast hvocabulary)
  norm_num at hcard

/-- The fixed V2 family has one three-to-one arrow and two decorated identities. -/
def exactBottomV2PositiveFamily : ExactBottomPositiveFamilyRaw where
  Parameter := Fin 3
  distinguished := 0
  source := Fin.cases finiteSelectiveThreeInput.semantic.source
    (fun parameter : Fin 2 => exactBottomV2DecoratedInstance parameter.succ)
  target := Fin.cases finiteSelectiveThreeInput.semantic.target
    (fun parameter : Fin 2 => exactBottomV2DecoratedInstance parameter.succ)
  hom := Fin.cases finiteSelectiveThreeInput.semantic.hom
    (fun parameter : Fin 2 => 𝟙 exactBottomV2DecoratedInstance parameter.succ)

/-- V2 family members zero and one cannot be isomorphic as semantic arrows. -/
theorem exactBottomV2PositiveFamily_not_isomorphic :
    ¬ Nonempty (CartSemanticInputIso
      finiteSelectiveThreeInput.semantic
      (cartSemanticInputOfHom (𝟙 exactBottomV2DecoratedInstance 1))) := by
  rintro ⟨iso⟩
  let sourceEquiv := extractionInstanceIsoSourceEquiv iso.sourceIso
  change FiniteSource 3 ≃ PUnit at sourceEquiv
...
nondegeneracy from the V2 raw family.  No qualification proposition is a field
...
noncomputable def endpointFiniteTargetCofiniteQualification :
  ...
  positiveFamily := exactBottomV2PositiveFamily
  positive_fires := by
    intro parameter
    change Fin 3 at parameter
    fin_cases parameter
    · change (Finite (FiniteSource 3) ∧ Finite (FiniteSource 1)) ∧
    ...
    · change (Finite PUnit ∧ Finite PUnit) ∧
        AllExtractionsFiniteOrCofinite (exactBottomV2DecoratedInstance 1)
      exact ⟨⟨inferInstance, inferInstance⟩,
        exactBottomV2Decorated_allExtractions 1⟩
    · change (Finite PUnit ∧ Finite PUnit) ∧
        AllExtractionsFiniteOrCofinite (exactBottomV2DecoratedInstance 2)
      exact ⟨⟨inferInstance, inferInstance⟩,
        exactBottomV2Decorated_allExtractions 2⟩
  positive_strictly_outside := by
    refine ⟨(show exactBottomV2PositiveFamily.Parameter from (1 : Fin 3)), ?_⟩
    simpa [ExactBottomPositiveFamilyRaw.input, exactBottomV2PositiveFamily] using
      exactBottomV2Decorated_not_strict 1
  positive_nonisomorphic_pair := by
    refine ⟨(show exactBottomV2PositiveFamily.Parameter from (0 : Fin 3)),
      (show exactBottomV2PositiveFamily.Parameter from (1 : Fin 3)), ?_, ?_⟩
    · change (0 : Fin 3) ≠ 1
      decide
    · simpa [ExactBottomPositiveFamilyRaw.input, exactBottomV2PositiveFamily] using
        exactBottomV2PositiveFamily_not_isomorphic
  positive_noninvertible := by
    refine ⟨(show exactBottomV2PositiveFamily.Parameter from (0 : Fin 3)), ?_⟩
    simpa [exactBottomV2PositiveFamily] using finiteSelectiveThreeInput_not_isIso

/--
G-112 K1 carrier-global characterized branch.  It packages the independently
constructed selection, qualification and sufficiency proof with the V2
per-universe uncovered grid identity; no branch conclusion is assumed.
-/
noncomputable def characterizedExactBottomCoverage :
  ...
  counterexampleCarrier := ExactBottomGridCarrier.{u}
  counterexampleDecidableEq := exactBottomGridCarrierDecidableEq
  counterexample := exactBottomGridIdentityInput.{u}
  counterexample_not_covered :=
    exactBottomGridIdentityInput_not_covered.{u}
