  ...
  inv :=
    { sourceMap := (finiteSourceEquiv doctrine.Source).symm
      ...
      extraction_iff := by
        intro source atom
        symm
        simpa using finiteCofiniteDoctrineCodeOf_extracts_iff doctrine hall
...
noncomputable def endpointFiniteTargetCofinitePresentation
  ...
  extraction_eq := by
    ...
    let sourceHall :=
      allExtractionsFiniteOrCofinite_source_of_hom input.hom htarget
    unfold finiteCofiniteDoctrineCodeOf
    simp only
    unfold finiteCofiniteExtractionCode
    simp only [Equiv.apply_symm_apply, hsourceRange, htargetRange,
      AtomPermutationCode.toEquiv_refl, AtomPredicateCode.transport_refl]
    apply finiteOrCofiniteAtomPredicateCode_congr
    · have hbase :=
        normalizedExtractionPredicate_finiteOrCofinite_of_mem_range
          input.target.doctrine htarget
          (input.target.doctrine.normalize
            (input.hom.doctrineHom.sourceMap
              (finiteSourceEquiv input.source.doctrine.Source source)))
          htargetRange
      ...
      · left
        exact hfinite.preimage
          (Set.injOn_of_injective input.hom.doctrineHom.atomEquiv.injective)
      · right
        exact hcofinite.preimage
          (Set.injOn_of_injective input.hom.doctrineHom.atomEquiv.injective)
    · simpa using
        normalizedExtractionPredicate_finiteOrCofinite_of_mem_range
          input.source.doctrine sourceHall
          (input.source.doctrine.normalize
            (finiteSourceEquiv input.source.doctrine.Source source))
          hsourceRange
    · intro atom
