...
/-- The selected quotient-level map satisfies its generated cartesian factor graph. -/
theorem finiteCodeSelectedCoreFiberReindexFunctor_map_fac
    {U : AtomCarrier.{u}} [DecidableEq U.Atom]
    {source target : FiniteInstanceCode U}
    (hom : FiniteCodeCartHom source target)
    {first second : CoreFiber target.toSemantic}
    (vertical : first ⟶ second) :
    ((finiteCodeSelectedCoreFiberReindexFunctor hom).map vertical).1 ≫
        (finiteCodeSelectedCoreFiberCartesianLift hom second).hom =
      (finiteCodeSelectedCoreFiberCartesianLift hom first).hom ≫ vertical.1 := by
  simpa only [finiteCodeSelectedCoreFiberReindexFunctor,
    finiteCodeSelectedCoreFiberCartesianLift] using
      selectedTypedCoreFiberReindexFunctor_map_fac hom.representative vertical
  ...
  calc
    ...
    _ = ((finiteCodeSelectedCoreFiberCartesianLift first
        ...
        (finiteCodeSelectedCoreFiberCompositorApp second third
          ...
          (finiteCodeSelectedCoreFiberCartesianLift (second ≫q third)
            targetPackage).hom := by
      have hmap := finiteCodeSelectedCoreFiberReindexFunctor_map_fac
        first
        (finiteCodeSelectedCoreFiberCompositorApp second third
          targetPackage).hom
      have hpost := congrArg
...
/--
The inverse quotient compositors cancel the right-associated associativity
route to the comparison generated by categorical associativity.
-/
theorem finiteCodeSelectedCoreFiberAssocRightRoute_inverse_normalization
    {U : AtomCarrier.{u}} [DecidableEq U.Atom]
    {firstObject secondObject thirdObject fourthObject : FiniteInstanceCode U}
    (first : FiniteCodeCartHom firstObject secondObject)
    (second : FiniteCodeCartHom secondObject thirdObject)
    (third : FiniteCodeCartHom thirdObject fourthObject)
    (targetPackage : CoreFiber fourthObject.toSemantic) :
    (finiteCodeSelectedCoreFiberCompositor first (second ≫q third)).inv.app
        targetPackage ≫
      (finiteCodeSelectedCoreFiberReindexFunctor first).map
        ((finiteCodeSelectedCoreFiberCompositor second third).inv.app
          targetPackage) ≫
      finiteCodeSelectedCoreFiberAssocRightRoute first second third
        targetPackage =
      (finiteCodeSelectedCoreFiberHomComparison
...
/--
The inverse target-identity compositor and unitor cancel the target-unit route
to the comparison generated by the categorical right-unit law.
-/
theorem finiteCodeSelectedCoreFiberRightUnitRoute_inverse_normalization
    {U : AtomCarrier.{u}} [DecidableEq U.Atom]
    {source target : FiniteInstanceCode U}
    (hom : FiniteCodeCartHom source target)
    (targetPackage : CoreFiber target.toSemantic) :
    ((finiteCodeSelectedCoreFiberCompositorApp hom (𝟙 target)
        ...
        finiteCodeSelectedCoreFiberRightUnitRoute hom targetPackage =
      (finiteCodeSelectedCoreFiberHomComparison
        (finiteCodeComp_id hom)).hom.app targetPackage := by
  simp only [finiteCodeSelectedCoreFiberRightUnitRoute, Category.assoc,
    Iso.map_inv_hom_id_assoc, Iso.inv_hom_id_assoc]

/--
The inverse source-identity compositor and unitor cancel the source-unit route
to the comparison generated by the categorical left-unit law.
-/
theorem finiteCodeSelectedCoreFiberLeftUnitRoute_inverse_normalization
    {U : AtomCarrier.{u}} [DecidableEq U.Atom]
    {source target : FiniteInstanceCode U}
    (hom : FiniteCodeCartHom source target)
    (targetPackage : CoreFiber target.toSemantic) :
    ((finiteCodeSelectedCoreFiberCompositorApp (𝟙 source) hom
        ...
        finiteCodeSelectedCoreFiberLeftUnitRoute hom targetPackage =
      (finiteCodeSelectedCoreFiberHomComparison
        (finiteCodeId_comp hom)).hom.app targetPackage := by
  simp only [finiteCodeSelectedCoreFiberLeftUnitRoute, Category.assoc,
      ...
      _ = (finiteCodeSelectedCoreFiberHomComparison
        ...
        exact
          finiteCodeSelectedCoreFiberAssocRightRoute_inverse_normalization
            third.unop second.unop first.unop targetPackage
      _ = _ := by
        simpa only using
  ...
  · intro source _ hom
    ...
    calc
      ...
      _ = (finiteCodeSelectedCoreFiberHomComparison
        ...
        exact finiteCodeSelectedCoreFiberRightUnitRoute_inverse_normalization
          hom.unop targetPackage
      _ = _ := by
        simpa only using
          finiteCodeSelectedCoreFiberHomComparison_hom_app_eqToHom
  ...
  · intro _ target hom
    ...
    calc
      ...
      _ = (finiteCodeSelectedCoreFiberHomComparison
        ...
        exact finiteCodeSelectedCoreFiberLeftUnitRoute_inverse_normalization
          hom.unop targetPackage
      _ = _ := by
        simpa only using
          finiteCodeSelectedCoreFiberHomComparison_hom_app_eqToHom
