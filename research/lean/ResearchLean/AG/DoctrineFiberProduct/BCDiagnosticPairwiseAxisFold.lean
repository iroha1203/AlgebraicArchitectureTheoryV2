...
/--
Retag one actual G-106 raw-cochain component along equality of its target
package.  This is the only endpoint identification consumed by
`pairwiseRawDefect`; it supplies no comparison or fold certificate.
-/
...
/-- Retagging preserves the underlying automorphism up to dependent equality. -/
theorem castTarget_heq
    {G : FiniteTransportPresentation.{u}} {U : AtomCarrier.{u}}
    (data : AdmissibleTransportData G U) {first second : G.TwoCell}
    (package_eq : data.lift.package (G.twoTarget first) =
      data.lift.package (G.twoTarget second))
    (automorphism : PackageFiberAut
      (data.lift.package (G.twoTarget first))) :
    HEq (castTarget data package_eq automorphism) automorphism := by
  exact cast_heq _ automorphism
...
/-- Characterization API for the ordered quotient, avoiding downstream unfold. -/
theorem pairwiseRawDefect_eq
    {G : FiniteTransportPresentation.{u}} {U : AtomCarrier.{u}}
    (data : AdmissibleTransportData G U) (cochain : DefectCochain data)
    (first second : G.TwoCell)
    (package_eq : data.lift.package (G.twoTarget first) =
    ...
    pairwiseRawDefect data cochain first second package_eq =
      cochain second * (castTarget data package_eq (cochain first))⁻¹ := rfl
...
/--
Replacing an already identified common right factor cannot change the pairwise
quotient.  This is a group-level cancellation lemma only; connecting actual
presentation replacement to such a common factor is a separate obligation.
-/
theorem commonCanonicalPairwiseQuotient_factor_invariant
    {U : AtomCarrier.{u}} {P : AATCorePackage U}
...
/--
Internal evidence for the G-110 double-diamond diagnostic: the two selected
G-106 cells have literally the same boundary paths and endpoint package, and
their computed quotient supplies the moved-axis witness.  The path equalities
prevent an arbitrary same-endpoint face from being used as a common-factor
certificate.
-/
structure PairwiseAxisFoldWitnessAt
  ...
  package_eq : data.lift.package (G.twoTarget first) =
    data.lift.package (G.twoTarget second)
  /-- The selected faces have the same dependently typed left boundary path. -/
  leftPath_heq : HEq (G.twoLeft first) (G.twoLeft second)
  /-- The selected faces have the same dependently typed right boundary path. -/
  rightPath_heq : HEq (G.twoRight first) (G.twoRight second)
...
/--
Auxiliary initial-coordinate relation for the pairwise diagnostic checkpoint.
It is intentionally not the public K2 `MateCoherentRel`: the latter must be
generated by the authored-table producer and audited under actual presentation
replacement.
-/
def DiagnosticPairwiseAxisFoldMateCoherentRel
    (U : AtomCarrier.{u}) [DecidableEq U.Atom] :
