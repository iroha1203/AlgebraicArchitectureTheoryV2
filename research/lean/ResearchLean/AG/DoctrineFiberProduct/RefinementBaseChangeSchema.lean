...
structure RefinementOverHom
  ...
  /--
  The upper exact package change observes the lower source map at the selected
  package point: extraction is reflected as well as preserved there.
  -/
  selected_extraction_iff : ∀ atom : U.Atom,
    Y.doctrine.extracts
        (lower.doctrineHom.sourceMap X.source)
        (lower.doctrineHom.atomMap atom) ↔
...
theorem ext
  ...
  rcases first with ⟨lower₁, rfl, upper₁, atom₁, extraction₁⟩
  rcases second with ⟨lower₂, rfl, upper₂, atom₂, extraction₂⟩
  cases hupper
  rfl
...
noncomputable def precomp
  ...
  atomEquiv_eq := by
    ...
    rfl
  selected_extraction_iff := hom.selected_extraction_iff
...
structure RefinementBCRegime (C : RefinementBCConfiguration U) where
  ...
  /--
  Cancellation form of the exact selected-lift universal property.  It is
  stated for the mixed source/target graph needed below, so route naturality is
  generated by comparing two factorizations rather than supplied as a law.
  -/
  mateRoute_factor_unique : ∀ {source target : CoreFiber C.DOne}
    (vertical : source ⟶ target)
    (first second : RefinementOverHom C.pulledRefinement
      ((exact_bottom_semantic_global_reindex_functor C.pulledFst).obj
        (baseCleavage.reverseFunctor.obj source))
      ((exact_bottom_semantic_global_reindex_functor C.pullbackFst).obj target)),
    first.upper.comp
        (exact_bottom_semantic_global_selected_lift C.pullbackFst target).hom.upper =
      ((exact_bottom_semantic_global_selected_lift C.pulledFst
          (baseCleavage.reverseFunctor.obj source)).hom.upper.comp
        (baseCleavage.lift source).hom.upper).comp vertical.1.upper →
    second.upper.comp
        (exact_bottom_semantic_global_selected_lift C.pullbackFst target).hom.upper =
      ((exact_bottom_semantic_global_selected_lift C.pulledFst
          (baseCleavage.reverseFunctor.obj source)).hom.upper.comp
...
/--
The two mixed route candidates have the same exact selected-lift graph, by the
two generated reverse-map factor equations and G-112 exact reindexing.
-/
theorem RefinementBCRegime.mateRoute_natural
  ...
  apply regime.mateRoute_factor_unique vertical
    ...
    change (((exact_bottom_semantic_global_reindex_functor C.pulledFst).map
        ...
        (regime.mateRoute target).upper).comp
          (exact_bottom_semantic_global_selected_lift
            C.pullbackFst target).hom.upper = _
