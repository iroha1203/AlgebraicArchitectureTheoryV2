...
- G3: `lake env lean` and `Research package build` pass. `BridgeCertificate` / `BridgeObstruction` / concrete trace obstruction are axiom-free; package and local exactness witnesses inherit standard `propext` / `Quot.sound` from imported exactness infrastructure.
