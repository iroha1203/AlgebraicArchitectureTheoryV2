...
capability_category: refinement-order, common-refinement, nerve-map-mismatch, conway-obstruction, finite-witness, receiver
...
proof_or_evidence_plan: `Formal/AG/Research/SFT/ConwaySupportReceiver.lean` に `CoverRefines`、`CommonRefinementSpan`、`SupportNerveEdge`、`SupportNerveFork`、`SupportNerveObstructionReceiver`、`supportReceiver_iff_conwayObstruction`、Cycle 1 atlas の receiver package を未証明穴なしで置く。
...
rival_advantage: CODEOWNERS / org-network dashboard / AI review は mismatch を指摘できるが、support-nerve edge/fork receiver と zero/nonzero theorem を返さない。
...
- `proof_or_evidence_plan`: `lake env lean Formal/AG/Research/SFT/ConwaySupportReceiver.lean`、`lake build FormalAGResearch`、`#print axioms` で検証する。
...
- 2026-07-04: 作成。`lake env lean Formal/AG/Research/SFT/ConwaySupportReceiver.lean` と
  `lake build Formal.AG.Research.SFT.ConwaySupportReceiver`、再実行後の `lake build FormalAGResearch` が通過。
