...
result:
  ...
  exact_declaration_map:
    ...
    sha256: 79b536ee95933ef26caa22a560cd6c4922fa84998337d05770d270110206c031
    generator: research/reports/evidence/g115_exact_declaration_map.rb
    generator_sha256: 078203cdc044110c1d994fbb3c1e769e4f2c2f37445fe50b6f8028844450a726
...
result:
  ...
  proof_state: target-proof-checkpoint
  completion_candidate: yes
  proof_obligation_delta: The source-only generator first requires the exact accepted Cycle manifest 1 through 83 with the intentional historical gaps 28 and 65, then verifies SHA-256 2b31e4c59f8e58c96ad6b8dcc3aa18879608c13127d1ef0a874a82a625524c44 over every accepted cycle's ordered target and artifact lists together with 1066 occurrences, 1063 report identities, and 80 targets. Invalid artifact tokens and non-Lean target entries are rejected rather than filtered. The manifest includes every non-planned claim_mapping theorem name: the 32 formerly omitted target-facing declarations, the Cycle 64 DecisionEdge and DecisionCell inductives and constructors, and nilPath and twistPath. Resolution remains cycle-local, but its complete ordered result is independently locked by SHA-256 018da6091b9f0754fece7204de0e8bcdff684ed2f4862d1c0bfa8022d7fa8d31 over cycle, report token, canonical identity, declaration kind, and module. Thus a same-suffix structure field or another declaration kind cannot replace a theorem. Its state-machine lexer removes nested block comments, line comments, escaped strings, multiline strings, and hash-delimited raw strings and rejects any unterminated lexical state. Namespace and section ends are balanced and named ends must match. The declaration parser recognizes structures, classes, definitions, abbreviations, theorems, lemmas, opaques, axioms, constants, local/scoped/named instances, inductives, constructors, and structure fields. Because mapped files currently need neither syntax quotation nor escaped identifiers, any comment/string-external backtick or guillemet is rejected fail closed rather than parsed. Every parsed declaration must lie under AAT.AG.DoctrineFiberProduct; unresolved, ambiguous, duplicate-source, input-manifest, or resolved-kind-manifest mismatch exits before output. The built-in probe covers nested comments, ordinary/multiline/raw strings, axiom, local instance, inductive, and constructor parsing. The Cycle 64 inductive commands include the deriving clauses that generate the DecidableEq and Fintype instances consumed by presentation inferInstance; those compiler-generated declarations have no separate source declaration command and are covered as provenance of the mapped inductive commands rather than falsely presented as parsed source declarations. The generator computes every module hash from the same in-memory byte snapshot used for identity resolution, completes the entire buffered YAML before one stdout write, and never rereads a source during output. Lean acceptance provenance remains the single-file focused evidence recorded in the corresponding Cycles. The final Cycle 84 source-map reproduction runs no Lean or lake command; earlier forbidden group-build attempts are disqualified below and are not evidence.
  lean_artifacts: []
  evidence_artifacts:
    map:
      path: research/reports/evidence/G-115-revision9-exact-declaration-map.yml
      sha256: 79b536ee95933ef26caa22a560cd6c4922fa84998337d05770d270110206c031
      blob_sha: 587aedcc387271f2e102d7a8ac168a34914014db
    generator:
      path: research/reports/evidence/g115_exact_declaration_map.rb
      sha256: 078203cdc044110c1d994fbb3c1e769e4f2c2f37445fe50b6f8028844450a726
      blob_sha: 62fdea3cfd730b2fe6bc0ac26a988c8da8b2b4b3
  ...
  evidence: [lossless accepted-cycle input-manifest SHA-256 and fixed count validation, locked canonical-identity module and declaration-kind manifest SHA-256, all non-planned claim_mapping theorem names covered, byte-exact source-map regeneration, comment ordinary-string multiline-string and raw-string aware cycle-local source namespace-stack resolution, syntax quotation fail-closed control, canonical-root namespace control, accepted-manifest identity mutation rejected before resolution with exit 1 and zero output, raw-string and declaration-kind substitution controls rejected with zero output, arbitrary and incomplete report inputs rejected with exit 1 and zero output, former partial-output arguments rejected with exit 2 and zero output, same-byte-snapshot defining-module hashes, single buffered stdout write, per-cycle single-file focused Lean evidence recorded in Cycles 1 through 83]
audits:
  declaration_identity: pass; all map identities are canonical source identities relative to AAT.AG.DoctrineFiberProduct and the ordered canonical identity module and declaration-kind resolution is locked independently of the report-token manifest
  ...
  declaration_kind_substitution_control: pass; replacing the actual nil theorem with a same-suffix structure field is rejected with exit 1 and zero stdout
  ...
  claim_mapping_coverage: pass; every syntactically named non-planned theorem_names entry through Cycle 83 occurs in the canonical map
  ...
  validation_refs:
    ...
    - "the canonical-manifest command-quotation, same-suffix structure-field, local-instance, and Bogus.Root namespace mutations were rejected with exit 1 and zero stdout"
