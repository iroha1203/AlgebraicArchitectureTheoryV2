...
result:
  ...
  lean_artifacts: [principal current-head declarations listed in claim_mapping, historical per-cycle declaration and source evidence in Cycles 1 through 81, exact declaration-to-module-to-head-hash map pending the schema-complete same-head final packet]
...
result:
  ...
  exact_declaration_map:
    ...
    sha256: fcd478a0f24ce15b7948eafccba5aa8bcb8f0adb9d76de780f3461c5feb85ad9
    generator: research/reports/evidence/g115_exact_declaration_map.rb
    generator_sha256: 381f3091f7edf8e34f631ec307164bf92428347a7a1edc138c3293c7da0e8a46
    counts: {artifacts: 1031, modules: 80, resolved: 1031, unresolved: 0, ambiguous: 0}
  evidence: [focused Lean check of UpperGeometryCompatibleSolutionEquivalence with 43 declarations under standard axioms only, focused Lean check of UpperGeometryExchangeExactness with five declarations under standard axioms only, source-field literal proof-use scan, canonical source-identity 1031-declaration map reproduction with same-snapshot module hashes, git diff check, hidden and bidirectional Unicode scan]
  claim_mapping:
...
audits:
  ...
  blocking_findings: []
  next_obligation: Bind the canonical namespace-qualified 1031-declaration map to one fixed head, publish a schema-complete replacement packet, run four wholly fresh completion lanes, post the formal ledger, require green CI, merge PR #4338, then synchronize GOAL report registry and Issue #4250 while leaving the tracking Issue open.
```
...
result:
  ...
  proof_state: target-proof-checkpoint
  completion_candidate: yes
  proof_obligation_delta: The source-only generator first requires the exact accepted Cycle manifest 1 through 83 with the intentional historical gaps 28 and 65, then verifies SHA-256 ef75933b0a960097daac0c6541feadc66bc7a213be3374a47142d46b4ad9710a over every accepted cycle's ordered target and artifact lists together with 1034 occurrences, 1031 report identities, and 80 targets. The manifest now names the Cycle 64 finite presentation's DecisionEdge and DecisionCell inductives, their constructors, nilPath, and twistPath as explicit K4 dependencies. Resolution remains cycle-local. Its state-machine lexer removes nested block comments, line comments, escaped strings, and multiline strings and rejects any unterminated lexical state. Namespace and section ends are balanced and named ends must match. The declaration parser recognizes structures, classes, definitions, abbreviations, theorems, lemmas, opaques, axioms, constants, named instances, inductives, constructors, and structure fields. Because mapped files currently need no syntax quotation, any comment/string-external backtick is rejected fail closed rather than parsed. The resolver requires every declaration to lie under AAT.AG.DoctrineFiberProduct, reconstructs its namespace stack, accepts a report token only as an exact identity or exact qualified suffix, rejects unresolved or ambiguous identities with exit 1, and contains no global fallback or manual resolution override. A built-in nested-comment, string, axiom, inductive, and constructor probe guards the parser. It accepts only the complete source-map invocation; former partition, audit, and all other extra arguments exit 2 before emitting output. Arbitrary documents and missing, renumbered, duplicate, or identity-mutated accepted-cycle manifests fail before resolution with zero stdout. Canonical-manifest source mutations that place an identity only inside nested comments or command quotation, move a defining namespace outside the canonical root, add an outside-root axiom, mismatch a named end, or leave a block comment unterminated are rejected with zero stdout. The generator computes every module hash from the same in-memory byte snapshot used for identity resolution, completes the entire buffered YAML before one stdout write, and never rereads a source during output. Lean acceptance provenance remains the single-file focused evidence recorded in the corresponding Cycles. The final Cycle 84 source-map reproduction runs no Lean or lake command; earlier forbidden group-build attempts are disqualified below and are not evidence.
  lean_artifacts: []
  evidence_artifacts:
    map:
      path: research/reports/evidence/G-115-revision9-exact-declaration-map.yml
      sha256: fcd478a0f24ce15b7948eafccba5aa8bcb8f0adb9d76de780f3461c5feb85ad9
      blob_sha: 464f95aa4f0319388e8d8adf4863505718c09ce8
    generator:
      path: research/reports/evidence/g115_exact_declaration_map.rb
      sha256: 381f3091f7edf8e34f631ec307164bf92428347a7a1edc138c3293c7da0e8a46
      blob_sha: 4189d5adb483631e98332d0377e5807a1df1091a
    counts: {accepted_artifact_occurrences: 1034, canonical_source_identities: 1031, modules: 80, resolved: 1031, unresolved: 0, ambiguous: 0}
...
audits:
  ...
  lexer_regression_probe: pass; nested block comments, line comments, escaped strings, and multiline strings cannot contribute declarations; unterminated lexical states fail closed
  declaration_parser_probe: pass; axioms, inductives, and constructors are recognized, namespace and section scopes must balance, and named ends must match
  ...
  input_manifest: pass; exact accepted cycle list, ordered target and artifact manifest SHA-256, 1034 occurrences, 1031 report identities, and 80 targets are checked before source resolution
