...
result:
  ...
  claim_mapping:
    declaration_map_contract: The exhaustive target-facing declaration map is the union of each accepted Cycle 1 through 81 `lean_artifacts` list, paired with that cycle's `source_sha256`; the grouped references below fix the exact cross-revision partition, and the following theorem names are its principal entry points rather than a substitute for that exhaustive union.
    declaration_map_refs:
