...
selection:
  ...
  proof_obligation: Define the certificate-free compatible input and generate both literal reverse-route geometries, route legs, coefficient identifications, and strong-cartesian qualifications solely from its one source transport.
...
audits:
  ...
  proof_use:
    used: [FixedCoefficientTwoLayerTransportOver, UpperGeometryCleavage.baseRouteGeometry, UpperGeometryCleavage.pulledRouteGeometry, UpperGeometryCleavage.baseRouteGeometryHom, UpperGeometryCleavage.pulledRouteGeometryHom, baseRouteGeometryHom_isStronglyCartesian, pulledRouteGeometryHom_isStronglyCartesian]
    unused: []
  structure_field_escape: the input has no route leg, route comparator, endpoint comparison, IsIso, cartesianness certificate, solution, or raw cochain field; its only comparator is inside sourceTransport
