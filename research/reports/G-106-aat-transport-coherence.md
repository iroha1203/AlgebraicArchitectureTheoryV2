...
audits:
  ...
  validation_refs:
    ...
    - corrected-head static gates, fixed-head review, and CI are pending
  review_history:
    ...
    - the current correction replaces face lists by indexed RewritePasting values and proves endpoint-gauge, canonical-comparator, raw-defect, cochain-translation, and state-action laws; a new fixed-head four-lane review is required
  blocking_findings:
    - corrected declaration surface requires static gates, fixed-head four-lane review, and CI
...
- 修正版の placeholder、hidden / bidirectional Unicode、privacy、Research import 方向、
  package 依存方向、manifest、`git diff --check`: pending
