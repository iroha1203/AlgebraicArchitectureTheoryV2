...
selection:
  ...
  raw_fixture_checkpoint: 779bd010
  raw_fixture_artifact: research/fixtures/G-112-k1-o6-raw-data.md
  selected_index: 11
...
result:
  ...
  certificate_provenance:
    ...
    - certificate: "O6 raw-data chronology"
      source: "commit 779bd010 fixes carrier, endpoint doctrines, arrows, candidate assignment, and final counterexample before the proof commit"
  proof_use:
    required:
  ...
  dependency_dag: pass
  validation_refs:
    - "cd research/lean && lake env lean ResearchLean/AG/DoctrineFiberProduct/ExactBottomCoverageClassification.lean / exit 0 / 88 declarations / standard axioms only"
  blocking_findings:
...
PR #4190 の初回4 lane は、Lean の数学構成ではなく O6 raw counterexample の
時系列 provenance を中心 finding とした。initial head `7f6b6317...` は fixture、
proof、ledger を同一 commit で追加しており、GOAL が要求する選定時固定を監査
できなかったため受理しない。新しい K1 attempt は commit `779bd010` で
[`research/fixtures/G-112-k1-o6-raw-data.md`](../fixtures/G-112-k1-o6-raw-data.md)
だけを先に固定した。この artifact は carrier、endpoint doctrine、semantic arrow、
候補 index への割当、final counterexample、qualification raw family の data のみを
記録し、発火・非coverage・qualification certificate を含まない。

同じ初回 review の非中心 finding に対して、全新規 public declarationへ docstring と
module-level Implementation notes を追加し、presentation proof が新規 code 定義を
直接 unfold していた箇所には
`finiteCofiniteExtractionCode_eq_of_mem_range` と
`finiteCofiniteDoctrineCodeOf_extraction` の no-unfold API を追加した。proof commit の
fixed head に対して fresh 4 lane を再実行する。
