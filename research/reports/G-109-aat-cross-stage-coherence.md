...
result:
  ...
  proof_obligation_delta:
    discharged:
      - endpoint-indexed `CellChainNode` / `CellChainStep` / `CellChain` with explicit endpoint casts and forward/backward orientation
      - twisted affine step `x ↦ u_c * x * φ₀(c)⁻¹` and inverse-oriented route transport
      - authored and arbitrary-reselection canonical factor words, including append and reversal laws
      ...
      - parallel-cell two-chain holonomy as the authored-comparator ratio
      - general necessity `JointVanishes → CellChainCoherent` (T2), obtained without an edge-level class assumption
      - w1 connection `CompatiblePairRefutation` fixture → a concrete nontrivial two-chain → `¬CellChainCoherent`
    remaining:
...
audits:
  ...
  certificate_provenance:
    discharged:
      - authored factors come from `upperAuthoredPastingComparator`
  ...
  proof_use:
    ...
    unused: []
  structure_field_escape: none-found
  route_integrity: pass; endpoint casts, orientation, authored/canonical factor order, and dependent fiber transport are theorem-visible
  target_fitting: none-found
  ...
  goal_or_report_reinterpretation: none-found
  validation_refs:
    - `./check_research_modules.sh --focused ResearchLean/AG/CrossStageCoherence/CellChain.lean`: pass; 140 declarations, standard axioms only
    - `./check_research_modules.sh --focused ResearchLean/AG/CrossStageCoherence/CellChainInstances.lean`: pass; 9 declarations, standard axioms only
    ...
    - focused `#print axioms` audit of the 15 public spine declarations: only `propext`, `Classical.choice`, and `Quot.sound`
