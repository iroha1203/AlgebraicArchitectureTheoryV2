...
現在の `research/GOALS.md` の SCORE threshold は 5000 であり、cycle 28 後の total SCORE は 3440 である。
次に進める場合は、lawful transport の下での repair/transport commutator criterion、
selected decomposition 上の commutator localization、
または visible-only ではない transport family の sharp obstruction を狙う。
