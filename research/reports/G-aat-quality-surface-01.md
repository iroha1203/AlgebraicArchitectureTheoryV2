...
formalization_quality: pass. `lake env lean research/lean/ResearchLean/AG/QualitySurface/SemanticResidualGeneratedTargetScannerProvenance.lean`, `lake build ResearchLean.AG.QualitySurface.SemanticResidualGeneratedTargetScannerProvenance`, and `lake build ResearchLean` passed. Axiom probe reported standard `propext` / `Quot.sound` only. No `sorryAx`, custom axiom, `Classical.choice`, or `unsafe` was reported.
