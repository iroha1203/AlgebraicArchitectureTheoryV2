...
- 主張は、その GOAL の claim scope（[GOAL カードの型](../goals/README.md#goal-カードの型)が定める）の上に相対化して述べる。
- GOAL の `rival` に対する delta を書く。既存の静的解析、ADL 解析、architecture conformance、metric dashboard が得意な検出や可視化を認めたうえで、成果がどの support、trace、transport、obstruction、repair、failure mode を新しく扱えるかを明示する。
...
- `Formal/AG` 本体を直接編集して得た成果を、このループの成果として扱わない。形式化は `research/lean/ResearchLean` 側に置く。
