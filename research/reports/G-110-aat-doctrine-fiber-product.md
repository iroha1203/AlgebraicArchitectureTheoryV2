...
result:
  ...
  claim_mapping:
    ...
    conjuncts:
      ...
      - semantic-equal presentation replacement preserves the transpose/unit/counit factor graph through the reviewed selected comparison, not a whole-functor cast
  ...
  dependency_dag:
    ...
    - selectedTypedCoreFiberPresentationComparisonApp_hom_fac -> typed presentation compatibility
    - finiteSelectiveTwoToSupportInput + finiteReindexAxisSwapHom -> noninvertible/nonidentity finite firing
  premise_audit:
  ...
  regression_scenarios:
    ...
    material_premise_unused: rejected; both strong lift APIs and the presentation-comparison triangle occur in the proof DAG
  ...
  verification:
    - focused direct check CoreTransportReindexAdjunction.lean: pass; 24 namespace declarations, standard axioms only
    - focused direct check CoreTransportReindexAdjunctionWitnesses.lean: pass; 13 namespace declarations, standard axioms only
  ...
  review:
    status: pending standard review-pr and root acceptance-contract integration
next:
  proof_obligation: construct the pointed pullback square from the existing compatible-point producer, then generate the canonical Beck--Chevalley mate from the Cycle 35 units/counits and the accepted push/pull compositors; keep packageProjection-specific exactness and authored-support relative obstruction as separate downstream subnodes if the typed mate surface closes first
```
