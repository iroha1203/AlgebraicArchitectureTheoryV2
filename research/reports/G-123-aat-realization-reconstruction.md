...
- current proof obligation: Cycle 62 source-proved involution law for the same bottom restriction-kernel element, two distinct points in every bottom lift fiber, and return after the second shift
- current target state: `target-proof-checkpoint`
- completion candidate: no
- next proof obligation: define and classify the subgroup orbit generated by the displayed involution, lift the canonical normalized and bottom-qualified comparison sections into source syntax, and extend kernel coverage without completed semantic group-element leaves
