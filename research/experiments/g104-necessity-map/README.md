...
9de3a00f37c20393df01985f5c43eeec6ec21f6906083e01636dd8dfdab93502  results-stop-b-summary.json
b8ac9461efe3dc4a039f986fa70daa0ab95842d83d7c9ddcb77d7024ffefc95e  regenerated full Stop-B results.json
```

Issue #3948 comment `5231857267` の人間裁定により、旧repo fileのSHA
`cabfbcae…2306` は regenerated parent full JSON の期待値へ役割を変更した。この配置補正は
探索上の進展ではない。Round 12までの2 SHAは final Stop-B artifactのimmutable parentとして
そのまま保持する。
...
current permanent structural contractは Issue #3948 comment `5246699114` に固定した。
compact canonical bytesは `314821`、SHA-256は
`955b75d7f88c2d7e3f7e516cb83928127fed9cbd8d28bb50572b17c49a7531af` である。
...
移行後 regression は comment `5246749681` に同期した。旧実行記録はcurrent contractの
観測意味を代用せず、opaque Git / Issue provenanceとしてだけ保持する。
...
- current checker: compact canonical bytes `56940`、SHA-256
