...
ループの進行状態の正本は、GOAL ごとに一本立てる GitHub の tracking Issue `Research Loop: <goal-id>` に置く。候補ごと、サイクルごとの tracking Issue は作らず、探索型 GOAL では active SCORE threshold、current SCORE、候補カード、PR、iteration comment をこの Issue に集約する。`goals/<goal-id>.md` は GOAL 定義、カードの frontmatter と検証結果のレポートは証拠 artifact であり、作業を中断してもこの Issue を読めば同じ地点から再開できる。`target-theorem` では候補カードを作らず、自然言語のtarget claimとcompletion criteriaは`goals/<goal-id>.md`が正本にし、task固有の完全Lean signatureを固定するstatement contractは`docs/aat/lean_quality_standard.md` §5.1と`target-theorem-loop`のcontract preflightで指定した一つのartifactを正本にする。tracking Issueにはproof state、完了 / 未完 proof obligation、blocker、PR、target_cycle_result、`$math-lean-review`のcompletion gate結果とcontractへの参照を置く。
