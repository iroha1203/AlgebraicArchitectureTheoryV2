...
- `target theorem boundary`: Lean置き場所は`Formal/AG/Research/QualitySurface/LawGeneratedConormalDescent.lean`または同directoryのsupport file。G-07の完了面はcover-relative first-order theoremとsemantic first-order representation。ideal-power higher stagesは後続theorem family、G-04はnonabelian / higher / stacky / universality、runtime extractionはtooling PRDへ接続する。
