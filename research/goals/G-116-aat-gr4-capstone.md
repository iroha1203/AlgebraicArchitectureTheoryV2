...
- `target theorem completion criteria`: 全 artifact が sorry なしで
  ...
  **さらに、達成記録(O19)を completion の必須義務とする**:
  ...
  3. **範囲併記**: coverage の到達段(第一段 / 第二段)、O6 /
     O12・G-113 の transport equivalence / base `IsIso` 関係決定・
...
| O18 | `Full` / `Faithful` / `EssentiallySurjective` producer と explicit equivalence、base `IsIso` との関係決定 | G-113 |
...
- n1005 §4.3 (D) の「情報損失の分類」は revision 1 の反証により
