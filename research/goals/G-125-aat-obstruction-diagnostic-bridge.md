...
2. Aの入力族に属する小さな有限例として、reading変更 `q_c ≼ q_f` の一組を選ぶ。
   canonical factor `q_f.Target → q_c.Target` の非単射性、両端の `R_qc`・`R_qf`、
   その変更の `ConditionC` を同じ入力で証明する。粗い側で係数・被覆・Law評価を
   固定し、零障害と非零障害を生む局所データを与え、変更に沿って細かい側の
   対応データを生成する。零・非零の両ケースで(B1)・(B2)、(C1)、指定障害類の
   輸送、(C2)を適用し、両readingの障害類と診断類がそれぞれ零・非零であることを
   証明する。入力値の選択は構成時に行い、reportに固定する。
