...
| lens族 | 全域get/putの三法則を満たす実現と、独立に指定した意味保存射の全範囲 |
...
2. **延長**: `S` に制限した整合条件を満たす任意の有限データが、全域の射・変更へ延長できる。
3. **実効**: `S` 上のデータの整合判定が決定可能で、延長が計算できる。

区別と延長を同時に満たす `S` を決定集合と書く。
...
- 決定集合の存在 ⟺ `π₀(Q)` が有限。各成分から一頂点を選ぶ `S` が決定集合になる
  ことも示す。
- `V`・`E`・`K` が有限のとき、`S` 上の整合判定の決定可能性と延長の計算可能性を与え、
  既存の有限例・fiber個数の宣言に接続する。
...
**E2. lens・プロトコルの有限決定と既存再構成の整合。** lensの全域put操作と
プロトコルの生成辺の操作を操作系としてDの判定へ接続する(既存のlens・プロトコル接続と
両群接続を使用)。lensでは全域put(三法則)が全fiberを基準fiberへ結ぶことから、
基準fiberの読み取りが決定集合になることを導き、既存の有限補完tableと一致させる。
プロトコルでは有限グラフの連結成分の有限性から決定集合の存在を導き、既存の
生成辺・観測tableによる再構成と接続する。

既存の有限表示によるKaroubi再構成(retract生成・`Arr` 版を含む)を、この二族の入力で
Bの主同値・Dの有限決定と整合させる自然同型・可換図式を構成する。有限表示からの
Karoubi再構成の強い結論はこの二族で保持し、全入力への一律の要求とはしない。
lens・プロトコルの既存比較群輸送をCの群同型と接続し、追随変更のsection・核・
...
| 操作系への既存接続(E2) | `RealizationReconstruction/FixedFLensConnection.lean`、`FixedFProtocolConnection.lean`、両GroupConnection |
...
| タグ変更族と `Ω` の無限性(E1) | 既存宣言・定理を入力として保持。群構造・逆極限・決定不能は証明義務 | (E1)・(E2)、非区別の構成、一様flip分離の保持 | Cycle 188の受理済み宣言からDの適用と有限性の限界の説明へ |
| lens・プロトコルの意味論と既存再構成(E2) | 意味論・法則は入力として保持。既存再構成・輸送は既存定理として使用。整合は証明義務 | E2の決定集合、自然同型・可換図式、群接続 | G-123の受理済み宣言からB・Dとの整合とCSへの帰結へ |
...
1. A–Eを同じ宣言・実現・局所表示の構成について確定し、Bの主同値、Cの回復、Dの判定、
   ...
   反証停止規則で扱う。
2. Cの三場合、E1の同一族での同時成立((E2)の同型・非区別・一様flip分離)、E2の
