...
- `program context`: Gr4 完遂 gate 第五項の担当カード(担当義務 =
  ...
  §5 判定線1)。依存は G-111。G-111 の statement 改訂は本カードの
  draft 差し戻しへ伝播し、本カードの改訂は G-116 の達成記録要件へ
  伝播する。
- `predecessor`: G-110(完遂済み。無条件 forward covariance と旧条件系
  の帰結。固定錨は G-111 カード ledger と同一)、G-111(indexed
  schema — draft、昇格順で先行)、G-106(reselection orbit 語彙)。
...
- `core tension`: 順方向の無条件性(G-110 で証明済み)に対し、逆方向は
  (d2) endpoint 群準同型が一般に非単射であることから自明に従わない —
  ここに保守性が class の理論になる根拠がある。最大リスクは二つ。
  class を「conservative と同値」の述語で立てる結論の埋め込み、および
  class が空・退化でも反射・検出が vacuous に立つこと(class 内の非
  自明成員の実在義務で防ぐ)。また本カードの反射(O14・O20)は G-109
  の段射影 `p` に沿う effectivity 反射(域外)と方向が異なる別物で
  あり、この分界を statement に明記する(n1007 §5 判定線3)。
- `rival`: conservative functor・descent の一般論。差は「診断障害
  ...
  置く。
- `claim boundary`: G-111 の full-domain indexed action 上で語る(schema
  型は G-111 の宣言を参照のみ)。G-106 系 raw defect / reselection
  語彙に一本化(`J_A` defect profile 枝は域外)。carrier change・係数・
  段射影方向の effectivity 反射は域外(G-116 カードの域外リストを
  継承)。
...
- `dullness filter`: 次を弾く。class を結論(conservative)と同値な
  ...
  構成、閉性が恒等成分で vacuous に立つ構成、G-110 の forward
  covariance の再証明を成果と数える構成。
...
- `target theorem`: **Diagnostic Conservativity Classification
  ...
  3. **(c) 反射 theorem**: class 上で target obstruction vanishing →
     ...
     準同型の非単射性により順方向無条件性から逆向きは従わない)を
     statement 側に一文固定する。
  ...
  5. **(e) pointwise raw-defect reflection の分類(O20)**: cochain 値
     ...
     vanishing 水準とは別 statement として証明する。
...
- `target theorem boundary`: Lean 置き場所は
  ...
  module。G-111 / G-110 / G-106 の reviewed module は参照のみ。完了面は
  (a)–(i) まで。順方向共変性(G-111)・coverage(G-112)・段射影方向の
  effectivity 反射(域外)は主張しない。
- `target proof artifacts`: `DiagnosticConservative` 定義、生成 class
  ...
  反射の分類 artifact(O20)、class 外破れ witness(O16)、class 内
  named nonvacuity witness、貼り合わせ閉性 theorem(O17)、
  `Full` + `Faithful` 決定 artifact(O18)、report
...
- `target material premise ledger`:
  ...
  - `G-111 indexed schema`: `ambient-boundary`(完遂後に final head を
    固定して昇格する — 昇格条件。schema 型の再定義禁止)。
  - `G-106 InReselectionOrbit 系`: `ambient-boundary`(固定錨は G-110
    カード ledger の G-106 錨を継承)。
  - `DiagnosticConservative 定義と生成 class`: `discharge-required`。
  - `反射・orbit 検出・O20 分類`: `discharge-required`。
  - `witness 対(class 外の破れ+class 内の実発火)`:
    `discharge-required`。
  - `貼り合わせ閉性`: `discharge-required`。
  - `Full + Faithful 決定`: `discharge-required`(どちらの決定でも
    成功)。
- `target route integrity gate`: 生成 class は探索前固定の構造条件で
  立て、fixture 値・結論由来の条件を持ち込まない。witness fixture は
  proof obligation 選定時に固定する。禁止経路 — 結論の埋め込み
  (class = conservative 同値述語)、可逆 fixture のみの反射発火、
...
- `target failure policy`: fail-closed を原則とする。(c)(d)(e) の反証は
  ...
  `target-refuted` とする。(i) は決定がどちらでも成功。witness の停滞は
