...
- **ArchSig** は観測(ArchMap)と法・方程式(LawPolicy / law-equation-surface / MeasurementProfile)の二系統の入力から、選ばれた語彙内の肯定的な diagnostic conclusion を計算する Rust tooling。第三の入力カテゴリは存在しない(入力トライアドの正本は AGENTS.md「責務範囲」)。結論の相対性は入力契約に由来する帰結であり、identity を「Lean 証明器ではない」のような否定形で書かない。Rust と Lean の対応は要求しない。
...
| 領域 | 場所 | 役割 |
| --- | --- | --- |
| Lean / AAT | `Formal/AG`, `Formal.lean` | AAT の形式化(定義・定理・例) |
