...
<!-- TODO: 公開時に前回記事(SAGA定理)の実URLへ差し替え -->

- この記事は [SAGA 定理の記事](https://zenn.dev/) の続編です。前回は「局所は正しいのに大域で壊れる」現象に Lean 4 で証明された定理を与えるところまでを書きました。今回は、その定理を現実世界に持ち出した報告です。
...
SAGA 定理は、このシリーズの土台理論 AAT(Algebraic Architecture Theory — ソフトウェアアーキテクチャを代数幾何として扱う理論)の中心定理です。名前は代数幾何の GAGA 定理へのオマージュで、分散トランザクションの Saga パターンとは無関係です(題材がマイクロサービスなので紛らわしいのですが、別物です)。Lean 4 で証明済み。`sorry`(証明の未完成箇所を示すプレースホルダ)はなく、依存公理は Lean 標準のものだけ。つまり定理が偽であるリスクは、形式化の時点で消えています。

では何のリスクが残っていたのか。空虚であるリスクです。

形式化された定理は、前提を満たす対象が数学の中にしか存在しない、という形で静かに死ぬことがあります。SAGA の前提の束は、かみ砕くとこうです。以後、各サービスを「チャート」、サービスが自分に課している規約を「法」と呼びます。各チャートは自分の法を守っている。チャートの間には値の受け渡しがある。それなのに、受け渡しを閉じたループに沿って一周すると、どこにも帳尻を合わせる場所のないズレが残る。この「一周して残るズレ」を数える量が、前回の記事の主役だったコホモロジー `H^1` でした。
...
**第1段(観測データのフルビルド)**: 42 サービス+共有モジュールの Java ソースから、ArchMap — コードから読み取った最小の観測事実(Atom)を集めた地図 — を正規フローで構築しました。2 系統の独立した読み取りパスで抽出し、差分を全件調停し、監査を通す。実ユーザー向けに定めた手順そのままです。結果は 2,118 個の Atom と 43 の観測単位(context)。読み取りはすべて軽量モデル(Claude Sonnet)のサブエージェントで行い、供給コストは実測で約 4.1 分/kLOC(壁時計、並列実行込み)でした。
...
第一に、前提の実在証明です。SAGA 定理は「証明済みだが、実例は合成のみ」から「証明済みで、実例が野生に在る」へ移りました。空虚であるリスクは、この1例で消えました。
...
- **良性と構造的の選別。** フルビルドで実測された食い違いは、観測できた受け渡し 6 辺中 5 辺に及びましたが、判定は零 — 局所の張り替えで貼り合う良性のドリフト — でした。非零が立ったのは三角形だけです。すべてを「問題」として列挙する指摘の洪水と、構造的な一点の指摘。この差は、レビュー疲れの時代には実用価値そのものです。
- **零の言明と再現性。** LLM は「問題ありませんでした」を保証付きで言えず、走らせるたびに指摘は揺れます(私たち自身の独立2パス観測でも、生の機械一致は 9.4% でした — §8)。計測は同じ入力から常に同じ判定を返し、根拠はソースの典拠まで遡れます。
...
- SAGA 階段が新しい障害を「検出」したのではありません。食い違いの検出は階段の最下段(Čech 計測)が担い、残差の配置は観測に基づいて私たちが供給したモデルです。SAGA 階段が加えたのは診断の構造 — 局所は合法という罠の明示、一周のズレの認証、修理案の事前検証、ゲートへの接続 — です。
- 修理は仮説状態です。PASS が示すのは「この修理案なら貼り合う」という事前検証であって、実装済みの修正ではありません。
- 実害の規模は測っていません。「丸め剰余がどこにも記帳されない」は静的に確実ですが、実運用でどの頻度・どの金額で現れるかは runtime の実測が要ります。数字がないのに数字を語らない、も規律のうちです。
- train-ticket は研究用ベンチマークです。本番システムの欠陥告発ではなく、現実にありがちな実装スタイルの混在が定理の前提をどう実現するかの観測として読んでください。この構造が「ありがち」の中に見つかったことは、現実のコードでも珍しくないことを示唆します。
