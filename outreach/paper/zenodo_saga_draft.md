...
本章は SAGA の中心定理を述べ、証明する。定義の完全な一般性と routine な検証は
canonical text 第X部に置き、比較定理を成立させる主要論証は本章内で連続して追える
ように書く。
...
| 段 | 内容 | 本文 | canonical |
| --- | --- | --- | --- |
| 係数同型 | soundness の導出と `Φ:M_sem≃Q_E` | §5.3 | 補題 6.2A、定理 6.3 |
| cochain 可換と `H^1` 同型 | degreewise `κ`、`κδ=δκ`、`κ_*` の誘導 | §5.4 | 定義 7.1、定理 7.2、系 7.3、定理 7.4 |
| residual 対応 | `κ_*([r_sem])=[r_E]` と統合 | §5.5 | 定理 7.5、定理 7.6 |
| global repair | true sheaf 条件下の三項同値と actual gluing | §5.6 | 系 4.5、補題 2.1A、定理 8.2、系 8.3 |
...
ordered Čech model との同一視と §5.6 の actual gluing で使用する。
...
である。`F_sem(V)` は `S(V)` 上の free abelian group なので、普遍性により `χ_V` は
一意な準同型 `χ̃_V:F_sem(V)→Q_E(V)` へ延長される。構成すべき `Φ` は `χ̃` を
repair relation で割った商上の写像であり、その well-definedness、単射性、全射性は
soundness、completeness、generation という三つの異なる条件が担う。

**Soundness の生成。** `x∈R_rep(V)` と `p∈P_sem(V)` を取る(定理 5.1 の下で、
selected local atlas の restriction により `P_sem(V)` は非空である)。affine
semantic repair system(第X部 定義 4.1)の relation soundness から、semantic 側の
...
である(第X部 補題 6.2A)。soundness を仮定として置かず local-state data から
導出するのは、この3行の議論である。
...
各 `Φ_V` が同型なので、`κ^n` は積上の同型である(第X部 定義 7.1)。
...
`(κ¹)^{-1}` が同じ形で誘導し、`κ_*` は可換群同型である(第X部 定理 7.4)。
...
が成立する(第X部 定理 7.5)。`κ_*` は同型で零を保存し反映するので、
`[r_sem]=0⟺[r_E]=0` と `[r_sem]≠0⟺[r_E]≠0` も従う。

以上、§5.3〜§5.5 で定理 5.1 の主張のうち true sheaf 条項を除くすべてが証明された
(束ねは第X部 定理 7.6)。残る三項同値を次節で証明する。

### 5.6 Global repair: Grounded Global Gluing

**定理 5.2(canonical 第X部 定理 8.2)。** 定理 5.1 の設定に加え、`P_sem` が
true semantic repair sheaf(canonical 定義 8.1)ならば、三項同値
...
同値なので(第X部 系 4.5)、`r_sem=δ⁰_sem a` なる `a∈C⁰_sem(𝒰)` が取れる。
corrected family `p_i^corr=(-a_i)+p_i` の residual は choice independence
(第X部 定理 4.4)により `r_sem-δ⁰a=0` であり、corrected family は全 nonempty
`i<j` overlap 上で一致する。sheaf amalgamation に渡すには、残りの ordered
overlap 上の一致も要る: self-overlap と逆順 overlap 上の equality は cover の
monomorphism から、積から除いた empty overlap 上の equality は empty-overlap
normalization(仮定 8)から従う(第X部 補題 2.1A)。定理 5.1 の仮定のうち
monomorphism と normalization が実際に働くのはこの箇所である。したがって
corrected family は全 ordered overlap 上の matching family であり、`𝒰` が
topology の cover で `P_sem` がその topology 上の sheaf なので、sheaf
amalgamation により一意な `p∈P_sem(W)` が存在して `p|_{U_i}=p_i^corr` を満たす。
...
**Equation 側。** 同じ結論が equation 側にも降りる。SAGA presentation exactness と
primary state correspondence が `S_X` 上で成立し、`P_E` も選ばれた topology 上の
sheaf であるとする。各 chart と nonempty overlap 上では、`Φ_V`-equivariant な
`β_V:P_sem(V)→P_E(V)` が全単射である(第X部 系 6.5: 単射性は semantic torsor の
transitivity、target torsor の freeness、`Φ_V` の単射性から、全射性は target torsor
の transitivity と `Φ_V` の全射性から出る)。この局所全単射性が両側の sheaf condition(amalgamation
と separatedness)を経て global の `β_W:P_sem(W)→P_E(W)` の全単射性へ持ち上がり、
...
`σ_V`、local repair relation を `2σ_V=0` とすると、semantic 側は独立に
...
**Equation 側。** `P_E=𝔽₂` とし、transition を `Φ(t_e)`、`β` を local coordinate
上の `Φ` とする。各 chart で `e_i=0` を選ぶと、これは §5.5 の対応選択
...
実在 architecture に現れた事例であり、例 5.3 はその有限雛形である。

### 5.9 Paper theorem と canonical theorem の対応
...
| 定理 5.2(§5.6) | 系 4.5、補題 2.1A、定理 8.2、系 8.3 | Grounded Global Gluing |
...
| --- | --- | --- | --- |
| Mathematics | 第4〜5章 | canonical math source(第X部)、theorem map(§5.9) | 対応表あり、notation 表 TODO |
