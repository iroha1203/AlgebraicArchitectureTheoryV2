...
are derived (\S\ref{app:C2}). Thus the only thing the repair plan adds
...
``stages'' mentioned by the condition matrix of \S\ref{app:C2} and its
note~1 refer to this correspondence.

\subsection{Conditions and input kinds: the condition matrix}\label{app:C2}
...
\subsection{Reproduction}\label{app:C3}

All input artifacts, primary outputs, and builder scripts are included
in the reproduction bundle. The
...
\code{reproduction/} (the execution commands and the expected outputs);
\code{audit/} (the claim-to-evidence
