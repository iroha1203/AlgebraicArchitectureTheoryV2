...
to the observation process outside AAT, and its reproducible fixing is
described in \S\ref{sec:75}. Inside AAT, Atoms are given generators.
...
effect, and semantic Atoms in mixture. This mixture is not excluded; the
...
\item $\mathrm{role}_E : K_E \to \{\text{required}, \text{optional},
      \text{derived}\}$: the role of each equation;
...
compatibility of $\nu$ it follows that
$\mathrm{res}_j(\IOb(W))\subset \IOb(W')$, so the context-wise ideals
form an ideal subpresheaf.
\end{definition}
...
architecture object, a required equation index $i_q$, and a support Atom
...
\addlinespace
proved &
repair-relation soundness, SAGA presentation exactness, the isomorphy of
$\Phi$, commutation with the differentials, preservation of cocycles /
