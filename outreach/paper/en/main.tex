...
\date{}% TODO: fix at release together with the release identity.
...
% REMOVE AT RELEASE ---------------------------------------------------------
\begin{center}
\fbox{\parbox{0.92\linewidth}{\small\textbf{Draft note.} This file is the
English source of the Zenodo preprint. \textbf{TODO} markers indicate items
to be fixed when the release identity (release tag, CI run, deposit paths,
metadata) is finalized. This note is removed at release.}}
\end{center}
% ---------------------------------------------------------------------------
...
semantic inconsistency may remain that appears only when the system is
traversed as a whole. This paper independently constructs two cohomologies
...
selected finite cover $\cU$ of Algebraic Architecture Theory (AAT) --- the
...
\input{sec01_intro}
\input{sec02_aat}
\input{sec03_foundations}
\input{sec04_two_complexes}
\input{sec05_comparison}
\input{sec06_lean}
\input{sec07_archsig}
\input{sec08_related}
\input{sec09_outlook}
...
Lean, and the probabilistic process of observation supply is fixed as
artifacts by the discipline of \S\ref{sec:75}. The AI agents are not
authors of this paper; the author bears responsibility for all conclusions.
