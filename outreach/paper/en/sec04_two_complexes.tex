...
Section~\ref{sec:5} --- the existence of the map, the fact that it is an
isomorphism, the correspondence of residual classes --- is the conclusion of a theorem,
...
The residual $\rsem$ is a cocycle: on $U_{ijk}$, abbreviating
$r_{ij}=r_{\mathrm{sem},ij}$, we have
$p_k=r_{jk}+p_j=r_{jk}+r_{ij}+p_i$ and $p_k=r_{ik}+p_i$, and freeness of
the action gives $r_{jk}-r_{ik}+r_{ij}=0$.

The class $[\rsem]\in
H^1_{\mathrm{sem}}(\cU):=\check H^1(\cU,\Msem)$ does not depend on the
