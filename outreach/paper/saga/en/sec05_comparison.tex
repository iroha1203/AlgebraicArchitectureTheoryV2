...
obstruction class --- three stages in all. The inputs 1--8 below
(referred to interchangeably as assumptions 1--8) can be read in four
groups: inputs 1--2 are the two coefficients; inputs 3--4
...
\kappa_*([\rsem])=[\rE].
\]
Because the selected local atlases restrict to each nonempty
intersection, the local-state systems used by this construction are
nonempty on each intersection. The correspondence of residuals holds at the
...
residual correspondence, is part of the specific content of the theorem.
That specific content is concentrated in (i), (iii), and the residual
...
The proof is organized in four steps, and
...
\subsection{Cochain commutation and the \texorpdfstring{$H^1$}{H1} isomorphism}\label{sec:54}
...
additivity and naturality of $\Phi$ proved in \S\ref{sec:53} are used;
...
$\kappa^1(c+\delta^0_{\mathrm{sem}}a)=\kappa^1c+\delta^0_E(\kappa^0a)$, and the class
...
is $\rsem-\delta^0_{\mathrm{sem}}a=0$ by choice independence (\S\ref{sec:42}), so the
...
$\check H^1(\cU,-)$ relative to the selected monomorphic AAT cover $\cU$.
...
between the semantic presentation $\Zint[\sigma_V]/(2\sigma_V)$ and the
...
$\delta^1\rsem=0$ hold automatically. On the other hand, for any
$a\in C^0(\cU)$ the components of $\delta^0a$ are the
$a_j|_{U_{ij}}-a_i|_{U_{ij}}$ on the overlaps, and taking the edge sum around the
