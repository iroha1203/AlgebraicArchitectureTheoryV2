...
| Lean 形式化 | `Formal/AG/`、`Formal/AG/AxiomAudit.lean`、release CI |
...
3. **ArchSig: finite case study**

   固定した microservice architecture を対象として、数学的対象が有限 architecture evidence から
...
数学面では、monomorphic AAT cover `𝒰` 上で semantic repair coefficient `M_sem` と
equation-generated coefficient `Q_E` を独立に構成する。二つの affine local-state system、
selected local atlases、empty-overlap normalization、repair-relation completeness、
equation-generator completeness、restriction-natural かつ generator-equivariant な
`β:P_sem→P_E` の下で、comparison map は次の同型と residual class 対応を誘導する。
...
true semantic repair sheaf 条件の下では、global repair を次の三項同値で特徴づける。
...
数学本文、Lean status、ArchSig measurement は同じ release identity を参照し、
各 claim から一次証拠へ到達できる provenance を構成する。
...
### 2.6 one-cent を ArchSig 章で明かす

Introduction と Abstract は、実在する microservice architecture に対する有限 case study と
repair comparison を成果として述べる。case study の金額と one-cent という名称は、
ArchSig の計算方法を説明した後に初めて示す。

one-cent は、SAGA comparison が実コード上の小さな semantic discrepancy へ到達する
payoff として提示する。Conclusion はこの到達を Cape of Good Hope として回収する。
...
| 章 | 仮題 | この章で説明するもの |
| --- | --- | --- |
| 1 | Introduction | local-to-global architecture problem、SAGA の研究課題、数学完成版・Lean status・ArchSig case study の三層 |
...
| 7 | ArchSig: Executable SAGA Diagnosis | ArchSig の有限計算、実コード事例の提示、SAGA comparison、repair 前後の変化、再現方法 |
...
- independently generated circle comparison による非零 residual class の有限 witness
...
- canonical な数学本文
- 第X部 定理 1.1、定理 7.6、定理 8.2、例 10.2
...
axiom 状況の正本は `Formal/AG/AxiomAudit.lean` とする。release CI は同じ commit に対する
focused check と監査結果を固定する。

## 7. ArchSig と one-cent で説明するもの

ArchSig は、one-cent obstruction を SAGA の有限 realization として示す。

章の前半で ArchSig の input、計算、出力を定義する。実コード事例へ進んだ時点で
one-cent を初めて明かし、repair 前後の比較までを一つの計算として示す。
...
- first-order conormal specialization
...
Zenodo metadata は、著者名、affiliation、ORCID、resource type、version、license、
related identifier を固定する。citation guide は concept DOI と version DOI の使い分けを記録する。
...
- [ ] title、中心主張、用語を固定する
- [ ] 中心二式と、その成立条件を canonical math source に合わせて固定する
...
- [ ] `Formal/AG/AxiomAudit.lean` と release CI の結果を固定する
- [ ] 未証明、未接続、未移植を正確に記録する
...
- [ ] one-cent を ArchSig 章で初めて明かし、Conclusion で回収する
- [ ] independently generated circle comparison の非零 class を示す
- [ ] SAGA comparison の可換図と one-cent の計算図を完成させる
...
- [ ] 著者名、affiliation、ORCID、resource type、version、license を固定する
...
- [ ] novelty claim を claim-to-evidence matrix に登録する
...
7. Related Work が完成し、SAGA の比較軸と主要な差異が原典に基づいている。
8. 数学、Lean status、ArchSig result、deposit に対する最終レビューが完了している。
