...
| DOI | **version DOI 予約済み(2026-07-26): `10.5281/zenodo.21603762`**。title page・§6・付録C.4・CITATION.md へ印字済み。concept DOI は publish 時に自動付与されるため bundle 内には書かず「Zenodo record ページ参照」とする **2026-08-21 追記**: 初版 record 21603762 は flat upload で不完全、完全版 v1.0.0 deposit は new version `10.5281/zenodo.21605207`、concept DOI `10.5281/zenodo.21603761`。v1.0.1(arXiv 投稿版・文言修正)は title page に concept DOI、付録 C.4 に version DOI 21605207 を印字 |
...
arXiv abstract は inline `$...$` のみ(標準 LaTeX / AMS マクロ限定、自作マクロ不可)、**上限 1,920 文字**、
`---` は三連ハイフンのまま表示されるので使わない。下記は 1,883 文字(2026-08-21、v1.0.1 投稿時に確定。`\mathcal{U}` は arXiv の abstract プレビューで欠字になるため `U` に簡略化。
v1.0.0 の変種 2,082 文字から、section 参照・末尾文の省略に加えて短縮)。
