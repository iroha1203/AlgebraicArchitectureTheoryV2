...
> cochain compatibility と `H¹` 同型を証明する。さらに restriction-natural かつ
...
| G-07 conormal package | 本節 §4.6.3（第X部の後段で読む first-order specialization） | `lawGeneratedConormalFirstOrderDescent_package` は `ResearchLean` で証明済み。本体への蒸留は Issue #3757 |
...
Tier 1 の各数学的構成に対応する G-07 declaration と、その downstream specialization を記録する。
これは Tier 1 数学本文の入力ではなく、数学本文の完成後に置く論文統合層の対応記録である。

| G-07 declaration | 対応分類 | 数学的役割 |
| --- | --- | --- |
| `PatchReadingSource` | generic counterpart | semantic local input と required-law support の一次 source |
| `OverlapLawCombination` | generic counterpart | pairwise reading difference を required-law generators で表す局所 relation |
| `evaluateCombination_mem_obstructionIdeal` | proof-pattern correspondence | generator relation が equation ideal で零になる soundness |
| `rawQ0_pair_compatible` | specialization | required-law relationから `O/I` compatibility を生成 |
| `LocalLiftData` | downstream realization | base section と explicit local lifts だけを入力にする local-lift problem |
| `localLiftDifferenceFor` | downstream realization | local lifts の差から kernel-valued residual を生成 |
| `localLiftDifferenceFor_cocycle` | downstream theorem | `d¹d⁰=0` と kernel identification による cocycle proof |
| `connectingClass_choice_independent` | downstream theorem | local lift choice の変更が `δ⁰`-像になる証明 |
| `correctedLocalLiftCochain_d_eq_zero` | downstream theorem | zero class から matching correction を構成 |
| `amalgamatedCorrectedLocalLift` | downstream theorem | true sheaf amalgamation による actual global lift |
| `connectingClassFor_isZero_iff_nonempty_globalLift` | downstream theorem | class zero と global lift existence の同値 |
...
次節の first-order conormal specialization として接続できる。

依存方向は一方向に固定する。

```text
Tier 1: 第X部の独立した SAGA 数学
  -> 論文統合層の G-07 対応
  -> G-07 declaration / Lean status
```

G-07 の定義、宣言、proof route、status は、第X部の定義または証明に入れない。

#### 4.6.3 G-07 first-order conormal specialization

この節は Tier 1 数学本文の一部ではなく、完成した第X部と既存G-07成果を接続する
論文統合 statement である。G-07 の short exact sequence
...
を証明する。論文では、これを第X部 定理 8.2 の local correction と
true sheaf amalgamation に対応する first-order conormal specialization として読む。
G-07 単独は第X部 定理 6.3 の semantic presentation comparison や
定理 7.6 の full SAGA comparison を instantiate しない。
...
- [ ] G-07 declaration の downstream correspondence / specialization と一方向依存を固定する
...
3. G-07 の downstream correspondence / specialization と一方向依存が declaration 単位で示されている。
