...
この改訂が数学本文、Lean declaration、Lean status へ反映され、独立レビューを通過するまで、
中心定理を完成済みとして論文本文、Abstract、Zenodo metadata に記載せず、release candidate としない。
...
> **SAGA: A Lean-Formalized Comparison Theorem for Local-to-Global Software Architecture**
>
> *From Semantic Repair Cohomology to Executable Architectural Diagnosis*

最終タイトルは、数学、Lean、ArchSig の三つが揃った研究成果であることを伝える。
...
> semantic-to-geometric comparison map と cochain compatibility を証明する。
> この定理連鎖を Lean で形式化し、
> ArchSig によって実在 software 上の有限診断として実行した。

中心式は次の二つである。

\[
H^1_{\mathrm{sem}}(E,\mathcal U)
\cong
...
二つ目の式は、その statement が要求する faithfulness、sheaf、gluing data とともに提示する。
仮定は定理の数学的入力として statement 内に明示する。
...
2. **Lean formalization**
   ...
   Atom、Law equation、witness ideal、商係数、site、Čech complex、比較写像、
   零類と非零類の witness までを結ぶ機械検証可能な定理連鎖。
...
semantic atom projection

\[
\pi:\Lambda\to K
\]

から residual locus、semantic closure、repair support を定義し、有限 repair-gluing complex

\[
C^0\xrightarrow{\delta^0}C^1
\]

を導入する。

この章は、semantic faithfulness の下で
...
と読めることを中心に置く。component coverage と semantic faithfulness の差は、
形式解を意味のある repair として読むための数学的構造として説明する。
...
3. semantic mismatch から生成される semantic residual
4. semantic data と Atom / equation / cover data から構成される comparison map
5. differential compatibility の証明
6. cocycle と coboundary relation の保存
7. 商上の well-defined map
8. induced maps の inverse
9. `H¹` equivalence
10. residual class と zero-predicate の対応
11. true sheaf descent を実際に用いる grounded global gluing
...
一般 comparison interface は下流の商構成に用いる。標準 AAT route では、comparison map 自体を
semantic data、equation-generated quotient、actual cover nerve、incidence data から構成し、
comparison interface の各性質を theorem として証明する。
...
law semantics の寄与は、degree-zero cochain の vanishing として独立に説明する。
cover geometry が非自明な `H¹` class を生むため、local lawfulness と global coherence を
別の invariant として測定できる。
...
```text
SemanticAtomProjection
  -> finite semantic repair gluing complex
  -> additive H¹
  -> cover-relative Čech complex
  -> cochain realization
  -> H¹ comparison
  -> grounded global gluing
  -> generated SAGA packet
```
...
- [ ] Issue #3757 の target statement を実装前に固定する
...
- [ ] 数学本文、Lean declaration、Lean status を同期する
- [ ] Issue #3757 を close する
...
Zenodo 向け SAGA 論文は、次の条件がすべて満たされた時点で release candidate となる。

1. Issue #3757 が完了し、semantic repair complex と Čech complex が独立に生成され、
   comparison map、cochain compatibility、residual 対応、商上の `H¹` 同値が証明されている。
2. 論文が数学、Lean、ArchSig の三つを一つの主張系列として読める。
3. 再構成後の SAGA comparison theorem が論文の数学的中心に置かれている。
4. 各主要 theorem が数学 source、Lean declaration、status へ対応している。
5. circle instance が非零 class に対する comparison の具体的 witness を与える。
6. 一セントが ArchSig case study で初めて提示され、喜望峰として結実する。
7. Future Work が Atom foundations、architecture schemes、SFT を Rising Sea として接続する。
8. 全 claim が固定された一次証拠へ到達できる。
9. paper PDF、source、Lean evidence、ArchSig evidence、reproduction bundle が同じ release
   identity を参照する。
10. clean checkout で再現確認が完了している。
11. 数学・Lean、tooling、docs、artifact の最終レビューが完了している。
