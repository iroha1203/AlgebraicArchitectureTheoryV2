...
2026-09-22、本文とLean sourceの固定版 `719f81f47d410701fd82c2bc88613cc140c59377` について、
第1〜8章の番号付き314項目と番号外の主張を[全件照合記録](lean-correspondence-audit.md)へ整理した。
確認できた定義・結果とその適用条件は[付録A](ja/15-appendix-a-lean-correspondence.md)に記す。
未確認箇所は同記録の1件の統合Issueで追跡する。

章別記録の「証明」「検算」「照合」という記載だけでは、
本文と同じ入力・量化・仮定・結論のLean形式化が確認済みとは判断しない。
形式化との対応を調べる際は、上記の全件照合記録と各宣言の型・証明本体を参照する。
今回、Lean sourceの変更・再実行とカーネル公理監査は行っていない。
