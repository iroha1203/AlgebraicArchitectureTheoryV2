...
[準備と記法](ja/03-preliminaries-and-notation.md)と、そこで引用した[文献](ja/14-references.md)の
下書きを作成している。
執筆・照合・PR レビューの完了は ToDo で管理する。

[文献確認記録](references.csv)の hash は、Markdown 原稿の段階では、
`bib_sha256` に `ja/14-references.md`、`manuscript_sha256` に各行で確認した
`ja/03-preliminaries-and-notation.md` の SHA-256 を記録する。
