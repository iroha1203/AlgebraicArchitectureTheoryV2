...
| 第IV部・第8章 | 表示・局所再構成・有限決定性 | 局所表示からの対象・射・比較・変更分類の回復と有限決定性 |
...
章末の「障害の読み取りと診断比較」では、前章の障害係数と本章の診断係数について、
係数・被覆の比較から両者の群を結ぶ写像を構成し、指定障害類の対応と図式の可換性を示す。
採用する入力で零性の保存・反映が得られる範囲を、診断比較による障害判定として述べる。
...
比較対象の自己同型群として同定する。全端点変更と底を固定する変更を扱い、
...
「比較と変更分類の再構成」節では、構成した読み取り関手の充満忠実性から、
元の比較保存群と表示側の比較保存群を同定する。正規化との整合を示した入力族について、
誘導準同型・核・持ち上げの fiber を回復する。指定した section の回復は、
元の構成と局所側の構成の一致を示して述べる。

続く有限決定性では、一般射の有限 table による回復と、前章で分類する可逆変更族に対する
