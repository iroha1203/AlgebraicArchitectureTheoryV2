...
本書は、AAT の基礎論文の主題、収録方針、章構成、各章の役割、執筆順序を定める。
この論文の数学内容の棚卸し、章別設計、原稿は本書に従う。構成を変更するときは、
本書へ反映してから後続文書へ適用する。

[n1012 の構想メモ](../../../docs/note/n1012_aat_unified_theory_foundations_paper_plan.md)
を初期構成の参考にした。n1012 を含む `docs/note/` の文書は検討の経緯と着想を伝える
参考資料であり、本論文の構成・収録範囲を拘束しない。

数学の定義と証明は [AAT 数学本文](../../../docs/aat/algebraic_geometric_theory/README.md)、
形式化の内容は Lean source、固定した研究課題は各 GOAL、研究の到達状態は report と
tracking Issue に照合する。本書は論文の編集方針を扱い、それらの定理や完了条件を改訂しない。
...
この表は収録する問いと結果の役割を定める。各定理の量化、仮定、結論、証明状態は、
次に作成する数学内容の棚卸しで一次資料と照合する。一般命題の証明、AAT の入力から
その仮定を満たす構成、CS の問題への対応は、それぞれの数学的仕事として記載する。

現行数学本文の全10部と付録を、棚卸しの対象に含める。Law algebra、obstruction ideal、
lawful locus など、基礎幾何を成立させる構成を第1〜2章で扱う。
derived geometry、特異性、monodromy、表現、測定、時間方向の構造についても所在と役割を
確認し、本論に必要な内容、補足、発展課題への配置を本書で決める。

後続の descent・H¹・Morita 形・幾何学的表現可能性は、当初の展望に置く。
これは第2章で用いる局所貼り合わせや Čech 障害、第4章の合成の障害を含めた
既存の基礎構成とは、対象とする問題を区別して記述する。

## 4. 全体構成

初期構成は四部八章とする。以下の章番号は理論本体の番号であり、原稿全体の通し番号は
組版時に整える。棚卸しで分量と依存関係を確認し、章の分割・統合が必要な場合は本書を更新する。
...
局所片の有限性、局所片の添字全体の有限性、有限個の読み取りで決まることを区別する。
lens・プロトコルとタグ変更族を、それぞれの意味と許容射を保持した適用として扱う。
第7章の変更の区別と分類を表示側で回復する結果を、前章への接続として配置する。
一般再構成原理と、AAT のデータからその仮定を満たす構成を、別々に証明する。

### 関連研究・結論・付録

Related Work の比較は、主定理の選定と並行して行う。主結果ごとに近接する原典を確認し、
...
| 役割 | 使い方 |
| --- | --- |
| 体系を通して読む共通主例 | 局所整合性、reading の変更、移送、比較、追随変更の問いを順に具体化する |
| 仮定の実質を示す有限例・反例 | どの仮定がどの結論に必要かを、実際の対象・写像・障害で示す |
| 異なる問題への共通定理の適用 | 再利用した証明、共通に導いた成立条件、問題間で移せる帰結を特定する |

共通主例の初期候補は、注文・請求・決済・台帳を持つ小さなシステムとする。
各章で使う入力、Law、許容変更、追加構造を揃え、章をまたぐ対応を確認して採用する。
第I部の整合性、第II部の解像度と輸送、第III部の二経路、第IV部の変更と表示に、
それぞれ具体的な問いと帰結を置く。

共通定理の適用には、モデル同期とプロトコルの追随変更を初期対象として置く。
モデル同期では get と put の意味、プロトコルでは操作の実行と adapter の意味を先に定める。
一般の意味保存射と可逆変更族を分け、それぞれに適用する定理と必要な条件を特定する。
これらの意味論から完全幾何や生成比較へ進む箇所は、具体的な写像と対応の証明を揃える。

## 7. 数学内容の棚卸しへ渡す事項

次の作業として、この構成に沿う数学内容の棚卸しを同じディレクトリに作成する。
章ごとに、必要な定義と構成、主要定理、証明の依存、反例、CS への帰結、
一次資料との対応を整理する。

棚卸しでは、各項目について次を確認する。

1. **数学的内容**: 対象、量化、仮定、結論、証明の要点。
2. **体系内の役割**: 使用する既出の構成、後続に渡す内容、共通化できる定義。
3. **証拠と到達状態**: 数学本文の箇所、Lean 宣言と版、受理済み成果、残る接続や証明。
4. **CS への帰結**: 独立した意味論、入力構成、対応命題、具体例。
5. **収録判断**: 本文、付録、発展課題への配置と、執筆までに必要な作業。

AAT 数学本文、SAGA、G-101〜G-122、G-123 の受理済み成果、G-124 の研究内容を
全章の素材として照合する。G-123 の個別成果の継承と旧固定 target 全体の帰趨を区別し、
G-124 の進行中の証明義務は、その時点の状態を明記する。
研究課題の進展は、関係する章の収録判断へ反映する。

## 8. 執筆の進め方

構成マスターの作成後、全章の数学内容を棚卸しする。
そこから各章の主要定義・定理・証明の構成を具体化し、CS の入力と対応命題、
近接研究の確認を並行して進める。依存する基本構成を揃えてから本文を展開し、
付録の証拠対応と再現資料を整える。

執筆順は、定義と証明の依存および素材の準備状況から決める。
本論全体の主題と各部の役割を保ち、研究の進行順とは独立に編集する。

文章は、対象と構成を肯定形で導入し、必要な仮定の下で得た結果を明確に述べる。
用語と記号を統一し、研究の管理番号・実行記録は証拠対応へ置く。
