...
| 第III部・第6章 | 冪等正規化と実現 | 実生成比較の正規化分解・可逆性分類と冪等像での実現 |
| 第IV部・第7章 | 比較を保つ変更と情報 | 正規化後の情報による識別条件、適合する持ち上げの構成と全選択肢の分類 |
| 第IV部・第8章 | 表示と局所再構成 | 共通の入力宣言と局所整合条件からの対象・構造保存射の再構成 |
...
**第8章**は、独立に定めた実現と局所モデルの圏同値を主要定理に据える。
対象の表示可能性と射の表示可能性を区別し、次の順に展開する。

1. 指定した入力族の有限表示・Karoubi 再構成と、有限構文による表示が成立する条件・反例。
2. 一つのパラメータ宣言、構造条件、原始読み取りからの実現圏と局所モデル圏の構成。
3. 読み取りの分離、整合する局所対象・局所射の組立て、Hom の両逆と対象の同型による圏同値。
...
タグ付き operation、固定した完全幾何の生成比較、lens、プロトコルの四族を同じ宣言の下に収める。
タグ族では canonical 正規化と全 source-choice 変更、生成比較の族では元の比較・正規化・
端点自己同型を保持する。lens とプロトコルでは、独立に定めた意味保存射の全範囲に
同じ局所再構成を適用する。lens の基準 fiber 上の写像、プロトコルの生成辺と観測を保つ
頂点写像の既存の再構成は、その内容を説明する具体例として置く。

各族の有限 table による回復やタグの全有限片からの回復は、例・系・補足に配置する。
それぞれに読み取り方式と証明を対応させ、局所片の有限性、添字全体の有限性、有限決定性を区別する。
...
reading 変更の可換平方と類の輸送を対応づける。付録Bには生成子・関係式、被覆・nerve、
零・非零例の計算を、付録Cにはこれらの証明ソースの固定版と検証記録を置く。
...
flowchart TD
  ...
  C5 -->|二つの経路と実生成比較| C6["第6章 冪等正規化・実現"]
