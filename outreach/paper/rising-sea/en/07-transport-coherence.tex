...
In this section we write $r:E\to B$ for the core projection $q$ of \eqref{eq:4.1} or for
the projection $qp$ from geometries to the extraction base. The fiber $E_b$ is the
category consisting of the objects satisfying $r(X)=b$ and the vertical morphisms
between them.
For the lift $\eta_{\sigma,X}:X\to T_\sigma X$ along each exact morphism
$\sigma:b\to b'$ we use the canonical construction, Construction~\ref{cons:4.3} for cores
and Theorem~\ref{thm:4.11} for geometries.
By Theorems~\ref{thm:4.5} and~\ref{thm:4.11} and Lemma~\ref{lem:4.12}, these are strongly
opcartesian with respect to $r$.
...
In \S\S\ref{sec:4.5}--\ref{sec:4.7} we use the category of cores of \eqref{eq:4.1} and the
projection $r=q:E_{\mathrm{core}}\to B$.
We fix the vocabulary of Atoms and place cores and strong lifts of edges on a finite
comparison diagram.
What is finite here are the indices of vertices, edges, and faces; no finiteness is
imposed on the objects of the cores or on the totality of operations.
The input that treats geometries and cores simultaneously is defined in \S\ref{sec:4.9}.

\begin{definition}[Finite comparison data]\label{def:4.17}

On each vertex $i$ of a finite directed graph we place a core $X_i$, and on each edge
...
Here we compute the comparison equations in the permutation group $S_3$ on three points.
...
In a comparison diagram of cores as in Definition~\ref{def:4.17}, the selection at each
...
This yields the automorphism and \eqref{eq:4.29}.
...
to the same morphism, hence are equal. Multiply \eqref{eq:4.31} by their common inverse
on the right.
...
\eqref{eq:4.31} is required.
Condition~\eqref{eq:4.31} is a relation among the specified comparisons; it does not
...
\end{equation*}
In particular, the left-hand side being the identity is equivalent to \eqref{eq:4.31}.
...
Equation~\eqref{eq:4.32} shows that, once the edge selection is fixed, a mismatch of the
...
Failure of \eqref{eq:4.31} therefore obstructs overall coherence for that edge selection.
...
The following are computations in the same permutation group as Example~\ref{ex:4.20}.
...
The mismatch of the two specified routes is
\begin{equation*}
(st)^{-1}ts=t^{-1}s^{-1}ts\ne1
...
On each vertex of a finite comparison diagram we place a geometry $G_i$.
We assume that each edge $L_e$ of geometries is strongly opcartesian with respect to $p$
and that its projection $p(L_e)$ is strongly opcartesian with respect to $q$. The two
paths of each face map to the same morphism in the extraction base.
By Lemma~\ref{lem:4.12}, $L_e$ is strongly opcartesian with respect to $qp$ as well.
The canonical transport of \S\ref{sec:4.4} satisfies these two-level conditions.
...
From \eqref{eq:4.37} and the uniqueness of canonical comparisons we obtain
$\bar\phi_f(a)=p(u_f)$, which is \eqref{eq:4.38}.
Hence $p(u_fm_f^{-1})=1$.
Equation~\eqref{eq:4.39} is checked by cancelling the middle $m_f^{-1}m_f$.
...
section can be chosen for which \eqref{eq:4.40} can be satisfied.
...
Equation~\eqref{eq:4.40} states that the canonical comparison of the geometries under
$h\widetilde a$ equals the specified comparison.
It is therefore the required coherent selection.
...
$c=h\widetilde a$ gives \eqref{eq:4.40}.
...
We illustrate the lifting condition by the following equations in finite groups.
Take the group $S_4$ permuting the four axes $1,2,3,4$, and from the permutations
preserving $\{1,2\}$ and a two-valued extra component form
\begin{equation*}
\begin{aligned}
...
The kernel is $\{1\}\times C_2$, a degree of freedom that does not change the image in
$S_4$.
...
The coherence conditions in $S_4$ are $a_e^2=b$ and $a_g=1$, and
$a_e=(1324)$, $a_g=1$ is a solution.

On the other hand, the permutation component of $K$ preserves $\{1,2\}$ and $\{3,4\}$
separately.
The permutation part is therefore $S_2\times S_2$, in which every element squares to the
identity.
No selection $c_e$ in $K$ can satisfy $c_e^2=(b,0)$.
The square root $(1324)$ in $S_4$ does not preserve the two selected axes.

The second face is coherent with the kernel selection $c_g=(1,1)$.
Thus the conditions projected to $S_4$ and the conditions belonging to the kernel each
have solutions.
Still, no overall solution exists.
The obstruction is that the solution in $S_4$ cannot be lifted to an element of $K$.
...
Fix a common view set $V$ and a reference view $v_0\in V$.
A relative morphism between lenses $L=(C,V,g,p)$ and $L'=(C',V,g',p')$ that satisfy the
three laws and have finite reference fibers is a pair of maps
$h:C\to C'$ and $u:V\to V$ satisfying
\begin{equation*}
g'h=ug,\qquad
h\,p=p'(h\times u)
\tag{4.43}\label{eq:4.43}
\end{equation*}
The state sets $C,C'$ may differ.

\end{definition}

\begin{proposition}[Typewise maps and the two commutative squares]\label{prop:4.37}

Assigning to the four roles state, view, read, and write of Definition~\ref{def:1.42} the
respective maps
\begin{equation*}
h:C\to C',\quad u:V\to V,\quad
h:C\to C',\quad
h\times u:C\times V\to C'\times V
\tag{4.44}\label{eq:4.44}
\end{equation*}
we obtain, for the named operations get and put, the commutative squares
\begin{equation*}
u\,g=g'\,h,\qquad
h\,p=p'(h\times u)
...
The maps for the four roles are the state map, the view map, the state map, and the
product of the two maps, respectively. The equations for get and put are the two
conditions of Definition~\ref{def:4.36}.

\end{proof}

The same $h$ is used for the input of reads, for the state component of the input of
updates, and for the output of updates.

\begin{proposition}[Connection with fixed-view morphisms and invertible changes]\label{prop:4.38}

The relative morphisms with $u=\mathrm{id}_V$ correspond bijectively to the fixed-view
morphisms of Chapter~\ref{chap:1}.
Moreover, when $h,u$ are each given as bijections, the two preservation equations
become the conditions for a lens change by bijections of states and views.
The operation that extracts from a relative morphism a change equipped with these
bijections and the operation that returns the two maps of a change to a relative morphism
...
Substituting $u=\mathrm{id}_V$ into \eqref{eq:4.43} gives the conditions for a fixed-view
morphism preserving reads and updates.
Conversely, attaching the identity view map to such a morphism gives a relative morphism.
Both operations keep the same state map, so they are mutually inverse.
In the case of given bijections as well, extracting the two maps with the same
preservation equations and returning them yields the two-sided inverse.

\end{proof}

\begin{example}[A semantics-preserving morphism that merges information]\label{ex:4.39}

Take the product lens with the one-element view set $V=\{*\}$ and $K=\{0,1\}$.
Let the reference view be $*$, and define an endomorphism by
\begin{equation*}
h(*,k)=(*,0),\qquad u=\mathrm{id}_V
\tag{4.46}\label{eq:4.46}
\end{equation*}
The read is always $*$, and an update to the view does not change the state, so this map
satisfies \eqref{eq:4.43}.
It sends the two states of the reference fiber to the same state, so the map on the
reference fiber is not injective.
...
Condition~\eqref{eq:4.47} is the condition of Proposition~\ref{prop:1.38} and gives a
unique semantics-preserving morphism.
Reading the preservation condition for adapters in \eqref{eq:1.23} componentwise as a
natural transformation gives \eqref{eq:4.48}.
...
\eqref{eq:4.48} also holds for the composed change.
...
In this application the view and the protocol schema are fixed.
The relative lens morphisms of Definition~\ref{def:4.36} handle the two
operation-preservation equations over a common view.

To connect the core defects of this chapter with changes of operations, we again supply
the input of Definition~\ref{def:4.17}.
We place a core on each vertex and, on each edge, a strong lift with respect to the
projection to the extraction base, and we fix two paths over the same base together with
specified comparisons.
When geometries are treated as well, we use the two-level universality and the edge
sections of \S\ref{sec:4.9}.
...
composition of the canonical transport. Triple composites and the projection to the core
are coherent as well (\S\ref{sec:4.4}).
\item \textbf{Obstructions for specified comparisons.} We expressed the difference from
the canonical comparison as a noncommutative automorphism and showed, for finite
comparison diagrams of cores, that vanishing of the obstruction is equivalent to an edge
...
\item \textbf{Comparisons preserving the meaning of operations.} For lenses over a common
view, the same state map is used for reads and updates.
For protocols, all named operations and adapters are preserved simultaneously.
For fixed-view lenses and fixed-schema protocols, the invertible changes preserving a
comparison can be recovered across the constructed typed categories
(\S\S\ref{sec:4.10}--\ref{sec:4.12}).
\end{itemize}
...
Given the core input of Definition~\ref{def:4.17}, Theorem~\ref{thm:4.23} characterizes
the existence of an edge selection realizing the specified comparisons on all faces as
the vanishing of the obstruction.
