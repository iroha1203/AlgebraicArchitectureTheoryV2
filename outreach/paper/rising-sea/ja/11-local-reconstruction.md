...
本章では、AATの完全幾何を構成する局所データから、
全体の構造と構造保存射を再構成する条件を明らかにする。
第7章で扱った構造を保つ変更について、その各成分の記述から一つの射を構成する問題を扱う。

例として、行内送金システムの刷新を、口座・送金・会計の各担当が分担する場合を考える。
口座担当は残高と引落し・入金操作を、送金担当は依頼と処理状態を、会計担当は仕訳と記帳操作を記述する。
同じ取引について、これらの記述が共通の識別子・金額・完了条件と整合することが必要である。
本例では、同一通貨・手数料なしの送金を対象とし、
完了した送金には同額の出金・入金と対応する仕訳が存在することをLawとして要求する。

刷新前後の対応も、各担当の記述だけでは全体の射を定めない。
同じ取引を新設計のどの取引へ対応づけるかが一致し、
さらに操作の作用とLawの評価が保存される必要がある。
この例を通じて、分担された設計記述から全体のモデルを構成する条件と、
...
ここで局所的な記述とは、有限個の型・値・等式の読み取りを指す。
再構成には、対象と射を決定するだけの読み取り項目と、それらの整合条件を指定する必要がある。
主定理の後では、設計変更への適用と、操作の法則によって有限の記述が十分となる場合を扱う。
有限なモデルについても、必要な情報の選択と、検査・再構成の計算費用を区別する。

| 課題 | 用いる構成 | 得られる結果 |
| --- | --- | --- |
| 対象と射の有限表示 | 有限例外codeと固定端点間の射 | 有限表示の成立条件と限界 |
| 有限片の貼り合わせ | 制限と型付き関数graph | 表の貼り合わせと、関数としての成立条件 |
| 局所再構成の十分条件 | 分離・射の組立て・対象の組立て | 読み取り関手による圏同値 |
| AATの原始データからの再構成 | operation・Law・幾何・実現の原始データ | 対象と全構造保存射の再構成 |
| 設計変更への適用 | 行内送金の局所記述と対応 | 分担された記述からのモデルと変更の構成 |
| 有限表による再構成 | 操作の法則による一意な延長 | 有限の記述から全体の射を回復する条件 |
| 有限なモデルでの検査 | 読み取り項目の選択、列挙と等号判定 | 再構成に十分な情報と計算手順の区別 |

第3章の診断保存は、選んだ診断の値が変わらないことを保証する。
第7章の反映は、変更後に成立する比較適合性から変更前の適合性を導く性質である。
...
次に、第5章の有限source、正規化表、有限例外表を持つcode $`P,Q`$ を固定する。
その意味を $`D_0P,D_0Q`$ と書く。
...
同じ意味的射を与える置換の記述は同一視し、この商による圏のdecoderを $`D_0`$ とする。
...
**証明.**
式(8.4)は評価を変えない。
有限集合上の置換は有限supportを持ち、既定値は両端とも0だから、
...
両側の自然性と三角恒等式は、$`N`$ の像で逆射を消去し、分離を適用することで従う。□
...
標的のraw systemが、係数とcontextを輸送したsourceのraw systemに等しいことを要求する。
Support・Axis・Observableの比較写像は、そこで選んだrestrictionについて自然である。

**明示写像による方式**では、同じ底・core・被覆・overlap・係数の成分を持ち、
raw systemの等号を、次の具体的な対応に置き換える。
輸送後の各contextで、raw座標、局所データ、関係の添字を全単射で対応させる。
この対応は係数写像を用いた多項式の評価、関係式、restrictionを保つ。
さらに各context射の行先を指定し、restrictionであるという性質を保つ。
各contextのSupport・Axis・Observableにも全単射を指定し、
読み取りの述語と、これらのcontext射に沿う自然性を要求する。

どちらの方式でも、底やarchitecture対象の写像、係数写像まで可逆とは限らない。
恒等は各成分の恒等であり、合成は対応する成分の合成である。
前者は選んだ表示の等号を使う輸送に、後者は実際の対応表を保持する変更に適している。

### 原始データと局所整合条件

**定義8.15（完全幾何の局所対象と局所射）.**
$`\Theta`$ を固定する。
対象の読み取り項目は、次の表の各データを、その値や関数graphの一点として読むものとする。
関数の定義域・値域となる集合は、対応する役割の型参照で指定する。

| 読む層 | 対象から読む原始データ | 射から読む原始データ |
| --- | --- | --- |
| 底とAtom | sourceの型、指定点、正規化graph、抽出述語 | source写像とAtomの全単射のgraph |
| architecture | 有限Atom族の合成関係、対象形成、operationの型・端点・Atomへの作用 | 対象写像とoperation写像のgraph |
| Lawと不変量 | Lawの添字・両辺・評価、検出子、不変量の型と値、signature | 添字対応、評価と作用の保存、signatureの対応 |
| contextとobservable | contextの型と関係、restriction、observableの環演算と読取り | context対応、observableの環同型とrestrictionの保存 |
| 選択された幾何 | 被覆要件、overlap、係数環、raw座標、関係式、restrictionの多項式 | 被覆とoverlapの保存、係数写像、方式に応じたrawの等号または対応graph |
| 実現 | Support・Axis・Observableの型、読取り述語、context射の作用 | 各実現成分の比較と自然性 |

たとえば、operation $`u:A\to B`$ の作用の一項目は、
そのconfiguration射がAtom $`a`$ をAtom $`b`$ へ送るという命題の真偽を表す。
係数環の乗法の一項目は、等式 $`a\cdot b=c`$ の真偽を表す。
rawのrestrictionは有限の多項式として記し、その変数と係数の型を対応づける。
局所値にはこれらの原始成分を指定し、幾何や射の全体はその組立てによって構成する。

射の読み取りでは、source対象、target対象、射自身の項目を区別する。
その共通の添字集合は
...
対象の場合には対象の項目だけを用いる。
射のsource・target部分には、指定された両端の局所対象と一致することを要求する。

**局所対象**は、対象の有限片の整合族で、次の条件を満たすものとする。

- 役割ごとに宣言された型が一致し、各関数graphが定義8.8を満たす。
- 第1章の完全幾何を構成する各構造の法則が、原始評価の式として成立する。
- coreで選ぶAtom族が有限である。

**局所射**は、射の有限片の整合族で、型と両端の一致、関数graphの全体性・一意性、
および表に挙げた保存条件を満たすものとする。
全単射が必要な成分には、逆向きのgraphと二つの逆律を指定する。
環準同型には0・1・加法・乗法の保存を課し、
Lawには添字の対応と両辺の評価の保存を課す。
自然性は、指定された各restrictionまたはcontext射についての成分等式である。
...
と書ける。restrictionとobservableの自然性は、
制限と成分写像をいずれの順序で適用しても評価値が等しくなることを要求する。
各式を固定した入力で展開すると有限supportを持つ。
式をすべての入力で要求することで、構造の保存条件全体を得る。

不変量の値の全単射が、射の成分ではなく存在条件として現れる場合には、
局所表示に補助の往復graphを付ける。
これらにも型・逆律・評価保存・有限片の制限との整合性を課す。
その後、保持すべき射の全成分が等しい表示同士を同一視する。
この商では補助graphの選択を同一視し、source・operation・context・係数・rawなどの成分は保持する。
同一視は、整合する表示を構成した後に行う。
...
**証明.**
局所射の恒等は対角graph、合成は式(8.9)で成分ごとに定める。
各graphは再び全体的かつ一意である。
二つの保存等式から合成の保存等式が従い、
逆方向のgraphは逆順に合成することで逆律を保つ。
被覆やrestrictionで要求した含意も、二つの含意の連鎖で保たれる。
rawの式では係数写像と変数の代入を順に合成する。

不変量の補助表示についても往復の対応を合成できる。
保持成分が等しい表示からは保持成分の等しい合成を得るので、
合成は同一視の後にも定まる。
単位則と結合則は、graphの合成および成分ごとの合成から従う。

実現の各成分を読み取れば、型と法則はその実現の定義から成立する。
関数graphの読み取りは恒等・合成を保つので、これらの読み取りは関手となる。□
...
また任意の実現 $`G,H`$ の間で、局所射 $`N_\Theta G\to N_\Theta H`$ と
完全幾何の射 $`G\to H`$ の読み取り・組立ては互いに逆である。
...
各graphから補題8.9によって正規化やoperationのAtomへの作用を構成し、
...
次にLawの添字と評価、不変量、signatureを構成する。
原始条件が要求する等式を、組み立てた関数の値として読み直せば、
Lawと各保存条件が得られる。
contextとobservableも同様に構成し、環の公理とrestrictionの条件を適用する。
以上によりcoreが定まる。

このcoreの上で、被覆要件とoverlapを取り、選択されたsiteを構成する。
係数環の型と演算は環の公理を満たす。
各contextのraw座標、局所データ、関係の添字、およびrestrictionの多項式を組み立てる。
原始条件には、関係式の保存と、restrictionの恒等・合成の等式が含まれるので、
これらはraw systemとなる。
最後にSupport・Axis・Observableとその読取り・context射の作用を合わせ、
完全幾何を得る。
この順序により、後のデータが参照する型は、先に構成したデータによって定まっている。

射についても、まず保持する各成分のgraphを関数へ組み立てる。
...
明示写像による方式では、rawの対応と実際のcontext射に沿う自然性を得る。
不変量の補助graphは、要求される値の全単射の存在を与える。
補助の選び方を変えても、組み立てた射の保持成分は変わらない。

この構成の各関数のgraphは、補題8.9によって元の表と一致する。
有限片の一致も補題8.7から従う。
逆に、完全幾何の射の読み取りを組み立てると、各関数の値が元の射の値と一致する。
...
定義8.15の局所モデルへの原始読み取り関手 $`N_\Theta`$ は圏同値を与える。
...
{\overset{\mathrm{read}_\Theta}{\rightleftarrows}}\
\mathrm{Hom}_{ℳ_\Theta}(N_\Theta X,N_\Theta Y)
\qquad\text{(8.17)}
```

は全単射であり、読み取りと組立ては両方向に恒等となる。

**証明.**
補題8.17により、どちらの射の方式でも、
原始読み取りは射を分離し、すべての局所射と局所対象を組み立てられる。
従って定理8.12の三条件が満たされ、$`N_\Theta`$ を前向きの関手とする圏同値を得る。□
...
**証明.**
式(8.18)の両辺を読めば、読み取りの関手性と式(8.17)によって等しくなる。
分離から元の射も等しい。
評価の一致は、graphの組立てで選んだ出力の定義による。
...
contextとその制限、選択した幾何および実現のデータを、定義8.15に従って指定する。
同じ読み取り項目が複数の記述に現れる場合、その値は一致しなければならない。
...
| 読み取り項目 | 旧設計の取引 | 不整合な対応候補 | このLawを満たす対応候補 |
| --- | --- | --- | --- |
| 完了判定 $`c`$ | 1 | 1 | 1 |
| 出金記録 $`d`$ | 1 | 0 | 1 |
| 入金記録 $`i`$ | 1 | 0 | 1 |
...
必要なデータを揃えて局所対象を定めたとき、その実現は読み取りとの同型を伴って得られる。
...
定理8.18は、これを読み取りとする一意な構造保存射
$`h:G_{\mathrm{old}}\to G_{\mathrm{new}}`$ を与える。
ここで構成される対象と射は、型・操作・Law・幾何・実現のデータを明示したAATモデルのものである。

![Reconstructing an AAT model and a structure-preserving change from local descriptions](../figures/ch08-local-reconstruction.svg)
...
を満たす。従って $`d\overline\beta=\overline\beta=\overline\beta e`$ である。
...
具体的には、$`\mathrm{Ad}(P_z)`$ を満たすcoreの上で、
二つの面が同じ両端を持ち、両経路の作用が恒等である比較図式を取る。
第一の面は恒等、第二の面は三つのsignature軸のうち二軸を交換する入力を取れる。
係数を $`ℤ`$ とし、構成5.37〜5.40を第二の面に適用する。
初期defectはその二軸の交換だから恒等ではなく、正規化の選択条件が成立する。
定理6.16により、非自明な冪等正規化と非可逆な $`\overline\beta`$ を得る。
同じ入力で恒等のcochainを選ぶ場合には正規化が恒等となり、
$`\overline\beta=\overline\alpha`$ となる。
どちらの場合も、再構成は、選ばれた比較とその両端の全許容射に適用される。
...
| --- | --- | --- |
| Lens | viewの集合 $`V`$ と基準値 $`v_0\in V`$ | 三法則と基準fiberの有限性を満たすLens、およびget・putを保つ全写像 |
| プロトコル | 有限表示 $`Q`$ と、商経路圏上の観測関手 $`O`$ | 各頂点の状態集合が有限な実現、および観測を保つ全自然変換 |
...
| --- | --- | --- |
| Lens | 状態の型、get・各putのgraph | 状態写像のgraph |
| プロトコル | 各頂点の状態の型、生成辺と観測のgraph | 頂点ごとの写像graph |

型はその役割ごとの参照で指定し、関数graphの各値を読み取り項目とする。
射の項目ではsource・target・射自身を区別し、§8.2の有限片と制限を用いる。
局所対象は、型の一致と各graphの全体性・一意性を満たす有限片の整合族とする。
Lensには三法則と基準fiberの有限性を、プロトコルには道の関係、観測の自然性、
各頂点の状態集合の有限性を原始評価の条件として課す。
有限性は、該当する集合の全要素を覆う有限リストの存在によって表す。

局所射には、指定した両端との一致とgraphの全体性・一意性を要求する。
Lensではget・putの保存を、プロトコルでは生成辺と観測の保存を、成分ごとの等式で課す。
操作と観測の保存等式は、入力を固定すると有限supportを持つ。
対角graphと式(8.9)の合成はこれらの条件を保つため、局所圏 $`ℳ_\Theta`$ を得る。
実現の各成分を読む関手を $`N_\Theta:ℛ_\Theta\toℳ_\Theta`$ と書く。

**補題8.23（Lensの組立てと分離）.**
Lensの局所対象と局所射から、Lensとその一般射を組み立てられる。
読み取りと組立ては各Homで互いに逆であり、対象も同型まで回復する。

**証明.**
局所対象から状態集合 $`C`$ を取り、
getのgraphと各view $`v`$ におけるputのgraphを組み立てる。
...
である。従って $`(C,g,p)`$ は全域Lensになる。
局所対象には $`g^{-1}(v_0)`$ を覆う有限リストが存在するという条件もあるので、
その基準fiberは有限である。

局所射のgraphを $`h:C\to C'`$ へ組み立てる。
射の二つの局所条件から
...
を得るため、$`h`$ はLensの射である。
補題8.9と補題8.7により、すべてのgraphと有限片について両逆が成立する。□
...
生成辺 $`e:v\to w`$ のgraphを $`X_e:X_v\to X_w`$ へ、
観測graphを $`o_v:X_v\to O(v)`$ へ組み立てる。

道 $`e_n\cdots e_1`$ の作用を $`X_{e_n}\cdots X_{e_1}`$ とし、
空の道には恒等を対応させる。
局所対象は指定された関係 $`p=q`$ について作用の等号を満たす。
その等号は前後への道の合成で保たれるので、道の作用は商経路圏へ降りる。
また、各生成辺での局所条件は
...
を与える。道の長さに関する帰納法から、$`o:X\Rightarrow O`$ は自然変換になる。
...
である。第一式を生成辺から道へ延ばすと $`h:X\Rightarrow Y`$ の自然性を得る。
第二式は観測保存である。
従って、これらの頂点写像は実現の射を定める。
すべての成分はgraphから一意に得られるので、読み取りと組立ての両逆も従う。□

**系8.25（CS意味論の局所再構成）.**
構成8.22の各入力に対し、原始読み取り関手 $`N_\Theta`$ は
圏同値 $`ℛ_\Theta\simeqℳ_\Theta`$ を与える。
対象は同型まで回復し、固定した任意の実現間では、
非可逆な射も含めて読み取りと組立てが互いに逆となる。
この再構成は恒等・合成と原始評価を保ち、同型を反映する。

**証明.**
補題8.23と補題8.24は、それぞれ定理8.12の三条件を与える。
従って原始読み取りを前向きの関手とする圏同値を得る。
恒等・合成の保存と同型反映は、読み取りの関手性と各Homの両逆から従う。
原始評価の一致は、補題8.7と補題8.9の読み取りと組立ての両逆による。□
...
**証明.**
式(8.21)より、$`p(c,v_0)`$ のviewは $`v_0`$ である。
また式(8.25)のviewは $`g(c)`$ なのでgetを保つ。
putの保存は、

```math
\begin{aligned}
\mathrm{ext}(t)(p(c,v))
&=p'\bigl(t(p(c,v_0)),v\bigr)\\
&=p'\bigl(\mathrm{ext}(t)(c),v\bigr)
\end{aligned}
\qquad\text{(8.26)}
```

と計算できる。
$`k\in K`$ では $`p(k,v_0)=k`$ であり、
$`t(k)`$ も基準fiberに属するため、
$`\mathrm{ext}(t)(k)=t(k)`$ となる。

逆にLensの射 $`h`$ を基準fiberへ制限してから延長すると、

```math
p'\bigl(h(p(c,v_0)),g(c)\bigr)
=
h\bigl(p(p(c,v_0),g(c))\bigr)
=
h(c).
\qquad\text{(8.27)}
```
...
式(8.28)はgetとputを保ち、命題8.26によって、この基準fiber上の表から一意に定まる。
...
すべての生成辺で式(8.24)を満たすことである。
延長は一意で、制限と延長は互いに逆になる。

**証明.**
必要性は自然性と観測保存から従う。
十分性については、道の長さに関する帰納法で、
生成辺の自然性を任意の道へ延ばす。
関係で同一視した道は、すでに $`X,Y`$ の作用が等しいため、
商経路圏でも自然性が成立する。
自然変換は頂点成分だけで定まるので、延長は一意である。□
...
**証明.**
Lensでは、命題8.26により各Hom上でdecoderが全単射である。
対象については、命題1.33の同型
...
と、有限な $`K`$ の列挙を合わせれば、任意のLensがdecoderの像と同型になる。
プロトコルでは各頂点の有限状態集合を列挙し、
生成辺と観測をその列挙へ輸送する。
命題8.28がHomの全単射を与えるので、同様に圏同値を得る。
式(8.29)はこれらと系8.25の合成である。
...
を得る。従って両経路は対象だけでなく一般射についても一致する。□

有限集合の列挙を実データとして与え、比較する値の等号を判定できれば、
式(8.22)や式(8.24)は有限表の検査として実行できる。
プロトコルの表示そのものも検査する場合には、指定された有限個の関係について、
有限長の道の作用を比較する。
従って、列挙と等号判定を明示することで、有限表示の条件を実行可能な検査として記述できる。

**命題8.31（retract・Karoubi・比較平方への接続）.**
Lensとプロトコルの両入力で、実現圏は冪等完備である。
有限decoderを $`D_\Theta`$ とすると、式(8.29)は
...
へ延び、有限表示の包含上では元のdecoderと自然同型で一致する。
また、これらの同値は射圏と比較の可換平方へ延び、
端点の評価、および第6章のKaroubiと射圏の交換と整合する。

**証明.**
Lensの冪等射は、命題8.26によって有限基準fiberの冪等写像
$`t:K\to K`$ に対応する。
$`t`$ の固定点集合 $`K_t`$ に対し、
$`k\mapsto t(k)`$ と包含が分裂を与える。
積Lens $`V\times K_t`$ へ延長すれば、Lensの冪等射の分裂になる。

プロトコルの冪等自然変換 $`p:X\Rightarrow X`$ に対しては、
...
を取る。自然性から、生成辺は固定点を固定点へ送る。
観測も制限でき、$`p_v`$ と包含がプロトコル実現の分裂を与える。

系8.30により、任意の実現は有限decoderの像と同型であり、特にその像のretractである。
充満忠実なdecoderと上の分裂を用いると、
$`(P,e)`$ を $`D_\Theta(e)`$ の分裂像へ送ることで、
第6章のKaroubiの構成から式(8.32)を得る。
$`e=1`$ の場合には元のdecoded対象と同型になる。

射圏への作用は、射と可換平方の両端に同値関手を適用することで定まる。
端点評価との整合はこの定義から従う。
第6章の同値 $`\mathrm{Kar}(\mathrm{Arr}(𝒞))\simeq\mathrm{Arr}(\mathrm{Kar}(𝒞))`$ も、
同じ両端の冪等射と平方を使う。
従って両方の順序で同じ端点・射・平方を得て、
...
モデルが採用する型・係数・contextと読み取り項目の範囲も指定する必要がある。
以下では、必要な全項目を表にする場合と、部分的な読み取りから法則によって回復する場合を分ける。
...
また全隣接対を交換する $`\sigma_{ℕ}`$ は無限supportを持つため、
...
この読み取りによる再構成と、可算な構文による全射的な表示は別の性質である。
...
例8.20のsource-choice族 $`\chi:\Omega\to\{0,1\}`$ と、
...
になる。
$`\Omega`$ が無限なら、どの一つの有限集合 $`S`$ 上の読み取りも、
このsource-choice族の全元を区別できない。
...
濃度による第一の反例とは異なり、この議論に必要なのは、読み取り範囲の外の一点だけで異なる変更である。

従って、これらの反例を適用する際には、モデルにおける無限集合、許す変更の範囲、読み取り方式を指定する必要がある。
有限なモデルに対する情報の十分性と計算費用の問題は、§8.8で述べた条件によって扱う。

## 本章のまとめ

本章では、表示からの回復を対象と射の両方について定式化した。
有限例外codeでは、対象の意味を回復できても、固定表示間の射が意味的なHom全体を表すとは限らない。
有限Atom上では表示の正規化によってこの非充満性を解消できる。

局所再構成では、型付きの読み取り項目、有限片の整合性、関数graphの全体性と一意性、
および構造の保存条件から局所圏を定義した。
完全幾何について分離、射の組立て、対象の組立てを証明し、
型・operation・Law・局所幾何・実現のデータから元の構造を回復した。
主定理は、原始読み取りが完全幾何の圏と局所モデルの圏との同値を与えることを示す。
この同値は、対象に加えて非可逆なものを含む全許容射を回復し、恒等と合成を保つ。

行内送金システムの刷新では、口座・送金・会計の各担当が、
同じ取引を残高の増減、処理状態、仕訳という異なる構造によって記述する。
個々の対応表の型が正しいことに加えて、共有する取引や金額の対応、
操作の作用、完了条件などのLawの評価が整合しなければならない。
§8.6の例では、完了・記帳済みの取引に引落し・入金前の記録を対応づける候補が、
完了条件の残差によって排除される。

必要な原始データと条件から局所対象を定めると、全体の完全幾何が同型まで構成される。
固定した旧設計と新設計の間で局所射を定めると、それを読み取りとする構造保存射が一意に得られる。
この意味で、分担された記述から設計モデルを構成する問題と、
部分ごとの変更から全体の設計変更を構成する問題が、同じ再構成原理の下で扱われる。
複数の刷新案について、指定した操作・Law・幾何を保存する対応の成立条件を比較できる。

同じ分離・組立ての原理をLensとプロトコルにも適用し、既存CSの対象・一般射・合成を回復した。
それぞれの法則により、有限の基準fiberや生成表から一意な延長が得られる。
有限なモデルへの適用では、読み取り項目が対象と変更を決定するのに十分であること、
列挙と等号判定に基づく計算手順があること、およびその計算費用を区別する必要がある。
行内送金の例でも、完了判定と仕訳の存在だけでは対応候補を区別できず、出金・入金の記録を要する。
再構成に必要な構造と、その検査に用いる情報を明示することで、設計変更の整合性を検討できる。
