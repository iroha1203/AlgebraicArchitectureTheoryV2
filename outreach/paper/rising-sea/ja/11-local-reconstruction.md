...
| 有限片の貼り合わせ | 制限と型付き関数のグラフ | 表の貼り合わせと、関数としての成立条件 |
...
従って、この表示から漏れる意味的な射は、無限個のAtomを動かすものか、
...
従って、有限片の整合性だけでは関数のグラフの条件は保証されない。
...
| 選択された幾何 | 被覆要件、overlap、係数環、raw座標、関係式、restrictionの多項式 | 被覆とoverlapの保存、係数写像、方式に応じたrawの等号または対応のグラフ |
...
この商により、source・operation・context・係数・rawなど、保持すべき射の成分を残す。
...
関数のグラフの読み取りは恒等・合成を保つので、これらの読み取りは関手となる。□
...
最後に、これらの構成と原始データの読み取りが互いに逆であることを確かめる。
...
続いてcontextとその関係、各contextのobservableの環演算とrestrictionを構成し、
それぞれの公理を確かめる。このcontextと環を用いて、Lawの両辺と評価を構成する。
...
一例として、$`\mathrm{Ad}(P_z)`$ を満たすcoreの上で、
二つの面が同じ両端を持ち、両経路の作用が恒等である比較図式を取る。
構成5.37の入力は、比較に用いる面とcochainの選択を含む。
第一の面は恒等とし、第二の面には三つのsignature軸のうち二軸を交換する入力を取る。
係数を $`ℤ`$ とし、構成5.37〜5.40を第二の面に適用する。

初期defectがその二軸の交換となる選択では、defectは恒等ではなく、正規化の選択条件が成立する。
定理6.16により、非自明な冪等正規化と非可逆な $`\overline\beta`$ を得る。
一方、同じ比較図式で恒等のcochainを選ぶ場合には正規化が恒等となり、
...
型はその役割ごとの参照で指定し、関数のグラフの各値を読み取り項目とする。
...
すべての成分はグラフから一意に得られるので、読み取りと組立ての両逆も従う。□
...
商経路圏でも自然性が成立する。
全頂点で仮定した第二式は観測保存を与えるので、得られた自然変換は実現の射となる。
...
有限decoderを $`D_\Theta`$ とすると、式(8.27)は
...
局所再構成では、型付きの読み取り項目、有限片の整合性、関数のグラフの全体性と一意性、
