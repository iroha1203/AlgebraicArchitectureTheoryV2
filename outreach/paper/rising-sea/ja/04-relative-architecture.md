...
既存のCSの意味論への接続は、AATの構成がソフトウェアの何を保持しているかを
明らかにするために行う。状態、操作、その満たすべき法則をAATとは独立に定めれば、
AATの対象へ移した後も、それらを回復できるかを問える。
対象間の写像についても、元の意味論で許された写像が過不足なく得られるかを確かめられる。

本章では、この対応を異なる二つの側面から調べる。

- **Lens**は、状態の読取りと更新の整合性を扱う。冒頭の例のように、表示を保つことと、
  更新の結果まで保つことの違いを明確にできる。
- **プロトコル**は、名前付き操作をつないだ実行と、その観測を扱う。
  個々の操作を保つ条件が、任意の長さの実行を保つ条件へどうつながるかを調べられる。

§1.10では、両者の状態と操作をAATの対象へ格納し、Lawの成立と元の法則の成立が
同値になるように構成する。さらに、その対象間の型付き射から、元の意味保存射を
一意に回復できることを示す。こうした対応は、後続の章で共通の定理の仮定を確かめ、
変更の分類や局所再構成の結論を元の問題へ戻すための基礎となる。
...
を対応させる規則である。有限な部分族へ関係と同一視を制限したconfigurationは、
局所的な有限配置を与える。これをmoleculeと呼ぶ。
...
これらは $`U\to W`$ と $`V\to W`$ との合成が等しく、同じ可換条件を満たす
$`T\to U`$、$`T\to V`$ は、ただ一つの $`T\to U\times_W V`$ を通るとする。
...
**例1.24（局所切断を追加する層化）.**
二点集合 $`\{p,q\}`$ の部分集合を文脈とし、和集合で覆う族を被覆とする。
空文脈上では一元集合、空でない文脈上では $`\{0,1\}`$ を取り、
空でない文脈間の制限を恒等写像とする前層を考える。
一元文脈上の値 $`s_p=0`$、$`s_q=1`$ は重なりで一致するが、
二点上のどちらの元にも貼り合わない。
層化は $`U\mapsto\{0,1\}^U`$ であり、二点上のこの族を一つの関数として保持する。
...
構造関係は局所的な表現を定め、Lawの残差はその上で満たすべき条件を定める。
第2章では、この区別に従って方程式のイデアルとその零点を構成する。
...
ここからは、状態と操作の意味論を独立に定め、§1.10でAATの構成へ結びつける。
...
成分ごとの恒等と合成によって圏 $`\mathrm{Prot}(Q,L,O)`$ を得る。
...
同じ状態関数として作用する二つのプロトコル辺も、この構成で区別される。
...
この型付き射は、状態と操作の意味を与える集合・写像を比較する。
§1.7のreadingの射は、抽出、方程式添字、係数、signatureなどの選択自体を比較する射である。
構成1.40は、ここでの型付き射が与える状態写像を、抽出doctrineのsource写像へ結びつける。
