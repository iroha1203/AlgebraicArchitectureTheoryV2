...
圏 $\mathcal C$ の対象全体を $\operatorname{Ob}(\mathcal C)$、
$X$ から $Y$ への射集合を $\operatorname{Hom}_{\mathcal C}(X,Y)$ とする。
圏が明らかな場合には後者を $\operatorname{Hom}(X,Y)$ と略す。
射 $f:X\to Y$ が同型であるとは、$f^{-1}:Y\to X$ が存在して
$f^{-1}f=\mathrm{id}_X$、$ff^{-1}=\mathrm{id}_Y$ となることをいう。
同型な対象を $X\cong Y$、$X$ の自己同型群を
$\operatorname{Aut}_{\mathcal C}(X)$ と書く。
...
\longrightarrow\operatorname{Hom}_{\mathcal D}(F(X),F(Y)),
\qquad f\longmapsto F(f)
$$

が全単射であることをいう。すべての $D\in\operatorname{Ob}(\mathcal D)$ が
...
$k$-加群では $1m=m$ を仮定し、その圏を $\operatorname{Mod}_k$ と表す。
...
商加群を $M/N$ と書く。加群の間の線形写像全体を $\operatorname{Hom}_k(M,N)$、
準同型 $u$ の核と像を $\ker u$、$\operatorname{im}u$ と表す。
