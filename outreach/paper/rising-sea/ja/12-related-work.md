...
reading に相対的な対象と構造保存射を定め、その幾何、輸送、比較、局所再構成を展開する。
Law の値が同じであること、修復の障害が消えること、変更が比較を保つことを、
それぞれの条件で捉え、構成と定理によって結びつける。

代数的仕様、層の意味論、抽象解釈、データ移送、双方向変換にも、
対象と射、局所整合性、保存すべき情報を定める蓄積がある。
AAT は、これらの問いに対し、Law から作る幾何と診断、
reading の変更から作る輸送、比較の正規化と変更の分類を結ぶ構成を与える。
...
（[GB92, 定理11, p. 108](https://courses.grainger.illinois.edu/cs522/sp2016/InstitutionsAbstractModelTheory.pdf)）。
...
接続する port が適合性の条件を満たすなら、接続後も deadlock-freedom が保たれる。
部品に課した局所的な条件が、接続したシステムの性質を保証するのである
（[AG97, §8.1, 定義8.1.2・定理8.2.3](https://ix.cs.uoregon.edu/~michal/Classes/f01/fsv/papers/Allen-Garlan.pdf)）。
...
操作を実行してから状態を移す経路と、状態を移してから操作する経路が一致する。
AAT の adapter とその合成は、この等式を読替えや移送へ展開する（第4章）。
...
AAT は、修復の作用と torsor によって、局所状態の差を、
補正によって消せる部分と障害類として残る部分に分ける（第2章）。
...
（[AB11, §§2.2–2.4, 定理8.1](https://arxiv.org/pdf/1102.0264v7)）。
...
層条件の下で、指定障害類の零性から大域的修復を構成する。
固定した被覆で障害類が零なら、その coboundary を
与える補正を局所状態に作用させ、得られた整合族を貼り合わせる（定理2.22）。
torsor の切断による自明化という標準事実を、Law の評価と修復状態に結ぶ構成である
（[Stacks, Tag 03AG](https://stacks.math.columbia.edu/tag/03AG)）。

この判定を別の診断へ移すには、係数がどこから来るかも問われる。
Young のプログラム解析の提案では、プログラムの部分構造を site とし、
意味的性質の完全束を値に持つ前層を用いる
（[Young26, §§3.1–3.4](https://arxiv.org/html/2603.27015v1)）。
AAT は、修復語の関係式と Law の方程式から、それぞれ可換群の係数を生成する。
この二つを比較するには、係数の写像が関係式を保ち、cochain の微分と可換し、
実際の残差を表す類を対応させることが必要になる。

この比較は、先行研究 SAGA で示した。
意味的修復と代数幾何的な descent から係数を独立に作り、局所状態の同変な対応から
関係式の健全性を導く。二つの完全性条件の下で得られる係数同型は、
cochain と残差類の比較を与え、修復状態の層条件を通じて大域的修復へつながる
（[SAGA, §§3.4–3.7・4–5, 定理5.1–5.2](https://arxiv.org/html/2608.21458v1)）。

AAT は、この SAGA の比較を基礎に（定理2.49・系2.50）、
さらに障害と診断を reading の変更に沿って比較する。
同じ局所データから Law による診断を生成し、係数の関係と共通代表に関する条件の下で、
...
（[CC77, §§5–7, pp. 241–243](https://www.di.ens.fr/~cousot/publications.www/CousotCousot-POPL-77-ACM-p238--252-1977.pdf)）。
Giacobazzi らは、さらに抽象化と意味操作が可換する completeness を調べ、
不動点の結果についての completeness と区別した
（[GRS00, §3, pp. 371–372](https://www.sci.unich.it/~scozzari/paper/JACM00.pdf)）。
何を近似するかに加え、指定した操作について何を正確に保つかが問題となる。

AAT は、同じ Law-value を持つ Atom の商から、Law の値を正確に保持する
最も粗い reading を標準解像度として構成する（定理3.4）。
コホモロジー的な診断には、さらに係数と被覆の比較が要る。
条件 C の下での一次コホモロジーの同型（定理3.17）と、
指定障害類の零性の反映（定理3.40）は、この情報の十分性を異なる強さで捉えている。
後者では、診断類が零のときに、元の障害類を消す補正を構成する。
...
画面の読取りから更新の保存まで判定したければ、読取りに現れない変更が
更新との比較を壊さないことが必要になる。ここでの十分性は、固定した性質の判定に関するものだ。
AAT の局所再構成は、さらに対象と全構造保存射を回復するだけの情報を要求する。
Law の値の保存には商の普遍性、変更の判定には観測の核が働き、
再構成には局所データの分離と組立てが必要になる（第8章）。
...
その輸送は、係数と評価を備えた完全幾何へも広がる。
Opcartesian な輸送と cartesian な引き戻しは、
fibred category の普遍性に基づく
（[Stacks, Tag 02XJ](https://stacks.math.columbia.edu/tag/02XJ)）。
単位と合成の比較も、その普遍性から得られる（定理4.5・4.11・4.15）。
この exact な条件の下では、core の輸送と引き戻しは随伴同値をなす（命題5.8）。
一般の data migration の随伴より強い結論を得る理由は、
ここで指定した reading と再添字づけの条件にある。
...
（[Shulman08, §13, 定義13.11](https://www.kurims.kyoto-u.ac.jp/EMIS/journals/TAC/volumes/20/18/20-18.pdf)）。
AAT は reading の平方からこの mate を作り、選択した輸送の比較と
標準的な比較との関係を示す（定理5.11・命題5.12）。
完全幾何に対しては、係数や評価を含む成分ごとに二経路を構成し、保存等式を証明する。

AAT は、輸送と基底変換の同じ入力から生成比較を作り、
可逆な射と冪等射の合成に分解する。
冪等射の像に制限することで、可逆な比較が得られる（定理6.16）。
正規化後の変更を元の対象へ持ち上げる問題は、
この像と制限写像の構成から生じる（第7章）。
...
更新族の閉性などの仮定の下では、translation と complement の関係も得られる
（[BS81, §§5・7, 定理5.6・7.1](https://www.inf.unibz.it/franconispace/lib/exe/fetch.php?media=organisation%3Asakt%3A2013%3A5.pdf)）。

Foster らの lens は、読取り get と更新 put の法則によって双方向変換を扱う。
AAT が扱う全域 lens は、原論文の total very well-behaved lens に対応する三法則を満たす
...
その記述を型付き射や比較へ接続し、局所再構成では get・put の graph と三法則から
状態と操作を組み立て、有限な基準 fiber を持つ対象の間の全意味保存射を回復する。
非可逆な射も、この再構成に含まれる（第8章）。

更新を持ち上げる普遍性について、Johnson らは categorical lens と
split opfibration を関係づけている
（[JRW12, §4, 命題4.1・4.3](https://mta.ca/~rrosebru/articles/Lens2Cambridge.pdf)）。
AAT は、この種の普遍性を輸送と引き戻しに用い、
さらに比較を保つ変更の全体を群として扱う。
比較保存群は射の圏における自己同型群として読め、端点への射影の非空な fiber は
その核の torsor となる（命題7.3・定理7.4）。
完全幾何の標準比較については、source core の条件 Ad と canonical な正規化の下で、
正規化後の適合変更を持ち上げる section を具体的に構成する（定理7.21）。
同じ正規化後の値を持つ不適合な変更も得られるため、
適合する持ち上げの存在と、その適合性を観測から判定することは、異なる問題になる。
...
この分類は、更新や処理の引継ぎを保つリファクタリングにも帰結を持つ。
対象となるのは、固定した意味論の下で可逆な状態対応を持つ変更である。

## R.6 表示と局所再構成

対象とその変更を局所的な表示で扱うには、構造保存射も回復できる必要がある。
AAT は、各入力族で独立に定めた局所データと整合等式から対象を組み立て、
局所射と全域の構造保存射を対応づける。この読取りと組立てによって、
実現の圏と局所モデルの圏の同値を与える（第8章）。
...
（[BW85, 第4章 §1.2](https://www.kurims.kyoto-u.ac.jp/EMIS/journals/TAC/reprints/articles/12/tr12.pdf)）。
型と整合等式を先に定めることで、何がモデルであり、何がその射であるかを指定できる。

読取りから全域の構造へ戻るには、三つの性質が要る。
射を区別できること、局所射を全域射へ組み立てられること、
局所対象を全域対象から同型を除いて得られることである。
AAT の原始読み取りでは、これらが忠実性、充満性、本質的全射性を与える。
そこから圏同値を導く判定は標準的であり
（[Stacks, Tag 02C3, 補題4.2.19](https://stacks.math.columbia.edu/tag/02C3)）、
AAT は、各入力族の独立な局所等式からこの三性質を導く。
射の組立てと読取りの両逆、恒等射と合成の保存も、その構成に含まれる。

nerve による表示も、射の分離と局所データの実現可能性をともに問う。
...
AAT は、原始読み取りの値と制限の整合性に、各入力族の構造条件を課して局所モデルを定める。
これらが、対象と射を組み立てるための具体的な条件となる。
...
それぞれ証明する。完全幾何では、局所 Hom の不変量による商と全域 Hom の対応を用いる。

組立てに使うデータは、対象の構造によって異なる。

- **Lens:** 状態 carrier、get・put の graph、閉じた法則、有限な基準 fiber。
  これらから状態と操作を定め、基準 fiber の写像から全意味保存射を得る。
- **プロトコル:** 頂点の carrier、生成辺と観測の graph、経路の関係式。
  局所射の条件から、全経路に自然な写像を得る。Spivak の圏の表示と instance の
  意味論を、原始データからの組立てとして具体化する構成でもある
  （[Spivak12, §§3.2・3.4–3.5](https://arxiv.org/pdf/1009.1166v3)）。
- **完全幾何:** 構成する成分と関数の graph、および係数と評価の保存条件。
  これらの局所条件から、対象と全構造保存射を回復する。
...
全域の構造を回復できることと、有限個の値だけで決定できることには、なお違いがある。
AAT のタグ変更族では、各添字で二値のタグを反転するかを選ぶ。
全有限部分集合への制限が整合していれば、全域の変更へ一意に組み立てられ、群構造も保たれる。
しかし添字集合が無限なら、どの有限部分集合の外にも読取りに現れない変更が残る。
全有限片を通じた再構成と、一つの有限片による決定は、必要とする情報量が異なる。

## R.7 AAT の位置づけ

AAT は、仕様の意味論、局所整合性、情報の保存、構造の移送をめぐる先行研究と問いを共有する。
これらに対し、Atom・Law・reading から対象と射を構成し、
障害と診断、輸送と比較、変更の分類を結ぶ体系を与える。
AAT の寄与は、各構成を結ぶ定理と、個々の問題の意味論との対応にある。

AAT が掲げる **Rising Sea** は、こうした基礎を築くことで、
個別の問題を共通の構成の特殊な場合として捉える方針である。
R.5 の lens とプロトコルに共通する変更分類は、この方針の具体例となる。
