...
**定理7.21（生成比較の識別と持ち上げ）.**
構成5.37の同じexactな底の平方、面、元のcore、係数と元の幾何から、
...
式(7.40)は式(7.43)の積になる。これはすべての変更を一意に表すので群同型である。□
...
式(7.44)の前半は定義7.23の観測条件、後半はすべての辺についての操作保存条件そのものである。
...
さらにadapter $`c:X\to Y`$ を固定し、命題1.43の充満忠実関手に定理4.41を適用する。
すると、$`vc=cu`$ という適合条件を満たす端点対の群が、元の意味論と型付き構成の間で同定される。
この同定は両端の成分を対応づけるので、射影、その核と非空fiberのtorsorも対応する。
...
lensでは全域putがすべてのviewを結び、プロトコルの例では独立なワーカーごとに変更が残る。
...
- **比較を保つ変更の群。** 一つの比較 $`c`$ に対し、$`vc=cu`$ を満たす端点対の群を構成した。
  その射影と核が、一方の変更に追随できる条件と、追随する全候補の自由度を与える。
...
- **操作による分類。** 操作グラフの連結成分を用い、lensとプロトコルの変更を共通の分裂短完全列で記述した。
  各成分では操作に沿って対応を揃え、独立な成分には独立な変更が残る（§§7.7–7.8）。
