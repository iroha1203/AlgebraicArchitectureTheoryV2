...
| ソフトウェアの操作は変更をどう制約するか | 操作グラフの連結成分 | Lensとプロトコルに共通する分裂短完全列と変更の個数（§§7.7–7.8） |
...
を用いる。$`\Gamma_c`$ を全端点群で作った場合の記法である。
固定するのは変更 $`u,v`$ の底への像であり、$`\pi(c)`$ は一般の比較のままでよい。
また、底を固定する条件と係数を固定する条件は別々に指定できる。

**命題7.3（比較対象から得る群）.**
式(7.2)は $`G_X\times G_Y`$ の部分群である。
全端点群を用いると、これは第6章の射の圏 $`\mathrm{Arr}(E)`$ における $`c`$ の自己同型群である。
恒等冪等による埋込み $`J:E\to\mathrm{Kar}(E)`$ を介して、
比較の圏 $`ℳ(E)=\mathrm{Arr}(\mathrm{Kar}(E))`$ における $`J(c)`$ の自己同型群とも同型になる。
この同定は両端への射影と可換し、AATでは底を固定する部分群にも制限できる。

**証明.**
恒等対は式(7.2)を満たす。二つの適合対に対し、
$`v_2v_1c=v_2cu_1=cu_2u_1`$ なので積も適合する。
$`vc=cu`$ の両側に逆を合成すると $`v^{-1}c=cu^{-1}`$ となる。
射の圏の自己同型は、両端が同型である可換平方そのものである。
さらに $`J(X)=(X,1_X)`$ の射には冪等射による追加の制約がなく、$`J`$ は充満忠実である。
両方向の対応はいずれも端点の射を保持するので、射影と底の条件も保つ。□

一方だけを変更したとき、他方で追随できるとは限らない。
追随できる場合の全候補は、比較をそのままにする変更によって記述できる。

**定理7.4（端点への射影と追随変更）.**
両端への射影 $`p_X:\Gamma_c\to G_X`$、$`p_Y:\Gamma_c\to G_Y`$ を取る。
その核はそれぞれ
...
である。$`u\in G_X`$ に追随する $`v`$ が存在する必要十分条件は、$`u\in\mathrm{im}\,p_X`$ である。
一つの追随変更 $`v_0`$ があれば、全候補は $`T_cv_0`$ となる。
同様に、$`v\in G_Y`$ に追随する $`u_0`$ があれば、全候補は $`S_cu_0`$ となる。
...
を得る。従って $`v=tv_0`$、$`t=vv_0^{-1}\in T_c`$ と一意に書ける。
...
**系7.5（同型比較の変更）.**
$`c`$ が同型で、共役 $`u\mapsto cuc^{-1}`$ が $`G_X`$ と $`G_Y`$ を対応づけるとき、
...
**証明.** 式(7.2)を $`v=cuc^{-1}`$ と解けばよい。
全端点群では共役の条件が自動的に成り立つ。
底を固定する群でも、$`\pi(cuc^{-1})=\pi(c)1\pi(c)^{-1}=1`$ なので成り立つ。□
...
**定理7.6（生成された変更の適合性）.**
第5章§5.8の共通source $`S`$、二経路の終点 $`X,Y`$、生成比較 $`c:X\to Y`$ を取る。
Sourceで底を固定する自己同型群を $`G_S^0`$ とする。
...
従って、一方の変更 $`a`$ を固定したとき、他方で使えるsourceの変更は $`Ja`$ である。

**証明.**
二経路のcartesianな脚を $`K_X:X\to S`$、$`K_Y:Y\to S`$ と書く。
ここでは第5章と同じく、必要に応じてrefinement側へ写して脚を合成する。
生成時の因子化と比較の三角形は
$`K_XF_*(a)=aK_X`$、$`K_YG_*(a)=aK_Y`$、$`K_Yc=K_X`$ である。
よって式(7.7)の両辺に $`K_Y`$ を合成すると、いずれも $`aK_X`$ になる。
...
**命題7.7（入力の表示変更と再生成）.**
第5章の共通source図式を $`(S_v,L_e,\kappa_f)`$ と書く。
...
両脚を $`K_X,K_Y`$ と略すと、これらは
$`K'_X=\theta^{-1}K_Xb`$、$`K'_Y=\theta^{-1}K_Yd`$ を満たす。
...
1. ある述語 $`D:R\to\{\text{真},\text{偽}\}`$ があり、全ての $`q\in Q`$ について
...
従って $`K/L`$ が一点であることと、定理7.9の判定条件は同値である。
...
底を固定する比較保存群の間にも準同型を得る。完全幾何の正規化関手にも同じことが成り立つ。
...
命題7.12は正規化関手の定義された全自己同型を扱い、構成7.13は中心化群を扱う。
...
$`\delta\in\Delta`$ の適合する持ち上げは、$`\delta\in r(\Gamma_0)`$ のときに存在する。
一つを $`\gamma_0`$ とすると、全ての適合する持ち上げは
...
$`\Delta`$ に属する $`r`$ の像は全て $`\Gamma_0`$ から来る。
...
従って $`\widehat a(s_P(C))=s_P(\sigma C)`$ であり、対象形成を保つ。
...
よって対象写像は恒等と合成を保つ。
選択した対象だけでなく、全ての付随データに対してこの等式が成立することが、
...
対象には構成7.17を用いる。$`n=n_P`$、operationの型の同一視を
...
式(7.30)は、片側を構成した後、元の比較を通じてもう片側を決める。
この順序によって、元の比較への適合性を保った持ち上げが得られる。

## 7.6 生成比較に残る情報と、全ての持ち上げ

Sectionがあれば、正規化後の適合変更から元の適合変更を作れる。
しかし、それと同じ正規化後の値を持つ変更が、全て適合するとは限らない。
...
はconfigurationと全ての選択対象を固定し、二回適用すると恒等になる。
たとえばcoreが指定するconfigurationで、交換される二対象があるため非恒等である。

この写像は完全幾何の自己同型へ延長できる。
Operationには $`\nu_{\tau_G A,\tau_G B}^{-1}\nu_{A,B}`$ を用い、
Atom・底・文脈・方程式添字・invariant添字・軸・係数・局所幾何の写像には恒等を使う。
両対象の正規化が一致するので、補題7.18と同じ正規化を介した計算で保存条件が従う。
付随データの交換とoperationの型の同一視は二回で戻る。従って
...
**定理7.21（生成比較の識別と持ち上げ）.**
構成5.37の同じexactな底の平方、面、source core、係数とsource geometryから、
...
両対の底と係数の観測は同じ恒等対である。従って、系7.11の情報損失は、
同じ入力から生成した完全幾何の比較でも生じている。

**系7.22（診断による正規化の選択）.**
構成5.40の同じ入力を取り、選択条件を $`\chi`$ と書く。
...
**定理7.24（操作保存と連結成分）.**
固定した $`u`$ に追随する状態変更は、頂点ごとの置換を使って一意に
...
である。従って、操作を保つ変更は、向きを忘れた連結成分の集合 $`\pi_0(Q)`$ ごとに
一つの置換を選ぶことと一対一に対応する。
...
操作を保つ組 $`(u,h)`$ 全体の群を $`A_Q`$ とし、可視変更への射影を $`p:A_Q\to H`$ とする。
...
を得る。Sectionは $`s(u)=(u,h_u)`$、$`h_u(v,k)=(u_V(v),k)`$ である。
...
さらに、$`H`$ の連結成分への作用を $`u\cdot j`$ と書けば、
...
である。右辺の積は $`(\psi,u)(\eta,u')=(\psi(u\cdot\eta),uu')`$ とする。

**証明.**
観測と操作の等式は合成・逆で保たれるので、$`A_Q`$ は群になる。
$`u=1`$ のfiberは定理7.24によって $`M_Q`$ と群同型である。
$`s(u)`$ は操作を保ち、$`s(uu')=s(u)s(u')`$、$`ps=\mathrm{id}`$ なので短完全列は分裂する。
各fiberの右torsor構造は定理7.14を $`p`$ に適用して得る。
定理7.24の置換族を数えれば式(7.42)となる。

半直積の表示では、族を出発点でなく到着点の成分で添字づける。
$`(\psi,u)`$ を $`h(v,k)=(u_V(v),\psi_{[u_V(v)]}(k))`$ へ送ると、
式(7.40)は式(7.43)の積になる。これは全ての変更を一意に表すので群同型である。□

**系7.26（選択状態を保つ版）.**
$`k_0\in K`$ を固定し、$`s_0(v)=(v,k_0)`$ を選択状態とする。
$`hs_0=s_0u_V`$ も要求する場合、定理7.24・7.25の $`\mathrm{Sym}(K)`$ を
$`\mathrm{Sym}(K)_{k_0}=\{\phi\mid\phi(k_0)=k_0\}`$ へ置き換えればよい。
...
この分類を観測の言葉でも読める。
操作保存をまだ課さない追随変更の群を $`B_Q`$ とすると、可視射影の核は
$`\prod_{v\in V}\mathrm{Sym}(K)`$ であり、その中で操作を保つ部分は $`M_Q`$ である。
従って定理7.9より、可視変更だけから操作保存を判定できるのは、この二つが一致するときである。
...
### Lens：全域更新が全てのviewを結ぶ

**命題7.27（Lensの変更の共通分類）.**
定義1.32の全域lens $`L=(C,g,p)`$ と、その有限な基準fiber $`K_L`$ を取る。
命題1.33の積表示 $`C\cong V\times K_L`$ を使い、
頂点をview、各順序対 $`(v,w)`$ の辺を $`w`$ への更新とするグラフを作る。
任意の許容可視群 $`H\le\mathrm{Sym}(V)`$ は、辺を $`(v,w)\mapsto(u(v),u(w))`$ と移す。
この入力は定義7.23を満たし、条件
...
を満たすlensの可逆変更と、その $`A_Q`$ は群同型になる。
$`V`$ は基準viewを持つので非空であり、グラフは連結である。従って
...
を得る。この場合、連結成分への作用は自明なので半直積は直積となる。
各可視変更に追随する変更は $`|K_L|!`$ 個である。

**証明.**
積表示では $`g(v,k)=v`$、$`p((v,k),w)=(w,k)`$ である。
式(7.44)の前半は定義7.23の観測条件、後半は全ての辺についての操作保存条件そのものである。
積表示を介する対応は両向きで、恒等と合成も保つ。
定理7.24により、全頂点で同じ置換 $`\phi`$ を使うことが必要十分となる。
残りは定理7.25から従う。□

選択section $`s:V\to C`$ に $`gs=\mathrm{id}`$、$`p(s(v),w)=s(w)`$ を要求すると、
積表示では $`s(v)=(v,k_0)`$ となる。
このsectionも保つ変更は系7.26に対応する。
...
観測先は定値一元集合とする。このとき、平行経路は全て同じ恒等写像を与えるので、
...
| --- | --- | --- |
| 制御点の読み取りへの追随 | $`2^4=16`$ 個 | $`2^4=16`$ 個 |
| 名前付き操作も保存 | $`2^2=4`$ 個 | $`2^2=4`$ 個 |

交換では頂点 $`0\leftrightarrow2`$、$`1\leftrightarrow3`$ と辺 $`a\leftrightarrow b`$ を同時に動かす。
内部状態を変えない交換がsectionとなり、その後に二成分の置換を選ぶことで全候補を得る。
恒等とワーカー交換からなる可視群では、核は $`S_2\times S_2`$ である。
交換はこの二因子を入れ替えるので、全変更群は $`(S_2\times S_2)\rtimes S_2`$、位数8となる。
...
可視変更を許す場合には、viewや制御点、操作名の再添字づけを両側で同じように指定する。
式(7.44)、(7.47)は、その型付き操作の可換条件である。

さらにadapter $`c:X\to Y`$ を固定すると、命題1.43の対応は合成と射の等しさを保ち、反映するので、
$`vc=cu`$ という適合条件も両側で同値になる。
従って、端点群、比較保存部分群、両端への射影、その核と非空fiberのtorsorが対応する。
この適用では、定義7.2の $`E`$ を型付き対象の圏に取り、
指定した操作と観測を保つ型付き変更を許容群に選ぶ。
すると、定理7.4と定理7.14の分類が元の意味論の分類となる。

操作の等式は、比較保存群のどの部分群を選ぶかを決め、持ち上げの自由度を制限する。
Lensでは全域putが全てのviewを結び、プロトコルの例では独立なワーカーごとに変更が残る。
両者の違いが、同じ連結成分の定理の異なる入力として説明できる。

## 本章のまとめ

一つの比較 $`c`$ を保つ変更は、$`vc=cu`$ を満たす端点対の群をなす。
その射影と核から、一方の変更に追随できる条件と、追随する全候補の自由度が分かる。
同じsourceから生成した変更は比較を保ち、入力を同型で表示し直して再生成しても、この関係は移送される。

変更の観測 $`O`$ だけで適合性を判定できる必要十分条件は、$`\ker O\subseteq\Gamma`$ である。
観測に現れない変更が一つでも比較を壊すなら、適合・不適合な変更が同じ観測を持つ。
一方、適合する持ち上げを作る問題は別である。
AATのcanonicalな正規化では、対象の付随データの共通表示からsectionを構成でき、
同じ生成比較について、適合する持ち上げの存在と適合性の反映の失敗が同時に成立した。
適合する持ち上げ全体は、制限準同型の核のtorsorとして記述された。

リファクタリングのレビューでは、表示や呼出関係の一致に加え、
共通部品への切出しやデータ型の分割で、処理間の引継ぎが変わっていないかを問う。
配送先を変えたときの決済データ、ジョブの実行を始めたときのコンテキストは、その具体的な確認対象になる。
観測がそれらを見ていないとき、見えない変更の中に操作やadapterとの対応を壊すものがあるか。
定理7.9は、この問いを $`\ker O\subseteq\Gamma`$ という条件で表す。
正規化後の設計案を具体化するときには、sectionが一つの適合案を与え、核が他の適合案を記述する。

Lensとプロトコルでは、この自由度が具体的な操作のつながりとして現れた。
Lensでは、配送先の表示と未設定値を保つ4候補のうち、更新も保つものは2候補である。
ワーカーの例では、制御点の対応だけを保つ16候補が、コンテキストの受渡しを保つ4候補へ絞られる。
独立なワーカーごとに表現を選べても、一つの受渡しの両端では対応を揃えなければならない。
操作保存の条件を揃えることで、同じ分裂短完全列とtorsorの定理が両者を分類する。
