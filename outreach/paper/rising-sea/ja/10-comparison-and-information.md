...
これは比較の圏 $`ℳ(E_{\mathrm{geom}})`$ における $`J(c)`$ の
...
選択sectionを $`s(v)=(v,k_0)`$ と固定した場合、
$`hs=su`$ は各fiberの置換が $`k_0`$ を固定する条件と同値である。
