...
本章の問いは、**各部分でLawを満たす状態を、いつ全体の状態へ貼り合わせられるか**である。
...
条件を課す前の空間と、各条件を課した後の空間を順に表す。

**構成2.6（方程式の幾何的実現）.**
各文脈で、対象のreading $`A_p`$ と、可換 $`k`$-代数 $`R`$ への準同型
$`e_p:O_E(W)\to R`$ からなる局所評価データ $`p`$ を考える。これらは係数環の変更で移送できるものとする。
次の代数的な表示が与えられる場合を扱う。
...
4. 貼り合わせ後のchartの対応 $`W\mapsto X_W`$（後の式(2.4)で定める）は文脈の射を開部分の包含へ送り、
...
ここで左辺と残差が結ばれるのは、条件1により $`u_W`$ が評価を表し、
条件2により $`r_{W,i,a}`$ が残差の評価を表すからである。
評価の合成則は環準同型の合成則から従う。
局所化したイデアルの元は生成元の有限和を分母付きで表したものであり、式(2.6)を得る。
各chart上で、加群 $`J_i(W)`$ に伴う層 $`\widetilde{J_i(W)}`$ を取り、同じ生成元を保つ重なりの同型で貼り合わせればよい。
...
定数係数の0-cochainに対し、制限の差 $`(u,v)\mapsto u|_L-v|_L`$ は全射である。
...
これらの族が生成する、文脈site上のGrothendieck topologyを用いる。
...
**定義2.43（意味的修復の表示）.**
各文脈 $`V`$ に、意味論の側で区別するAtomの集合 $`\Lambda(V)`$、
...
**構成2.46（Atom・方程式の対応）.**
各 $`\lambda\in S(V)`$ に、必須の方程式添字 $`i_\lambda`$ と対象のreading $`A_\lambda`$ を指定し、
制限によってこれらが保たれるものとする。
...
これらは、命題2.41と同じくchart上の状態をアフィン遷移で貼り合わせた状態の層を定める。
...
冒頭の三つのサービスでは、一周した補正量が、各部分の基準を選び直しても残る食い違いを表す。
その障害類が零なら、局所補正を作用させて重なりの状態を一致させられる。
補正後の状態が一つの大域状態になることは、実際の状態の層条件によって保証される。
