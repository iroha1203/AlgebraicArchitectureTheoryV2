...
第1章では、lensとプロトコルの操作・法則・意味保存射を、独立に定めた意味論から回復した。
...
**定義2.14（Circuitから係数への実現）.**
アーベル群の層 $`F`$ を選ぶ。
Law $`i`$ の局所circuit $`c`$ に、対応する生成元 $`\nu(c)\in I_i^E(W)`$ を割り当て、
制限と可換する加法的写像
...
局所circuitとその生成元への割当ても制限と可換するものとする。
たとえば商への射を層化した $`I_i^E\to a_J(I_i^E/(I_i^E)^2)`$ は、このような写像の候補となる。
その像がcircuitを検出するかは、選んだ生成元について調べる。

**命題2.15（係数による有限失敗の検出）.**
必須Lawに対する局所circuitが健全かつ完全であり、各circuitの式(2.9)の像が非零であるとする。
このとき、局所的な必須Lawの失敗は、非零のcircuit像の存在と同値である。
...
である。三重の重なりがないので全1-cochainがcocycleになる。
...
**定義2.20（局所状態とTorsor）.**
局所状態の前層 $`P`$ と係数前層 $`F`$ を取り、被覆の各交差上で
...
そのために、被覆が $`E`$-adequateであり、必要な座標・相互作用を読めることを要求する。
さらに、局所circuitの健全性・完全性、採用した軸の零判定と対応残差の零性の同値、
必要な全witnessが被覆内で検査されることを確かめる。
命題2.15のように像を集約するなら、その非零性と相殺しない条件も含める。
...
**定義2.30（抽出の不変性による二相）.**
第1章の抽出doctrine $`D`$ に対し、語彙、正規化、resolution、sourceの構造を保ち、
...
**命題2.32（意味側での障害の保持）.**
条件Eが成立し、$`H^1(C^\bullet_{\mathrm{str}})=0`$ なら、誘導写像
...
各Lawには値の集合 $`V_\ell`$ と評価 $`v_\ell:S\to V_\ell`$ がある。
生成子の集合を $`G=L\times S`$ とし、$`g=(\ell,s)`$ のラベルを
$`(\ell,v_\ell(s))`$ とする。
原始関係 $`R\subseteq G\times G`$ は、このラベルを保つ関係として宣言する。
$`R`$ の反射・対称・推移閉包による商集合を $`B`$ とし、生成子 $`g`$ の成分を $`[g]_R\in B`$ と書く。
整数係数の群を
...
**補題2.36（係数の標準形）.**
$`e_g\mapsto e_{[g]_R}`$ は同型 $`M_R\congℤ^{(B)}`$ を誘導する。
...
四つの頂点 $`a_0,a_1,b,c`$ と、四つの辺点 $`k,ab,bc,ac`$ を持つ集合 $`X`$ を取る。
辺点の両端を
$`k:(a_0,a_1)`$、$`ab:(a_1,b)`$、$`bc:(b,c)`$、$`ac:(a_0,c)`$ とする。
...
被覆には、開集合を覆い、読まれる生成子も覆う族を採用し、それらで生成される位相を用いる。
...
```math
\begin{aligned}
U_{a_0}&=\{a_0,k,ac\},& U_{a_1}&=\{a_1,k,ab\},\\
...
$`U_{a_0}`$ と $`U_{a_1}`$ は $`k`$ で交わるので、その和も連結である。
四chartの非空交差は $`\{k\},\{ab\},\{bc\},\{ac\}`$、
三chartの非空交差は $`\{ab\},\{bc\},\{ac\}`$ である。
