...
def TwoChartGloballyUFlat {U : AtomCarrier.{u}} {A : ArchitectureObject U}
  ...
  ∃ s : D.twoChartCech.C0, b.b_U = D.twoChartCech.d0 s ∧ D.twoChartCech.d0 s = 0
...
This predicate records the actual equation `b_U = d0(s)` without requiring the
selected boundary agreement to vanish.  `TwoChartGloballyUFlat D b` is the
stronger zero-boundary case.
-/
