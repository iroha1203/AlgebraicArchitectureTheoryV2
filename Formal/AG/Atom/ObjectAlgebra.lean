...
Implementation notes: circuits are derived from `lawReading` at an object and
law index, while operations are indexed by object pairs.  Selected circuit and
selected operation fields were rejected because the algebra is a family of
objects and morphisms rather than a package containing one witness of each.
-/
structure ObjectAlgebra (U : AtomCarrier.{u}) where
  ...
  /-- The law universe and indexed circuit reading. -/
...
/-- The finite circuit fiber selected at an object and law index. -/
