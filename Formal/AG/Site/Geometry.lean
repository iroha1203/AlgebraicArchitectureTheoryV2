...
The law universe and architecture signature are read from `core`; this package
selects only the context preorder, coverage requirements, and overlap data.
-/
structure SelectedGeometryReading {U : AtomCarrier.{u}}
    (core : AATCorePackage U) where
  /-- The selected preorder of readable contexts on the generated object. -/
  contextPreorder : ContextPreorderCategory core.object
  /-- Coverage requirements typed by the generated law universe and signature. -/
  requirements : CoverageRequirements core.object
    core.algebra.lawReading.lawUniverse core.algebra.signatureReading
  /-- Selected overlap data on the same context preorder. -/
  overlap : ContextOverlapPullback contextPreorder
...
def toAATSite {U : AtomCarrier.{u}} {core : AATCorePackage U}
  ...
  contextPreorder := reading.contextPreorder
  lawUniverse := core.algebra.lawReading.lawUniverse
...
/-- The selected site law universe is the generated core law universe. -/
@[simp]
theorem toAATSite_lawUniverse {U : AtomCarrier.{u}}
    {core : AATCorePackage U} (reading : SelectedGeometryReading core) :
