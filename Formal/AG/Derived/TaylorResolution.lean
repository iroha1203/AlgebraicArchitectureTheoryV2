...
/--
V-3: regularity condition sufficient for exactness of the concrete
two-generator Taylor/Koszul surface.

The first clause says that `a` is a non-zero-divisor.  The second clause is the
visible quotient regularity condition: if `b*x` is divisible by `a`, then `x`
is already divisible by `a`.
-/
def IsRegularPair (a b : A) : Prop :=
  (∀ x : A, a * x = 0 -> x = 0) ∧
    ∀ x : A, a ∣ b * x -> a ∣ x

/-- V-3: the image submodule generated by the two visible Taylor generators. -/
def spanPair (a b : A) : Submodule A A :=
  Submodule.span A ({a, b} : Set A)

/-- V-3: the ideal generated by the two visible Taylor generators. -/
def idealSpanPair (a b : A) : Ideal A :=
  Ideal.span ({a, b} : Set A)

/-- V-3: the submodule span is the underlying module of the ideal `(a,b)`. -/
theorem spanPair_eq_idealSpanPair (a b : A) :
    spanPair a b = idealSpanPair a b :=
  rfl
...
/--
V-3: regular pairs have the principal-syzygy exactness needed by the concrete
two-generator Taylor/Koszul complex.
-/
theorem principalSyzygyExact_of_isRegularPair
    (a b : A) (hreg : IsRegularPair a b) :
    PrincipalSyzygyExact a b := by
  intro x hx
  rcases x with ⟨p, q⟩
  have hbdiv : a ∣ b * q := by
    refine ⟨-p, ?_⟩
    ...
    calc
      ...
      _ = a * -p := by ring
  rcases hreg.2 q hbdiv with ⟨r, hq⟩
  refine ⟨r, ?_⟩
  ext
  · have hpzero : a * (p + b * r) = 0 := by
      rw [d₁_apply] at hx
      rw [hq] at hx
      simpa [mul_add, mul_assoc, mul_left_comm, mul_comm] using hx
    have hp : p + b * r = 0 := hreg.1 (p + b * r) hpzero
    have hp' : p = -(b * r) := by
      calc
        p = (p + b * r) + -(b * r) := by abel
        _ = -(b * r) := by rw [hp]; simp
    calc
      p = -b * r := by simpa [neg_mul] using hp'
      _ = (d₂ a b r).1 := by
        simp [d₂_apply]
  · calc
      q = a * r := hq
...
/--
V-3: the image of the concrete degree-one differential is exactly the ideal
`(a,b)` as a submodule of the ambient ring.
-/
theorem range_d₁_eq_spanPair (a b : A) :
    LinearMap.range (d₁ a b) = spanPair a b := by
  ext x
  constructor
  · rintro ⟨y, rfl⟩
    rw [d₁_apply]
    exact Submodule.add_mem (spanPair a b)
      (by
        simpa [spanPair, smul_eq_mul, mul_comm] using
          Submodule.smul_mem (spanPair a b) y.1
            (Submodule.subset_span (by simp)))
      (by
        simpa [spanPair, smul_eq_mul, mul_comm] using
          Submodule.smul_mem (spanPair a b) y.2
            (Submodule.subset_span (by simp)))
  · intro hx
    change x ∈ Submodule.span A ({a, b} : Set A) at hx
    refine Submodule.span_induction
      (p := fun y _ => y ∈ LinearMap.range (d₁ a b))
      ?hmem ?hzero ?hadd ?hsmul hx
    · intro y hy
      ...
      · refine ⟨(0, 1), ?_⟩
        simp [d₁_apply]
    · exact ⟨0, by simp [d₁_apply]⟩
    · rintro y z _ _ ⟨hy, rfl⟩ ⟨hz, rfl⟩
      exact ⟨hy + hz, by simp⟩
    · intro c y _ hy
      exact Submodule.smul_mem (LinearMap.range (d₁ a b)) c hy

/--
V-3: quotient bridge from the cokernel of the visible degree-one Taylor
differential to the concrete quotient by `(a,b)`.
-/
noncomputable def quotientLinearEquivSpanPair (a b : A) :
    (A ⧸ (LinearMap.range (d₁ a b))) ≃ₗ[A] (A ⧸ spanPair a b) :=
  Submodule.quotEquivOfEq (LinearMap.range (d₁ a b)) (spanPair a b)
    (range_d₁_eq_spanPair a b)

/--
V-3: quotient bridge from the cokernel of the visible degree-one Taylor
differential to the ideal quotient `A/(a,b)`.
-/
noncomputable def quotientLinearEquivIdealSpanPair (a b : A) :
    (A ⧸ (LinearMap.range (d₁ a b))) ≃ₗ[A] (A ⧸ idealSpanPair a b) :=
  Submodule.quotEquivOfEq (LinearMap.range (d₁ a b)) (idealSpanPair a b)
    (by rw [range_d₁_eq_spanPair, spanPair_eq_idealSpanPair])
...
/--
V-3: exactness of the two-generator Taylor/Koszul complex under the explicit
regular-pair hypotheses, together with the quotient bridge to `A/(a,b)`.
-/
theorem exact_visible_complex_of_isRegularPair
    (a b : A) (hreg : IsRegularPair a b) :
    Function.Exact (d₂ a b) (d₁ a b) ∧
      Function.Exact (d₁ a b) (LinearMap.range (d₁ a b)).mkQ ∧
        Nonempty ((A ⧸ (LinearMap.range (d₁ a b))) ≃ₗ[A] (A ⧸ idealSpanPair a b)) :=
