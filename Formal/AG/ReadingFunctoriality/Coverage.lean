...
/-- The canonical projection from a tuple overlap to one of its charts. -/
noncomputable def canonicalTupleOverlapProjection
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    {n : Nat} (σ : Fin (n + 1) → 𝒰.Index) (k : Fin (n + 1)) :
    canonicalTupleOverlap 𝒰 n σ ⟶
      Site.ContextCategoryObject.of S.contextPreorder (𝒰.patch (σ k)) :=
  homOfLE (canonicalTupleOverlap_le_patch 𝒰 n σ k)
...
/--
Map an object into a selected tuple overlap from maps to every chart in the
tuple. The morphism is generated by the iterated overlap universal property.
-/
noncomputable def canonicalTupleOverlapLift
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    {n : Nat} (σ : Fin (n + 1) → 𝒰.Index) {X : S.category}
    (h : ∀ k, X ⟶ Site.ContextCategoryObject.of S.contextPreorder (𝒰.patch (σ k))) :
    X ⟶ canonicalTupleOverlap 𝒰 n σ := by
  apply homOfLE
  apply canonicalTupleOverlap_lift
  intro k
  exact leOfHom (h k)

/-- Every component of the tuple-overlap lift is the supplied chart map. -/
theorem canonicalTupleOverlapLift_comp_chart
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    {n : Nat} (σ : Fin (n + 1) → 𝒰.Index) {X : S.category}
    (h : ∀ k, X ⟶ Site.ContextCategoryObject.of S.contextPreorder (𝒰.patch (σ k)))
    (k : Fin (n + 1)) :
    canonicalTupleOverlapLift 𝒰 σ h ≫
        canonicalTupleOverlapProjection 𝒰 σ k = h k :=
...
-/
noncomputable def canonicalTupleOverlapMap
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
...
/-- The overlap map of the identity simplex morphism is the identity. -/
@[simp] theorem canonicalTupleOverlapMap_id
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    (x : SimplexCategory)
    (σ : Fin (x.len + 1) → 𝒰.Index) :
    canonicalTupleOverlapMap 𝒰 (𝟙 x) σ = 𝟙 _ :=
  Subsingleton.elim _ _

/-- Overlap maps compose contravariantly with simplex morphisms. -/
theorem canonicalTupleOverlapMap_comp
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    {x y z : SimplexCategory} (f : x ⟶ y) (g : y ⟶ z)
    (σ : Fin (z.len + 1) → 𝒰.Index) :
    canonicalTupleOverlapMap 𝒰 (f ≫ g) σ =
