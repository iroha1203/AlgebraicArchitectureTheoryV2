...
The site-equation standard route starts instead from `S.equationSystem`, its
canonical global-section realization, and the generated witness ideals.  It
transports that reading, generated ideal sheaf, and lawful closed subscheme
without accepting a semantic-core scheme bridge.
...
section SiteEquationCoefficientChange

variable {kSite' : Type u} [CommRing kSite']
variable [HasSheafify S.topology (AATCommAlgCat kSite')]

/--
The equation generated by the site-owned equation system after the canonical
coefficient-change projection.
-/
noncomputable def baseChangedSiteEquationGlobalEquation
    (W : S.category)
    (siteRaw :
    ...
    (f : FlatCoefficientChange
      (S.equationSystem.Observable W) kSite')
    [S.topology.HasSheafCompose
      (f.coefficientExtension :
        AATCommAlgCat.{u, u} (S.equationSystem.Observable W) ⥤
          AATCommAlgCat.{u, u} kSite')]
    (i : S.equationSystem.Index) (a : U.Atom) :
    Γ((Y.baseChange siteRaw f).underlying, ⊤) :=
  (Y.baseChangeMap siteRaw f).appTop
    (siteEquationGlobalEquation W siteRaw Y hcontext i a)

/--
The coefficient-changed closed-equational reading generated from the
site-owned equation system.  Its source reading and transported equations
are canonical; no scheme bridge is supplied.
-/
noncomputable def ClosedEquationalLawReading.baseChangeOfSiteEquationSystem
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
    ...
    ClosedEquationalLawReading
      ...
      S.equationSystem where
  geometric := {
    HoldsOn := fun s i =>
      GlobalEquationsVanishAlong
        (siteRaw.baseChange f.hom) (Y.baseChange siteRaw f)
        (baseChangedSiteEquationGlobalEquation
          W siteRaw Y hcontext f i) s
  }
  closed := Set.univ
  selected := fun _ => Set.univ
  witness := fun i _ =>
    ClosedEquationalLawWitness.ofGlobalSections
      (siteRaw.baseChange f.hom) (Y.baseChange siteRaw f) i
      (baseChangedSiteEquationGlobalEquation W siteRaw Y hcontext f i)

/-- Transported site-equation vanishing is source vanishing after the actual projection. -/
theorem ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_geometric_iff
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
    ...
    [S.topology.HasSheafCompose
      (f.coefficientExtension :
        AATCommAlgCat.{u, u} (S.equationSystem.Observable W) ⥤
          AATCommAlgCat.{u, u} kSite')]
    {T : Scheme} (s : T ⟶ (Y.baseChange siteRaw f).underlying)
    (i : S.equationSystem.Index) :
    (ClosedEquationalLawReading.baseChangeOfSiteEquationSystem
      W siteRaw Y hcontext f).geometric.HoldsOn s i ↔
      (ClosedEquationalLawReading.ofSiteEquationSystem
        W siteRaw Y hcontext).geometric.HoldsOn
          (s ≫ Y.baseChangeMap siteRaw f) i :=
  Iff.rfl

/-- The coefficient-changed site-equation reading satisfies the recognition laws. -/
theorem ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_valid
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
  ...
  witness_compatible := by
    intro i hi
    exact ClosedEquationalLawWitness.ofGlobalSections_valid
      (siteRaw.baseChange f.hom) (Y.baseChange siteRaw f) i _
  selected_closed := fun _ i _ => Set.mem_univ i
  selected_basicOpen := fun _ _ i =>
    iff_of_true (Set.mem_univ i) (Set.mem_univ i)

/-- Every required transported site equation is closed and selected. -/
theorem ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_requiredClosed
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
...
/-- Every transported site equation is selected. -/
theorem ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_allLawsSelected
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
...
end SiteEquationCoefficientChange
...
section SiteEquationIdealTransport

variable {kSite' : Type u} [CommRing kSite']
variable [HasSheafify S.topology (AATCommAlgCat kSite')]

/--
Each required witness ideal sheaf generated by the site-owned equation system
pulls back to the witness ideal sheaf generated by the transported equation.
-/
theorem siteEquationWitnessIdealSheaf_baseChange
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
    ...
    let targetValid :=
      ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_valid
        W siteRaw Y hcontext f
    (siteEquationWitnessIdealSheaf W siteRaw Y hcontext i).comap
        (Y.baseChangeMap siteRaw f) =
      lawWitnessIdealSheaf
        (siteRaw.baseChange f.hom) (Y.baseChange siteRaw f)
        targetReading targetValid.witness_compatible
        i.1 (Set.mem_univ i.1) := by
  dsimp only
  rw [siteEquationWitnessIdealSheaf_eq_ofIdealTop_map]
  rw [ofIdealTop_comap_baseChangeMap siteRaw Y]
  rw [lawWitnessIdealSheaf_ofGlobalSections
    (siteRaw.baseChange f.hom) (Y.baseChange siteRaw f)
    (ClosedEquationalLawReading.baseChangeOfSiteEquationSystem
      W siteRaw Y hcontext f)
    (ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_valid
      W siteRaw Y hcontext f).witness_compatible
    i.1 (Set.mem_univ i.1)
    (baseChangedSiteEquationGlobalEquation W siteRaw Y hcontext f i.1)
    rfl]
  congr 1
  rw [S.equationSystem.witnessIdeal_eq_span,
    Ideal.map_map, Ideal.map_span]
  congr 1
  ext x
  constructor
  · rintro ⟨_, ⟨a, rfl⟩, rfl⟩
  ...
  · rintro ⟨a, rfl⟩
    exact
      ⟨S.equationSystem.violationCoordinate W i.1 a,
        ⟨a, rfl⟩, rfl⟩

/--
The required generated ideal sheaf of the site-owned equation system pulls
back to the generated ideal sheaf of its transported equations.
-/
theorem siteEquationGeneratedIdealSheaf_baseChange
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
    ...
    let targetValid :=
      ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_valid
        W siteRaw Y hcontext f
    let targetRequiredClosed :=
      ClosedEquationalLawReading.baseChangeOfSiteEquationSystem_requiredClosed
        W siteRaw Y hcontext f
    (siteEquationGeneratedIdealSheaf W siteRaw Y hcontext).comap
        (Y.baseChangeMap siteRaw f) =
      lawGeneratedIdealSheaf
        (siteRaw.baseChange f.hom) (Y.baseChange siteRaw f)
        targetReading targetValid targetRequiredClosed := by
  dsimp only
  rw [siteEquationGeneratedIdealSheaf]
  rw [lawGeneratedIdealSheaf_eq_iSup_required siteRaw Y
    (ClosedEquationalLawReading.ofSiteEquationSystem
  ...
  congr 1
  funext i
  exact siteEquationWitnessIdealSheaf_baseChange
    W siteRaw Y hcontext f i

/--
The canonical map from the coefficient-changed lawful closed subscheme to
the lawful closed subscheme generated by the site-owned equation system.
-/
noncomputable def siteEquationLawfulClosedSubschemeBaseChangeMap
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
  ...
  Scheme.IdealSheafData.subschemeMap
    ...
    (Scheme.IdealSheafData.le_map_iff_comap_le.mpr
      (le_of_eq
        (siteEquationGeneratedIdealSheaf_baseChange
          W siteRaw Y hcontext f)))

/-- The generated closed-subscheme map lies over the coefficient-change projection. -/
@[reassoc] theorem siteEquationLawfulClosedSubschemeBaseChangeMap_immersion
    (W : S.category)
    (siteRaw :
      RawAmbientRestrictionSystem S (S.equationSystem.Observable W))
    [HasSheafify S.topology
    ...
    [S.topology.HasSheafCompose
      (f.coefficientExtension :
        AATCommAlgCat.{u, u} (S.equationSystem.Observable W) ⥤
          AATCommAlgCat.{u, u} kSite')] :
    siteEquationLawfulClosedSubschemeBaseChangeMap
        W siteRaw Y hcontext f ≫
      siteEquationLawfulClosedImmersion W siteRaw Y hcontext =
