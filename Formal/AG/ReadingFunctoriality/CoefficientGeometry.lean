...
change.  The representing equivalence must be natural and its universal point
must be the pullback of the source universal point along the concrete
coefficient-change morphism.
-/
structure BaseChangeRepresentation
  ...
  representingEquiv_natural :
    ∀ {T T' : AlgebraicGeometry.Scheme.{max u v}}
      ...
      representingEquiv T' (g ≫ s) =
        EquationArchitecturePoint.pullback g (representingEquiv T s)
  /-- The represented universal point is the actual coefficient pullback. -/
  universalPoint :
    representingEquiv (X.baseChange raw f).underlying
...
theorem baseChange_valid
  ...
  EquationObservableRealization.ofRepresentingEquiv_valid
    R.reading P.representingEquiv P.representingEquiv_natural
