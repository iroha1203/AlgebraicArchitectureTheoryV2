...
noncomputable def weakLawEquationCore :
  ...
  Index := RingedSite.FiniteModel.site.equationSystem.Index
  role := RingedSite.FiniteModel.site.equationSystem.role
  ...
  equationResidual W A _i _a :=
    (AAT.AG.FiniteModel.noCycleResidual A : rawSystem.rawAlgebra W)
  equationResidual_restrict := by
    intro source target f A i a
    rw [map_intCast]
...
theorem weakCore_witnessIdeal_reflected
    ...
    Ideal.comap (weakSchemeBridge.toSheafifiedSection W)
        (Ideal.map (weakSchemeBridge.toSheafifiedSection W)
          (weakLawEquationCore.witnessIdeal W i)) =
      weakLawEquationCore.witnessIdeal W i :=
...
private theorem weak_componentA_raw_equation :
    weakLawEquationCore.violationCoordinate base PUnit.unit FiniteAtom.componentA =
...
private theorem strong_componentA_raw_equation :
    strongLawEquationCore.violationCoordinate base PUnit.unit FiniteAtom.componentA =
      ((rawSystem.relationFamily base).quotientMap (MvPolynomial.X ()) - 1 :
        rawSystem.rawAlgebra base) := by
...
private theorem strong_componentB_raw_equation :
    strongLawEquationCore.violationCoordinate base PUnit.unit FiniteAtom.componentB =
      ((rawSystem.relationFamily base).quotientMap (MvPolynomial.X ()) + 1 :
        rawSystem.rawAlgebra base) := by
  simp only [strongLawEquationCore]
  rw [orientedRawCoordinate, gauge_base]
  ...
  change
    (AATReadingDecoration.ofContext rawSystem base).interpretation
      (weakSchemeBridge.toSheafifiedSection base
        (weakLawEquationCore.violationCoordinate base PUnit.unit
  ...
  change
    (AATReadingDecoration.ofContext rawSystem base).interpretation
      (strongSchemeBridge.toSheafifiedSection base
        (strongLawEquationCore.violationCoordinate base PUnit.unit
          FiniteAtom.componentA)) =
      (AATReadingDecoration.ofContext rawSystem base).coordinateSection
        rawSystem () - 1
...
theorem weakCore_leftChart_provenance_fires
    ...
    ((twoChartReferenceModel.atlas.chart leftIndex).map.appTop ≫
      ...
      weakSchemeBridge.toSheafifiedSection
        ...
        (weakLawEquationCore.violationCoordinate
...
theorem weakCore_leftChart_witnessIdeal_realization_fires :
    Ideal.map
        (weakSchemeBridge.toSheafifiedSection
          (twoChartReferenceModel.atlas.chart leftIndex).context)
        (weakLawEquationCore.witnessIdeal
...
private theorem finiteCore_lawIdealExact
    (G : ArchitecturalEquationSystem
      RingedSite.FiniteModel.site.contextPreorder)
...
theorem weakReading_requiredLawIdealExact :
    RequiredLawIdealExact rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed := by
  intro i hi
  cases i
  intro T s
...
theorem strongReading_requiredLawIdealExact :
    RequiredLawIdealExact rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed := by
  intro i hi
  cases i
  intro T s
  simpa only [twoChart_underlying] using
...
def weakToStrong :
    ClosedEquationalLawInclusion rawSystem twoChartReferenceModel weakReading strongReading where
  lawMap i := i
  atomMap _ a :=
    match a with
...
/-- The raw weak equation has the same NoCycle fulfillment as the selected site equation. -/
theorem weakLawEquationCore_equationHolds_iff_noCycleLaw
    (A : ArchitectureObject carrier) :
    weakLawEquationCore.EquationHolds PUnit.unit A ↔ noCycleLaw.holds A := by
  constructor
  · intro hequation
    apply (AAT.AG.FiniteModel.noCycleLaw_holds_iff A).mpr
    intro hcycle
    have hzero := hequation base FiniteAtom.componentA
    change (AAT.AG.FiniteModel.noCycleResidual A :
      rawSystem.rawAlgebra base) = 0 at hzero
    have hone : (1 : rawSystem.rawAlgebra base) = 0 := by
      simpa [AAT.AG.FiniteModel.noCycleResidual, hcycle] using hzero
    have honeInt := congrArg rawEvalOne hone
    norm_num at honeInt
  · intro h W atom
    have hnoCycle :=
      (AAT.AG.FiniteModel.noCycleLaw_holds_iff A).mp h
    have hzero : AAT.AG.FiniteModel.noCycleResidual A = 0 := by
      ...
      simpa [AAT.AG.FiniteModel.equationSystem] using
        (RingedSite.FiniteModel.site_equationHolds_iff_noCycle A).mpr
          hnoCycle W atom
    change (AAT.AG.FiniteModel.noCycleResidual A :
...
private noncomputable def weakLawSignatureComparison :
  ...
  axisOf _ atom := {
    equation := ⟨PUnit.unit, rfl⟩
    atom := atom
  }
  axis_selected := by intros; trivial
  selected_axis_covered := by
    intro axis _
    rcases axis with ⟨⟨index, hrequired⟩, atom⟩
    cases index
    exact ⟨⟨PUnit.unit, rfl⟩, atom, rfl⟩
  residual_zero_iff_axis_zero := by
    intro A index atom
    rcases index with ⟨index, hrequired⟩
    cases index
    change
      (∀ W : RingedSite.FiniteModel.site.category,
        (AAT.AG.FiniteModel.noCycleResidual A :
          rawSystem.rawAlgebra W) = 0) ↔
      (∀ _W : Site.ContextCategoryObject
          (Site.contextMorphismPreorderCategory AAT.AG.FiniteModel.object),
        AAT.AG.FiniteModel.noCycleResidual A = 0)
    constructor
    · intro hsource _W
      have hbase := hsource base
      have hmapped := congrArg rawEvalOne hbase
      simpa using hmapped
    · intro htarget W
      have hzero := htarget
        (Site.ContextCategoryObject.of
          (Site.contextMorphismPreorderCategory AAT.AG.FiniteModel.object)
          AAT.AG.FiniteModel.equationProbeContext)
      change (AAT.AG.FiniteModel.noCycleResidual A :
        rawSystem.rawAlgebra W) = 0
      rw [hzero]
      simp

/-- The count valuation is sound for the weak equation-generated legacy display. -/
theorem weakLawEquationCore_obstructionSound :
    ObstructionSound noCycleValuation
      (weakLawEquationCore.toLegacyLawUniverse.law PUnit.unit) := by
  intro A hholds
  exact noCycleSound A
    ((weakLawEquationCore_equationHolds_iff_noCycleLaw A).mp hholds)

/-- The count valuation is complete for the weak equation-generated legacy display. -/
theorem weakLawEquationCore_obstructionComplete :
    ObstructionComplete noCycleValuation
      (weakLawEquationCore.toLegacyLawUniverse.law PUnit.unit) := by
  intro A hnot
  apply noCycleComplete A
  intro hholds
  exact hnot ((weakLawEquationCore_equationHolds_iff_noCycleLaw A).mpr hholds)
...
private theorem weak_local_top_eq_span
  ...
  rw [← lawWitnessIdealSheaf_ideal rawSystem twoChartReferenceModel weakReading weakReading_valid.witness_compatible PUnit.unit hi
    ambientTopAffineOpen]
  rw [lawWitnessIdealSheaf_ofGlobalSections rawSystem twoChartReferenceModel weakReading weakReading_valid.witness_compatible PUnit.unit hi
    (semanticCoreGlobalEquation rawSystem twoChartReferenceModel
...
private theorem weak_generated_top_eq_span :
    (lawGeneratedIdealSheaf rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed).ideal
        ambientTopAffineOpen =
      Ideal.span (Set.range (semanticCoreGlobalEquation rawSystem
        twoChartReferenceModel weakLawEquationCore weakSchemeBridge PUnit.unit)) := by
...
private theorem strong_local_top_eq_span
  ...
  rw [← lawWitnessIdealSheaf_ideal rawSystem twoChartReferenceModel strongReading strongReading_valid.witness_compatible PUnit.unit hi
    ambientTopAffineOpen]
  rw [lawWitnessIdealSheaf_ofGlobalSections rawSystem twoChartReferenceModel strongReading strongReading_valid.witness_compatible PUnit.unit hi
    (semanticCoreGlobalEquation rawSystem twoChartReferenceModel
...
private theorem strong_generated_top_eq_span :
    (lawGeneratedIdealSheaf rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed).ideal
        ambientTopAffineOpen =
      Ideal.span (Set.range (semanticCoreGlobalEquation rawSystem
        twoChartReferenceModel strongLawEquationCore strongSchemeBridge PUnit.unit)) := by
  rw [lawGeneratedIdealSheaf_ideal]
...
theorem weak_ideal_lt_strong :
    lawGeneratedIdealSheaf rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed <
      lawGeneratedIdealSheaf rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed := by
...
private theorem integerTestPoint_idealLawful :
  ...
  (witnessVanishes_iff_idealLawfulAlong rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed integerTestPoint).mp
...
private theorem modTwoTestPoint_idealLawful :
  ...
  (witnessVanishes_iff_idealLawfulAlong rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed
    modTwoTestPoint).mp
    ((semanticLawfulAlong_iff_witnessVanishes rawSystem
...
theorem weakSubscheme_nonempty :
    Nonempty (lawfulClosedSubscheme rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed) := by
...
theorem strongSubscheme_nonempty :
    Nonempty (lawfulClosedSubscheme rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed) := by
...
theorem weakImmersion_not_isIso :
    ¬ IsIso (lawfulClosedImmersion rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed) := by
  intro hIso
  letI : IsIso (lawfulClosedImmersion rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed) := hIso
  apply negativeOneTestPoint_not_weak_factors
  exact ⟨⟨negativeOneTestPoint ≫
      inv (lawfulClosedImmersion rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed), by simp⟩⟩

/-- The strong closed immersion excludes the concrete integer coordinate-one point. -/
theorem strongImmersion_not_isIso :
    ¬ IsIso (lawfulClosedImmersion rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed) := by
  intro hIso
  letI : IsIso (lawfulClosedImmersion rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed) := hIso
  apply integerTestPoint_not_strong_factors
  exact ⟨⟨integerTestPoint ≫
      inv (lawfulClosedImmersion rawSystem twoChartReferenceModel strongReading strongReading_valid strongReading_requiredClosed), by simp⟩⟩
...
theorem weakToStrongAllMap_not_isIso :
  ...
  let comparison := allLawfulClosedSubschemeMap rawSystem
    ...
    weakToStrong weakToStrong_valid
  letI : IsIso comparison := hIso
  let weakLift := allLawFactorizationLift rawSystem twoChartReferenceModel weakReading weakReading_valid integerTestPoint
    integerTestPoint_fullWeakIdealLawful
  apply integerTestPoint_not_strong_all_factors
  refine ⟨⟨weakLift ≫ inv comparison, ?_⟩⟩
  calc
    (weakLift ≫ inv comparison) ≫
        allLawfulClosedImmersion rawSystem twoChartReferenceModel strongReading strongReading_valid =
      (weakLift ≫ inv comparison) ≫
...
noncomputable def requiredAllLawEquationCore :
    ArchitecturalEquationSystem requiredAllSite.contextPreorder where
  Index := requiredAllSite.equationSystem.Index
  role := requiredAllSite.equationSystem.role
  ...
  equationResidual W A i _a :=
    (requiredAllResidual i A : requiredAllRawSystem.rawAlgebra W)
  equationResidual_restrict := by
    intro source target f A i a
    rw [map_intCast]
...
private theorem requiredLaw_componentA_raw_equation :
    requiredAllLawEquationCore.violationCoordinate base .base
        FiniteAtom.componentA =
...
private theorem strengtheningLaw_componentB_raw_equation :
    requiredAllLawEquationCore.violationCoordinate base .strengthening
        FiniteAtom.componentB =
      ((requiredAllRawSystem.relationFamily base).quotientMap
          (MvPolynomial.X ()) + 1 :
        requiredAllRawSystem.rawAlgebra base) := by
...
noncomputable def requiredAllReading :
    ClosedEquationalLawReading requiredAllRawSystem requiredAllReferenceModel requiredAllLawEquationCore :=
...
theorem requiredAllReading_allLawsSelected :
    AllLawsSelected requiredAllRawSystem requiredAllReferenceModel requiredAllReading :=
...
private theorem required_ideal_top_le_ker_globalEvalOne :
    (lawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid
...
theorem required_ideal_lt_all_ideal :
    lawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid
      ...
      allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid := by
  refine lt_of_le_of_ne
  ...
  have hfull :
      semanticCoreGlobalEquation requiredAllRawSystem requiredAllReferenceModel
          requiredAllLawEquationCore requiredAllSchemeBridge .strengthening
          FiniteAtom.componentB ∈
        (allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid).ideal
            ambientTopAffineOpen := by
...
theorem fullToRequiredLawfulMap_not_isIso :
  ...
  have hker := Scheme.Hom.ker_comp_of_isIso comparison
    (lawfulClosedImmersion requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid
      requiredAllReading_requiredClosed)
  have heq :
      allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid =
        lawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid
          requiredAllReading_requiredClosed := by
    calc
      allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid =
        (allLawfulClosedImmersion requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid).ker :=
      ...
      _ = (lawfulClosedImmersion requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid
          requiredAllReading_requiredClosed).ker := hker
      _ = lawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid
          requiredAllReading_requiredClosed :=
...
private theorem all_ideal_top_le_ker_globalEvalModTwo :
    (allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid).ideal ambientTopAffineOpen ≤
        RingHom.ker requiredAllGlobalEvalModTwo := by
  rw [allLawGeneratedIdealSheaf_ideal]
  refine iSup_le fun i => ?_
  rw [requiredAll_local_top_eq_span]
...
private theorem strengthening_equation_mem_all_top :
    semanticCoreGlobalEquation requiredAllRawSystem requiredAllReferenceModel
        requiredAllLawEquationCore requiredAllSchemeBridge .strengthening
        FiniteAtom.componentB ∈
      (allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid).ideal ambientTopAffineOpen := by
  have hlocal :
      semanticCoreGlobalEquation requiredAllRawSystem requiredAllReferenceModel
          requiredAllLawEquationCore requiredAllSchemeBridge .strengthening
          FiniteAtom.componentB ∈
...
private theorem all_ideal_le_selectedModTwoPoint_ker :
    allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid ≤ selectedModTwoPoint.ker := by
  haveI : IsAffine requiredAllReferenceModel.underlying := by
    rw [requiredAllReferenceModel_underlying, twoChart_underlying]
    infer_instance
  have hring :
...
theorem selected_point_not_factors_all :
  ...
  rintro ⟨t⟩
  have hle :
      allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid ≤ selectedRequiredPoint.ker := by
    rw [← t.2]
    calc
      allLawGeneratedIdealSheaf requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid =
        (allLawfulClosedImmersion requiredAllRawSystem requiredAllReferenceModel requiredAllReading requiredAllReading_valid).ker :=
...
theorem integerPoint_objectComparison :
    RequiredObjectPointComparison rawSystem twoChartReferenceModel weakReading integerPoint
  ...
  · intro _
    ...
    apply (AAT.AG.FiniteModel.noCycleLaw_holds_iff _).mpr
...
theorem integerPoint_objectComparison_fails_for_cyclic :
  ...
  exact (AAT.AG.FiniteModel.noCycleLaw_holds_iff _).mp
...
theorem integerPoint_omega_fires :
  ...
  exact semanticLawfulAlong_iff_omegaU_zero rawSystem twoChartReferenceModel weakReading integerPoint
    AAT.AG.LawAlgebra.FiniteExamples.CycleCorrespondenceExample.acyclicObject
    ...
    (fun index => by
      ...
      | mk index _ =>
          cases index
          exact weakLawEquationCore_obstructionSound)
    (fun index => by
      cases index with
      | mk index _ =>
          cases index
          exact weakLawEquationCore_obstructionComplete)
...
theorem integerPoint_not_semanticLawful_strong :
    ¬ SemanticLawfulAlong rawSystem twoChartReferenceModel strongReading integerPoint := by
...
theorem integerPoint_fullySemanticLawful_weak :
    FullySemanticLawfulAlong rawSystem twoChartReferenceModel weakReading integerPoint := by
...
theorem integerPoint_not_fullySemanticLawful_strong :
    ¬ FullySemanticLawfulAlong rawSystem twoChartReferenceModel strongReading integerPoint := by
  intro hfull
...
theorem weak_correspondence_fires
  ...
  lawfulnessIdealFactorizationCorrespondence rawSystem twoChartReferenceModel weakReading weakReading_valid weakReading_requiredClosed
...
private theorem restrictionBroken_globalEquation_eq_weak :
  ...
  change
    twoChartReferenceModel.decoration.interpretation
        (restrictionBrokenSchemeBridge.toSheafifiedSection base
          (weakLawEquationCore.violationCoordinate base PUnit.unit a)) =
      twoChartReferenceModel.decoration.interpretation
        (weakSchemeBridge.toSheafifiedSection base
          (weakLawEquationCore.violationCoordinate base PUnit.unit a))
  rw [SemanticLawEquationSchemeBridge.toSheafifiedSection,
...
private theorem restrictionBroken_left_rawIdeal_eq
    ...
    weakLawEquationCore.witnessIdeal RawPresheaf.left PUnit.unit =
      Ideal.map leftRawSignFlip
        (weakLawEquationCore.witnessIdeal RawPresheaf.left PUnit.unit) := by
  let brokenReading : ClosedEquationalLawReading rawSystem
      twoChartReferenceModel weakLawEquationCore :=
    ClosedEquationalLawReading.ofSemanticCore rawSystem
      twoChartReferenceModel weakLawEquationCore restrictionBrokenSchemeBridge
  ...
  have hsheaf :
    ...
    rw [lawWitnessIdealSheaf_ofGlobalSections rawSystem twoChartReferenceModel brokenReading brokenCompatible PUnit.unit brokenClosed
      (semanticCoreGlobalEquation rawSystem twoChartReferenceModel
        weakLawEquationCore restrictionBrokenSchemeBridge PUnit.unit)]
    ...
    Ideal.map gammaInv
      ...
      Ideal.map gammaInv
        (Ideal.map (weakSchemeBridge.toSheafifiedSection RawPresheaf.left)
          (weakLawEquationCore.witnessIdeal RawPresheaf.left PUnit.unit)) at houter
    ...
    Ideal.map canonical
      ...
      Ideal.map canonical
        (Ideal.map (RingEquiv.refl _).toRingHom
          (weakLawEquationCore.witnessIdeal RawPresheaf.left PUnit.unit)) at hinner'
  have hraw := congrArg
...
private theorem weak_componentA_left_raw_equation :
    weakLawEquationCore.violationCoordinate RawPresheaf.left PUnit.unit
        FiniteAtom.componentA =
      (-(rawSystem.relationFamily RawPresheaf.left).quotientMap
...
private theorem restrictionBroken_left_mappedIdeal_le_evalKer :
  ...
  change RawPresheaf.quotientOneEval
      (leftRawSignFlip
        (weakLawEquationCore.violationCoordinate RawPresheaf.left PUnit.unit a)) = 0
  cases a
...
theorem restrictionBrokenSchemeBridge_not_realized :
  ...
  let generator := weakLawEquationCore.violationCoordinate RawPresheaf.left
...
noncomputable def baseChangeBrokenGeometricReading :
    GeometricLawReading rawSystem twoChartReferenceModel weakLawEquationCore where
  HoldsOn s _ := IsIso s

/-- Exhibits the base-change failure of the broken geometric reading. -/
theorem baseChangeBrokenGeometricReading_not_valid :
    ¬ IsGeometricLawReading rawSystem twoChartReferenceModel baseChangeBrokenGeometricReading := by
  intro hvalid
...
noncomputable def baseChangeBrokenReading :
    ClosedEquationalLawReading rawSystem twoChartReferenceModel weakLawEquationCore :=
...
theorem baseChangeBrokenReading_witnessCompatible :
    IsClosedEquationalWitnessReading rawSystem twoChartReferenceModel baseChangeBrokenReading := by
...
theorem baseChangeBrokenReading_not_valid :
    ¬ IsClosedEquationalLawReading rawSystem twoChartReferenceModel baseChangeBrokenReading := by
  intro hvalid
...
theorem missingRequiredReading_valid :
    IsClosedEquationalLawReading rawSystem twoChartReferenceModel missingRequiredReading where
  geometric_stable := weakGeometricReading_valid
...
theorem missingRequiredReading_not_allLawsSelected :
    ¬ AllLawsSelected rawSystem twoChartReferenceModel missingRequiredReading := by
  intro hall
...
theorem restrictionBrokenSelectionReading_not_valid :
    ¬ IsClosedEquationalLawReading rawSystem twoChartReferenceModel restrictionBrokenSelectionReading := by
  intro hvalid
...
theorem missingRequiredSelectionReading_valid :
    IsClosedEquationalLawReading rawSystem twoChartReferenceModel missingRequiredSelectionReading where
  geometric_stable := weakGeometricReading_valid
  witness_compatible := by
...
theorem missingRequiredSelectionReading_not_requiredClosed :
    ¬ RequiredClosed rawSystem twoChartReferenceModel missingRequiredSelectionReading := by
  intro hrequired
...
theorem semanticMismatchReading_valid :
    IsClosedEquationalLawReading rawSystem twoChartReferenceModel semanticMismatchReading where
  geometric_stable := by
    intro T T' s f i hs
    exact hs.elim
  witness_compatible := by
...
theorem semanticMismatchReading_witnessCompatible :
    IsClosedEquationalWitnessReading rawSystem twoChartReferenceModel semanticMismatchReading :=
...
theorem semanticMismatchReading_allLawsSelected :
    AllLawsSelected rawSystem twoChartReferenceModel semanticMismatchReading where
...
theorem semanticMismatchReading_not_exact :
    ¬ RequiredLawIdealExact rawSystem twoChartReferenceModel semanticMismatchReading semanticMismatchReading_valid
      semanticMismatchReading_requiredClosed := by
  intro hexact
  have hideal :
      (lawWitnessIdealSheaf rawSystem twoChartReferenceModel semanticMismatchReading semanticMismatchReading_witnessCompatible
        PUnit.unit
...
theorem semanticMismatchReading_not_lawIdealExact :
    ¬ LawIdealExact rawSystem twoChartReferenceModel semanticMismatchReading semanticMismatchReading_witnessCompatible
      PUnit.unit
      (semanticMismatchReading_requiredClosed.closed PUnit.unit
        (RingedSite.FiniteModel.site_equation_required PUnit.unit)) := by
  intro hexact
...
theorem semanticMismatchReading_not_complete :
    ¬ LawIdealComplete rawSystem twoChartReferenceModel semanticMismatchReading semanticMismatchReading_witnessCompatible
      PUnit.unit
      (semanticMismatchReading_requiredClosed.closed PUnit.unit
        (RingedSite.FiniteModel.site_equation_required PUnit.unit)) := by
  intro hcomplete
...
theorem semanticMismatch_full_correspondence_fails :
    ¬ (FullySemanticLawfulAlong rawSystem twoChartReferenceModel semanticMismatchReading integerPoint ↔
        FullIdealLawfulAlong rawSystem twoChartReferenceModel semanticMismatchReading semanticMismatchReading_valid
          integerPoint) := by
  intro hcorrespondence
  have hideal : FullIdealLawfulAlong rawSystem twoChartReferenceModel semanticMismatchReading semanticMismatchReading_valid integerPoint := by
...
theorem semanticOverclaimReading_valid :
    IsClosedEquationalLawReading rawSystem twoChartReferenceModel semanticOverclaimReading where
  geometric_stable := by
    intro T T' s f i hs
    trivial
  witness_compatible := by
...
theorem semanticOverclaimReading_witnessCompatible :
    IsClosedEquationalWitnessReading rawSystem twoChartReferenceModel semanticOverclaimReading :=
...
noncomputable def restrictionBrokenWitness :
    ClosedEquationalLawWitness rawSystem twoChartReferenceModel weakLawEquationCore PUnit.unit where
  coordinate V _ := by
    classical
...
theorem restrictionBrokenReading_not_witnessCompatible :
    ¬ IsClosedEquationalWitnessReading rawSystem twoChartReferenceModel restrictionBrokenReading := by
  intro hcompatible
...
theorem restrictionBrokenReading_not_valid :
    ¬ IsClosedEquationalLawReading rawSystem twoChartReferenceModel restrictionBrokenReading := by
  intro hvalid
