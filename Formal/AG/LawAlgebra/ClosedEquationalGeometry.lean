...
generated from `E.witnessIdeal` at that chart's context.
-/
noncomputable def chartWitnessIdeal
  ...
  Ideal.map (X.atlas.chart j).map.appTop.hom
    (R.contextWitnessIdeal (X.atlas.chart j).context i)

/-- The selected pair-context Scheme for two affine charts. -/
noncomputable abbrev overlapChart
    (R : EquationObservableRealization raw X E)
    (j l : X.atlas.Index) : AlgebraicGeometry.Scheme :=
  architectureChartSpec raw (X.atlas.pairContext raw j l)

/-- The actual selected morphism from a pair-context chart to its left chart. -/
noncomputable def overlapToLeft
    (j l : X.atlas.Index) :
    R.overlapChart j l ⟶ (X.atlas.chart j).domain :=
  architectureChartRestriction raw (X.atlas.pairToLeft raw j l)

/-- The actual selected morphism from a pair-context chart to its right chart. -/
noncomputable def overlapToRight
    (j l : X.atlas.Index) :
    R.overlapChart j l ⟶ (X.atlas.chart l).domain :=
  architectureChartRestriction raw (X.atlas.pairToRight raw j l)
...
structure EquationChartLocalization
  ...
  /--
  The ambient generated ideal restricts to the ideal generated by the
  equation system at each selected chart context.
  -/
...
theorem overlapWitnessIdeal_left_eq_right
  ...
  rw [overlapWitnessIdealLeft, overlapWitnessIdealRight,
    ...
    ← L.globalWitnessIdeal_restricts l i,
    Ideal.map_map, Ideal.map_map]
  have h := congrArg (fun q => q.appTop)
    (X.atlas.overlap_commutes raw X.overlaps X.overlapsValid j l)
...
theorem equationIdealFactorizationCorrespondence
  ...
  ⟨R.equationHoldsAlong_iff_witnessIdeal hR s i,
    R.equationHoldsAlong_iff_nonempty_factorsThrough hR s i,
    R.equationWitnessChartRealized L i⟩
