...
quotient of the equation system, and the two carriers are definitionally
distinct types (`Finsupp` quotient vs `Ideal.Quotient`), so the comparison
`Φ` produced by presentation exactness cannot reduce to `AddEquiv.refl`
(the #3718 negative condition).
...
(the empty-overlap normalization).
-/
...
/-- 例10.2 / X.系6.7: `Φ` generated by exactness discharge, with
definitionally distinct carriers (`Finsupp` presentation quotient vs
equation obstruction quotient), so it cannot be `AddEquiv.refl`
(#3718 negative condition). -/
