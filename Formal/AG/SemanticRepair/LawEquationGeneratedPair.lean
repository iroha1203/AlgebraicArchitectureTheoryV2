...
Unlike the generic defect source, the chart index and chart object are fixed
by the cover geometry.  Consequently the body Cech source is generated from
this one material input rather than accepted as a separate premise.
-/
structure FinitePosetLawEquationDefectSourceBody
  ...
  lawSupport_nonempty :
    forall i, exists lawIndex : G.equationSystem.Index,
      lawIndex ∈ lawSupport i (input i)
  lawSupport_required :
    forall i (lawIndex : G.equationSystem.Index),
      lawIndex ∈ lawSupport i (input i) -> G.equationSystem.Required lawIndex
