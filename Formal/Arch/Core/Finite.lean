...
/-- Ordered component-pair witnesses generated by a finite component universe. -/
def componentPairWitnesses {C : Type u} {G : ArchGraph C}
    (U : ComponentUniverse G) : List (C × C) :=
  ArchitectureSignature.componentPairs U.components

/-- Component pairs required by the finite universe are exactly pairs in it. -/
def ComponentPairRequired {C : Type u} {G : ArchGraph C}
    (U : ComponentUniverse G) : C × C -> Prop :=
  fun pair => pair.1 ∈ U.components ∧ pair.2 ∈ U.components

/--
Membership in the component-pair witness list is exactly membership of both
endpoints in the finite component universe.
-/
theorem mem_componentPairWitnesses_iff {C : Type u} {G : ArchGraph C}
    (U : ComponentUniverse G) {pair : C × C} :
    pair ∈ componentPairWitnesses U ↔ ComponentPairRequired U pair := by
  exact ArchitectureSignature.mem_componentPairs_iff

/--
The ordered component-pair witness list generated by a finite component
universe covers the corresponding required pair universe.
-/
theorem coversWitnesses_componentPairWitnesses {C : Type u} {G : ArchGraph C}
    (U : ComponentUniverse G) :
    CoversWitnesses (ComponentPairRequired U) (componentPairWitnesses U) := by
  intro pair hRequired
  exact (mem_componentPairWitnesses_iff U).mpr hRequired
