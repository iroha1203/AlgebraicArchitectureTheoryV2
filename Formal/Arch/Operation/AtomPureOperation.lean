...
/--
Pure Atom-AAT together with a surface-preserving atom presentation operation.

This is the Signature-free operation theorem package.  It proves preservation
of the selected pure atom surface; Signature-level operation packages can
consume it, but do not define it.
-/
structure AtomAxiomatizedPureOperationPackage
    (C : Type u) (E : Type v) (D : Type w) where
  surface : AATPureTheorySurface C E D
  operation : AtomPresentationOperation surface
...
namespace AtomAxiomatizedPureOperationPackage

/-- The packaged pure atom operation preserves the selected pure atom surface. -/
theorem preservesSurfaceInvariant
    {C : Type u} {E : Type v} {D : Type w}
    (pkg : AtomAxiomatizedPureOperationPackage C E D) :
    PreservesInvariant
      (AtomPresentationOperation.source (surface := pkg.surface))
      (AtomPresentationOperation.target (surface := pkg.surface))
      (AtomPresentationOperation.atomSurfacePresentedHolds pkg.surface)
      pkg.operation () := by
  exact AtomPresentationOperation.preservesSurfaceInvariant pkg.operation

/-- The packaged pure atom operation belongs to the atom-surface `Ops` family. -/
theorem ops_mem_surfaceInvariant
    {C : Type u} {E : Type v} {D : Type w}
    (pkg : AtomAxiomatizedPureOperationPackage C E D) :
    Ops
...
/-- The pure operation package remains independent of observation tooling. -/
theorem independent_of_observation
    {C : Type u} {E : Type v} {D : Type w}
    (pkg : AtomAxiomatizedPureOperationPackage C E D) :
    pkg.surface.noObservationDependency :=
  AATPureTheorySurface.independent_of_observation pkg.surface

/-- The pure operation package remains independent of SFT forecasting. -/
theorem independent_of_sft
    {C : Type u} {E : Type v} {D : Type w}
    (pkg : AtomAxiomatizedPureOperationPackage C E D) :
    pkg.surface.noSFTDependency :=
  AATPureTheorySurface.independent_of_sft pkg.surface

/-- The pure operation package remains independent of Signature packages. -/
...
end AtomAxiomatizedPureOperationPackage
