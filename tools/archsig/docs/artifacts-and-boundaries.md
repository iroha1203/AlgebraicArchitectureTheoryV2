...
この文書は `archsig` と `fieldsig` が出す主要 artifact と読み方の境界をまとめる。
...
FieldSig `intent-forecast` は AlignmentMap から `operation-support-estimate-v0`、`forecast-cone-skeleton-v0`、
...
FieldSig `sft-forecast` は Markdown PRD / Spec / Issue / AI proposal、GitHub Issue JSON、
AI proposal JSON から
