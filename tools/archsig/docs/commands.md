...
AAT Observable Bundle を出力または検査する。

```bash
cargo run --manifest-path tools/archsig/Cargo.toml -- aat-observable-bundle \
  --fixture \
  --out .archsig/aat/aat-observable-bundle.json
...
`aat-observable-bundle-v0` は ArchitectureObject / ComponentUniverse、ObstructionWitness、
TheoremBoundary / NonConclusion、ArchitectureOperation、Projection / Observation / LSP / DIP、
FeatureExtension、semantic diagram、state/effect law、repair / synthesis、analytic axis を
同じ bundle で保持する。validation は dangling review action refs、claim level、measurement
status、nonConclusions、deterministic / LLM / human / formal proof の責務境界を検査する。
これは AAT theorem proof でも extractor completeness でもない。
