...
reference; those entries are not accepted as law-surface/v0.5.2 bindings.
...
reference but cannot add a declared generator to the obstruction ideal. For Tor,
the shared law binding vocabulary (`square-free` plus `support` or
`cooccurrence`) selects the variable coordinates, while the observed Tor
artifact remains `tor` / `lawIdealGenerator`. Undeclared atom variables are
ignored for the law-coordinate projection; missing required observations make
the Tor measurement incomplete rather than measured zero. Law changes must be
made in the law surface rather than by editing support atoms.
