...
description: Create bounded archmap/v0.5.4 finite-poset-site artifacts from repository evidence.
---

# ArchMap Creater

Use this skill to create `archmap/v0.5.4` documents and their authoring
...
3. **Pass A Survey**
   ...
   - Each sub-agent reads every source in its chunk and emits one
     `archmap-candidate-packet/v0.5.4` with a `surveyRows` entry for every worklist
     row in the chunk.
...
7. **Coverage Ledger**
   - Generate `archmap-coverage-ledger/v0.5.4` from candidate packet survey rows.
