...
pub fn build_archsig_analysis_packet(
    ...
    let mut signature_axes = build_signature_axes(archmap, law_policy, &obstruction_circuits);
    let flatness_reading = build_flatness_reading(archmap, law_policy, &signature_axes);
    let mut repair_operation_candidates =
    ...
    let signature_distance_readings =
        build_signature_distance_readings(&signature_axes, law_policy, &part4_distance_foundation);
    attach_signature_distance_reading_refs(&mut signature_axes, &signature_distance_readings);
    ...
    promote_signature_geometry_supporting_distance(
        &mut part4_distance_foundation,
    ...
    let mut operation_deltas =
        build_operation_deltas(archmap, &repair_operation_candidates, &signature_axes);
    let operation_distance_readings = build_operation_distance_readings(
        &repair_operation_candidates,
        &operation_deltas,
        &signature_distance_readings,
        &part4_distance_foundation,
    );
    attach_operation_distance_refs(
        &mut repair_operation_candidates,
        &mut operation_deltas,
        &operation_distance_readings,
    );
    promote_operation_geometry_supporting_distance(
        &mut part4_distance_foundation,
        &operation_distance_readings,
    );
...
include!("analysis/distance/foundation.rs");
...
include!("analysis/distance/atom_configuration.rs");
    ...
    law_policy
        ...
        .map(|definition| {
            ...
            ArchSigSignatureAxisReadingV0 {
                ...
                value,
                signature_distance_reading_refs: Vec::new(),
...
include!("analysis/distance/signature.rs");
...
include!("analysis/distance/curvature.rs");
...
include!("analysis/distance/representation.rs");
...
include!("analysis/distance/operation.rs");
...
include!("analysis/distance/homotopy.rs");
...
include!("analysis/distance/diagnosis.rs");
...
include!("analysis/validation/mod.rs");
...
include!("analysis/validation/helpers.rs");
...
mod tests {
    ...
    #[test]
    fn signature_distance_readings_keep_axis_distance_and_gap_blockers() {
        let packet = static_archsig_analysis_packet();
        let reading = packet
            .signature_distance_readings
            .first()
            .expect("static fixture has signature distance reading");

        assert!(!reading.axis_distances.is_empty());
        assert!(reading.axis_distances.iter().all(|axis| {
            axis.rho_i
                .evaluator_basis_refs
                .iter()
                .any(|basis| basis.starts_with("rho_i:"))
                && !axis.source_refs.is_empty()
        }));
        assert!(
            !reading.measured_axis_refs.is_empty(),
            "selected signature axes must be classified"
        );
        assert_eq!(reading.safe_region_margin.status, "blocked");
        assert!(
            reading
                .safe_region_margin
                .blocker_refs
                .iter()
                .any(|blocker| blocker.contains("gap-runtime-user-db-trace")),
            "coverage gap must block safe-region margin instead of becoming zero"
        );
        assert!(
            packet
                .signature_axes
                .iter()
                .all(|axis| !axis.signature_distance_reading_refs.is_empty())
        );
    }

    #[test]
    fn signature_distance_negative_fixture_axis_without_distance_ref_fails_validation() {
        let mut packet = static_archsig_analysis_packet();
        let axis = packet
            .signature_axes
            .first_mut()
            .expect("static fixture has signature axis");
        axis.signature_distance_reading_refs.clear();

        let report = validate_archsig_analysis_packet_report(
            &packet,
            "negative-fixture-signature-axis-without-distance-ref.json",
        );

        assert_eq!(report.summary.result, "fail");
        assert!(report.checks.iter().any(|check| {
            check.id == "archsig-analysis-packet-signature-distance-readings"
                && check.result == "fail"
                && check.examples.iter().any(|example| {
                    example
                        .target
                        .as_deref()
                        .is_some_and(|target| target == "signatureDistanceReadingRefs")
                })
        }));
    }

    #[test]
    fn signature_distance_negative_fixture_rho_without_basis_fails_validation() {
        let mut packet = static_archsig_analysis_packet();
        let axis_distance = packet
            .signature_distance_readings
            .first_mut()
            .and_then(|reading| reading.axis_distances.first_mut())
            .expect("static fixture has signature axis distance");
        axis_distance.rho_i.evaluator_basis_refs.clear();

        let report = validate_archsig_analysis_packet_report(
            &packet,
            "negative-fixture-signature-distance-rho-without-basis.json",
        );
            ...
            check.id == "archsig-analysis-packet-signature-distance-readings"
                ...
                && check.examples.iter().any(|example| {
                    example
                        .target
                        .as_deref()
                        .is_some_and(|target| target == "rho_i:")
                })
        }));
    }

    #[test]
    fn operation_distance_readings_keep_cost_and_side_effect_blockers() {
        let packet = static_archsig_analysis_packet();
        let reading = packet
            .operation_distance_readings
            .first()
            .expect("static fixture has operation distance reading");
        ...
        assert!(
            reading
                ...
                .evaluator_basis_refs
                .iter()
                .any(|basis| basis.starts_with("operationCost:"))
        );
        assert!(
            reading
                ...
                .evaluator_basis_refs
                .iter()
                .any(|basis| basis.starts_with("targetDecrease:"))
        );
        assert_eq!(reading.side_effect_bound.status, "blocked");
        assert!(
            !reading.transfer_risk_refs.is_empty()
                && !reading.side_effect_bound.blocker_refs.is_empty()
        );
        assert!(
            packet
                .repair_operation_candidates
                .iter()
                .all(|candidate| !candidate.part4_distance_refs.is_empty())
        );
        assert!(
            packet
                .operation_deltas
    ...
    #[test]
    fn operation_distance_negative_fixture_missing_repair_ref_fails_validation() {
        let mut packet = static_archsig_analysis_packet();
        let candidate = packet
            .repair_operation_candidates
            .first_mut()
            .expect("static fixture has repair candidate");
        candidate.part4_distance_refs.clear();

        let report = validate_archsig_analysis_packet_report(
            &packet,
            "negative-fixture-operation-distance-missing-repair-ref.json",
        );

        assert_eq!(report.summary.result, "fail");
        assert!(report.checks.iter().any(|check| {
            check.id == "archsig-analysis-packet-operation-distance-readings"
                && check.result == "fail"
                && check.examples.iter().any(|example| {
                    example
                        .target
    ...
    #[test]
    fn operation_distance_negative_fixture_side_effect_zero_with_risk_fails_validation() {
        let mut packet = static_archsig_analysis_packet();
        let reading = packet
            .operation_distance_readings
            .first_mut()
            .expect("static fixture has operation distance reading");
        reading.side_effect_bound.status = "zero".to_string();
        reading.side_effect_bound.measured_value = Some(0);
        reading.side_effect_bound.blocker_refs.clear();

        let report = validate_archsig_analysis_packet_report(
            &packet,
            "negative-fixture-operation-distance-side-effect-zero.json",
        );
            ...
            check.id == "archsig-analysis-packet-operation-distance-readings"
                ...
                && check.examples.iter().any(|example| {
                    example
                        .target
                        .as_deref()
                        .is_some_and(|target| target == "sideEffectBound")
                })
        }));
    }
