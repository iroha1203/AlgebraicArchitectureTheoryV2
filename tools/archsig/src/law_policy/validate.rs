...
pub fn validate_law_policy_v1_report_with_profiles(
    ...
    LawPolicyValidationReportV1 {
        schema_version: "law-policy-validation-report/v0.5.4".to_string(),
...
fn check_v1_schema(policy: &LawPolicyDocumentV1) -> ValidationCheck {
    let mut check = validation_check(
        ...
        "LawPolicy v0.5.4 uses the selector schema discriminator",
...
fn check_v1_identity(policy: &LawPolicyDocumentV1) -> ValidationCheck {
    ...
    if policy.id.trim().is_empty() {
        examples.push(generic_validation_example(
            "id",
            "empty",
            "LawPolicy v0.5.4 id must be non-empty",
        ));
    }
    if policy.policies.is_empty() {
        examples.push(generic_validation_example(
            "policies",
            "empty",
            "LawPolicy v0.5.4 must select at least one policy entry",
        ));
    }
    check_examples(
        "law-policy-schema052-identity",
        "LawPolicy v0.5.4 identity and selected policies are recorded",
        examples,
    )
}
...
fn check_v1_reserved_fields(policy: &LawPolicyDocumentV1) -> ValidationCheck {
    ...
    if policy.law_surface_ref.is_none() {
        examples.push(generic_validation_example(
            "lawSurfaceRef",
            "missing",
            "lawSurfaceRef is required by LawPolicy v0.5.4",
        ));
    }
    check_examples(
        "law-policy-schema052-reserved-fields",
        "LawPolicy v0.5.4 declares a law surface reference",
        examples,
    )
}
...
fn check_v1_law_surface_resolution(
    ...
    let Some(surface) = law_surface else {
        examples.push(generic_validation_example(
            "--law-surface",
            "missing",
            "LawPolicy v0.5.4 requires --law-surface; witness and law declarations are supplied by that artifact",
        ));
...
fn check_v1_measurement_profile_selector(
    ...
    check_examples(
        ...
        "LawPolicy v0.5.4 selects an external MeasurementProfile artifact for AG evaluators",
        examples,
    )
}
...
fn measurement_profile_v1_checks(profile: &MeasurementProfileV1) -> Vec<ValidationCheck> {
    ...
    vec![
        check_examples(
            ...
            "MeasurementProfile v0.5.4 declares site, cover, coefficients, predicates, certificates, verdict discipline, and finite bounds",
            examples,
        ),
...
fn check_measurement_profile_reserved_fields(profile: &MeasurementProfileV1) -> ValidationCheck {
    ...
    check_examples(
        ...
        "MeasurementProfile v0.5.4 accepts only registered diagnostic ceiling stages",
        examples,
    )
}
...
fn measurement_profile_errors(
    ...
    if profile.schema != MEASUREMENT_PROFILE_V1_SCHEMA {
        examples.push(generic_validation_example(
            &profile.profile_id,
            &profile.schema,
            "measurement profile schema must be measurement-profile/v0.5.4",
        ));
    }
    for (field, value) in [
    ...
    if profile.verdict_discipline != "five-valued-structural-verdict@1" {
        examples.push(generic_validation_example(
            ...
            "verdict discipline must select the v0.5.4 five-valued structural verdict rule",
        ));
    }
}
