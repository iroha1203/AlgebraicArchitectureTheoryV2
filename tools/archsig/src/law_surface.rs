...
pub const LAW_EQUATION_SURFACE_V1_SCHEMA: &str = "law-equation-surface/v0.5.4";
pub const LAW_EQUATION_SURFACE_VALIDATION_REPORT_SCHEMA: &str =
    "law-equation-surface-validation-report/v0.5.4";
pub const LAW_SURFACE_BINDING_VOCABULARY_SCHEMA: &str = "aat-atom-vocabulary-binding/v0.5.4";
...
fn check_schema(surface: &LawEquationSurfaceV1) -> ValidationCheck {
    let mut check = validation_check(
        ...
        "law-equation-surface uses the v0.5.4 schema discriminator",
...
fn check_bindings(
    ...
    for (law_index, law) in surface.laws.iter().enumerate() {
        ...
        for (variable_index, variable) in law.witness_variables.iter().enumerate() {
            ...
            for field in ["archmapVariable", "axis", "predicate"] {
                ...
                "cech" => {
                    if variable.binding.archmap_variable.is_some() {
                        examples.push(generic_validation_example(
                            &format!("{path}.archmapVariable"),
                            "present",
                            "cech bindings derive context edges from ArchMap and do not accept an ArchMap variable alias",
                        ));
                    }
                }
...
fn check_stage3_fields(surface: &LawEquationSurfaceV1, raw: &Value) -> ValidationCheck {
    ...
    check_examples(
        ...
        "law surface quotient condition uses the v0.5.4 contract",
        examples,
    )
}
...
fn check_vocabulary(vocabulary: &LawSurfaceBindingVocabularyV1) -> ValidationCheck {
    ...
    if vocabulary.schema != LAW_SURFACE_BINDING_VOCABULARY_SCHEMA {
        examples.push(generic_validation_example(
            ...
            &vocabulary.schema,
            "binding vocabulary must use the v0.5.4 manifest",
        ));
    }
