...
Python Sig0 output も `archsig air`、`validate-air`、`feature-report`、`theorem-check` に渡せる。
AIR では static relation の `extractionRule` を `python-import-graph-v0`、evidence kind を
`python_import` として保持し、外部 dependency target は `external-dependency` component に正規化する。
theorem precondition checker は Python import graph を tooling evidence として扱い、
Lean `ComponentUniverse` bridge precondition なしの formal claim promotion は block する。
