...
runtime reads supplied `archmap/v0.5.3` evidence and selected
`law-policy/v0.5.3`, `law-equation-surface/v0.5.3`, and
`measurement-profile/v0.5.3` artifacts. Reproducible runs fix those three
policy components with `archsig-policy-bundle/v0.5.3`; the AG measurement path
emits `archsig-measurement-packet/v0.5.3`.
...
| MeasurementProfile validation | `measurement-profile` | Validates a standalone `measurement-profile/v0.5.3` artifact, including finite bounds against evaluator registry hard caps. |
| RepairPlan validation | `repair-plan` | Validates the supplied `archsig-repair-plan/v0.5.3` SAGA input side. Faithfulness, true-sheaf, gluing, coefficient, and comparison slots are checked; generated conclusion tokens and unimplemented transport fields fail closed. |
| AG measurement | `analyze` | When `law-policy/v0.5.3` selects `measurementProfileRef` and the input is finite-poset-site `archmap/v0.5.3`, `analyze` emits `archsig-measurement-packet/v0.5.3`, conclusion-first summary, insight report, viewer data, and run manifest. `ag.saga-descent` can additionally consume a checked RepairPlan via `--repair-plan`; `ag.refactor-transport` consumes a validated `refactor-morphism/v0.5.3` via `--refactor-morphism`. |
| Compare | `compare` | Compares two current `analyze` output directories and computes `archmap-diff/v0.5.3` plus `archsig-comparison-report/v0.5.3`. Add `--refinement refinement-comparison/v0.5.3` only when the coarse-to-fine class-zero contract is supplied. |
| Gate | `gate` | Applies `archsig-gate-policy/v0.5.3` to a measurement packet and optional comparison report. This is the CI decision surface. |
...
| Authored delta artifact | `compare` computes `archmap-diff/v0.5.3` from two run directories. |
...
`archview-sequence/v0.5.3`. It uses CDN Three.js and projects measured AAT geometry
...
| Skill | Purpose |
| --- | --- |
| `archmap-creater` | Create bounded `archmap/v0.5.3` artifacts from repository evidence. It keeps ArchMap as source-grounded Atom observations, not law-relative analysis or removed v0 helper fields. |
| `law-policy-creater` | Create project-specific `law-policy/v0.5.3` profiles from repository coding conventions, architecture rules, and user decisions. If docs do not define the evaluator universe, ask the user before selecting laws. |
...
| `repair-plan-creater` | Author `archsig-repair-plan/v0.5.3` artifacts for SAGA descent runs, validated through `repair-plan` and consumed through `analyze --repair-plan`. |
...
5. When a measured obstruction needs a repair route, use `repair-plan-creater`
   to author `archsig-repair-plan/v0.5.3`, validate it with `repair-plan`, and
...
Use version-only tags such as `v0.5.3` or prerelease tags such as
`v0.5.3-rc.1`. Do not include `archsig` in the tag name; asset names already
