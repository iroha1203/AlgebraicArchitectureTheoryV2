...
runtime reads supplied `archmap/v0.5.4` evidence and selected
`law-policy/v0.5.4`, `law-equation-surface/v0.5.4`, and
`measurement-profile/v0.5.4` artifacts. Reproducible runs fix those three
policy components with `archsig-policy-bundle/v0.5.4`; the AG measurement path
emits `archsig-measurement-packet/v0.5.4`.
...
| RepairPlan validation | `repair-plan` | Validates the supplied `archsig-repair-plan/v0.5.6` SAGA input side. Slot shapes and references are validated here; slot values are checked at evaluation time against the derived residual; the residual is derived by `analyze` from observed cover sections and law-surface witness bindings, and the coefficient is the MeasurementProfile declaration. Generated conclusion tokens and unimplemented transport fields fail closed. |
...
| Authored delta artifact | `compare` computes `archmap-diff/v0.5.4` from two run directories. |
...
Use version-only tags such as `v0.5.4` or prerelease tags such as
`v0.5.4-rc.1`. Do not include `archsig` in the tag name; asset names already
