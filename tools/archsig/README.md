...
| Compare | `compare` | Compares two current `analyze` output directories and computes `archmap-diff/v0.5.4` plus `archsig-comparison-report/v0.5.6`. Add `--refinement refinement-comparison/v0.5.4` only when the coarse-to-fine class-zero contract is supplied. |
