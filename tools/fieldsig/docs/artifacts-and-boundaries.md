...
| --- | --- | --- |
| ArchSig handoff | `archsig-measurement-packet/v0.5.3` から生成する `operation-support-estimate/v0.5.0`。 | ArchSig measurement packet と三つの component fingerprint を current AG measurement state として読む。raw ArchMap observation、PR diff analysis、forecast truth、causal proof ではない。 |
...
| 出力 | schema | 用途 |
| --- | --- | --- |
| Sig0 output | `archsig-sig0/v0.5.0` | 単一 revision の component、edge、signature、metric status。 |
| Validation report | `component-universe-validation-report/v0.5.0` | Sig0 output の duplicate、edge closure、policy status などの検査結果。 |
| Snapshot | `signature-snapshot-store/v0.5.0` | repository revision ごとの保存用 signature record。 |
...
| ArchSig measurement packet | `archsig-measurement-packet/v0.5.3` | FieldSig の現行 ArchSig handoff。三つの component fingerprint と structural verdict、computed invariants、analytic readings、assumption ledger、non-conclusions を bounded SFT input として読む。 |
...
| AAT Observable Bundle | `aat-observable-bundle/v0.5.0` | AAT concept mapping、selected universe、witness catalog、operation candidates、theorem boundary、review actions、responsibility boundary を束ねる。 |
| Policy decision report | `policy-decision-report/v0.5.0` | organization policy に基づく pass / warn / fail / advisory decision。 |
| PR comment summary | `pr-comment-summary/v0.5.0` | Feature Extension Report と policy decision を GitHub Checks / PR comment 向け Markdown に整形する。 |
| Baseline suppression report | `baseline-suppression-report/v0.5.0` | baseline/current report の差分、suppression / accepted risk metadata を保持する。 |

## Policy / Registry / Schema Artifacts

| 出力 | schema | 用途 |
| --- | --- | --- |
| Organization policy validation report | `organization-policy-validation-report/v0.5.0` | warn / fail / advisory policy と formal claim promotion boundary を検査する。 |
| Law policy template registry validation report | `law-policy-template-registry-validation-report/v0.5.0` | policy template registry の boundary refs と non-conclusions を検査する。 |
| Architecture policy | `architecture-policy/v0.5.0` | project-local adopted laws、layer selectors、allowed / forbidden dependency、exception、SRP taxonomy を保持する。 |
| Architecture policy validation report | `architecture-policy-validation-report/v0.5.0` | law-aware policy の selector / SRP evidence boundary / non-conclusions を検査する。 |
| Law violation report | `law-violation-report/v0.5.0` | Layered Architecture の deterministic violation、allowed exception、unmeasured selector、SRP review cue を分ける。 |
| Custom rule plugin registry validation report | `custom-rule-plugin-registry-validation-report/v0.5.0` | custom rule plugin registry の theorem refs と claim boundary を検査する。 |
| Measurement unit registry validation report | `measurement-unit-registry-validation-report/v0.5.0` | measurement unit selection と evidence adapter boundary を検査する。 |
| Repair rule registry validation report | `repair-rule-registry-validation-report/v0.5.0` | repair rule registry の selected obstruction boundary と advisory non-conclusions を検査する。 |
| Synthesis constraint validation report | `synthesis-constraint-validation-report/v0.5.0` | synthesis candidate / no-solution boundary と solver completeness non-conclusion を検査する。 |
| No-solution certificate validation report | `no-solution-certificate-validation-report/v0.5.0` | valid certificate claim refs と finite candidate universe boundary を検査する。 |
| Schema compatibility report | `schema-compatibility-check-report/v0.5.0` | schema migration / compatibility の field mapping、non-conclusions、coverage / exactness boundary を検査する。 |
| Detectable values / reported axes catalog | `detectable-values-reported-axes-catalog/v0.5.0` | reported axes と detectable values の catalog。 |

## Dataset Artifacts

| 出力 | schema | 用途 |
| --- | --- | --- |
| Dataset record | `empirical-signature-dataset/v0.5.0` | PR metadata と before / after signature を結合した実証研究用 record。 |
| PR history dataset | `pr-history-dataset/v0.5.0` | GitHub PR metadata と artifact refs を履歴 record として保存する。 |
| Feature extension dataset | `feature-extension-dataset/v0.5.0` | PR history と Feature Extension Report / theorem check を join する。 |
| Outcome linkage dataset | `outcome-linkage-dataset/v0.5.0` | feature extension dataset と rollback / incident / MTTR などの outcome observation を join する。 |

## Operational Feedback Artifacts

| 出力 | schema | 用途 |
| --- | --- | --- |
| Report outcome daily ledger | `report-outcome-daily-ledger/v0.5.0` | outcome linkage dataset と architecture drift ledger を daily aggregation window で join する。 |
| Calibration review record | `calibration-review-record/v0.5.0` | false positive / false negative review と metric calibration input を保存する。 |
| Team threshold policy | `team-threshold-policy/v0.5.0` | team-specific threshold tuning と CI mode policy を保存する。 |
| Ownership boundary monitor | `ownership-boundary-monitor/v0.5.0` | ownership scope と boundary erosion signal を operational observation として保存する。 |
| Repair adoption record | `repair-adoption-record/v0.5.0` | repair suggestion の adopted / rejected / deferred decision と follow-up outcome refs を保存する。 |
| Incident correlation monitor | `incident-correlation-monitor/v0.5.0` | incident / rollback / MTTR correlation、confounder、missing / private data boundary を保存する。 |
| Hypothesis refresh cycle | `hypothesis-refresh-cycle/v0.5.0` | empirical hypothesis の retained / rejected / proposed update を versioned research tracking として保存する。 |

## SFT Forecasting Input Artifacts

| 出力 | schema | 用途 |
| --- | --- | --- |
| ArtifactDescriptor | `artifact-descriptor/v0.5.0` | PRD / Spec / Issue / AI proposal の source refs、action class candidates、scope、missing evidence、measurement boundary、forecast non-conclusions を保持する。 |
...
| ConsequenceEnvelope | `consequence-envelope-report/v0.5.0` | affected regions、signature axes、axis delta ranges、obstruction candidates、missing / theorem boundary items、typed boundary failures、reviewer actions、review / CI recommendation を保持する。 |
| ConsequenceEnvelope validation report | `consequence-envelope-report-validation-report/v0.5.0` | source refs、measurement boundary、forecast non-conclusions、unknown remainder の欠落を検査する。 |
| SFT review summary | `sft-review-summary/v0.5.0` | opened futures、closed futures、boundary failures、next actions、evidence refs、boundary refs、LLM judgement contract を保持する。 |
| SFT review summary validation report | `sft-review-summary-validation-report/v0.5.0` | status、evidence refs、boundary refs、forbidden readings、non-conclusions を検査する。 |
...
`archsig-analysis-sft-input` は `archsig-measurement-packet/v0.5.3`（三つの component fingerprint を含む）を
`operation-support-estimate/v0.5.0` へ射影する現行 handoff command である。structural verdict、
...
AI proposal JSON から
`artifact-descriptor/v0.5.0`、`operation-support-estimate/v0.5.0`、
`forecast-cone-skeleton/v0.5.0`、`consequence-envelope-report/v0.5.0`、
`sft-review-summary/v0.5.0` と各 validation report を
...
`ai-proposal-governance/v0.5.0` は `artifact-descriptor/v0.5.0` から生成できる reviewer-facing
projection であり、`operation-support-estimate/v0.5.0`、`consequence-envelope-report/v0.5.0`、
`forecast-calibration-hook/v0.5.0` と B10 / B11 operational artifact の refs を保持する。
...
`consequence-envelope-report/v0.5.0` は `forecast-cone-skeleton/v0.5.0` からの
...
`sft-review-summary/v0.5.0` は `consequence-envelope-report/v0.5.0` から reviewer / LLM judgement
...
`aat-observable-bundle/v0.5.0` は AAT の主要概念を ArchSig artifact として追跡する共通 bundle である。
...
| Layered law violation | `law-violation-report/v0.5.0.deterministicViolations[]`, `policyViolations[]` | resolved layer selector と measured import edge に対する deterministic forbidden dependency。 | selector 未解決、dynamic import、framework convention は unmeasured。policy pass は architecture lawfulness proof ではない。 |
| SRP review cue | `signatureAxes[]`, `repairCandidates[]`, `law-violation-report/v0.5.0.srpCues[]` | LLM Review Skill が probable violation / acceptable orchestrator / unmeasured を判断するための semantic evidence。 | tool 単独で SRP violation を断定しない。 |
