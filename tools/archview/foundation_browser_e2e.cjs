  ...
  try {
    ...
    const analysisCases = await page.send("Runtime.evaluate", {
      ...
      expression: `(async () => {
        ...
        const observedAsUnobservedState = await window.__archview.loadAnalysisObject({ ...bundle, packet: observedAsUnobservedPacket }, 'observed edge marked unobserved');
