...
正式入口は`archview.html`、runtime資産は`archview-assets/`にある。Paper AtlasのTop Bar、
...
train-ticket fixtureは2,118 Atoms、43 Contexts、3 Covers、440 Sourcesの全件を保持する。
大規模入力ではContext skeletonを先に表示し、Atom glyphをframeごとのbatchで追加する。
Scope ExplorerはSubject選択時だけそのAtom buttonsを展開し、検索は全Atom / Sourceを対象とする。
overviewではContext labelを使うLODとし、全Atomをsampleせず描画する。

## Local preview

```bash
python3 -m http.server 8000 --directory tools/archview
```

`http://localhost:8000/archview.html`を開く。
既定でtrain-ticket fixtureを読む。`?archmap=<同一originのJSON path>`または
Top Barの`Load ArchMap`で別のArchMapを指定できる。`?analysis=<同一originのrun directory path>`
または`Load ArchSig run`でanalysisを追加できる。
...
foundation browser testは、ArchMapの正常・empty・malformed・unresolved入力、
...
scale browser testはtrain-ticket件数、unsampled progressive rendering、search、Subject LOD、
30 fps target、決定論的screenshotを検査する。

```bash
node tools/archview/foundation_browser_e2e.cjs tools/archview/archview-assets
node tools/archview/scale_browser_e2e.cjs tools/archview .tmp/archview-train-ticket.png
git diff --check
```

## Release

ArchSig release archiveは次を同梱する。
...
directory全体をローカルHTTP server経由で開く。Three.jsはCDNから読み込む。
