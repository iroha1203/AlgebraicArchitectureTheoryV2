...
- **形は測定量が駆動する。** 座標は計測から導出される: **X,Z = 実測 nerve の Laplacian スペクトル埋め込み**（同一成分内の近さ ≈ 拡散距離。等長変換・固有値縮退を除いて一意）、**Y = restriction poset の深さ**（深い重なりほど下）。成分間配置・絶対スケール・スペクトル一致点の最小分離だけが layout。hash は atom の declump jitter のみ。
...
- **幾何/装飾の申告(fidelity declaration)** … どの要素をクリックしても、詳細パネルが視覚チャネルごとに
  `measured`(測定量駆動)/ `layout`(実測adjacencyの埋め込み。座標はverdictなし)/ `decoration`(bloom・pulse・アーチ等の演出)
  を申告する。ツールバーの **Decoration** トグルをOFFにすると、演出を全て落とし測定駆動の幾何だけを描画する
...
| **Gluing & H¹ obstruction** | 局所の一致は大域の section に貼り合うか、どこで閉じないか（headline。閉じない縫い目 = amber seam）。**Twist view** トグルで F₂ cochain をその二重被覆として描く: coboundary なら 2 枚のシートに分離し、H¹≠0 なら捻れて 1 枚に連結する（「閉じない」の幾何的実体） |
| **Étale space / sections** | 宣言された section 値はどこまで延び、大域切断はあるか（値ごとのレーンが context の上を走り、途切れる場所が見える） |
