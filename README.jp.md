...
その破れはどの selected measurement coordinate / law-evaluation profile に現れるか。
...
| ArchSig / FieldSig Tooling | ArchSig は supplied ArchMap evidence、LawPolicy、MeasurementProfile から bounded diagnostic / measurement packet を作る。FieldSig は ArchSig refs と workflow evidence から SFT software evolution evidence を測定する。 | [AAT Tooling Documentation](docs/tool/README.md) |
...
lawful locus / obstruction ideal sheaf / selected finite theorem package の一部は
Lean で証明済みですが、runtime metrics、empirical hypotheses、一般数値 curvature、
実コード extractor の完全性はこの QED には含めません。
