...
ここでの「曲率」は、初期形式化では数値差分ではない。
まず、法則宇宙に含まれる witness family ごとに
`bad : W -> Prop` と測定候補 `List W` を置き、violation count が 0 であることを
測定済み阻害証人の不在として読む。可換図式はこの generic witness-count kernel の
重要な特殊例である。

この定理候補を支える研究上の基本主張は次である。
...
1. 設計原則は、どの法則宇宙と要求法則族を導入するものとして説明できるか。
2. その要求法則族の破れを、有限な阻害証人として提示できるか。
3. その阻害証人の族を、実コードベース上で測れる多軸 signature として表現できるか。
4. signature の変化は、変更波及、レビューコスト、障害修正コストと関係するか。
...
設計原則は万能な善悪判断ではなく、どの不変量を保存・改善する変換クラスを誘導するかで分類する。
...
`archsig` は Lean source file と import 文から component list と import edge を
...
この研究の貢献は、自然言語の設計原則をそのまま Lean theorem や単一の数学的操作に押し込むことではない。
貢献は、形式証明できる構造的事実と、実データで検証する仮説を分離し、それらを
`ArchitectureSignature` という多軸診断で接続する点にある。

具体的には次を提供する。

1. 設計原則を、保存・改善する不変量と、それを実現する変換クラスの違いとして分類する枠組み。
...
10. Conclusion: 設計原則を不変量保存・改善変換を誘導するものとして扱い、品質を多軸 signature として診断する。
...
- `archsig` は Lean の `ComponentUniverse` 証明 witness を生成しない。
