...
本研究では、設計原則を「どの不変量を保証・保存・改善する変換クラスを誘導するか」で分類する。

SOLID は唯一の万能原理ではない。SOLID は局所的な契約・抽象・置換の健全性を与えるが、大域的な分解可能性や実行時収束性を単独では保証しない。

この文書の分類は、自然言語で語られる設計原則に対する唯一の正しい形式化ではない。本研究における操作的な整理である。設計原則そのものを単一の数学的操作とは呼ばず、`remove edge`, `redirect dependency through abstraction`, `split component`, `introduce interface` のような前後状態を持つ設計変換のクラスを誘導するものとして扱う。

この分類は、中心定理候補である **アーキテクチャ零曲率定理** の法則宇宙を構成するための整理でもある。各設計原則は、それぞれが要求する法則族と、その破れを示す阻害証人の型を導入する。
たとえば Layered Architecture は閉 walk や層違反を、DIP は projection soundness violation を、LSP は同じ抽象 fiber 内の観測不一致を、Event Sourcing / Saga は replay / compensation law の破れを witness family として持つ。

static structural core の QED として読む Signature-integrated theorem の required law は、現時点では
`WalkAcyclic`, `ProjectionSound`, `LSPCompatible`, boundary policy soundness,
abstraction policy soundness の 5 条件に固定する。`LocalReplacementContract` は
projection / LSP 系の derived corollary、Event Sourcing / Saga の state-effect law は
