...
- **反例3(正解不在)**: 「精度」を語るのに参照裁定スライスが存在せず、
  事実の取りこぼし(再現率)と過剰生成が計算できなければ fail。
...
- **反例6(既知への過適合)**: 規約のチューニングに使った chunks や、prompt-pack が
  リテラル例として埋め込んでいるサービスの上で**だけ**測り、held-out(規約例に
  現れないコード)での値と区別しない報告が残れば fail。
...
鍵収束率・事実再現率・過剰生成率などの裁定値系ベンチ指標だけ)**、
...
| --- | --- | --- |
| 規約なし | 0.149 | 0.321 |
| 規約 v2(現行 prompt-pack) | 0.477 | 0.743 |
...
上記数値は単一 run 対・チューニングセット上の値であり、分散も held-out 値も未知 —
これがベースラインの最大の弱点である。
...
- **指標定義**:
  ...
  - 鍵収束率 = matched / (matched + 調停で同一事実と裁定された merge グループ数)。
    ...
    **merge グループは R3 の調停記録拡張で機械可読に記録されたものだけを数える**
    (行数からの推定・除算はしない)
  - 事実再現率 = 参照裁定スライスの atom のうち、新規供給が回収した割合
...
- **反証可能性の成立範囲**: 機械指標(matchRate・機械照合下限値)は入力 artifacts から
  バイト決定論で再計算可能。裁定値系指標(鍵収束率・裁定値再現率・過剰生成率)は
  **記録された調停の関数**であり、調停記録込みで第三者が計算を再検証できるが、
  調停そのものの再現は主張しない。この区別を正本に明記する
- **実行プロトコル**: corpus 版、読者モデル、**調停者(モデル id / 人間)**、
  SKILL references の版(git blob hash)、独立 run 対数(既定 3 = 6 独立 run)、
  調停手順、報告の置き場所と節構成(reports 規律準拠)
...
- **claim boundary**: 測定値は corpus・読者モデル・規約版・調停者の四つ組に相対する
  記録であり、他言語・他規模・他モデルへの一般化を主張しない。分散は記述統計
  (最小・最大・平均)であり分布・有意性の主張はしない。参照裁定スライスに対する
  再現率は「その参照が体現する規約系統の下での」相対値であり、規約系統が共有する
  盲点は検出できない(この盲点への防御は held-out chunk と反例駆動レビューが担う)
...
- train-ticket(commit `313886e99bef`)から**4 chunks 以上**を固定する:
  ...
  - **held-out チャンクを1つ以上**: prompt-pack のリテラル例(サービス名・クラス名・
    route)に一切現れないサービスだけで構成する。選定時に prompt-pack との交差を
    機械的に検査し(サービス名・クラス名の文字列照合)、交差ゼロの記録を fixture に残す
...
- **identity(肯定形)**: 参照裁定スライスは「フルビルドの全件調停(2,819 件・
  ...
  精度指標はこの参照に相対する
- フルビルド調停済み ArchMap から R2 の対象 chunk(tuned・held-out の両方)に相当する
...
- ベンチ run は per-run で次を記録する: runId、scope-manifest digest、
  ...
  実行日、chunk 範囲(tuned / held-out の別)、raw packets と拡張調停記録
...
- 鍵収束率・事実再現率・過剰生成率・分散・コストを計測し、
  **tuned チャンクと held-out チャンクの値を分離して**
...
2. **初回実測の成立**: 独立 3 run 対以上の実測が完了し、全指標が分散付き・
   tuned / held-out 分離で `docs/reports/archmap_supply_bench/` に第1報として
   凍結されている。証拠束は inputDigests / runId / 読者モデル / 調停者 / 規約版を完備し、
   README の再現手順から第三者が同じ測定を再実行できる。corpus の git bundle が
   証拠束に含まれる
3. **参照裁定スライスの成立**: R4 参照が独立再読レビューと人間受理を経て
   @2 再キー・規約版メタデータ付きで fixture 化され、初回実測で事実再現率・
   過剰生成率が実際に計算されている。held-out チャンクの prompt-pack 交差ゼロ記録が
   fixture に存在する
...
6. **claim boundary**: R1 正本と第1報の両方に、測定は corpus・読者モデル・規約版・
   調停者に相対する記録である旨、分散は記述統計である旨、参照への再現率は規約系統を
   共有する盲点を検出できない旨が明記され、一般化主張が成果物に含まれない
