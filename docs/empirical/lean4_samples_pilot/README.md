...
- measurement root: repository root。sample project ごとの比較が必要な場合は、各
  subproject root を別々に測る。
- after commit role: PR `head`
- policy file: 未指定
- runtime edge evidence: 未指定

`lakefile.lean` は build configuration metadata として扱い、component universe には
含めない。multi-project repository では、この除外規則により build 設定 file が
import graph の設計 component として混入することを避ける。
...
component id は package root から見た importable module 名とし、`src/app/service.py` は
package root が `src` の場合 `app.service` として扱う。repository root は checkout と
artifact path の基準、source root は scan 対象 file の境界、package root は local import
解決と id 正規化の境界であり、policy selector の `componentIdKind = "python-module"` は
この正規化済み module id を参照する。
...
namespace package や test directory は extractor evidence と policy group で明示し、
Lean `ComponentUniverse` の完全性や Python runtime semantics の完全捕捉は主張しない。
