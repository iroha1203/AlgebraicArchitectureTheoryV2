...
SFT は AAT の概念を、予測、制御、feedback、organization field、
AI operation field のモデルへ片方向に写す。
...
この変換は片方向である。
SFT 側の force、trajectory、forecast、dissipation は、AAT 側の theorem を強めない。
...
| --- | --- |
| AAT theorem level | SFT formal theorem schema |
...
| AAT witness absence | absence of latent force |
| SFT forecast cone narrowing | global risk reduction |
| observed signature delta | identified causal force |
