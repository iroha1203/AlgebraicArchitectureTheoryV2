...
| finite local witness coordinate | `OntologicalAtom` / `ArchitectureAtom` / `AtomSignature` |
...
Atomic Theory では、SFT は AAT atom を field observable として引き戻せる。
...
`AtomAxiomSystem` から立ち上がった `AATCore system` を SFT が local algebra
として読む。
...
`AATCoreTransition` が atom / semantic / circuit delta を new Atom core に接続する。
...
`AATLocalAlgebraForSFT` / `AATPremisedConsequenceEnvelope` /
`AtomTraceForecastBoundary` は後続 migration が残る compatibility surface であり、
不要化した時点で #1307 で archive せず削除する。
`ArchitectureAtom` は互換 wrapper であり、SFT / presentation が読む pure theory object は
`ArchitectureAtom.ontology` で取り出される `OntologicalAtom` である。
...
Atom foundation bridge では、validated `AtomTrace` と law-relative
`CircuitTrace` を `AtomTraceForecastBoundary` として selected `ForecastCone`
...
new Atom core bridge では `AATCorePremisedConsequenceEnvelope` が
`AATCoreTransition` を selected `ForecastCone` / envelope boundary に接続する。
FieldSig / ArchSig report 側の接続も同じ境界に従い、
`ArchSigAATCoreTransition` と `FieldSigAATCoreTransitionAnalysis` は ArchSig transition
