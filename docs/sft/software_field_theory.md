# Software Field Theory: A Computational Theory of Field-Shaped Software Evolution（ソフトウェア場の理論）
...
### 6. ArchSig: Observing Architecture Signatures and Obstruction Witnesses（アーキテクチャ署名と obstruction witness の観測）
...
### 11. Forecast Cones and Consequence Envelopes（予測円錐と帰結 envelope）
...
### 12. Observation Boundaries and Governance Interventions（観測境界と governance intervention）
...
### 13. Field Update and Closed-Loop Feedback（場の更新と閉ループ feedback）
...
sound constraint addition
  ...
  -> その witness family に関して forecast cone が縮小する
```
...
### 16. Proposal Accounting and Review Mediation（proposal accounting と review mediation）
...
### 17. Feedback Boundary Update（feedback による境界更新）
...
### 18. Stable Regions and Recurrent Development Paths（安定領域と反復的開発 path）
...
### 19. SFT Computational Problems（SFT の計算問題）
...
### 20. PRD-to-ConsequenceEnvelope Simulator（PRD から ConsequenceEnvelope への simulator）
...
### 21. AI-Agent Governance under SFT（SFT における AI agent governance）
...
### 22. Review / CI / Type Systems as Governance Interventions（governance intervention としての review・CI・type system）
...
### 23. Lifecycle, Migration, End-of-Life, and Benchmarks（lifecycle・migration・end-of-life・benchmark）
...
### 27. AI-Generated Shortcut（AI が生成する shortcut）
...
### 28. End-of-Life Decision（end-of-life 判断）
...
SFT の違いは、ソフトウェア進化そのものを計算対象として扱う点にある:
  ...
  consequence envelope は到達可能な architecture future を記述する。
```

### 30. What SFT Does Not Claim（SFT が主張しないこと）
...
forecast cone が calibration なしに empirical prediction になる。
AAT theorem から future trajectory safety が従う。
observed signature delta から causal artifact action が一意に同定できる。
...
### 31. Open Problems in the Computational Theory of Software Evolution（ソフトウェア進化の計算理論における未解決問題）
