...
| ID | 仮説 | 主な説明変数 | 主な目的変数 |
| --- | --- | --- | --- |
| H1 | Signature の悪化は変更波及の増加と相関する。 | `deltaSignature`, `reachableConeSize`, `maxDepth` | changed files, changed components |
...
- `signatureBefore`
- `signatureAfter`
- `deltaSignature = signatureAfter - signatureBefore`
