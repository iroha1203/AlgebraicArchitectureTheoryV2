...
`analysis-summary` や packet detail を人間向け onboarding の主画面にしない。
...
- `archmap-creater` Skill で `archmap-observation-map-v0` を authoring する。
- `law-policy-v0` を合成する。
- ArchSig `analyze` を実行する。
- validation、`analysis-summary`、packet refs、coverage repair queue を読む。
- 必要な実コードを再検査する。
- review findings、修正候補、確認ポイントを返す。
- 必要に応じて `pr-review`、`codebase-inspection`、FieldSig handoff へ進める。
...
- `/archsig/manual/`
  - 出力の読み方、主要 artifact、v0.3.0 measurement families、measurement basis を
    一つの manual page にまとめる。
  - `analysis-summary` は Skill / LLM の first reading surface として説明する。
  - `archsig-analysis-packet-v0` は detail evidence store として説明する。
...
- `/archsig/reference/`
  ...
  - `llm-native-workflow` / `north-star-workflow` は compatibility alias として扱う。
...
- `law-policy-v0` は current JSON name for selected interpretation profile。
- `analysis-summary` は Skill / LLM の first reading surface。
- `archsig-analysis-packet-v0` は detail evidence store。
- `pr-review`、`codebase-inspection`、FieldSig handoff は役割を分ける。
...
- v0.3.0 measurement expansion が後半 manual で体系的に読める。
