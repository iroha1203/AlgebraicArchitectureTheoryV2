...
/archsig/
  ...
  できること、できないこと、読む順序、LLM-native ArchMap / LawPolicy / analysis surface、
  CLI / artifact / workflow / reference への入口。

/archsig/getting-started/
  最短手順。
  LLM-native ArchMap / LawPolicy / ArchSig analysis workflow、明示的な adapter-scan、validate。advanced surface は
  command / artifact / workflow page に分ける。

/archsig/inputs/
  Inputs and configuration。
  policy file、runtime edge evidence、PR metadata、Python source roots、
  SFT proposal inputs の準備。

/archsig/reading-output/
  Reading the Output。
  ArchMap、LawPolicy、analysis packet、validation report、Feature Extension Report、
  policy decision、PR comment summary の読み方。

/archsig/air/
  AIR。
  analysis evidence、policy、runtime evidence、semantic witness、coverage、claim、
  theorem precondition check、review report を接続する中間表現。

/archsig/archmap/
  ArchMap。
  codebase、docs、tests、runtime traces、PR context から architecture representation へ写す
  evidence-carrying map candidate。LLM / agent / reviewer を architecture judge ではなく
  map author / semantic observer として扱い、source inventory、preserves、forgets、
  coverage、missing evidence、non-conclusions を ArchSig analysis へ渡す。

/archsig/commands/
  Command reference。
  llm-native-workflow、archmap、law-policy、archsig-analysis、adapter-scan、
  theorem-check、feature-report、policy-decision、pr-comment、
  registry / schema / SFT forecasting commands。

/archsig/artifacts/
  Artifact reference。
  ArchMap observation map、LawPolicy、ArchSig analysis packet、validation report、Sig0 adapter evidence、
  archmap-validation-report-v0、AIR、Feature Extension Report、policy decision、
  PR comment、dataset、operational feedback、SFT forecasting artifacts。
...
/archsig/workflows/
  Workflow manual。
  first adoption、PR review、generated-change provenance、scheduled review、
...
```text
/ -> /archsig/ -> /archsig/getting-started/ -> /archsig/inputs/ -> /archsig/reading-output/ -> /archsig/air/ -> /archsig/commands/ -> /archsig/artifacts/
```
...
/archsig/inputs/
  ...
  next:     /archsig/reading-output/

/archsig/reading-output/
  previous: /archsig/inputs/
  next:     /archsig/air/

/archsig/air/
  previous: /archsig/reading-output/
  next:     /archsig/commands/

/archsig/commands/
  previous: /archsig/air/
...
| --- | --- | --- |
| `/archsig/` | `tools/archsig/README.md`, `docs/tool/README.md` | Manual landing and conceptual boundary. |
| `/archsig/getting-started/` | `tools/archsig/README.md`, `tools/archsig/docs/commands.md` | Minimal LLM-native ArchMap / LawPolicy / ArchSig analysis workflow and explicit adapter evidence path. |
| `/archsig/inputs/` | `tools/archsig/README.md`, `tools/archsig/docs/commands.md`, `tools/archsig/docs/artifacts-and-boundaries.md` | ArchMap observation input, LawPolicy input, optional adapter evidence, and boundary preparation. |
| `/archsig/reading-output/` | `tools/archsig/docs/artifacts-and-boundaries.md`, `tools/archsig/docs/commands.md` | Field-level guide for analysis packets, validation reports, AIR projections, theorem checks, feature reports, observable bundles, and adapter diffs. |
| `/archsig/air/` | `tools/archsig/docs/artifacts-and-boundaries.md`, `tools/archsig/docs/commands.md` | Architecture Interpretation Record as claim, coverage, evidence, and report normalization layer. |
| `/archsig/commands/` | `tools/archsig/docs/commands.md` | CLI command reference grouped by LLM-native workflow, adapter evidence, review projections, policy, and schema surfaces. |
| `/archsig/artifacts/` | `tools/archsig/docs/artifacts-and-boundaries.md` | Artifact and report reference grouped by primary and adapter surfaces. |
| `/archsig/boundaries/` | `tools/archsig/docs/artifacts-and-boundaries.md`, `docs/tool/README.md` | Measurement / claim boundary reading rules. |
| `/archsig/workflows/` | `tools/archsig/docs/commands.md`, `tools/archsig/README.md` | LLM-native analysis workflow and optional adapter / diff workflow. |
| `/archsig/schemas/` | `tools/archsig/docs/artifacts-and-boundaries.md`, `tools/archsig/docs/commands.md` | Schema versioning and compatibility reference. |
| `/archsig/operational-feedback/` | `tools/archsig/docs/operational-feedback.md`, `docs/tool/README.md` | Historical boundary note; operational feedback is FieldSig-owned unless a current ArchSig artifact explicitly references it. |
| `/archsig/examples/` | `tools/archsig/README.md`, `tools/archsig/docs/commands.md` | Runnable ArchMap workflow examples and adapter evidence examples. |
