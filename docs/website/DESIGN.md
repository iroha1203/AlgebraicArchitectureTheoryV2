...
| `/aat/*/` chapter pages | implemented | canonical AAT theory に忠実な first-class formal theory article。 | #772 closed。chapter-cluster pages are implemented with definitions, examples, boundary notes, and Lean/status links where relevant。 | source commit pin、Lean status link、claim boundary wording を継続点検する。 |
