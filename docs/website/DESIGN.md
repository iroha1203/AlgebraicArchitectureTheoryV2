...
- `website/` は Eleventy で authoring し、配信 artifact は純静的とする
  (stack / preview の詳細は [guideline.md](guideline.md) を正とする)。
...
- AAT の数学的 source of truth は `docs/aat/algebraic_geometric_theory/` の分割本文とする。
  `website/aat/**` はこれを置き換える正典ではなく、同文書に忠実な publication
  edition / web-native monograph として肉付けする。
- AAT は first-class formal theory article として、`docs/aat/algebraic_geometric_theory/`
...
- AAT website は、Gauss の `Disquisitiones Arithmeticae` が整数論を体系化した
  ...
  新規 claim は先に `docs/aat/algebraic_geometric_theory/` へ反映する。
...
/archsig/
  ArchSig v1 output contract の Skill-first landing。
...
/archsig/manual/
  ...
  archsig-analysis-summary.json、viewer data、archsig-analysis-packet/v1、measurement families、
...
AAT は Web では一枚にしない。
`docs/aat/algebraic_geometric_theory/` は分割された第一級本文として維持しつつ、
公開サイトでは章クラスタ単位に分ける。
...
`docs/aat/algebraic_geometric_theory/` は分割された source of truth として維持し、
...
| --- | --- | --- |
| `/archsig/` | `tools/archsig/README.md`, `docs/tool/README.md`, `docs/website/ARCHSIG_RENEWAL_PLAN.md` | Skill-first landing and v1 output contract positioning. |
...
| `/archsig/reference/` | `tools/archsig/docs/commands.md`, `tools/archsig/README.md`, `docs/tool/README.md` | Commands, schemas, removed runtime surfaces, and artifact reference. |
...
| Canonical-source fidelity | `docs/` または tool docs の source of truth に忠実か。 | AAT は `docs/aat/algebraic_geometric_theory/`、SFT は `docs/sft/software_field_theory.md`、ArchSig は `tools/archsig/README.md` と `tools/archsig/docs/` を source とし、`docs/tool/README.md` は再構築中の入口として扱う。 |
...
`website/aat/**` は `docs/aat/algebraic_geometric_theory/` を正典として参照しつつ、
...
そのような変更が必要な場合は、先に `docs/aat/algebraic_geometric_theory/`、
