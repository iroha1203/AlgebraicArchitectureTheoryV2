...
| --- | --- | --- | --- |
| `/aat/` landing | AG 版に改訂済み(W1a) | 正典宣言・中心図式・claim taxonomy・部索引 | V〜X 実装時に部索引のリンクを更新 |
| `/aat/` Part I〜IV | 実装済み(W1a) | 正典第I〜IV部の publication edition | W1b 後に相互参照を点検 |
| `/aat/` Part V〜X+付録 | 未実装 | — | W1b で実装 |
| `/aat/related-work/`, `/aat/status/`, `/aat/formal-anchors/` | 旧版残置 | classical 基準 | W1b で AG 基準へ改訂 |
| `/sft/**` | v1 凍結 | v1 三部構成として整合 | Wave 2(P4/P5 完了後)で v2 へ |
| `/archsig/**` | 旧版 | 概念説明中心・密度低 | W1c で concepts / analyses / archview、W3 で manual / reference |
...
Wave 1a(進行中): AAT landing + 第I〜IV部、中心図式 SVG、_redirects、DESIGN.md 改訂
Wave 1b: 第V〜X部+付録、status / formal-anchors / related-work の AG 化、残る旧ルートの 301
Wave 1c: ArchSig concepts / analyses 増強、archview デモページ
Wave 2(発火条件: SFT P4/P5 完了): SFT v2 リライト
Wave 3(発火条件: ArchSig v0.5.0 出荷): ArchSig manual / reference 完成
```
