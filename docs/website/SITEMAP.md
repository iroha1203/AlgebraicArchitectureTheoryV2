...
この文書は、公開 website の canonical route と役割の一覧(人間向け)である。
website の設計方針、読者導線、ページネーション、本文成熟度は
[DESIGN.md](DESIGN.md) に置く。

SEO 用の公開 `sitemap.xml` と `llms.txt` は Eleventy collections からの
**自動生成物**である(`website/src/sitemap.njk` / `website/src/llms.njk`)。
route の追加・削除は `website/src/**/index.html` の増減がそのまま反映されるため、
この文書と手動同期する対象は「役割説明」だけになる。

## 公開設定

- 公開ドメイン: `iroha1203.dev`
- production base URL: `https://iroha1203.dev/`
- sitemap / llms: ビルド出力 `_site/sitemap.xml` / `_site/llms.txt`(自動生成)
- robots file: `website/src/robots.txt`(passthrough)
- custom domain file: `website/src/CNAME`(passthrough)
...
| `/sft/field-and-force/field/` | `website/src/sft/field-and-force/field/index.html` | 1-1. Field。コードベースと開発組織を、memory、artifact、agent、governance、feedback からなる development field として扱う。 |
| `/sft/field-and-force/force/` | `website/src/sft/field-and-force/force/index.html` | 1-2. Force。PRD、issue、spec、review、incident、AI proposal を future operation support に作用する artifact force として扱う。 |
| `/sft/field-and-force/organization/` | `website/src/sft/field-and-force/organization/index.html` | 1-3. Organization。開発者、AI agent、ownership、communication path、routine、recurrent path を field structure として扱う。 |
| `/sft/field-and-force/governance/` | `website/src/sft/field-and-force/governance/index.html` | 1-4. Governance。review、CI、operational feedback、governance が future support を更新することを扱う。 |
| `/sft/computation/` | `website/src/sft/computation/index.html` | 2. Making Software Evolution Computable。ソフトウェア進化を計算可能な対象にする第2部の入口。 |
| `/sft/computation/interface/` | `website/src/sft/computation/interface/index.html` | 2-1. Interface。SFT の field estimate を AAT projection、ArchSig observation、observation boundary へ接続する。 |
| `/sft/computation/computational-core/` | `website/src/sft/computation/computational-core/index.html` | 2-2. Computational Core。ArtifactAction、OperationSupport、OperationPolicy、StepRelation、bounded path を SFT 計算の中核として扱う。 |
| `/sft/computation/forecast-cone/` | `website/src/sft/computation/forecast-cone/index.html` | 2-3. ForecastCone。support と step semantics から、horizon 付きの bounded reachable futures を計算する。 |
| `/sft/computation/workbench/` | `website/src/sft/computation/workbench/index.html` | 2-4. Workbench。ForecastCone を ConsequenceEnvelope、proposal accounting、ArchSig artifact、calibration hook、計算問題へ接続する。 |
| `/sft/software-evolution/` | `website/src/sft/software-evolution/index.html` | 3. Software Evolution。governance、attractor engineering、AI proposal policy、lifecycle feedback により field を steer する第3部の入口。 |
| `/sft/software-evolution/field-shaping/` | `website/src/sft/software-evolution/field-shaping/index.html` | 3-1. Field Shaping。review、CI、type systems、runtime guards、ownership、feedback により support、policy、observation を変形する。 |
| `/sft/software-evolution/attractor-engineering/` | `website/src/sft/software-evolution/attractor-engineering/index.html` | 3-2. Attractor Engineering。stable regions、design levers、measurement surface により望ましい futures を自然にする。 |
| `/sft/software-evolution/ai-proposal-governance/` | `website/src/sft/software-evolution/ai-proposal-governance/index.html` | 3-3. AI Proposal Governance。AI-generated proposal support を theorem boundary、shortcut witness、review、CI の中で制御する。 |
| `/sft/software-evolution/lifecycle-closed-loop/` | `website/src/sft/software-evolution/lifecycle-closed-loop/index.html` | 3-4. Lifecycle and Closed-Loop Evolution。repair、migration、contraction、deletion、posterior update により evolution loop を閉じる。 |
| `/sft/references/` | `website/src/sft/references/index.html` | SFT reference appendix。formal anchors、research program、glossary への入口。 |
...
- `sitemap.xml` / `llms.txt` は Eleventy collections から自動生成されるため、
  route の増減で手動更新するファイルはない。
- route を追加・削除・rename した場合は、この文書の役割一覧を更新し、
  新ページの front matter に `order`(読書順)と `llmsLabel` を設定する。
- sitemap に `docs/website/` や GitHub repository 内部文書を載せない。
