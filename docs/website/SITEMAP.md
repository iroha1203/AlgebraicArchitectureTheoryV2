...
| `/archsig/ci/` | `website/archsig/ci/index.html` | CI / PR review 導入。before / after scan、signature diff、AIR、feature report、PR comment、review gate policy。 |
...
| `/archsig/reading-output/` | `website/archsig/reading-output/index.html` | Reading the Output。Sig0、validation、diff、feature report、policy decision の読み方。 |
| `/archsig/anatomy/` | `website/archsig/anatomy/index.html` | The Anatomy of an Architecture Signal。ArchSig の中核思想を一枚で読むページ。 |
| `/archsig/air/` | `website/archsig/air/index.html` | AIR。測定値を claim、coverage、theorem precondition、review report へ接続する中間表現。 |
| `/archsig/archmap/` | `website/archsig/archmap/index.html` | ArchMap。source universe から architecture representation への evidence-carrying map candidate。 |
| `/archsig/semantic-air/` | `website/archsig/semantic-air/index.html` | Semantic AIR。LLM を semantic observer として扱う AIR 生成と evidence boundary。 |
