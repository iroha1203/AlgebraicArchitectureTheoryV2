...
- PRDに置いてよい内容は、問い、現状診断、現行source of truth・非PRD依存・Issueへの参照、
  ...
  non-goals、停止条件に限定する。
- PRDに恒久的な規律、編集ルール、固定statement、schema / artifact contract、status語彙、
...
4. 成果物、固定statement、contract、恒久ルール、statusが現行source of truthへ反映されている。
...
- AAT / Lean: 数学本文、`docs/aat/lean_quality_standard.md`、分割索引・台帳、schema化されたstatement contract。
...
PRDのarchiveによって失われる規律・contract・statusがある場合、archive条件4は未達である。
