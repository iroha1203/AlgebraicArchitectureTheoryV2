...
この原理の診断面 — adequate な範囲内では解像度の選択は診断を変えない —
は、G-104(2026-08-08、`target-theorem-proved`、report
`research/reports/G-104-aat-resolution-invariance.md`)で Lean 定理化された
(§8 候補4)。
...
4. **診断の解像度不変性**: 二つの L-adequate な読み `q ≤ q'` に対し、
   ...
   **本候補は G-104(2026-08-08、`target-theorem-proved`、report
   `research/reports/G-104-aat-resolution-invariance.md`)で証明済み。**
   条件 C の正体は C0–C6(C1–C4 は係数座標 `(law, 値)` の台 subnerve へ
   相対化)+係数生成契約 K0 / K1 として特定され、その下で actual
   comparison `H^1` map の bijectivity(claim (ii))と過剰解像度の
   無益性の系(claim (iii))が、条件を満たす任意の有限入力について
   証明された。対の反例は偽の類の発生・真の類の隠蔽・条件 C 破れの
   3種の有限 witness で、さらに全条件成立+非零類の単一非退化発火
   witness で vacuity を排除した。条件 C の特定までに4度の
   `target-refuted`(C4 / C5 / C6 / 退化宣言の hereditary 性の各欠落に
   対する有限反例)と off-loop 計算探索(incidence-only 条件の条件付き
   no-go)を要したこと自体が「C の正体こそが定理の中身」の実証である。
   偽の類・隠蔽の定量化は存在 witness の frontier のまま残り、条件 C の
   必要性の一般特徴づけ、`ℚ` 以外の係数体、無限 regime は claim に
   含まれない。
...
Gr2(G-101)・候補1(G-102)・候補4(G-104、2026-08-08)はいずれも
達成済みであり、one-cent 実例と合わせて投稿 gate は充足した。
研究 GOAL の受け入れ規準はこの査読水準を目線に書く。

**論文Aの実証節計画(2026-08-03 合意)**。実証に求めるのは規模ではなく
「定理が現実に触れた証拠」である: (a) 形式的機構が実データ上で実際に
動くこと、(b) 定理の条件・結論が実システムで非自明に発火する
(vacuous でない)こと、(c) R4 の反証可能予言が既存の経験的知見と
整合すること。計装の効いた1ケーススタディ+文献三角測量を、浅い
大規模調査より優先する。実証節は次の三段重ねで構成する。

```text
1. 文献三角測量(執筆作業のみ): 候補1の系「構造サイクルは結合欠陥の
   必要条件」を、依存サイクルと欠陥率の経験的 SE 研究(依存グラフと
   欠陥予測の系列、cyclic dependencies の有害性の実証文献)と接続し、
   「定理が独立した経験的観測を系として予言する」形で R4 を裏打ちする。
2. train-ticket の定理照準再測定(ArchSig 改修を含む): 既存フルビルド
   (42 サービス / 2118 atoms、証拠束・runId 再現つき)の測定項目を
   論文Aの定理に揃え直す。(i) 宣言変形族の下での二相統計 — 両相が
   非空で、contract / authority 型の混合 schema が実在することの実測。
   (ii) サブシステムごとの nerve 非輪状性と、非零障害類の相帰属の
   突合 — 候補1 (iii) の系の実データ発火。(iii) SAGA 第2段の実在
   one-cent 三角形(非零類 -> BLOCKED -> repair -> PASS)の定理番号への
   紐づけ。依存 profile の測定(宣言変形族での抽出不変性判定)は
   ArchSig 側の新規機能であり、改修スコープに含める。
3. 解像度スイープ(候補4の実データ再演): 同一サンプルを複数解像度
   (service / モジュール / メソッド粒度)で抽出し直し、adequate 範囲内で
   H^1 類が安定すること、adequacy を破る粗化で偽の類の発生または
   真の類の隠蔽が実際に起きることを実測する。入力トライアドの
   解像度を振る運用で、候補4の定理と反例対をそのまま再演する。
```

異種別インスタンス(データパイプライン等、R5 の例証強化)は stretch と
して保留、欠陥カタログ(実 issue の翻訳)は選定バイアス防御の負担から
今回は見送る。着手は PRD 化してから行い、採否規準(どの測定が出たら
実証節に採録するか)を PRD の問い節で事前固定する。段2・3 の測定語彙が
依存する候補4(G-104)の確定定義(条件 C=C0–C6 の座標 subnerve
相対化・係数生成契約 K0 / K1)は 2026-08-08 の `target-theorem-proved`
で確定したため、実証節 PRD は起草可能である。
