...
### 6.3 G-122の固定入力で生成データの十分性を調べる

最初の設計試験にはカードDの固定入力を使う。比較の定義や許容変更群を先に固定し、
その同じ対象の射を `res` がどこまで決定するかを調べる。
始めに棚卸しするデータと、対応する確認を次に置く。

| 固定入力の成分 | 生成データとして調べるもの | 十分性を判定する射の情報 |
| --- | --- | --- |
| finite axis-foldのAtom・Source・supportと選択点 | 有限の原始tableと対象形成・抽出の関係 | `objectMap`の全域の値とoperationの全端点での値 |
| 係数 `ℤ`、各contextの一座標と関係 `X²−X` | 係数の宣言、座標・関係式、恒等restrictionを与える式 | 全contextの幾何写像とrestrictionの保存 |
| 実生成 `barAlpha`・`barBeta`・canonical正規化 | 両端と比較・冪等射の生成部への作用 | 比較の等号、冪等性、(R4)の逆性、全比較群の各元 |
| 底と観測への投影 | 同じ生成データからの評価 | 底固定の資格、section、二種類の核とfiberの対応 |

一座標・一関係・恒等restrictionという既存の幾何の定義だけでは、最終列の全域射が
生成されたことにはならない。選んだ生成部で一致する二つの異なる射を構成できれば、
その候補の `res` は単射ではない。整合写像に延長が存在しない証拠を得れば、
その候補の整合条件または生成データを修正する。特定のsectionと核の代表元を
表示できたことは、全群を運ぶ十分性の判定と分ける。

この試験から得た原始データの型・関係・生成規則を、意味上の条件 `D_Θ` へ一般化する。
カードCの分離例とEの二入力族も同じ宣言に収録する。
候補生成部への反例はその候補の評価として記録し、既存のGOALの許容射を削って回避しない。
具体的な生成部・整合式・構文の定義の確定と、(R2)・(R3)の証明を区別して進める。
