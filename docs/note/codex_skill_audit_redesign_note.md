...
5. Research 成果索引の初期整備(遡及登録)は別 PRD として Codex に委任する
   (B-3)。索引の器と記載契約は本ノートの実装で作る。
...
- **実装者の自己申告を証拠から除外**: PR 本文の「証明した定理」リスト、
  ...
  claim mapping はレビュー側で**独立に再構築**する。B-4 で実装者が
...
- **移植元 conjunct 対応**: 対象が蒸留・移植タスク(B-4 のタスク型宣言)の
...
- `unported (Research-proved)`: Research 側(`Formal/AG/Research/`)に
...
`docs/aat/research_evidence_index.md`(仮)を新設する。

- 1行 = 1受理成果: theorem 名、ファイル、対応する本文ラベル、
  結論(conjuncts)の要旨、受理サイクル、本体移植状況
  (`ported` / `unported` / `not-for-porting`)。
- `research-loop` / `target-theorem-loop` の受理時の出力義務に、
  この索引への登録を追加する(ledger-templates に組み込む)。
- 既存の受理成果の遡及登録(初期整備)は、本ノートの実装スコープから
  外し、**別 PRD として起こして Codex(prd-loop)に委任する**
  (2026-07-07 ユーザー決定)。本ノートの実装では索引ファイルの新設、
  記載契約の定義、loop 側の登録義務の追加までを行い、
  初期整備 PRD の受け入れ基準の土台にする。
- 実装・レビュー側の Research 下限検索は、巨大 Lean ファイルの実読ではなく
  まずこの索引を grep する。索引に無ければ `rg` で Research 直接検索に落ちる。
...
#### B-5. 是正 Issue の source of truth ポインタ
...
#### B-6. アウトカム工程(prd-loop / prd-completion-review の改訂)
...
| `.codex/skills/research-loop/`, `target-theorem-loop/` | 索引登録義務の追加(ledger-templates 改訂)。target-theorem-loop の既存 math-lean-review ゲートは変更しない |
| `docs/aat/guideline.md` | `unported (Research-proved)` 語彙、境界資格条件(A-4 正本への参照形) |
| `docs/aat/research_evidence_index.md` | 新設(器と記載契約のみ。初期整備は別 PRD で Codex 委任) |
...
- **hard fail scan の欠陥**: glob が Research 集約ルート
  `Formal/AG/Research.lean` を除外せず 198 行の false positive
