...
4. 補強の候補のうち、CS の外に一文で通じる**定理水準の candidate** は四つある
   (§4): (a) 比較写像の Leray 分解と、そこから出る**初めての必要条件**
...
5. 必須は R2(差分表の充填)と R1(実システムの多解像度観測)。R1 は決定論的
   静的抽出で service / class / method の三段を複数系で測り、欠損プロファイルが**領域
...
6. 「CS に届いた」の判定規準は三つ(§6): 差分表の各行が肯定形で埋まる、実測で
   欠損プロファイルが領域ごと・源ごとに割れる、R8 / R10 / R4a / R3-r のいずれかが
...
### 1.2 構造像(candidate): 比較写像の Leray 分解

比較写像は「粗側の係数から順像係数への unit 射」と「Leray の edge map」の
合成に分かれる:
...
- **順像の系**: 粗い観測者が自前の係数を作らず細かい観測の順像 `φ_*F` を
  係数に採れば phantom は恒等的に零で、hidden は fiber 内部の閉路だけになる。偽の類の
...
- **実測**: 実システムで欠損プロファイルが領域ごとに割れることが示されていない。
...
- 位置: 土台補強。論文A 実証節計画(n1001 §12)の第三段と同一対象であり、二重に
  ...
  R12 の下流タスク)。
- 判定: 欠損プロファイルが**領域ごとに、かつ源ごとに**割れること。全域 `J ≠ 0` なら
...
- 位置: 支線。CS に可読な問いであり、かつツールの量化域の設計(一様モードの
  実現可能性)に直結する。
...
### R8 Leray 分解・必要条件 C3′・C^loc・順像の系(最優先の後継 GOAL)

- 内容: §1.2 の分解を有限 2 次元・退化宣言つき部分写像で固定する。成果として
  ...
  witness が候補。C0 / C2 / C4 / C5 の成立は要確認)、(iii) 順像の系
  ...
  する)。完全列全体(5 項完全列の Lean 化。Mathlib にスペクトル系列は実用水準で
  無いと見られ、有限次元線形代数で直接書く)は同カードの frontier。
- なぜ CS の外が気にするか: 「粗 chart 内部の閉路は粗い観測からは見えず、隠蔽される類の個数は
  chart ごとの `H¹` の和以上」「偽の類は集計でなく再観測から生まれる」の二文が
  定理で言える。計測の読者には誤差予算、SE の読者には blame、CS には有限 Leray 型
  不等式。
- 既知との関係: 分解は Leray / Grothendieck の系であり新規性なし。必要条件は
...
- 判定: 必要条件 C3′ と strictness witness が Lean で立つ。分解は「古典の完全列の AAT への適用」と書き、新定理と売らない(禁忌 5)。
- 規模: 必要条件 小〜中、完全列全体 中〜大。
...
- 内容: 細読み `q'` と粗化の規約(商 nerve の作り方 — 平行辺の同一視、fiber 内辺の
  ...
  極大元の反鎖、(d) よって貪欲併合は失敗する。いずれも例示スクリプトと同じ線形代数で検算済み
  (証拠ではない)。
...
- Atlas 論文(2 本目)の骨格: 枠 = コード観測と解像度(§2)/ 対象 = 比較写像
  ...
  の限界(G-107 (v))+ 必要条件 C3′ と順像の系(R8)/ 実測 = R1a / 差分 = R2 /
...
- 打ち切りの条件: R2 の判定で実証研究が無く、R1 の欠損プロファイルが全域非零なら、Atlas は
  「AAT の測定器の校正理論」として正確に主張し、CS への接点 は exactness と「局所 ⟹
...
2. 実システムで欠損プロファイルが領域ごと・欠損源ごとに割れ、phantom と hidden の両方に
...
- §1.2・§4 の R3 / R4 / R8 / R10 に書いた数学(Leray 分解、必要条件 C3′、C^loc、
