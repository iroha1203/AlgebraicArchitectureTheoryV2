...
として、第VI部の Gauss-Manin 系(定義10.2)の**最小実例**に位置づける。一般の rho_U が「与えられていれば」の仮定付き対象である一方、square-level transport は P1 の表示から構成できる。これにより VIII 原則7.4 の「no groupoid → no monodromy measurement」が「P1 の groupoid があるので測れる」に転じ、W2 が解消する。旧 Lean 資産は `Formal/AG` source と本文ラベルを直接突合する(W6)。
...
Lean 化の優先順位(GitHub Issue / GOALで管理する順): P7 停止性 → G2(a)(d) → G4 随伴性 → G3(a)(b) → G2(c)。いずれも有限・decidable で、AG版の本文命題とLean sourceの対応を直接確認する。
