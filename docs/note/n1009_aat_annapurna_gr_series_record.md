...
まとめると、Gr 系列によって AAT は、意味の空間を構成する段階から、空間の間で
何を運べるか、どの変更が比較を保つか、何が観測から失われるかを計算する段階へ
進んだ。実現が保持すべき比較構造を明らかにする点で SHIGURE への、意味の空間の
中で許される変更を記述する点で SFT への基盤になる。
...
しかもこの固定実例では、比較を保つ相手側の変更が一意に存在し、相手を恒等に
した対は不適合であることまで証明されている(B2 で両端の stabilizer が自明と
決定され、相手の fiber は一点になる)。それでも係数観測は、正しい変更対と
不適合な変更対を区別しない。失われているのは設計の選択肢の広さではなく、
唯一必要な追随変更を見分けるための構造情報である。その情報は生成された比較の
側にあるので、変更案を検査するだけでなく、必要な追随変更を構造から導く方向が
開けている。

[one-cent ドリフト実例](../../tools/archsig/examples/practical-rust-service/README.md)が
不整合の測定可能性を示したのに対し、本結果はその測定の限界の位置を確定した。
同じ実例には、`ctx:settlement` の追加で site 自体が変わり、変更前後の比較が
`RUNS_NOT_COMPARABLE_WITHOUT_COMPARISON_DATA` として留保される段もある。
解析の土台が変わる変更前後の比較は、相対化の計算体系の具体的な応用先であり、
Gr 系列の比較と移送の構造をこの実例へ接続することは後続の ArchSig 実装の
題材になる。
...
もう一つの教訓は、正規化がいまの観測を保存することと、正規化の後にも変更や
比較の構造が保たれることは別問題だ、ということである。前者は G-116 の
observable exactness が与え、後者が一つの自然な束ねとしては成り立たない
ことを G-117 の反証が示した。意味を同一視する基準を決めるだけでは足りず、
その同一視のもとでどの操作が整合的に行えるかまでが研究対象になった。
...
§5.3 の表示独立性と合わせると、これは解析を毎回やり直さないための理論と
読める。表示独立性は解析結果を別の表示へ移す根拠を与え、被覆と診断輸送の
分類は局所結果を組み合わせる条件を与える。次の応用目標は、既存の解析結果を
再利用し、変更された部分とその重なりに絞って再検証することである。大規模な
コードベースや複数エージェントの並列開発と相性がよい。
