...
5週間で Gr 系列の全 GOAL の帰趨が確定した。本ノートはこの期間の実装記録を残し、
AAT にもたらされた数学的成果と CS への意義を書き、系列の総称の愛称を定める。
各定理の statement と証明状態は各 GOAL カードと report にある。
Gr 階梯の定義は [n1001 §3.5](n1001_atom_is_all_you_need_discussion.md) にある。

## 要旨

1. Gr 階梯は「AAT は Grothendieck 的である」という比喩を段階的に定理へ換金する
   ための階梯である。Gr2(構成された実例)と Gr3(擬関手的整合)は定義どおり達成
   した。Gr4(base change 完備)は定義どおりには成り立たなかった。代わりに、
   三層の定理群と、その上の冪等正規化因子・descent transport の層が確定した。
2. 5週間の帰趨は `target-theorem-proved` が 16、`target-refuted` が 2(G-105、
...
n1001 §3.5 は、初版が比喩として置いた「Grothendieck 的」という装置名を、
5段の階梯で定理へ換金すると宣言した。
...
Gr4 は G-110 で土台を作り、単責務 6 枚(G-111 から G-116)で攻めた。
6 枚は全て証明で確定したが、Gr4 は n1001 の定義どおりには成り立たなかった
(§3)。後続の G-117 と G-118 は、成り立たなかった理由の層を対象化した。
...
帰趨の列の proved は `target-theorem-proved`、refuted は `target-refuted` を指す。
G-117 のループは (c) の反証定理
...
この 5 週間の特徴は、反証が次のカードの入力になる回路である。
...
と定義した。6 枚の証明の帰結として、この定義どおりの達成は成り立たなかった。
...
そして三層の上に、閉じない部分そのものを対象化する層が乗った。G-116 が
...
- **予想A(Morita 形)**: package 圏の Karoubi 包絡は configuration 層を付け
  加え、n1001 の因子化テーゼは Morita 同値の形で定理化できる。定理化の条件は
  包絡の上での実現関手の構成である。
- **予想B(H¹ の細分)**: obstruction 類は base change で不変だが、descent の
  両立性はそうでない。G-118 の torsor 分類は、コホモロジー類より細かい不変量
  が実在することの構成的証拠である。定理化の条件は、この不変量の well-defined
  な class 化である。
- **予想C(compactness)**: coverage の有限性条件は compactness の実例である。
  定理化の条件は、G-112 の分類を位相の言葉へ移すことである。

住処はいずれも後続 GOAL であり、G-118 カードの frontier(variable-comparator
...
数値だけでは統治できず、構造データそのものを見る必要がある」という現場の
...
Gr 系列(G-101 から G-118 までの弧)の総称の愛称をアンナプルナと定める。
...
4. 名はサンスクリットで「食物に満ちた者」を意味し、豊穣の女神の名である。
   山が地図どおりでなかったことが収穫をもたらした。反証が最も肥沃だった。
5. 単一峰ではなく、一つの山体に複数の頂を持つ連峰である。単責務 6 枚への
   分割は、一本の経路では登れない山体だという地形の認識だった。山群には
