...
archsig-analysis
  ...
  --out <private-archmap-root>/north-star-target-python-repo-profile/archsig-analysis-packet.json
  --validation-out <private-archmap-root>/north-star-target-python-repo-profile/archsig-analysis-validation.json
```
...
```text
molecule:repository-transaction-boundary
  status   = needsBoundaryPreparation
  score    = 30
  bridges  = 2
  boundary = policy

molecule:source-backed-domain-artifacts
  status   = needsBoundaryPreparation
  score    = 30
  bridges  = 2
  boundary = transactionBoundary

molecule:tenant-http-request-boundary
  status   = needsBoundaryPreparation
  score    = 45
  bridges  = 1
  boundary = policy

molecule:llm-generation-job-surface
  status   = needsBoundaryPreparation
  score    = 53
  bridges  = 1
...
- obstruction に参加している
- bridge edge を持つ
- source-backed domain / transaction / tenant / LLM surface を横断している
- 推奨 boundary operation が interface だけでは足りない

対象リポジトリでは、repository transaction boundary と source-backed domain artifacts が
...
```text
tenant-http-request-boundary
  -> repository-transaction-boundary
  -> source-backed-domain-artifacts
  -> llm-generation-job-surface
```
...
ここで見えているのは、対象リポジトリにおいて tenant / repository / domain / LLM が
...
これは、対象リポジトリ における LLM generation job surface が
workflow risk の強い行として現れ、同時に source-backed domain cohesion が
全 workflow 横断の支配的な列として現れていることを意味する。

つまり、LLM 周辺だけが危ないのではない。
LLM job surface が、source-backed domain cohesion の圧力を強く受ける位置にある。
...
これは tenant HTTP request boundary が、atom / atom family overlap の観点で
大きな結合質量を持つことを示す。

対象リポジトリでは tenant boundary が単なる edge adapter ではなく、
authority、contract、relation をまとめて担う semantic hub として現れている。

このため、tenant boundary を局所修正すると、
auth lifecycle、repository transaction、observability、permission coverage へ
影響が転送されやすい。
...
dominant obstruction:
  obstruction:target-python-repo-state-effect-reconciliation-gap:computed

dominant axis:
  sig-axis:target-python-repo-state-effect-reconciliation-gap
```
...
dominant operation delta:
  operation-delta:repair-obstruction-target-python-repo-state-effect-reconciliation-gap-computed
  value = 5
...
authority / trust
permission coverage
source-backed domain cohesion
LLM output mediation
state-effect reconciliation
```
...
LLM surface 側では capability / state / effect / trust として、
tenant boundary 側では authority / contract / relation として現れていることを示す。
...
LLM 実行面と tenant authority 面の 2 つの dominant mode として読める。
...
tenant-http-request-boundary
  <-> http-observability-and-edge-policy
  overlapScore = 11
  sharedAtomFamilies = authority, contractSpecification

tenant-http-request-boundary
  <-> refresh-token-auth-lifecycle
  overlapScore = 9
  sharedAtomFamilies = contractSpecification

user-admin-email-auth-lifecycle
  <-> admin-destructive-governance
  overlapScore = 9
...
対象リポジトリの場合、tenant/auth 周辺と external provider 周辺に、
...
まず単一 repair を当てるのではなく、tenant boundary、repository transaction、
source-backed domain、LLM surface の shared atom families を読み、
...
```text
law:target-python-repo-authority-trust-boundary
law:target-python-repo-state-effect-reconciliation
law:target-python-repo-source-backed-domain-cohesion
law:target-python-repo-llm-output-mediation
law:target-python-repo-permission-coverage
```
...
authority / trust の曲率と、state / effect の曲率と、LLM output mediation の曲率は
...
```text
target-python-repo-authority-trust-boundary-stress
target-python-repo-state-effect-reconciliation-gap
target-python-repo-source-backed-domain-cohesion-stress
target-python-repo-llm-output-mediation-stress
target-python-repo-permission-coverage-gap
```
...
- tenant / repository / source-backed domain / LLM surface の間に bridge pressure がある
- repository transaction boundary と source-backed domain artifacts は先に境界整理すべき
- state/effect reconciliation は transition algebra の問題として見える
- LLM output mediation は anti-corruption layer を必要とする
...
2. source-backed-domain-artifacts
   first concern:
     source-backed domain cohesion and transaction boundary

3. tenant-http-request-boundary
   first concern:
     authority / contract / relation hub

4. llm-generation-job-surface
   first concern:
     anti-corruption layer for LLM effects and trust boundary
