...
site 全域の presheaf(`M_sem` や `Q_E`)からの制限は generated 構成
(実装割当は R5 改訂: `M_sem` 用は C1、`Q_E` 用は **C2 #3789 で着地済み**。
補題5.4 の residual engine と同居させるため、C2 が `equationSitePresheaf`
(site 全域読み)と `equationComplex`(定義5.2 `C_E`)を実装した):

```lean
/-- site 全域 presheaf の Int_{≤2}(𝒰) への制限(M_sem 用は C1)。 -/
def MSem.onIntersections (P : SemanticRepairPresentation S R)
    (𝒰 : MonomorphicOrderedCover S) : IntersectionCoefficientData 𝒰

/-- X.定義5.1: E から生成される Q_E = O_E / I_Ob^E の site 全域読み
(C2 #3789 実装。LawEquation.lean の ObstructionQuotient を再利用)。 -/
def equationSitePresheaf (S : Site.AATSite A) : SitePresheafData S

/-- X.定義5.1 の Int_{≤2}(𝒰) 読み。C3a は独立再定義せず、これを使う。 -/
...
| C3a #3771 | §4.2(realization), §4.9 | `EquationSemanticRealization`、`chiE`、命題6.1A、系6.7(`SagaEquationPacket`)。`Q_E` 読みは C2 の `equationSitePresheaf` を再利用し独立再定義しない |
