...
status: R0 fixed(本 note が #3757 R0 の固定成果物。R1 改訂: PR #3775 レビュー finding 1–7 反映)
...
| `Site.AATSite.topology` | `Formal/AG/Site/Geometry.lean:40` | 生成 Grothendieck topology `J_E`。定義8.1 条件3 の membership の座(§3.2) |
...
本文 定義6.1 / 6.1A / 6.1B は係数・対応・local state を **cover intersection diagram 上で**
量化する。site 全域を量化対象にすると本文にない premise の追加になるため、
...
inductive Face {S : Site.AATSite A} (𝒰 : MonomorphicOrderedCover S) :
  ...
  | tripleIK (i j k hij hjk hkeep) (hkeepP : ¬ 𝒰.omittedPair i k) :
      Face 𝒰 (.triple i j k hij hjk hkeep)
        (.pair i k (𝒰.indexOrder.lt_trans hij hjk) hkeepP)
...
triple の pair face は非省略 pair を仮定に持つ。省略 pair へ落ちる restriction 成分は
複体に現れず(定義2.1 の積は非省略成分のみ)、その値は入力8 が零に固定する。
...
/-- 補題2.1A の比較先: 第IV部の全 ordered tuple cover への bridge。
simplex n := Fin (n+1) -> Index、tuple overlap は ofOverlapPackage と同じ
iterated overlap で生成する。 -/
def MonomorphicOrderedCover.toCoverRelativeCechCover
    (𝒰 : MonomorphicOrderedCover S) : Cohomology.CoverRelativeCechCover S
```

補題2.1A(C1)はこの bridge の上で、増加添字複体と
`CoverRelativeCechComplex.AdditiveCechH1` の `Z¹`/`B¹`/`H¹` 同型を証明する。
比較は次数 ≤ 2 の成分だけを使うが、bridge 自体は全次数の tuple データを
iterated overlap で供給する(第IV部 structure の要求)。
...
| (6) `P_E` + lift atlas | `IntersectionLiftSystem` + `CoefficientLiftAtlas` | `Int_{≤2}(𝒰)` | C2 |
...
intersection ごとに確認する」)。第1条件(repair-relation soundness)は入力にせず、
補題6.2A(C3)が入力5–7 から導出する。
...
定理4.3–系4.5・補題6.2A・定理8.2 の証明はこの範囲しか使わない。
...
`P_E` は本文どおり intersection 上の量化で足りる(定義5.3 は「各 intersection `V` で」
と述べる。site 全域への拡張は系8.3 だけが追加仮定として受け取る。§4.9)。

```lean
/-- X.定義5.3: Q が作用する equation-lift local state(Int_{≤2}(𝒰) 上)。 -/
structure IntersectionLiftSystem {S : Site.AATSite A}
    {𝒰 : MonomorphicOrderedCover S} (Q : IntersectionCoefficientData 𝒰) :
    Type (max u w (y + 1)) where
  State : IntersectionIndex 𝒰 -> Type y                         -- P_E(V)
  restrictState : ∀ {σ τ : IntersectionIndex 𝒰}, Face 𝒰 σ τ -> State τ -> State σ
  restrictState_coh :
      ∀ {σ τ₁ τ₂ ρ : IntersectionIndex 𝒰}
        (f₁ : Face 𝒰 σ τ₁) (g₁ : Face 𝒰 τ₁ ρ)
        (f₂ : Face 𝒰 σ τ₂) (g₂ : Face 𝒰 τ₂ ρ) (e : State ρ),
        restrictState f₁ (restrictState g₁ e) = restrictState f₂ (restrictState g₂ e)
  act : ∀ σ : IntersectionIndex 𝒰, Q.carrier σ -> State σ -> State σ
  act_zero : ∀ (σ) (e : State σ), act σ 0 e = e
  act_add : ∀ (σ) (q q' : Q.carrier σ) (e : State σ),
      act σ (q + q') e = act σ q (act σ q' e)
  act_restrict : ∀ {σ τ : IntersectionIndex 𝒰} (f : Face 𝒰 σ τ)
      (q : Q.carrier τ) (e : State τ),
      restrictState f (act τ q e) = act σ (Q.restrict f q) (restrictState f e)
  free : ∀ (σ) (e : State σ) (q : Q.carrier σ), act σ q e = e -> q = 0
  transitive : ∀ (σ) (e e' : State σ), ∃ q : Q.carrier σ, e' = act σ q e

/-- X.定義5.3: selected local equation-lift atlas。 -/
structure CoefficientLiftAtlas {S : Site.AATSite A}
    {𝒰 : MonomorphicOrderedCover S} {Q : IntersectionCoefficientData 𝒰}
    (PQ : IntersectionLiftSystem Q) : Type (max u y) where
  localLift : ∀ i : 𝒰.Index, PQ.State (.chart i)                -- e_i
```
...
structure PrimaryStateCorrespondence {S : Site.AATSite A}
    ...
    (PQ : IntersectionLiftSystem Q) : Type (max u v w x y) where
  beta : ∀ σ : IntersectionIndex 𝒰, Psem.State σ.ctx -> PQ.State σ   -- β_V
  beta_natural : ∀ {σ τ : IntersectionIndex 𝒰} (f : Face 𝒰 σ τ)
      (p : Psem.State τ.ctx),
      PQ.restrictState f (beta τ p) = beta σ (Psem.restrictState (Face.hom f) p)
...
structure EmptyOverlapNormalization {S : Site.AATSite A}
    ...
    (Q : IntersectionCoefficientData 𝒰)
    (Psem : AffineSemanticRepairSystem P 𝒰)
    (PQ : IntersectionLiftSystem Q) : Prop where
...
- `Q` / `PQ` の Int_{≤2} データは省略 intersection を対象に持たないため、
  幾何側の normalization は `Q_E` の site 全域実体
  (`ObstructionQuotient`)と `P_sem` の site 全域 carrier に対して述べる。
  `P_E` 側の subsingleton 性は、系8.3 の site 全域拡張(§4.9)を受けた場合に
  その `State` に対して同形の条項を追加する(C5 が固定)。
...
structure SagaEquationPacket (S : Site.AATSite A) : Type _ where
  ...
  liftSystem : IntersectionLiftSystem (equationCoefficient S cover)  -- 入力6a
...
| `TrueSheafConditionCertificate`, `coverSheafConditionFor_of_trueSheafCertificate`, `coverDescent_of_trueSheafCertificate` | 置換候補(certificate とその直接 consumer を同区分で扱う。C5 定義8.1 は per-cover certificate を置かず topology-wide sheaf condition から導出) |
...
`amalgamatedCorrectedLocalLift`)は small-cover 版
(`RQS.LawGeneratedConormalDescent.*`)と large-coefficient 版
(`RQS.LawGeneratedLargeConormalDescent.*`)の2宣言が同名で存在する。
...
| `localLiftDifferenceFor` | `RQS.LawGeneratedLargeConormalDescent.localLiftDifferenceFor`(補助: `RQS.LawGeneratedConormalDescent.localLiftDifferenceFor:352`) | 同 `:234` | reuse |
| `localLiftDifferenceFor_cocycle` | `RQS.LawGeneratedLargeConormalDescent.localLiftDifferenceFor_cocycle`(補助: small 版 `:369`) | 同 `:249` | reuse |
| `connectingClass_choice_independent` | `RQS.LawGeneratedLargeConormalDescent.connectingClass_choice_independent`(補助: small 版 `:447`) | 同 `:330` | reuse |
| `correctedLocalLiftCochain_d_eq_zero` | `RQS.LawGeneratedLargeConormalDescent.correctedLocalLiftCochain_d_eq_zero`(補助: small 版 `:658`) | 同 `:541` | reuse |
| `amalgamatedCorrectedLocalLift` | `RQS.LawGeneratedLargeConormalDescent.amalgamatedCorrectedLocalLift`(補助: small 版 `:849`) | 同 `:711` | reuse |
| `connectingClassFor_isZero_iff_nonempty_globalLift` | `RQS.LawGeneratedLargeConormalDescent.connectingClassFor_isZero_iff_nonempty_globalLift` | 同 `:779` | reuse |
...
| --- | --- | --- |
| C1 #3762 | §3, §4.1, §4.2(generic), §4.8 | `Face.hom`、増加添字三項複体、補題2.1A bridge(`ofOverlapPackage` / `toCoverRelativeCechCover`)、`SupportedWord`/`wordRestrict`/`relSpan`/`MSem`、命題3.3、定義3.4、mono 自動成立 instance |
| C2 #3763 | §4.5, §4.6 | torsor 帰結(freeness / faithfulness)、`r_sem`/`r_E` 生成、補題4.3–系4.5、補題5.4 |
| C3 #3764 | §4.3, §4.4, §4.7 | `chiHom`、補題6.2A、定理6.3(`SagaPresentationCore` + completeness 仮定)、系6.4–6.5、例6.6 negative fixture |
...
| C5 #3766 | §3.2, §4.8, §4.9 | 定義8.1(`TopologicalMonomorphicCover` + topology-wide sheaf condition)、定理8.2、系8.3(site 全域拡張仮定込み)、`P_E` 側 normalization 条項 |
