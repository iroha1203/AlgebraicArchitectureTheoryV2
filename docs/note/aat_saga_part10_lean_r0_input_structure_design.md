...
status: R0 fixed(本 note が #3757 R0 の固定成果物。R1 改訂: PR #3775 レビュー finding 1–7 反映。R2 改訂: 敵対セルフレビュー4並列の finding 反映)
...
- `Q` / `PQ` の Int_{≤2} データは省略 intersection を対象に持たないため、
  ...
  その `State` に対して同形の条項を追加する(C5 が固定)。入力8 のうち型を
  R0 で固定しないのはこの `P_E` 条項の1点のみであり、本 note の固定範囲からの
  明示的除外として扱う(`IntersectionLiftSystem.State` が省略 intersection に
  carrier を持たないための構造的先送り。消費先の系8.3 と同じ C5 に帰属)。
