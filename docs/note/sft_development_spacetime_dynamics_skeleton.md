...
- 作成: 2026-07-04(第1版)/ 同日 第2版(敵対レビュー 5 本・MAJOR 29 件 MINOR 11 件を反映。§13 参照)
...
1. **fiber は抽象の Atom family で index する**。「コードベース C_t が A_t を定める」という
   ...
   AAT 側の正規記法は `X^{V,U,J,k}`(付録 A.2)であり、SFT はその selected family 版を使う。
...
橋渡し定理(第I部必置):
  実現した歴史(𝒯 上の整合族)は T_Π の解軌道である
  (solutions-as-orbits。これで「記録」と「法則」が接続される)。
```
...
追跡力学(pursuit dynamics):
  ...
  誤差力学(模型の標準形):
    e_{t+1} <= e_t − δ_t + v_t
    δ_t = 修復容量(その段で費やされた repair の効果)
    v_t = 目標移動によるギャップ増分(要求変更・環境変化)
...
- **追跡可能性**: 修復帯域が目標速度を上回る regime(sup v < inf δ)では
  ギャップは有界に留まる(究極有界性)。
- **追跡不可能性**: 逆の regime(v > δ)ではギャップは線形成長する。
  修復に必要な最小コストの下界は AAT 第VIII部 系8.7(Essential Repair Lower Bound)が与える
  — **修復下界は開発速度の限界**として読める。この理論で最も本物になり得る不可能性定理である。
- **負債の定義**: 技術的負債 = 追跡誤差の時間積分(ギャップの累積)。
...
| --- | --- | --- | --- |
| 序 | 新しい対象 | 開発系 𝔇、生成子と挙動空間、規律、非主張 | README・付録(規律) |
...
| VI | 力学: 運動の法則 | 追跡定理 T0、質量収支則、方針階層と CLF、アトラクター(吸収類)、ゲージ商定理 | 第IX部 §5–6(certified)、第VIII部 定理8.5–8.6・系8.7(certified)、第VII部 §8–12(定義) |
...
- cover 公理は「宣言された admissible family から生成される位相」で閉じる(D8)。
  正準有限実例に criss-cross(base 選択の非一意性)を必ず含める。
...
力(再定式化):
  f = (e, u_f, W_f, C_f, φ_f)
    e : t -> t′ は 𝒯 の辺、u_f は selected 輸送(complement 同一視を含む)
    W_f = 台(触る範囲の selected context family)
    C_f = selected complement family({W_f, C_f} が selected cover をなす。
          complement は補集合演算ではなく選択データである)
    φ_f : Φ|_{W_f} -> Φ′|_{W′_f}
  「台の外では恒等」の正確な形: u_f に沿った C_f 上の層データ比較が恒等。

locality 公理:
  φ_f は Φ|_{W_f} のみに依存し、W_f 上の domain 条件を満たす任意の配置に適用できる
  (patch 型の部分局所作用素)。これが力の再適用・輸送を可能にする。

commute(primitive; Darcs 型):
...
**T1 は単一の分類ではなく、マージ構成の進行に相対的な三段 filtration である。**
...
```text
T0 追跡定理 [Model theorem](旗艦):
  修復帯域 r > 目標ドリフト v ⇒ ギャップ汎函数は有界に留まる(Foster–Lyapunov 型)
  r < v ⇒ ギャップは線形成長
  r ≈ v が第VIII部の相境界を与える(D6 の標準形)
...
ゲージ商定理(「Noether 型」の置き換え; 不変量命題):
  cost がゲージ(semantic 零の力)不変なら、値函数・到達集合・衝突類は
  ゲージ商に降下する(商力学系の well-definedness)。
  「リファクタは可能性を変えない」の正確な形。変分構造を定義するまで Noether を名乗らない。
```
...
| --- | --- | --- | --- | --- |
| T0 | 追跡定理 | 修復帯域 r > 目標速度 v ⇒ ギャップ究極有界; r < v ⇒ 線形成長; r ≈ v が相境界 | VI, VIII | Model theorem / AAT VIII 系8.7(certified)を下界側で使用 |
| T1 | マージ降下定理 | merge は三段 filtration(cocone 判定 / 法則 Čech H¹ / 導来 Tor)で診断され、解決は貼り合わせの選択である(最後の一文は原則) | III | Certified bounded 目標 / AAT IV・V・X(certified) |
...
| T5 | 測地線・悪い吸引域定理 | 内生 cost + 需要過程の下で、貪欲方針は mud 類に吸収され、修復比率 ρ > ρ_c で質量有界(比較形) | VI | Model theorem(D6 充足: ρ_c が相境界) |
...
| 輸入禁止 | — | — | Bianchi 恒等式、Jacobi 場・測地線偏差、gradient flow(離散貪欲降下に存在・一意性理論はない)、臨界指数・普遍性、エネルギー保存則 |
...
| **ForecastCone** | **廃棄** | 後継 = 変形空間(第VII部)+ 挙動空間上の到達集合(第VI部)。専用ブランドは持たない |
...
| 資産 | 現状 | 提案 |
| --- | --- | --- |
| `docs/sft/software_field_theory.md` | 旧本文 1551 行。**保護ファイル**(CLAUDE.md・sft guideline) | 同 path で全面書き換え(SFT の同一性は保つ)。授権 = 2026-07-04 のユーザー指示と §12 決定 |
...
| `docs/sft/sft_theorem_roadmap_and_research_vision.md` | ForecastCone Descent を主定理とする定理ロードマップ | `docs/archive` へ退避(新旗艦 T0–T8 が後継)。`docs/aat/lean_theorem_index_classical_aat.md` からの参照を P5 で更新し dangling を防ぐ |
...
| 項目 | 部 | リスク | 進め方 |
| --- | --- | --- | --- |
| 生成子 T_Π と挙動空間、solutions-as-orbits 橋渡し | I | 低〜中 | 本文内で構成(有限 coalgebra)+ Lean L1 |
...
L2: commute 部分演算と遠隔可換定理(台が素 ⇒ commute 定義済み・transport 自明)(第II部)
...
| P1 | 序 + 第I・II・III部(時空・力・マージ = 数学的心臓部)+ 正準有限実例 + H1–H4 の台帳先置き | Claude 直接執筆 + 敵対レビュー。**Lean L1–L3 を並行着手** |
| P2 | 第IV・V部(組織・Conway。V は研究課題どまりでよい) | Claude 直接執筆 + 敵対レビュー |
| P3 | 第VI・VII部(力学・変形) | Claude 直接執筆 + 敵対レビュー |
| P4 | 第VIII・IX・X部 + 付録(統計力学・制御・寿命・境界) | Claude 直接執筆 + 敵対レビュー |
