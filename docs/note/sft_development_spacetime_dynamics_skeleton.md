...
D5 主張の梯子(claim ladder):
  ...
  Empirical hypothesis(現実の開発系についての仮説)は SFTのGOALとIssueで管理し、
  AAT本文の数学命題とは分ける。SFTのFormal theorem / 定理候補は
  対応する `Formal/` declaration と本文を直接突合する。
...
- Part X(SAGA 蒸留)の進行と独立(AAT 本文は無変更のため衝突しない)。
