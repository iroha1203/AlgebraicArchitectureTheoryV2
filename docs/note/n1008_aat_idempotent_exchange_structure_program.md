...
configuration の上に相異なる二つの `ArchitectureObject` がある。G-116 Cycle 6 は、任意の
carrier と任意の configuration に対し `StructureMaps` を `ULift PUnit` と `ULift Bool` に
選んだ二 object を構成し、この存在を
`exists_distinct_architectureObjects_over_configuration` として証明した。したがって no-go
は witness を theorem の引数にせず、この一般定理から取る。`adm` は省けない。
