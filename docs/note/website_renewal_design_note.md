...
| --- | --- | --- | --- |
| AAT | `docs/aat/algebraic_geometric_theory/`(第I〜X部+付録) | **確定**。第X部 SAGA まで含め正典宣言済み。Lean status は `Formal/AG` source を直接確認 | 全面リライト可能。今回の主戦場 |
...
| ArchSig | `tools/archsig/README.md` + `docs/tool/` | v0.4.0 稼働中。ただし v0.5.0 再設計で CLI 名(pr-review → compare/gate)、フラグ、artifact 版数、後方互換が全面変更予定 | 概念・責務境界・5値 verdict・theory anchor は v0.4.0→v0.5.0 で不変なので先行執筆可。CLI / schema の具体は v0.5.0 出荷後 |
...
- production branch = `main`。**ブランチごとに自動プレビュー URL が出る**ため、
  Wave 1 以降の受け入れテストはプレビュー URL 上の実ページ確認で行える。
- 移行手順: 現行の「`website/` を直接配信」から上記ビルド設定への切替は
  Pages ダッシュボード操作(ユーザー作業)。rollout W0 の受け入れ手順に切替タイミングを
...
- Cloudflare Pages のブランチプレビューでビルド・配信が確認でき、production 切替
  手順(ビルド設定変更 → マージ)が `docs/website/README.md` に記録されている。
...
- website 独自の theorem claim を置かない。theorem ラベルの Lean proved 表示は
  `Formal/AG` に対応 declaration がある場合のみ。それ以外は「本文内の数学命題」。
...
- 現行 20 ページ(v1 三部構成: field-and-force / computation / software-evolution)は
  ...
  再編する。ルート詳細は発火時に `docs/website/SITEMAP.md` で確定する(v2 本文がまだ動いているため、
...
### 7.4 v0.5.0 tooling との結合(推奨)

v0.4.0 準拠の詳細マニュアルは v0.5.0 再設計で書き捨てになるため書かない(従来方針の
維持)。その上で、Wave 3 を v0.5.0 の後始末ではなく**受け入れ面**として結合する:

- website マニュアル完全版は `docs/website/DESIGN.md` と現行 website source に置く。
  v0.5.0 の恒久的な受け入れ規律は tooling guideline / test に置き、getting-started の手順が
  fixture 上でコピペ完走することを検査する。
...
2026-07-05 ユーザー決定。web フロントは Claude が直接実装し、PR 単位で進める。
恒久的な設計は `docs/website/DESIGN.md`、route は `SITEMAP.md`、運用規律は guideline / README を
正本とする。受け入れテストはユーザーが Cloudflare Pages のブランチプレビューで行う。
...
| D | 実装体制 | Claude 直接実装。PR-W0 / W1a / W1b / W1c の 4 本(§9) |
