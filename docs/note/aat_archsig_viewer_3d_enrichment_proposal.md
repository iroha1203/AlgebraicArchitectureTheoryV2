...
### 既存 PRD #2082 との関係(重複回避・補完)

memory の「未マージ」記述は古い。**#2082(`archsig_viewer_gluing_geometry_prd.md`)は PRD だけでなく PR #2088/#2094/#2103/#2110/#2116 で実装まで完了済み**。#2082 は「貼り合わせの歪みを幾何で見せる」を問いとし、nerve / cocycle ribbon / locus field / obstruction cage / repair morph の**静的な幾何描画**と、`axisMapping` の HUD 表示、visual encoding の意味固定、scene 単位の non-claim を提供している。
