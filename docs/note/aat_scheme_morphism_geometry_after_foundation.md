...
\[
\mathcal X_\rho:\mathcal T^{op}\longrightarrow\mathbf{AATSch}^{\mathrm{law}}_U
\]

として読む。\(\rho\) はtrace objectとarrowをAtom / Law geometryへ送るdevelopment readingである。

各trace arrow \(t\to t'\) はarchitecture event

\[
f_e:X_{t'}\longrightarrow X_t
\]

へ送られ、現実のevent \(e:t\to t'\) との関係を

\[
\operatorname{Realizes}_\rho(e,f_e)
\]

で表す。この反変性はcoordinate-ring mapからscheme morphismを得る向きと一致する。AAT内部の
architecture event categoryは通常の向きのcategoryであり、反変なのはdevelopment traceのreadingである。
...
その疎結合client contractの最小形を、次の `ArchitectureEventEvidence` として固定できる。

```text
ArchitectureEventEvidence
  source:
    ArchMap
    LawPolicy
  ...
  event:
    ...
    candidate ring homomorphism
    decoration compatibility evidence
    ideal transport certificate
  reading:
    AAT event identifier
    realization relation
    applicable theorem references
    selected evidence scope
```

これはAAT objectやmorphismの定義を変更しない。AAT側で定義済みのeventを、ArchSig clientが選択された
evidence contractから同定し、その幾何学的性質をanalysis packetへ写すためのdownstream contractである。
...
### 15.5 Algebraic graph transformation

[Wermelinger and Fiadeiro, *A Graph Transformation Approach to Software Architecture Reconfiguration*](https://doi.org/10.1016/S0167-6423(02)00036-9)は、
software architectureのreconfigurationをalgebraic graph transformationとして与え、component state、
structural constraint、architecture-level computationを一つの変換意味論へ接続する。

AATはAtom graph上のrewriteを入力presentationとして受け取り、

\[
\text{algebraic graph rewrite}
\longrightarrow
\text{law-algebra homomorphism}
\longrightarrow
\text{decorated scheme morphism}
\]

というcomparisonを構成できる。double-pushout / single-pushout側の普遍性と、scheme側のlawful closed
subscheme、nilpotent structure、Tor、deformationを接続することが研究上の交点になる。
...
発火させる。このtrackは二段目でlaw idealがunit idealとなるlaw-extinction referenceであり、
lawful closed subschemeが空になる過程をexact transportとして記述する。

これと並行して、lawful geometryが二つのnon-identity eventを通して残るpersistence referenceを固定する。
...
\[
\psi_1:B_0\longrightarrow B_1,
\qquad
\psi_2:B_1\longrightarrow B_2,
\]
...
各 \(K_i\) をactual closed-equational Law readingから生成し、対応するarchitecture scheme morphismを
\(G_1:Y_1\to Y_0\)、\(G_2:Y_2\to Y_1\) とすると、IdealSheafDataの等式

\[
G_1^*{}_{\mathrm{ideal}}\mathcal K_0=\mathcal K_1,
\qquad
G_2^*{}_{\mathrm{ideal}}\mathcal K_1=\mathcal K_2
\]

を証明できる。最終lawful locusは

\[
Y_2^{\mathrm{law}}
\cong
\operatorname{Spec}\mathbb Z[x,x^{-1},(1-x)^{-1}]
\]

であり、二つのnon-identity exact eventとそのcompositionを通して非空のlawful geometryが残る。

object-levelのlaw comparisonにはreference readingが生成するglobal-section ideal chain
...
7. extinction referenceとpersistence referenceの両方で、exact transport caseのlawful squareがpullbackになることを証明する。
...
各研究軸は、必要な標準幾何と最初のreference modelに応じて次の三段階へ配置する。

| 構成段階 | 研究軸 | 必要なregime | 最初のreference model |
| --- | --- | --- | --- |
| foundation直後に構成 | categorical law geometry、morphism calculus、correspondence、arithmetic / formal、Rees / blow-up、derived zero locus | actual scheme morphism、generated ideal sheaf、closed immersion、base change | §17のlocalization、§20.1--20.5 |
| 明示仮定の下で定理化 | six operations、singularity / dynamics、moduli / higher descent | proper / smooth / perfect等のstandard hypotheses、effective decorated descent、representability | §20.6--20.7と個別のfinite-type family |
| 長期統合 | motive、spectral geometry、classifying topos、étale Galois、operad、Poisson / Hall structure | 前二段階のcomparison theoremとcoherence | classical truncationまたはrealization functorを持つ有限presentation model |

第一段階はfoundation completion後のactual Mathlib scheme APIから開始する。第二段階以降は表に記した
standard hypothesesをstatementへ組み込み、各reference modelでcomparison mapを構成してから一般化する。
...
では \(2\in k^\times\) のregimeでdegree、ramification locus、discriminant \(4t\) が一つのevent invariantになる。
...
最後にfpqc descentを構成する。固定されたglobal scheme上ではmorphism、quasi-coherent generated ideal、
decoration mapのfpqc descentを用いて、Čech cocycleを満たすlocal law-compatible eventをglobal eventへ
貼り合わせる。scheme object自体の再構成は、affine chart descent、quasi-affine structure morphism、または
effective decorated descent datumを備えたregimeで行う。ideal inclusionとexact equalityはfaithfully flat
base change後に検査できる。最初のreference theoremはaffine faithfully flat base changeに固定する。
...
はclosed immersion \(Z_n\hookrightarrow Z_{n+1}\) のtowerが定めるformal schemeであり、その構造層は

\[
\mathcal O_{\widehat X_Z}
=
\varprojlim_n\mathcal O_X/I^{n+1}
\]

で与えられる。これにより全infinitesimal neighborhoodを保持する。associated graded
...
=
Z^{\mathbf R}(\lambda_X\alpha-\varphi^*\lambda_Y)
\]
...
### 20.2 Exact-event persistence package
...
\mathbb Z[x,y,x^{-1}]
\longrightarrow
\mathbb Z[x,y,x^{-1},(1-x)^{-1}],
\qquad
(xy)\longmapsto(y)\longmapsto(y)
\]

を使い、二つのnon-identity scheme morphism、generated IdealSheafDataのexact transport、lawful
closed-subscheme squareのpullback、composition、非空の最終lawful locusを同じdiagram上で証明する。

### 20.3 Arithmetic specialization package
...
### 20.4 Rational event completion package
...
### 20.5 Derived Law dependency package
...
### 20.6 Semistable degeneration package
...
### 20.7 Finite branching and dynamics package
