...
**Semantic Geometry of Architecture**(命名記録は表示的意味論ノート §11。
本リポジトリでは略さない)の
...
2. 意味のモジュライへの問いを台帳にすると、静力学(存在・修復・座標・決定
   可能性・位置・観測限界)は定理で埋まっている。戦略的重心は点の不変量
   から、射(輸送・自然性・base change)の理論へ移る。
...
現在既証なのは、独立した二系列である(両者を結ぶ cover–nerve 辞書は未構成
であり、SAGA の `Q_E` は torsion を持ちうる一方、G-107 の K0 / K1 は `ℚ`
固定 — §3 参照)。その上で、統合正方形(§3)が成立した暁の到達像を一行で
言えばこうなる: **SAGA が修復障害の意味を与え、G-107 がその意味を診断・
比較できる範囲を与える。** この分業の成立自体が、本ノート固有の中心定理
候補である。
...
既証の範囲を正確に言えば「この source-labelled・全-support 生成の regime
では、選別なしの充填が非自明性を消す」である。その先の一般命題 — 診断幾何の
非自明性は選択(law-selection・relative cohomology・semantic quotient の
いずれか)を要する — は未証の candidate であり、この三方式の比較こそが
係数選択理論の内容になる。反証は、その理論が空でないことを示した。反証までに放電
...
(c) ソフトウェア設計理論の内製不可能性定理。ただしこの調査は簡易検索に
基づくものであり、論文化の際は系統的文献調査を前提とする。新規性の主張は
related-work matrix で行う — Hanf / Gaifman、1-WL、LCL / local
certification、有限モデルの非定義可能性、mechanized lower bounds を列に
取り、G-107 の独自性を「局所不可視」一般ではなく
`decidable + fixed observation grammar + machine-checked separating pair +
software-design semantics` の**交点**として絞る(LCL 系はそれ自体が厚い
系譜を持つため、この絞り方が安全で強い)。
...
| 選択の自由度 | 大域表示は `H^0` torsor | 証明済み(conormal descent の global lift fiber torsor 定理) |
...
第一に、**静力学の上半分が定理で埋まった**。存在は障害類消滅へ還元され
(第X部定理8.2 — 一般の existence decider ではない)、一様不変性の側には
sound / complete decider がある(G-107 (ii))。既知の安全地帯の位置が確定
し、観測限界線まで引かれた測量済み地帯を、意味の空間は持つ。

第二に、**戦略的重心は「射」の側へ移った**。輸送・変形・base change は
意味の空間の点ではなく射の振る舞いを問う。ただし未測量域のすべてが射側
なのではない — 飽和の一般化・底空間の供給・affine 被覆の存在・representing
object の存在は対象・存在側の未測量域である。台帳はおよそ三群(対象・
存在/射・整合/係数・選択)に分かれ、後続カードはこの三群のどれを埋めるか
で位置づけられる。
...
の ambient 版(`S_X` 上の presheaf 自然同型を与える第2文)と系 8.3 が既に
この形を採っている。ただし全 `S_X` への量化は過剰で
...
stack 経路のみ。S4 は二層に分けて依存を読む — 数学的な零類同値(一段目)は
S0–S3 と G-107 (i) を消費し、(ii) の decider は S4 を計算可能な判定系に
する場合(executable corollary)にのみ要る。S5 は部ごとに依存が違う:
S5(1)(2) は S3 に、S5(3) は S3 と modification calculus に依存する。

したがって並走計画の実質は「G-106 が走る間に S0 → S1 / S2 を放電可能な形
まで固める」ことであり、統合定理の可行性は当初見立てより高い。左列は
G-106 の完成を待たない。

failure policy の素描。方針として、各 S の失敗は **falsifier**(現
candidate の反証)と **salvage**(救済先)を区別し、salvage への移行は
**別候補への交代**として数える — 現 candidate の成功に数えない。この区別が
統合定理の反証可能性を保つ。

- S0 が material class で成立しない場合、現統合定理は **Stop** である
  (別の辞書設計はゼロからの新候補)。
- (H_Q) の非空性 witness が立たない場合、`Q_E` の filtration の graded
  piece が subnerve 上定数になる形の直接比較定理へ後退する — これは
  **別候補**であり、S1′ の成功ではない。G-107 は graded piece ごとの判定
  装置として間接利用する。
...
- S3 の反例は統合定理の refuted ではなく、`τ_sem` の輸送設計(G-106 の
  2-cell 語彙)の改訂材料として扱う — ただし statement 改訂後の再挑戦は
  新 candidate であり、現候補の成功に数えない。
...
の」合成整合は G-106 の固定 target statement の対象外(frontier の観察に
留まり未構成)なので、
...
を設計する)に従うと、幹より準備コストが高い(G-106 の readiness は
§4.2 の通り)。
...
推奨路線は幹優先である。本ノートは判断材料の提供までを担い、採否・実行順の
運用上の正本は GOAL カードと tracking Issue 側に置く。順序と成果物:
...
**2. 並走: 統合定理の設計 — S0 辞書ノートを先行させる(推奨)。** S0 で
object class・像・比較写像を型として固定できて初めて、「二つの頂」という
比喩は一つの定理圏に入る。S0 が material class で成立しなければ現統合定理は
Stop であり、その判定を先に取ることが S1–S5 への投資を守る。S0 完成後に
「S1 / S2 / S3 設計ノート」を続ける(一本化は起草時に判断)。目次案:
...
**3. 並走: 論文A — 実証節 PRD の起草と本文執筆。** 投稿 gate の充足は
Atom ノート §12 が記録しており、隣接する近接研究(sheaf-cohomological
program analysis 系)が既に公開されている以上、証明済み成果の非公開期間の
延伸は先取権リスクである。執筆・PRD は loop と資源競合しない見込み。PRD は
規律(冒頭「問い」節+候補複数提示)に従う。
...
2. **係数 base change カード**(`ℚ` → R)— 山頂着手条件 (a) そのもの。
   ...
   引き受ける。設計方向の candidate として、単なる係数置換ではなく
   **jump-locus の相対幾何**へ上げる案を持つ: `J_A = (dim ker, dim coker)`
   は体上の点ごとの数値なので、一般 base では kernel / cokernel module・
   Fitting ideal・determinantal locus を本体に据える。すると G-107 の
   decider は各点での計算、jump locus はそれらを束ねる scheme-theoretic な
   対象、SHIGURE はその moduli 的受け皿になる — G-107 から山頂への直結路
   候補であり、起票時の第一検討事項とする。
...
路線の理由は三つ。G-106 の readiness(§4.2)。統合定理・base change・Gr4 は
相互に独立に進められる部分が大きく、S0–S2 は G-106 と並走できること。
ノート・PRD・執筆は loop と資源競合が小さい見込みであること。後継
certificate 素材は G-106 成果の後のほうが設計精度が上がる(modification
calculus の対象 `J_A` の輸送的性質が固まるため)。
...
- SFT(意味の空間の上の動力学)と tooling への換金(decider の ArchSig
  ...
  その座位だけを記録する。
- 命名の参照: Semantic Geometry of Architecture は略さない(記録は表示的
  意味論ノート §11)。SHIGURE は山頂 candidate の呼び名案である。
- 隊列は指針であり、採否・順序はユーザー裁定を正とする。運用上の状態
  (active 化・Issue・loop 起動順)は GOAL カードと tracking Issue 側が
  正本である。
