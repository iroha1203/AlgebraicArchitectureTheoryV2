...
- `Formal/`(Lean, 139ファイル): graph/walk/monodromy 等、**先行体系(有限・構成的AAT)の形式化**。AG版の定理ラベル(Stanley–Reisner、Nullstellensatz、LawConflict、gerbe 等)は対応するLean declarationのstatementとproofを直接確認する必要がある。本文に書かれたlabelだけではLean proved claimとは読まない。
...
**E4. 未定義語の台帳化。**
`exact` / `complete` / `witness exactness` / `U-flat` / `NoHigherBoundaryObstruction` / `trace topos` / `operation homotopy` / `tangent rank` 等、定理の仮定に現れる未定義語を本文とGOALで整理し、各語に「定義予定 / 意図的プリミティブ / 廃止」の状態を付ける。型1病の系統的解消法。
