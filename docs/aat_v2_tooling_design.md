...
PR は、作成者が人間、AI agent、generator、またはそれらの組み合わせであっても、
同じ architecture evaluation pipeline で扱う。tool は作者属性ではなく、
変更が architecture extension として何を追加・変更したかを報告する。
`ai_session` は provenance metadata の一種であり、判定基準そのものではない。

```text
Generated-change provenance metadata:
...
AI generated であること、または human-authored であることは、良い claim にも悪い claim にも
直接変換しない。PR は evidence、measurement boundary、theorem precondition、
policy によって平等に評価する。provenance metadata は traceability と review のための
evidence として扱う。
...
- 作者 provenance や generated status を architecture lawful とみなさない。
...
すべての PR は、同じ claim taxonomy と measurement boundary で評価する。
`feature.source = ai_session` は、その変更に AI session provenance があることを示す
metadata であり、PR の評価基準を変えない。AI が生成したという事実も、
人間が作成したという事実も、良い claim にも悪い claim にも直接変換しない。
AI session metadata は、traceability、human review、unsupported construct、
hidden interaction の保守的評価に使う。
...
generated patch summary を作り、追加 component、追加 relation、policy touch を
...
- generated-change provenance / human-review checker
```

この checker の結果は、claim taxonomy と split status に直接反映する。
たとえば coverage が不足している場合、non-split witness が出ていないことを
split の証拠にしてはならない。
generated-change provenance / human-review checker は、作者属性を positive / negative claim に
変換せず、未レビューの generated formal claim を `missingPreconditions` として report に残す。
...
CI report は、PR の作成者にかかわらず次を強調する。

```text
- code diff が増えた場所ではなく、architecture extension が起きた場所
- before invariant が保存されたか
- hidden interaction が増えたか
...
generated-change provenance がある PR では、unsupported construct と unmeasured axis を
保守的に扱う。ただしこの保守性は AI 由来だからではなく、測定境界と review evidence が
未確定な claim を昇格しないためのものである。adapter、cache access、policy bypass、
implicit state sharing は、作者にかかわらず hidden interaction candidate として report する。
この candidate は `hidden_interaction` の measured witness ではなく、
`generatedPatchSummary.reviewWarnings` の conservative review signal として表示する。
`relationComplexity` や changed relation の増加も同様に complexity transfer warning として表示し、
`introducedObstructionWitnesses` とは混同しない。
...
source_witness_refs: list
source_coverage_gap_refs: list
...
Feature Extension Report の `repairSuggestions` は、対象 witness または coverage gap、
適用した registry rule、必要前提、期待される局所効果、副作用、non-conclusions を
traceable に保持する。registry rule がない manual suggestion も advisory signal に留まり、
repair success、all obstruction removal、global flatness preservation、empirical cost 改善は
結論しない。
...
- generated-change provenance strict-mode example
```

各 benchmark は、input repository / AIR / Signature artifact / Feature Extension Report /
expected claim classification を持つ。

B9 では benchmark suite を `archsig-benchmark-suite-v0` として freeze し、対象 fixture、
期待する measurement boundary、更新ルールを
`detectable-values-reported-axes-catalog-v0` に記録する。fixture 更新は単なる snapshot
更新ではなく、axis の追加・改名・削除・measurement boundary 変更を伴う場合に
compatibility review を要求する。

Generated-change provenance workflow の canonical AIR fixtures は、次の report 境界を固定する。
これらは AI 由来 PR を特別扱いするためではなく、すべての PR に対して provenance、
review evidence、measurement boundary を同じ claim taxonomy で扱うための fixture である。

```text
- ai_session_split_candidate.json:
    human-reviewed generated patch を split candidate として trace する。ただし生成由来を
    lawfulness evidence へ直接変換しない。
- ai_session_generated_patch.json:
    generated patch artifact / evidence / operation trace が追加 component / relation を指す。
- ai_session_unreviewed_formal_claim.json:
    human review 前の formal claim promotion を BLOCKED_FORMAL_CLAIM に留める。
- ai_session_hidden_complexity_warning.json:
    hidden interaction candidate と complexity transfer warning を conservative review signal として扱う。
- ai_metadata_missing_unmeasured.json:
    generated patch artifact があっても AI session metadata と測定境界が欠ける場合は
    unmeasured のまま扱う。
```
...
Repair / synthesis prototype の canonical fixtures は、次の tooling 境界を固定する。

```text
- repair_synthesis_boundary.json:
    repair suggestion、synthesis constraint artifact、valid no-solution certificate を同じ
    AIR 上で trace する。ただし repair success、global flatness preservation、
    solver completeness へ直接変換しない。
- synthesis_constraints_candidate.json:
    produced candidate を soundness package / theorem precondition boundary へ接続するが、
    solver completeness は主張しない。
- synthesis_constraints_no_solution_certificate.json:
    no-solution claim は certificate refs と valid certificate claim refs で支えられる。
- no_solution_certificate_valid.json:
    finite candidate universe と case refs に相対化された certificate checker の小例。
```
...
- generated-change human-review checker
- 全 PR を同じ claim taxonomy / measurement boundary で扱う Feature Extension Report
- generated patch と architecture operation の対応
- hidden interaction / complexity transfer の warning
  - `generatedPatchSummary.reviewWarnings` は conservative review signal であり、measured obstruction ではない
```
...
```text
- PR history dataset
  - schema: `docs/design/pr_history_dataset_schema.md`
  - CLI: `archsig pr-history-dataset`
...
Detectable values / reported axes catalog は
`detectable-values-reported-axes-catalog-v0` として versioning し、
`tools/archsig/tests/fixtures/minimal/detectable_values_reported_axes_catalog.json` に
canonical fixture を置く。catalog は `measuredZero`, `measuredNonzero`,
`unmeasured`, `outOfScope` の区別、axis ごとの evidence requirements、theorem refs、
non-conclusions、benchmark expected boundary を保持する。
...
Axis の追加・改名・削除・measurement boundary 変更は `archsig schema-compatibility` の
backward compatibility test で `requiresMigration` として扱い、unmeasured value を
measured zero evidence に丸めない。
...
- generated code や human-authored code の正しさを作者 provenance から自動保証する。
