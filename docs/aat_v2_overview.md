...
`runtimePropagation`, `relationComplexity`, `empiricalChangeCost` は static structural core の QED には含めない。`runtimePropagation` の初期 Lean metric は、0/1 `RuntimeDependencyGraph` 上の `runtimeExposureRadius` として `reachableConeSizeOfFinite` を再利用する。この 0/1 graph が与えられた後の zero metric と runtime obstruction absence の bridge は、将来の数学的コア拡張として Lean 証明対象にできる。最初の obstruction は `reachesWithin G components components.length` ベースの measured / bounded obstruction として定義し、`some 0` は測定 universe 内の `source != target` な runtime reachable cone が空であることを表す。semantic `Reachable` 版は `ComponentUniverse` coverage / edge-closure 下の bridge を介して扱う。一方、runtime edge metadata から 0/1 graph への projection rule、実コード extractor の完全性、incident scope / 障害修正コストとの相関は tooling / empirical protocol 側の課題として残す。既存名 `runtimePropagationRadius` / `runtimePropagation` は exposure 側の互換名であり、障害源から影響を受け得る範囲を測る `runtimeBlastRadius` は reverse reachability 由来の別 metric として分ける。Circuit Breaker coverage は raw `runtimePropagation` に混ぜず、未保護 edge から作る `unprotectedRuntimePropagationRadius` / `unprotectedRuntimeBlastRadius` と coverage ratio の policy-aware extension として分離する。`relationComplexity` の初期設計では、状態遷移代数層の `constraints`, `compensations`, `projections`, `failureTransitions`, `idempotencyRequirements` を構成要素として保持し、合計値は派生指標として扱う。
