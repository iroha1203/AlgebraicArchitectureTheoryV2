...
以下の Lean コードは、実装済み API から記事用に抜粋したものです。読みやすさのために namespace、import、proof body の一部は省いています。

AAT は、ソフトウェア開発を単なるコード変更の列としてではなく、architecture の拡大、分解、修復、合成の理論として扱います。

中心命題は、Lean では一つの大きな等式ではなく、operation と proof obligation を束ねる package として現れます。
...
ここから続く力学編も同じ方針です。重要なのは、AAT が「それっぽい数学用語」ではなく、何を仮定し、何を証明し、何を結論しないかを型に持たせようとしている点です。
