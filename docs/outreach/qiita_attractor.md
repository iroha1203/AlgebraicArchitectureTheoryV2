...
- ArchSig（Architecture Signature）は、その軌道を多軸のシグネチャとして観測するための道具である
:::
...
AI は、その場にある局所的な書き方を高速に再生産する参加者です。
...
- 新機能を足す場所、触ってよい境界、触ってはいけない境界を明確にする。SOLID は局所契約、Layered / Clean Architecture は大域的な依存方向や境界を整える原則として使い、迷ったときに `common` や巨大 service へ吸い込まれないようにする。
- 良いお手本、テンプレート、テスト例、型、interface を用意する。正しく使う方が自然で、悪い接続を作りにくい局所的な作法を作る。
- CI/CD、architecture test、lint、type check、review によって、bad force を早い段階で散逸させる。大きすぎる PR は、force を観測、議論、rollback できる粒度に分ける。
- legacy や generated code を隔離し、悪い局所的な作法が新規実装へ伝播しないようにする。問題が起きたときは、refactor、rollback、feature flag、追加テストで safe region へ戻れるようにする。
...
AI エージェントが出す変更案を、まだ加工されていない力と見ます。

ハーネス、CI、テスト、型チェック、lint、レビューは、それをそのまま通すのではなく、拒否、射影、修復、縮小、再生成させます。

```mermaid
flowchart LR
    raw["AI が出した変更案"] --> dissipate["ハーネス / CI / 型チェック / lint / レビュー"]
    dissipate --> accepted["受け入れられる変更"]
    dissipate --> rejected["拒否される変更"]
    dissipate --> repaired["修正・縮小・再生成される変更"]
    accepted --> codebase["更新されたコードベース"]
    repaired --> codebase
    rejected -. "コードベースには入らない" .-> outside["散逸した力"]
```
...
よいハーネスは、機能を前に進める力を残しながら、負債を増やす力を弱めます。
...
ArchSig は Architecture Signature の略です。

私たちのリポジトリで開発している、コードベースや PR の変化を複数の軸で読むための観測枠組みを指します。依存、境界、抽象化、実行時露出、意味のずれ、テストでの観測しやすさなどを、一つの点数ではなく多軸のシグネチャとして扱います。
...
| 軸 | 見たいこと |
| --- | --- |
| static dependency | 依存方向や禁止依存の破れ |
| boundary policy | 境界を越えた接続や policy bypass |
| abstraction leakage | 抽象を飛び越えた具象依存 |
| hidden interaction | 宣言されていない相互作用 |
| runtime exposure | 実行時依存や露出の増加 |
| semantic mismatch | 観測上の意味のずれ |
| type precision | 型情報の精度や型穴の増減 |
| test observability | 変更がテストで観測できるか |
| repair / refactor | 破れの修復や悪い basin からの脱出量 |
| PR signature delta | PR ごとの多軸変化 |
...
少なくとも、次の三つを分けて見る必要があります。

- 良い変更が自然に選ばれやすい場になっているか。
- 悪い近道が便利な局所解になりにくいか。
- PR の速度に対して、観測、review、CI、rollback の散逸能力が足りているか。
...
コードベース、要求、設計境界、canonical example、review、CI/CD、ArchSig が作る場によって、次に選ばれる PR の向きは変わります。
...
そこで、変更が選ばれ、PR になり、review / CI を通り、merge され、更新されたコードベースが次の変更分布を変えるまでの流れを分解します。状態、操作、観測、不変量、obstruction witness、proof obligation を分けて見ることで、経験則を検証しやすい形にします。
