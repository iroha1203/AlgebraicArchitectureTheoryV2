...
要求の切り方、優先順位、締切、受け入れ条件、レビュー方針は、後続する PR の出方を変えます。

コードを書かない人も、コードベースという場に間接的な力を加えています。

特に AI 駆動開発では、この間接的な影響が大きくなります。

上流で曖昧な要求が入ると、その曖昧さは AI によって高速に PR として具体化されます。
...
だから、AI 時代に重要なのは AI エージェント単体の性能だけではなく、AI が参加する場そのものの設計です。
...
逆に、巨大な `common`、便利すぎるヘルパー、曖昧なサービスがあると、変更はそこへ吸い寄せられやすくなります。
...
- 要求を、良い PR に変換されやすい粒度へ切る。顧客価値、運用負荷、技術的負債、将来の変更容易性を同じ場に入れ、短期的な機能追加だけに偏らないようにする。
- 新機能を足す場所、触ってよい境界、触ってはいけない境界を明確にする。SOLID は局所契約、Layered / Clean Architecture は大域的な依存方向や境界を整える原則として使い、迷ったときに `common` や巨大サービスへ吸い込まれないようにする。
- 良いお手本、テンプレート、テスト例、型、インターフェースを用意する。正しく使う方が自然で、悪い接続を作りにくい局所的な作法を作る。
- CI/CD、アーキテクチャテスト、静的検査、型チェック、レビューによって、悪い変更の力を早い段階で散逸させる。大きすぎる PR は、変更の影響を観測、議論、ロールバックできる粒度に分ける。
- レガシーコードや生成コードを隔離し、悪い局所的な作法が新規実装へ伝播しないようにする。問題が起きたときは、リファクタリング、ロールバック、機能フラグ、追加テストで安全な領域へ戻れるようにする。
- ArchSig やずれの記録で、PR 群がどの方向へ軌道を作っているかを見る。単発の違反検出だけでなく、要求、実装、レビュー、CI が作った力の蓄積を見る。
...
ハーネス、CI、テスト、型チェック、静的検査、レビューは、それをそのまま通すのではなく、拒否、絞り込み、修復、縮小、再生成させます。
...
```
未加工の変更力 = 機能を前に進める力 + 負債を増やす力
ハーネス(未加工の変更力) ≒ 機能を前に進める力 + 弱められた負債の力
```
...
```
Vibe Coding がうまくいくのは、
高速に作られる変更案が、
良いお手本実装、速いフィードバック、測定できる境界を持つ
アーキテクチャの場に流れ込み、
悪い力がたまる前に、
繰り返しの変更が良い領域へ引き寄せられるときである。
```

つまり、高速な PR 生成が、悪い局所文法を増幅するのではなく、良い領域に落ちるよう設計された場に注入される場合です。
...
AI 時代に PR が不要になるのではなく、PR はアーキテクチャの動きを観測し制御する単位として、より重要になります。
...
しかし、強いモーターだけでは、意味や責務のずれ、守りたい設計上の性質の劣化、マージ時の混乱、技術的負債に吸い寄せられる流れが起こりえます。
...
最も安全な AI コーディング環境とは、外部ハーネスが最も強い環境ではありません。

良い変更が局所的に自然で、模倣しやすく、観測可能で、低摩擦であり、悪い近道に落ちる領域が浅く、見えやすく、入りにくいアーキテクチャの場を持つ環境です。

## アトラクターエンジニアリング実践チェックリスト
