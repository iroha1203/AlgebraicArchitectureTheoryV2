...
前半では、設計レビューで使える見方を話します。後半では、その見方を支える数学的な背景と Lean で証明している範囲を軽く紹介します。

この研究で中心に置く名前は、**アーキテクチャ零曲率定理**です。

ざっくり言うと、次の主張を目指しています。

> よいアーキテクチャとは、要求された設計法則を破る witness が見つからない状態である。
> ArchitectureSignature は、その破れを多軸で記録する診断表である。

ここでの「零曲率」は比喩としての単一スコアではありません。
循環依存、抽象化違反、LSP 違反、補償失敗、replay 不整合、境界違反などを、
それぞれ「どの法則を破る witness か」として提示できる状態を目指します。
...
Lean 側では、まず「依存のたびにランクが厳密に下がる」という強いモデルを使います。これは実務上の Layered Architecture をすべて表す定義ではなく、循環依存が起きない構造をはっきり扱うためのモデルです。
...
もう少し設計寄りの例で言うと、具象コンポーネントを抽象コンポーネントへ写す場面があります。
...
このとき、具象側で依存をたどれる関係が、抽象側でも意味のある依存として保たれているかを見たいです。これは単に「辺があるか」を数えるだけではなく、「構造を保つ写し方になっているか」という問いです。

圏論の言葉を使うと、この種の写し方を関手的に読めます。依存関係や到達可能性を抽象側に写しても、必要な構造が壊れていないかを確認する、という見方です。
...
Lean で扱うのは、定義がはっきりしていて、数学的に証明できる構造です。現時点では、たとえば次のような構造的事実を証明済みです。
...
この見方は、AI 駆動開発でも役に立つはずです。

AI にコードを書かせると、局所的には動くコードが素早く出てきます。一方で、生成された変更が依存の向きを壊していないか、境界をまたぐ依存を増やしていないか、fanout を増やしていないかは別の問題です。

AI が実装を速くするほど、人間側には「そのコードはアーキテクチャとして大丈夫か」を確かめる仕組みが必要になります。`ArchitectureSignature` のような多軸診断は、AI が生成した差分をレビューするときの構造的なチェックリストとして使えるはずです。
