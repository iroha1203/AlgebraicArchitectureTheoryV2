...
目的は、変わり続けるソフトウェアに対して、よりよい問いを立てるための語彙を用意することである。
...
つまり、組織構造は operation support を変え、operation support は日々の設計変更を変える。その反復が architecture として codebase に沈着する。
...
ここまでの話を、三つの問いとして整理する。

### 1. AI proposal: 正しさだけでなく次の変更を見る

AI coding agent が変更を提案する。test は通り、表面的には大きな問題はないように見える。
...
AI coding agent が短時間でコードを書けるほど、この問いは重要になる。
問題の核心は、生成速度そのものではなく、生成された変更が field に沈着し、次の変更を誘導する点にある。

### 2. Incident response: 学びを field memory に残す

incident が起きる。原因を調べ、修正 PR を出し、test を追加し、再発防止策を書く。

これらはもちろん必要である。しかし、SFT の観点ではもう一つ問いたい。
...
incident は単なる失敗ではない。見えていなかった invariant や observation boundary を発見する機会でもある。
良い incident response は、bug を直すだけでなく、次の未来の形を変える。

### 3. End-of-life: field を再構成する

ある subsystem が古くなったとき、それを修復すべきか、移行すべきか、縮約すべきか、削除すべきかを判断しなければならない。

この判断も、単なるコスト比較ではない。

```text
repair（修復）は、どの future を残すのか。
migration（移行）は、どの path を開くのか。
contraction（縮約）は、どの operation support を減らすのか。
deletion（削除）は、どの obstruction を消し、どの capability を失うのか。
```

end-of-life は失敗とは限らない。それは、field を再構成する判断である。
...
良い理論は、すぐにすべての答えを出すとは限らない。しかし、よい問いを作り、その問いにどこまで答えられるのかを教えてくれる。
