...
- アーキテクチャの問題は、いつも「明らかに悪い PR」として現れるわけではありません。
- 局所的には妥当な変更を積み重ねた結果、次の変更がだんだん難しくなることがあります。
- この記事では、codebase、Issue、PR、review、CI、incident、AI agent を含む開発文脈を field として捉えます。
- AAT は、変更が何を保存し、何を破ったかを見るための語彙です。
- ArchSig は、review comment を蓄積可能な diagnostic record にする観測層です。
- SFT は、どの future が開きやすくなるかを扱うための field model です。
- 実務上の目標は Attractor Engineering です。良い変更が選ばれやすく、悪い shortcut が繰り返されにくい場を作ります。

## はじめに: 良い PR だけでも、アーキテクチャは悪くなる

アーキテクチャの問題は、いつも明らかに悪い判断から始まるわけではありません。

ある PR は重複を消します。別の PR は既存の service を再利用します。次の PR は、期限に間に合わせるために infra の helper を少しだけ直接呼びます。どの変更も小さい。テストも通る。レビューでも説明できる。Issue も閉じる。

けれど数か月後、システムの手触りが変わっています。一つの機能を直すだけなのに、触る file が増える。ある module を理解するには、別の module の内側の事情を知らなければならない。一度だけのつもりだった helper が、いつの間にか標準ルートになる。図の上では境界が残っているのに、実際の変更はその境界を前提にしていない。

この記事で扱いたいのは、この現象です。

アーキテクチャは、静的な構造であるだけではありません。次の変更がどこへ向かいやすいかを変える field でもあります。

## running example: checkout に coupon を追加する

以降では、checkout に coupon support を追加する例を使います。対象になる領域は、たとえば次の三つです。

- checkout
- payment
- discount

coupon support の実装 path は一つではありません。`DiscountPolicy` を追加し、payment authorization を declared interface の内側に保つ path があります。`CouponService` が必要な情報を得るために `PaymentAdapter` を直接呼ぶ path もあります。UI 側だけで discount を適用し、refund や invoice の意味がずれていく path もあります。

これらは、同じ Issue を閉じるかもしれません。けれど、変更後に残る architecture は同じではありません。この違いを明示することが、この記事全体の出発点です。
...
coupon の例で言えば、AAT が見たいのは「coupon support を追加した」という操作名だけではありません。その変更が payment boundary を保ったのか、discount calculation の rule を保ったのか、refund behavior の意味を保ったのかです。同じ Issue を閉じる変更でも、保存した性質が違えば、architecture としては違う状態に移っています。
...
Invariant は、設計変更の前後で守りたい性質です。coupon の例なら、次のような性質が invariant になりえます。

- payment authorization は payment boundary の内側に残る
- discount calculation は declared policy interface を通る
- refund behavior は同じ domain rule から説明できる
- infra の詳細が domain model に漏れない
...
coupon の例なら、次のようなものが witness になります。

- `CouponService` が `PaymentAdapter` を直接 import している
- UI 側だけで discount logic が適用され、domain model がそれを知らない
...
冒頭の coupon 例に戻ると、良い extension では、coupon の計算は declared interface を通り、payment core への相互作用は明示された boundary に収まります。悪い extension では、coupon service が `PaymentAdapter` や hidden cache に直接依存するかもしれません。あるいは、rounding order や discount application order が場所によってずれるかもしれません。
...
| obstruction type | coupon example |
| --- | --- |
| inherited core obstruction | 既存 payment core にすでに boundary leak がある |
| feature-local obstruction | coupon logic 自体が複雑すぎる |
| interaction obstruction | coupon と refund の意味がずれる |
| lifting failure | declared interface 経由で coupon rule を表現できない |
...
## Part 2: ArchSig - review comment を診断 record にする
...
```text
CouponService should not call PaymentAdapter directly.
```
...
boundary: discount -> payment
invariant: discount logic must use declared policy interface
witness:
  type: forbidden_import
  source: CouponService
  target: PaymentAdapter
...
ソフトウェア進化を研究対象として見る発想には、M. M. Lehman の software evolution research という先行研究があります。Lehman は、長寿命のソフトウェアは運用環境や要求の変化に応じて継続的に変化し、放置すれば複雑性が増す、という見方を示しました。SFT はその問題意識を引き継ぎつつ、Issue、PR、review、CI、incident、AI agent などの artifact と governance を、より細かい計算対象として扱います。
...
PRD が来たとき、それは一つの future を決めるわけではありません。複数の path を開きます。coupon support なら、次のような path がありえます。

- lawful policy insertion path
- hidden adapter dependency path
...
| path class | likely risk | missing invariant | recommendation |
| --- | --- | --- | --- |
| lawful policy insertion | low | discount composition law | acceptance criteria に追加する |
| hidden adapter dependency | high | payment boundary | direct adapter import を block する |
| UI-only discount drift | high | invoice / refund semantics | domain-level discount rule を要求する |
| rounding mismatch | medium | rounding order | shared rounding policy を追加する |
| unknown remainder | unknown | observation boundary | unmeasured として記録する |

この report から、PRD の段階で不足している invariant が分かります。rounding order、discount composition law、payment authorization boundary、refund / cancellation semantics を acceptance criteria に入れるべきかもしれません。SFT は実装後のレビューだけでなく、要求や Issue の切り方にも関わります。
...
Attractor Engineering は、この「未来の変更がどこへ吸い寄せられるか」を設計対象にします。具体的には、field を次のように形づくることを目指します。
...
Incident は、runtime observation が field update を強制する場面です。障害が起きると、これまで見えていなかった invariant が見つかることがあります。missing boundary が見つかることがあります。review で見逃していた shortcut が見つかることがあります。CI に足りない observation axis が見つかることがあります。
