...
この流れによって、従来の lint、依存グラフ、静的解析では見えにくかった設計上の圧を読めるようになります。
...
この記事では、まず `Atom` という観測単位から入り、`ArchMap` と AAT によって ArchSig がどのように設計上の圧を読むのかを紹介します。
...
## ArchSig: ArchMap + LawPolicy から設計圧を計算する
...
ArchSig は、これらを単一スコアに潰しません。軸ごとに圧を出します。

出力される reading は、たとえば次のようなものです。
...
コードベースのどの境界に、どの種類の設計圧がかかっているかを読むための measurement です。

## ケーススタディ: あるバックエンドリポジトリを分析してみた

ここからは、あるバックエンドリポジトリを ArchSig で分析した例を見ます。
...
その上で、どの law を選ぶかを LawPolicy として与え、ArchSig で構造的な圧を測る。
...
しかし、ArchMap と ArchSig analysis packet を渡せば、コードベースの構造的な圧も渡せます。
...
コードベースを Atom の地図として読み、AAT に基づいて設計上の圧を見える形にするための、AI時代のアーキテクチャ解析機です。
