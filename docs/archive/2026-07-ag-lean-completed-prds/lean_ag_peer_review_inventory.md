...
| VI-1 | Part VI toy models | 実装済み | #3099 再是正で `SingularBoundary` は既存実質化を維持し、`OperationSquare` / `TransportDescent` / `RefactorGalois` / `DecompositionGerbe` の有限 toy instance を意味的非退化 witness へ差し替えた。`OperationSquare` は `toyLoopGenerator : Multiplicative (ZMod 2)` の非恒等 monodromy `toyMonodromyAction_nonidentity` と、`equalityDefect false = toyMonodromyDefectFromAction` / moved coefficient を含む `toyMeasuredSquareNonzero_defect_from_moved_coefficient` から defect を読む。selected filling は `mu = 0` ではなく、選択 monodromy が係数を固定するという独立条件で読む。`TransportDescent` は `toyNonemptyFreeWord` が `toyLoopGenerator ≠ 1` へ写る `toyTransportNonzero_uses_second_pi1_element` を `relationBoundaryZero_iff_sendsRelators` と non-descend 証明に接続する。`RefactorGalois` は endpoint equality component と Bool tag を持つ非 subsingleton Hom と `toyGaloisData_preserves_positive` / `toyGaloisData_preserves_negative` により `Preserves` の充足・反証を両方持つ。`DecompositionGerbe` toy instance は `ToyOverlapCompatible` / `ToyEffectiveDescent` / `ToyAutSheafDefined` / `ToyNonAbelianReading` / `ToyLocalDecompositionsExist` / `ToyGlobalCanonicalDecomposition` の positive/negative theorem pair を持つ。`toyGerbeClassFromLocalData loc` は二点局所データの不一致から計算され、`toyGerbeClass_eq_computed_local_class` と `toyGerbeClassFromLocalData_nonzero` が selected gerbe class と local-data computed class の一致および非零性を固定する。`ToyGlobalCanonicalDecomposition loc := ∃ global, ∀ i, loc i = global` は gerbe class zero の別名ではなく、局所 decomposition が単一定数の大域 decomposition へ貼れることだけを読む。`ToyEffectiveDescent classOf := ∀ loc, classOf loc = false -> ToyGlobalCanonicalDecomposition loc` は no-canonical 結論や非零 obstruction を含めず、computed class が zero の場合だけ大域化できる finite descent criterion として読む。negative theorem は片側 chart だけを読む `toyBadGerbeClassFromLocalData` が `toyLocalDecomposition` で破綻することを示す。toy instance の `soundness` は `toyGlobalCanonicalDecomposition_vanishes_class` により、定数大域化が局所差分 class を消す計算から `gerbeClass = zero` への bounded implication を満たす。数学査読 hardening で `concreteDecompositionGerbeToyModel_fires` を強い合成発火 theorem に昇格し、overlap compatibility、effective descent、local decomposition existence、automorphism sheaf、non-abelian reading、computed class equality、computed nonzero class、no global canonical decomposition を同じ theorem statement 内で証明する。no-global 単独射影は `concreteDecompositionGerbeToyModel_no_global` として分離し、`AxiomAudit.partVIDecompositionGerbeConcrete` は強い合成発火 theorem を監査対象にする。 |
...
| PRD-6 | AC20 | five finite toy models are selected finite witness packages. R7 で transport descent の空型 package は zero/nonzero case へ分割済み。#3099 再是正で singular boundary は concrete `ob true != zeroObstruction` を維持し、operation square は nonidentity `ZMod 2` monodromy action から equality defect を読む。transport descent は `toyNonemptyFreeWord` が `toyLoopGenerator != 1` へ写る直接 theorem を持ち、non-descend 証明がその非単位性へ接続する。refactor Galois は endpoint equality component と Bool tag を持つ non-subsingleton Hom と `Preserves` の positive/negative pair を持つ。decomposition gerbe toy instance は local decomposition 存在、二点局所データの不一致から計算される gerbe class、selected class と computed class の一致 `toyGerbeClass_eq_computed_local_class`、computed class 非零 `toyGerbeClassFromLocalData_nonzero`、定数大域化 predicate `ToyGlobalCanonicalDecomposition loc := ∃ global, ∀ i, loc i = global`、および zero class から大域化を読む `ToyEffectiveDescent classOf := ∀ loc, classOf loc = false -> ToyGlobalCanonicalDecomposition loc` の positive/negative pair を持つ。negative pair は片側 chart だけを読む `toyBadGerbeClassFromLocalData` を使う。toy instance の `soundness` は global canonical data に class-zero 等式を埋め込まず、定数大域化が二点局所差分 class を消す `toyGlobalCanonicalDecomposition_vanishes_class` を経由して bounded implication を満たす。数学査読 hardening 後は `concreteDecompositionGerbeToyModel_fires` が overlap compatibility / effective descent / local decomposition existence / automorphism sheaf / non-abelian reading / computed class equality / computed class nonzero / no global canonical decomposition を同時に証明する強い合成発火 theorem になっている。 | `proved example theorem` / `selected finite witness package` | `Formal/AG/Examples/SingularityMonodromyStackPart6.lean`, `toyBoundaryObstruction_nonzero`, `toyOperationCategory_operation_nontrivial`, `toyMonodromyAction_nonidentity`, `toyMeasuredSquareNonzero_defect_from_moved_coefficient`, `toyTransportNonzero_uses_second_pi1_element`, `toyRefactorGroupoid_hom_nontrivial`, `toyGaloisData_preserves_positive`, `toyGaloisData_preserves_negative`, `toyDecompositionGroupoid_hom_kind_refactor`, `toyGlobalCanonicalDecomposition_positive`, `toyGlobalCanonicalDecomposition_negative`, `toyGerbeClass_eq_computed_local_class`, `toyGerbeClassFromLocalData_nonzero`, `toyEffectiveDescent_positive`, `toyEffectiveDescent_negative`, `concreteDecompositionGerbeToyModel_fires`, `concreteDecompositionGerbeToyModel_no_global` |
...
| `AAT.AG.AxiomAudit.partVIOperationSquareFillingPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toySquareFilling_positive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIOperationSquareFillingNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toySquareFilling_negative` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIOperationMonodromyNonidentity` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyMonodromyAction_nonidentity` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIOperationDefectFromMovedCoefficient` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyMeasuredSquareNonzero_defect_from_moved_coefficient` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `AAT.AG.AxiomAudit.partVITransportUsesSecondPi1Element` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyTransportNonzero_uses_second_pi1_element` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVITransportRelationBoundaryZeroPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyTransportDescent_relationBoundaryZero_positive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVITransportRelationBoundaryZeroNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyTransportDescent_relationBoundaryZero_negative` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `AAT.AG.AxiomAudit.partVIRefactorHomNontrivial` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyRefactorGroupoid_hom_nontrivial` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIRefactorPreservesPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGaloisData_preserves_positive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIRefactorPreservesNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGaloisData_preserves_negative` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIDecompositionGerbeConcrete` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.concreteDecompositionGerbeToyModel_fires` | `propext`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIDecompositionGerbeNoGlobal` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.concreteDecompositionGerbeToyModel_no_global` | `propext`, `Quot.sound` |
| `AAT.AG.AxiomAudit.partVIDecompositionHomKindRefactor` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyDecompositionGroupoid_hom_kind_refactor` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionHomNontrivial` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyDecompositionGroupoid_hom_nontrivial` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionOverlapPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyOverlapCompatible_positive` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionOverlapNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyOverlapCompatible_negative` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionEffectiveDescentPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyEffectiveDescent_positive` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionEffectiveDescentNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyEffectiveDescent_negative` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionGlobalCanonicalPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGlobalCanonicalDecomposition_positive` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionGlobalCanonicalNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGlobalCanonicalDecomposition_negative` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionGlobalCanonicalVanishesClass` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGlobalCanonicalDecomposition_vanishes_class` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionGerbeClassEqualsComputedLocalClass` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGerbeClass_eq_computed_local_class` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionGerbeClassComputedNonzero` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyGerbeClassFromLocalData_nonzero` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionAutSheafPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyAutSheafDefined_positive` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionAutSheafNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyAutSheafDefined_negative` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionNonAbelianPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyNonAbelianReading_positive` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionNonAbelianNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyNonAbelianReading_negative` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionSoundnessFromLocalData` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyNoCanonicalDecomposition_soundness_from_local_data` | `propext` |
| `AAT.AG.AxiomAudit.partVIDecompositionLocalExistPositive` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyLocalDecompositionsExist_positive` | none |
| `AAT.AG.AxiomAudit.partVIDecompositionLocalExistNegative` | `AAT.AG.FiniteModel.SingularityMonodromyStackPart6.toyLocalDecompositionsExist_negative` | none |
...
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedPairGroundedComparisonPacket` | `AAT.AG.SemanticRepair.lawEquation_constructs_generatedPair_groundedComparisonPacket_fromCoverRelativeComplex` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedBoundaryGroundedComparisonPacket` | `AAT.AG.SemanticRepair.lawEquation_constructs_generatedBoundary_groundedComparisonPacket_fromPrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedBoundaryLawIndependentConclusions` | `AAT.AG.SemanticRepair.lawEquation_generatedBoundary_lawIndependentConclusions_fromPrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedBoundaryLawIndependentResearchConjuncts` | `AAT.AG.SemanticRepair.lawEquation_generatedBoundary_lawIndependentResearchConjuncts_fromSource` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedBoundaryGroundedPointwiseResearchConjuncts` | `AAT.AG.SemanticRepair.lawEquation_constructs_groundedPointwiseResearchConjuncts_fromSource` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75StandardFinitePosetSourceC0DifferentialGroundedResearchConjuncts` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_groundedResearchConjuncts_fromStandardFinitePosetSource` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FinitePosetGeneratedBoundaryGroundedComparisonPacket` | `AAT.AG.SemanticRepair.lawEquation_constructs_generatedBoundary_groundedComparisonPacket_fromFinitePosetComparisonPrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75StandardFinitePosetGeneratedBoundaryGroundedComparisonPacket` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_generatedBoundary_groundedComparisonPacket_fromStandardFinitePosetPrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75CanonicalTupleGeneratedBoundaryGroundedComparisonPacket` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_generatedBoundary_groundedComparisonPacket_fromCanonicalTuplePrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75LawEquationSpineGeneratedBoundaryGroundedComparisonPacket` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_generatedBoundary_groundedComparisonPacket_fromLawEquationSpinePrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeLawIndependentConjuncts` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_finiteFreeLawIndependentConjuncts` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeGroundedComparisonPacket` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_groundedComparisonPacket_finiteFree` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeSelectedRealizationLayer` | `AAT.AG.SemanticRepair.cochainRealization_constructs_finiteFreeSelectedRealizationLayerBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeSemanticH1ZeroIffResidualBoundary` | `AAT.AG.SemanticRepair.generatedSemanticH1ZeroBody_iff_residualBoundarySurfaceBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75NoFiniteFreeSemanticH1ZeroWithoutResidualBoundary` | `AAT.AG.SemanticRepair.no_generatedSemanticH1ZeroBody_without_residualBoundary` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75BoundedResidualBoundaryIffFiniteFree` | `AAT.AG.SemanticRepair.generatedResidualBoundaryBody_iff_surfaceBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75BoundedSelectedLayerToFiniteFree` | `AAT.AG.SemanticRepair.SelectedSemanticCoefficientDirectRealizationLayerBody.toFiniteFree` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeSelectedLayerToBounded` | `AAT.AG.SemanticRepair.SelectedSemanticCoefficientFiniteFreeRealizationLayerBody.toBounded` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedSemanticCoefficientGeometry` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.FinitePosetLawEquationDefectSourceBody.toGeneratedSemanticCoefficientGeometry` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75AtomSupportedFiniteFreeRealizationSource` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.FinitePosetLawEquationDefectSourceBody.toAtomSupportedFiniteFreeRealizationSource` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75SelectedFiniteFreeLayerProjection` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.LawEquationSelectedSemanticCoefficientFiniteFreeLayerBody.toFiniteFreeLayer` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75DisplayedInterpretationRealization` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.displayedRequiredLawsHoldOn_constructs_displayedInterpretationRealization` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75CommonRestrictionRealization` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.displayedRequiredLawsHoldOn_constructs_commonRestrictionRealization` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75BaseRestrictionPreservesDisplayedInterpretation` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.displayedRequiredLawsHoldOn_constructs_baseRestrictionSourcePreservesDisplayedInterpretation` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75ArrowCompatibilityLaw` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.displayedRequiredLawsHoldOn_constructs_arrowCompatibilityLaw` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75NoCommonRestrictionWithoutArrowCompatibility` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.no_commonRestrictionRealization_without_arrowCompatibilityLaw` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75NonzeroDisplayedRestrictionRejectsRealization` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.restrictedDisplayedInterpretation_ne_zero_prevents_displayedInterpretationRealization` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75RestrictionLevelLawEvaluator` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.displayedRequiredLawsHoldOn_constructs_displayedRequiredLawRestrictionEvaluator` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75RestrictionEvaluatorRecoversArrowCompatibility` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.displayedRequiredLawsHoldOn_and_restrictionEvaluator_constructs_arrowCompatibilityLaw` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75NoRestrictionEvaluatorWithoutArrowCompatibility` | `AAT.AG.SemanticRepair.LawEquationBodyCechSource.no_displayedRequiredLawRestrictionEvaluator_without_arrowCompatibilityLaw` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedCoverSelectedRealizationLayer` | `AAT.AG.SemanticRepair.cochainRealization_constructs_selectedRealizationLayerBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75SelectedRealizationLayerCoverIsGenerated` | `AAT.AG.SemanticRepair.SelectedSemanticCoefficientDirectRealizationLayerBody.cover_is_generated` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75NoSelectedRealizationLayerWithoutGeneratedCover` | `AAT.AG.SemanticRepair.no_selectedRealizationLayer_without_generatedCover` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem81StandardDegreeZeroDifferentialEliminator` | `AAT.AG.Cohomology.StandardFinitePosetCech.standardDifferential_degreeZero_eq_zero_iff_faceRestrictions_eq` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem81StandardSourceC0CechZero` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.displayedRequiredLawsHoldOn_constructs_standardSourceC0CechZero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem81UnequalFacesRejectSourceC0CechZero` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.displayedFaceRestriction_ne_prevents_standardSourceC0CechZero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75SourcePrimitiveC0CechZero` | `AAT.AG.SemanticRepair.LawEquationGroundedComparisonConjunctsBody.sourcePrimitiveC0CechZero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75OverlapGeneratedLawEquationSpineGroundedComparisonPacket` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.lawEquation_constructs_generatedBoundary_groundedComparisonPacket_fromOverlapGeneratedSpinePrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75CompleteSupportClosure` | `AAT.AG.SemanticRepair.lawEquationCompleteRepairSupport_semanticRepairClosed` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75CompleteSupportComponentFaithfulness` | `AAT.AG.SemanticRepair.lawEquationCompleteRepairSupport_componentCoverage_and_faithfulness` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeGeneratedBoundarySurface` | `AAT.AG.SemanticRepair.coverRelativeGeneratedBoundaryAdditiveH1Surface` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FiniteFreeGeneratedCochainRealization` | `AAT.AG.SemanticRepair.coverRelativeGeneratedBoundaryCochainRealization` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedBoundarySurfaceConstructsResidualBoundary` | `AAT.AG.SemanticRepair.generatedBoundarySurface_constructs_residualBoundaryBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75ResidualBoundaryConstructsSemanticH1Zero` | `AAT.AG.SemanticRepair.residualBoundaryBody_constructs_semanticH1ZeroBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75SemanticH1ZeroConstructsAdditiveH1Zero` | `AAT.AG.SemanticRepair.semanticH1ZeroBody_constructs_additiveH1Zero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75SemanticH1ZeroConstructsResidualBoundary` | `AAT.AG.SemanticRepair.semanticH1ZeroBody_constructs_residualBoundaryBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75SemanticAtomProjectionFromLawInput` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.LawEquationSemanticAtomInputBody.toSemanticAtomProjection` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FinitePosetDefectSourceToGenericSource` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.FinitePosetLawEquationDefectSourceBody.toLawEquationDefectSource` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairTheorem75FinitePosetDefectSourceToBodyCechSource` | `AAT.AG.SemanticRepair.StandardFinitePosetGeneratedBoundary.FinitePosetLawEquationDefectSourceBody.toLawEquationBodyCechSource` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91RestrictionEvaluatorWithoutPointwiseZero` | `AAT.AG.Examples.SemanticRepairPart10.mixedBodySource_has_displayedRequiredLawRestrictionEvaluator` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91NoGeneratedInterpretationPointwiseZero` | `AAT.AG.Examples.SemanticRepairPart10.mixedDefectSource_not_generatedInterpretationPointwiseZero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91NonlawfulDisplayedInterpretationNonzero` | `AAT.AG.Examples.SemanticRepairPart10.nonlawfulDefectSource_interpret_ne_zero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91NonlawfulBodySourceRejectsDisplayedRealization` | `AAT.AG.Examples.SemanticRepairPart10.generatedLawStandardNonlawfulBodySource_not_displayedInterpretationRealization` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91MixedBodySourceRejectsArrowCompatibility` | `AAT.AG.Examples.SemanticRepairPart10.mixedBodySource_not_arrowCompatibilityLaw` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91MixedBodySourceRejectsCommonRestriction` | `AAT.AG.Examples.SemanticRepairPart10.mixedBodySource_not_commonRestrictionRealization` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91MixedBodySourceRejectsZeroBasePreservation` | `AAT.AG.Examples.SemanticRepairPart10.mixedBodySource_zeroBase_not_preservesDisplayedInterpretation` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91LegacySingletonStandardBoundedConjuncts` | `AAT.AG.Examples.SemanticRepairPart10.legacyFiring_generatedLawSingletonStandardSourceC0_boundedConjuncts` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `AAT.AG.AxiomAudit.semanticRepairExample91CircleRejectsGeneratedResidualBoundarySurface` | `AAT.AG.Examples.SemanticRepairPart10.generatedLawCircle_not_generatedResidualBoundarySurfaceBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91CircleRejectsGeneratedSemanticH1ZeroBody` | `AAT.AG.Examples.SemanticRepairPart10.generatedLawCircle_not_generatedSemanticH1ZeroBody` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91FiniteSiteAllPresheavesSheaf` | `AAT.AG.Examples.SemanticRepairPart10.finiteSite_allPresheaves_isSheaf` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerQuotientIsSheaf` | `AAT.AG.Examples.SemanticRepairPart10.integerGeneratedLawQuotientIsSheaf` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerLawWitnessIdealLe` | `AAT.AG.Examples.SemanticRepairPart10.integerLaw_lawWitnessIdeal_le` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerObstructionIdealLe` | `AAT.AG.Examples.SemanticRepairPart10.integerLaw_obstructionIdeal_le` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerOneNotMemObstructionIdeal` | `AAT.AG.Examples.SemanticRepairPart10.integerLaw_one_notMem_obstructionIdeal` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerQuotientOneNonzero` | `AAT.AG.Examples.SemanticRepairPart10.integerGeneratedLawQuotient_one_ne_zero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerQuotientNontrivial` | `AAT.AG.Examples.SemanticRepairPart10.integerGeneratedLawQuotient_nontrivial` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerViolationWitnessClassZero` | `AAT.AG.Examples.SemanticRepairPart10.integerLaw_violationWitness_class_eq_zero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerDisplayedLawsHold` | `AAT.AG.Examples.SemanticRepairPart10.integerLawFiniteFreeDisplayedRequiredLawsHoldOn` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91IntegerFiniteFreeTenConjunctPacket` | `AAT.AG.Examples.SemanticRepairPart10.lawfulFiring_integerLawSingletonStandardSourceC0_finiteFreeTenConjunctPacket` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `AAT.AG.AxiomAudit.semanticRepairExample91GeneratedLawCircleGeneratedPairSAGAPacket` | `AAT.AG.Examples.SemanticRepairPart10.lawfulFiring_generatedLawCircle_generatedPairSAGAPacket` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91GeneratedLawCircleNonzeroResidualWithGeneratedPairSAGAPacket` | `AAT.AG.Examples.SemanticRepairPart10.generatedLawCircle_nonzeroResidual_withGeneratedPairSAGAPacket` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91SharedCoefficientSeparateCechWitnesses` | `AAT.AG.Examples.SemanticRepairPart10.generatedLaw_sharedCoefficient_separateCircleNonzero_andSingletonStandardPackets` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91GeneratedLawCircleComplexZeroBoundaryPacketFromZeroPrimitive` | `AAT.AG.Examples.SemanticRepairPart10.lawfulFiring_generatedLawCircleComplex_zeroBoundaryPacket_fromZeroPrimitive` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91SingletonStandardFiniteFreeTenConjunctPacket` | `AAT.AG.Examples.SemanticRepairPart10.lawfulFiring_generatedLawSingletonStandardSourceC0_finiteFreeTenConjunctPacket` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91SingletonStandardBodySourceZero` | `AAT.AG.Examples.SemanticRepairPart10.generatedLawSingletonStandardBodySource_toPrimitive_eq_zero` | `propext`, `Classical.choice`, `Quot.sound` |
| `AAT.AG.AxiomAudit.semanticRepairExample91SingletonStandardGeneratedResidualZero` | `AAT.AG.Examples.SemanticRepairPart10.generatedLawSingletonStandardGeneratedResidual_eq_zero` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `SemanticRepairLawEquationNativeGeneratedInputs` grounding route | `Formal/AG/LawAlgebra/LawEquation.lean`, `Formal/AG/SemanticRepair/SagaComparison.lean`, `Formal/AG/SemanticRepair/Boundary.lean` | `LawEquationDefectSource`, `DisplayedRequiredLawsHoldOn`, `GeneratedInterpretationPointwiseZero`, `displayedRequiredLawsHoldOn_constructs_interpret_eq_zero`, `displayedRequiredLawsHoldOn_constructs_generatedInterpretationPointwiseZero`, `interpret_eq_zero_iff_defect_mem_obstructionIdeal`, `interpret_ne_zero_of_defect_notMem_obstructionIdeal`, `obstructionQuotientCoefficient`, `obstructionQuotientPresheaf`, `lawEquation_grounding_packet`, `SemanticRepairNativeGeneratedEndToEndInputs`, `lawEquation_constructs_groundedComparisonPacket`, `lawEquation_constructs_groundedComparisonPacket_fromNativeGeneratedInputs`, `displayedRequiredLawsHoldOn_constructs_generatedSourceC0_zeroPackage` | law equation は degree-zero contribution と native generated quotient coefficient surface を供給する。native route の cover-relative complex は `ObstructionSheaf.ofAddCommGrpValued G.obstructionQuotientCoefficient certificate.sheafCondition` 上に型で固定される。旧 arbitrary-`F` generated route と field-copy provenance route は Lean 本体・AxiomAudit・台帳の completion surface から削除した。 |
| `SemanticRepairLawEquationEndToEndInstance` | `Formal/AG/SemanticRepair/LawEquationGeneratedPair.lean`, `Formal/AG/Examples/SemanticRepairPart10.lean`, `Formal/AG/AxiomAudit.lean` | R1 common-restriction realization / negative rejection、R2 restriction-level evaluator、R4 generated-cover provenance、R3 actual displayed-source C0 Cech zero、R5 rich material-input / atom-supported finite-free ten-conclusion final theorem、R6 integer lawful firing / circle-nonzero negative-instance package | `lawEquation_constructs_groundedComparisonPacket_finiteFree` は source、law、K、generated cover、realization、comparison、residual/H1-zeroを内部生成し、1–3だけが`hholds`を使う。PRD R9(b) は `integerLawEquationCore` の `Observable = ℤ`、witness `2`、proper quotient と最終 packet で発火する。別の Fin-3 circle route は residual/H1 非零と generated-zero 述語の不成立を固定する。AxiomAudit は R1–R5 の headline、bridge、negative theorem と R6 examples を直接監査する。 |
...
| VI-1 | 実装済み | `SingularBoundary` は既存実質化を維持し、`OperationSquare` / `TransportDescent` / `RefactorGalois` / `DecompositionGerbe` finite toy instance は nontrivial Bool / `Multiplicative (ZMod 2)` instance と positive/negative theorem pair で発火済み。monodromy action 由来の defect、Pi1 第2元使用、非 subsingleton Hom、Gerbe toy instance の定数大域化 predicate、computed gerbe class 非零 theorem、zero class descent criterion、5 predicate の充足・反証 pair は AxiomAudit に配線済み。 | #3099 |
