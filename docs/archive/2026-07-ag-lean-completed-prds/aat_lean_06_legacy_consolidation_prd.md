...
旧版が要求した全public declarationの完全型展開は、負債を排除する本PRD自身が
9MB超の生成Lean sourceという新しい保守負債を作る設計だったため撤回する。
signature preservationはtracking Issueで固定したcore statement、変更sourceの実読、
focused elaboration、AxiomAudit、clean CI buildで検証し、全宣言の型を展開した生成物を
Git管理しない。
- 対象module:
...
1. tracking Issue #3588本文の`Statement contract index`が`Statement contract policy`、
   ...
   - `consumer_closure`の各宣言について、exact name、処置、proof owner、owner module、batch、
     focused check。signatureを変更する行だけ旧 / 新signatureまたは`SD3:<exact-name>`参照を持つ。
     `delete-repackage`行の
...
| `Formal.AG.StatementContracts` | `keep` | 既存の小さいstatement contract aggregateを維持 |
| `Formal.AG.AxiomAudit` | `append [Formal.AG.Examples.LegacyConsolidation]` | `B`へ既存`Audit_reuse`または新規`A_seed`を一対一対応させ、namespace全数audit |
...
`A_seed`、`Ω`をこの順で固定する。
`A_seed`は`AAT.AG.AxiomAudit`内のpublic aliasだけとする。`K_seed ⊆ K`、`H ⊆ M₀`、
...
seed外のitemも、最終処置を実読して`K` rowを明示するまで未分類のまま残す。
`P = T ∩ K`は変更file内でsignatureを変えないpublic declaration / projectionを表すが、
完全型を展開したartifactは作らない。pinned mainから全child PR merge後のintegrated snapshotまでの
...
`M_audit`の旧・shape変更APIをtypeまたはvalueから直接参照する既存entryは
...
SD6のsingle synthesis packageとderived projectionを`N_seed`とする。
...
全public declarationの完全型をLean sourceへ展開したmigration専用contractは作成しない。
core statementはIssue contractと最終sourceを直接照合し、consumer preservationは変更sourceの実読、
focused elaboration、AxiomAudit、clean CI buildで判定する。名前の存在、wrapper、alias、
より弱い同値statementは達成に数えない。削除対象`L`はSD10のno-hit検査で判定する。

## PRD-fixed audit and no-hit contract

### SD10 — audit、no-hit contract

- core statementはIssueの固定signatureと最終Lean declarationを直接照合する。
- automatic projectionを含む変更structure fieldは、最終sourceと利用箇所を実読する。
- `AnalyticRepresentation`がdefinitionally Mathlib `Functor`であることを実装とfocused elaborationで確認する。
- AxiomAuditは対象source theoremを直接参照し、standard axioms onlyを検査する。
- 全宣言の完全型を展開するmigration専用Lean source、deprecated contract、削除済み宣言の再定義を残さない。
...
- 対象main commitを固定し、`M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、`Q_delete`、
  ...
  `disposition_rows`、`L`、`K`、`N`、`C`、`E`、`A_audit`、`P`を
  exact nameで生成する。
...
- `consumer_closure`の各宣言についてexact name、処置分類、proof owner、owner module、batchを固定する。
  signature変更行だけ旧 / 新signatureまたはSD3参照を持ち、全`delete-repackage`行の`L`収載・独立内容なしと、最終`C`の処置が
...
- AxiomAudit、tracking IssueのAC evidence mapをpackage、projection、reference firingの
  現行宣言へ同期する。
...
- `derive`行はcanonical APIのproof-useをsourceとreviewで確認する。
...
### R9 — audit、aggregate、tracking state

- migration専用の巨大statement contractを作成せず、既存`StatementContracts.lean`を肥大化させない。
- `AxiomAudit.lean`、aggregate importをcanonical moduleへ同期する。
...
- `editable_live_nonmath_reference_scope`のaccepted legacy hitをcanonical owner、現行module path、現行statusへ同期する。
  ...
  Lean statusまたは実装説明は、最終Lean declarationが直接支持する最小記述へ改める。
  新しい数学claim、無関係なcopy、layout、navigation、機能は変更しない。各変更blockについて旧記述、
  新記述、根拠declaration、candidate採否、review URLをpacketへ残す。
...
- audit sourceが`B`、audit aliasが`Audit_reuse ∪ A_audit`、
...
- [ ] AC2: consumer inventoryが`consumer_closure`の件数とowner moduleを覆い、変更対象はexact name、
  処置、proof owner、batchを持つ。signature変更行は旧 / 新signatureまたはSD3参照を持つ。
  全`delete-repackage`行は`L_seed ∪ Q_delete ∪ Audit_drop = L`に入り、最終`C`の処置は
  `preserve | derive`だけである。全preserve宣言の完全型を展開したartifactは要求しない。
- [ ] AC3: tracking Issue #3588本文の`Statement contract index`が列挙するcomment群だけがSD2〜SD6のcore statement、
  ...
  `Approved`は新規の未編集commentで、索引順commentとconsumer inventory snapshotの種別、URL、database ID、
  `updated_at`、正規化本文SHA-256を持ち、approval commentの`created_at = updated_at`が成立する。
- [ ] AC4: migration専用の完全型展開Lean sourceが存在しない。SD2〜SD6、SD3列挙consumer、SD9 firingは
  Issueの固定signatureと最終sourceの直接照合、focused elaboration、clean CI buildで一致を確認する。
...
- [ ] AC22: `AATSch.forget`がfaithfulで、object / map projectionがIssueの固定statementと最終sourceで一致する。
...
- [ ] AC42: migration専用statement contract、old statement contract、`Audit_drop`全exact nameのpre-archive scope hitがzeroである。
...
- [ ] AC54: `live_nonmath_reference_scope`の全short identifierを装飾の有無に依存せずtoken単位で
  ...
  各変更blockに旧記述、新記述、根拠declarationがある。`website-review`が合格し、
...
- audit / tracking Issueだけを新名へ変更し、proof consumerを旧routeに残す。
- 全public declarationの完全型を展開したmigration専用sourceまたは同等の巨大生成artifactをGit管理する。
...
- nonpreserved / support public names、audit source `B`、
...
4. **Statement preservation**
   ...
   - approval commentの`created_at = updated_at`と現行index URL列の一致
   - revision chainの直前approval comment database ID。初回は`none`
   - consumer inventoryの件数、owner module、変更対象の処置
   - `P`の変更source実読とclean CI build結果
   - `P_IV`のexact name展開と三accessor以外のsignature diff zero
   - signature変更対象の旧 / 新signatureまたはSD3参照とstatement diff zero
...
9. **Deletion evidence**
   ...
   - website candidateのchild Issue / PR、各変更blockの旧記述 / 新記述 / 根拠declaration、
...
10. **Kernel evidence**
   ...
   - fixed core statementと最終sourceの直接照合結果
   - `Audit_reuse`のsource injectivity、`B_new = B \ B_reused`、
     `auditSource : (Audit_reuse ∪ A_seed) ≃ B`、各aliasの完全signature / `@source`右辺、
