...
ArchSig CLI は [Issue #51](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/51)
で `tools/archsig/` に置く。これは Lean module import graph から
...
Flatness–Obstruction Conjecture の数学面の草案は
`docs/math/Flatness–Obstruction Conjecture.md` に残し、Lean 化の設計は
[アーキテクチャ零曲率定理 Lean 化設計](formal/flatness_obstruction_lean_design.md)
に分離する。Lean 側では、最初から数値的な curvature を導入せず、
generic witness-count kernel を最小核にし、required diagram の非可換性を
その重要な特殊例として扱う。初期 proof obligation は、有限 witness list 上の
zero-count bridge、witness 不在と全 required diagram の可換性の同値、
および complete coverage 下での finite measured law universe から global
lawfulness への bridge である。

この中心定理候補を本文では **アーキテクチャ零曲率定理** と呼ぶ。
用語は次の日本語を優先する。
...
Issue [#237](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/237)
で Lean 証明済みである。この obstruction は、まず
...
runtime reachable cone が空」と読む。semantic `Reachable` ベースの
`NoSemanticRuntimeExposureObstruction` へ上げる bridge は Issue
[#238](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/238)
で Lean 証明済みである。`runtimeBlastRadius` は reverse runtime graph を明示的に与える
...
では、状態遷移履歴と effect boundary を `Expr` / `Semantics` による diagram
law family として表す最小 API を追加した。Lean status は、式型と finite case /
diagram constructors は `defined only`、各 replay / roundtrip / compensation family の
...
- `cycle -> rho(A) > 0`: `proved` for finite `ComponentUniverse`
  complex adjacency matrices,
...
Issue [#95](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/95)
では、有限 `ComponentUniverse` 上の非空閉 walk から
`0 < spectralRadiusOfAdjacency U` を得る bridge theorem を証明した。
証明は、閉 walk の反復が任意の nilpotence cutoff を超えて正の対角
adjacency-power entry を作ること、finite complex matrix では
`spectralRadius = 0` が nilpotence を強制することを接続する。
この theorem も構造的な finite adjacency matrix statement であり、
変更コストや障害伝播増幅との相関は empirical hypothesis のまま残す。
...
Issue [#80](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/80)
...
Issue [#80](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/80)
では、`ObservationFactorsThrough` と projection bridge の関係を
...
では、外部 repository `leanprover-community/lean4-samples` の PR #10 に
`lean4-samples-single-boundary-v0` policy を適用し、before / after の
`boundaryViolationCount` と `abstractionViolationCount` が測定済み 0 として
dataset に入ることを確認した。一方、runtime edge evidence はこの sample repository
から安定して抽出できないため入力せず、`runtimePropagation = null` と
`metricDeltaStatus.runtimePropagation.comparable = false` を維持した。これは
外部 pilot validation であり、Lean theorem ではない。
...
Issue [#207](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/207)
では、上記 selected required axes を concrete `LawFamily` として統合した。
Issue [#212](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/212)
では、その公開 entry point を `ArchitectureWitness`, `architectureBad`,
`architectureMeasuredWitnesses`, `architectureRequiredWitness`,
`architectureRequiredAxis`, `witnessForAxis` として明示し、final theorem
`architectureLawful_iff_requiredSignatureAxesZero` を追加した。Lean では
`ArchitectureLawModel.signatureOf` が concrete count で埋めた
`ArchitectureSignatureV1` を作り、`architectureLawFamily_completeCoverage`,
`architecture_requiredAxisFamilyExact`, `architecture_requiredAxisExact` により
complete witness coverage と required axis exactness を構成する。最終 theorem
`architectureLawful_iff_requiredSignatureAxesAvailableAndZero` /
`architectureLawful_iff_requiredSignatureAxesZero` は、`ArchitectureLawful X` と
`RequiredSignatureAxesAvailableAndZero (ArchitectureLawModel.signatureOf X)` /
...
Issue [#210](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/210)
では、hasCycle の concrete Signature axis exactness を公開 theorem として追加した。
`boolRisk_eq_zero_iff` は 0/1 risk indicator の zero 性を `Bool.false` へ戻す
小補題である。`hasCycle_axisExact` は `v1OfFiniteWithRequiredLawAxes` の
`.hasCycle` が available-and-zero であることと `WalkAcyclic` を、finite
`ComponentUniverse` の coverage 下で同値にする。
`hasCycle_axisExact_no_closedWalkObstruction` は同じ axis を closed-walk
...
`runtime-edge-projection-v0` を `archsig --runtime-edges` として実装した。
...
ことまでであった。Issue
[#237](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/237)
では、0/1 graph 上の runtime exposure zero と runtime obstruction absence の bridge を
`runtimePropagationOfFinite_eq_zero_iff_noRuntimeExposureObstruction` として証明した。
この obstruction は executable metric と同じ bounded search に合わせ、
`reachesWithin G components components.length` ベースの measured obstruction として
定義される。Issue
[#238](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/238)
では、semantic `Reachable` 版の
`NoSemanticRuntimeExposureObstruction` を追加し、
`ComponentUniverse` coverage / edge-closure 下で measured / bounded obstruction と
semantic obstruction が同値であること、および
`runtimePropagationOfFinite = 0` と semantic obstruction absence が同値であることを
証明した。blast radius、coverage policy を数学的に扱う場合も、reverse graph や
...
- ArchSig output と PR metadata から empirical dataset v0 record を生成する
  CLI は [#117](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/117)
  で `archsig dataset --before ... --after ... --pr-metadata ...` として
...
- GitHub PR detail / files / reviews JSON から dataset input 用 PR metadata を生成する
  ...
  で `archsig pr-metadata --pull-request ... --files ...` として実装済みである。
...
- RelationComplexityObservation extractor rule set v0 は
  ...
  で `archsig relation-complexity --input ...` として実装済みである。
...
- ArchSig が boundary / abstraction policy file を読み、
...
- ArchSig output から上記 validation report を生成する CLI は
  [#116](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/116)
  で `archsig validate --input ...` として実装済みである。report は入力 JSON の
...
- `archsig` の外部利用向け hardening は
  ...
  `cargo test --manifest-path tools/archsig/Cargo.toml` として追加する。
...
では、最初の pilot study 用に仮説を primary analysis と exploratory analysis に分け、
必要データ、測定方法、除外条件、extractor tooling と data analysis の前提分離を
[empirical_protocol.md](design/empirical_protocol.md) に固定する。Issue
[#164](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/164) では、
H1 を pre-change risk の H1a と future outcome の H1b に分け、first pilot の
primary analysis を H1a/H3 に絞る。H1b/H2/H4/H5 は必要データが揃う場合だけ
exploratory analysis として扱う。
...
```text
PR 前の Signature risk は、変更ファイル数や変更モジュール数の増加と相関する。
```

測定候補:

- pull request ごとの変更ファイル数
- pull request ごとの changed lines
- 変更されたコンポーネント数
- 変更前の reachable cone, depth, fanout, SCC risk
...
```text
PR 前後の Signature delta は、将来 co-change や repair scope と相関する。
```
...
- 将来 repair PR または bug fix issue の変更範囲
...
記録し、H5 を exploratory / not evaluated として報告する。Lean status は
`empirical hypothesis` / protocol design である。
