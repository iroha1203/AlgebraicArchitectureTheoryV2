...
## PRD v3 IntentMap / AlignmentMap boundary

PRD / Epic / Spec 由来の planning forecast では、LLM と ArchSig の責務を分ける。

```text
LLM-authored IntentMap / AlignmentMap
  -> ArchSig schema validation
  -> deterministic operation-support / ForecastCone / ConsequenceEnvelope projection
  -> reviewer-facing planning boundary
```

LLM は semantic extraction、source refs、missing decision、ambiguous intent、alignment candidate を
書ける。LLM は forecast result、causal conclusion、Lean proof、quality ranking、merge approval を
直接書かない。ArchSig validation は dangling refs、boundary field、non-conclusions を検査するが、
generated map の semantic correctness、implementation plan completeness、forecast correctness を
保証しない。

`intentUnaligned`、unsupported intent、ambiguous intent、missing decision、missing evidence は
measured zero ではない。これらは `operation-support-estimate-v0`、`forecast-cone-skeleton-v0`、
`consequence-envelope-report-v0`、`intent-calibration-record-v0` へ planning boundary として残す。
