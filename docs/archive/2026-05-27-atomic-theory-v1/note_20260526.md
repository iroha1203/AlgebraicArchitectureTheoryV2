...
Status: archived on 2026-05-27. Superseded by
[AAT/SFT Atomic Theory v2](../../note/aat_sft_atomic_theory_v2.md)
for the definition of `ArchitectureAtom`.

このメモの boundary-relative `ArchitectureAtom_B` / obstruction atom 読みは、
現行 v2 では primitive atom ではなく `ObstructionCircuit`、`ObservedMolecule`、
または `ObservationGap` として読む。原子そのものは universal primitive
architectural fact であり、boundary は観測状態を表す。
