...
- **R6(law-policy 側の解決規則)** `law-policy/v0.5.1`: `lawSurfaceRef` を必須化し、
  supplied surface の `id` との一致を検査。`policies[].law` と
  `policies[].lawPair[]` は surface の `lawId` への解決必須 (未解決は validation fail)。
  `ag.law-conflict-tor` の `lawPair` は exactly two distinct law ids とし、law-id naming
  convention から参加 law を推測しない。`policies[].evaluator` と law の
  `conditionType` / binding.axis の整合(square-free axis の closed law に tor 系 evaluator
  を張る等の不整合)は registry manifest 所有の対応表で検査する。severity / scope / basis
  の運用判断面は不変 (LawPolicy は selector のまま。憲章 C5)。
...
- **Changed**: 全 schema 文字列 v0.5.1(R8)、law-policy(lawSurfaceRef 必須化 +
  policies[].law / policies[].lawPair の surface 解決)、cech / square-free / tor / section-factorization 系
