...
instance selectedCechComplexFunctor_preservesZeroMorphisms
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base) :
    (selectedCechComplexFunctor 𝒰).PreservesZeroMorphisms
...
@[simp] theorem selectedCechResolutionBicomplexMap_refl
    ...
    selectedCechResolutionBicomplexMap
      ...
      𝟙 (selectedCechResolutionBicomplex 𝒰 Ob)
...
@[simp] theorem selectedCechResolutionBicomplexMap_comp
    ...
    selectedCechResolutionBicomplexMap (r.comp s) Ob =
      selectedCechResolutionBicomplexMap r Ob ≫
        selectedCechResolutionBicomplexMap s Ob
...
theorem selectedCechResolutionAugmentation_refinement_naturality
    ...
    selectedCechResolutionAugmentation 𝒰 Ob ≫
      ...
      r.selectedCechMap.app Ob.toAddCommGrpSheaf.val ≫
        selectedCechResolutionAugmentation 𝒱 Ob
...
theorem zeroObstructionSheaf_toAddCommGrpSheaf_isZero
    {U : AtomCarrier.{u}} {A : ArchitectureObject U}
    {S : Site.AATSite A} :
    Limits.IsZero (zeroObstructionSheaf S).toAddCommGrpSheaf
...
theorem zeroObstructionSheaf_isLerayFor
    ...
    [HasExt.{u + 2} (Sheaf S.topology AddCommGrpCat.{u + 1})]
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base) :
    IsLerayFor 𝒰 (zeroObstructionSheaf S)
...
theorem IsLerayFor.overlapBaseResolution_exactAt
    ...
    (σ : (canonicalCoverRelative 𝒰).simplex p) :
    (baseResolutionComplex
      (base := (canonicalCoverRelative 𝒰).overlap p σ) Ob).ExactAt q
...
theorem IsLerayFor.selectedCechResolutionColumn_exactAt
    ...
    (hLeray : IsLerayFor 𝒰 Ob)
    (q : ℕ) (hq : 0 < q) (p : ℕ) :
    (selectedCechResolutionColumn 𝒰 Ob p).ExactAt q
...
theorem IsLerayFor.selectedCechResolutionColumn_homology_subsingleton
    ...
    (q : ℕ) (hq : 0 < q) (p : ℕ) :
    Subsingleton
      ((selectedCechResolutionColumn 𝒰 Ob p).homology q : Type (u + 1))
...
theorem selectedCechResolutionAugmentation_exactAtZero
    ...
    [HasSheafify S.topology AddCommGrpCat.{u + 1}]
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    (Ob : ObstructionSheaf S) (p : Nat) :
    Function.Exact
      ((selectedCechResolutionAugmentation 𝒰 Ob).f p).hom
      (((selectedCechResolutionBicomplex 𝒰 Ob).d 0 1).f p).hom
...
theorem injectiveSheaf_selectedCech_exactAt
    ...
    [Injective I]
    (p : Nat) (hp : 0 < p) :
    ((selectedCechComplexFunctor 𝒰).obj I.val).ExactAt p
...
noncomputable def selectedCechToResolutionTotalHomologyEquiv
    ...
    (𝒰 : Site.AATCoverageFamily S.requirements S.overlap base)
    (Ob : ObstructionSheaf S)
    (hLeray : IsLerayFor 𝒰 Ob) (n : Nat) :
    (((selectedCechComplexFunctor 𝒰).obj
      Ob.toAddCommGrpSheaf.val).homology n : Type (u + 1)) ≃+
      ((selectedCechResolutionTotalComplex 𝒰 Ob).homology n : Type (u + 1))
...
`IsLerayFor`から`IsLerayFor.selectedCechResolutionColumn_homology_subsingleton`までを
R5c7のfixed signatureとし、以後のaugmented exactness、injective-row acyclicity、
両edge `QuasiIso`、homology equivalenceをR5c8〜R5c9のfixed signatureとする。
`IsLerayFor`はselected overlap上の
...
`selectedCechResolutionAugmentation_exactAtZero`はresolution unitを各overlapで評価し、
degree zeroのkernelをproductへ組み立てて証明する。
...
両edgeの`QuasiIso`は、Mathlib totalの有限対角coproductについてcomponent projection、
inclusion-component、total differentialの二成分公式を内部構成し、degreeごとの有限帰納法で証明する。
selected Čech edgeは`hLeray`、augmentationを含むcolumn exactnessをproof-useし、base-resolution edgeは
actual sheaf gluingとinjective-row exactnessをproof-useする。`QuasiIso`、homology iso、spectral-sequence
collapse、total cycleのpreimageをcallerから受け取ってはならない。
二つのhomology equivalenceはそれぞれactual edge mapの`isoOfQuasiIsoAt`だけから構成し、
`cechToSheafHAtBaseEquiv`はこれらを直接合成する。後続の`cechToSheafHAtBase`は
このcanonical equivalenceのadditive homを使う。
...
`selectedCechResolutionBicomplex`は`InjectiveResolution.cocomplex`の各次数へ
`sheafToPresheaf ⋙ selectedCechComplexFunctor`を適用したactual first-quadrant bicomplexとする。
outer indexはresolution degree、inner indexはselected Čech degreeであり、二方向の微分と可換性は
`mapHomologicalComplex`から構成する。`selectedCechResolutionAugmentation`はresolution unit、
`baseResolutionToSelectedCechZero`はbase sectionのselected chartへのrestrictionから構成する。
refinement mapとidentity、composition、両edgeのnaturalityまでを固定し、arbitrary bicomplex、
edge map、commutation witness、collapse conclusionをcallerから受け取らない。
...
| `CoverageTopologyRefinement.refineCover` | 本文由来 | `J'`が`J`の各coverをactualに細分するdata | R3の`finiteSite`とは独立な`topologySite`で具体sieve inclusionを構成 |
| `CommonCoefficientSheaf` | 本文由来 | chosen coefficientがcoarse / fine両topologyでsheafになるcompatibility | `topologySite`上の同一presheafと二つのactual sheaf proofから構成し、fine側auxiliary coverでのdescentも直接放電 |
...
| `HasSheafify` / `HasExt` | 本文由来のambient premise | actual `Sheaf.H`とcanonical additive sheafification | `HasSheafify S.topology AddCommGrpCat.{u + 1}`をgeneric APIで明示し、AAT sheafでは`aatSheafHasExt`が`HasExt.standard`から結果universe `u + 2`を与えるnamed instance chainを記録する。`topologySite`のcoarse / fine両topologyでは、それぞれのnamed `HasSheafify` / `HasExt.standard` instanceをfiring blockに置く。module presheafのR-actionはadditive sheafificationへtransportし、`HasSheafify ... (ModuleCat R)`を要求しない |
...
| finite linear Čech model | firing限定 | strict-diamond selected cover上のdegree-one nonzero計算とcanonical coefficient compatibilityの放電 | refinement / small additive sheaf例の`finiteSite` / `coarseCover`とは別の`finiteLinearSite` / terminal base / `finiteLinearCover`としてprimitive dataから構成し、abstract complex theoremの明示引数やtypeclassへ追加しない |
| finite topology-change model | firing限定 | strict topology refinement、selected cover refinement、degree-one actual `Sheaf.H` mapの同時放電 | `finiteSite` / `finiteLinearSite`のaliasではない`topologySite`として、terminal strict-diamondとauxiliary branchをprimitive dataから構成する。auxiliary base / patchはleft / right / selected overlapと相互に比較不能で、いずれも`topologyBase`へ写る。auxiliary baseとleft / rightのactual overlapがauxiliary patchを経由しないことを固定し、coarse diamond coverのpullbackからauxiliary sieveがcoveringにならないことを証明する。coarse / fineの差はauxiliary側で作り、diamond側の同一Čech classを消さない |
...
| topology / cover | `coarseFineTopologyRefinement`、`coarseFineTopologyRefinement_selects_fineCover`、`topologyCoarseToFineCover`、`topologyCoarseToFineCover_not_bijective`、`topologyCoarseToFineCechHom_nonzero` | `topologyBrokenFaceMap_not_refinement` | 全宣言が同じ`topologySite` / terminal base / strict-diamond coverを参照する。actual refined sieveがselected `topologyFineCover`の生成sieveと一致し、fine-to-coarse index mapが全単射でなく、degree-one cochain mapが非零。R3の`finiteSite` duplicated coverは本laneの完了根拠に数えない |
| selected sheaf H | `finiteLerayCover`、`finite_cechToSheafH_bijective` | `nonLerayCover_not_completionEvidence` | actual `Sheaf.H`を参照し、cover-relative aliasを使わない |
| topology sheaf H | `topologyCoefficient_presheaf`、`topologyCechOneClass_ne_zero`、`topologySourceHOneClass_eq_cech`、`topologyCechToFineHOneEquiv`、`topologyCechToFineHOneEquiv_eq_sheafHMap_comp_cech`、`topologySheafHMap_on_sourceClass`、`topologyTargetHOneClass_ne_zero`、`coarseFineSheafHMap_nonzero` | `fineTopology_ne_top` | 同じ`topologySite`、coefficient、degree-one Čech classからsource / target actual `Sheaf.H` classを独立に構成し、topology-change mapがsource classを非零target classへ送る。fine側equivalenceはactual fine-side injective resolutionとauxiliary descentから構成し、topology-change mapとの一致を別定理で証明する。`fineTopology = ⊤`、zero coefficient、unrelated Ext class、caller-supplied `Nontrivial` / map / class premiseは禁止 |
...
| linear Čech | `finiteClass_baseChange_nonzero` | `zeroClass_not_firing` | `finiteLinearSite`のstrict-diamond cover上のdegree-one nonzero classとnonidentity scalar extension |
...
| actual coefficient H | `finiteCechCoefficientCompatible`、`finiteLinearLerayCover`、`finiteTargetLinearLerayCover`、`finiteSheafHBaseChangeMap`、`finiteSheafHBaseChangeIso`、`finiteSheafHClass_baseChange_nonzero` | `infiniteProductCech_not_compatible`、`intZModTwo_not_flat`、`zeroClass_not_firing` | `finiteLinearSite`上のcanonical source / base-changed target coefficientとsource / target `IsLinearLerayFor` proofを使い、actual terminal `Sheaf.H` degree-one classのcanonical unit像が非零。infinite duplicated coverではflat scalar extensionが無限積と可換でないことを検出 |
...
noncomputable def finiteLinearBaseIsTerminal : IsTerminal finiteLinearBase
...
theorem finiteLinearStrictDiamond :
    finiteLinearSite.contextPreorder.le
      ...
      ¬ finiteLinearSite.contextPreorder.le
        finiteLinearRightObject.ctx finiteLinearOverlapObject.ctx

noncomputable def finiteLinearCover :
    Site.AATCoverageFamily finiteLinearSite.requirements
      finiteLinearSite.overlap finiteLinearBase

def finiteLinearCoverIndexEquiv : finiteLinearCover.Index ≃ Bool

theorem finiteLinearCover_twoBranches :
    ∃ i j : finiteLinearCover.Index,
      i ≠ j ∧
        finiteLinearCover.patch i = finiteLinearLeftObject.ctx ∧
        finiteLinearCover.patch j = finiteLinearRightObject.ctx

noncomputable def finiteLinearCoefficientSheaf :
    Cohomology.LinearCoefficientSheaf Int finiteLinearSite
...
noncomputable def topologyBaseIsTerminal : IsTerminal topologyBase
...
theorem topologyStrictDiamond :
    topologySite.contextPreorder.le
      ...
      ¬ topologySite.contextPreorder.le
        topologyRightObject.ctx topologyOverlapObject.ctx

def topologyAuxBase : topologySite.category

def topologyAuxPatch : topologySite.category

noncomputable def topologyAuxInclusion :
    topologyAuxPatch ⟶ topologyAuxBase
...
theorem topologyAuxOffDiamond :
    (¬ topologySite.contextPreorder.le
      ...
      (¬ topologySite.contextPreorder.le
        topologyAuxPatch.ctx topologyOverlapObject.ctx)
...
noncomputable def topologyAuxPrecoverage :
    Precoverage topologySite.category
...
def topologyCoarseCoverIndexEquiv : topologyCoarseCover.Index ≃ Bool

def topologyFineCoverIndexEquiv :
    topologyFineCover.Index ≃ Bool × Bool
...
theorem topologyCoarseCover_presieve_eq_fineCover :
    topologyCoarseCover.presieve = topologyFineCover.presieve
...
theorem topologyCoarseToFineCover_not_bijective :
    ¬ Function.Bijective topologyCoarseToFineCover.indexMap
...
theorem topologyBaseCoefficient_subsingleton :
    Subsingleton
      ((topologyObstructionSheaf.toAddCommGrpSheaf.val.obj
        (Opposite.op topologyBase) : Type 1))

theorem topologyLeftCoefficient_subsingleton :
    Subsingleton
      ((topologyObstructionSheaf.toAddCommGrpSheaf.val.obj
        (Opposite.op topologyLeftObject) : Type 1))

theorem topologyRightCoefficient_subsingleton :
    Subsingleton
      ((topologyObstructionSheaf.toAddCommGrpSheaf.val.obj
        (Opposite.op topologyRightObject) : Type 1))
...
theorem topologyAuxBaseCoefficient_subsingleton :
    Subsingleton
      ((topologyObstructionSheaf.toAddCommGrpSheaf.val.obj
        (Opposite.op topologyAuxBase) : Type 1))

theorem topologyAuxPatchCoefficient_subsingleton :
    Subsingleton
      ((topologyObstructionSheaf.toAddCommGrpSheaf.val.obj
        (Opposite.op topologyAuxPatch) : Type 1))
...
theorem topologyCoarsePullbackAtAuxBase_not_le_auxSieve :
    ¬ topologyCoarsePullbackAtAuxBase ≤ topologyAuxSieve
...
theorem topologyAuxSieve_mem_fineTopology :
    topologyAuxSieve ∈ fineTopology topologyAuxBase

theorem topologyAuxSieve_not_mem_coarseTopology :
    topologyAuxSieve ∉ coarseTopology topologyAuxBase
...
theorem topologyFineCover_mem_fineTopology :
    Sieve.generate topologyFineCover.presieve ∈ fineTopology topologyBase

theorem topologyCoarseLerayCover :
    Cohomology.IsLerayFor topologyCoarseCover topologyObstructionSheaf
...
theorem topologyCechOneClass_ne_zero : topologyCechOneClass ≠ 0
...
/-
`topologySite`は`finiteSite`、`FiniteModel.twoPatchSite`、`finiteLinearSite`の
aliasにしない。terminal strict-diamondと`topologyAuxBase` / `topologyAuxPatch`を
primitive dataから構成する。auxiliary base / patchはleft / right / selected overlapと
相互に比較不能で、いずれも`topologyBase`へ写る。
`topologyLeftAuxOverlap` / `topologyRightAuxOverlap`はいずれも`topologyAuxPatch`以下では
ないことを固定し、coarse diamond coverの`topologyAuxBase`へのactual pullbackが
`topologyAuxSieve`以下にならないことを直接証明する。
`fineTopology`はcoarse側の生成coverに`topologyAuxSieve`を加えて生成し、
`topologyAuxSieve_mem_fineTopology`と
`topologyAuxSieve_not_mem_coarseTopology`をstrictnessの根拠にする。
duplicated cover labelだけをtopology strictnessの根拠にしてはならない。

`topologyCoefficient`はdiamond上で非零degree-one classを持ち、auxiliary側では
zeroとなる同一presheafのcoarse / fine両sheaf proofから構成する。
`topologySourceHOneClass`は`topologyCechOneClass`をactual terminal
`Sheaf.H` comparisonへ送ったclassである。`topologyCechToFineHOneEquiv`はactual
fine-side injective resolutionと、同じČech classを表すexplicit non-split extensionの
auxiliary descentから構成し、`topologyTargetHOneClass`をそのequivalenceで独立に定義する。
`topologyCechToFineHOneEquiv_eq_sheafHMap_comp_cech`は、このfine-side constructionと
`sheafHMap_eq_ext`のactual Ext合成との一致を証明するsubstantive theoremであり、rflで閉じない。
class、class equality、nonzero、comparison map / isoをstructure field、certificate field、
theorem引数、typeclassで受け取ってはならない。`fineTopology = ⊤`、zero coefficient、
unrelated Ext classをpositive firingに使用しない。`topologyTargetHOneClass_ne_zero`は
`topologyCechToFineHOneEquiv`のinjectivityと`topologyCechOneClass_ne_zero`から証明し、
`topologySheafHMap_on_sourceClass`は上記一致定理をproof-useする。
-/
...
noncomputable def finiteActualSourceClass :
    finiteLinearCoefficientSheaf.terminalLerayHModule
      finiteLinearCover finiteLinearBaseIsTerminal finiteLinearLerayCover finiteDegree :=
...
`nonLerayHPrimeOne_nontrivial`を`not_isLerayFor_of_nontrivialHPrime`へ適用して
`nonLerayCover_not_completionEvidence`を前提なしで閉じる。arbitrary Ext class、
`Nontrivial H'` premise、caller-supplied comparisonまたは既存custom cover-relative H¹だけで
negative firingを閉じてはならない。

`finiteLinearSite`はrefinement / small additive sheaf例の`finiteSite`または
`FiniteModel.twoPatchSite`のaliasにしてはならない。既存two-patch contextは選択点の
任意の二点間にactual restriction射を持つため、`coarseCover`をdegree-one nonzero
linear Čech classの根拠にしない。linear / actual-H firingでは、独立な
`finiteLinearSite`内のactual pair overlap、left、right、terminal baseが
`finiteLinearStrictDiamond`を満たし、actual terminalityを
`finiteLinearBaseIsTerminal`、exactly two branchesを
`finiteLinearCoverIndexEquiv`と`finiteLinearCover_twoBranches`で直接検査する。
`finiteLinearCoefficientSheaf`、そのcanonical base change、source / target Leray proof、
degree-one cocycle / class、actual `Sheaf.H` map / isoはすべてこの同一modelを参照する。
terminality、strict diamond、class非零をcaller-supplied premiseまたはcertificate fieldから
読んではならない。
...
- resolution augmentationを含むcolumn degree-zero exactness、injective sheafのrestriction surjectivity、
  actual sheaf gluingとsheafified augmented Čech chain resolutionによるinjective-row全次数exactnessを証明する。
- Mathlib totalの有限対角component formulaと両edgeのactual `QuasiIso`を構成し、
  `isoOfQuasiIsoAt`から二つのcanonical homology equivalenceを生成する。
...
- `finiteLinearSite`のstrict-diamond、terminal base、degree-one actual-H classに必要な
  補助定理を先に実装し、その後に別definitionの`topologySite`をprimitive dataから
  構成してtopology / coverとtopology-change H¹ firingを実装する。依存順は
  R9e → R9h → R9c → R9d → final integrationとし、R9hのfinite linear helperを
  R9cが再利用した後にR9dを接続する。`topologySite`を`finiteLinearSite`のaliasにしない。
- `topologySite`内でterminal strict-diamondとauxiliary coverを構成する。auxiliary
  base / patchはleft / right / selected overlapと相互に比較不能で、いずれも
  `topologyBase`へ写る。auxiliary baseとleft / rightのactual overlapが
  auxiliary patchを経由しないことを証明し、
  同一model上でactual strict topology、selected fine sieve、non-bijective cover refinement、
  degree-one nonzero cochain、同じČech class由来のsource / target actual `Sheaf.H` classを
  順に発火させる。R3の`finiteSite` / duplicated coverは回帰対象として保持するが、
  このtopology completion evidenceには数えない。
...
- `finiteLinearSite`内のleft / right / overlapとactual terminal base、Leray二枝cover、
  degree-one nonzero linear Čech classを構成し、同じcanonical source / base-changed target
  coefficientからactual `Sheaf.H` class mapのnonzero firingまで接続する。
...
- [ ] AC20: 独立な`topologySite`でactual overlap / left / right / terminal baseがstrict diamondを成す。同一site上の`topologyCoarseCover` / `topologyFineCover`について、topology refinementのactual refined sieveが`topologyFineCover`の生成sieveと一致し、`topologyCoarseToFineCover.indexMap`が非全単射、degree-one canonical cochain mapが非零である。`topologySite.topology = topologyCoarsePrecoverage.toGrothendieck`と`fineTopology = coarseTopology ⊔ topologyAuxPrecoverage.toGrothendieck`をfixed theoremで検査する。auxiliary base / patchはleft / right / selected overlapと相互に比較不能で、いずれも`topologyBase`へ写る。`topologyLeftAuxOverlap` / `topologyRightAuxOverlap`が`topologyAuxPatch`以下でないこと、およびcoarse diamond coverの`topologyAuxBase`へのactual pullbackが`topologyAuxSieve`以下でないことを直接証明する。`fineTopology ≠ ⊤`を検査し、`topologyAuxSieve`がfine-coveringかつcoarse非coveringであることからstrictnessを証明する。duplicated cover labelだけをstrictnessの根拠にしない。既存`finiteSite` / `coarseCover` / `fineCover`はR3回帰対象として保持するが、このACのcompletion evidenceに数えない。
- [ ] AC21: `topologyBrokenFaceMap`からvalid `topologyCoarseCover` / `topologyFineCover` refinementを構成できないnegative exampleがある。既存R3の`brokenFaceMap_not_refinement`も回帰contractとして保持する。
...
- [ ] AC25: topology changeで`r.le`と`Presieve.isSheaf_of_le`から`coarseRestriction`を構成し、`fineSheafificationAdjunction`を証明する。そこから`fineSheafification`の`PreservesFiniteColimits`、left-exact sheafificationから`PreservesFiniteLimits`、pointwise additive forget / `presheafToSheaf_additive`から`Additive`を導く。`sheafHMap_eq_ext`が`mapExtAddHom`、constant-sheaf iso、common-coefficient isoの具体合成を固定し、identity / compositionを証明する。その上で独立`topologySite`のcommon coefficientについて、同じ明示degree-one Čech classから`topologySourceHOneClass`をactual terminal `Sheaf.H` comparisonで構成する。fine側ではactual injective resolutionと同じČech classを表すexplicit non-split extensionのauxiliary descentから`topologyCechToFineHOneEquiv`を独立に構成し、`topologyTargetHOneClass`を定義する。`topologyCechToFineHOneEquiv_eq_sheafHMap_comp_cech`でfine-side constructionと`sheafHMap_eq_ext`のactual Ext合成との一致をrflではない証明で固定し、そこから`topologySheafHMap_on_sourceClass`、`topologyTargetHOneClass_ne_zero`、`coarseFineSheafHMap_nonzero`を証明する。`fineTopology_ne_top`を直接参照し、class、class equality、nonzero、comparison map / isoを外部premise、structure / certificate field、typeclassで受け取らない。`finiteSite := FiniteModel.twoPatchSite`、`fineTopology := ⊤`、zero coefficient、unrelated Ext classをpositive firingに使用しない。
...
- [ ] AC38: `Int → Polynomial Int` modelで`coefficientExtension_hasSheafCompose`、section-object / Spec pullback iso、`coefficientBridge_valid`、`coefficientSemanticCore_realized`、`coefficientSemanticCore_baseChangedChart`、nonidentity lawful-locus map、proper ideal、source Tor₁ witnessのgeneric base-change iso像、nonzero linear Čech class、nonzero actual `Sheaf.H` class mapが同じcoefficient changeから発火する。linear / actual-H firingは、既存`finiteSite` / `coarseCover`とは別の`finiteLinearSite`、actual terminal base、strict-diamond two-branch `finiteLinearCover`をprimitive dataから構成し、`finiteCechCoefficientCompatible`とsource / target Leray proofをその同一modelで放電してfinal actual H isoを構成する。さらに`Int → Rat`と`Nat`添字のinfinite duplicated coverでdegree-zero canonical componentがisoでないことと`infiniteProductCech_not_compatible`を証明し、Čech coefficient compatibilityの不成立を直接示す。
...
- `FiniteModel.twoPatchSite` / `coarseCover`をdegree-one nonzero linear Čech classの根拠にし、
  `finiteLinearSite`のstrict diamond、terminal base、two-branch coverを構成しない。
...
- `finiteLinearSite`の同一strict-diamond / terminal-base modelで
  `CechCoefficientBaseChangeCompatible`、target Leray、nonzero actual H classを同時に放電できない。
...
- actual coefficient H routeが`finiteCechCoefficientCompatible`、source / target Leray、
  `finiteSheafHBaseChangeIso`を`finiteLinearSite`のstrict-diamond / terminal-base
  concrete modelで放電し、identity / composition theoremが
