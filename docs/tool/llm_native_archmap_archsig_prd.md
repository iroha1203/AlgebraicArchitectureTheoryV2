...
AST、symbol index、route list、import graph、class/function map は、source inventory
候補、探索 queue、blind spot、review queue を作るための navigation aid である。
それらは static source-observation output であって、ArchMap の Atom truth ではない。機械抽出で
得られた候補は、LLM が該当 source を読んで primitive fact と evidence boundary を
確認した場合にだけ observed atom へ昇格できる。未読・未展開・private・runtime-only の
候補は、`observationGaps` または低 confidence の review cue として保持する。
...
大規模 repository では、LLM は直列に全体を読もうとせず、authority/authentication、
state/model、effects/jobs、provider/trust、domain/contracts、runtime/framework、
docs/governance などの surface に分けて並列探索してよい。各 sub-agent の出力は
`reviewedRefs`、navigation aid、candidate observations、gaps、concern hints、
uncertainty、non-conclusions を持つ候補 packet にとどめる。最終 ArchMap では integrator が
...
- ArchMap が Atom truth、lawfulness、zero curvature、Lean theorem
  discharge を証明する。
...
- AST / symbol index / route list / import graph が navigation aid であり、Atom
...
- Non-Goals が、LLM truth、Lean theorem discharge、future safety、
