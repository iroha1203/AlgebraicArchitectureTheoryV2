...
| SFT 接続表現 | operation support、workflow / event / state transition、force / attractor / consequence envelope の source cue。ただし forecast correctness や causality theorem には昇格しない。 |
...
SFT forecast correctness、LLM accuracy を結論しない。pass が意味するのは、対象 construct が
...
archmap-v0 JSON
  ...
  -> bounded AATStructurePreserved / AAT-supported SFT boundary
```
...
- preservation が構成できない例では、Lean theorem を作らず、tooling report 側で
  ...
  に落ちることを固定する。
- `ArchMapPreservationPackage` から AAT-supported SFT boundary へ接続する既存 helper と、
  ArchMap PRD の表現力 fixture が同じ claim boundary を使うことを確認する。
...
- SFT 接続では operation / workflow / state transition / consequence cue を保持するが、forecast correctness、incident causality、quality ranking には昇格しない。
...
- `ArchMapPreservationPackage` から `AATStructurePreserved`、および AAT-supported SFT boundary へ接続できることが accessor theorem で確認されている。
...
- SFT の force / attractor / ForecastCone / ConsequenceEnvelope に接続する ArchMap item を `operationSupport` 系に寄せるか、新しい mapping kind として分けるか。
...
- SFT 接続用の Lean bridge は `ArchMapPreservationPackage` の accessor に留めるか、SFT-specific preservation predicate を追加するか。
...
- ArchMap の SFT cue が forecast correctness、incident causality、quality ranking を示す。
