...
`law-equation-surface/v0.5.3` は、制度選択が参照する law universe と witness
...
ArchSig は、`ArchMap + LawPolicy + supplied law-equation-surface + MeasurementProfile` の入力検証が通った `analyze` run で `archsig-measurement-packet/v0.5.3` を作る AG measurement layer である。
...
`refactor-morphism/v0.5.3` は既存 verdict の declared transport compatibility を供給し、
`refinement-comparison/v0.5.3` は coarse-to-fine の class-zero preservation data を供給する。
