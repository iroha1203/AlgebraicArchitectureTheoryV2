...
- `ArchitectureHomotopyReport` is the primary Homotopy / Holonomy / Stokes codebase-inspection surface. It summarizes filled loops, unfilled loops, nonzero holonomy loops, local curvature cells, bounded aggregate readings, coverage gaps, measured boundary, and review focus. It is not path truth, global homology, a single architecture quality score, theorem proof, automatic violation proof, FieldSig forecast, future incident prediction, or repair-safety certificate.
