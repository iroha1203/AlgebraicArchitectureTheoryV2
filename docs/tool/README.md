...
- `analysis-summary` is the LLM-native compact reading surface for an `archsig-analysis-packet-v0`. It reports `verdict`, `qualityMeasurement`, `dominantFindings`, compact full `actionQueue`, `axisSummary`, `workflowRiskSummary`, `architecturalHoleSummary`, `bridgeSummary`, `coverageGapSummary`, `detailIndex`, `measurementBasis`, and metadata so users first see what the supplied ArchMap + LawPolicy measured without reprinting raw packet detail. Full evidence remains in `archsig-analysis-packet.json` and is reached through `detailRefs` / `packet:<json-pointer>` refs.
