...
- ArchMapStore is the forward storage boundary for ArchMap history. It models `ArchMapDelta`, `ArchMapCommit`, `ArchMapSnapshot`, `ArchMapIndex`, and validation reports so PR review can read ArchMap-level deltas instead of raw language diffs. Raw diffs may be scoping hints, not canonical semantic inputs.
...
- `pr-review` is the lightweight CI / PR surface. It reads `archmap-delta-v0`, `archmap-commit-v0`, and base/head `archsig-analysis-packet-v0` as canonical inputs. A raw diff path may be recorded only as an optional scoping hint.
