...
- `analysis-summary` is the conclusion-first reading surface for an `archsig-analysis-packet-v0`. It reports `verdict`, `qualityMeasurement`, `actionQueue`, `measurementBasis`, and metadata so users first see what the supplied ArchMap + LawPolicy measured.
