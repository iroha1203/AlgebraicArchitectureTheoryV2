...
- FieldSig / SFT forecast and governance surfaces are not owned by ArchSig. FieldSig consumes `archsig-analysis-packet-v0` as bounded current AAT structural state and projects obstruction circuits, signature axes, repair candidates, structural review boundaries, current-state/evolution boundary, and coverage gaps into SFT input boundaries for PR / diff / change-vector evolution analysis. It does not read raw ArchMap observations as forecast truth.
