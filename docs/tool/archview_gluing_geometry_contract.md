...
この文書は`archsig-viewer-gluing-geometry-golden-ux/v0.5.4`の恒久design contractである。
...
- Cech H1: nonzero cocycle supportをbounded ribbonとして表示し、monodromy verdictを生成しない。
  H1 classはsection観測(sectionValue / cocycleValue)を持つ辺だけから計測される。
  観測の無い選択辺は0値として計測に入らず、nerve edgeの`sectionObservation: not_observed`と
  packetの`silence_by_design` statement(whatNext付き)として沈黙のまま表示する。
  全選択辺が未観測の場合、cech行は`not_computed`(`sections_not_observed`)であり
  `NO_MEASURED_H1_OBSTRUCTION`系のconclusionを生成しない。
...
`archsig-gate-report/v0.5.4`は同一ディレクトリまたはtoolbarから供給する任意の第二入力である。
