...
| comparison data(incidence bridge + H¹ comparison) | PRD R4 / 定義7.1 / 定理6.4 | RepairPlan `comparison` + `h1-comparison-data/v0.5.2` | chart-indexed 集合一致または explicit bridge + identity fingerprint / 両側逆・差保存・zero保存・微分可換 | `tests/fixtures/ag_measurement/repair_plan_comparison.json` | 層 D の転送 invariant | PR-2済 |
