...
| gluing data | Current tooling contract / 第X部 定理7.3 | RepairPlan `gluingData` | declared overlap subset + sectionRef の一対一対応検査。residual class 認証では subset が residual support component の overlap 集合と完全一致する | `tests/fixtures/ag_measurement/repair_plan_component_aware_one_cent.json` | grounded gluing refs | #3784でcomponent帰属を固定 |
