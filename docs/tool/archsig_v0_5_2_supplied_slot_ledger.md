...
| component-local cocycle + additive 係数 | Current tooling contract / 定義4.2 / 補題4.5 | residual support component の `complex.tripleOverlaps` + `coefficient` | additive / δ¹∘δ⁰=0 / δ¹(0)=0。`complex.enumerationComplete=true` の component に triple が無い場合だけ `automatic-c2-zero` 認証を出力する | `tests/fixtures/ag_measurement/repair_plan_component_aware_one_cent.json` | 層 C の class 語彙 | #3784 |
| true sheaf certificate | Current tooling contract / 定義4.7 | RepairPlan `trueSheafCertificate` | cover membership + `globalCondition=assumed` の assumption 記録。residual class 認証では certificate の cover / memberCharts が residual support component と完全一致し、class row の `dependsOnAssumptions` がこの assumptionId を参照する | `tests/fixtures/ag_measurement/repair_plan_component_aware_one_cent.json` | 定理4.8 package 読み | #3784でcomponent帰属を固定 |
