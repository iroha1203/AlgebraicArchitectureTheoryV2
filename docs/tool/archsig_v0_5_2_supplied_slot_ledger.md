...
| cost model(Lipschitz L + harmonic resolution) | PRD R7 / 第VIII部 系8.7 | `measurement-profile/v0.5.2` analytic 宣言 | innerProduct weights + costModel kind/L/resolution、assumption ledger 記録 | `tests/fixtures/ag_measurement/measurement_profile_harmonic_debt.json` | harmonicDebtNorm、cost model供給時のみrepair下界行、未供給時は invariant / analytic reading の `whatNext` を一致させた typed silence | PR-5実装済 |
