...
その component の triple cocycle-zero 検査、または triple overlap cell が無い `C²=0` の
`automatic-c2-zero` である。`trueSheafCertificate.memberCharts` / `coverRef` と
`gluingData.overlapRefs` / `sectionRefs` は residual component の chart / overlap 集合に完全一致し、
別 component の supplied row は residual class 認証に使えない。この前提未供給は比較違反として扱わない。source class が計測済みの場合だけ、有限 map の適合検査または target class の zero predicate の検査へ進む。
