...
ポリシーが未定義の場合、対応する violation count は 0 として出力し、出力 metadata
に「未評価」であることを記録する。将来の v1 core では、層や境界の設定 file から
policy を読む。

## v0 signature の抽出軸

最小 extractor が計算する軸は、現行 `ArchitectureSignature` v0 と同じ 6 軸に限る。
...
| `fanoutRisk` | `totalFanout` として計算する | `empirical hypothesis` / tooling output |
| `boundaryViolationCount` | policy 未指定なら 0 と未評価 metadata を出す | `empirical hypothesis` / tooling output |
| `abstractionViolationCount` | policy 未指定なら 0 と未評価 metadata を出す | `empirical hypothesis` / tooling output |
