...
| [#358](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/358) | operation role schema | `defined only` / `proved` for schema accessor theorems | `ArchitectureOperationRole` と `OperationRoleSchema` により、Preserve / Reflect / Improve / Localize / Translate / Transfer / Assume を theorem package から参照できる。role tag / schema だけから preservation、reflection、improvement、localization、translation、transfer、assumption discharge は主張しない。 |
...
| [#360](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/360) | calculus law package と concrete theorem の接続 | `defined only` / `proved` | `finiteComposeEdgeUnionLaw`, `finiteReplaceEdgeEquivalenceLaw`, `finiteProtectIdempotenceLaw`, `finiteReverseInvolutionLaw`, `repairStepDecreasesLaw`, `synthesisCandidateSoundnessLaw`, `noSolutionCertificateSoundnessLaw` により、bounded `ArchitectureCalculusLaw` entrypoint と concrete graph / repair / synthesis theorem を接続済み。無条件の associativity、全 operation の冪等性、global flatness preservation、solver completeness、all obstruction removal は主張しない。 |
...
| --- | --- | --- |
| `compose` | concrete graph kernel + interface compatibility law | edge union kernel は実装済み。interface-mediated composition と bounded associativity は coverage / exactness 前提に相対化する。 |
| `refine` | semantic / contract package | tag / proof obligation constructor のみ実装済み。refinement soundness は future proof obligation。 |
| `abstract` | projection / observation package | projection / observation bridge は既存 API を使う。abstraction soundness は quotient / observation coverage に相対化する。 |
| `replace` | concrete graph kernel + local contract package | edge equivalence replacement と `LocalReplacementContract` は既存 API。観測同値は explicit contract 前提に相対化する。 |
| `split` | feature extension / split package | static split evidence と runtime / semantic split preservation を別前提として扱う。 |
| `merge` | contract / observation package | merged boundary の external contract preservation は future proof obligation。 |
| `isolate` | runtime / semantic localization package | selected boundary 内への localization theorem として扱う。runtime telemetry completeness は主張しない。 |
| `protect` | concrete graph identity kernel + runtime policy package | static graph kernel は identity。policy-aware runtime protection は別 theorem package。 |
| `migrate` | runtime / semantic package | staged migration と compatibility window は future proof obligation。 |
| `reverse` | concrete graph kernel | graph reverse kernel は実装済み。診断指標への接続は別 theorem package。 |
| `contract` | contract / observation package | implicit expectation を explicit contract へ写す soundness は future proof obligation。 |
