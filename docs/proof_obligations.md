...
| Event Sourcing | 状態遷移代数層 | `defined only` / future proof obligation | `StateTransitionCarrier` と `stateTransitionDesignPattern` により、event sequence / replay / projection law を selected finite law cases に相対化する入口を定義済み。Event Sourcing 固有 theorem package は Issue [#416](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/416) で扱う。 | 現時点では [relationComplexity 設計](design/relation_complexity_design.md) と pilot dataset 側で測る。実コード event log の完全性は主張しない。 |
| Saga | 状態遷移代数層 | `defined only` / future proof obligation | `StateTransitionCarrier` と `stateTransitionDesignPattern` により、compensation / weak recovery law を selected finite law cases に相対化する入口を定義済み。Saga 固有 theorem package は Issue [#416](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/416) で扱う。 | 補償が逆射であるという主張、全 failure mode の網羅、運用回復コストの証明。 |
...
で定義した `StateTransitionCarrier` / `stateTransitionDesignPattern` により、
state transition algebra の Lean carrier と selected law family の入口を持つ。
ただし、Event Sourcing / Saga 固有の operation family と closure law は
Issue [#416](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/416)
に残す。CRUD は引き続き empirical / relation-complexity 側に留める。
