...
| Issue | 対象 | Lean status | 次の扱い |
| --- | --- | --- | --- |
| [#276](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/276) / [#383](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/383) | operation family と invariant family の弱い Galois 対応、および `Ops` / `Inv` closure API | `defined only` / `proved` | `PreservesInvariant`, `Ops`, `Inv`, `ClosedInvariantSet`, `ClosedOperationSet`, `DesignPattern`, `operationInvariant_galois`、`Ops_antitone`, `Inv_antitone`, `inv_ops_extensive`, `ops_inv_extensive`, `inv_ops_monotone`, `ops_inv_monotone`, `inv_ops_idempotent`, `ops_inv_idempotent` は実装済み。束同型、完全分類、selected preservation relation 外の保存性は non-conclusions として扱う。 |
