...
| ArchitecturePath / homotopy skeleton | `defined only` / `proved` | [Lean theorem index](lean_theorem_index.md#architecture-path), parent Issue [#447](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/447), observation invariance Issue [#449](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/449). `PathHomotopy.observation_eq_append` / `PathHomotopy.observation_eq` により、independent-square / same-contract / repair-fill generator と selected head-context congruence が observation を保存する場合に限って homotopic path pair の selected observation 一致を得る。global semantic completeness や全 observation axis の保存は結論しない。 |
