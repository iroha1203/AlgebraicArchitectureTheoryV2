...
| [#482](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/482) | 第11章 Analytic Representation の公開入口と docs 索引 | `Chapter11AnalyticRepresentation.Candidate`, `Chapter11AnalyticRepresentation.Candidate.representativeDeclarations`, `Chapter11AnalyticRepresentation.Candidate.schematicCorrespondences`, `Chapter11AnalyticRepresentation.Candidate.nonConclusionBoundary` が、`AnalyticRepresentation`, `ObstructionValuation`, coupon analytic snapshot, coupon static hidden dependency example, coupon semantic rounding-order valuation, measurement boundary API の代表入口を索引する。 | docs-facing metadata / import entrypoint に限る。analytic value 単独からの flatness、global witness completeness、runtime / semantic universe completeness、実証的な cost 改善、実コード extractor completeness は追加しない。tooling report の `unmeasured` / `outOfScope` は measured zero と区別する。 |
