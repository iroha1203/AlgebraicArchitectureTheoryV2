...
- comparison: 係数環は **整数**(`coefficientRing: "integers"`)。`0.8 × price = 4p/5` なので
  ...
  生成することとして**非自明な rank で発火する**。equation generator は合成名ではなく、
  法曲面が実際に宣言している witness variable(`e_cancel_insidepay` 等)と
  skeleton simplex(`vertex:money-*` / `edge:money-*` / `triple:money-*`)に束縛する。
...
| 幕 | 結果 |
| --- | --- |
| head analyze | `MEASURED_NONGLUING_RESIDUAL_CLASS`(`run:12bbf63146e4`) |
...
| repaired analyze | `REPAIR_GLUES_WITHIN_SELECTED_COMPLEX`(`run:3087fb5359d3`) |
...
5. **equation lift atlas の独立性は検査していない。** atlas は semantic 側と別の入力欄として
   ...
   両側が独立に構成されたことではない。
6. **供給 UX は未整備。** presentation cell を含む RepairPlan の authoring は builder を直接読む必要がある。
