...
各報告の数値・結論コードの一次根拠を `evidence/` にコミットしている。ツールは ArchSig v0.5.3。
...
| `evidence/saga/` | SAGA 用 ArchMap 変種(head / repaired)、law surface・policy・profile・gate policy、repair-plan(head / repaired)、builder スクリプト |
