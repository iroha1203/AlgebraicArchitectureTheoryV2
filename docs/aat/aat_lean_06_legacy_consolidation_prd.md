...
- tracking Issue: 実装開始時に作成し、対象main commit、SD0の`M_base` / `M_delete` / `M₀` /
  4 seed / consumer closure / `Q_delete` / `H` / `Ω₀` / `Ω` / `disposition_rows` /
  `T` / `L` / `K` / `N` / `C` /
...
1. tracking Issueに次が登録されている。
   - 対象main commit SHA
   - SD0の`M_base`、`M_delete`、`M₀`、`L_seed` / `K_seed` / `N_seed` / `E_seed`、
     consumer closure、`Q_delete`、`H`（予定変更Lean module）、`Ω₀`、`Ω`、`disposition_rows`、
...
8. Part 3〜5の実装・review・CI・現行source同期が完了し、次のPRDが`docs/guideline.md`に従う
   content-invariant moveで`docs/archive/**`へ移され、active pathが存在しない。
   - `aat_lean_03_closed_equational_geometry_prd.md`
   - `aat_lean_04_reading_functoriality_prd.md`
   - `aat_lean_05_standard_geometry_reference_models_prd.md`
...
- 対象main commitを固定し、`M_base`、`M_delete`、4 seed、consumer closure、`Q_delete`、
  `M₀`、`H`、`Ω₀`、`Ω`、`disposition_rows`、`T`、`L`、`K`、`N`、`C`、`E`、`P`を
  exact nameで生成する。
...
- [ ] AC1: tracking Issueが対象main commit、`M_base`、`M_delete`、4 seed、consumer closure、
  `Q_delete`、`M₀`、`H`、
  ...
  `K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、
...
- least-fixed-point closure安定、最終`C`の処置が`preserve | derive`だけ、`H ⊆ M₀`、
  `K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、`P = T ∩ K`
...
1. **Snapshot**
   ...
   - Part 3〜5 PRDのarchive path、content-invariant move commit、active path不存在
   - `M_base`、`M_delete`、4 seed、consumer closure、`Q_delete`、`M₀`、`H`のexact集合と、
     closure安定結果
