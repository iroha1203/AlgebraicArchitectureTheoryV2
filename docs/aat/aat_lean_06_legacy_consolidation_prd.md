...
1. tracking Issueに次が登録されている。
   ...
   - `inventory_at(pinned main, M_delete) ⊆ L_seed`、`L = L_seed ∪ Q_delete ∪ Audit_drop`、closure安定、
     ...
     `Audit_drop ⊆ L`、`Audit_keep ⊆ K`、`A_seed ∩ Audit₀ = ∅`、
...
C_seed = consumer_closure \ (L_seed ∪ N_seed ∪ E_seed ∪ Q_delete ∪ Audit_drop)
...
- [ ] AC1: tracking Issueが対象main commit、`M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、
  ...
  `Audit_drop ∩ Audit_keep = ∅`、`Audit_drop ⊆ L`、`Audit_keep ⊆ K`、
...
- least-fixed-point closure安定、最終`C`の処置が`preserve | derive`だけ、`H ⊆ M₀`、
  `K_seed ⊆ K`、`Q_delete ∩ Audit₀ = ∅`、`Audit_drop ⊆ L`、`Audit_keep ⊆ K`、
