...
- tracking Issue: 実装開始時に作成し、対象main commit、SD0の`M_base` / `M_delete` / `M_audit` / `M₀` /
  5 seed / consumer closure / `Q_delete` / `H` / `T` / `Audit₀` / `Audit_reuse` / audit source `B` /
  `B_new` / `Ω₀` / `Ω` /
...
1. tracking Issueに次が登録されている。
   ...
   - SD0の`M_base`、`M_delete`、`M_audit`、`M₀`、`L_seed` / `K_seed` / `N_seed` / `E_seed` /
     ...
     `Audit₀`、`Audit_reuse`、`B_reused`、`B_new`、`Ω₀`、`Ω`、`disposition_rows`、
   ...
   - `inventory_at(pinned main, M_delete) ⊆ L_seed`、`L = L_seed ∪ Q_delete`、closure安定、
     `H ⊆ M₀`、`K_seed ⊆ K`、`unclassified_inventory_items = ∅`、
   ...
   - `B`、`Audit_reuse`、`B_reused`、`B_new`、`A_seed`のauditSource map、alias完全signature、
     `@source`右辺、owner file。`Audit₀`のexact declaration diff zeroも記録する
...
Q_delete = exact items in (consumer_closure \ L_seed) whose approved manifest
  disposition is delete-repackage
C_seed = consumer_closure \ (L_seed ∪ N_seed ∪ E_seed ∪ Q_delete)
...
Audit₀ = decl(Ω₀) whose owner path is in M_audit
T = decl(Ω₀) whose owner path is in (H \ M_audit)
B = theorem declaration items in
  ((N_seed ∪ C_seed ∪ E_seed ∪ T) \ (L_seed ∪ Q_delete))
Audit_reuse = items in Audit₀ whose normalized right-hand side is @b for some b in B
...
Q_delete ⊆ L
Audit₀ ⊆ K
exact declaration diff for Audit₀ = ∅
...
axiom_audit_alias_names = A_audit
~~~
...
安定するまで計算してから`M₀`、`Ω₀`、`T`、`Audit₀`、`B`、`Audit_reuse`、`B_new`、
`A_seed`、`Ω`をこの順で固定する。
...
これによりsignatureを維持してproof bodyだけを移行するtheoremも監査対象になる。`L_seed ∪ Q_delete`の
削除theoremは除く。`Audit₀`に既存のdirect aliasがあるsourceは`Audit_reuse`として再利用し、同じsourceへの
...
`M_audit`はexecutable contractをimportするため、その既存public alias `Audit₀`を`T` / `P`へ入れて
逆向きimportを作らない。`Audit₀ ⊆ K`とexact declaration diff zeroを専用検査し、新規aliasだけを
`A_audit = A_seed`へ分類する。
...
結論弱化、対象縮小が必要になった場合も停止する。`delete-repackage`行は`L_seed ∪ Q_delete = L`へ収載し、
...
対応させる。`Audit_reuse`はexact declaration diff zeroで再利用し、未登録の`B_new`だけに
...
- 対象main commitを固定し、`M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、`Q_delete`、
  `M₀`、`H`、`T`、`Audit₀`、`Audit_reuse`、`B`、`B_reused`、`B_new`、`Ω₀`、`Ω`、
...
- `Audit₀ ⊆ K`、既存audit aliasのexact declaration diff zero、`Audit_reuse`上のsource injectivity、
  `B_new = B \ B_reused`、`A_seed = A_audit`、`auditSource : (Audit_reuse ∪ A_seed) ≃ B`を確認する。
...
- `delete-repackage`行はR0で`L_seed ∪ Q_delete = L`へ収載したことを確認し、全consumer移行後に削除する。
...
- contract targetが`decl(N ∪ C ∪ E ∪ P)`、audit sourceが`B`、audit aliasが`A_audit`、
...
- [ ] AC1: tracking Issueが対象main commit、`M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、
  `Q_delete`、`M₀`、`H`、`T`、`Audit₀`、`Audit_reuse`、`B`、`B_reused`、`B_new`、
  `Ω₀`、`Ω`、defaultなしの`disposition_rows`、
  `L`、`K`、`N`、`C`、`E`、`A_audit`、`P`、
  `D = M₀`、全`I₀(M)`、module DAG、batch、依存順をexact nameで固定し、closure安定、
  `inventory_at(pinned main, M_delete) ⊆ L_seed`、`L = L_seed ∪ Q_delete`、`H ⊆ M₀`、
  `K_seed ⊆ K`、`Audit₀ ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit`、未分類zero、重複分類zero、
...
- [ ] AC2: `consumer signature manifest`が`consumer_closure`の全public declarationを覆い、
  ...
  `L_seed ∪ Q_delete = L`に入り、最終`C`の処置は`preserve | derive`だけである。
...
- [ ] AC5: `B`が`N_seed ∪ C_seed ∪ E_seed`の全theoremと、`T \ (L_seed ∪ Q_delete)`の
  全theoremを覆う。`Audit_reuse`のsource mapはinjective、`B_new = B \ B_reused`、
  `A_seed = A_audit`は`B_new`と一対一であり、最終`auditSource : (Audit_reuse ∪ A_seed) ≃ B`が成立する。
  各aliasは対応する`@sourceTheorem`を右辺に持ち、`#assert_standard_axioms_only`がcurrent moduleの
  全aliasを監査する。`Audit₀`は`K`に留まり、exact declaration diffがzeroである。
...
- least-fixed-point closure安定、最終`C`の処置が`preserve | derive`だけ、`H ⊆ M₀`、
  `K_seed ⊆ K`、`Audit₀ ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit`、未分類zero、
  重複分類zero、`P = T ∩ K`
...
- nonpreserved / support public names、contract target names、audit source `B`、audit alias `A_audit`の一致
- `Audit_reuse`のsource injectivity、`B_new = B \ B_reused`、
  `auditSource : (Audit_reuse ∪ A_seed) ≃ B`、各aliasの完全signature / `@source`右辺、
  `Audit₀`のexact declaration diff zero、audit commandの最終非空行
...
1. **Snapshot**
   ...
   - `M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、`Q_delete`、`M₀`、`H`、
     `T`、`Audit₀`、`Audit_reuse`、`B`、`B_reused`、`B_new`のexact集合と、closure安定結果
...
3. **Inventory and disposition**
   ...
   - consumer closure安定、最終`C`の処置が`preserve | derive`だけ、`H ⊆ M₀`、`K_seed ⊆ K`、
     `Audit₀ ⊆ K`とexact declaration diff zero
...
4. **Statement preservation**
   ...
   - 全`delete-repackage`行の`L_seed ∪ Q_delete = L`収載と独立内容なし判定
...
10. **Kernel evidence**
   ...
   - `Audit_reuse`のsource injectivity、`B_new = B \ B_reused`、
     `auditSource : (Audit_reuse ∪ A_seed) ≃ B`、各aliasの完全signature / `@source`右辺、
     `Audit₀`のexact declaration diff zero
