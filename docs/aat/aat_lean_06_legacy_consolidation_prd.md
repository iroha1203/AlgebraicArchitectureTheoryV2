...
- tracking Issue: 実装開始時に作成し、対象main commit、SD0の`M₀` / 4 seed / consumer closure /
  `Ω` / `disposition_rows` / `T` / `L` / `K` / `N` / `C` / `E` / `P`、
...
1. tracking Issueに次が登録されている。
   ...
   - SD0の`M₀`、`L_seed` / `K_seed` / `N_seed` / `E_seed`、consumer closure、`Ω`、
     `disposition_rows`、`T`（変更module内public surface）、`L`（must-not-remain）、
...
- 対象main commitを固定し、`M₀`、4 seed、consumer closure、`Ω`、`disposition_rows`、
  `T`、`L`、`K`、`N`、`C`、`E`、`P`をexact nameで生成する。
...
- [ ] AC1: tracking Issueが対象main commit、`M₀`、4 seed、consumer closure、`Ω`、defaultなしの`disposition_rows`、`T`、`L`、`K`、`N`、`C`、`E`、`P`、`D = M₀`、全`I₀(M)`、module DAG、batch、依存順をexact nameで固定し、closure安定、`K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、`P = T ∩ K`を独立計算で確認している。
...
1. **Snapshot**
   ...
   - `Ω`、defaultなしの`disposition_rows`、`T`、`L`、`K`、`N`、`C`、`E`、`P`のexact name集合
