...
- tracking Issue: 実装開始時に作成し、対象main commit、SD0の`M₀` / 4 seed / consumer closure /
  `Ω` / `T` / `L` / `K` / `N` / `C` / `E` / `P`、Module import delta contractの`D` / `I₀`、
...
1. tracking Issueに次が登録されている。
   - 対象main commit SHA
   - SD0の`M₀`、`L_seed` / `K_seed` / `N_seed` / `E_seed`、consumer closure、`Ω`、
     `T`（変更module内public surface）、`L`（must-not-remain）、
     `K`（保持）、`N`（新規・shape変更core）、`C`（変更対象consumer）、`E`（firing）、
     `P = T ∩ K`（signature固定）のexact name集合
   - closure安定、`K_seed ⊆ K`、`unclassified_inventory_items = ∅`、
...
次にないmoduleのdirect import変更、表にない追加、
...
N_seed = every new or shape-changed core item fixed by SD2–SD6
E_seed = every top-level public item and automatic projection fixed by SD9
...
Ω = public inventory items in M₀ at the pinned main commit
    ∪ L_seed ∪ K_seed ∪ N_seed ∪ C_seed ∪ E_seed
...
`consumer_closure`からexact pathで追加する。`L_seed` / `K_seed` / `N_seed` / `E_seed`は分類結果を
参照せず本PRDとpin済みsourceから先に展開し、qualified Lean reference graphのleast fixed pointを
安定するまで計算してから`Ω`を固定する。`K_seed ⊆ K`またはclosure安定式が失敗した場合は開始しない。
...
`multiply_classified_inventory_items`は抽出済み`Ω`と各seed / closureから独立計算する。
`P = T ∩ K`により、変更file内でsignatureを変えない全public declaration / projectionを
statement contractの対象に含める。
...
exact nameへ展開した`N_seed`とする。SD3冒頭のmerged actual factorizationとCohomology main statementは
...
`E = E_seed`はSD9のLean blockにtop-levelで現れる全public declarationとautomatic projectionである。
...
pre_archive_no_hit_scope = lean_source_scope ∪ live_nonmath_reference_scope
...
declaration、field、accessor、constructor、namespaceは`lean_source_scope`をLean syntaxとして読み、
comment / docstringを除いた各identifier occurrenceをparser-resolved fully-qualified nameへ戻して
...
自動reference hitとする。backtick内のshort identifierはcandidateとして実読し、Lean declaration、
Lean status、source referenceを指す場合だけhit、数学用語として使う場合は非hitとする。
protected math sourceと他PRDはlegacy declaration scanの対象ではない。

各検査はcandidate hitのpath、line、syntax kind、resolved name、採否を保存し、zero件の場合も検索keyと
...
AC1〜AC53、review、CI、merge、現行source同期を確認した後にcontent-invariant archive moveを行い、
...
- 対象main commitを固定し、`M₀`、4 seed、consumer closure、`Ω`、`T`、`L`、`K`、`N`、`C`、`E`、`P`をexact nameで生成する。
...
- closure安定、`K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、
  `P = T ∩ K`を独立計算で確認する。
...
- [ ] AC1: tracking Issueが対象main commit、`M₀`、4 seed、consumer closure、`Ω`、`T`、`L`、`K`、`N`、`C`、`E`、`P`、`D = M₀`、全`I₀(M)`、module DAG、batch、依存順をexact nameで固定し、closure安定、`K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、`P = T ∩ K`を独立計算で確認している。
...
- least-fixed-point closure安定、`K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E`、
...
- inventory kindごとの`search_key`、typed scope、parser-resolved candidate一覧とpre-archive no-hit
...
1. **Snapshot**
   ...
   - `M₀`、4 seed、consumer closureのexact集合と、closure安定結果
   - `Ω`、`T`、`L`、`K`、`N`、`C`、`E`、`P`のexact name集合
...
9. **Deletion evidence**
   - `pre_archive_no_hit_scope`、scope file一覧、全`search_key`、candidate hitとsyntax adjudication
