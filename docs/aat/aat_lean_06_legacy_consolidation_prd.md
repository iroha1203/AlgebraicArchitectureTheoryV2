...
- tracking Issue: GitHub [#3588](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/3588)で、
  対象main commit、SD0の`M_base` / `M_delete` / `M_audit` / `M₀` /
  5 seed / consumer closure / `Q_delete` / `H` / `T` / `Audit₀` / `Audit_drop` / `Audit_keep` /
  `Audit_reuse` / audit source `B` /
  `B_new` / `Ω₀` / `Ω` /
  `disposition_rows` / `L` / `K` / `N` / `C` / `E` / `A_audit` / `P`、最終public inventory、
  Module import delta contractの`D` / `I₀` / `contractTargetNames` / `contractTargetOwnerModules` /
  `contractDirectImports` / `contractImportClosure`、
  consumer signature manifest、dependencyをexact nameで固定する。進捗は本PRDへ書かない
- statement contract正本: GitHub [tracking Issue #3588](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/3588)
  本文の`Statement contract index`が列挙するSD2〜SD9のcomment群。
  SD3外の既存public consumerは同Issueの`consumer signature manifest`、`P`は同Issueでpinした
  対象main commitを正本とし、同じsignatureを複製しない
...
1. tracking Issue #3588本文の`Statement contract index`が`Statement contract policy`、
   `共通namespaceとparameter`、`SD2`〜`SD9`のcomment URLを一意に列挙し、次が登録されている。
   - 対象main commit SHA
   - SD0の`M_base`、`M_delete`、`M_audit`、`M₀`、`L_seed` / `K_seed` / `N_seed` / `E_seed` /
     `A_seed`、consumer closure、`Q_delete`、`H`（予定変更Lean module）、audit source `B`、
     `Audit₀`、`Audit_drop`、`Audit_keep`、`Audit_reuse`、`B_reused`、`B_new`、`Ω₀`、`Ω`、`disposition_rows`、
     `T`（変更module内の既存contractable public surface）、`L`（must-not-remain）、
     `K`（保持）、`N`（新規・shape変更core）、`C`（変更対象consumer）、`E`（firing）、
     `A_audit`（AxiomAudit alias）、
     `P = T ∩ K`（signature固定）のexact name集合
   - `inventory_at(pinned main, M_delete) ⊆ L_seed`、`L = L_seed ∪ Q_delete ∪ Audit_drop`、closure安定、
     `H ⊆ M₀`、`K_seed ⊆ K`、`Q_delete ∩ Audit₀ = ∅`、
     `Audit₀ = Audit_drop ∪ Audit_keep`、`Audit_drop ∩ Audit_keep = ∅`、
     `C_seed ∩ Audit₀ = ∅`、`Audit_drop ⊆ L`、`Audit_keep ⊆ K`、`A_seed ∩ Audit₀ = ∅`、
   ...
   - Module import delta contractの`D = M₀`と、全`M ∈ D`のordered `I₀(M)`。新規fileは`[]`。
     `contractTargetNames`、`contractTargetOwnerModules`、`contractDirectImports`、
     `contractImportClosure`をexact nameでmaterializeし、
     `ownerPaths(contractTargetNames) ⊆ M₀`、
     `contractModule ∉ contractTargetOwnerModules`、
     `ownerPaths(contractTargetNames) ⊆ modulePaths(contractImportClosure)`、
     `contractModule ∉ contractImportClosure`、`Formal.AG ∉ I₁(contractModule)`を確認する
   - `protected_sft_reference_scope`のexact path集合、全candidateと採否、
     `protected_sft_accepted_hits = ∅`。非zeroなら開始条件未達として停止する
   - `consumer_closure`の各宣言について、旧完全signature、新完全signatureまたは
     `SD3:<exact-name>`参照、処置、proof owner、batch、focused check。`delete-repackage`行の
     独立内容なし判定と、最終`C`行が`preserve | derive`だけであることも記録する
   - `B`、`Audit_reuse`、`B_reused`、`B_new`、`A_seed`のauditSource map、alias完全signature、
     `@source`右辺、owner file。`Audit_drop`の削除処置とno-hit、`Audit_keep`の
     exact declaration diff zeroも記録する
...
8. tracking Issueで固定した全predecessor dependencyがclosedであり、各dependencyのmerged declaration、
   review、CI、現行source同期の一次証拠linkがIssueに記録されている。predecessorのPRD filename、path、
   title、archive commitは本PRDへ記録しない。
...
executable contractだけはtarget集合から次のexact direct import列を機械生成する。

~~~text
contractModule = Formal.AG.StatementContractsLegacyConsolidation
ownerModules(X) = exact module names corresponding to ownerPaths(X)
modulePaths(J) = exact .lean paths corresponding to module names in J
transitiveImportClosure(J) = J and every module reached recursively from the final
  ordered direct imports of modules in J
contractTargetNames = decl(N ∪ C ∪ E ∪ P)
contractTargetOwnerModules = ownerModules(contractTargetNames)
contractDirectImports = stable topological order of contractTargetOwnerModules in the
  approved final module DAG, with exact module name as the tie-breaker
I₁(contractModule) = contractDirectImports
contractImportClosure = transitiveImportClosure(I₁(contractModule))

ownerPaths(contractTargetNames) ⊆ M₀
contractModule ∉ contractTargetOwnerModules
ownerPaths(contractTargetNames) ⊆ modulePaths(contractImportClosure)
contractModule ∉ contractImportClosure
Formal.AG ∉ I₁(contractModule)
~~~

`contractDirectImports`はtracking Issueへ順序を含めて全件materializeする。
手入力の代表owner一覧、`Formal.AG` aggregate、偶然のtransitive importだけでtarget到達性を満たしたことにしない。
target owner moduleの追加・削除があれば`contractTargetOwnerModules`と`contractDirectImports`を再生成し、
module DAG、statement review、全contract targetのscratch elaborationを再承認する。
...
| `Formal.AG.StatementContractsLegacyConsolidation` | `replace contractDirectImports` | `decl(N ∪ C ∪ E ∪ P)`の全owner moduleをdirect importするexact-type contractだけを持つ |
| `Formal.AG.StatementContracts` | `append [Formal.AG.StatementContractsLegacyConsolidation]` | executable contract aggregate |
| `Formal.AG.AxiomAudit` | `append [Formal.AG.StatementContractsLegacyConsolidation, Formal.AG.Examples.LegacyConsolidation]` | `B`へ既存`Audit_reuse`または新規`A_seed`を一対一対応させ、namespace全数audit |
...
listを比較する。`replace contractDirectImports`はIssueへmaterializeした生成結果とのexact一致を要求する。
`Formal/AG`から`ResearchLean.AG`をimportしない。

## PRD-fixed inventory and owner contract

### SD0 — exact inventoryと処置関数

対象main commit、本PRDの不変要求、tracking Issueのfixed statementから、次の集合をexact nameでmaterializeする。inventory itemは
public declaration、automatic projection、field / accessor、constructor、named instance、namespace、
module pathのいずれかとし、
...
M_delete = {Formal/AG/LawAlgebra/Scheme.lean}
M_audit = {Formal/AG/AxiomAudit.lean}
Audit₀ = decl(inventory_at(pinned main commit, M_audit))
L_named = exact expansion of the initial L table named entries at the pinned main commit
deleteOwnershipClosure(X) = every public inventory item owned by a structure or
  namespace item in X at the pinned main commit
L_seed = L_named ∪ inventory_at(pinned main commit, M_delete)
  ∪ deleteOwnershipClosure(L_named)
...
consumer_step(X) = X
  ∪ direct public Lean declaration consumers of decl(X) at the pinned main commit
  ∪ module-path items of modules whose ordered direct imports contain a module-path item in X
consumer_closure = least fixed point of consumer_step starting from (L_seed ∪ N_seed)
Q_delete = exact items in ((consumer_closure \ L_seed) \ Audit₀) whose approved manifest
  disposition is delete-repackage
directResolvedRefs(a) = exact public declaration items resolved from the fully elaborated
  type and value of declaration a at the pinned main commit
Audit_drop = (Audit₀ ∩ L_seed)
  ∪ {a ∈ Audit₀ |
      directResolvedRefs(a) ∩ decl(L_seed ∪ Q_delete ∪ N_seed) ≠ ∅}
Audit_keep = Audit₀ \ Audit_drop
C_seed = consumer_closure \ (L_seed ∪ N_seed ∪ E_seed ∪ Q_delete ∪ Audit₀)
...
T = decl(Ω₀) whose owner path is in (H \ M_audit)
B = theorem declaration items in
  ((N_seed ∪ C_seed ∪ E_seed ∪ T) \ (L_seed ∪ Q_delete ∪ Audit_drop))
Audit_reuse = items in Audit_keep whose normalized right-hand side is @b for some b in B
B_reused = image of Audit_reuse under its normalized @source theorem
B_new = B \ B_reused
A_seed = exact public alias definitions in AAT.AG.AxiomAudit in bijection with B_new;
  each alias has the inferred complete type and right-hand side @sourceTheorem
A_seed ∩ Audit₀ = ∅
auditSource is injective on Audit_reuse
auditSource : (Audit_reuse ∪ A_seed) ≃ B
ownerPaths(A_seed) ⊆ M_audit ⊆ M_base
Ω = Ω₀ ∪ N_seed ∪ E_seed ∪ A_seed

disposition_rows ⊆ Ω × {L, K, N, C, E, AUDIT}
...
A_audit = {x ∈ Ω | (x, AUDIT) ∈ disposition_rows}
P = T ∩ K

L = L_seed ∪ Q_delete ∪ Audit_drop
N = N_seed
C = C_seed
E = E_seed
A_audit = A_seed
...
Audit_drop ⊆ L
Audit_keep ⊆ K
Audit₀ = Audit_drop ∪ Audit_keep
Audit_drop ∩ Audit_keep = ∅
C_seed ∩ Audit₀ = ∅
exact declaration diff for Audit_keep = ∅
every C manifest disposition is preserve or derive
K_seed ⊆ K
...
L, K, N, C, E, A_audit are pairwise disjoint
Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit
...
nonpreserved_or_support_public_names = decl(N ∪ C ∪ E ∪ A_audit)
statement_contract_target_names = contractTargetNames
axiom_audit_source_names = B
axiom_audit_alias_names = Audit_reuse ∪ A_audit
~~~
...
参照せず本PRD、tracking Issueのfixed statement、pin済みsourceから先に展開し、qualified Lean reference graphのleast fixed pointを
安定するまで計算する。`Audit₀`はpin済み`M_audit`から先に固定し、consumer closureと`Q_delete`の確定後に
`Audit_drop` / `Audit_keep`を分割する。その後`M₀`、`Ω₀`、`T`、`B`、`Audit_reuse`、`B_new`、
`A_seed`、`Ω`をこの順で固定する。
`A_seed`は`AAT.AG.AxiomAudit`内のpublic aliasだけとし、`B`のsource theoremをstatement contractへ
混ぜない。`K_seed ⊆ K`、`H ⊆ M₀`、
closure安定式のいずれかが失敗した場合は開始しない。

`M_delete`の全public inventory item、`L_named`中のstructure / namespaceが所有するconstructor、field、
accessor、named instance、nested def / theorem、namespaceは`L_seed`へ機械展開する。説明上の「全field」や
module削除だけを代表rowとして残さない。manifestで`delete-repackage`と承認したclosure itemは、
既に`L_seed`ならその所属を維持し、それ以外は`Q_delete`へ移して全件を`L`として扱う。
`Audit_drop`のmanifest行も`delete-repackage`とし、`Audit_keep`または最終`C`に削除処置を残さない。

R0のsource inspectionで`L_seed` / `Q_delete` / `N_seed`にない削除対象またはshape変更coreが見つかった場合、
そのitemを`K`へ流さない。本PRD欠陥としてseed、consumer closure、`Q_delete`、`Ω`、module DAGを再計算し、
...
`B`は`N_seed ∪ C_seed ∪ E_seed`のtheoremに加え、変更moduleに残る既存theoremを`T`から全数収載する。
これによりsignatureを維持してproof bodyだけを移行するtheoremも監査対象になる。
`L_seed ∪ Q_delete ∪ Audit_drop`の削除theoremは除く。`Audit_keep`に既存のdirect aliasがあるsourceは
`Audit_reuse`として再利用し、同じsourceへの
重複aliasを追加しない。`B_new = B \ B_reused`の各sourceだけにexactly one `A_seed` aliasを追加し、
最終`auditSource : (Audit_reuse ∪ A_seed) ≃ B`を要求する。aliasは`@b`を右辺に持つ`def`または
`noncomputable def`とし、追加premise、wrapper theorem、代表targetだけの登録を認めない。
`M_audit`はexecutable contractをimportするため、その既存public declaration `Audit₀`を`T` / `P`へ入れて
逆向きimportを作らない。旧・shape変更APIをtypeまたはvalueから直接参照する既存entryは
`Audit_drop ⊆ L`として削除し、exact nameのno-hitを要求する。残る`Audit_keep ⊆ K`だけに
exact declaration diff zeroを要求し、新規aliasだけを`A_audit = A_seed`へ分類する。

`L_seed`の固定specは次である。field、constructor、accessor、named instance、namespace内の
public def / theoremもpin済みsourceからexact nameへ展開し、説明上の「全field」や「module削除」を
未展開のままIssueへ置かない。

| owner | `L_seed`へ展開する宣言 / field |
| --- | --- |
| `LawAlgebra.Scheme` | `inventory_at(pinned main commit, {Formal/AG/LawAlgebra/Scheme.lean})`の全item。`affineChartMathlibSpecLocallyRingedSpace`、`RingedAATTopos`、`ChartCompatibility`、旧`ArchitectureScheme`、`SchemeGluingData`、`AffineReturnConditions`、`affineReturnAssumptions`を含むが、これらを代表例として全数抽出を省略しない |
...
`Audit₀`のdeclarationは`C_seed`から全件除外し、`Audit_drop` / `Audit_keep`の排他的な処置だけを持つ。
`M_audit`のmodule pathはこの宣言単位の除外に含めず、import deltaと最終inventoryで別に処置する。
...
SD2〜SD6のcore declaration、SD3の列挙consumer declaration、SD9のfiring declarationの完全signatureは
本PRDへ置かず、tracking Issue #3588本文の`Statement contract index`が列挙するcomment群を唯一の正本とする。
同じsignatureをPRD、子Issue、consumer manifestの別行、別comment、現行docsへ正本として複製しない。
tracking Issue本文は少なくとも`Statement contract policy`、`共通namespaceとparameter`、`SD2`〜`SD9`の
comment URLを一意に列挙する。索引外のcommentはstatement contractに数えない。

Issue側のcontractは、実装開始前に次を完全に固定する。

- SD2〜SD6のcore declaration、SD3の列挙consumer declaration、SD9のfiring declarationの完全signature
- target名、namespace、量化対象、引数順、universe、implicit / typeclass引数、結論
- SD7のconsumer signature preservation規則、SD8のmaterial premise分類
- SD3外の既存public consumerについては、commentへ完全signatureを複製せず、
  `consumer signature manifest`とpinned mainを参照する対応

初回`Approved`は新規の未編集commentとして投稿し、次を持つ`contract revision manifest`を本文に置く。

- 索引順の各contract commentと、SD3外consumerの正本である一意な`consumer signature manifest` commentについて、
  種別、URL、database ID、`updated_at`、正規化本文SHA-256
...
初回`Approved`後または実装開始後にcontract変更が必要になった場合は関連batchを停止し、変更理由、旧 / 新signature、
追加・削除したpremise、consumer manifestとinventoryへの影響をユーザーへ報告する。人間の明示承認前にcommentを
変更しない。承認後に該当commentを編集し、scratch elaboration、独立statement review、新しいrevision manifestを
記録する。再度`Approved`になるまで変更後contractに基づく実装を完了根拠に数えない。
Issue側の編集で数学的正典、本PRDの削除・保持・置換要求、Acceptance Criteria、Failure Contract、
`preserve`対象のstatementを弱めてはならない。

`Formal/AG/StatementContractsLegacyConsolidation.lean`は、Issue contractが固定した
`decl(N ∪ C ∪ E ∪ P)`を`example : <fixed type> := <implementation>`で一対一に検査する。`#check`、
名前の存在、wrapper、alias、より弱い同値statementは達成に数えない。削除対象`L`はLean contractへ
再定義せず、本PRDのSD10が固定するno-hit検査で判定する。

## PRD-fixed audit and no-hit contract
...
- ordered direct import listを`contractDirectImports`とexact一致させ、全target owner pathが
  `modulePaths(transitiveImportClosure(I₁(contractModule)))`へ入ることを独立検査する。
- `contractModule ∉ contractTargetOwnerModules`を要求し、自己owner混入をtarget除外で隠さない。
- `contractModule ∉ contractImportClosure`かつ`Formal.AG ∉ I₁(contractModule)`を要求する。
...
protected_sft_reference_scope = tracked_snapshot_files ∩
  {docs/sft/software_field_theory.md, docs/sft/aat_interface.md}
live_nonmath_reference_scope = tracked_snapshot_files
  ...
  - live_contract_path
  - .git/**
editable_live_nonmath_reference_scope =
  live_nonmath_reference_scope - protected_sft_reference_scope
protected_sft_accepted_hits = accepted legacy declaration, Lean status,
  and source-reference candidates in protected_sft_reference_scope
protected_sft_resume_evidence = primary evidence from separate work satisfying
  the protected-source update discipline in AGENTS.md
...
候補を減らす実装を認めない。
protected math sourceとこのPRD自身はlegacy declaration scanの対象ではない。他のactive PRDは
`live_nonmath_reference_scope`へ含め、一律除外しない。
`protected_sft_reference_scope`はscan対象に含めるが、自動同期の対象にはしない。
`protected_sft_accepted_hits`が一件でも
見つかった場合は編集せず該当batchを停止する。再開には`protected_sft_resume_evidence`を要求する。
別作業の完了後にmain commitを再pinし、
inventory、consumer closure、全typed scope、candidate採否を再計算・再承認する。
...
`old_audit_hits`は`Audit_drop`の各exact nameをLean syntax、public docstring、live nonmath referenceで
検索したcandidate hitから判定する。`Audit_keep`のnameはold audit keyとして数えず、
exact declaration diff検査の対象にする。

`AxiomAudit.lean`は`B`の各source theoremに対して`AAT.AG.AxiomAudit`内へexactly one direct public aliasを
対応させる。`Audit_reuse ⊆ Audit_keep`はexact declaration diff zeroで再利用し、未登録の`B_new`だけに
`A_seed = A_audit`を追加する。各右辺は対応する`@sourceTheorem`、型はその完全な推論型とする。
`#assert_standard_axioms_only AAT.AG.AxiomAudit`を最終非空行に維持し、current module内の全aliasを監査する。
source theoremを直接importしただけで登録済みと数えること、代表theoremだけの登録、追加premiseを持つalias、
aggregate経由の偶然の到達、旧audit aliasの温存を認めない。
...
7. analytic representationはMathlib `Functor`であり、`schemeRepresentation`がproper chart inclusionを
   非可逆なtarget morphismへ写し、custom category wrapperがない。
...
12. live docsとwebsiteのaccepted legacy reference、Lean status、実装説明がcanonical owner、現行path、
    最終Lean evidenceへ同期される。
...
- 対象main commitを固定し、`M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、`Q_delete`、
  `M₀`、`H`、`T`、`Audit₀`、`Audit_drop`、`Audit_keep`、`Audit_reuse`、`B`、`B_reused`、`B_new`、`Ω₀`、`Ω`、
  `disposition_rows`、`L`、`K`、`N`、`C`、`E`、`A_audit`、`P`、`contractTargetNames`、
  `contractTargetOwnerModules`、`contractDirectImports`、`contractImportClosure`を
  exact nameで生成する。
...
- `protected_sft_reference_scope`の全candidateを採否判定し、`protected_sft_accepted_hits = ∅`を
  実装開始前に確認する。非zeroなら`protected_sft_resume_evidence`が揃うまで停止する。
- closure安定、`H ⊆ M₀`、`K_seed ⊆ K`、`Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit`、未分類zero、
  重複分類zero、`P = T ∩ K`を独立計算で確認する。
- `ownerPaths(contractTargetNames) ⊆ M₀`、`contractModule ∉ contractTargetOwnerModules`、
  `I₁(contractModule) = contractDirectImports`、全contract target ownerのimport到達性、
  `contractModule ∉ contractImportClosure`、`Formal.AG ∉ I₁(contractModule)`を独立計算で確認する。
...
- `Q_delete ∩ Audit₀ = ∅`、`Audit₀ = Audit_drop ∪ Audit_keep`、両集合の交差zero、
  `C_seed ∩ Audit₀ = ∅`、
  `Audit_drop ⊆ L`とexact name no-hit、
  `Audit_keep ⊆ K`とexact declaration diff zero、`Audit_reuse ⊆ Audit_keep`上のsource injectivity、
  `B_new = B \ B_reused`、`A_seed ∩ Audit₀ = ∅`、`A_seed = A_audit`、
  `auditSource : (Audit_reuse ∪ A_seed) ≃ B`を確認する。
- `consumer_closure`の各宣言について旧完全signature、新完全signatureまたはSD3参照、処置分類、
  proof owner、batchを固定し、全`delete-repackage`行の`L`収載・独立内容なしと、最終`C`の処置が
  `preserve | derive`だけであることを確認する。
...
- `AATSch.forget readingParameter`を`schemeRepresentation`として発火させ、`leftChartMorphism`のimage、
  underlying chart map、`IsOpenImmersion`、`¬ IsIso`をSD9のfixed theoremで検査する。
...
- `delete-repackage`行はR0で`L_seed ∪ Q_delete ∪ Audit_drop = L`へ収載したことを確認し、全consumer移行後に削除する。
- 最終`C`に`delete-repackage`行が残った場合は削除を実行せず、inventoryを再承認する。
...
- `schemeRepresentation = AATSch.forget readingParameter`をactual Functorとして構成し、proper chart inclusionを
  非可逆なtarget morphismへ写す。constant graph representationはFunctor lawの補助fixtureに限定する。
...
- target ownerから`contractTargetOwnerModules`と`contractDirectImports`を再生成してIssueへmaterializeし、
  executable contractのordered direct import listとowner-path到達性をexact検査する。
...
- `editable_live_nonmath_reference_scope`のaccepted legacy hitをcanonical owner、現行module path、現行statusへ同期する。
  `website/**`のhitは専用child Issue / PRで更新し、`website-review`を通す。旧APIを現行実装として述べる
  Lean statusまたは実装説明は、最終Lean declarationとcontractが直接支持する最小記述へ改める。
  新しい数学claim、無関係なcopy、layout、navigation、機能は変更しない。各変更blockについて旧記述、
  新記述、根拠declaration / contract、candidate採否、review URLをpacketへ残す。
- `protected_sft_reference_scope`はcandidate抽出と採否判定だけを行い、
  `protected_sft_accepted_hits = ∅`を要求する。非zeroなら自動編集せず停止し、
  `protected_sft_resume_evidence`を要求する。別作業の完了後は対象main commitを更新し、
  R0とSD10の全集合・scopeを再計算する。
...
- contract targetが`contractTargetNames = decl(N ∪ C ∪ E ∪ P)`、全target ownerが
  `contractImportClosure`へ入り、`I₁(contractModule) = contractDirectImports`、
  `contractModule ∉ contractTargetOwnerModules`、`contractModule ∉ contractImportClosure`、
  `Formal.AG ∉ I₁(contractModule)`であること、audit sourceが`B`、
  audit aliasが`Audit_reuse ∪ A_audit`、
...
- [ ] AC1: tracking Issueが対象main commit、`M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、
  `Q_delete`、`M₀`、`H`、`T`、`Audit₀`、`Audit_drop`、`Audit_keep`、`Audit_reuse`、`B`、`B_reused`、`B_new`、
  `Ω₀`、`Ω`、defaultなしの`disposition_rows`、
  `L`、`K`、`N`、`C`、`E`、`A_audit`、`P`、
  `D = M₀`、全`I₀(M)`、`contractTargetNames`、`contractTargetOwnerModules`、
  `contractDirectImports`、`contractImportClosure`、module DAG、batch、依存順をexact nameで固定し、closure安定、
  `inventory_at(pinned main, M_delete) ⊆ L_seed`、`L = L_seed ∪ Q_delete ∪ Audit_drop`、`H ⊆ M₀`、
  `K_seed ⊆ K`、`Q_delete ∩ Audit₀ = ∅`、`Audit₀ = Audit_drop ∪ Audit_keep`、
  `Audit_drop ∩ Audit_keep = ∅`、`C_seed ∩ Audit₀ = ∅`、
  `Audit_drop ⊆ L`、`Audit_keep ⊆ K`、
  `Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit`、未分類zero、重複分類zero、
  `P = T ∩ K`、`changed_lean_module_paths = H`、`final_public_inventory = Ω \ L`を独立計算で確認している。
- [ ] AC2: `consumer signature manifest`が`consumer_closure`の全public declarationを覆い、
  旧完全signature、新完全signatureまたはSD3参照、処置、proof ownerを持つ。全`delete-repackage`行は
  `L_seed ∪ Q_delete ∪ Audit_drop = L`に入り、最終`C`の処置は`preserve | derive`だけである。
- [ ] AC3: tracking Issue #3588本文の`Statement contract index`が列挙するcomment群だけがSD2〜SD6のcore statement、
  SD3の列挙consumer、SD9のfiring statementの正本であり、consumer manifestは同じsignatureを重複して正本化していない。
  `Approved`は新規の未編集commentで、索引順commentと`consumer signature manifest`の種別、URL、database ID、
  `updated_at`、正規化本文SHA-256を持つ。completion snapshotで現行index URL列とmanifestのcontract URL列がexact一致し、
  全manifest rowが現行artifactからの再計算値と行単位で一致し、approval commentの`created_at = updated_at`が成立する。
- [ ] AC4: executable contractのtarget name集合が`contractTargetNames = decl(N ∪ C ∪ E ∪ P)`と一致し、
  全targetをexact-type `example`で検査する。`I₁(contractModule) = contractDirectImports`であり、
  `ownerPaths(contractTargetNames) ⊆ M₀`、`contractModule ∉ contractTargetOwnerModules`、
  `ownerPaths(contractTargetNames) ⊆ modulePaths(contractImportClosure)`が成立する。
- [ ] AC5: `B`が`N_seed ∪ C_seed ∪ E_seed`の全theoremと、`T \ (L_seed ∪ Q_delete ∪ Audit_drop)`の
  全theoremを覆う。`Audit_reuse`のsource mapはinjective、`B_new = B \ B_reused`、
  `A_seed ∩ Audit₀ = ∅`かつ`A_seed = A_audit`は`B_new`と一対一であり、
  最終`auditSource : (Audit_reuse ∪ A_seed) ≃ B`が成立する。
  各aliasは対応する`@sourceTheorem`を右辺に持ち、`#assert_standard_axioms_only`がcurrent moduleの
  全aliasを監査する。`Audit₀ = Audit_drop ∪ Audit_keep`かつ交差zeroであり、`Audit_drop`は`L`に入り
  exact nameのpre-archive hitがzero、`Audit_keep`は`K`に留まりexact declaration diffがzeroである。
...
- [ ] AC35: SD9の`leftChartMorphism.toSchemeMap`がreference modelのleft chart mapに一致する。
  `schemeRepresentation.map leftChartMorphism`のunderlying mapも同じproper chart inclusionであり、
  `IsOpenImmersion`と`¬ IsIso`が証明されている。
- [ ] AC36: SD9に少なくとも一つの非恒真compatibility predicate、成立例、不成立例がある。
- [ ] AC37: `schemeRepresentation = AATSch.forget readingParameter`がactual Mathlib Functorであり、
  `leftChartMorphism`を`toSchemeHom`へ写し、同Functor上でidentity / compositionを発火する。
  imageのunderlying Scheme mapはreference modelのproper chart inclusionと一致し、
  `IsOpenImmersion`かつ`¬ IsIso`である。
  constant `graphRepresentation`のFunctor lawは補助fixtureとしてのみ検査する。
...
- [ ] AC39: main firingがPUnit、zero ideal、empty Scheme、identity mapだけに依存せず、analytic laneは
  `schemeRepresentation_map_leftChartMorphism_not_isIso`を主証拠に含む。
...
- [ ] AC42: old statement contractと`Audit_drop`全exact nameのpre-archive scope hitがzeroである。
...
- [ ] AC47: `B`の全source theoremが`Audit_reuse ∪ A_audit`のalias経由でstandard axiomsのみを使用する。
...
- [ ] AC49: 全`M ∈ D`について、`delete`行はfile / aggregate importがzero、それ以外は最終ordered direct import listが`I₀(M)`へdeltaを適用した`I₁(M)`とexact一致し、Research reverse importがない。
  contract moduleでは`I₁(contractModule) = contractDirectImports`のordered diffがzero、
  `contractModule ∉ contractImportClosure`、`Formal.AG ∉ I₁(contractModule)`である。
...
- [ ] AC54: `live_nonmath_reference_scope`の全short identifierを装飾の有無に依存せずtoken単位で
  candidate抽出し、このPRD以外のactive PRDも含める。`editable_live_nonmath_reference_scope`のaccepted hitは
  canonical owner / path / statusへ同期され、`protected_sft_accepted_hits = ∅`である。
  protected SFT sourceにhitがあった場合は、停止記録、`protected_sft_resume_evidence`、
  再pin後のR0 / SD10再計算を一次証拠で確認する。`website/**`の
  accepted hitと旧API依存のLean status / 実装説明は専用child PRで最終Lean evidenceへ同期され、
  各変更blockに旧記述、新記述、根拠declaration / contractがある。`website-review`が合格し、
  新しい数学claim、無関係なcopy、layout、navigation、機能のdiffがzeroである。
...
- constant objectとidentity mapだけのFunctorをanalytic laneの主firingまたはAC39の根拠に数える。
...
- executable contractのtarget ownerをdirect / transitive import closureから落とす、contract自身への
  reachabilityで充足する、または`Formal.AG` aggregate importで不足ownerを隠す。
...
- このPRD以外のactive PRDをlegacy reference scanから一律除外する。
- protected SFT sourceのaccepted hitを自動編集する。
- protected SFT sourceの変更を`protected_sft_resume_evidence`なしに完了根拠へ数える。
- websiteのaccepted legacy referenceまたは旧API依存のLean status / 実装説明を残したまま、
  websiteを対象外としてno-hitを宣言する。
...
- `schemeRepresentation`で`leftChartMorphism`をactual Scheme morphismへ写せない、またはそのunderlying
  chart inclusionについて`IsOpenImmersion`と`¬ IsIso`を同時に証明できない。
...
- `contractTargetOwnerModules`から生成したdirect import列がcycleを作る、contract自身または`Formal.AG`
  aggregateを必要とする、またはtargetを落とさず全ownerへ到達できない。
- `protected_sft_reference_scope`にaccepted hitがあり、`protected_sft_resume_evidence`がない。
...
- websiteの新機能、layout、navigation、新しい数学claim、無関係なcopy変更。R9のaccepted legacy
  referenceと、それに依存するLean status / 実装説明の最小同期だけを扱う。
- `docs/sft/software_field_theory.md`と`docs/sft/aat_interface.md`の自動編集。accepted hitがあれば停止し、
  別作業は`AGENTS.md`の保護対象更新規律に従う。
...
- `inventory_at(pinned main, M_delete) ⊆ L_seed`、delete ownership closure全数展開、
  `L = L_seed ∪ Q_delete ∪ Audit_drop`
- least-fixed-point closure安定、最終`C`の処置が`preserve | derive`だけ、`H ⊆ M₀`、
  `K_seed ⊆ K`、`Q_delete ∩ Audit₀ = ∅`、`C_seed ∩ Audit₀ = ∅`、
  `Audit_drop ⊆ L`、`Audit_keep ⊆ K`、
  `Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit`、未分類zero、
  重複分類zero、`P = T ∩ K`
...
- `contractTargetNames`のname-to-owner全数表、`contractTargetOwnerModules`、生成した
  `contractDirectImports`のordered list、`I₁(contractModule)`とのexact diff zero、
  全owner pathの`M₀`収載と`contractImportClosure`収載、
  `contractModule ∉ contractTargetOwnerModules`、`contractModule ∉ contractImportClosure`、
  `Formal.AG ∉ I₁(contractModule)`
- nonpreserved / support public names、contract target names、audit source `B`、
  audit alias `Audit_reuse ∪ A_audit`の一致
- `Audit_reuse`のsource injectivity、`B_new = B \ B_reused`、
  `auditSource : (Audit_reuse ∪ A_seed) ≃ B`、各aliasの完全signature / `@source`右辺、
  `Audit_drop`のexact name no-hit、`Audit_keep`のexact declaration diff zero、audit commandの最終非空行
...
- live nonmathの装飾なしshort identifierを含むtoken candidate一覧、accepted hitの同期先、
  active PRDを含むscope file一覧、protected SFT sourceのcandidate採否と最終accepted hit zero、
...
1. **Snapshot**
   ...
   - predecessor dependency Issue、merged declaration、review、CI、現行source同期の一次証拠link
   - `M_base`、`M_delete`、`M_audit`、5 seed、consumer closure、`Q_delete`、`M₀`、`H`、
     `T`、`Audit₀`、`Audit_drop`、`Audit_keep`、`Audit_reuse`、`B`、`B_reused`、`B_new`のexact集合と、closure安定結果
   - `Ω₀`、`Ω`、defaultなしの`disposition_rows`、`L`、`K`、`N`、`C`、`E`、`A_audit`、
     `P`のexact name集合
   - `changed_lean_module_paths = H`と`final_public_inventory = Ω \ L`の再計算結果
   - `D = M₀`と、全`M ∈ D`のordered `I₀`、delta、最終`I₁`
   - `contractTargetNames`のname-to-owner全数表、`contractTargetOwnerModules`、
     `contractDirectImports`のordered list、`contractImportClosure`、contract moduleの最終`I₁`、
     全owner pathの`M₀`収載
2. **AC evidence map**
...
3. **Inventory and disposition**
   ...
   - `inventory_at(pinned main, M_delete) ⊆ L_seed`、delete ownership closureの全item、
     `L = L_seed ∪ Q_delete ∪ Audit_drop`
   - consumer closure安定、最終`C`の処置が`preserve | derive`だけ、`H ⊆ M₀`、`K_seed ⊆ K`、
     `Q_delete ∩ Audit₀ = ∅`、`Audit₀ = Audit_drop ∪ Audit_keep`、交差zero、
     `C_seed ∩ Audit₀ = ∅`、`Audit_drop ⊆ L`とexact name no-hit、
     `Audit_keep ⊆ K`とexact declaration diff zero
   - `Ω = L ∪ K ∪ N ∪ C ∪ E ∪ A_audit`、未分類zero、重複分類zero、`P = T ∩ K`
...
4. **Statement preservation**
   ...
   - 索引順contract commentと`consumer signature manifest`の種別 / URL / database ID / `updated_at` /
   ...
   - 全`delete-repackage`行の`L_seed ∪ Q_delete ∪ Audit_drop = L`収載と独立内容なし判定
...
6. **AATSch and analytic representation**
   ...
   - `schemeRepresentation = AATSch.forget readingParameter`
   - `schemeRepresentation.map leftChartMorphism = leftChartMorphism.toSchemeHom`とunderlying chart map equality
   - imageの`IsOpenImmersion`と`¬ IsIso`
   - 同FunctorのMathlib `map_id` / `map_comp`
   - `StandardGeometryReferenceModels.left_chart_map`、`left_chart_isOpenImmersion`、
     `left_chart_not_isIso`への直接proof-use
   - constant graph Functor lawを補助fixtureとした分類、およびAC37 / AC39から除外した確認
...
9. **Deletion evidence**
   ...
   - live nonmathの全short identifier token candidate、装飾kind、採否、canonical同期先、
     このPRD以外のactive PRDを含むscope file一覧
   - protected SFT sourceの全candidate、採否、最終accepted hit zero。hitがあった場合は停止記録、
     `protected_sft_resume_evidence`、再pin後のR0 / SD10再計算結果
   - website candidateのchild Issue / PR、各変更blockの旧記述 / 新記述 / 根拠declaration・contract、
     `website-review` URL、新しい数学claim・無関係なcopy・layout・navigation・機能のdiff zero
...
10. **Kernel evidence**
   ...
   - `Audit_reuse`のsource injectivity、`B_new = B \ B_reused`、
     `auditSource : (Audit_reuse ∪ A_seed) ≃ B`、各aliasの完全signature / `@source`右辺、
     `Audit_drop`のexact name no-hit、`Audit_keep`のexact declaration diff zero
   - `#assert_standard_axioms_only AAT.AG.AxiomAudit`の対象全alias一覧と最終非空行確認
   - static scans
   - final `I₁` import comparison。contract moduleについてはtarget name-to-owner全数表、
     `contractDirectImports`とのordered diff zero、全owner pathの`contractImportClosure`収載、
     全owner pathの`M₀`収載、`contractModule ∉ contractTargetOwnerModules`、
     `contractModule ∉ contractImportClosure`、`Formal.AG ∉ I₁(contractModule)`
   - AC44 / AC45の専用検査結果
   - CI URLと結果
...
12. **Lifecycle**
    ...
    - predecessor dependency Issueのclosed確認
