...
- tracking Issue: 実装開始時に作成し、対象main commit、SD0の`M₀` / `Ω` / `T` / `L` / `K` / `N` /
  `C` / `E` / `P`、Module import delta contractの`I₀`、consumer signature manifest、dependencyを
...
1. tracking Issueに次が登録されている。
   - 対象main commit SHA
   - SD0の`M₀`（seed module exact path集合）、`Ω`（抽出全集合）、`T`（変更module内public surface）、`L`（must-not-remain）、
     `K`（保持）、`N`（新規・shape変更core）、`C`（変更対象consumer）、`E`（firing）、
     `P = T ∩ K`（signature固定）のexact name集合
   - `unclassified_inventory_items = ∅`、`multiply_classified_inventory_items = ∅`
   - Module import delta contractの全対象moduleについて、対象main commitから抽出したordered `I₀(M)`
...
tracking Issueでpinした対象main commitについて、各対象module `M`のordered direct import listを
`I₀(M)`として機械抽出し、編集開始前に全文をIssueへmaterializeする。本PRDは`I₀`から最終list
`I₁`へのdeltaを正本とする。`I₀(M)`は「当時のimport」等の文章へ置き換えず、exact module名の列として
保存する。

`keep`は`I₁(M) = I₀(M)`、`append [A, …]`は既存順を保った末尾追加、`remove [A, …]`はexact name削除、
`replace [A, …]`は`I₁(M)`全体の置換を表す。次にないmoduleのdirect import変更、表にない追加、
並び順の変更、循環依存が必要になった場合は実装を止め、このcontractとmodule DAGを再承認する。

| module `M` | `I₁(M)` | 責務 |
| --- | --- | --- |
| `Formal.AG.LawAlgebra.Scheme` | file自体を削除 | 旧ringed / chart / scheme / gluing routeを残さない |
...
AC49はIssueにmaterializeした`I₀`へ上表のordered deltaを適用した`I₁`と最終sourceのdirect import listを
全moduleで比較する。`Formal/AG`から`ResearchLean.AG`をimportしない。
...
~~~text
M₀ = every exact path ending in .lean in the 対象module metadata above
Ω = public inventory items in M₀ at the pinned main commit
    ∪ repository-wide transitive public consumers of names classified into L or N
    ∪ every new public item fixed by SD2–SD6 and SD9
T = public declarations / projections / fields in every module whose content changes
...
Ω = L ∪ K ∪ N ∪ C ∪ E
unclassified_inventory_items = Ω \ (L ∪ K ∪ N ∪ C ∪ E) = ∅
...
directoryや「consumer」という説明行を`M₀`のglobとして扱わない。それらのowner moduleは
transitive public consumer抽出からexact pathで追加する。`Ω`は分類集合の和から逆算せず、先に
source抽出結果とSD2〜SD6 / SD9のtop-level targetを結合して
固定し、その各itemへ処置関数`disposition : Ω → {L, K, N, C, E}`を一度だけ適用する。
`unclassified_inventory_items`と`multiply_classified_inventory_items`は抽出結果と処置関数から独立に
計算する。どちらかにhitがあれば実装開始条件を満たさない。`P = T ∩ K`により、変更file内で
signatureを変えない全public declaration / projectionをstatement contractの対象に含める。
...
次のanchorはすべて`K`へ入れる。完全な`K`は`Ω`の処置関数で得たexact setであり、
このanchor一覧を全数inventoryの代用にしない。
...
本PRDは削除対象のexact nameをtask contractとして列挙するため、archive前とarchive後を同じscopeで
検査すると自己hitが生じる。no-hit contractを次の二段階に固定する。

~~~text
live_contract_path = docs/aat/aat_lean_06_legacy_consolidation_prd.md
pre_archive_no_hit_scope = current repository
  - docs/archive/** - .git/** - live_contract_path
post_archive_no_hit_scope = current repository
  - docs/archive/** - .git/**
~~~

AC6、AC10、AC13、AC24、AC27、AC40〜AC43は`pre_archive_no_hit_scope`で判定する。
AC1〜AC53、review、CI、merge、現行source同期、対象PRDへのlive reference zeroを確認した後に
content-invariant archive moveを行い、同じ検索語を`post_archive_no_hit_scope`で再実行する。
git historyとGitHub Issue / PR / reviewは両scopeのrepository scan対象ではない。
...
- 対象main commitを固定し、`M₀`、`Ω`、`T`、`L`、`K`、`N`、`C`、`E`、`P`をexact nameで生成する。
- declaration、field、constructor、namespace、module import、docstring、example、contract、auditを抽出する。
- `Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、`P = T ∩ K`を独立計算で確認する。
...
- [ ] AC1: tracking Issueが対象main commit、`M₀`、`Ω`、`T`、`L`、`K`、`N`、`C`、`E`、`P`、全`I₀(M)`、module DAG、batch、依存順をexact nameで固定し、`Ω = L ∪ K ∪ N ∪ C ∪ E`、未分類zero、重複分類zero、`P = T ∩ K`を独立計算で確認している。
...
- [ ] AC49: 全対象moduleの最終ordered direct import listが`I₀`へModule import delta contractを適用した`I₁`とexact一致し、Research reverse importがない。
...
- [ ] AC53: `docs/guideline.md`のarchive前条件として、AC1〜AC52、review、CI、merge、
  現行sourceへの成果反映、対象PRDへのrepository-wide live reference zero、
  `pre_archive_no_hit_scope`の全no-hit式が満たされる。

AC53を満たした後、contentを変更せずPRDをarchiveする。移動後にcontent-invariant move、
現行repositoryからの参照zero、`post_archive_no_hit_scope`の全no-hit式、archive文書が現行sourceとして
参照されていないことを確認し、tracking Issueへpost-archive resultを追記する。
...
- 全対象moduleの`I₀`、delta、`I₁`、最終ordered direct import listのexact一致
...
- removed declaration / field / module / adapter / contract / auditのpre-archive no-hit
...
1. **Snapshot**
   ...
   - `M₀`のexact path集合と`Ω`、`T`、`L`、`K`、`N`、`C`、`E`、`P`のexact name集合
   - 全対象moduleのordered `I₀`、delta、最終`I₁`
2. **AC evidence map**
   - AC1〜AC53の各行について、対象declaration / source path、検査commandまたはreview、結果、一次証拠link
...
9. **Deletion evidence**
   - `pre_archive_no_hit_scope`、全検索語、removed declaration / field / module pathのzero結果
...
archive後はcontent-invariant move、現行reference zero、`post_archive_no_hit_scope`の全検索結果を
post-archive checkとしてtracking Issueへ追記する。
