...
見つかった場合は編集せず該当batchを停止する。再開には、対象文書と変更方針を特定した人間の明示指示、
実装者以外のLLMレビュー、人間による差分確認とmergeの三条件を要求する。merge後のmain commitを再pinし、
...
- `protected_sft_reference_scope`の全candidateを採否判定し、`protected_sft_accepted_hits = ∅`を
  実装開始前に確認する。非zeroならR9の三条件が満たされるまで停止する。
...
- `protected_sft_reference_scope`はcandidate抽出と採否判定だけを行い、
  ...
  人間の対象・方針を特定した明示指示、実装者以外のLLMレビュー、人間による差分確認とmergeを経た
  別PRの一次証拠を要求する。merge後は対象main commitを更新し、R0とSD10の全集合・scopeを再計算する。
...
- [ ] AC54: `live_nonmath_reference_scope`の全short identifierを装飾の有無に依存せずtoken単位で
  ...
  protected SFT sourceにhitがあった場合は、停止記録、人間の対象・方針を特定した明示指示、実装者以外の
  LLMレビュー、人間が差分確認してmergeした別PR、再pin後のR0 / SD10再計算を一次証拠で確認する。`website/**`の
...
- protected SFT sourceのaccepted hitを、人間の対象・方針を特定した明示指示なしに編集する。
- protected SFT sourceの変更を、実装者以外のLLMレビューまたは人間の差分確認・mergeなしに完了根拠へ数える。
...
- `protected_sft_reference_scope`にaccepted hitがあり、対象文書と変更方針を特定した人間の明示指示、
  実装者以外のLLMレビュー、人間による差分確認とmergeが揃っていない。
...
- `docs/sft/software_field_theory.md`と`docs/sft/aat_interface.md`の自動編集。accepted hitがあれば停止し、
  人間が対象文書と変更方針を特定して別途指示した作業だけを、AGENTS.mdの三条件で扱う。
...
9. **Deletion evidence**
   ...
   - protected SFT sourceの全candidate、採否、最終accepted hit zero。hitがあった場合は停止記録、
     人間の対象・方針を特定した明示指示、実装者以外のLLMレビュー、人間merge済み別PR、
     再pin後のR0 / SD10再計算結果
