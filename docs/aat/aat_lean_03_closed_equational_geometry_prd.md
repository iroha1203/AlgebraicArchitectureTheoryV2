...
- [ ] AC34: SD9 firing matrixに列挙した全new predicate / factorization typeに、独立したpositive/negative named declarationがある。`SemanticCoreIdealSheafRealized`については`weak_semanticCore_correspondence_fires`と`restrictionBrokenSchemeBridge_not_realized`をstatement contractと`AxiomAudit.lean`の双方へ登録する。
