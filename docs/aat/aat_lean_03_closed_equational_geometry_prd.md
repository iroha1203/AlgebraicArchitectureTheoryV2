...
- `docs/aat/algebraic_geometric_theory/**`の数学claimはread-onlyとし、本PRDの実装で変更しない。
  Lean status同期が必要な場合はAC40の3条件を満たすstatus-only差分に限る。
...
- protected math sourceの数学claim差分がzeroであることを確認し、status-only差分がある場合は
  AC40の3条件を満たす証拠を記録する。
...
- [ ] AC40: protected math sourceの数学claim差分がzeroである。Lean status同期が必要な場合は、
  `AGENTS.md`の人間による明示許可、実装者以外のLLM review、人間mergeを満たす
  status-only差分に限る。
...
- [ ] AC45: `docs/guideline.md`が定めるarchive前条件として、AC1〜AC44、review、CI、merge、
  現行sourceへのstatus反映、repository-wideのlive reference zeroがすべて満たされている。
...
- legacy wrapper自体の削除。
- Research成果の本体蒸留。
- protected math sourceの数学claimへの編集。AC40のstatus-only同期は除く。
...
8. protected math sourceの数学claim差分がzeroであることを確認し、status-only差分がある場合は
   AC40の3条件を満たす証拠を確認する。
...
| source evidence | protected math sourceの数学claim差分zero、status-only差分がある場合のAC40証拠、runtime status同期。 |
