# AAT代数幾何版 Lean形式化 PRD-2: 第II部 Architecture Geometry・Site・Sheaf

対象本文: [第II部 Architecture Geometry・Site・Sheaf](algebraic_geometric_theory/part_2_architecture_geometry_sites_sheaves.md)

先行PRD: [PRD-1 第I部 Atom・対象・法則](lean_ag_part_1_atoms_objects_laws_prd.md)
...
- PRD-1(PR #1911 でマージ)が第I部の形式化を定義した。本PRDの実装は
  PRD-1 の成果物(Atom carrier、公理系 A0–A8、ArchitectureObject、
  LawUniverse、ObstructionCircuit、有限モデル)に依存する。PRD-1 の実装は
...
3. 命題7.2C(finite poset regime での Čech complex の有限性と nerve 次元での
   ...
   Lean 上に存在する。
4. PRD-1 の有限モデルが第II部の水準へ拡張され、有限 poset site・
...
- 本PRDの実装は PRD-1 の `Formal/AG/Atom` 系列に依存する。各要求の着手時に
  依存する PRD-1 成果物(該当する Lean 宣言)の存在を確認し、未実装の場合は
  その要求を `blocked` として tracking Issue に記録する。PRD-1 側の実装を
  本PRDのループで先取りしない。
...
- 定義2.1 の architecture geometry `X_S^{V,U,J} = (A_S^V, ArchCtx(A_S^V), J_U)`
  を、PRD-1 の generated object と接続して定義する。第III部以降が載せる
...
- cover-relative Čech complex の有限 poset 版を定義する: 加群値係数
  ...
  `H^n(𝒰,F)`。一般 site での Čech complex は PRD-4(第IV部)の仕事であり、
  そちらで本定義との一致を完了条件にする。
...
- PRD-1 の有限モデル(`Formal/AG/Examples/`)を第II部の水準へ拡張する:
...
- この拡張モデルは PRD-3 以降(obstruction ideal、cohomology)のゴールデン例
  として再利用できる形を保つ。
...
- [ ] AC1. `Formal/AG/Site` が新設され、CI の `lake build` でビルドされる。
      PRD-1 依存の blocked 判定が tracking Issue で運用されている(R0)
...
時点から着手できる。R11 の有限モデル拡張は、PRD-1 の有限モデルが merge 済みで
