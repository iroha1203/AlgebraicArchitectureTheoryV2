...
### 定義 6.5 Architecture Monodromy Index as Analytic Reading

Architecture Monodromy Index は、operation square family 上の numerical curvature である。
...
ここで `mu_x(sigma)` は axis `x` 上の continuation trace の差であり、`AMI_X(A)` は
single quality score ではなく、top contributor を選ぶための aggregate reading である。

この reading を安全に使うには、次を区別する。

```text
proved / formalized guardrail:
  finite family, selected axis, nonnegative aggregate,
  aggregate zero -> positive-weight local zero,
  nonzero defect -> selected observation difference

design-level theorem candidate:
  covered operation complex 上の path independence / fillability equivalence

conjecture:
  feature extension の residual obstruction を boundary holonomy として局在化する式

tool diagnosis:
  ArchSig report が measured witness、missing evidence、coverage gap、review focus を出す
```

ArchSig の `AMI` や `nonzeroMonodromyWitness` は theorem witness ではなく、bounded measurement
...
この差は static dependency ではなく selected semantic continuation の差である。たとえば
...
```text
Cont_semantic(p) != Cont_semantic(q)
mu_semantic(coupon,tax;Checkout) > 0
```

なら、coupon / tax / rounding の operation square は selected semantic axis で
order-sensitive に見える。これは review で見るべき measured witness であり、global semantic
truth や merge safety ではない。
...
Boundary holonomy の読みでは、coupon feature 自体と payment core が単体では selected law を
満たしていても、境界で次の軸が戻らないことがある。

```text
Hol_semantic(Boundary(Checkout, coupon)) != 0
Hol_effect(Boundary(Checkout, coupon)) != 0
```

これは conjectural な feature-extension geometry の一部であり、現時点では theorem としてではなく
future proof obligation として扱う。
