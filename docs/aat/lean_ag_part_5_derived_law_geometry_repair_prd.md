# AAT代数幾何版 Lean形式化 Part V: 第V部 Derived Law Geometry と Repair
...
Part I〜4 で確立した方針を引き継ぐ(塔を下から積む / 忠実性契約 /
...
**(3) Part IV placeholder の清算と境界の引き直し。** Part III / Part IV が第V部へ
委譲した derived intersection(`⊗^L`、Koszul)は本PRDで実体化する。一方、
Part IV の `DerOb_U`(余接複体 `L_{Flat/X}` と `Ext¹`)は、第V部本文が余接
複体を展開しないため、本PRDでも placeholder のまま維持する。余接複体の
形式化はどのPartの実装計画 にも割り当てられていない未来課題として台帳に記録する。
...
- 当初計画では第IV部の次に第VIII部(measurement)を予定していたが、
  ...
  変更した(2026-06-13 ユーザー判断)。これにより Part III / Part IV が委譲した
  derived placeholder も部の順序で自然に解消される。
- 本PRDの実装は Part III(`O_X^U`、`I_Ob^U`、`Flat_U(X)`、square-free regime、
  Stanley-Reisner)と Part IV(障害コホモロジー、有限モデル)の成果物に
...
| 本文ラベル | 処遇 | 備考 |
| --- | --- | --- |
| 定義2.1 Lawful Locus for a Law Universe | 定義 | Part III の `I_Ob^U` の略記 `I_U` と pair `(U,V)` の記法整備 |
...
- 本PRDの実装は Part III(`Formal/AG/LawAlgebra`)と Part IV
...
- Part IV の `DerOb_U` placeholder は変更しない。余接複体の形式化が未割当の
...
- `I_U` 略記と law universe pair `(U,V)` の記法を整備し、classical joint
  lawful locus `V(I_U + I_V)` を定義する(Part III の `Flat_U` / zeroLocus を
  再利用する)。
...
- monomial law conflict regime(共有 ambient `R_W = k[E_W]` 上の
  square-free monomial ideal 対)を定義する(Part III の square-free regime を
  pair へ拡張)。
...
- 有限モデル(`Formal/AG/Examples/`)を第V部の水準へ拡張する:
  (a) 例5.6 の `Tor_1(R/<xy>, R/<xz>) != 0` を example theorem として検証
      (Part III の例5.6E の Stanley-Reisner chart と接続)。
...
- 余接複体 `L_{Flat/X}` と `Ext¹`(Part IV の `DerOb_U` placeholder の中身)は
