...
| `AATPureTheorySurface`, `AATPureTheorySurface.selectedAtomUniverse`, `SemanticMoleculeOnSurface`, `independent_of_observation`, `independent_of_sft`, `atom_of_selected_molecule`, `selected_molecule_supportedBy_selected_atoms`, `semantic_atom_of_selected_semantic_molecule` | `structure` / `def` / `theorem` | Atom / Molecule / DesignLaw / ObstructionCircuit / pattern interpretation boundary からなる pure AAT surface。選択された molecule は同じ surface の selected atom universe に閉じ、selected semantic molecule 内 atom は selected かつ semantic/interpretation atom として読める。ArchMap / ArchSig / FieldSig / SFT forecast へ依存しないことも accessor theorem で読む。 | `defined only` / `proved` |
...
| `ArchitectureAtom` | `structure` | kind、axis、support、typedPredicate、predicate label、evidence boundary、non-conclusion を持つ primitive typed fact。`typedPredicateKindAligned` と `typedPredicateAxisAligned` により constructor-level predicate と atom coordinate のずれを許さない。 | `defined only` |
