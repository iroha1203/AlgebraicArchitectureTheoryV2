...
| `GeneratedArchitectureLawModel.generated_projection_sound`, `generated_lsp_compatible`, `generated_boundary_policy_sound`, `generated_abstraction_policy_sound`, `generatedArchitectureLawful`, `generated_law_model_from_generated_object`, `generated_law_model_from_generated_graph_rank`, `directedRelation_law_model_from_graph_rank`, `generated_requiredSignatureAxesZero`, `generatedArchitectureLawful_iff_requiredSignatureAxesZero`, `generatedArchitectureLawful_of_requiredSignatureAxesZero` | `theorem` | generated law model から selected lawfulness と required signature axes zero を得る。`generated_law_model_from_generated_graph_rank` は generated object と generated graph rank certificate から law model / lawfulness を構成し、directed relation acceptance では実際の API -> database generated edge に対して rank が下がることから law model を作る。`generated_law_model_from_generated_object` は raw `WalkAcyclic` premise を受ける互換入口であり、`generatedWalkAcyclicLawModelLegacySurface` により theorem-package registry source row ではないことを固定する。`generatedArchitectureLawful_iff_requiredSignatureAxesZero` は hand-authored graph / law model や `architectureLawfulFromAAT` field を入力にしない。 | `proved` |
