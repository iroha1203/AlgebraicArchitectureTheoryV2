...
| `AtomicExamples.noEdgeAtomSuite_independent_of_observation`, `independent_of_sft`, `pure_repair_independent_of_signature`, `pure_synthesis_independent_of_signature`, `operation_preservesSurfaceInvariant`, `pure_repair_target_atomZeroCurvature`, `pure_synthesis_candidate_atomZeroCurvature`, `pure_repair_law_does_not_create_atoms`, `pure_synthesis_law_does_not_create_atoms`, `repair_architectureLawful`, `synthesis_candidate_satisfies`, `synthesis_architectureLawful`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | central theorem suite から observation / SFT / Signature 非依存、operation preservation、pure repair / pure synthesis 由来の atom zero curvature、repair / synthesis 由来の architecture lawfulness、zero-curvature theorem package を取り出す smoke test。 | `proved` |
...
| atom theorem suite | `AtomAxiomatizedTheoremSuite` | 同じ pure atom core / selected law / required molecule を使って zero-curvature、operation preservation、repair clearing、synthesis soundness を束ねる central Atom-AAT entrypoint。 |
...
| `AtomAxiomatizedTheoremSuite`, `atomZeroCurvatureTheoremPackage`, `pureOperationPackage`, `pureRepairPackage`, `pureRepairZeroCurvatureTheoremPackage`, `pureSynthesisPackage`, `pureSynthesisZeroCurvatureTheoremPackage`, `operationPackage`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedPureAAT` と `AtomAxiomatizedAAT` 上に atom-only zero-curvature、pure operation preservation、pure repair clearing、pure synthesis soundness、operation / repair / synthesis component を揃え、既存 package API へ戻せる central theorem suite。 | `defined only` |
