...
| `AAT.OperationPreservationPackage`, `AAT.RepairClearingPackage`, `AAT.SynthesisSoundnessPackage` | `structure` | operation / repair / synthesis を AATCore と zero-curvature package の後段 theorem package として読む。 | `defined only` |
...
| `GeneratedOperation`, `GeneratedOperation.atomShape_transformed`, `GeneratedOperation.operation_does_not_create_atoms` | `structure` / `theorem` | generated object 間の AtomShape transformation を、atom existence を生成しない operation として読む。 | `defined only` / `proved` |
| `GeneratedRepairTarget`, `GeneratedRepairTarget.shapeLevel`, `GeneratedRepair`, `GeneratedRepair.clears_selected_target`, `GeneratedRepair.repair_does_not_create_atoms`, `GeneratedRepairProblemConfiguration`, `GeneratedRepairProblem`, `GeneratedRepairProblemOperation`, `GeneratedRepairProblemCleared`, `GeneratedRepairFromProblem`, `GeneratedRepairFromProblem.clears_selected_problem`, `GeneratedRepairFromProblem.target_generated_molecule`, `GeneratedRepairFromProblem.repair_problem_does_not_create_atoms`, `GeneratedSynthesisCandidate`, `GeneratedSynthesisCandidate.candidate_flatWithin`, `GeneratedSynthesisCandidate.candidate_totalCurvature_eq_zero`, `GeneratedSynthesisCandidate.synthesis_does_not_create_atoms` | `inductive` / `structure` / `def` / `theorem` | valid generated object 内だけで repair target を仮定せず、`GeneratedMolecule` 前の failed atom configuration から port / slot / valence level repair problem を読み、generated repair operation によって valid generated target object / molecule へ到達する。synthesis candidate は generated flatness / curvature package に接続する。 | `defined only` / `proved` |
...
| `AtomGeneratedSignatureExamples.*`, `AtomGeneratedRepairExamples.*`, `IncompatibleAtomCompositionExamples.*` | `def` / `theorem` | positive acceptance は source-like AtomShape から generated law model / signature / SFT handoff までを通す。generated repair acceptance は missing required port configuration が直接 generated molecule にならず、repair 後に generated target molecule / object へ到達することを固定する。negative acceptance は incompatible slot、missing required port、relation endpoint 欠落、authority 欠落、concern hint、observation gap、hand-authored graph 非含意を固定する。 | `proved` |
