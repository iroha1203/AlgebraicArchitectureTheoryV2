...
| `AtomAxiomatizedSynthesisPackage` | `structure` | `AtomAxiomatizedAAT`、atom synthesis spec、candidate soundness、coverage / exactness assumptions を束ねる atom-derived synthesis package。 | `defined only` |
...
| `AtomicExamples.noEdgeAtomSuite_independent_of_observation`, `independent_of_sft`, `pure_repair_independent_of_signature`, `operation_preservesSurfaceInvariant`, `pure_repair_target_atomZeroCurvature`, `pure_repair_law_does_not_create_atoms`, `repair_architectureLawful`, `synthesis_candidate_satisfies`, `synthesis_architectureLawful`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | central theorem suite から observation / SFT / Signature 非依存、operation preservation、pure repair 由来の atom zero curvature、repair / synthesis 由来の architecture lawfulness、zero-curvature theorem package を取り出す smoke test。 | `proved` |
...
| `AtomAxiomatizedTheoremSuite`, `atomZeroCurvatureTheoremPackage`, `pureOperationPackage`, `pureRepairPackage`, `pureRepairZeroCurvatureTheoremPackage`, `operationPackage`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedPureAAT` と `AtomAxiomatizedAAT` 上に atom-only zero-curvature、pure operation preservation、pure repair clearing、operation / repair / synthesis component を揃え、既存 package API へ戻せる central theorem suite。 | `defined only` |
| `AtomAxiomatizedTheoremSuite.pure_core_surface_matches`, `independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `pure_operation_preservesSurfaceInvariant`, `pure_repair_independent_of_architecture_signature`, `pure_repair_target_noRequiredObstructionCircuit`, `pure_repair_target_atomZeroCurvature`, `pure_repair_law_does_not_create_atoms`, `independent_of_observation`, `independent_of_sft`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredObstructionCircuit`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `architectureLawful_of_synthesis`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | 同じ pure atom core から atom-level zero curvature、Signature-free operation preservation、Signature-free repair clearing、Signature 非依存、observation / SFT 非依存、zero-curvature law の atom-existence 非生成、repair-derived lawfulness、synthesis-derived lawfulness、Signature-level zero-curvature theorem package を取り出す。 | `proved` |
...
| `AtomAATSynthesisConstraint`, `AtomAATSynthesisSpec`, `AtomAxiomatizedSynthesisPackage`, `AtomConstraintContradiction` | `inductive` / `structure` / `def` | atom-generated synthesis constraints、candidate soundness、atom contradiction no-solution certificate を既存 `SynthesisConstraintSystem` / `NoSolutionCertificate` へ接続する。 | `defined only` |
| `AtomAxiomatizedSynthesisPackage.candidate_satisfies`, `candidate_noRequiredObstructionCircuit`, `architectureLawful_of_synthesis`, `requiredSignatureAxesZero_of_synthesis`, `architectureZeroCurvatureTheoremPackage_of_synthesis`, `AtomConstraintContradiction.noArchitectureSatisfies`, `noArchitectureSatisfies_of_valid_certificate` | `theorem` | atom constraint candidate から synthesis soundness と zero-curvature theorem package を導き、atom contradiction から valid no-solution certificate soundness を導く。 | `proved` |
