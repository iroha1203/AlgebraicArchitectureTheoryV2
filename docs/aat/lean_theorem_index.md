...
| `AtomSurfacePresented`, `AtomDeltaPreservesSurface`, `AtomPresentationOperation` | `def` / `structure` | pure AAT surface 上の selected atoms が presentation で測定付きに観測され、atom delta で保存されることを束ねる operation-level bridge。 | `defined only` |
...
| `AtomAxiomatizedOperationPackage`, `AtomAxiomatizedOperationPackage.preservesSurfaceInvariant`, `ops_mem_surfaceInvariant`, `architectureZeroCurvatureTheoremPackage` | `structure` / `theorem` | atom 公理系 AAT と surface-preserving atom operation を束ね、operation preservation と零曲率 theorem package を同じ package から読めるようにする。 | `defined only` / `proved` |
...
| `AtomAxiomatizedTheoremSuite`, `atomZeroCurvatureTheoremPackage`, `operationPackage`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedPureAAT` と `AtomAxiomatizedAAT` 上に atom-only zero-curvature、operation / repair / synthesis component を揃え、既存 package API へ戻せる central theorem suite。 | `defined only` |
| `AtomAxiomatizedTheoremSuite.pure_core_surface_matches`, `independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `independent_of_observation`, `independent_of_sft`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredObstructionCircuit`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `architectureLawful_of_synthesis`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | 同じ pure atom core から atom-level zero curvature、Signature 非依存、observation / SFT 非依存、zero-curvature law の atom-existence 非生成、operation preservation、repair-derived lawfulness、synthesis-derived lawfulness、Signature-level zero-curvature theorem package を取り出す。 | `proved` |
| `AtomSurfacePresented`, `AtomDeltaPreservesSurface`, `AtomPresentationOperation` | `def` / `structure` | selected pure atom surface が source / target presentation 間で保存されることを、atom delta に相対化して表す。 | `defined only` |
...
| `AtomAxiomatizedOperationPackage`, `AtomAxiomatizedOperationPackage.preservesSurfaceInvariant`, `ops_mem_surfaceInvariant`, `architectureZeroCurvatureTheoremPackage` | `structure` / `theorem` | atom 公理系 AAT package と surface-preserving operation を束ね、operation preservation と atom-derived zero curvature theorem package を同じ入口から取り出す。 | `defined only` / `proved` |
