...
| Lean 名 | 種別 | 意味 | Status |
| --- | --- | --- | --- |
| `Axis`, `AtomKind`, `MeasurementStatus`, `ObservationStatus` | `inductive` | Atom Core の coordinate、boundary-free primitive atom family、measurement boundary、observation state。`AtomKind` は `existence`, `relation`, `capability`, `dataState`, `effect`, `boundaryAuthority`, `contractSemantic`, `runtimeInteraction` を初期 core family として持つ。 | `defined only` |
