...
| `FiniteRouteCost`, `FiniteFillerCost`, `FiniteHomotopyCost`, `LowerBoundForSelectedCandidates`, `SelectedFiniteOptimum`, `SelectedFiniteOptimum.lowerBound_for_selected_candidates` | `structure` / `def` / `theorem` | supplied finite route / filler / homotopy sequence、selected finite candidate universe、lower-bound / selected optimum を記録する。global `inf` や all-route completeness は主張しない。 | `defined only` / `proved` |
| `AbstractInfimumInterface`, `AbstractInfimumInterface.lowerBound`, `AbstractInfimumInterface.exists_approximatingWitness`, `AbstractInfimumInterface.ofSelectedFiniteOptimum` | `structure` / `def` / `theorem` | finite witness / lower-bound layer の上に、candidate predicate、cost、lower bound、近似 witness を束ねる abstract infimum interface。computable global optimizer ではない。 | `defined only` / `proved` |
