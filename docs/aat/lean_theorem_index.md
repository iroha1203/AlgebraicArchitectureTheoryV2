...
| `SignatureDistanceBundle.measuredPayload?_measured`, `SignatureDistanceBundle.unmeasuredAxis_not_measuredPayload`, `SignatureDistanceBundle.unavailableAxis_not_measuredPayload`, `SignatureDistanceBundle.incomparableAxis_not_measuredPayload`, `SignatureDistanceBundle.infiniteAxis_not_measuredPayload`, `SignatureDistanceBundle.unmeasuredAxis_not_measuredZero`, `SignatureDistanceBundle.records_measurementBoundary`, `SignatureDistanceBundle.records_nonConclusions` | `theorem` | measured subtotal は measured payload だけから読まれ、未測定・利用不可・比較不能・無限 axis を measured zero や measured payload evidence として扱わないことを固定する。 | `proved` |
| `AtomRootDistanceBundle`, `AtomRootDistanceBundle.layoutDistance`, `AtomRootDistanceBundle.layoutDistance_eq_weighted_components`, `GeneratedAtomShapeDistanceBridge`, `GeneratedAtomShapeDistanceBridge.generatedDistance_eq_coordinateMismatch` | `structure` / `def` / `theorem` | fiber / carrier / valence / semantic-anchor component を選択された root Atom geometry として束ね、generated shape distance との bridge を与える。semantic ontology calibration は結論しない。 | `defined only` / `proved` |
| `Part4DistanceMeasureGeometry.generatedCarrierShapeDistanceBridge`, `generatedCarrierAtomRootDistanceBundle`, `generatedCarrierShapeDistanceBridge_unfolds`, `generatedCarrierShapeDistanceBridge_recordsRootBoundary`, `generatedCarrierRootDistance_layout_eq_generatedCarrierShapeDistance` | `def` / `theorem` | `GeneratedArchitectureObject` / `GeneratedCarrier` から root Atom distance bridge と selected layout-distance bundle を構成し、generated carrier shape distance が AtomShape coordinate mismatch に展開されることを Part IV root geometry へ接続する。 | `defined only` / `proved` |
...
| `LipschitzRepresentation.analyticDistance_le_of_structuralDistance_le`, `SpectralStabilityPackage`, `SpectralStabilityPackage.spectralDistance_le` | `structure` / `theorem` | structural distance が `ε` 以下なら analytic / spectral distance が `L * ε` 以下になる selected-scope stability surface。 | `defined only` / `proved` |
| `Part4DistanceMeasureGeometry.Candidate`, `Part4DistanceMeasureGeometry.Candidate.schematicCorrespondences`, `Part4DistanceMeasureGeometry.Candidate.nonConclusionBoundary` | `inductive` / `def` | Part IV の distance foundation、atom/configuration geometry、Atom-generated root / operation / diagnostic connection、signature path geometry、DistanceValue-aware signature aggregation、finite witness / infimum core、distance-to-region、metric Galois、operation/contractive repair、curvature/filling/transport、quantitative homotopy、representation/spectral、docs/classification boundary を横断する docs-facing entrypoint。 | `defined only` |
...
| `Part4DistanceMeasureGeometry.signatureEndpointDistance_le_pathLength`, `signatureEndpointDistance_add_hiddenExcursion_eq_pathLength`, `selectedFiniteOptimum_lowerBound_for_selected_candidates`, `selectedDistanceToRegion_region_of_distance_zero`, `metricGalois_operation_nonExpansive`, `boundedSideEffectRepair_targetDistance_decreases`, `finiteRepairSequence_allStepsDecrease`, `contractiveRepairStep_target_contracts`, `fillingCostLowerBound_observationGap_le`, `quantitativeHomotopyBound_observationDistance_le`, `finiteDehnBound_fillingArea_le`, `curvatureTransport_target_curvature_decreases`, `lipschitzRepresentation_analyticDistance_le`, `lipschitzRepresentation_analyticDistance_le_of_structuralDistance_le`, `spectralStabilityPackage_spectralDistance_le`, `abstractInfimumInterface_lowerBound` | `theorem` | Part IV entrypoint から各 bounded theorem package の代表 accessor / bridge を参照する wrapper。 | `proved` |
| `Part4DistanceMeasureGeometry.signatureDistanceBundle_measuredSubtotal_eq_selectedAxes`, `signatureDistanceBundle_measuredPayload_measured`, `signatureDistanceBundle_unmeasuredAxis_not_measuredPayload`, `signatureDistanceBundle_unmeasuredAxis_not_measuredZero`, `signatureDistanceBundle_records_measurementBoundary`, `signatureDistanceBundle_records_nonConclusions` | `theorem` | DistanceValue-aware signature aggregation layer を Part IV entrypoint から参照する wrapper。measured subtotal と non-measured axis boundary を finite path geometry から分離して読む。 | `proved` |
...
`GeneratedRepairProblemOperation` から Part IV root / operation / diagnostic package への入口であり、
