...
| `AtomGeneratedMoleculeExamples.generatedCapabilityDataStateMolecule_not_arbitrary_set`, `generatedCapability_from_capability_atom`, `generatedDataState_from_data_state_atom`, `generatedCapabilityDataStateSupport_from_data_state_atom` | `def` / `theorem` | capability atom の required data-state payload port が同じ generated molecule 内の data-state atom で満たされ、generated capability / generated data-state から capability/data-state support が構成されることを固定する。global capability completeness、runtime permission correctness、extractor completeness は結論しない。 | `proved` |
