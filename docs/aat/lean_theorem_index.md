...
| `AtomAxiomatizedPureAAT.independent_of_observation`, `independent_of_sft`, `independent_of_architecture_signature`, `selected_atom_is_primitive`, `selected_law_does_not_create_atoms`, `selected_law_does_not_change_atom_existence`, `selected_law_atoms_exist_before_law`, `circuit_on_surface` | `theorem` | pure Atom-AAT core が observation / SFT / ArchitectureSignature に依存せず、selected atoms は primitive であり、selected laws は atom existence を生成・変更せず、selected obstruction circuits は pure surface 上で閉じることを accessor theorem として読む。 | `proved` |
