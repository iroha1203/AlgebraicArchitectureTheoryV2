...
| 本文ラベル | Lean 名 | 種別 | 意味 | Status |
| --- | --- | --- | --- | --- |
| `VI.R0` | `Formal.AG.SingularityMonodromyStack`, `Formal.AG.SingularityMonodromyStack.Stratum`, `Formal.AG.SingularityMonodromyStack.CotangentInterface`, `Formal.AG.SingularityMonodromyStack.SmoothSingular`, `Formal.AG.SingularityMonodromyStack.SingularityTheorems`, `Formal.AG.SingularityMonodromyStack.Kuranishi`, `Formal.AG.SingularityMonodromyStack.OperationCategory`, `Formal.AG.SingularityMonodromyStack.OperationHomotopy`, `Formal.AG.SingularityMonodromyStack.Monodromy`, `Formal.AG.SingularityMonodromyStack.MonodromyTheorems`, `Formal.AG.SingularityMonodromyStack.RefactorGalois`, `Formal.AG.SingularityMonodromyStack.ArchitectureStack` | `import` | 第VI部 Singularity・Monodromy・Stack module を `Formal/AG.lean` から import し、PRD-3 `LawAlgebra`、PRD-4 `Cohomology`、PRD-5 `Derived` の成果物上に立ち上げる。 | `defined only` |
...
Non-conclusions: この entrypoint は codebase essence quotient stack、
decomposition stack、gerbe obstruction、finite Part6 examples をまだ形式化しない。Kuranishi Local Model の一般証明、
