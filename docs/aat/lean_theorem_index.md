...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage` | `structure` | selected atom law、required atom molecule、atom lawfulness bridge、no required obstruction circuit、layering / projection / LSP / policy arrangement readings を束ね、atom 側から static zero-curvature theorem package へ渡す entrypoint。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage` | `theorem` | no required atom obstruction circuit から `LawfulWithinAtomConfiguration` を得て、既存 `ArchitectureLawful X`、`RequiredSignatureAxesZero (signatureOf X)`、`ArchitectureZeroCurvatureTheoremPackage X` を導く。 | `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT` | `structure` | `AATPureTheorySurface` と `AtomDerivedZeroCurvaturePackage` を束ね、selected law / required molecule / required obstruction circuit が pure atom surface 上にあることを記録する atom 公理系 package。 | `defined only` |
...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage` | `structure` | atom lawfulness bridge と atom arrangement law readings から、既存 `ArchitectureLawModel` の zero-curvature theorem package へ接続する atom-derived package。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage` | `theorem` | no required atom obstruction circuit から atom lawfulness、static architecture lawfulness、required Signature axes zero、zero-curvature theorem package を順に導く。 | `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT` | `structure` | `AATPureTheorySurface` と atom-derived zero-curvature package を接続し、law / molecule / obstruction circuit が pure atom surface 上で選択されることを記録する。 | `defined only` |
