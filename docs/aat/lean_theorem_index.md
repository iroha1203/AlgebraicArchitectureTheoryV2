...
| `AATCoreLocalAlgebraForSFT`, `AATCoreAtomDelta`, `AATCoreSemanticDelta`, `AATCoreCircuitDelta`, `AATCoreTransition`, `AATCoreTransportTransition`, `AATCorePremisedConsequenceEnvelope`, `ArchSigAATCoreTransition`, `FieldSigAATCoreTransitionAnalysis`, `GeneratedArchSigAATCoreTransition`, `GeneratedFieldSigAATCoreTransitionAnalysis`, `GeneratedArchSigAATCoreTransportTransition`, `GeneratedFieldSigAATCoreTransportTransitionAnalysis` | `structure` | SFT / FieldSig が `AATCore` preservation / transport transition または ArchSig analysis state を input として読む bridge。generated transition は generated law model から source / target Signature bridge を構成し、SFT / FieldSig は Atom / AAT を再定義せず、ArchSig handoff から forecast correctness を結論しない。 | `defined only` |
| `AtomFoundationExamples.*`, `AATMoleculeLawExamples.*`, `AATZeroCurvatureExamples.*`, `AATOperationRepairSynthesisExamples.*`, `ArchMapObservationExamples.*`, `AtomSFTInterfaceExamples.*`, `AATCoreSmokeExamples.*` | `theorem` | Atom 公理 root、molecule / law / circuit、zero curvature、operation / repair / synthesis、observation separation、SFT / FieldSig transition bridge を検査する current smoke tests。 | `proved` |
...
| `GeneratedOperation`, `GeneratedOperation.atomShape_transformed`, `GeneratedOperation.operation_does_not_create_atoms`, `GeneratedOperation.toOperationTransportPackage`, `Chapter7TheoremPackages.generatedOperation_atomShape_transformed`, `Chapter7TheoremPackages.generatedOperation_toOperationTransportPackage` | `structure` / `def` / `theorem` | generated object 間の AtomShape transformation を、atom existence を生成しない operation として読み、source/target の generated AATCore 間の `OperationTransportPackage` へ接続する。Chapter 7 entrypoint からも generated operation transport を公開する。 | `defined only` / `proved` |
