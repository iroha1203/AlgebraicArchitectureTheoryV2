...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ofAtomZeroCurvatureTheoremPackage` | `structure` / `def` | selected atom law、required atom molecule、atom lawfulness bridge、no required obstruction circuit、layering / projection / LSP / policy arrangement readings を束ねる Signature-level interpretation。`ofAtomZeroCurvatureTheoremPackage` は pure atom-only zero-curvature theorem package からこの bridge を構成する。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomZeroCurvature`, `atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `architectureZeroCurvatureTheoremPackage_of_atomZeroCurvatureTheoremPackage` | `theorem` | atom-only zero curvature / no required atom obstruction circuit から `LawfulWithinAtomConfiguration` を得て、既存 `ArchitectureLawful X`、`RequiredSignatureAxesZero (signatureOf X)`、`ArchitectureZeroCurvatureTheoremPackage X`、matrix diagnostic corollaries を導く。 | `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT`, `ofPureAtomZeroCurvature` | `structure` / `def` | `AATPureTheorySurface` と `AtomDerivedZeroCurvaturePackage` を束ね、selected law / required molecule / required obstruction circuit が pure atom surface 上にあることを記録する atom 公理系 package。`ofPureAtomZeroCurvature` は pure Atom-AAT core と atom-only zero-curvature package から構成する。 | `defined only` |
...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ofAtomZeroCurvatureTheoremPackage`, `ofPureTheoremSuite` | `structure` / `def` | atom lawfulness bridge と atom arrangement law readings から、既存 `ArchitectureLawModel` の zero-curvature theorem package へ接続する atom-derived package。pure atom-only zero-curvature theorem package または `AtomAxiomatizedPureTheoremSuite` から構成できる。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomZeroCurvature`, `atomLawful`, `strictLayered`, `acyclic`, `walkAcyclic`, `projectionSound`, `noProjectionObstruction`, `lspCompatible`, `boundaryPolicySound`, `abstractionPolicySound`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `architectureZeroCurvatureTheoremPackage_of_atomZeroCurvatureTheoremPackage`, `architectureLawful_of_pureTheoremSuite`, `requiredSignatureAxesZero_of_pureTheoremSuite`, `architectureZeroCurvatureTheoremPackage_of_pureTheoremSuite`, `matrixDiagnosticCorollaries_of_pureTheoremSuite` | `theorem` | atom-only zero curvature / no required atom obstruction circuit / pure atom theorem suite から atom lawfulness を読み、Signature arrangement readings の下で strict layering、walk acyclicity、projection soundness、LSP、policy soundness、static architecture lawfulness、required Signature axes zero、zero-curvature theorem package、matrix diagnostic corollaries を順に導く。 | `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT`, `ofPureAtomZeroCurvature`, `ofPureTheoremSuite` | `structure` / `def` | `AATPureTheorySurface` と atom-derived zero-curvature package を接続し、law / molecule / obstruction circuit が pure atom surface 上で選択されることを記録する。pure Atom-AAT core と atom-only zero-curvature package、または pure theorem suite から構成できる。 | `defined only` |
