...
| `GeneratedOperation`, `GeneratedOperation.atomShape_transformed`, `GeneratedOperation.target_atom_primitive`, `GeneratedOperation.operation_does_not_create_atoms`, `GeneratedOperation.toOperationTransportPackage`, `Chapter7TheoremPackages.generatedOperation_atomShape_transformed`, `Chapter7TheoremPackages.generatedOperation_toOperationTransportPackage`, `AtomGeneratedMoleculeExamples.generatedApiExpansionOperation`, `generatedApiExpansionOperation_target_atom_primitive`, `generatedApiExpansionOperationTransportPackage`, `generatedApiExpansionOperation_target_molecule_is_distinct` | `structure` / `def` / `theorem` | generated object 間の AtomShape transformation を、atom existence を生成しない operation として読み、source/target の generated AATCore 間の `OperationTransportPackage` へ接続する。operation target primitive は caller-supplied field ではなく target generated object の carrier primitive theorem から導出される。API-only source object から API+database target object への非identity transport acceptance により、source molecule と target molecule が別物でも transport package が通ることを固定する。Chapter 7 entrypoint からも generated operation transport を公開する。 | `defined only` / `proved` |
| `GeneratedRepairTarget`, `GeneratedRepairTarget.shapeLevel`, `GeneratedRepair`, `GeneratedRepair.clears_selected_target`, `GeneratedRepair.repair_does_not_create_atoms`, `GeneratedRepair.toRepairClearingPackage`, `GeneratedRepairProblemConfiguration`, `GeneratedRepairProblem`, `GeneratedRepairProblemOperation`, `GeneratedRepairProblemOperation.target_atom_primitive`, `GeneratedRepairProblemCleared`, `GeneratedRepairFromProblem`, `GeneratedRepairFromProblem.clears_selected_problem`, `GeneratedRepairFromProblem.target_generated_molecule`, `GeneratedRepairFromProblem.repair_problem_does_not_create_atoms`, `GeneratedRepairFromProblem.toRepairClearingPackage`, `AtomGeneratedRepairExamples.repairedGeneratedGraphRank`, `AtomGeneratedRepairExamples.repaired_law_model_from_graph_rank`, `AtomGeneratedRepairExamples.generatedMissingPortRepair_target_atom_primitive`, `GeneratedSynthesisCandidate`, `GeneratedSynthesisCandidate.candidate_flatWithin`, `GeneratedSynthesisCandidate.candidate_flatWithin_eq_lawModel_generated`, `GeneratedSynthesisCandidate.candidate_totalCurvature_eq_zero`, `GeneratedSynthesisCandidate.synthesis_does_not_create_atoms`, `GeneratedSynthesisCandidate.toSynthesisSoundnessPackage`, `Chapter7TheoremPackages.generatedRepair_target_shapeLevel`, `Chapter7TheoremPackages.generatedRepair_clears_selected_target`, `Chapter7TheoremPackages.generatedRepair_toRepairClearingPackage`, `Chapter7TheoremPackages.generatedRepairFromProblem_shapeLevel`, `Chapter7TheoremPackages.generatedRepairFromProblem_clears_selected_problem`, `Chapter7TheoremPackages.generatedRepairFromProblem_toRepairClearingPackage`, `Chapter7TheoremPackages.generatedSynthesisCandidate_flatWithin`, `Chapter7TheoremPackages.generatedSynthesisCandidate_toSynthesisSoundnessPackage` | `inductive` / `structure` / `def` / `theorem` | valid generated object 内だけで repair target を仮定せず、`GeneratedMolecule` 前の failed atom configuration から port / slot / valence level repair problem を読み、generated repair operation によって valid generated target object / molecule へ到達する。repair problem operation target primitive は caller-supplied field ではなく target generated object から導出される。repair / synthesis candidate は generated graph rank から構成された generated target law model から既存 AAT repair-clearing / synthesis-soundness package に接続し、synthesis flatness は candidate field ではなく target law model の `generatedArchitectureFlatWithin` から導出し、Chapter 7 entrypoint からも公開する。 | `defined only` / `proved` |
