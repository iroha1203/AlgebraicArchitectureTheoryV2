...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `architectureZeroCurvatureTheoremPackage_of_atomZeroCurvatureTheoremPackage`, `architectureLawful_of_pureTheoremSuite`, `requiredSignatureAxesZero_of_pureTheoremSuite`, `architectureZeroCurvatureTheoremPackage_of_pureTheoremSuite` | `theorem` | atom-only zero curvature / no required atom obstruction circuit / pure atom theorem suite から atom lawfulness を読み、Signature arrangement readings の下で static architecture lawfulness、required Signature axes zero、zero-curvature theorem package を順に導く。 | `proved` |
