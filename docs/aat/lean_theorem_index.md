...
| `AATTheoremStatus.ofPureAtomTheoremSuite` | `def` | pure Atom theorem suite と Signature arrangement readings から、Atom 由来の zero-curvature theorem package を SFT interface 用 AAT theorem-status record として読む。 | `defined only` |
...
| atom-derived entrypoint | `AtomAxiomatizedPureAAT`, `AtomZeroCurvatureTheoremPackage`, `AtomAxiomatizedPureTheoremSuite`, `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ArchitectureSignature.AtomAxiomatizedAAT` | Signature 以前の pure Atom Core 公理系を入口にし、pure atom theorem suite / atom-only zero curvature から atom lawfulness を読み、別途 Signature arrangement readings を与えたときに既存 static lawfulness、required Signature axes zero、zero-curvature theorem package へ接続する。 |
...
| atom theorem suite bridge | `AtomAxiomatizedTheoremSuite`, `pureTheoremSuite` | pure atom theorem suite を既存 `ArchitectureSignature` theorem package へ接続し、必要に応じて pure suite へ忘却できる bridge。 |
...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ofAtomZeroCurvatureTheoremPackage`, `ofAtomArrangementLaws`, `ofPureTheoremSuiteArrangementLaws` | `structure` / `def` | selected atom law、required atom molecule、atom lawfulness bridge、no required obstruction circuit、layering / projection / LSP / policy arrangement readings を束ねる Signature-level interpretation。`ofAtomArrangementLaws` は full static atom arrangement law readings からこの bridge を構成する。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomZeroCurvature`, `atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `architectureZeroCurvatureTheoremPackage_of_atomZeroCurvatureTheoremPackage`, `architectureZeroCurvatureTheoremPackage_of_atomArrangementLaws`, `architectureZeroCurvatureTheoremPackage_of_pureTheoremSuiteArrangementLaws` | `theorem` | atom-only zero curvature / no required atom obstruction circuit から `LawfulWithinAtomConfiguration` を得て、full static atom arrangement law readings の下で既存 `ArchitectureLawful X`、`RequiredSignatureAxesZero (signatureOf X)`、`ArchitectureZeroCurvatureTheoremPackage X`、matrix diagnostic corollaries を導く。 | `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT`, `ofPureAtomZeroCurvature`, `ofPureAtomArrangementLaws`, `ofPureTheoremSuiteArrangementLaws` | `structure` / `def` | `AATPureTheorySurface` と `AtomDerivedZeroCurvaturePackage` を束ね、selected law / required molecule / required obstruction circuit が pure atom surface 上にあることを記録する atom 公理系 package。pure Atom-AAT core と atom-only zero-curvature package、または full static atom arrangement law readings から構成する。 | `defined only` |
...
| `AtomAxiomatizedPureTheoremSuite`, `AtomAxiomatizedTheoremSuite.pureTheoremSuite`, `AtomAxiomatizedTheoremSuite.independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `operation_preservesSurfaceInvariant`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `zeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `structure` / `theorem` | operation / repair / synthesis を同じ pure atom core に揃える Signature-free central suite と、その suite を既存 atom 公理系 AAT / Signature theorem package / matrix diagnostics へ接続する bridge。 | `defined only` / `proved` |
