...
poset 比較 target、AC5/R3、AC6/R4、AC7/R5、AC8/R6 first half、AC9/R6、AC10/R7、AC11/R8 に対応する entrypoint である。現時点では
...
Local-to-Global Flatness corollary までを Lean 上に置く。
...
Flatness hypotheses 下の criterion theorem だけを示す。任意の finite-poset regime が
...
仮定なしの Cohomological Flatness Criterion、non-abelian torsor triviality、スペクトル系列一般論、derived category、cotangent complex、`Ext^1` はまだ形式化しない。
