...
| `AtomGeneratedSignatureExamples.*`, `IncompatibleAtomCompositionExamples.*` | `def` / `theorem` | positive acceptance は source-like AtomShape から generated law model / signature / SFT handoff までを通す。negative acceptance は incompatible slot、missing required port、relation endpoint 欠落、authority 欠落、concern hint、observation gap、hand-authored graph 非含意を固定する。 | `proved` |
