...
| `AtomAxiomatizedTheoremSuite`, `AtomAxiomatizedTheoremSuite.independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `operation_preservesSurfaceInvariant`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `structure` / `theorem` | operation / repair / synthesis を同じ pure atom core / atom 公理系 AAT に揃え、主要 theorem package を central suite から読む。 | `defined only` / `proved` |
