...
| `AATCoreLocalAlgebraForSFT`, `AATCoreAtomDelta`, `AATCoreSemanticDelta`, `AATCoreCircuitDelta`, `AATCoreTransition`, `AATCorePremisedConsequenceEnvelope`, `ArchSigAATCoreTransition`, `FieldSigAATCoreTransitionAnalysis`, `GeneratedArchSigAATCoreTransition`, `GeneratedFieldSigAATCoreTransitionAnalysis` | `structure` | SFT / FieldSig が `AATCore` transition または ArchSig analysis state を input として読む bridge。generated transition は generated law model から source / target Signature bridge を構成し、SFT / FieldSig は Atom / AAT を再定義せず、ArchSig handoff から forecast correctness を結論しない。 | `defined only` |
