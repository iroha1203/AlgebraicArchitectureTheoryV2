...
| `AATPart4DistanceExamples.*` | `def` / `theorem` | `DistanceValue`、detailed diagnostic boundary、signature path bound、bounded repair、selected finite optimum / `NoRouteBelow`、Lipschitz / spectral stability を小さな selected scope で実例化する。 | `defined only` / `proved` |
...
明示 coverage / witness assumptions に相対化される。
