...
Non-conclusions: この Part VIII 初期 surface は R0/R1 entrypoint / verdict discipline、R2 finite computability
package、R3 square-free repair package だけを実装する。refactor invariance、Hodge decomposition、
LawConflict measurement、support-localized transfer、measurement packet、AAT-GAGA comparison、finite golden
examples は後続 Issue に残る。`Zero` と `NonZero` を論理補集合として扱う theorem、任意 coefficient
universe の algorithm completeness theorem、repair hitting set から actual operation semantics への自動昇格は置かない。
