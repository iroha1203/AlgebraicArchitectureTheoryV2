...
| `ArchitectureSignature.AATCoreSignatureLawfulnessBridge` | `structure` | `AAT.ZeroCurvaturePackage` を消費し、`ArchitectureLawModel` の Signature lawfulness / required axes zero へ渡す後段 analysis bridge。ArchSig / LawPolicy tooling artifact はこの bridge の入力候補を記録できるが、AAT core を定義しない。 | `defined only` |
...
| `GeneratedArchitectureLawModel.generated_noRequiredObstructionCircuit`, `generatedLawfulnessBridge`, `generated_lawfulWithinMoleculeConfiguration` | `theorem` / `def` | generated law model から pure AAT lawfulness / no required obstruction circuit bridge を構成し、既存 `ZeroCurvaturePackage` へ接続する。 | `proved` |
