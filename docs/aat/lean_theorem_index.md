...
| `AtomAxiomatizedRepairPackage` | `structure` | `AtomAxiomatizedAAT` と finite atom repair package を束ね、repair clearing を atom-derived zero-curvature entrypoint へ接続する。 | `defined only` |
...
| `AtomicExamples.noEdgeAtomSuite_independent_of_observation`, `independent_of_sft`, `operation_preservesSurfaceInvariant`, `repair_architectureLawful`, `synthesis_candidate_satisfies`, `synthesis_architectureLawful`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | central theorem suite から observation / SFT 非依存、operation preservation、repair / synthesis 由来の architecture lawfulness、zero-curvature theorem package を取り出す smoke test。 | `proved` |
...
| `AtomAxiomatizedTheoremSuite`, `atomZeroCurvatureTheoremPackage`, `pureOperationPackage`, `operationPackage`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedPureAAT` と `AtomAxiomatizedAAT` 上に atom-only zero-curvature、pure operation preservation、operation / repair / synthesis component を揃え、既存 package API へ戻せる central theorem suite。 | `defined only` |
| `AtomAxiomatizedTheoremSuite.pure_core_surface_matches`, `independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `pure_operation_preservesSurfaceInvariant`, `independent_of_observation`, `independent_of_sft`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredObstructionCircuit`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `architectureLawful_of_synthesis`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | 同じ pure atom core から atom-level zero curvature、Signature-free operation preservation、Signature 非依存、observation / SFT 非依存、zero-curvature law の atom-existence 非生成、repair-derived lawfulness、synthesis-derived lawfulness、Signature-level zero-curvature theorem package を取り出す。 | `proved` |
...
| `AtomRepairWitness`, `AtomRepairUniverse`, `AtomFiniteRepairPackage`, `AtomRepairTransitionPackage`, `AtomAxiomatizedRepairPackage` | `structure` / `def` | atom obstruction circuit を repair witness として通常の selected repair universe に渡し、finite repair clearing から target no required circuit / atom lawfulness / zero-curvature theorem package へ接続する。 | `defined only` |
| `AtomRepairWitness.bad`, `AtomRepairUniverse.repairStepDecreases_of_admissible`, `AtomFiniteRepairPackage.target_noRequiredObstructionCircuit`, `target_lawful_of_bridge`, `AtomRepairTransitionPackage.source_not_lawful_of_circuitBad`, `target_lawful_of_bridge`, `AtomAxiomatizedRepairPackage.architectureLawful_of_repair`, `requiredSignatureAxesZero_of_repair`, `architectureZeroCurvatureTheoremPackage_of_repair` | `theorem` | law-relative atom obstruction circuit を repair witness として読み、selected finite repair clearing から target lawfulness と atom-derived zero curvature を導く。 | `proved` |
