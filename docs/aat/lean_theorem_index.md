...
| `AtomAxiomatizedPureSynthesisPackage.candidate_satisfies`, `synthesisSoundnessPackage`, `candidate_satisfies_via_synthesisPackage`, `candidate_noRequiredObstructionCircuit`, `candidate_atomZeroCurvature`, `candidate_atomLawful`, `law_does_not_create_atoms`, `law_does_not_change_atom_existence`, `independent_of_architecture_signature` | `theorem` / `def` | atom-generated constraints に対する candidate soundness を通常の synthesis package として読み、candidate の no-required-circuit から atom zero curvature、atom lawfulness、law separation、Signature 非依存を導く。 | `proved` / `defined only` |
