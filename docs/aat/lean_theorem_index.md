...
| `AATCoreLocalAlgebraForSFT`, `AATCoreAtomDelta`, `AATCoreSemanticDelta`, `AATCoreCircuitDelta`, `AATCoreTransportCircuitDelta`, `AATCoreTransition`, `AATCoreTransportTransition`, `AATCorePremisedConsequenceEnvelope`, `ArchSigAATCoreTransition`, `FieldSigAATCoreTransitionAnalysis`, `GeneratedArchSigAATCoreTransition`, `GeneratedFieldSigAATCoreTransitionAnalysis`, `GeneratedArchSigAATCoreTransportTransition`, `GeneratedFieldSigAATCoreTransportTransitionAnalysis` | `structure` | SFT / FieldSig が `AATCore` preservation / transport transition または ArchSig analysis state を input として読む bridge。transport circuit delta は source / target law・molecule を別々に保持し、generated transition は generated law model から source / target Signature bridge を構成する。SFT / FieldSig は Atom / AAT を再定義せず、ArchSig handoff から forecast correctness を結論しない。 | `defined only` |
...
| `TheoremPackageClass`, `ReconstructionAction`, `ClassificationEvidence`, `ActionAllowed`, `TheoremPackageClassification`, `AATCandidate`, `classifyAATCandidate`, `classifyChapter7`, `classifyChapter8`, `classifyChapter9`, `classifyChapter10`, `classifyChapter11`, `classifySFT`, `allClassifications` | `inductive` / `structure` / `def` | 再構成計画 8.2 の theorem package 分類 registry。各 row は `atomGenerated` / `bridgeAssumed` / `representationLevel` のいずれかで、分類ごとに generated entrypoint、bridge assumption、representation surface の evidence list を要求する。`unclassified` constructor は存在しない。 | `defined only` |
| `registry_rows_have_classification_evidence`, `registry_rows_have_allowed_actions`, `generic_signature_bridge_is_temporary_bridge`, `generated_signature_bridge_is_atom_generated`, `nonidentity_transport_handoff_is_atom_generated`, `generated_filling_failure_bridge_is_atom_generated`, `generated_split_lifting_bridge_is_atom_generated`, `finite_static_core_is_downstream_representation_library` | `theorem` | registry row が分類 evidence と許可された migration action を持つこと、generic Signature bridge は temporary bridge、generated Signature bridge / 非identity transport handoff / generated filling-failure bridge / generated split-lifting bridge は Atom-generated、finite static core は downstream representation library であることを固定する。 | `proved` |
| `AtomGeneratedSignatureExamples.*`, `AtomGeneratedRepairExamples.*`, `AtomGeneratedClassificationExamples.*`, `IncompatibleAtomCompositionExamples.*` | `def` / `theorem` | positive acceptance は source-like AtomShape から generated law model / signature / generated AATCore / operation transport / synthesis soundness / SFT handoff / generated ArchSig-FieldSig preservation and transport transition analysis までを通す。classification acceptance は theorem package registry の row が分類 evidence と allowed action を持ち、generated filling-failure bridge と generated self-view split-lifting bridge が Chapter 7/9/10 の non-split / filling-failure / split-lifting / lifting-failure packages を Atom-generated として分類することを固定する。generated repair acceptance は missing required port configuration が直接 generated molecule にならず、repair 後に generated target molecule / object / repair-clearing package へ到達することを固定する。negative acceptance は incompatible slot、missing required port、relation endpoint 欠落、authority 欠落、concern hint、observation gap、hand-authored graph 非含意を固定する。 | `proved` |
