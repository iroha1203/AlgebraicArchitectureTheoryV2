...
の AC1/R0、AC2/R1、AC3/R2 の一般 complex surface、AC5/R3 に対応する
entrypoint である。現時点では
...
`ker d / im d` quotient、条件付き `H^n(X, Ob_U)` notation package までを Lean 上に置く。
...
cover-relative cohomology notation、条件付き space-level notation だけを示す。
PRD-2 finite poset Cech との一致、Local Flatness Gap、Boundary Residue、
