...
poset 比較 target、AC5/R3、AC6/R4、AC7/R5、AC8/R6 first half、AC9/R6、AC10/R7 に対応する entrypoint である。現時点では
...
failure `H^2` class、低次五項完全列形の theorem-candidate statement までを Lean 上に置く。
...
R7 の `H^2` class と低次五項 statement だけを示す。任意の finite-poset regime が
...
Cohomological Flatness Criterion、スペクトル系列一般論、derived category、cotangent complex、`Ext^1` はまだ形式化しない。
