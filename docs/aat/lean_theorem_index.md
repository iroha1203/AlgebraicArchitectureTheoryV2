...
| `AtomAxiomatizedPureAAT.independent_of_architecture_signature`, `selected_law_does_not_create_atoms`, `circuit_on_surface` | `theorem` | pure Atom-AAT core が Signature に依存せず、selected law が atom existence を生成せず、selected obstruction circuit が pure surface 上で閉じることを読む。 | `proved` |
...
| `AATPureTheorySurface`, `AATPureTheorySurface.selectedAtomUniverse`, `independent_of_observation`, `independent_of_sft`, `atom_of_selected_molecule`, `selected_molecule_supportedBy_selected_atoms` | `structure` / `def` / `theorem` | Atom / Molecule / DesignLaw / ObstructionCircuit / pattern interpretation boundary からなる pure AAT surface。選択された molecule は同じ surface の selected atom universe に閉じ、ArchMap / ArchSig / FieldSig / SFT forecast へ依存しないことを accessor theorem で読む。 | `defined only` / `proved` |
...
| `AtomAxiomatizedPureAAT.independent_of_observation`, `independent_of_sft`, `independent_of_architecture_signature`, `selected_atom_is_primitive`, `selected_molecule_atom_on_surface`, `selected_molecule_supportedBy_surface_atoms`, `selected_molecule_atom_is_primitive`, `selected_law_does_not_create_atoms`, `selected_law_does_not_change_atom_existence`, `selected_law_atoms_exist_before_law`, `circuit_on_surface` | `theorem` | pure Atom-AAT core が observation / SFT / ArchitectureSignature に依存せず、selected molecules は selected atoms だけで構成され、その atom は primitive であり、selected laws は atom existence を生成・変更せず、selected obstruction circuits は pure surface 上で閉じることを accessor theorem として読む。 | `proved` |
