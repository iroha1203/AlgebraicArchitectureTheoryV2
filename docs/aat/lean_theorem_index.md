...
`Formal/Arch/Repair/AtomPureRepair.lean`,
`Formal/Arch/Repair/AtomRepair.lean`
...
| `AtomRepairWitness` | `structure` | selected design law に相対化された required atom obstruction circuit を repair witness として読む。primitive atom ではなく law-relative minimal bad molecule。 | `defined only` |
| `AtomRepairWitness.selected`, `obstructionCircuit`, `bad` | `theorem` | atom repair witness から required molecule、obstruction circuit、law.Bad を取り出す。 | `proved` |
| `AtomRepairUniverse`, `AtomRepairUniverse.selectedObstructionUniverse` | `structure` / `def` | atom obstruction circuit witness を通常の `SelectedObstructionUniverse` として repair theorem package へ渡す bridge。 | `defined only` |
| `AtomRepairUniverse.selected`, `nonSplitWitness_of_witnessAt`, `repairStepDecreases_of_admissible` | `theorem` | atom repair witness が selected repair witness であること、state-local witness が `NonSplitExtensionWitness` になること、admissible rule から repair measure decrease を得ることを示す。 | `proved` |
| `AtomFiniteRepairPackage` | `structure` | finite repair package と target circuit witness completeness を束ね、target の required atom obstruction circuit clearing を導く atom-derived repair package。 | `defined only` |
| `AtomFiniteRepairPackage.selectedObstructionsCleared`, `target_noRequiredObstructionCircuit`, `target_lawful_of_bridge` | `theorem` | finite repair clearing から selected atom obstruction witness absence、`NoRequiredObstructionCircuit`、lawfulness bridge 経由の target atom lawfulness を得る。 | `proved` |
| `AtomRepairTransitionPackage` | `structure` | source には selected atom obstruction circuit があり、target は atom finite repair package で circuit family を cleared する、という bounded repair transition。 | `defined only` |
| `AtomRepairTransitionPackage.source_has_obstructionCircuit`, `source_not_lawful_of_circuitBad`, `target_noRequiredObstructionCircuit`, `target_lawful_of_bridge` | `theorem` | source 側の circuit 存在と non-lawfulness、target 側の no required circuit と lawfulness を取り出す。 | `proved` |
| `AtomAxiomatizedPureRepairPackage`, `AtomAxiomatizedPureRepairPackage.atomZeroCurvatureTheoremPackage` | `structure` / `def` | pure Atom-AAT core と finite atom repair package を束ね、repair clearing を Signature-free な atom zero-curvature theorem package として読む。 | `defined only` |
| `AtomAxiomatizedPureRepairPackage.target_noRequiredObstructionCircuit`, `target_atomZeroCurvature`, `target_atomLawful`, `law_does_not_create_atoms`, `law_does_not_change_atom_existence`, `law_atoms_exist_before_law`, `independent_of_architecture_signature` | `theorem` | finite atom repair clearing から no required circuit、atom-level zero curvature、atom lawfulness、law separation、Signature 非依存を取り出す。 | `proved` |
| `AtomAxiomatizedRepairPackage` | `structure` | `AtomAxiomatizedAAT` と finite atom repair package を束ね、pure repair clearing を既存 Signature theorem package へ接続する bridge。 | `defined only` |
| `AtomAxiomatizedRepairPackage.target_noRequiredObstructionCircuit`, `target_atomLawful`, `target_strictLayered`, `target_walkAcyclic`, `target_projectionSound`, `target_noProjectionObstruction`, `target_lspCompatible`, `target_boundaryPolicySound`, `target_abstractionPolicySound`, `architectureLawful_of_repair`, `requiredSignatureAxesZero_of_repair`, `architectureZeroCurvatureTheoremPackage_of_repair` | `theorem` | finite atom repair clearing から atom lawfulness、主要 static law components、既存 `ArchitectureLawful`、required Signature axes zero、zero-curvature theorem package を導く。 | `proved` |
...
に相対化され、全 repair synthesis、extractor completeness、全 semantic repair correctness は主張しない。

## Repair Synthesis

File: `Formal/Arch/Repair/RepairSynthesis.lean`,
`Formal/Arch/Repair/AtomPureSynthesis.lean`,
`Formal/Arch/Repair/AtomSynthesis.lean`
...
| `AtomAATSynthesisConstraint` | `inductive` | pure atom surface から生成する atom / molecule / law / no-required-circuit synthesis constraint。 | `defined only` |
| `AtomAATSynthesisSpec`, `AtomAATSynthesisSpec.constraintSystem` | `structure` / `def` | synthesized state が selected atoms / molecules / laws / no-required-circuit constraint を満たす意味を与え、通常の `SynthesisConstraintSystem` へ落とす。 | `defined only` |
| `AtomAATSynthesisSpec.noRequiredObstructionCircuit_of_state` | `theorem` | candidate state の no-required-circuit constraint から `NoRequiredObstructionCircuit` を得る。 | `proved` |
| `AtomAxiomatizedPureSynthesisPackage`, `AtomAxiomatizedPureSynthesisPackage.atomZeroCurvatureTheoremPackage` | `structure` / `def` | pure Atom-AAT core、atom synthesis spec、candidate soundness、coverage / exactness assumptions を束ね、candidate no-required-circuit から Signature-free な atom zero-curvature theorem package を読む。 | `defined only` |
| `AtomAxiomatizedPureSynthesisPackage.candidate_satisfies`, `synthesisSoundnessPackage`, `candidate_satisfies_via_synthesisPackage`, `candidate_noRequiredObstructionCircuit`, `candidate_atomZeroCurvature`, `candidate_atomLawful`, `law_does_not_create_atoms`, `law_does_not_change_atom_existence`, `law_atoms_exist_before_law`, `independent_of_architecture_signature` | `theorem` / `def` | atom-generated constraints に対する candidate soundness を通常の synthesis package として読み、candidate の no-required-circuit から atom zero curvature、atom lawfulness、law separation、Signature 非依存を導く。 | `proved` / `defined only` |
| `AtomAxiomatizedSynthesisPackage` | `structure` | `AtomAxiomatizedAAT`、atom synthesis spec、candidate soundness、coverage / exactness assumptions を束ねる Signature-level atom-derived synthesis bridge。 | `defined only` |
| `AtomAxiomatizedSynthesisPackage.candidate_satisfies`, `synthesisSoundnessPackage`, `candidate_satisfies_via_synthesisPackage`, `candidate_noRequiredObstructionCircuit`, `candidate_atomLawful`, `candidate_strictLayered`, `candidate_walkAcyclic`, `candidate_projectionSound`, `candidate_noProjectionObstruction`, `candidate_lspCompatible`, `candidate_boundaryPolicySound`, `candidate_abstractionPolicySound`, `architectureLawful_of_synthesis`, `requiredSignatureAxesZero_of_synthesis`, `architectureZeroCurvatureTheoremPackage_of_synthesis` | `theorem` / `def` | atom-generated constraints に対する candidate soundness を通常の synthesis package として読み、candidate の no-required-circuit から atom lawfulness、主要 static law components、architecture lawfulness、required Signature axes zero、zero-curvature theorem package を導く。 | `proved` / `defined only` |
| `AtomConstraintContradiction`, `AtomConstraintContradiction.noArchitectureSatisfies`, `noSolutionCertificate`, `noArchitectureSatisfies_of_valid_certificate` | `structure` / `theorem` / `def` | required constraint が同一 atom の存在と非存在を要求する場合、proof-carrying no-solution certificate として通常の `NoSolutionCertificate` へ渡す。 | `defined only` / `proved` |

File: `Formal/Arch/Repair/RepairSynthesisExamples.lean`
and `Formal/Arch/Examples/AtomRepairExamples.lean`,
`Formal/Arch/Examples/AtomSynthesisExamples.lean`,
`Formal/Arch/Examples/AtomPureAATExamples.lean`
...
| `AtomicExamples.forbiddenEdgeAtomRepairTransitionPackage` | `def` | forbidden edge atom obstruction circuit を source に持ち、target repaired molecule family を cleared する atom repair transition 例。 | `defined only` |
| `AtomicExamples.forbiddenEdgeAtomRepair_source_has_circuit`, `source_not_lawful`, `target_noRequiredCircuit`, `target_lawful` | `theorem` | source の selected circuit / non-lawfulness と、target の no required circuit / target lawfulness を確認する atom repair smoke test。 | `proved` |
| `AtomicExamples.noEdgePureAtomRepairPackage`, `noEdgePureAtomRepairZeroCurvatureTheoremPackage` | `def` | no-edge pure Atom-AAT core に finite atom repair package を接続し、repair clearing を atom-only zero-curvature theorem package として読む例。 | `defined only` |
| `AtomicExamples.noEdgePureAtomRepair_independent_of_signature`, `target_noRequiredCircuit`, `target_atomZeroCurvature`, `target_atomLawful`, `law_does_not_create_atoms` | `theorem` | Signature を経由せず、finite atom repair clearing から atom zero curvature / atom lawfulness / law-separation を得る pure repair smoke test。 | `proved` |
| `AtomicExamples.noEdgeAtomAxiomatizedRepairPackage` | `def` | no-edge atom 公理系 AAT に finite atom repair package を接続した例。 | `defined only` |
| `AtomicExamples.noEdgeAtomRepair_target_atomLawful`, `target_strictLayered`, `target_walkAcyclic`, `target_projectionSound`, `target_lspCompatible`, `architectureLawful`, `requiredSignatureAxesZero`, `zeroCurvatureTheoremPackage` | `theorem` | atom repair clearing から atom lawfulness、主要 static law components、architecture lawfulness、required Signature axes zero、zero-curvature theorem package を得る example theorem。 | `proved` |
| `AtomicExamples.noEdgePureAtomSynthesisPackage` | `def` | no-edge pure Atom-AAT core に atom-generated synthesis spec と candidate を接続した Signature-free synthesis example。 | `defined only` |
| `AtomicExamples.noEdgePureAtomSynthesis_independent_of_signature`, `candidate_satisfies`, `candidate_atomZeroCurvature`, `candidate_atomLawful`, `law_does_not_create_atoms` | `theorem` | Signature を経由せず、atom-generated synthesis candidate から atom zero curvature / atom lawfulness / law-separation を得る pure synthesis smoke test。 | `proved` |
| `AtomicExamples.noEdgeAtomAxiomatizedSynthesisPackage` | `def` | no-edge atom 公理系 AAT に atom-generated synthesis constraint system と candidate を接続する例。 | `defined only` |
| `AtomicExamples.noEdgeAtomSynthesis_candidate_satisfies`, `candidate_satisfies_via_package`, `candidate_noRequiredCircuit`, `candidate_strictLayered`, `candidate_walkAcyclic`, `candidate_projectionSound`, `candidate_lspCompatible`, `architectureLawful`, `requiredSignatureAxesZero`, `zeroCurvatureTheoremPackage` | `theorem` | atom-generated synthesis constraints の candidate soundness と、candidate no-required-circuit から主要 static law components、zero-curvature theorem package まで導く smoke test。 | `proved` |
| `AtomicExamples.relationAtomConstraintContradiction`, `relationAtomNoSolutionCertificate`, `relationAtomContradiction_noArchitecture`, `relationAtomNoSolution_from_valid_certificate` | `def` / `theorem` | 同一 relation atom の required / forbidden contradiction から no-solution certificate soundness を得る atom contradiction example。 | `defined only` / `proved` |
| `AtomicExamples.noEdgeAtomAxiomatizedPureTheoremSuite` | `def` | no-edge pure Atom-AAT core、atom-only zero curvature、pure operation preservation、finite atom repair clearing、atom synthesis soundness を Signature-free に揃えた central theorem-suite example。 | `defined only` |
| `AtomicExamples.noEdgePureAtomSuite_independent_of_observation`, `independent_of_sft`, `independent_of_architecture_signature`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredCircuit`, `repair_target_atomZeroCurvature`, `repair_target_atomLawful`, `synthesis_candidate_satisfies`, `synthesis_candidate_atomZeroCurvature`, `synthesis_candidate_atomLawful`, `zeroCurvatureLaw_does_not_create_atoms`, `zeroCurvatureLaw_atoms_exist_before_law`, `repair_law_does_not_create_atoms`, `repair_law_does_not_change_atom_existence`, `repair_law_atoms_exist_before_law`, `synthesis_law_does_not_create_atoms`, `synthesis_law_does_not_change_atom_existence`, `synthesis_law_atoms_exist_before_law`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `axiomatizedAAT_zeroCurvatureTheoremPackage` | `theorem` | Signature を経由せず、central pure suite から observation / SFT / Signature 非依存、operation preservation、repair-derived zero curvature / lawfulness、synthesis-derived zero curvature / lawfulness、law-separation を取り出し、別途 grouped static arrangement package を与えたときだけ Signature-level lawfulness / axes zero / zero-curvature theorem package / matrix diagnostics へ接続する smoke test。 | `proved` |
| `AtomicExamples.noEdgeAtomAxiomatizedTheoremSuite` | `def` | no-edge atom 公理系 AAT、operation preservation、repair clearing、synthesis soundness を同じ atom foundation に揃えた central theorem-suite example。 | `defined only` |
| `AtomicExamples.noEdgeAtomSuite_independent_of_observation`, `independent_of_sft`, `pure_repair_independent_of_signature`, `pure_synthesis_independent_of_signature`, `operation_preservesSurfaceInvariant`, `zeroCurvatureLaw_does_not_change_atom_existence`, `zeroCurvatureLaw_atoms_exist_before_law`, `staticArrangementLawPackage`, `zeroCurvature_architectureLawful`, `zeroCurvature_requiredSignatureAxesZero`, `pure_repair_target_atomZeroCurvature`, `pure_synthesis_candidate_atomZeroCurvature`, `pure_repair_law_does_not_create_atoms`, `pure_repair_law_does_not_change_atom_existence`, `pure_repair_law_atoms_exist_before_law`, `pure_synthesis_law_does_not_create_atoms`, `pure_synthesis_law_does_not_change_atom_existence`, `pure_synthesis_law_atoms_exist_before_law`, `pureTheoremSuite_atomZeroCurvature`, `pureTheoremSuite_independent_of_signature`, `noEdgeAtomSuite_requiredMolecule_supported_on_aat_surface`, `noEdgeAtomSuite_requiredMolecule_supported_on_pureCore_surface`, `noEdgeAtomSuite_requiredMolecule_atom_is_primitive`, `repair_architectureLawful`, `synthesis_candidate_satisfies`, `synthesis_architectureLawful`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_from_staticArrangement`, `matrixDiagnosticCorollaries`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `def` / `theorem` | Signature-connected central theorem suite から pure theorem suite へ忘却でき、そこから atom zero curvature / Signature 非依存、required molecule の surface 閉包と primitive atom 性を取り出せること、および carried static arrangement package から architecture lawfulness、required Signature axes zero、zero-curvature theorem package、matrix diagnostics を取り出す smoke test。 | `defined only` / `proved` |
...
| `AATTheoremStatus.ofPureAtomTheoremSuite` | `def` | pure Atom theorem suite と grouped Signature static arrangement package から、Atom 由来の zero-curvature theorem package を SFT interface 用 AAT theorem-status record として読む。 | `defined only` |
| `AATTheoremStatus.records_theoremPackage_of_pureAtomTheoremSuite` | `theorem` | pure Atom theorem suite constructor から stored theorem package evidence を取り出す。 | `proved` |
...
| `ArchSigAATCoreTransition`, `FieldSigAATCoreTransitionAnalysis` | `structure` | ArchSig が観測した `AATCoreTransition` を FieldSig / SFT analysis input として読む boundary package。ArchSig / FieldSig は AAT を定義しない。 | `defined only` |
| `ArchSigAATCoreTransition.records_transition_boundary`, `archsig_transition_does_not_define_aat`, `records_fieldsig_analysis_boundary`, `transition_does_not_create_atoms`, `FieldSigAATCoreTransitionAnalysis.fieldsig_reads_archsig_transition_as_sft_analysis`, `fieldsig_does_not_define_aat`, `transition_does_not_create_atoms`, `forecast_correctness_remains_boundary`, `theorem_and_nonConclusions_remain_boundaries` | `theorem` | ArchSig transition / FieldSig analysis が AATCore transition を SFT analysis として読むだけで、AAT 定義、atom creation、forecast correctness へ昇格しないことを取り出す。 | `proved` |
...
`Formal/Arch/Signature/AtomZeroCurvature.lean`,
`Formal/Arch/Operation/AtomOperation.lean`,
...
| atom-derived entrypoint | `AtomAxiomatizedPureAAT`, `AtomZeroCurvatureTheoremPackage`, `AtomAxiomatizedPureTheoremSuite`, `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ArchitectureSignature.AtomAxiomatizedAAT` | Signature 以前の pure Atom Core 公理系を入口にし、pure atom theorem suite / atom-only zero curvature から atom lawfulness を読み、別途 grouped Signature static arrangement package を与えたときに既存 static lawfulness、required Signature axes zero、zero-curvature theorem package へ接続する。 |
| atom operation preservation | `AtomPresentationOperation`, `AtomAxiomatizedOperationPackage` | atom presentation delta が pure atom surface を保存することを、通常の `PreservesInvariant` / `Ops` bridge として読む。 |
| pure atom theorem suite | `AtomAxiomatizedPureTheoremSuite` | Signature を経由せず、同じ pure atom core / selected law / required molecule を使って zero-curvature、operation preservation、repair clearing、synthesis soundness を束ねる central Atom-AAT entrypoint。 |
| atom theorem suite bridge | `AtomAxiomatizedTheoremSuite`, `staticArrangementLawPackage`, `pureTheoremSuite` | pure atom theorem suite と carried grouped static arrangement package を既存 `ArchitectureSignature` theorem package へ接続し、必要に応じて pure suite へ忘却できる bridge。 |
...
| `AtomAxiomatizedPureAAT` | `structure` | `AATPureTheorySurface`、law-separation、selected circuit closure、ArchitectureSignature 非依存 witness を束ね、Signature 以前の pure Atom-AAT core として読む entrypoint。 | `defined only` |
| `AtomAxiomatizedPureAAT.independent_of_architecture_signature`, `selected_law_does_not_create_atoms`, `selected_semantic_molecule_atom_on_surface_and_semantic`, `circuit_on_surface` | `theorem` | pure Atom-AAT core が Signature に依存せず、selected law が atom existence を生成せず、selected semantic molecule 内 atom が pure surface 上で selected かつ semantic/interpretation atom として読め、selected obstruction circuit が pure surface 上で閉じることを読む。 | `proved` |
...
| `ArchitectureSignature.AtomAxiomatizedAAT`, `ofPureAtomZeroCurvature`, `ofPureAtomArrangementLaws`, `ofPureAtomStaticArrangementPackage`, `ofPureTheoremSuite`, `ofPureTheoremSuiteArrangementLaws`, `ofPureTheoremSuiteStaticArrangementPackage` | `structure` / `def` | `AATPureTheorySurface` と `AtomDerivedZeroCurvaturePackage` を束ね、selected law / required molecule / required obstruction circuit が pure atom surface 上にあることを記録する atom 公理系 package。pure Atom-AAT core と atom-only zero-curvature package、pure theorem suite、または full static atom arrangement law package から構成する。 | `defined only` |
| `ArchitectureSignature.AtomAxiomatizedAAT.independent_of_observation`, `independent_of_sft`, `requiredMolecule_on_surface`, `atom_of_requiredMolecule`, `requiredMolecule_supportedBy_surface_atoms`, `requiredMolecule_atom_is_primitive`, `requiredCircuit_on_surface`, `atomZeroCurvature`, `atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries` | `theorem` | atom 公理系 package から observation / SFT 非依存、surface 上の required molecule / circuit 選択、required molecule の selected atom universe 閉包、atom zero curvature、atom lawfulness、static lawfulness、required Signature axes zero、zero-curvature theorem package、matrix diagnostics を取り出す。 | `proved` |
...
| `AtomAxiomatizedPureOperationPackage`, `AtomAxiomatizedPureOperationPackage.preservesSurfaceInvariant`, `ops_mem_surfaceInvariant`, `independent_of_architecture_signature` | `structure` / `theorem` | Signature 以前の pure atom operation theorem package。selected pure atom surface の operation preservation を Signature に依存せず読む。 | `defined only` / `proved` |
| `AtomAxiomatizedOperationPackage`, `pureOperationPackage`, `AtomAxiomatizedOperationPackage.preservesSurfaceInvariant`, `ops_mem_surfaceInvariant`, `architectureZeroCurvatureTheoremPackage` | `structure` / `def` / `theorem` | atom 公理系 AAT と surface-preserving atom operation を束ね、Signature-free operation package と atom-derived zero curvature theorem package を同じ入口から取り出す。 | `defined only` / `proved` |
...
Files: `Formal/Arch/Atomization.lean`,
`Formal/Arch/Signature/AtomZeroCurvature.lean`,
`Formal/Arch/Examples/AtomicExamples.lean`,
`Formal/Arch/Examples/AtomZeroCurvatureExamples.lean`
...
| `AtomPredicate`, `AtomPredicate.kind`, `AtomPredicate.axis` | `inductive` / `def` | primitive atom の machine-readable predicate grammar。component / relation / capability / data state / effect / boundary authority / contract specification / semantic interpretation / runtime interaction を constructor として持ち、人間向け `predicate : String` だけに primitive fact を閉じない。 | `defined only` |
| `AtomAxiomSystem`, `AATFromAtomAxioms` | `structure` / `abbrev` | pure AAT が出発する Atom 公理系 root。`Atom : Type` と typed predicate interpretation、kind / axis alignment、`singleFact`, `predicatePreserving`, `boundaryIndependent`, `lawIndependent`、observation boundary / law / tool output / SFT event が atom を生成しない boundary を持つ。`AATFromAtomAxioms` は later AAT core が observation/presentation record ではなく Atom 公理系から始まることを読む root alias。 | `defined only` |
| `AtomAxiomSystem.primitive`, `predicate_kind`, `predicate_axis`, `single_fact`, `predicate_preserving`, `boundary_independent`, `law_independent`, `observation_boundary_does_not_create_atoms`, `law_does_not_create_atoms`, `tool_output_does_not_create_atoms`, `sft_event_does_not_create_atoms`, `AATFromAtomAxioms.begins_from_atom_axioms`, `independent_of_observation_boundary`, `independent_of_law` | `theorem` | Atom 公理系が与える atom は primitive typed fact であり、predicate / kind / axis alignment を持ち、boundary / law / tool output / SFT event から生成されないことを root accessor theorem として取り出す。 | `proved` |
| `AAT.Molecule`, `AAT.MoleculeSubset`, `AAT.ProperSubmolecule`, `AAT.RoleInterpretation`, `AAT.PatternInterpretation` | `structure` / `def` | `system : AtomAxiomSystem` 上の finite atom configuration と inclusion / proper inclusion、role / pattern を primitive atom ではなく molecule interpretation として読む pure AAT surface。 | `defined only` |
| `AAT.Molecule.atom_primitive`, `atom_boundary_independent`, `atom_law_independent`, `AAT.MoleculeSubset.refl`, `trans`, `AAT.RoleInterpretation.toMolecule`, `AAT.PatternInterpretation.toMolecule`, `does_not_create_atoms` | `theorem` / `def` | pure molecule 内 atom が root system の primitive atom であり、observation boundary / law から atom existence が生成されないこと、role / pattern interpretation が underlying molecule へ忘却されることを読む。 | `proved` / `defined only` |
| `AAT.DesignLaw`, `AAT.ObstructionCircuit`, `AAT.FiniteMoleculeUniverse`, `AAT.LawfulWithinMoleculeConfiguration`, `AAT.NoRequiredObstructionCircuit`, `AAT.LawfulnessBridge` | `structure` / `def` | `AtomAxiomSystem` 上の molecule を評価する design law、law-relative minimal bad molecule、selected finite molecule universe、lawfulness / no required circuit bridge。 | `defined only` |
| `AAT.DesignLaw.does_not_create_atoms`, `atom_law_independent`, `AAT.obstructionCircuit_bad`, `obstructionCircuit_minimal`, `AAT.FiniteMoleculeUniverse.contains_minimal_bad`, `bad_iff_contains_obstruction_circuit`, `AAT.LawfulnessBridge.lawful_iff_no_obstructionCircuit` | `theorem` | design law が atom existence を生成せず、obstruction circuit が law-relative minimal bad molecule であり、selected finite universe / bridge assumptions の下で badness と no required circuit を接続する。 | `proved` |
| `AAT.PureTheory`, `AAT.PureTheory.atom_of_selected_molecule`, `independent_of_observation`, `laws_do_not_create_atoms`, `circuit_on_surface` | `structure` / `theorem` | Atom 公理系から直接立ち上がる pure AAT surface。molecule / law / obstruction circuit を `ArchitectureAtom C E D` wrapper ではなく root `AtomAxiomSystem` 上に置く。 | `defined only` / `proved` |
| `AAT.AATCore`, `AAT.LawfulWithinAATCore`, `AAT.ZeroCurvature`, `AAT.ZeroCurvaturePackage` | `abbrev` / `structure` / `def` | `AtomAxiomSystem` 起点の pure AAT core と、その core 上の selected law / required molecule / coverage-exactness bridge / no required obstruction circuit を束ねる zero-curvature theorem package。 | `defined only` |
| `AAT.zeroCurvature_iff_noRequiredObstructionCircuit`, `AAT.ZeroCurvaturePackage.noRequiredObstructionCircuit`, `lawfulWithinMoleculeConfiguration`, `lawfulWithinAATCore`, `requiredMolecule_on_core`, `requiredCircuit_on_core`, `atom_of_requiredMolecule`, `core_independent_of_observation`, `law_does_not_create_atoms` | `theorem` / `def` | Atom 公理系上の zero curvature から no required circuit、molecule-level lawfulness、core 内 lawfulness、required molecule / required circuit の core selection、required molecule 内 atom の primitive reading、observation independence、law non-creation を取り出す。 | `proved` / `defined only` |
| `AAT.OperationPreservationPackage`, `AAT.RepairClearingPackage`, `AAT.SynthesisSoundnessPackage` | `structure` | `AATCore system` 上の operation preservation、repair clearing、synthesis soundness package。operation / repair / synthesis を pure AAT core と zero-curvature package の後段 theorem package として読む。 | `defined only` |
| `AAT.OperationPreservationPackage.target_molecule`, `target_law`, `operation_does_not_create_atoms`, `AAT.RepairClearingPackage.zeroCurvaturePackage`, `target_noRequiredObstructionCircuit`, `target_lawfulWithinMoleculeConfiguration`, `repair_does_not_create_atoms`, `AAT.SynthesisSoundnessPackage.zeroCurvaturePackage`, `candidate_noRequiredObstructionCircuit`, `candidate_lawfulWithinMoleculeConfiguration`, `synthesis_does_not_create_atoms` | `theorem` / `def` | operation が selected molecule / law を target core に保存すること、repair / synthesis が zero-curvature package を生成し no required circuit / lawfulness を与えること、各 package が atom existence を生成しないことを取り出す。 | `proved` / `defined only` |
| `ArchitectureSignature.AATCoreSignatureLawfulnessBridge` | `structure` | `AAT.ZeroCurvaturePackage` を消費し、`ArchitectureLawModel` の Signature lawfulness / required axes zero へ渡す後段 ArchSig analysis bridge。AAT core を定義しない。 | `defined only` |
| `ArchitectureSignature.AATCoreSignatureLawfulnessBridge.architectureLawful`, `requiredSignatureAxesZero`, `archsig_does_not_define_aat`, `archsig_does_not_create_atoms`, `unknown_rejected_unmeasured_separated`, `noRequiredObstructionCircuit` | `theorem` | AAT zero curvature から Signature lawfulness / required Signature axes zero を読み、ArchSig が AAT を定義しないこと、atom existence を生成しないこと、unknown / rejected / unmeasured と measured zero の境界を取り出す。 | `proved` |
| `Observation.AtomObservationStatus`, `Observation.RawAtomCandidate`, `ObservedAtom`, `AtomObservationGap`, `AtomPresentation`, `ArchMapObservationLayer` | `inductive` / `structure` | raw candidate、observed / validated / rejected / uncertain / missing、presentation、ArchMap observation layer を pure AAT core の外で分離する observation namespace。 | `defined only` |
| `Observation.RawAtomCandidate.does_not_create_atoms`, `ObservedAtom.observation_does_not_create_atoms`, `ObservedAtom.atom_primitive`, `AtomObservationGap.gap_does_not_imply_atom_absence`, `AtomPresentation.raw_candidate_is_not_atom_truth`, `rejected_is_not_measured_zero`, `uncertain_is_not_measured_zero`, `missing_is_not_atom_absence`, `ArchMapObservationLayer.observes_atoms`, `archmap_does_not_create_atoms`, `archmap_does_not_define_aat`, `raw_candidate_is_not_atom_truth`, `missing_is_not_atom_absence` | `theorem` | raw candidate は atom truth ではなく、observation は atom を生成せず、missing observation は atom absence ではなく、ArchMap は AAT を定義しないことを accessor として読む。 | `proved` |
| `AtomAxiomSystemPureAAT` | `abbrev` | `AAT.PureTheory` を `Formal/Arch/AtomCoreAAT.lean` 側から読む alias。後続 migration が完了するまで、既存 compatibility bridge と新 pure source-of-truth surface の境界を明示する。 | `defined only` |
| `AtomAxiomSystemZeroCurvaturePackage` | `abbrev` | `AAT.ZeroCurvaturePackage` を `Formal/Arch/AtomCoreAAT.lean` 側から読む alias。旧 `AtomZeroCurvatureTheoremPackage` の migration boundary を明示する。 | `defined only` |
| `AtomAxiomSystemOperationPackage`, `AtomAxiomSystemRepairClearingPackage`, `AtomAxiomSystemSynthesisSoundnessPackage` | `abbrev` | `AAT.OperationPreservationPackage`, `AAT.RepairClearingPackage`, `AAT.SynthesisSoundnessPackage` を `Formal/Arch/AtomPureAAT.lean` 側から読む alias。旧 central suite migration の境界を明示する。 | `defined only` |
...
| `AtomKind.isPrimitive`, `primitiveArchitectureAtom_constructive`, `ArchitectureAtom.typedPredicate_kind`, `ArchitectureAtom.typedPredicate_axis`, `boundaryIndependent_of_primitive`, `lawIndependent_of_primitive`, `singleFact_of_primitive`, `predicatePreservation_of_primitive` | `theorem` | Atom Core の `AtomKind` は primitive architecture fact として扱われ、typed predicate と atom kind / axis の alignment、および primitive atom criteria を accessor theorem として取り出せる。 | `proved` |
| `OntologicalAtom`, `PrimitiveOntologicalAtom`, `AtomOntology`, `AtomObservationBoundary` | `structure` / `def` | support / evidence boundary / non-conclusion を持たない first-class atom ontology と、互換 wrapper 側に置く observation boundary を分離する。`AtomOntology` は AAT が前提にする atom universe を observation / law / tool output / SFT forecast 非依存の axiom-system surface として読む。 | `defined only` |
| `primitiveOntologicalAtom_constructive`, `OntologicalAtom.typedPredicate_kind`, `typedPredicate_axis`, `boundaryIndependent_of_primitive`, `lawIndependent_of_primitive`, `AtomOntology.independent_of_observation`, `law_does_not_create_atoms`, `tool_output_does_not_create_atoms`, `sft_forecast_does_not_create_atoms`, `selected_atom_is_primitive` | `theorem` | pure ontology atom の primitive criteria と、Atom ontology が observation / law / tool output / SFT forecast から atom を生成しない boundary を accessor theorem として読む。 | `proved` |
| `ArchitectureAtom.ontology`, `ArchitectureAtom.observationBoundary`, `ArchitectureAtom.ontology_primitive`, `ontology_independent_of_observation_boundary`, `ontology_independent_of_law` | `def` / `theorem` | 既存 `ArchitectureAtom` を互換 wrapper として読み、boundary-free ontology atom と presentation/evidence boundary を API 上で分離する。 | `defined only` / `proved` |
...
| `ArchitectureAtom.HasSemanticMeaning`, `isSemanticInterpretation_of_hasSemanticMeaning`, `semantic_axis_of_hasSemanticMeaning` | `def` / `theorem` | constructor-level の semantic meaning payload を持つ atom が semantic/interpretation atom であり、semantic axis 上の atom として読めることを取り出す。 | `defined only` / `proved` |
...
| `SemanticAtomMolecule`, `SemanticAtomMoleculeWitness`, `SemanticAtomMolecule.atom_is_semantic`, `SemanticAtomMoleculeWitness.atom_selected`, `atom_is_semantic`, `atom_selected_and_semantic` | `def` / `structure` / `theorem` | semantic/interpretation atoms だけからなる atom molecule と、その selected universe support witness を表す。semantic molecule 内 atom が selected atom universe に属し、semantic/interpretation atom であることを取り出す。 | `defined only` / `proved` |
...
| `AtomLawSeparation`, `AtomLawSeparation.atoms_exist_before_law`, `law_does_not_create_atoms`, `law_does_not_change_atom_existence` | `structure` / `theorem` | DesignLaw は既存 AtomMolecule を評価し、Atom Core を生成せず、Atom existence を変更しないことを明示する boundary package。 | `defined only` / `proved` |
...
| `SemanticAtomArrangementLaw`, `SemanticObstructionCandidate`, `semantic_molecule`, `atom_is_semantic`, `bad`, `no_candidate_of_lawful` | `structure` / `theorem` | required molecule が semantic atoms だけからなることを law arrangement として記録し、semantic mismatch / conflict を primitive atom ではなく law-relative obstruction candidate として読む。lawful な semantic arrangement では semantic obstruction candidate が存在できない。 | `defined only` / `proved` |
| `AATPureTheorySurface`, `AATPureTheorySurface.selectedAtomUniverse`, `atomOntology`, `atomOntologySurface`, `SemanticMoleculeOnSurface`, `independent_of_observation`, `independent_of_sft`, `atom_of_selected_molecule`, `selected_molecule_supportedBy_selected_atoms`, `semantic_atom_of_selected_semantic_molecule`, `selected_atom_reads_ontology`, `selected_atom_ontology_is_primitive` | `structure` / `def` / `theorem` | Atom / Molecule / DesignLaw / ObstructionCircuit / pattern interpretation boundary からなる existing pure AAT surface。互換 API を維持しつつ、selected `ArchitectureAtom` wrapper が first-class `AtomOntology` の atom を読むことを accessor theorem として公開する。 | `defined only` / `proved` |
| `AATAtomOntologySurface`, `AATAtomOntologySurface.selected_atom_reads_ontology`, `selected_atom_ontology_is_primitive`, `independent_of_observation`, `independent_of_sft`, `atom_of_selected_molecule`, `selected_molecule_atoms_read_ontology` | `structure` / `theorem` | AAT が `AtomOntology` を直接読む successor surface。selected wrapper / molecule は ontology atom へ閉じ、observation / SFT forecast は定義条件ではない。 | `defined only` / `proved` |
| `AtomAxiomatizedPureAAT` | `structure` | `AATPureTheorySurface` に law-separation、selected law/molecule circuit closure、ArchitectureSignature 非依存 witness を加え、AAT を Signature 以前の Atom Core 公理系として読む pure entrypoint。 | `defined only` |
| `AtomAxiomatizedPureAAT.independent_of_observation`, `independent_of_sft`, `independent_of_architecture_signature`, `selected_atom_is_primitive`, `selected_molecule_atom_on_surface`, `selected_molecule_supportedBy_surface_atoms`, `selected_molecule_atom_is_primitive`, `selected_semantic_molecule_atom_on_surface_and_semantic`, `selected_law_does_not_create_atoms`, `selected_law_does_not_change_atom_existence`, `selected_law_atoms_exist_before_law`, `circuit_on_surface` | `theorem` | pure Atom-AAT core が observation / SFT / ArchitectureSignature に依存せず、selected molecules は selected atoms だけで構成され、その atom は primitive であり、selected semantic molecule 内 atom は semantic/interpretation atom として読める。selected laws は atom existence を生成・変更せず、selected obstruction circuits は pure surface 上で閉じることを accessor theorem として読む。 | `proved` |
| `AtomZeroCurvature`, `AtomZeroCurvatureTheoremPackage` | `def` / `structure` | selected required obstruction circuit が存在しないことを pure Atom-AAT core 上の atom-level zero curvature として束ねる theorem package。Signature-level zero-curvature package はこれを消費する後段の解釈として読める。 | `defined only` |
| `atomZeroCurvature_iff_noRequiredObstructionCircuit`, `AtomZeroCurvatureTheoremPackage.noRequiredObstructionCircuit`, `atomLawful`, `requiredMolecule_on_surface`, `atom_of_requiredMolecule`, `requiredMolecule_supportedBy_surface_atoms`, `requiredMolecule_atom_is_primitive`, `requiredCircuit_on_surface`, `law_does_not_create_atoms`, `law_does_not_change_atom_existence`, `law_atoms_exist_before_law` | `theorem` | atom-level zero curvature から no required circuit、atom lawfulness、required molecule の selected atom closure / primitive atom reading、surface circuit selection、law-separation 境界を取り出す。 | `proved` |
...
| `AATCoreLocalAlgebraForSFT`, `AATCoreAtomDelta`, `AATCoreSemanticDelta`, `AATCoreCircuitDelta`, `AATCoreTransition` | `structure` | SFT が `AtomAxiomSystem` 上の `AATCore` を local algebra として読み、atom / semantic / circuit delta と operation transition を new core に接続する API。 | `defined only` |
| `AATCoreLocalAlgebraForSFT.reads_aatcore_as_local_algebra`, `sft_does_not_redefine_atoms`, `sft_does_not_redefine_aat`, `aatcore_alone_does_not_prove_forecast_correctness`, `core_independent_of_observation`, `sft_event_does_not_create_atoms`, `AATCoreAtomDelta.source_atom_primitive`, `target_atom_primitive`, `preserved_atom_on_source_and_target`, `delta_does_not_create_atoms`, `AATCoreSemanticDelta.source_semantic_atom_primitive`, `target_semantic_atom_primitive`, `semantic_delta_does_not_create_atoms`, `AATCoreCircuitDelta.source_molecule_atom_primitive`, `target_molecule_atom_primitive`, `law_does_not_create_atoms`, `circuit_delta_does_not_create_atoms`, `AATCoreTransition.operation_does_not_create_atoms`, `atom_delta_does_not_create_atoms`, `semantic_delta_does_not_create_atoms`, `circuit_delta_does_not_create_atoms` | `theorem` | SFT transition bridge が `AATCore` を局所代数として読むだけで Atom / AAT を再定義せず、各 delta / transition が atom existence を生成しないことを accessor として読む。 | `proved` |
| `ObservationStatus`, `ObservedAtom`, `ObservedAtom.observedOntology`, `ObservationGap`, `ObservationGap.AtomExistenceClaim`, `AtomPresentation` | `inductive` / `structure` / `def` | observed atom / rejected / uncertain / private unavailable / observation gap を atom existence から分離する presentation layer。`ObservedAtom` は ontology atom を読むが生成しない。`ObservationGap.AtomExistenceClaim` は gap から atom existence を結論しない guardrail。 | `defined only` |
| `rejectedCandidate_not_supportsMeasurement`, `uncertainCandidate_not_supportsMeasurement`, `ObservedAtom.observes_boundary_free_ontology`, `observed_ontology_law_independent`, `observedAtom_rejected_not_measured`, `observedAtom_uncertain_not_measured`, `observationGap_not_measuredZero`, `ObservationGap.no_atom_existence_claim` | `theorem` | rejected / uncertain candidate と observation gap は measured-zero atom evidence ではなく、observation metadata は ontology atom の存在条件ではない。 | `proved` |
...
| `LayeringAtomArrangementLaw`, `ProjectionAtomArrangementLaw`, `ObservationAtomArrangementLaw`, `LSPAtomArrangementLaw`, `EdgePolicyAtomArrangementLaw` | `structure` | `StrictLayered`、`ProjectionSound`、`ObservationallyEquivalent`、`LSPCompatible`、edge-policy soundness を AtomMolecule / DesignLaw 上の selected lawfulness から読む theorem package。 | `defined only` |
| `LayeringAtomArrangementLaw.strictLayered_of_lawful`, `acyclic_of_lawful`, `ProjectionAtomArrangementLaw.projectionSound_of_lawful`, `noProjectionObstruction_of_lawful`, `ObservationAtomArrangementLaw.observationallyEquivalent_of_lawful`, `LSPAtomArrangementLaw.lspCompatible_of_lawful`, `noLSPObstruction_of_lawful`, `EdgePolicyAtomArrangementLaw.policySound_of_lawful`, `noPolicyViolation_of_lawful` | `theorem` | 既存 AAT invariant の主要 subset と required static law components を atom arrangement law の lawfulness から復元する。 | `proved` |
| `ArchitectureSignature.StaticAtomArrangementLawPackage`, `strictLayered_of_lawful`, `projectionSound_of_lawful`, `lspCompatible_of_lawful`, `boundaryPolicySound_of_lawful`, `abstractionPolicySound_of_lawful`, `architectureLawful_of_lawful` | `structure` / `theorem` | current required static law components の 5 つの atom arrangement law readings を一つに束ね、atom-configuration lawfulness から `ArchitectureLawful` を復元する。 | `defined only` / `proved` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ofAtomZeroCurvatureTheoremPackage`, `ofAtomArrangementLaws`, `ofStaticAtomArrangementPackage`, `ofPureTheoremSuite`, `ofPureTheoremSuiteArrangementLaws`, `ofPureTheoremSuiteStaticArrangementPackage` | `structure` / `def` | atom lawfulness bridge と grouped atom arrangement law package から、既存 `ArchitectureLawModel` の zero-curvature theorem package へ接続する atom-derived package。pure atom-only zero-curvature theorem package または `AtomAxiomatizedPureTheoremSuite` から構成でき、`StaticAtomArrangementLawPackage` を `staticArrangements` として直接保持する。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.staticArrangementLawPackage`, `layering`, `projection`, `lsp`, `boundaryPolicy`, `abstractionPolicy`, `lspCompatibleFromLawful`, `boundaryPolicySoundFromLawful`, `abstractionPolicySoundFromLawful`, `atomZeroCurvature`, `atomLawful`, `strictLayered`, `acyclic`, `walkAcyclic`, `projectionSound`, `noProjectionObstruction`, `lspCompatible`, `boundaryPolicySound`, `abstractionPolicySound`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `architectureZeroCurvatureTheoremPackage_of_atomZeroCurvatureTheoremPackage`, `architectureZeroCurvatureTheoremPackage_of_atomArrangementLaws`, `architectureZeroCurvatureTheoremPackage_of_staticAtomArrangementPackage`, `architectureLawful_of_pureTheoremSuite`, `requiredSignatureAxesZero_of_pureTheoremSuite`, `architectureZeroCurvatureTheoremPackage_of_pureTheoremSuite`, `architectureZeroCurvatureTheoremPackage_of_pureTheoremSuiteArrangementLaws`, `architectureZeroCurvatureTheoremPackage_of_pureTheoremSuiteStaticArrangementPackage`, `matrixDiagnosticCorollaries_of_pureTheoremSuite` | `def` / `theorem` | atom-only zero curvature / no required atom obstruction circuit / pure atom theorem suite から atom lawfulness を読み、full static atom arrangement law package の下で strict layering、walk acyclicity、projection soundness、LSP、policy soundness、static architecture lawfulness、required Signature axes zero、zero-curvature theorem package、matrix diagnostic corollaries を順に導く。 | `defined only` / `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT`, `ofPureAtomZeroCurvature`, `ofPureAtomArrangementLaws`, `ofPureAtomStaticArrangementPackage`, `ofPureTheoremSuite`, `ofPureTheoremSuiteArrangementLaws`, `ofPureTheoremSuiteStaticArrangementPackage` | `structure` / `def` | `AATPureTheorySurface` と atom-derived zero-curvature package を接続し、law / molecule / obstruction circuit が pure atom surface 上で選択されることを記録する。pure Atom-AAT core と atom-only zero-curvature package、または pure theorem suite から構成でき、full static arrangement law package を明示できる。 | `defined only` |
| `ArchitectureSignature.AtomAxiomatizedAAT.independent_of_observation`, `independent_of_sft`, `requiredMolecule_on_surface`, `atom_of_requiredMolecule`, `requiredMolecule_supportedBy_surface_atoms`, `requiredMolecule_atom_is_primitive`, `requiredCircuit_on_surface`, `atomZeroCurvature`, `atomLawful`, `strictLayered`, `acyclic`, `walkAcyclic`, `projectionSound`, `noProjectionObstruction`, `lspCompatible`, `boundaryPolicySound`, `abstractionPolicySound`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries` | `theorem` | pure atom surface の observation / SFT 非依存を保ったまま、required molecule の surface 閉包と primitive atom 性を保存し、atom 公理系 package から atom zero curvature、主要 static law components、零曲率 theorem package、matrix diagnostics まで導く。 | `proved` |
| `AtomAxiomatizedPureTheoremSuite`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedPureAAT` と `AtomZeroCurvatureTheoremPackage` 上に pure operation preservation、finite atom repair clearing、pure synthesis soundness を揃える Signature-free central theorem suite。 | `defined only` |
| `AtomAxiomatizedPureTheoremSuite.independent_of_observation`, `independent_of_sft`, `independent_of_architecture_signature`, `operation_surface_matches`, `zeroCurvature_law_does_not_create_atoms`, `zeroCurvature_law_does_not_change_atom_existence`, `zeroCurvature_law_atoms_exist_before_law`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `requiredMolecule_on_surface`, `atom_of_requiredMolecule`, `requiredMolecule_supportedBy_surface_atoms`, `requiredMolecule_atom_is_primitive`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredObstructionCircuit`, `repair_target_atomZeroCurvature`, `repair_target_atomLawful`, `repair_law_does_not_create_atoms`, `repair_law_does_not_change_atom_existence`, `repair_law_atoms_exist_before_law`, `synthesis_candidate_satisfies`, `synthesis_candidate_atomZeroCurvature`, `synthesis_candidate_atomLawful`, `synthesis_law_does_not_create_atoms`, `synthesis_law_does_not_change_atom_existence`, `synthesis_law_atoms_exist_before_law` | `theorem` | Signature を経由せず、同じ pure atom core から atom-level zero curvature、required molecule closure、operation preservation、repair-derived zero curvature / lawfulness、synthesis-derived zero curvature / lawfulness、law-separation を取り出す。 | `proved` |
| `AtomAxiomatizedTheoremSuite`, `atomZeroCurvatureTheoremPackage`, `staticArrangementLawPackage`, `pureTheoremSuite`, `pureOperationPackage`, `pureRepairPackage`, `pureRepairZeroCurvatureTheoremPackage`, `pureSynthesisPackage`, `pureSynthesisZeroCurvatureTheoremPackage`, `operationPackage`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedPureAAT` と `AtomAxiomatizedAAT` 上に atom-only zero-curvature、grouped static arrangement package、pure operation preservation、pure repair clearing、pure synthesis soundness、operation / repair / synthesis component を揃え、pure theorem suite への忘却と既存 package API への bridge を提供する。 | `defined only` |
| `AtomAxiomatizedTheoremSuite.pure_core_surface_matches`, `independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `zeroCurvature_law_does_not_change_atom_existence`, `zeroCurvature_law_atoms_exist_before_law`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `zeroCurvature_layeringArrangement`, `zeroCurvature_projectionArrangement`, `zeroCurvature_lspArrangement`, `zeroCurvature_boundaryPolicyArrangement`, `zeroCurvature_abstractionPolicyArrangement`, `zeroCurvature_architectureLawful`, `zeroCurvature_requiredSignatureAxesZero`, `zeroCurvature_strictLayered`, `zeroCurvature_walkAcyclic`, `zeroCurvature_projectionSound`, `zeroCurvature_noProjectionObstruction`, `zeroCurvature_lspCompatible`, `zeroCurvature_boundaryPolicySound`, `zeroCurvature_abstractionPolicySound`, `requiredMolecule_on_aat_surface`, `requiredMolecule_on_pureCore_surface`, `atom_of_requiredMolecule_on_aat_surface`, `atom_of_requiredMolecule_on_pureCore_surface`, `requiredMolecule_supportedBy_aat_surface_atoms`, `requiredMolecule_supportedBy_pureCore_surface_atoms`, `requiredMolecule_atom_is_primitive`, `pure_operation_preservesSurfaceInvariant`, `pure_repair_independent_of_architecture_signature`, `pure_repair_target_noRequiredObstructionCircuit`, `pure_repair_target_atomZeroCurvature`, `pure_repair_law_does_not_create_atoms`, `pure_repair_law_does_not_change_atom_existence`, `pure_repair_law_atoms_exist_before_law`, `pure_synthesis_independent_of_architecture_signature`, `pure_synthesis_candidate_satisfies`, `pure_synthesis_candidate_atomZeroCurvature`, `pure_synthesis_law_does_not_create_atoms`, `pure_synthesis_law_does_not_change_atom_existence`, `pure_synthesis_law_atoms_exist_before_law`, `independent_of_observation`, `independent_of_sft`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredObstructionCircuit`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `architectureLawful_of_synthesis`, `zeroCurvatureTheoremPackage_from_staticArrangement`, `zeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `def` / `theorem` | 同じ pure atom core から atom-level zero curvature、grouped static arrangement package、主要 static law components、required molecule の Signature-connected surface / pure core surface 閉包、Signature-free operation preservation、Signature-free repair clearing、Signature-free synthesis soundness、Signature 非依存、observation / SFT 非依存、zero-curvature law の atom-existence 非生成・非変更・先在性、repair-derived lawfulness、synthesis-derived lawfulness、Signature-level zero-curvature theorem package と matrix diagnostics を取り出す。 | `defined only` / `proved` |
| `AtomSurfacePresented`, `AtomDeltaPreservesSurface`, `AtomPresentationOperation` | `def` / `structure` | selected pure atom surface が source / target presentation 間で保存されることを、atom delta に相対化して表す Signature-free operation layer。 | `defined only` |
| `AtomPresentationOperation.target_presented_of_source`, `preservesSurfaceInvariant`, `ops_mem_surfaceInvariant` | `theorem` | atom presentation operation が surface を保存するなら、target presentation も同じ surface を観測し、通常の operation/invariant API で `PreservesInvariant` / `Ops` として読める。 | `proved` |
| `AtomAxiomatizedPureOperationPackage`, `AtomAxiomatizedPureOperationPackage.preservesSurfaceInvariant`, `ops_mem_surfaceInvariant`, `independent_of_architecture_signature` | `structure` / `theorem` | Signature に依存しない pure atom operation package から operation preservation を読む。 | `defined only` / `proved` |
| `AtomAxiomatizedOperationPackage`, `pureOperationPackage`, `AtomAxiomatizedOperationPackage.preservesSurfaceInvariant`, `ops_mem_surfaceInvariant`, `architectureZeroCurvatureTheoremPackage` | `structure` / `def` / `theorem` | atom 公理系 AAT package と surface-preserving operation を束ね、Signature-free operation package と atom-derived zero curvature theorem package を同じ入口から取り出す。 | `defined only` / `proved` |
| `AtomRepairWitness`, `AtomRepairUniverse`, `AtomFiniteRepairPackage`, `AtomRepairTransitionPackage`, `AtomAxiomatizedPureRepairPackage`, `AtomAxiomatizedRepairPackage` | `structure` / `def` | atom obstruction circuit を repair witness として通常の selected repair universe に渡し、finite repair clearing から pure atom zero curvature / target lawfulness / Signature zero-curvature theorem package へ接続する。 | `defined only` |
| `AtomRepairWitness.bad`, `AtomRepairUniverse.repairStepDecreases_of_admissible`, `AtomFiniteRepairPackage.target_noRequiredObstructionCircuit`, `target_lawful_of_bridge`, `AtomRepairTransitionPackage.source_not_lawful_of_circuitBad`, `target_lawful_of_bridge`, `AtomAxiomatizedPureRepairPackage.target_atomZeroCurvature`, `atomZeroCurvatureTheoremPackage`, `independent_of_architecture_signature`, `AtomAxiomatizedRepairPackage.architectureLawful_of_repair`, `requiredSignatureAxesZero_of_repair`, `architectureZeroCurvatureTheoremPackage_of_repair` | `theorem` / `def` | law-relative atom obstruction circuit を repair witness として読み、selected finite repair clearing から Signature-free atom zero curvature と target lawfulness、さらに Signature-level zero curvature を導く。 | `proved` / `defined only` |
| `AtomAATSynthesisConstraint`, `AtomAATSynthesisSpec`, `AtomAxiomatizedPureSynthesisPackage`, `AtomAxiomatizedSynthesisPackage`, `AtomConstraintContradiction` | `inductive` / `structure` / `def` | atom-generated synthesis constraints、pure candidate soundness、Signature-level synthesis bridge、atom contradiction no-solution certificate を既存 `SynthesisConstraintSystem` / `NoSolutionCertificate` へ接続する。 | `defined only` |
| `AtomAxiomatizedPureSynthesisPackage.candidate_atomZeroCurvature`, `atomZeroCurvatureTheoremPackage`, `independent_of_architecture_signature`, `AtomAxiomatizedSynthesisPackage.candidate_satisfies`, `candidate_noRequiredObstructionCircuit`, `architectureLawful_of_synthesis`, `requiredSignatureAxesZero_of_synthesis`, `architectureZeroCurvatureTheoremPackage_of_synthesis`, `AtomConstraintContradiction.noArchitectureSatisfies`, `noArchitectureSatisfies_of_valid_certificate` | `theorem` / `def` | atom constraint candidate から Signature-free atom zero curvature と synthesis soundness、Signature-level zero-curvature theorem package を導き、atom contradiction から valid no-solution certificate soundness を導く。 | `proved` / `defined only` |
| `AtomAxiomatizedPureTheoremSuite`, `AtomAxiomatizedTheoremSuite.pureTheoremSuite`, `AtomAxiomatizedTheoremSuite.staticArrangementLawPackage`, `AtomAxiomatizedTheoremSuite.independent_of_architecture_signature`, `zeroCurvature_law_does_not_create_atoms`, `atomZeroCurvature`, `atomLawful_from_atomZeroCurvature`, `zeroCurvature_architectureLawful`, `zeroCurvature_requiredSignatureAxesZero`, `operation_preservesSurfaceInvariant`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `zeroCurvatureTheoremPackage_from_staticArrangement`, `zeroCurvatureTheoremPackage`, `matrixDiagnosticCorollaries`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `structure` / `def` / `theorem` | operation / repair / synthesis を同じ pure atom core に揃える Signature-free central suite と、その suite が保持する static arrangement package を既存 atom 公理系 AAT / Signature theorem package / matrix diagnostics へ接続する bridge。 | `defined only` / `proved` |
...
| `ArchitectureAtom` | `structure` | 既存 API 互換の atom wrapper。kind、axis、support、typedPredicate、predicate label、evidence boundary、non-conclusion を保持するが、pure theory object は `ArchitectureAtom.ontology` として分離される。`support` / `evidenceBoundary` / `nonConclusions` は ontology existence condition ではなく observation/presentation metadata として読む。 | `defined only` |
...
| `AtomicExamples.semanticInterpretationAtom_has_meaning`, `semanticInterpretationAtom_is_semantic_from_meaning`, `semanticInterpretationAtom_axis_from_meaning`, `semanticMolecule_all_semantic`, `semanticAtomMoleculeWitness_selected_and_semantic`, `semanticAATSurface_semantic_atom_on_surface`, `semanticPureAtomCore_semantic_atom_on_surface` | `theorem` | semantic/interpretation atom の meaning payload、semantic molecule witness、pure AAT surface / pure Atom-AAT core 上で semantic atom が selected かつ semantic atom として読めることを確認する smoke tests。 | `proved` |
| `AtomicExamples.componentAtom_ontology_primitive`, `componentAtom_ontology_boundary_independent`, `componentAtom_observation_boundary_separated`, `pureAATSurface_reads_component_ontology`, `pureAATSurface_component_ontology_primitive`, `pureAATOntologySurface_independent_of_observation`, `pureAATOntologySurface_molecule_atom_reads_ontology`, `rejectedPrimitiveCandidate_observes_ontology_not_existence`, `runtimeObservationGap_no_atom_existence_claim`, `primitiveComponentAtom_primitive`, `primitiveRelationAtom_primitive`, `contractSpecificationAtom_allowedBy_current`, `contractSpecificationAtom_primitive_of_policy`, `semanticInterpretationAtom_allowedBy_current`, `semanticInterpretationAtom_primitive_of_policy`, `exampleSemanticDelta_created_is_semantic`, `repositoryRoleAssignment_reads_molecule`, `simpleLayeringPattern_reads_molecule`, `singletonForbiddenMolecule_obstruction`, `selectedForbiddenEdgeUniverse_contains_minimal_bad`, `forbiddenEdgeLaw_does_not_create_atoms`, `forbiddenEdge_lawful_iff_no_required_circuit`, `pureAATSurface_independent_of_observation`, `pureAATSurface_independent_of_sft`, `pureAtomCore_independent_of_observation`, `pureAtomCore_independent_of_sft`, `pureAtomCore_independent_of_architecture_signature`, `pureAtomCore_forbiddenLaw_does_not_create_atoms`, `pureAtomCore_forbiddenCircuit_on_surface`, `noEdgePureAATSurface_selected_molecule_supported`, `noEdgePureAtomCore_independent_of_architecture_signature`, `noEdgePureAtomCore_noBadLaw_does_not_create_atoms`, `noEdgePureAtomCore_noBadLaw_atoms_exist_before_law`, `noEdgePureAtomCore_componentMolecule_atom_is_primitive`, `noEdgePureAtomZeroCurvature`, `noEdgePureAtomZeroCurvature_atomLawful`, `noEdgePureAtomZeroCurvature_law_does_not_create_atoms`, `noEdgePureAtomZeroCurvature_law_atoms_exist_before_law`, `noEdgePureAtomSuite_requiredMolecule_supported`, `noEdgePureAtomSuite_requiredMolecule_atom_is_primitive`, `rejectedPrimitiveCandidate_not_measured`, `uncertainPrimitiveCandidate_not_measured`, `runtimeObservationGap_not_measuredZero`, `exampleAtomPresentation_recordsPromotionBoundary`, `exampleAtomPresentationAATPackage_reads_selected_atom`, `exampleAtomPresentationAATPackage_records_raw_guardrail`, `noEdgeLayeringAtomArrangement_strictLayered`, `noEdgeLayeringAtomArrangement_acyclic`, `identityProjectionAtomArrangement_projectionSound`, `identityProjectionAtomArrangement_noProjectionObstruction`, `unitObservationAtomArrangement_equivalent`, `apiResponsibilityRole_coherent`, `selectedApiResponsibility_coherent`, `apiSRPAtomArrangement_boundaryFunctional`, `apiSRPAtomArrangement_localCohesion`, `noEdgeAtomDerived_from_pure_atom_zero_curvature`, `noEdgeAtomDerived_atomZeroCurvature`, `noEdgeAtomDerived_architectureLawful`, `noEdgeAtomDerived_matrixDiagnosticCorollaries`, `noEdgeAtomDerived_requiredSignatureAxesZero`, `noEdgeAtomDerived_zeroCurvatureTheoremPackage`, `noEdgeAtomAxiomatized_independent_of_observation`, `noEdgeAtomAxiomatized_independent_of_sft`, `noEdgeAtomAxiomatized_atomZeroCurvature`, `noEdgeAtomAxiomatized_zeroCurvatureTheoremPackage`, `noEdgeAtomAxiomatized_matrixDiagnosticCorollaries`, `noEdgeFullAtomSurfacePresented`, `noEdgeAtomOperation_target_presented`, `noEdgeAtomOperation_preservesSurfaceInvariant`, `noEdgeAtomOperation_ops_mem_surfaceInvariant`, `noEdgePureAtomOperation_preservesSurfaceInvariant`, `noEdgePureAtomOperation_independent_of_signature`, `noEdgeAtomAxiomatizedOperation_zeroCurvature`, `forbiddenEdgeAtomRepair_source_has_circuit`, `forbiddenEdgeAtomRepair_source_not_lawful`, `forbiddenEdgeAtomRepair_target_noRequiredCircuit`, `forbiddenEdgeAtomRepair_target_lawful`, `noEdgeAtomRepair_target_atomLawful`, `noEdgeAtomRepair_architectureLawful`, `noEdgeAtomRepair_requiredSignatureAxesZero`, `noEdgeAtomRepair_zeroCurvatureTheoremPackage`, `noEdgeAtomSynthesis_candidate_satisfies`, `noEdgeAtomSynthesis_candidate_satisfies_via_package`, `noEdgeAtomSynthesis_candidate_noRequiredCircuit`, `noEdgeAtomSynthesis_architectureLawful`, `noEdgeAtomSynthesis_requiredSignatureAxesZero`, `noEdgeAtomSynthesis_zeroCurvatureTheoremPackage`, `relationAtomContradiction_noArchitecture`, `relationAtomNoSolution_from_valid_certificate`, `noEdgeAtomSuite_independent_of_architecture_signature`, `noEdgeAtomSuite_zeroCurvatureLaw_does_not_create_atoms`, `noEdgeAtomSuite_zeroCurvatureLaw_does_not_change_atom_existence`, `noEdgeAtomSuite_zeroCurvatureLaw_atoms_exist_before_law`, `noEdgeAtomSuite_atomZeroCurvature`, `noEdgeAtomSuite_atomLawful_from_atomZeroCurvature`, `noEdgeAtomSuite_requiredMolecule_supported_on_aat_surface`, `noEdgeAtomSuite_requiredMolecule_supported_on_pureCore_surface`, `noEdgeAtomSuite_requiredMolecule_atom_is_primitive`, `noEdgeAtomSuite_pure_operation_preservesSurfaceInvariant`, `noEdgeAtomSuite_pure_repair_law_atoms_exist_before_law`, `noEdgeAtomSuite_pure_synthesis_law_atoms_exist_before_law`, `noEdgeAtomSuite_matrixDiagnosticCorollaries`, `noEdgePureAtomSuiteTheoremStatus_records_theoremPackage`, `exampleAtomTraceForecastBoundary_length_le_horizon`, `exampleAtomTraceForecastBoundary_governed_or_typedFailure`, `staticSignatureZero_no_static_bad_atom`, `exampleAtomVanishingBridge_no_required_bad_atom`, `example_validatedPresentation_excludes_raw_candidates`, `example_atomicSFTPresentation_excludes_raw_candidates`, `example_atomicSFTPresentation_records_no_forecast_correctness`, `AtomFoundationExamples.exampleAtomAxiomSystem_atom_primitive`, `exampleAAT_begins_from_atom_axioms`, `exampleAtomAxiomSystem_boundary_independent`, `exampleAtomAxiomSystem_law_independent`, `exampleAtomAxiomSystem_observation_boundary_does_not_create_atoms`, `exampleAtomAxiomSystem_tool_output_does_not_create_atoms` | `theorem` | Atom ontology / compatibility wrapper / grammar policy / contract-vs-semantic split / semantic delta / finite molecule witness / role and pattern separation / law separation / pure Atom-AAT ontology surface / selected molecule closure / required molecule closure / atom-only zero curvature / circuit / lawfulness / observation / AAT package / atom arrangement law / atom 公理系 zero curvature / atom operation preservation / atom repair bridge / atom synthesis bridge / pure Atom suite の SFT theorem-status reading / SRP molecule / forecast boundary / vanishing / presentation bridge smoke tests。`AtomFoundationExamples.*` は `AtomAxiomSystem` root が support / evidence / observation status を持たず、AAT が Atom 公理系から始まることを確認する。 | `proved` |
| `AATMoleculeLawExamples.apiMolecule_atom_primitive`, `singletonBadLaw_does_not_create_atoms`, `singletonBadLaw_obstruction`, `examplePureTheory_independent_of_observation`, `examplePureTheory_laws_do_not_create_atoms`, `examplePureTheory_circuit_on_surface` | `theorem` | `AtomAxiomSystem` 上の pure molecule / design law / obstruction circuit / pure theory surface が `ArchitectureAtom` wrapper なしで build され、law が atom existence を生成しないことを確認する smoke tests。 | `proved` |
| `AATZeroCurvatureExamples.noBadZeroCurvature`, `noBadPackage_noRequiredObstructionCircuit`, `noBadPackage_lawfulWithinMoleculeConfiguration`, `noBadPackage_lawfulWithinAATCore`, `noBadPackage_requiredMolecule_on_core`, `noBadPackage_atom_of_requiredMolecule`, `noBadPackage_core_independent_of_observation`, `noBadPackage_law_does_not_create_atoms` | `theorem` | `AtomAxiomSystem` 起点の pure AAT core と zero-curvature package が、observation / Signature / forecast 前提なしに no required circuit、lawfulness、required molecule closure、law non-creation を出せることを確認する smoke tests。 | `proved` |
| `AATOperationRepairSynthesisExamples.identityOperation_target_molecule`, `identityOperation_target_law`, `identityOperation_does_not_create_atoms`, `noBadRepair_target_noRequiredObstructionCircuit`, `noBadRepair_target_lawful`, `noBadRepair_does_not_create_atoms`, `noBadSynthesis_candidate_noRequiredObstructionCircuit`, `noBadSynthesis_candidate_lawful`, `noBadSynthesis_does_not_create_atoms` | `theorem` | `AATCore` 上の operation / repair / synthesis package が selected molecule / law preservation、no required circuit、lawfulness、atom non-creation を出せることを確認する smoke tests。 | `proved` |
| `AtomSFTInterfaceExamples.exampleAATCoreLocalAlgebra_reads_core`, `exampleAATCoreLocalAlgebra_no_forecast_correctness`, `exampleAATCoreTransition_operation_does_not_create_atoms`, `exampleAATCoreTransition_atom_delta_does_not_create_atoms`, `exampleAATCorePremisedEnvelope_records_boundaries`, `exampleAATCorePremisedEnvelope_no_forecast_correctness`, `exampleFieldSig_reads_archsig_transition_as_sft_analysis`, `exampleFieldSig_forecast_correctness_remains_boundary` | `theorem` | `AATCore` local algebra、transition delta、ConsequenceEnvelope、FieldSig transition analysis の smoke tests。SFT / FieldSig は AATCore transition を読むが forecast correctness を結論しない。 | `proved` |
| `ArchMapObservationExamples.rawCandidate_does_not_create_atoms`, `observedAtom_is_primitive`, `gap_does_not_imply_atom_absence`, `presentation_raw_candidate_is_not_atom_truth`, `presentation_missing_is_not_atom_absence`, `archMap_observes_atoms`, `archMap_does_not_create_atoms`, `archMap_does_not_define_aat` | `theorem` | observation layer が raw candidate / observed atom / gap / presentation / ArchMap を分離し、ArchMap が atom を生成せず AAT を定義しないことを確認する smoke tests。 | `proved` |
| `AATCoreSmokeExamples.atom_axiom_system_root_primitive`, `aat_begins_from_atom_axioms`, `molecule_law_circuit_rooted_in_atom_axioms`, `zero_curvature_package_rooted_in_aatcore`, `operation_repair_synthesis_do_not_create_atoms`, `archmap_observation_layer_is_outside_pure_core`, `sft_fieldsig_reads_aatcore_transition_without_forecast_correctness` | `theorem` | Atom 公理 root、molecule / law / circuit、zero curvature、operation / repair / synthesis、ArchMap observation separation、SFT / FieldSig transition bridge を横断する new AATCore smoke tests。 | `proved` |
| `AtomicExamples.noEdgeAtomDerived_strictLayered`, `noEdgeAtomDerived_walkAcyclic`, `noEdgeAtomDerived_projectionSound`, `noEdgeAtomDerived_noProjectionObstruction`, `noEdgeAtomDerived_lspCompatible`, `noEdgeAtomDerived_boundaryPolicySound`, `noEdgeAtomDerived_abstractionPolicySound`, `noEdgeAtomAxiomatized_strictLayered`, `noEdgeAtomAxiomatized_projectionSound`, `noEdgeAtomAxiomatized_lspCompatible`, `noEdgeAtomSuite_zeroCurvature_strictLayered`, `noEdgeAtomSuite_zeroCurvature_walkAcyclic`, `noEdgeAtomSuite_zeroCurvature_projectionSound`, `noEdgeAtomSuite_zeroCurvature_lspCompatible`, `noEdgeAtomSuite_zeroCurvature_boundaryPolicySound`, `noEdgeAtomSuite_zeroCurvature_abstractionPolicySound` | `theorem` | atom-derived zero-curvature package / atom-axiomatized AAT / central theorem suite から主要 static law components を直接読む smoke tests。 | `proved` |
