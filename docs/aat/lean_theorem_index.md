...
| `AATTheoremFamily`, `AATFieldImplementationStatus`, `AATImplementationFrontier`, `AtomGeneratedAATWorld`, `AATTheoremSuite`, `AATCompleteFormalization`, `allAATTheoremFamilies`, `currentImplementationFrontier`, `initialTheoremSuite`, `initialCompleteFormalization` | `inductive` / `structure` / `def` | AAT 完全形式化を並列で進めるための coordination skeleton。`AtomGeneratedAATWorld` は Atom root、shape presentation、generated object、generated law model、generated Signature 用 decidability evidence を共有 world として束ねる。`AATTheoremSuite` は最初の source-of-truth field として generated molecule non-arbitrariness、generated lawfulness、generated required Signature axes zero、generated static structural core、generated AATCore no-observation / circuit-boundary bridge fields、classification registry の source/downstream 分離を持つ。`AATImplementationFrontier` は suite field、既存 entrypoint、次 work package、parallel / coordination flag、docs target を concrete row として列挙し、generated law / Signature row は AATCore bridge まで connected として扱う。 | `defined only` |
| `AtomGeneratedAATWorld.molecule_not_arbitrary_set`, `generated_lawful`, `required_signature_axes_zero`, `static_structural_core`, `generated_aat_core_noObservationDependency`, `generated_aat_core_circuitBoundary`, `currentImplementationFrontier_covers_all_families`, `connected_frontier_rows_have_existing_entrypoints`, `parallel_frontier_rows_are_not_core_coordination`, `initialCompleteFormalization_frontier_covers_all_families`, `AATCompleteFormalizationExamples.generatedComponentTheoremSuite_has_required_axes_zero`, `generatedComponentTheoremSuite_has_static_structural_core`, `generatedComponentTheoremSuite_has_generated_aat_core_bridge_fields`, `generatedComponentTheoremSuite_registry_has_no_bridge_assumed_rows`, `generatedComponentCompleteFormalization_frontier_covers_all_families`, `generatedComponentCompleteFormalization_parallel_rows_are_safe` | `theorem` | 既存 generated component model で complete-formalization skeleton が inhabited になり、generated law / Signature static core から generated AATCore no-observation / circuit-boundary bridge fields まで suite field として読めること、current frontier が全 theorem family を覆うこと、connected row は existing entrypoint を持つこと、parallel row が core coordination row ではないことを固定する。これは AAT 完全形式化の完了宣言ではなく、suite field 単位でサブエージェントを投入するための共有 contract である。 | `proved` |
