...
| `AtomicExamples.noEdgePureAtomSuite_independent_of_observation`, `independent_of_sft`, `independent_of_architecture_signature`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredCircuit`, `repair_target_atomZeroCurvature`, `repair_target_atomLawful`, `synthesis_candidate_satisfies`, `synthesis_candidate_atomZeroCurvature`, `synthesis_candidate_atomLawful`, `zeroCurvatureLaw_does_not_create_atoms`, `repair_law_does_not_create_atoms`, `synthesis_law_does_not_create_atoms` | `theorem` | Signature を経由せず、central pure suite から observation / SFT / Signature 非依存、operation preservation、repair-derived zero curvature / lawfulness、synthesis-derived zero curvature / lawfulness、law-separation を取り出す smoke test。 | `proved` |
...
| atom-derived entrypoint | `AtomAxiomatizedPureAAT`, `AtomZeroCurvatureTheoremPackage`, `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ArchitectureSignature.AtomAxiomatizedAAT` | Signature 以前の pure Atom Core 公理系を入口にし、atom-only zero curvature から atom lawfulness、既存 static lawfulness、required Signature axes zero、zero-curvature theorem package へ接続する。 |
...
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ofAtomZeroCurvatureTheoremPackage` | `structure` / `def` | atom lawfulness bridge と atom arrangement law readings から、既存 `ArchitectureLawModel` の zero-curvature theorem package へ接続する atom-derived package。pure atom-only zero-curvature theorem package から構成できる。 | `defined only` |
| `ArchitectureSignature.AtomDerivedZeroCurvaturePackage.atomLawful`, `architectureLawful`, `requiredSignatureAxesZero`, `architectureZeroCurvatureTheoremPackage`, `architectureZeroCurvatureTheoremPackage_of_atomZeroCurvatureTheoremPackage` | `theorem` | atom-only zero curvature / no required atom obstruction circuit から atom lawfulness、static architecture lawfulness、required Signature axes zero、zero-curvature theorem package を順に導く。 | `proved` |
| `ArchitectureSignature.AtomAxiomatizedAAT`, `ofPureAtomZeroCurvature` | `structure` / `def` | `AATPureTheorySurface` と atom-derived zero-curvature package を接続し、law / molecule / obstruction circuit が pure atom surface 上で選択されることを記録する。pure Atom-AAT core と atom-only zero-curvature package から構成できる。 | `defined only` |
