...
| atom-derived entrypoint | `ArchitectureSignature.AtomDerivedZeroCurvaturePackage`, `ArchitectureSignature.AtomAxiomatizedAAT` | no required atom obstruction circuit から atom lawfulness、既存 static lawfulness、required Signature axes zero、zero-curvature theorem package へ接続し、その bridge を pure atom surface 上の AAT 公理系 package として束ねる。 |
...
| atom theorem suite | `AtomAxiomatizedTheoremSuite` | 同じ pure atom surface / selected law / required molecule を使って zero-curvature、operation preservation、repair clearing、synthesis soundness を束ねる central Atom-AAT entrypoint。 |
...
| `AtomAxiomatizedTheoremSuite`, `operationPackage`, `repairPackage`, `synthesisPackage` | `structure` / `def` | 一つの `AtomAxiomatizedAAT` 上に operation / repair / synthesis component を揃え、既存 package API へ戻せる central theorem suite。 | `defined only` |
| `AtomAxiomatizedTheoremSuite.independent_of_observation`, `independent_of_sft`, `operation_preservesSurfaceInvariant`, `operation_ops_mem_surfaceInvariant`, `repair_target_noRequiredObstructionCircuit`, `architectureLawful_of_repair`, `synthesis_candidate_satisfies`, `architectureLawful_of_synthesis`, `zeroCurvatureTheoremPackage`, `zeroCurvatureTheoremPackage_of_repair`, `zeroCurvatureTheoremPackage_of_synthesis` | `theorem` | 同じ atom foundation から observation / SFT 非依存、operation preservation、repair-derived lawfulness、synthesis-derived lawfulness、zero-curvature theorem package を取り出す。 | `proved` |
