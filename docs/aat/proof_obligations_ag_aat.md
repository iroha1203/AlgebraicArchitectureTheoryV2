...
AC 再判定: PRD-R R0 では AC8 / AC19 を `packaged (assumption-relative)` として読む。
根拠宣言と再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| PRD-7 | AC19 | AAT synthesis theorem is package-relative and lacks finite model firing. | `packaged (assumption-relative)` | `Formal/AG/RepresentationAnalysis/Synthesis.lean`, `AATSynthesisPackage`; 詳細は VII-1。 |
