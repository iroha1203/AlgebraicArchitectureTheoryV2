...
| `III.例5.6E / 例7.2D / R13 finite examples` | `proved examples`. `Formal/AG/LawAlgebra/FiniteExamples.lean` が Stanley-Reisner chart、`I_Ob = <xy>`、faces/nonfaces、`1 = x - (x - 1)` concrete certificate から生成される `NSdepth = 1` profile、cycle witness coordinate `c_ABC` と `I_cycle = <c_ABC>`、acyclic / cyclic pullback lawful locus の点あり/なし、定理11.1 の acyclic / cyclic finite package、R4 III-1 の `acyclicLocalSection_lawful_from_witnessIdeals`、R4 III-2 の `acyclic_sectionPrimeMap_mem_localLawfulLocus` を持つ。 | examples は selected finite surfaces であり、Nullstellensatz 7.2A の一般証明、全 finite graph の分類、または set-theoretic image containment から ideal vanishing への逆向き一般 theorem は主張しない。 |
