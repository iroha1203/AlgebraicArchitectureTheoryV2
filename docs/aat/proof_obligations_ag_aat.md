...
| `X.例9.1` Lawful Firing Instance | `proved lawful firing instance`. `Formal/AG/Examples/SemanticRepairPart10.lean` が singleton quotient presheaf の `quotientIsSheaf` を theorem として放電し、`lawfulObject_noCycleLaw`、`defectSource.DisplayedRequiredLawsHoldOn`、`trueSheafCertificate`、`h1Comparison` から `lawfulFiring_endToEndPacket` と `lawfulFiring_generatedSourceC0Zero` を構成する。 | 例9.1 は lawful zero firing witness であり、circle nerve 非零 class / transfer packet は例9.2 / AC17 に残す。一般 quotient sheaf comparison は explicit data なしに構成しない。 |
