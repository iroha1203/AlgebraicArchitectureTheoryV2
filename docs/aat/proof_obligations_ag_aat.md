...
AC 再判定: PRD-R R0 では AC13 を `packaged (assumption-relative)`、
AC20 を `packaged (assumption-relative)` / `statement-only` として読む。
根拠宣言と再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| PRD-9 | AC13 | theorem 4.2 Temporal Descent Criterion is selected temporal package relative. | `packaged (assumption-relative)` | `Formal/AG/Evolution/ReplayDescent.lean`, `TemporalDescentCriterion`; 詳細は IX-1。 |
