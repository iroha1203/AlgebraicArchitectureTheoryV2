...
| `X.例9.1` Lawful Firing Instance | `proved lawful firing instance`. `Formal/AG/Examples/SemanticRepairPart10.lean` が singleton quotient presheaf の `quotientIsSheaf` を theorem として放電し、`lawfulObject_noCycleLaw`、`defectSource.DisplayedRequiredLawsHoldOn`、`trueSheafCertificate`、`h1Comparison` から `lawfulFiring_endToEndPacket` と `lawfulFiring_generatedSourceC0Zero` を構成する。`h1Realization` と `lawfulFiring_endToEndPacket_fromRealization` は comparison を cochain realization から生成する theorem-7.5 route を発火する。 | 例9.1 は lawful zero firing witness であり、circle nerve 非零 class / transfer packet は例9.2 の別 witness で扱う。一般 quotient sheaf comparison は explicit data なしに構成しない。 |
