...
| --- | --- | --- | --- | --- |
| PRD-1 | AC11 | 定理9.3 後段の三読み一致は explicit package field 由来の theorem package。 | `packaged (assumption-relative)` | `Formal/AG/Atom/ThreeReading.lean`, `ThreeReadingAgreementAssumptions.*`; 詳細は `lean_ag_peer_review_inventory.md` I-1。 |
| PRD-1 | AC13 | 定理10.5 AAT Core は selected components を singleton tower に束ねる theorem package。 | `packaged (assumption-relative)` | `Formal/AG/Atom/AATCore.lean`, `AATCorePackage.*`; 詳細は I-2。 |
