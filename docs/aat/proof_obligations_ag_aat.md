...
PRD-R R0 の査読耐性棚卸しは
[`lean_ag_peer_review_inventory.md`](lean_ag_peer_review_inventory.md) に置く。
この台帳の theorem 系 status は、同台帳の `proved` /
`packaged (assumption-relative)` / `statement-only` 三分化語彙で正規化して読む。
既存の限定句は証拠として残すが、`proved accessor`、explicit assumption package、
certificate field 由来の theorem package は `packaged (assumption-relative)`、
`statement-only candidate` は `statement-only` に分類する。
`defined only`、future proof obligation、explicit non-goal、boundary note は
三分化対象外である。
個別行の R0 再分類はこの台帳本文を重複更新せず、同 inventory の review layer 表と
...
| `I.定理9.3 後段` 三読み一致 | `proved` / `packaged (assumption-relative)`. `Formal/AG/Atom/ThreeReading.lean` が `requiredLawWitnessFamily`, `requiredLawSignatureAxes`, `semanticLawful_iff_noRequiredObstruction_requiredLawWitness`, `semanticLawful_iff_requiredSignatureAxesZero_requiredLawAxes`, `noRequiredObstruction_iff_requiredSignatureAxesZero_requiredLaw`, `concreteThreeReadingAgreement` を持つ。旧 surface として `ThreeReadingAgreementAssumptions` と accessor theorem も残る。 | PRD-R R2 で required-law index から canonical witness family / signature axes を作り、三読み一致を `Lawfulness` の実述語から証明した。旧 package field 版は Research 互換の packaged surface。 |
...
| `I.定理10.5` AAT Core theorem package | `proved` / `packaged (assumption-relative)`. `Formal/AG/Atom/AATCore.lean` が `AATCorePackage`, `AATCorePackage.AtomTowerRealization`, `AtomTowerRealization.realized_configuration_eq`, `AtomTowerRealization.realized_family_eq`, `AATCorePackage.ofComponents`, `AATCorePackage.ofAxiomRealization`, `AATCorePackage.exists_ofComponents`, `AATCorePackage.exists_ofComponents_noHEq`, `AATCorePackage.exists_ofAxiomRealization_noHEq`, configuration-family / object-configuration 整合性 theorem、各 component accessor theorem を持つ。 | PRD-R R2 で `AtomAxiomSystem.configurationOf` から Part I tower を読む realization bridge と HEq-free existence theorem を追加した。旧 `exists_ofComponents` の dependent obstruction equality は Research 互換の packaged surface。concrete graph / category / algebra / diagram / state transition instance は後続 Issue に残る。 |
| R10 finite model / 例8.3 / 例8.4 / 定理9.3 example theorem | `defined only` / `proved`. `Formal/AG/Examples/FiniteModel.lean` が `FiniteAtom`, `carrier`, `axiomSystem`, `finite_atom_exists`, `allFamily_listFinite`, `acyclicConfiguration`, `acyclicObject`, `hasCycleWitness`, `cycleObstructionCircuit`, `cycleObstructionCircuit_listFinite`, `substitutionObstructionCircuit`, `substitutionObstructionCircuit_listFinite`, `concreteNoCycleWitnessFamily`, `concreteNoCycleSignatureAxes`, `acyclic_concreteThreeReadingAgreement`, `object_concreteThreeReadingAgreement_fires`, `noCycleValuation`, `finite_lawfulness_iff_omega_zero`, `atomTowerRealization`, `corePackageFromAxiomRealization`, `corePackageFromAxiomRealization_exists_noHEq`, `corePackage` を持つ。 | finite model は component / depends / contract / substitution atoms を持つ selected example universe として扱う。PRD-R R2 で required-law witness family の非退化発火、acyclic/cyclic の三読み一致、explicit finite support、AAT Core realization bridge の finite firing を追加した。第II部以降の concrete graph / category / algebra / diagram / state transition instance は後続 Issue に残る。 |
...
| `II.定義3.1 / §5.1-5.4` Architecture Context と射 role | `defined only` / `proved`. `Formal/AG/Site/Context.lean` が `MinimalContext`, `ArchitectureContext`, `ContextMorphism`、support / axis / observable / non-generation に相対化された restriction / projection / refinement / base change concrete role predicate と accessor theorem を持つ。R3 II-3 で `MinimalContext.supportReads_objectFamily` により context support reading を `A.configuration.family` へ接続した。 | 全 runtime / source observation context の完全列挙は主張しない。 |
| `II.定義4.1 / 命題4.2 / 仮定4.3` Context Category と meet-pullback | `defined only` / `proved` / `packaged (assumption-relative)`. `Formal/AG/Site/ContextCategory.lean` が readable morphism 付き preorder-category package、restriction-morphism canonical preorder、componentwise product meet、readable equivalence quotient、quotient object 上の finite-meet poset construction、pullback / overlap package、pullback lifting property、meet-overlap 補題を持つ。 | R3 II-2 で `ContextPreorderCategory + ContextFiniteMeet` から quotient finite-meet poset を構成する theorem を追加した。旧 supplied package theorem は compatibility surface として残る。 |
...
| `II.定義7.1後半 / R4` `J_U` と Mathlib bridge | `defined only` / `proved`. `Formal/AG/Site/Topology.lean` が context preorder の Mathlib thin category wrapper、admissible cover generated precoverage、`AATGrothendieckTopology`、identity/top cover、base-change stability、transitivity、admissible generated cover membership、明示 pullback / base-change 前提下の `Coverage.toGrothendieck` bridge theorem を持つ。R3 II-6 で `Formal/AG/Examples/FiniteModel.lean` が singleton finite equality-thin site の `siteAdmissiblePrecoverage_hasPullbacks` / `siteAdmissiblePrecoverage_stableUnderBaseChange` と `siteTopology_eq_coverage_toGrothendieck` を追加し、bridge theorem を selected concrete instance 上で発火させる。 | `Coverage.toGrothendieck` bridge は明示 pullback / base-change stability assumptions または selected finite singleton instance に相対化される。任意 precoverage の無条件 stability は主張しない。 |
| `II.定義7.2 / 補題7.2A` U-adequate cover と witness-closure | `defined only` / `proved` / `packaged (assumption-relative)`. `Formal/AG/Site/Adequate.lean` が U-adequate cover、selected reading 上の witness ideal predicate とその restriction 保存、legacy witness-closure cover construction package、補題7.2A `witnessClosureCover_uAdequate`、および R3 seed-driven counterpart `SeedWitnessClosureCover.toAATCoverageFamily_admissible` / `SeedWitnessClosureCover.uAdequate` を持つ。 | R3 counterpart は support / axis coverage を seed 側に限定し、required witness support と seed overlap で closed cover を生成する。旧 `WitnessClosureCover` は Research-compatible packaged surface として残る。 |
...
| `II.定義7.2B / 命題7.2C` 標準有限 poset Čech route | `defined only` / `proved` / `packaged (assumption-relative)`. R3 II-4 で `Formal/AG/Site/FinitePosetGeometry.lean` と `Formal/AG/Cohomology/FinitePosetStandardComplex.lean` の PRD-10 R1 route を II-4 の evidence として接続した。`FinitePosetCoverGeometry`、canonical tuple simplex / nerve source / simplicial face action、obstruction coefficient regime、`standardAdditiveData`、`standardDifferential`、double-face cancellation law、`standardFinitePosetCechComplex`、`canonicalTupleStandardFinitePosetCechComplex`、および `*_differential_from_restrictions` / `*_differential_comp` / `*_cohomology_vanishes_above_nerveDimension` theorem 群により、旧 selected complex signature は canonical route で充足できる compatibility surface として読む。`standardFinitePosetCechComplex` は `StandardDifferentialCompLaw` を受け取る packaged surface であり、canonical tuple route がその law を double-face cancellation から生成する。 | canonical tuple route は標準 face action と `d ∘ d = 0` を生成するが、任意 cover の無条件 finite-dimensional nerve bound は主張しない。次元消滅は明示 `FinitePosetNerveDimension` 仮定の下で標準 complex 上に証明される。 |
...
| `II.R11 finite model examples` | `defined only` / `proved`. `Formal/AG/Examples/FiniteModel.lean` が PRD-1 finite model 上の singleton site、concrete context role firing、restriction preorder quotient finite-meet poset firing、singleton admissible cover、legacy witness-closure adequate cover、seed-driven witness-closure admissible / adequate cover、selected finite poset regime、top cover membership、degree 0 nerve dimension、positive degree Čech vanishing theorem を持つ。R3 II-5 で non-singleton two-patch finite site (`twoPatchSite`) を追加し、two-patch adequate cover、selected overlap-to-patch morphisms、unit sheaf descent、selected sheafification gap、one-dimensional selected nerve、nonzero Boolean Čech differential calculation を発火例として持つ。R3 II-6 で singleton finite equality-thin site の `siteAdmissiblePrecoverage_hasPullbacks`、`siteAdmissiblePrecoverage_stableUnderBaseChange`、`siteTopology_eq_coverage_toGrothendieck` を発火例として持つ。 | finite examples は selected finite fixture であり、任意 precoverage の無条件 base-change stability、任意 finite site の sheafification、一般 sheaf cohomology 計算は主張しない。 |
...
| `II.定義7.1 Coverage.toGrothendieck bridge` | `proved under explicit pullback / base-change stability assumptions`; R3 II-6 で selected singleton finite equality-thin site 上の concrete firing theorem を追加 |
| `II.補題7.2A` | `proved` / `packaged (assumption-relative)` |
...
AC 再判定: PRD-R R0 では AC5 / AC11 を `packaged (assumption-relative)` として読む。
根拠宣言と再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| `III.定義2.1 / 条件4.5` Law Algebra Sheaf と Presentation Stability | `defined only` / `proved`. `Formal/AG/LawAlgebra/StructureSheaf.lean` が `AATCommAlgCat`, `AlgebraValuedAATPresheaf`, `LawAlgebraSheaf`, `RawAmbientAlgebraPresheafBridge`, `LawAlgebraSheafificationBridge`, `LawAlgebraSheafificationBridge.mathlib_sheafification_lift_unique`, `LawAlgebraSheafificationBridge.ofMathlibSheafification`, `PresentationStableAATSite`, `LawAlgebraSheafPackage` と accessor theorem を持つ。raw ambient restriction と presheaf map の自然性、canonical map の sheafification universal property、selected generator / relation の canonical preservation を明示する。R4 III-3 では明示 `HasSheafify S.topology (AATCommAlgCat k)` 仮定下で Mathlib `toSheafify` / `sheafifyLift` / `sheafifyLift_unique` surface から selected bridge を構成する theorem を追加した。 | `O_X^U = (O_raw^U)^+` は selected sheafification bridge として扱う。`AATCommAlgCat` の無条件 `HasSheafify` instance や一般 site の algebra-valued sheafification 理論は新規証明しない。scheme-level atlas は R11 で別 package として追加済み。 |
...
| `III.定義7.1 / 定義7.2` Lawful Locus / Lawful Section | `defined only` / `proved bridge`. `Formal/AG/LawAlgebra/LawfulLocus.lean` が `lawfulLocus`, `localLawfulLocus`, `LawfulSectionData`, `pulledObstructionIdeal`, `sectionPrimeMap`, `Lawful`, `FactorsThroughLawfulLocus`, `lawful_iff_pulledObstructionIdeal_eq_bot`, `sectionPrimeMap_mem_lawfulLocus_of_le_ker`, `sectionPrimeMap_mem_lawfulLocus_of_pulledObstructionIdeal_eq_bot`, `sectionPrimeMap_mem_lawfulLocus_of_lawful`, `factorsThroughLawfulLocus_of_lawful`, `lawful_of_factorsThroughLawfulLocus`, `lawful_iff_factorsThroughLawfulLocus`, `LocalLawfulSectionData`, `localSectionPrimeMap`, `localSectionPrimeMap_mem_localLawfulLocus_of_lawful` を持つ。 | R4 III-2 では `PrimeSpectrum.comap` による section prime map と、`I_Ob ≤ ker(s^*)` / `s^* I_Ob = 0` から induced image が zeroLocus / local lawful locus に入る片方向 bridge を追加済み。legacy factorization package は compatibility surface として残す。逆向きの set-theoretic image containment から ideal vanishing への一般同値は radical gap のため主張しない。scheme-level gluing surface は R11 で追加済み。 |
| `III.定理候補7.2A / 定義7.2B / 命題7.2C` Architecture Nullstellensatz / NSdepth | `future proof obligation` / `defined only` / `proved`. `Formal/AG/LawAlgebra/Nullstellensatz.lean` が `booleanLawIdeal`, `VanishesAtIdeal`, `BooleanLawZeroSetEmpty`, `ArchitectureNullstellensatzCandidate`, `RadicalUnitCertificate`, `UnlawfulnessCertificate`, `HasCertificateAt`, `NSdepthProfile`, `NSdepth`, `GeneratorExtension`, `NSdepth_mono_of_generatorExtension`, `GeneratorUnitCertificate`, `HasSelectedGeneratorUnitCertificateAt`, `GeneratorNSdepthProfile`, `GeneratorNSdepthProfile.toNSdepthProfile`, `GeneratorNSdepthProfile.NSdepth_eq_depth`, `GeneratorNSdepthProfile.GeneratorCertificateExtension`, `GeneratorNSdepthProfile.NSdepth_mono_of_generatorCertificateExtension` を持つ。定理候補7.2A は Lean 上では statement-only `Prop` surface としてコンパイルされ、証明済み theorem へは昇格していない。R4 III-5 では NSdepth が selected generator family の finite linear-combination display degree から生成される surface に接続された。 | 定理候補7.2A の証明は future proof obligation として残す。Nullstellensatz 証明昇格、radical algorithm completeness、closed-field 一般定理は別作業。scheme / gluing surface は R11 で追加済み。 |
...
| `III.定義9.1 / 定義9.2 / 定義9.3 / 定義10.3` Ringed AAT Topos / Architecture Scheme / Scheme Gluing | `defined only` / `proved for forgetful bridge and single-affine compatibility`. `Formal/AG/LawAlgebra/Scheme.lean` が `RingedAATTopos`, `ChartCompatibility`, `ArchitectureScheme`, `SchemeGluingData`, `AffineReturnConditions`、既存 `Site.AATSh S` / `LawAlgebraSheafPackage S k` への接続、Mathlib `LocallyRingedSpace` forgetful bridge theorem、`affineChartMathlibSpecLocallyRingedSpace`、`ChartCompatibility.selfAffineSpec`、`ArchitectureScheme.singleAffineSpec`、`ArchitectureScheme.singleAffineSpec_compatibility_allConditions` を持つ。 | R4 III-4 は selected affine chart algebra の Mathlib `Spec` locally ringed space reading と単一 chart identity compatibility を実質化する。decorated gluing の一般論、Mathlib open immersion theorem、任意 atlas construction は主張しない。単一 affine chart への復帰は `AffineReturnConditions` の明示仮定としても残す。 |
| `III.定理11.1` Lawfulness-Ideal Correspondence | `proved under explicit correspondence assumptions` / `proved bridge`. `Formal/AG/LawAlgebra/Correspondence.lean` が `LawfulnessIdealCorrespondenceAssumptions`, `LawfulnessIdealCorrespondencePackage`, `lawful_iff_pulledObstructionIdeal_eq_bot`, `localObstructionIdeal_le_ker_iff_lawful`, `lawful_of_selectedWitnessIdeals_le_ker`, `lawful_of_generatedLawWitnessIdeals_le_ker`, `displayedRequiredLawsHoldOn_defect_mem_localObstructionIdeal`, `pulledObstructionIdeal_eq_bot_iff_factorsThroughLawfulLocus`, `factorsThroughLawfulLocus_iff_omegaU_zero`, `omegaU_zero_iff_requiredSignatureAxesZero`, `lawfulnessIdealCorrespondence` を持つ。 | legacy package の obstruction soundness / completeness、axis exactness、witness coverage、U-adequate cover、`Ob_U` sheaf descent、ring restriction compatibility は明示仮定として扱う。R4 III-1 の bridge は selected witness ideals / local obstruction ideal / PRD-10 R5 generated law-equation defects に相対化した片方向であり、任意 correspondence 全体同値は主張しない。Nullstellensatz 7.2A の証明昇格は主張しない。 |
| `III.定義11.3` Law-equation generated coefficient core | `defined only` / `proved under explicit restriction-compatibility data`. `Formal/AG/LawAlgebra/LawEquation.lean` が `SemanticLawEquationWitnessIdealCore`, `lawWitnessIdeal`, `selectedLawWitnessIdealFamily`, `obstructionIdeal`, `lawWitnessIdeal_le_obstructionIdeal`, `map_lawWitnessIdeal_le`, `restrictionCompatible`, `map_obstructionIdeal_le`, `obstructionQuotientRestrict`, `obstructionQuotientRestrict_mk`, `obstructionQuotientCoefficient`, `quotient_mk_eq_zero_iff_mem_obstructionIdeal`, `generatedCoefficient_package` を持つ。 | PRD-10 §5 の laws-as-equations core を第III部の obstruction/lawful-locus vocabulary へ接続する。商係数 `O/I_Ob` と restriction は explicit restriction compatibility に相対化され、一般 quotient-sheaf 生成や任意 atlas への自動 gluing は主張しない。 |
| `III.定理11.4` Law-equation defect grounding | `proved theorem package construction`. `Formal/AG/LawAlgebra/LawEquation.lean` が `LawEquationDefectSource`, `interpret`, `DisplayedRequiredLawsHoldOn`, `GeneratedRestrictionEvaluator`, `displayedRequiredLawsHoldOn_constructs_interpret_eq_zero`, `displayedRequiredLawsHoldOn_constructs_restrictionEvaluator`, `interpret_eq_zero_iff_defect_mem_obstructionIdeal`, `interpret_ne_zero_of_defect_notMem_obstructionIdeal`, `lawEquation_grounding_packet` を持つ。 | displayed local law-equation defect を generated quotient coefficient の degree-zero reading として読む。zero iff defect membership / nonzero defect theorem / grounding packet は selected chart, defect source, required-law display に相対化される。 |
| `III.例5.6E / 例7.2D / R13 finite examples` | `proved examples`. `Formal/AG/LawAlgebra/FiniteExamples.lean` が Stanley-Reisner chart、`I_Ob = <xy>`、faces/nonfaces、selected generator family `{x, x - 1}` の coefficient vector `1 = x - (x - 1)`、span membership、生成系表示次数から生成される concrete `NSdepth = 1` profile、legacy `I=<x>` と `B=<x-1>` の certificate profile、cycle witness coordinate `c_ABC` と `I_cycle = <c_ABC>`、acyclic / cyclic pullback lawful locus の点あり/なし、定理11.1 の acyclic / cyclic finite package、R4 III-1 の `acyclicLocalSection_lawful_from_witnessIdeals`、R4 III-2 の `acyclic_sectionPrimeMap_mem_localLawfulLocus` を持つ。 | examples は selected finite surfaces であり、Nullstellensatz 7.2A の一般証明、全 finite graph の分類、radical algorithm completeness、または set-theoretic image containment から ideal vanishing への逆向き一般 theorem は主張しない。 |
...
| `III.定義2.1 / 条件4.5` | `proved under explicit HasSheafify, sheafification-bridge, and presentation-stability assumptions` |
...
| `III.定義7.1 / 定義7.2` | `defined only` / `proved bridge` |
...
| `III.定義9.1 / 定義9.2 / 定義9.3` | `defined only` / `proved for forgetful bridge and single-affine compatibility` |
| `III.定義10.3` | `defined only under explicit affine-return assumptions` |
| `III.定理11.1` | `proved under explicit correspondence assumptions` / `proved bridge` |
| `III.定義11.3` | `defined only` / `proved under explicit restriction-compatibility data` |
| `III.定理11.4` | `proved theorem package construction` |
| `III.R13 finite model examples` | `proved examples` |

AC 再判定: PRD-R R0 では AC12 を `statement-only`、AC18 を
`packaged (assumption-relative)` として読む。根拠宣言と再判定理由は
末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| `IV.定義2.4` Canonical Obstruction Package | `defined only` / `proved accessor`. `Def_U` は PRD-3 `SelectedLawWitnessIdealFamily.localObstructionIdeal`、`ConDef_U` は Mathlib `Ideal.Cotangent` による `I_U/I_U^2`、`PushforwardConDef` は `i_* ConDef_U` の selected carrier boundary、`DerOb_U(F,M)` は `ConDef_U` class と係数 module element を束ねる selected naive cotangent-coefficient carrier として扱う。 | `DerOb_U` は opaque placeholder ではない。一般 cotangent complex、`Ext^1`、derived category の無条件構成は主張しない。 |
...
| `IV.定義4.1` Cover-relative and conditional cohomology notation | `defined only` / `proved accessor`. `CechCocycle`、`CechCoboundarySetoidSucc`、`CechCohomologySucc`、`CoverRelativeHn`、`AdditiveCochain`、`CechCocycleSubgroup`、`coboundaryCocycle`、`CechCoboundarySubgroupSucc`、`AdditiveCechH1`、`additiveH1Class`、`additiveH1Class_eq_zero_iff`、`additiveH1Class_eq_iff_legacy_setoid`、`cohomologyClassSucc_eq_iff_additiveH1Class_eq`、`CechToSheafComparisonHypothesis`、`RefinementSystemHypothesis`、`ConditionalSpaceCohomology`、`HnX_eq_coverRelative` を持つ。R5 IV-1 では selected `d ∘ d = 0` witness を使い、degree-one coboundary image を cocycle subgroup 内の additive subgroup として固定し、additive `H^1 = Z^1/B^1` と legacy quotient surface を bridge した。 | `H^n(X, Ob_U)` は比較仮定または refinement system を持つ条件付き notation に限る。無条件 sheaf cohomology、任意 cover の計算可能性、finite-poset 比較の自動生成は構成しない。 |
...
| `IV.定理9.2` Boundary Residue theorem | `proved under explicit Boundary Residue hypotheses` / `concrete two-chart soundness added`. `Formal/AG/Cohomology/BoundaryResidue.lean` が `BoundaryHolonomyVanishes`、`TwoChartGloballyUFlat`、`TwoChartGloballyUFlat.exists_boundary_agreement`、`TwoChartGloballyUFlat.of_boundary_agreement`、`TwoChartConnectingHomomorphism.boundaryHolonomy_zero_of_twoChartBoundaryAgreement`、`TwoChartConnectingHomomorphism.boundaryHolonomy_zero_of_boundaryMismatch_zero`、`BoundaryResidueHypotheses`、`boundaryResidueSoundness`、`boundaryResidueCompleteness`、`globallyUFlat`、`boundaryResidueTheorem`、`globallyUFlat_of_boundaryHolonomy_zero`、`boundaryHolonomy_zero_of_globallyUFlat` を持つ。`AxiomAudit.boundaryResidueTwoChartBoundaryAgreementSoundness` が theorem-level soundness route を監査する。 | `TwoChartGloballyUFlat` は selected core/feature degree-zero cochain の concrete boundary agreement `d0 s = 0` として global flatness surface を読む。2チャート boundary agreement `b_U = d0 s` かつ `d0 s = 0` から `Hol_U = 0` への soundness は package field を読まずに証明する。`C_core / F` の U-flatness、boundary witness coverage、axis exactness、boundary-exact 係数、ring restriction compatibility、effective torsor/module descent、holonomy completeness、NoHigherBoundaryObstruction は明示 field として扱い、completeness data はこれらの witness を受け取る。仮定なしの global flatness criterion、holonomy completeness の package 脱却、derived Mayer-Vietoris triangle は主張しない。 |
...
| `IV.定理11.1 / 系11.2` Cohomological Flatness Criterion | `proved under explicit finite C0 adjustment data` / `proved under explicit Cohomological Flatness hypotheses`. `Formal/AG/Cohomology/FlatnessCriterion.lean` が `H1ClassVanishes`、`GluingObstructionClass`、`AdditiveH1ClassVanishes`、`additiveH1ClassVanishes_iff_exists_c0_coboundary`、`FiniteC0AdjustmentSurface`、`adjustedOverlapResidual`、`adjustedOverlapAgreement`、`adjustedOverlapAgreement_iff_c0_coboundary`、`FiniteC0AdjustedGlobalLawfulSection`、`finiteC0AdjustedGlobalLawfulSection_iff_exists_c0_coboundary`、`finiteC0AdjustedGlobalLawfulSection_iff_additiveH1ClassVanishes`、`finiteC0AdjustedGlobalLawfulSection_of_additiveH1ClassVanishes`、`additiveH1ClassVanishes_of_finiteC0AdjustedGlobalLawfulSection`、`EffectiveAbelianObstructionTorsor`、`CohomologicalFlatnessCriterionHypotheses`、`cohomologicalFlatnessCriterion`、`adjustedGlobalLawfulSection_of_h1Class_zero`、`noAdjustedGlobalLawfulSection_of_h1Class_nonzero`、`localToGlobalFlatness` を持つ。 | finite additive `H^1` surface では `[g]=0 ↔ ∃t, g=d⁰t` を証明し、finite `C⁰` adjustment `t` の concrete residual `g - d⁰t` vanishing から finite adjusted global lawful-section condition との同値を読む。既存の local flatness、abelian coefficients、effective torsor、U-adequate cover、soundness / completeness、axis exactness、witness coverage、descent、effective adjustment による一般 site 版は packaged theorem として残す。任意 cover の descent theorem、任意 lawful section restriction functor、abelianized class から non-abelian torsor の自明性復元は主張しない。 |
| `IV.定義12.1 / 定理12.2 / 系12.3 / 定理12.4 / 系12.5` Cover nerve and topological debt accounting | `proved under explicit finite-dimensional accounting data` / `proved from selected finite cochain complex and edge-support absorption certificate`. `Formal/AG/Cohomology/CoverNerve.lean` が `CoverNerve`、`ParallelEdgeComponents`、`ParallelFaceComponents`、`edge_component_selected`、`face_component_selected`、`FiniteDimensionalNerveCohomologyData`、`topologicalDebtCapacity`、`FiniteNerveCochainComplex`、`boundaryToCycles`、`H1`、`finrank_H1_add_finrank_boundary`、`topologicalDebtCapacity_fromComplex`、`ConstantCoefficientNerveReading`、`h1_equiv`、`dimSpace_eq_dimNerve`、`dimH1_eq_b1`、`ForestCoverGluingData`、`localGluingSufficiency`、`localGluingSufficiency_subsingleton`、`FiniteForestEdgeAbsorptionData`、`classAt_eq_start`、`final_no_edgeSupport`、`forestVanishing`、`forestVanishing_subsingleton`、`EulerCochainAccounting`、`eulerAccounting`、`ShapeStalkPreservingRefactor`、`chi_invariant_under_refactor` を持つ。 | cover nerve は chart / pairwise-overlap component / triple-overlap component の明示 structure であり、多重 component を許す。旧 rank-nullity lower bound、constant coefficient nerve reading、forest vanishing、Euler invariance は、有限 poset regime、有限次元 `k`-linear coefficient data、selected `k`-linear comparison、tree-induction absorption、shape/stalk-preserving refactor data に相対化する。R5 IV-5 では selected finite cochain complex `C0 -> C1 -> C2` 上で additive nerve `H^1 = ker d1 / im d0` を定義し、Mathlib rank-nullity から debt-capacity inequality を証明する。この selected complex は任意 cover から自動構成されるものではない。forest vanishing は finite edge-support absorption certificate と no-edge-support zero criterion によって selected `H^1` の各 class が `0` になる theorem としても扱う。任意 cover の無条件計算定理や一般 spectral sequence は主張しない。 |
| `IV.R10(a) / AC13` Pseudo-circle golden example | `proved finite example` / `proved nonzero circle-nerve firing`. `Formal/AG/Cohomology/FiniteExamples.lean` が `FiniteExamples.PseudoCircleGolden.Chart`、`BoundaryEdge`、`coverNerve`、`each_chart_lawful`、`h0_witness_counting_sees_no_obstruction`、`boundaryCocycle`、`boundaryCocycle_AB_nonzero`、`CoverRelativeH1NonzeroWitness`、`concreteClass_nonzero`、`no_global_lawful_section_by_localFlatnessGap`、`no_global_lawful_section_by_finiteGlobalRestrictionPointwiseSoundness`、`pseudoCircle_h0_invisible_h1_obstructed`、`PartIVCircleNonzeroH1Firing`、`h0_invisible_and_coverRelativeH1_nonzero`、`additiveResidualClass_nonzero`、`h0_invisible_and_additiveH1_nonzero` を持つ。`Formal/AG/Examples/SemanticRepairPart10.lean` が PRD-10 R9 の circle instance から `circlePartIVPseudoCircleFiring`、`circlePartIV_h0_invisible_coverRelativeH1_nonzero`、`circlePartIV_h0_invisible_additiveH1_nonzero` を構成する。 | 擬円周例は selected finite surface として扱う。H0 的 witness count は zero、finite boundary cocycle は visibly nonzero。`CoverRelativeH1NonzeroWitness` は hidden-coupling package への compatibility surface として残す一方、PRD-10 R9 の selected circle-nerve nonzero H1 instance は Part IV `PartIVCircleNonzeroH1Firing` として発火済み。R5 IV-3 では finite overlap-wise soundness から theorem 7.1 へ入る pseudo-circle theorem も追加した。legacy cover-relative class 非零と IV-1 additive H1 quotient 非零の両方を読む。ArchSig fixture JSON の検証、任意 pseudo-circle 生成、一般 cover の nonzero H1 判定、一般 descent theorem、定理9.2 両方向例、forest / cycle nerve 例は主張しない。 |
| `IV.R10(b)(c) / AC14` Boundary Residue and nerve finite examples | `proved finite examples under explicit packages` / `concrete two-chart soundness example`. `Formal/AG/Cohomology/FiniteExamples.lean` が `BoundaryResidueGolden.zero_boundaryResidue_glues`、`BoundaryResidueGolden.zero_boundaryAgreement_forces_zero_holonomy`、`nonzero_boundaryResidue_blocks_gluing`、`boundaryResidue_two_direction_example`、`NerveGolden.forestCoverNerve`、`forestCoverGluingData`、`forestCover_H1_zero`、`cycleCoverNerve`、`cycleConstantCoefficientReading`、`cycleNerve_dimH1_eq_b1` を持つ。 | Boundary Residue 両方向例は既存 `BoundaryResidueHypotheses` に相対化し、`delta(b)=0` から global U-flat、`delta(b)≠0` から global U-flat 不在を読む。zero boundary agreement 例は `b_U = d0 s` と `d0 s = 0` から package field なしで `Hol_U = 0` を証明する。forest/cycle nerve 例は AC12 の `ForestCoverGluingData` と `ConstantCoefficientNerveReading` を finite selected examples として instantiate する。任意 cover の一般計算や ArchSig fixture JSON 検証は主張しない。 |
| `IV.定義13.1 / 定理13.2 / 定義13.4` Period Stokes and holonomy accounting | `proved under explicit pairing/accounting data` / `proved finite one-simplex basis calculation`. `Formal/AG/Cohomology/PeriodStokes.lean` が `FiniteCechChainComplex`、`boundaryOp`、`boundary_comp_zero`、`CechCochainChainPairing`、`pair`、`StokesCompatiblePairing`、`cechStokes`、`ConnectingBoundaryRepresentative`、`connectingStokes`、`IntervalBasisStokes.Vertex`、`Edge`、`Cochain`、`Chain`、`d0`、`boundary0`、`pair0`、`pair1`、`finiteIntervalStokes_basis`、`finiteIntervalConnectingStokes_basis`、`ExtensionHolonomyAccounting`、`kappa_U_additive`、`kappa_U_zero` を持つ。`AxiomAudit.finiteIntervalStokesBasis` / `finiteIntervalConnectingStokesBasis` が basis 計算を監査する。 | Stokes theorem は selected chain complex、cochain-chain pairing、Stokes-compatible condition に相対化する。IV-7 追補では one-simplex basis model 上で `d0 ω(edge)=ω(right)-ω(left)`、`boundary0 γ=(-γ,γ)`、dot-product pairing を具体化し、`StokesCompatiblePairing` field なしに `<dω,γ>=<ω,∂γ>` と connecting 版を証明する。定義13.4 は `kappa_U` の加法性を defining property とする会計規約であり、定理13.2 から自動的に従う系ではない。任意 cover の自動 Stokes pairing 構成は主張しない。 |
...
| `IV.定義2.4 DerOb_U` | `defined only` / `selected naive cotangent-coefficient carrier` |
...
| `IV.定理7.1` | `proved under explicit finite overlap-wise restriction/coboundary soundness` |
...
| `IV.定理11.1 / 系11.2` | `proved under explicit finite C0 adjustment data` / `proved under explicit Cohomological Flatness hypotheses` |
...
| `IV.定義13.1 / 定理13.2 / 定義13.4` | `proved under explicit pairing/accounting data` / `proved finite one-simplex basis calculation` |
| `IV.定義14.1 / 定理候補14.2 / 定義14.3` | `statement only` / `defined only` / `proved accessor` |
| `IV.定理候補10.4 / 定理候補14.2` | `future proof obligation` |

AC 再判定: PRD-R R0 では AC3 / AC4 / AC11 / AC13 を
`packaged (assumption-relative)` として読む。根拠宣言と再判定理由は
末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| `V.定義4.1 / 5.1 / 5.2` Derived intersection and LawConflict bridge | `defined only` / `proved accessor`. `Intersection.classicalJointQuotient`、`mathlibTor`、`mathlibTorZeroIsoTensor`、`classicalJointLinearEquivTensor`、`classicalJointLinearEquivMathlibTorZero`、`canonicalSelectedTorBridge`、`canonicalSelectedTorBridgeLawConflict0AlgEquivClassicalJoint`、`canonicalSelectedTorBridgeLawConflict0LinearEquivMathlibTor`、`SelectedDerivedTensorComplex`、`SelectedDerivedTensorComplex.C0`、`homology0`、`homology0LinearEquivClassicalJoint`、`ChartDerivedIntersection`、`ChartDerivedIntersection.classicalQuotient`、`structureSheafComplex`、`homology0LinearEquivClassicalJoint`、`ChartDerivedIntersection.toCanonicalLawConflictPackage`、`SelectedTorBridge`、`SelectedTorBridge.torLinearEquivMathlib`、`SelectedTorBridge.LawConflict`、`LawConflict₁`、`lawConflict_eq_tor`、`lawConflictLinearEquivMathlibTor`、`lawConflict0AlgEquivClassicalJoint`、`lawConflict0RingEquivClassicalJoint`、`LawConflictPackage`、`LawConflictPackage.LawConflict`、`LawConflictPackage.LawConflict₁`、`LawConflictPackage.lawConflict_eq_tor`、`LawConflictPackage.lawConflictLinearEquivMathlibTor`、`LawConflictPackage.lawConflict0AlgEquivClassicalJoint`、`LawConflictPackage.lawConflict0RingEquivClassicalJoint`、`SelectedGlobalLawConflictReading`、`SelectedGlobalLawConflictReading.H0`、`H0LawConflict₁`、`h0LinearEquivSelectedLawConflict`、`hypercohomology`、`LawfulLoci.LawUniversePair.DerivedIntersectionFor`、`LawfulLoci.LawUniversePair.derivedIntersection` を持つ。R6 V-2 では Mathlib `Tor_0` zero-th left-derived/tensor 同型と quotient tensor 同型から canonical Tor0 bridge を構成し、`AxiomAudit.canonicalSelectedTorBridgeLawConflict0AlgEquivClassicalJoint` / `canonicalSelectedTorBridgeLawConflict0LinearEquivMathlibTor` で標準公理範囲を固定する。 | selected complex / bridge / global notation data に相対化しており、Cech-to-sheaf comparison、global cohomology 計算、一般 Tor calculator は主張しない。 |
| `V.R4 / 定義5.4 / 命題5.5` Monomial conflict calculation | `proved under explicit Mathlib projective-resolution / Taylor identification data` / `concrete two-generator regular-pair complex added`. `FreeResolution.SelectedFiniteFreeResolution`、`SelectedFiniteFreeResolution.termLinearEquivFree_certificate`、`exact_certificate`、`exact_at_succ_holds`、`augmentation_exact_holds`、`SelectedTensorComplex`、`SelectedTensorComplex.homology`、`tensorOfResolution_certificate`、`FiniteFreeResolutionTorComputation`、`mathlibTorLinearEquivTensorHomology`、`tensorHomologyLinearEquivMathlibTor`、`FreeResolution.MathlibResolution.quotientModule`、`tensorAppliedComplex`、`torIsoProjectiveResolutionHomology`、`FiniteFreeMathlibResolution`、`FiniteFreeMathlibResolution.termIsoFree_certificate`、`torIsoTensorHomology`、`tensorHomologyIsoTor`、`TaylorResolution.SquareFreeMonomialIdealPresentation`、`generatedIdeal_eq`、`squareFree_certificate`、`MonomialLawConflictRegime`、`lcmSupport`、`lcmSupport_eq_union`、`left_forbiddenSupport_subset_lcmSupport`、`right_forbiddenSupport_subset_lcmSupport`、`mem_lcmSupport_iff`、`TaylorComplex`、`TaylorComplex.resolvesQuotient_certificate`、`d_comp_d_certificate`、`multidegree_union_eq`、`multidegree_pair_eq_union`、`TwoGeneratorPresentation`、`TwoGeneratorPresentation.leftGenerator`、`TwoGeneratorPresentation.rightGenerator`、`TwoGeneratorPresentation.face_card_le_two`、`TwoGeneratorPresentation.selected_pair_multidegree_eq_union`、`TwoGeneratorPrincipalTaylor.mulLeft`、`d₂`、`d₁`、`d₁_apply`、`d₂_apply`、`PrincipalSyzygyExact`、`IsRegularPair`、`spanPair`、`idealSpanPair`、`spanPair_eq_idealSpanPair`、`d₁_comp_d₂_eq_zero`、`principalSyzygyExact_of_isRegularPair`、`exact_d₂_d₁`、`exact_d₁_mkQ_range`、`range_d₁_eq_spanPair`、`quotientLinearEquivSpanPair`、`quotientLinearEquivIdealSpanPair`、`exact_visible_complex`、`exact_visible_complex_of_isRegularPair`、`AxiomAudit.twoGeneratorPrincipalTaylorExactVisible`、`AxiomAudit.twoGeneratorPrincipalTaylorExactVisibleOfRegularPair`、`TaylorSelectedFreeResolutionData`、`TaylorSelectedFreeResolutionData.termLinearEquivSelectedResolution`、`differentialCompatible_certificate`、`resolvesQuotient_from_selectedResolutionBridge`、`resolvesQuotient_from_selectedResolution`、`TwoGeneratorTaylorResolutionPackage`、`TwoGeneratorTaylorResolutionPackage.taylor`、`TwoGeneratorTaylorResolutionPackage.face_card_le_two`、`TwoGeneratorTaylorResolutionPackage.selected_pair_multidegree_eq_union`、`TwoGeneratorTaylorResolutionPackage.resolvesQuotient_from_selectedExactBridge`、`TwoGeneratorTaylorResolutionPackage.resolvesQuotient_from_selectedExact`、`TaylorMathlibResolution`、`TaylorMathlibResolution.identifiesTaylorTerms_certificate`、`identifiesTaylorDifferentials_certificate`、`taylorResolution_certificate`、`mathlibResolutionTorIsoTensorHomology`、`DirectTaylorResolutionPackage`、`DirectTaylorResolutionPackage.selectedTaylor_resolvesQuotient_from_exact`、`mathlibTaylor_resolvesQuotient_from_selectedExact`、`taylorResolution_theorem`、`differential_square_zero`、`MonomialConflictCalculation`、`computesLawConflict_certificate`、`lawConflictLinearEquivTensorHomology`、`lcm_multidegree_eq_union`、`Proposition55TheoremPackage`、`Proposition55DirectTheoremPackage`、`ComputedTorAndLcmSupport`、`torComputationAndLcmSupport` を持つ。 | `SelectedFiniteFreeResolution.exact` は Mathlib `Function.Exact (d 0) augmentation ∧ ∀ n, Function.Exact (d n.succ) (d n)` そのものであり、augmentation exactness と正次数 exactness を accessor theorem として読む。Mathlib `Tor` は second tensor factor を left-deriveするため、selected right Taylor package に付随する Mathlib finite-free resolution から `Tor_i(A/I_U,A/I_V)` を計算する。Taylor complex が quotient resolution であることは、selected finite-free resolution に付随する selected bridge `resolvesQuotient_from_selectedResolutionBridge` と `DirectTaylorResolutionPackage.mathlibTaylor_resolvesQuotient_from_selectedExact` / `TwoGeneratorTaylorResolutionPackage.resolvesQuotient_from_selectedExactBridge` で読む assumption-relative theorem surface、および `TaylorMathlibResolution` の Mathlib projective-resolution identification data に相対化する。2生成 presentation では任意 Taylor face の cardinality が 2 以下であり、選択された2生成 face の multidegree は forbidden support union と一致する。V-3 追補では concrete pair `(a,b)` に対し `d₂(r)=(-b*r,a*r)`、`d₁(p,q)=a*p+b*q` を実構成し、`d₁ ∘ d₂ = 0`、regular pair 条件から syzygy exactness、`Function.Exact d₂ d₁`、cokernel exactness、`coker(d₁) ≃ₗ[A] A/(a,b)` を証明する。`k[x,y]` の `(X,Y)` など concrete nondegenerate `IsRegularPair` instance、Scarf complex、lcm-lattice resolution、minimality、concrete Tor nonzero examples、transversality theorem までは主張しない。 |
| `V.定理6.1 / 定義7.1-7.4 / 定理7.3` Derived transversality | `proved under explicit selected transversality criterion data`. `Transversality.LawConflictVanishes`、`PositiveLawConflictVanishing`、`LawConflictNonzero`、`FirstLawConflictNonzero`、`DerivedNonTransverse`、`MathlibTorVanishes`、`PositiveMathlibTorVanishing`、`Transversality.LawConflictPackage.derivedNonTransverse_of_firstLawConflictNonzero`、`not_positiveLawConflictVanishing_of_firstLawConflictNonzero`、`lawConflictVanishes_iff_mathlibTorVanishes`、`positiveLawConflictVanishing_iff_mathlibTorVanishing`、`not_positiveMathlibTorVanishing_of_firstLawConflictNonzero`、`DerivedTransversalityCriterion`、`SelectedClassicalAgreementData`、`SelectedDerivedTensorClassicalAgreement`、`SelectedDerivedTensorClassicalAgreement.iff_positiveMathlibTorVanishing`、`iff_positiveLawConflictVanishing`、`of_positiveLawConflictVanishing`、`positiveLawConflictVanishing`、`of_positiveMathlibTorVanishing`、`toDerivedTransversalityCriterion`、`toSelectedClassicalAgreementData`、`SelectedClassicalAgreementData.toDerivedTransversalityCriterion`、`classicalAgreement_of_positiveLawConflictVanishing`、`positiveLawConflictVanishing_of_classicalAgreement`、`DerivedTransversalityCriterion.criterion_positiveTorVanishing_iff_classicalAgreement`、`positiveLawConflictVanishing_iff_classicalAgreement` を持つ。 | 定理7.3 は selected `classicalAgreement` statement を、正次数 Mathlib Tor 消滅の明示 equivalence data だけでなく selected LawConflict vanishing data から構成できる criterion surface として読む。GAP2 の concrete agreement reading では selected derived tensor surface の `H0 ≃ O/(I_U+I_V)` と正次数 Mathlib Tor 消滅を束ね、LawConflict bridge を通じて同値を返す。global derived category 一般論、Cech-to-sheaf comparison、global cohomology 計算、任意係数環での concrete Tor nonzero family は主張しない。#3098 V-1 の `ZMod 2` concrete theorem は `V.命題9.2` 行で管理する。 |
...
| `V.命題9.2` Shared-witness repair counterexample | `proved concrete ZMod 2 counterexample plus legacy package-relative surface`. `Counterexample.SharedWitnessCoord`、`SharedWitnessCoord.ChartRing`、`PathRing`、`xy`、`xz`、`idealU`、`idealV`、`pathAssignment`、`sectionFamily`、`section_x`、`section_xy`、`section_xz`、`sharedWitness_fixed_to_one`、`R2`、`ZeroMod2`、`QV2`、`QU2`、`dXZ`、`principalComplexV2`、`principalProjectiveResolutionV2`、`tensorComplexV2`、`yTensorCycleRepresentative`、`y_not_mem_idealU_zmod2`、`yTensorCycleRepresentative_ne_zero`、`yTensorCycleRepresentative_is_cycle`、`yTensorConcreteHomologyAt1_ne_zero`、`yTensorHomologyAt1_ne_zero`、`example56ZMod2Tor1Class`、`example56ZMod2Tor1Class_ne_zero`、`example56_zmod2_mathlibTor1_nonzero`、`ResidueEndpointPath`、`ResidueEndpointPath.sharedWitness`、`sharedWitness_uStart`、`sharedWitness_uEnd`、`sharedWitness_vStart`、`sharedWitness_vEnd`、`UImproves`、`VNonIncreasing`、`VWorsens`、`sharedWitness_UImproves`、`sharedWitness_VWorsens`、`sharedWitness_not_VNonIncreasing`、`sharedWitness_UImproves_and_not_VNonIncreasing`、`SharedWitnessTorNonzeroCertificate`、`SharedWitnessPrincipalResolutionCalculation`、`SharedWitnessPrincipalKernelQuotientCalculation`、`mathlibTorClass_ne_zero`、`mathlibTor1_nonzero_of_kernelQuotientCalculation`、`GeneralizedCoord`、`GeneralizedSharedWitnessCounterexample`、`SharedWitnessRepairCounterexample` を持つ。 | residue 計算と U/V endpoint 非含意は concrete に証明する。#3098 V-1 は `k = ZMod 2`、`R = (ZMod 2)[x,y,z]` に固定し、`R --xz--> R -> R/<xz>` の principal projective resolution、`R/<xy> ⊗ -` の degree-one cycle `[y] ⊗ 1`、not-boundary、Mathlib projective-resolution Tor iso を通じて `Tor_1(R/<xy>,R/<xz>) != 0` を certificate field なしで証明する。任意 `CommRing k` の非零定理、一般 principal-resolution theorem、一般 Tor calculator は主張しない。legacy package-relative surface は後方参照用に残るが、V-1 完了根拠ではない。この path は `x |-> 1` のため support-localized transfer の例ではない。 |
...
| `V.定義12.1 / 定理12.2` Hilbert series conflict accounting | `proved under explicit Hilbert-series / Euler-characteristic / basis-count data`. `HilbertSeriesTheory.HilbertSeries`、`HilbertSeries.ext`、`HilbertSeries.ofNatCoefficients`、`HilbertSeries.zero_coeff`、`add_coeff`、`sub_coeff`、`HilbertSeries.shift`、`GradedMonomialConflictRegime`、`GradedMonomialConflictRegime.homogeneousMonomialIdeals_certificate`、`DegreewiseShortExactPackage`、`DegreewiseShortExactPackage.hilbertSeries_additivity`、`DegreewiseShiftedFreePackage`、`DegreewiseShiftedFreePackage.shiftedFree_eq_rank_smul_shift`、`DegreewiseEulerCharacteristicPackage`、`DegreewiseEulerCharacteristicPackage.eulerCharacteristic_eq`、`HilbertSeriesBasisCountPackage`、`HilbertSeriesBasisCountPackage.coefficient_eq_basis_card`、`HilbertSeriesShortExactPackage`、`HilbertSeriesShortExactPackage.additivity_certificate`、`ShiftedFreeHilbertSeriesPackage`、`ShiftedFreeHilbertSeriesPackage.shiftedFree_certificate`、`FiniteComplexEulerCharacteristicPackage`、`FiniteComplexEulerCharacteristicPackage.eulerCharacteristic_certificate`、`HilbertSeriesConflictIdentityPackage`、`HilbertSeriesConflictIdentityPackage.denominatorClearedIdentity_certificate`、`eulerCharacteristic_certificate`、`interferenceSeries`、`interferenceSeries_coeff`、`HilbertSeriesConflictCoefficientIdentityPackage`、`HilbertSeriesConflictCoefficientIdentityPackage.denominatorClearedIdentity_of_coefficients`、`HilbertSeriesConflictCoefficientIdentityPackage.toIdentityPackage`、`HilbertSeriesFiniteWindowConflictAuditPackage`、`HilbertSeriesFiniteWindowConflictAuditPackage.coefficientIdentity_certificate` を持つ。`Formal/AG/Examples/DerivedPart5.lean` 側では `sharedWitnessG5_all_degree_coefficient_identity` と `sharedWitnessG5_denominatorClearedIdentity` が concrete shared-witness G5 identity を全次数で証明し、`sharedWitnessG5CoefficientIdentityPackage` / `sharedWitnessG5IdentityPackage` と `sharedWitnessAmbientBasisCountPackage` / `sharedWitnessQuotientBasisCountPackage` / `sharedWitnessJointBasisCountPackage` / `sharedWitnessTorOneBasisCountPackage` が package surface を返す。 | Hilbert series は degree-wise coefficient reading として扱い、有理関数表示は主張しない。定理12.2 は selected graded monomial regime と Euler-characteristic package に相対化し、全次数の係数等式から denominator-cleared identity を復元する theorem surface として読む。finite example では全次数 theorem と selected finite monomial basis cardinality package を主 surface とし、有限 degree window は派生 audit として残す。 |
| `V.定義13.1 / 定義13.2 / 定理13.3 / 定理13.4` Well-founded repair | `proved under explicit well-founded repair profile / finite synthesis data`. `WellFoundedRepair.RepairComparisonProfile`、`RepairComparisonProfile.step_decreases_certificate`、`InfiniteRepairSequence`、`no_infinite_repair_sequence`、`SoundRepairStepEvidence`、`SoundRepairStepEvidence.sound_or_certificate`、`SynthesisOutput`、`SoundRepairSynthesisPackage`、`emitsOnlySoundStepsOrNoSolutionCertificate_certificate`、`output_cleared_or_noSolution` を持つ。旧 `finite_trace_certificate` は `∃ n, trace.length = n` の無内容 theorem として削除済み。 | 定理13.3 は selected step relation が well-founded comparison で下降することに相対化する。定理13.4 は finite `List` trace と cleared / no-solution output package に相対化し、solver completeness、global repair optimality、全 law universe 同時改善は主張しない。 |
| `V.R11 finite examples` | `packaged examples plus #3098 V-1/V-2 closure surfaces`. `Formal/AG/Examples/DerivedPart5.lean` が `Example56TorCalculation`、`Example56TorCalculation.ofPrincipalKernelCalculation`、`Example56TorCalculation.tor1_nonzero`、`Example56DirectTorCalculation`、`Example56DirectTorCalculation.tor1_nonzero`、`toExample56TorCalculation`、`Example56LawConflictPackageFiring`、`Example56LawConflictPackageFiring.ofCanonicalIntersection`、`Example56LawConflictPackageFiring.toDirectTorCalculation`、`lawConflict0AlgEquivClassicalJoint`、`lawConflict1_nonzero`、`tor1_nonzero`、`derivedNonTransverse`、`derivedTransversalityCriterion`、`positiveTorVanishing_iff_classicalAgreement`、`sharedWitness_numeric_residue_path`、`sharedWitness_numeric_u_improves_not_v_nonincreasing`、`sharedWitnessAmbientCoeff`、`sharedWitnessQuotientCoeff`、`sharedWitnessJointCoeff`、`sharedWitnessTorOneCoeff`、`sharedWitnessAmbientHilbertSeries`、`sharedWitnessQuotientHilbertSeries`、`sharedWitnessJointHilbertSeries`、`sharedWitnessTorOneHilbertSeries`、`sharedWitnessConflictAlternatingSeries`、`sharedWitnessHilbertRegime`、`sharedWitnessG5_all_degree_coefficient_identity`、`sharedWitnessG5_denominatorClearedIdentity`、`sharedWitnessG5CoefficientIdentityPackage`、`sharedWitnessG5IdentityPackage`、`sharedWitnessG5WindowAuditPackage`、`smallRepairProfile`、`smallRepair_no_infinite_sequence`、`smallRepairClearedSynthesis`、`smallRepairNoSolutionSynthesis` を持つ。`Counterexample.SharedWitnessCoord.example56_zmod2_mathlibTor1_nonzero` が #3098 V-1 の concrete theorem で、`AxiomAudit.example56ZMod2MathlibTor1Nonzero` が標準公理範囲を固定する。旧 `sharedWitnessG5_window_interference_zero` は `X - X = 0` の誤ラベル theorem として削除済み。 | R6 V-2 では canonical `LawConflictPackage` が `ChartDerivedIntersection.toCanonicalLawConflictPackage` で構成され、`LawConflict_0 ≃ O/(I_U+I_V)` bridge、`derivedNonTransverse` が定理6.1 surface、`positiveTorVanishing_iff_classicalAgreement` が定理7.3(V) surfaceとして発火する。`AxiomAudit.canonicalSelectedTorBridgeLawConflict0AlgEquivClassicalJoint`、`canonicalSelectedTorBridgeLawConflict0LinearEquivMathlibTor`、`example56LawConflictPackageFiringDerivedNonTransverse`、`example56LawConflictPackageFiringDerivedTransversalityCriterion`、`example56LawConflictPackageFiringPositiveTorVanishingIffClassicalAgreement` は標準公理範囲で固定済み。V-1 は `ZMod 2` concrete theorem で閉じ、任意 `CommRing k` の非零性は主張しない。G5 数値例は `I_U=<xy>` / `I_V=<xz>` の concrete coefficient data から全次数係数恒等式と denominator-cleared identity を証明する。有理関数展開や一般 monomial Hilbert series 計算、一般 Tor calculator、global derived category は主張しない。 |
| `V.R12 final ledger / scan` | `proved final ledger sync`. `rg -n "^\s*(axiom\|admit\|sorry\|unsafe)\b" Formal/AG Formal/AG.lean` は no match。`Formal/AG.lean` は `Formal.AG.Derived` と `Formal.AG.Examples.DerivedPart5` を import し、`lake build` 対象に入る。 | R12 は台帳・検証の完了確認であり、新しい数学 claim は追加しない。余接複体 `L_{Flat/X}` と `Ext^1` は未割当 future proof obligation のまま維持する。 |
| `IV.定義2.4 DerOb_U` の一般 cotangent / Ext 構成 | `future proof obligation` / `unassigned`. PRD-R IV-6 では opaque placeholder を selected naive cotangent-coefficient carrier に置換した。PRD-5 本文が展開する chart-level Koszul / Tor 構成とは別境界として、一般余接複体 `L_{Flat/X}` と `Ext^1` の形式化は将来義務に残す。 | 余接複体 `L_{Flat/X}` と `Ext^1` の一般形式化は PRD-5 の現在 scope では実装しない。 |
...
| `V.命題9.2` | `proved concrete ZMod 2 Tor_1 nonzero plus legacy package-relative surface` |
...
| `V.R11 finite examples` | `packaged examples; #3098 V-1 closed over ZMod 2 and V-2 canonical bridge proved` |
| `V.R12 final ledger / scan` | `proved final ledger sync` |

AC 再判定: PRD-R R0 では AC5 / AC6 / AC16 を
`packaged (assumption-relative)` として読む。根拠宣言と再判定理由は
末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| `VI.定義3.1 / 定義3.2` Cotangent and tangent interface | `defined only` / `proved accessor` / `proved bridge`. `CotangentData`、`CotangentData.baseMap_eq`、`CotangentData.ofConDef`、`CotangentData.ofConDef_cotangentComplex_eq`、`CotangentData.ofConDef_pullbackComplex_eq`、`TangentData`、`TangentData.zeroObstruction_eq`、`TangentData.ofDerOb`、`TangentData.ofDerOb_obstructionTarget_eq`、`TangentData.ofDerOb_zeroObstruction_conDefClass`、`TangentData.ofDerOb_zeroObstruction_coefficient` を持つ。AxiomAudit では `partVICotangentConDefBridge` と `partVIDerObObstructionTargetBridge` を監査する。 | selected interface であり、一般余接複体構成、RHom 一般構成、Ext 一般理論は主張しない。VI-4 bridge は IV-6 の `ConDef_U = I_U/I_U²` と `DerOb_U` を Part VI の selected cotangent / obstruction target として読む bounded constructor に限る。 |
...
| `VI.定義9.3 / 定義9.4 / 定義9.5` Operation homotopy presentation and `pi_1^AAT` | `defined only` / `proved PresentedGroup bridge` / `compatibility package`. `HomotopyGeneratorFamily`、`pathCellFintype`、`loopRelatorFintype`、`pathCell_commonEndpoints`、`relator_based_holds`、`PresentationTwoComplex`、`vertices_read_states`、`twoCells_read_generators`、`FormalEdgeStep`、`FreeEdgeWord`、`FreeEdgeWord.toFreeGroup`、`PresentedArchitectureFundamentalGroup`、`selectedFreeGroupWord`、`presentedRelators`、`pi1AAT`、`presentedQuotientMap`、`presentedRelator_maps_to_identity`、`presentedGroupLift`、`presentedGroupLift_of`、`presentedGroupLift_unique`、`relator_maps_to_identity_holds`、`pathCellRelator_selected_holds`、`loopRelator_selected_holds`、`relator_generated_by_selected_generator_holds`、`quotient_universal_property` を持つ。 | R8 で selected free edge words を Mathlib `FreeGroup (FormalEdgeStep G)` に読み替え、selected relator set から `PresentedGroup` として `pi1AAT` を定義し、relator identity と `PresentedGroup.toGroup` の extension / uniqueness を証明した。既存 selected quotient carrier / transport package は compatibility surface として残す。未選択の operation、未選択の semantic equivalence、未構成 path は含めない。形式的 inverse edge は reverse architecture operation の存在を主張しない。 |
...
| `VI.定義13.1 / 定義13.3` Architecture stack and algebraic architecture stack | `defined only` / `proved accessor` / `proved cocycle equality accessor`. `ArchitectureStackBase`、`ArchitecturePresheaf`、`pullbackObj_eq`、`pullbackIso_holds`、`TripleOverlapCocycle`、`LocalArchitectureObjects`、`ArchitectureDescentDatum`、`EffectiveArchitectureDescent`、`ArchitectureStack`、`effectiveDescent_holds`、`cocycleCondition_holds`、`tripleCocycle_eq`、`AlgebraicArchitectureStackData`、`AlgebraicArchitectureStack`、`representableDiagonal_holds`、`atlasAdmissible_holds`、`obstructionIdealsDescend_holds`、`lawSheavesDescend_holds`、`signatureSheavesDescend_holds`、`structureSheavesDescend_holds` を持つ。 | Architecture stack は selected base / restriction identity and composition / triple-overlap context / triple から pairwise overlap への restriction arrow / groupoid-valued architecture presheaf / local objects / overlap isomorphisms / typed triple cocycle equality / effective descent predicate として扱う。旧 `cocycleCondition` は selected overlap / triple / local comparison data に相対化した境界として残し、VI-3 の `TripleOverlapCocycle` は triple context 上で `ij ≫ jk = ik` の実等式を保持する。AlgebraicArchitectureStack は representable diagonal、ArchitectureScheme atlas、selected structure descent の explicit data。一般 algebraic stack theory、stackification、未選択 descent completeness は主張しない。 |
...
| `VI.R12 finite examples` Part VI finite witness examples | `proved example theorem` / `selected witness package`. `Formal/AG/Examples/SingularityMonodromyStackPart6.lean` が `SingularBoundaryToyModel`、`verifies_singular_boundary`、`OperationSquareToyModel`、`verifies_square_nonfillability`、`TransportDescentZeroToyModel`、`nonempty_of_relationBoundaryZero`、`zero_case_descends`、`TransportDescentNonzeroToyModel`、`nonempty_of_not_relationBoundaryZero`、`nonzero_case_not_descend`、`RefactorGaloisToyModel`、`verifies_galois_connection`、`DecompositionGerbeToyModel`、`verifies_no_canonical_decomposition` に加え、#3099 の concrete carriers `toyCotangentData`、`toyTangentData`、`toyDeformationTheory`、`toyOperationCategory`、`toyMeasuredSquareZero`、`toyMeasuredSquareNonzero`、`toyTransportDescentZero`、`toyTransportDescentNonzero`、`toyGaloisData`、`toyGerbeObstructionData` と、発火 theorem `concreteSingularBoundaryToyModel_fires`、`concreteOperationSquareToyModel_fires`、`concreteTransportDescentZero_descends`、`concreteTransportDescentNonzero_not_descend`、`concreteRefactorGaloisToyModel_fires`、`concreteDecompositionGerbeToyModel_fires` を持つ。 | R12 examples は finite selected witness package として、既存 Part VI theorem API を検証する。#3099 では `Bool` / `Fin 2` ベースの concrete nondegenerate data instance から5 toy model を発火し、空型 package / PUnit-only reading を completion evidence として扱わない。第VII部 golden examples への calibration や全 concrete codebase extraction は扱わない。 |
| `CotangentComplexGeneralConstruction` | `future proof obligation` / `unassigned` | PRD-6 では `CotangentData.ofConDef` により Part IV conormal quotient `ConDef_U` を selected cotangent carrier として接続した。一般余接複体構成、RHom 一般構成、`Ext^1` 一般理論は証明しない。 |
...
| `VI.定義9.3-9.5` | `defined only` / `proved PresentedGroup bridge` / `compatibility package` |
...
| `VI.R12 finite examples` | `proved example theorem` / `selected witness package` |

AC 再判定: PRD-R R0 では AC10 / AC20 を `packaged (assumption-relative)` として読む。
根拠宣言と再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
Issue [#3051](https://github.com/iroha1203/AlgebraicArchitectureTheoryV2/issues/3051)
...
| `VII.定義2.1` AATSch and AnalyticRepresentation | `defined only` / `proved accessor` / `Mathlib category and functor bridge`. `AATSchReadingParameter` が PRD-3 `ArchitectureScheme` 上の selected scheme morphism interface、identity / composition law、Atom label / law / obstruction ideal / signature / interpretation map readings、および fixed compatibility predicate の identity / composition closure を保持する。`AATSch` は fixed parameter `p` に相対化した decorated scheme、`AATSchMorphism` は reading parameter 側の fixed compatibility predicate に対する certificate を持つ morphism であり、`Category (AATSch p)` が成立する。`AATSchIdentityData` / `AATSchCompositionData` は category identity / composition からも作れる selected data を保持する。`AATSchFiberProductData` は underlying pullback と decoration pullback compatibility が与えられた場合だけ存在する optional surface として置く。`AnalyticTargetCategory` は target hom の identity / composition law を持ち、target wrapper category と `AnalyticRepresentation.toFunctor` が Mathlib `CategoryTheory.Functor` への bridge を与える。 | bridge は selected reading parameter / target category contract に相対化される。一般 scheme category、一般 target category の存在、外部 artifact validation は主張しない。 |
...
| `VII.定義3.3 / 定義3.5` Graph and Matrix representation | `defined only` / `proved accessor` / `target category laws`. `FiniteDirectedGraphTarget` は fixed carrier 上の finite vertex / edge、source / target、selected relation label を保持する。`FiniteDirectedGraphHom` と `MatrixRepresentationHom` は identity / composition law を持ち、graph / matrix target category interfaces が `AnalyticRepresentation.toFunctor` の target category bridge に接続できる。`GraphRepresentationProfile` は selected relation data を graph edge へ写し、`AnalyticRepresentation` へ橋渡しする。`adjacencyMatrix`、`incidenceMatrix`、`transitionMatrix`、`MatrixRepresentationTarget`、`MatrixRepresentationProfile` は matrix reading target を定義し、canonical graph-derived matrix package と accessor theorem を持つ。 | AC4 は定義 surface まで。R13 graph / matrix selected finite examples は別行で実装済み。 |
...
| `VII.定義10.1 / 10.2 / 11.1 / 11.2` Distance to Flatness and Obstruction Mass | `defined only` / `proved accessor` / `proved actual GLB constructor`. `OperationDistanceProfile` は selected operation path cost / path length / homotopy filling cost に相対化した `d_op(A,B)` reading を持つ。`CostInfimumDomain` と `DistanceToFlatnessProfile` は selected flat candidate `F` の cost が `d_op(A,F)` を読む certificate、selected flat candidate family 上の `dist_flat_U(A)` infimum reading、GLB certificate を持つ。#3100 VII-5 では `CostInfimumDomain.completeLatticeInfimumDomain` が Mathlib `iInf` と ordinary lower-bound / greatest-lower-bound predicate から actual GLB domain を構成し、finite example の `enatInfimumDomain` / `marginDistanceToFlatness_dist_flat_glb` が fake `True` GLB を置換する。`dist_flat_U(A)=0 -> factorization` は theorem ではなく `DistanceZeroReflectsFactorization` predicate としてだけ定義する。`SelectedObstructionMeasure` は `mu_Ob` / `mu_I` と selected support を保持し、`FiniteSupportObstructionMass` は evaluation point ごとの finite support list、finite sum、finite integral interface、`Mass_U(A)` を保持する。 | 一般 measure theory、距離ゼロからの lawfulness / flatness reflection は主張しない。distance-zero reflection が必要な後続定理では明示 predicate として渡す。 |
...
| `VII.定理15.4` Representation Conservativity under Adequacy | `proved under explicit adequacy and witness-exactness assumptions`. `RepresentationConservativityUnderAdequacy` は selected `AnalyticReadingContext` に対し、coverage adequacy、witness exactness、axis exactness、coefficient discipline、`WitnessZero_U alpha -> alpha = zeroClass`、および `IsZeroClass zeroClass` を明示 field として持つ。`representation_conservativity_under_adequacy` は全 selected representation reading が analytic zero の selected obstruction class `alpha` について、U-detecting で `WitnessZero_U alpha` を得て、explicit exactness field により `alpha = zeroClass` を証明する。`zeroClass_isZero_holds` と `representation_zero_under_adequacy` は `zeroClass` が actual zero predicate を満たすこと、および全 reading zero の `alpha` が actual zero predicate を満たすことを証明する。 | theorem は selected obstruction class と明示 assumptions に相対化される。全 representation universe の completeness、任意 obstruction universe の完全検出、measurement verdict、外部 artifact validation は主張しない。 |
| `VII.定理16.1` Algebraic-Geometric AAT Synthesis | `proved theorem package` / `proved finite synthesis firing`. `AATSynthesisAssumptions` は `Site.PartIPrerequisites`、`ArchitectureGeometry`、`AATSite`、`RingedAATTopos`、`ArchitectureScheme`、scheme atlas としての `AffineAATChart` family、`LawfulLocus` / `LawfulSectionData`、cover-relative Cech obstruction cohomology、`RepairComparisonProfile`、PRD-6 `ArchitectureStratum`、第VII部 `AnalyticReadingContext` を明示前提として束ねる。`AATSynthesisPackage` は同 tower を theorem package として保持し、`ringedAATTopos_is_scheme_ringedTopos` と `affineAATCharts_are_scheme_charts` が scheme 側 topoi / atlas との coherence を公開する。`AATSynthesisAssumptions.toPackage` と `algebraicGeometricAATSynthesis` が package 構成と selected site、scheme coherence、lawful locus、obstruction cohomology、derived geometry、singularity stack、analytic reading の保持を証明する。R9 では `finiteSynthesisAATSynthesisAssumptions` / `finiteSynthesisAATSynthesisPackage` / `finiteSynthesis_algebraicGeometricAATSynthesis_fires` が PRD-1 finite model 上の singleton predecessor tower から theorem application を発火させる。 | theorem は構成済み predecessor tower と selected reading context に相対化される。finite firing は singleton `PUnit` coefficient fixture であり、measurement verdict、外部 artifact validation、全 representation universe の completeness は主張しない。 |
| `VII.R13 finite examples` graph/matrix toy model, period separation, pseudo-circle strict period, margin stability, observation gap, detecting representation, finite synthesis tower firing | `proved selected examples`. `Formal/AG/Examples/RepresentationAnalysisPart7.lean` が `dependencyDAG` / `dependencyDAGMatrix`、`dependencyDAG_walkCount_ab_positive`、`dependencyDAG_matrixWalkReading_ab`、`dependencyDAG_length_one_walkCount_eq_edgeFiber_card`、`dependencyDAG_ab_edgeFiber_card`、`dependencyDAG_nilpotent_at_card`、`periodSeparation_toy_model`、`pseudoCircleSyncChain`、`pseudoCircleAsyncChain`、`pseudoCircle_sync_async_shared_boundary`、`pseudoCircle_cycle_boundary_zero`、`pseudoCircleStrictPeriodRepresentative`、`pseudoCircle_strictPeriodRepresentative_eq_one`、`pseudoCircle_strictPeriodData_reads_representative`、`marginStability_toy_endpoint_safe`、`marginStability_toy_no_boundary_crossing`、`observationGap_toy_lipschitz_bound`、`observationGap_toy_div_lipschitz_lower_bound`、`observationGap_toy_quotient_certificate`、`toyDetectingFamily`、`toyAnalyticReadingContext`、`detectingRepresentation_toy_syncGap_not_all_zero`、`detectingRepresentation_toy_semanticGap_not_all_zero`、`detectingRepresentation_toy_zero_conservative`、`detectingRepresentation_toy_all_zero_actual_zero`、`detectingRepresentation_toy_zero_is_actual_zero`、`finiteSynthesisAATSynthesisPackage`、`finiteSynthesis_algebraicGeometricAATSynthesis_fires` を持つ。 | examples は selected finite witness surfaces であり、一般 graph/matrix completeness、一般 singular homology realization、measurement verdict、外部 artifact validation は主張しない。 |
...
| `VII.定義2.1` `AATSch p` / `AnalyticRepresentation` | `defined only` / `proved accessor` / `Mathlib category and functor bridge` |
| `VII.定義3.1 / 4.1-4.3` representation family and preservation / reflection | `defined only` / `proved accessor` |
| `VII.定義3.3 / 3.5` graph representation and matrix representation | `defined only` / `proved accessor` / `target category laws` |
...
| `VII.定理16.1` Algebraic-Geometric AAT Synthesis | `proved theorem package` / `proved finite synthesis firing` |
| `VII.R13 finite examples` graph/matrix toy model, pseudo-circle period, margin, observation gap, detecting representation, finite synthesis tower firing, and golden-example packaging beyond the AC9 selected period-separation witness | `proved selected examples` |
...
AC 再判定: PRD-R R0 では AC8 を `packaged (assumption-relative)` として読み、AC19 は R9 の finite synthesis firing により `proved finite synthesis firing` へ更新した。
根拠宣言と再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| `VIII.定義4.1 / R2` Finite Measurement Regime and EffCoeff | `defined only` / `proved accessor` / `proved Fintype accessor`. `Formal/AG/Measurement/FiniteRegime.lean` が `EffCoeff` と `FiniteMeasurementRegime` を追加し、kernel / image / quotient / ideal-membership / finite presentation / resolution の selected object、`Fintype` carrier、certificate、および finite site、finite cover、effective coefficient、explicit restriction maps、finite witnesses、finitely generated obstruction ideal、selected finite resolutions、zero / nonzero predicate certificate fields を保持する。`FiniteMeasurementRegime.finiteSite_holds`、`finiteCover_holds`、`effectiveCoefficient_holds`、`finiteWitnessVariables_holds`、`selectedFiniteResolutions_holds`、`kernelObjectFintype`、`imageObjectFintype`、`quotientObjectFintype`、`idealMembershipObjectFintype`、`finitePresentationObjectFintype`、`resolutionObjectFintype` が accessor として存在する。 | 任意の係数圏の決定可能性や algorithm completeness は主張しない。 |
| `VIII.定理4.2 / R2` Finite AAT Computability | `proved theorem package construction` / `proved finite carrier lemma`. `Formal/AG/Measurement/Computability.lean` が `FiniteCechComplexRepresentation`、`FiniteCocycleRepresentative`、`FiniteVerdictComputationObject`、`FiniteSquareFreeObstructionIdeal`、`FiniteStanleyReisnerComplex`、`FiniteMonomialTorComplex`、`FiniteConflictSupport`、`FiniteAATComputability`、`finiteAATComputabilityPackage`、`finiteAATComputability` を追加し、selected invariants が finite linear algebra / finitely presented module / finite combinatorics / finite resolution reduction object へ落ちる theorem package を構成する。VIII-3 では各 finite computation object が `Fintype carrier` を保持し、`FiniteAATComputability.*CarrierFintype` と `cechComplexCarrier_finite` / `cocycleRepresentativeCarrier_finite` / `computationCarriers_finite` が selected cochain / computation carrier の有限性を actual finite type として読む。 | theorem は selected finite regime と supplied finite objects に相対化される。Scarf / arbitrary minimal resolution、新規 Tor algorithm、全 coefficient universe の計算可能性は主張しない。 |
| `VIII.定義5.1 / 定理5.2 / 定義5.4 / 定理候補5.5 / R3` Square-Free Repair and Alexander Dual | `defined only` / `proved theorem package construction` / `finite Alexander dual bridge` / `theorem candidate statement-only`. `Formal/AG/Measurement/SquareFreeRepair.lean` が `UsesSquareFreeWitnessRegime`、`SquareFreeRepairRegime`、`SquareFreeRepairRegime.source_obstructionIdeal_eq_stanleyReisnerIdeal`、`SquareFreeRepairRegime.source_minimalGeneratorSupport_iff_minimalForbidden`、`finiteAlexanderDualFaces`、`mem_finiteAlexanderDualFaces_iff`、`finiteAlexanderDual`、`FinsetHitsForbidden`、`MinimalHittingSetForForbidden`、`not_mem_finiteAlexanderDual_iff_hitsForbidden`、`FiniteAlexanderDualRepairBridge`、`FiniteAlexanderDualRepairBridge.minimalRepairTarget_iff_minimalHitting`、`AlexanderDualComplex`、`MinimalVertexCover`、`MinimalWitnessHittingSet`、`MinimalRepairHittingSet`、`StanleyReisnerAlexanderDualRepair`、`stanleyReisnerAlexanderDualRepairPackage`、`stanleyReisner_alexanderDual_repair`、`DiscreteMorseRepairReading`、`MorseLowerBoundForStructuralRepairCandidate` を追加する。`SquareFreeRepairRegime` は `sourceWitnessRegime : UsesSquareFreeWitnessRegime M.WitnessVariables` と profile witness lift / source-reading certificate を持ち、PRD-3 square-free witness regime を measurement profile に持ち上げる。定理5.2 package は PRD-3 の obstruction ideal / Stanley-Reisner ideal equality と minimal generator / minimal forbidden support theorem を source regime へ再利用し、finite Alexander dual nonface と forbidden-support hitting set の同値、および repair target から minimal finite hitting set への bridge を保持する。 | minimal repair hitting set は combinatorial target であり actual architecture repair operation semantics ではない。Morse lower bound は operation semantics compatibility / legality / side-effect profile を仮定に持つ candidate statement に留める。 |
...
| `VIII.定義8.1-8.3 / 定理8.5 / 定理8.6 / 系8.7 / 定義8.8 / 定義8.9 / 定理候補8.10 / R6` Cellular Laplacian, Hodge, and Harmonic Debt | `defined only` / `proved theorem package construction` / `real finite inner-product bridge` / `theorem candidate statement-only`. `Formal/AG/Measurement/CellularLaplacian.lean` が `CellularMeasurementModel`、`CellularMeasurementModel.finiteCells_holds`、`finiteDimensionalCochains_holds`、`differentialSquaresZero_holds`、`SheafLaplacianReading`、`SheafLaplacianReading.laplacian_eq_formula_holds`、`DistanceToFlatnessReading`、`DistanceToFlatnessReading.residual_eq_norm_d0_holds`、`distanceZeroDoesNotImplyLawful_holds` を追加する。`Formal/AG/Measurement/Hodge.lean` が legacy theorem package に加え、`RealFiniteInnerProductComplex`、`RealFiniteInnerProductComplex.dPrevAdjoint`、`dNextAdjoint`、`laplacian`、`dPrev_adjoint_inner_right`、`dNext_adjoint_inner_right`、`RealFiniteHodgeDecomposition`、`RealHarmonicDebtMinimality`、`RealHarmonicDebtMinimality.harmonicDebt_eq_norm_holds`、`selected_minimizes_holds`、`RealEssentialRepairLowerBound`、`RealEssentialRepairLowerBound.lowerBound_reads_harmonicDebt_holds`、`lowerBound_le_repairCost_holds` を追加する。PRD-R VIII-1 では finite-dimensional real inner-product cochain spaces、Mathlib `LinearMap.adjoint`、実等式 `d_next.comp d_prev = 0`、range witness、kernel membership、decomposition equality、orthogonality equality、cochain-relative real residual inequality / lower-bound inequality を load-bearing surface として扱う。 | 一般 Hilbert complex、infinite-dimensional analytic theory、near-flatness から lawfulness、Perron-Frobenius / spectral hotspot theorem は主張しない。 |
| `VIII.定義9.1 / 定理候補9.2 / R7` Common Ambient Pair and LawConflict Measurement | `defined only` / `proved accessor` / `proved Derived bridge` / `theorem candidate statement-only`. `Formal/AG/Measurement/LawConflict.lean` が `CommonAmbientPair`、`CommonAmbientPair.commonRingedSite_holds`、`lawIdealsInCommonAmbient_holds`、`coefficientsCompatible_holds`、`noComparisonWithoutCommonAmbient_holds`、`LawConflictMeasurement`、`LawConflictMeasurement.lawConflictTorReading_holds`、`selectedClassSupportReading_holds`、`commonAmbientRequired_holds`、`ofDerivedLawConflictPackage`、`derivedLawConflictLinearEquivMathlibTor`、`FlatBaseChangeCandidate`、`FlatBaseChangeCandidate.affineBaseChangeStatement_shape_holds`、`sheafBaseChangeStatement_shape_holds`、`candidateOnly_holds` を追加する。LawConflict は selected common ambient pair と coefficient compatibility に相対化された Tor reading として扱い、VIII-4 では Part V `Derived.Intersection.LawConflictPackage` から Measurement の LawConflict reading を `Nonempty` linear equivalence witness として構成し、同 package が持つ Mathlib Tor equivalence を Measurement 側から参照できる bridge を追加する。flat base change は affine / sheaf-ringing-site の statement shape だけを保持する theorem candidate として扱う。 | 異なる topology / coefficient ring / ambient scheme の law ideals を comparison morphism なしに比較しない。一般 Tor base-change theorem、non-flat base change preservation、support pullback 未指定での conflict support comparison は主張しない。 |
...
| `VIII.定理12.3 / 原則12.4 / R10` AAT-GAGA Finite Measurement Comparison | `defined only` / `proved theorem package construction` / `certified fields backed by theorem packages`. `Formal/AG/Measurement/GAGA.lean` が `SelectedFiniteHodgeTheoremPackage`、`SelectedPeriodStokesTheoremPackage`、`SelectedTopologicalDebtTheoremPackage`、`SelectedDerivedConflictTheoremPackage`、各 theorem-package accessor、`AATGAGACertifiedFields`、`AATGAGACertifiedFields.hodgeComparisonCertified_holds`、`derivedConflictAccountingCertified_holds`、`finiteHodgeDecomposition_holds`、`finiteHodgeHarmonicCohomology_holds`、`periodStokesTheorem_holds`、`periodStokesAccountingAdditive_holds`、`topologicalDebtTheorem_holds`、`topologicalDebtCapacityFromComplex_holds`、`derivedConflictLawConflictTorReading_holds`、`AATGAGACandidateInterfaces`、`AATGAGACandidateInterfaces.candidateInterfacesSeparatedFromCertified_holds`、`AATGAGAComparisonAssumptions`、`AATGAGABoundary`、`AATGAGABoundary.noExternalDataSourceFidelity_holds`、`candidateDependentFieldsNotCertified_holds`、`AATGAGAComparisonData`、`aatGAGAComparisonStatement`、`aatGAGAComparisonStatement_cert`、`AATGAGAComparisonPacket`、`AATGAGAComparisonPacket.finiteProfileComparison_holds`、`certifiedCandidateSeparation_holds`、`aatGAGAComparisonPacketOfData`、`AATGAGAFiniteMeasurementComparison`、`AATGAGAFiniteMeasurementComparison.finite_profile_comparison_holds_of_package`、`candidate_interfaces_separated_holds_of_package`、`aatGAGAFiniteMeasurementComparisonPackage`、`aatGAGAFiniteMeasurementComparison` を追加する。certified fields と candidate-interface fields を分離し、VIII-5 では certified harmonic decomposition と derived conflict accounting に加え、Period/Stokes が selected `ExtensionHolonomyAccounting` の `kappa_U_additive` theorem を読み、topological debt が selected `FiniteNerveCochainComplex.topologicalDebtCapacity_fromComplex` theorem を読む package を持つ。定理12.3 は raw comparison data から finite measurement regime / finite cover / inner-product coefficient sheaf / cellular cochain model / square-free regime / common ambient / stability distance と comparison maps、および certified fields 全体と boundary certificates に相対化された finite-profile comparison packet を構成する theorem package として扱う。 | `GAGA` は external data source、external procedure、任意 law universe、外部忠実性を主張しない。candidate-dependent fields は certified conclusion に昇格しない。Period/Stokes package は一般 Čech Stokes pairing theorem までは主張せず、selected extension accounting の additive theorem を読む。topological debt package は selected finite nerve cochain complex に相対化し、任意 cover からの自動構成は主張しない。 |
| `VIII.R11 finite examples` | `proved selected finite examples` / `Hodge fixture fired on real 3D complex` / `finite Alexander dual fixture fired` / `GAGA Hodge certified field backed by theorem package`. `Formal/AG/Measurement/Examples.lean` が `PseudoCircleMeasurementDomain`、`SquareFreeSupportVertex`、`SquareFreeRepairTarget`、`TinyMeasurementSite`、`LowDegreeCochain`、`LowDegreeBoundaryCochain`、`LowDegreeRealCochain`、`TransferResidueFlag`、`pseudoCircleMeasurementProfile`、`pseudoCircleBoundaryCocycleVerdict`、`pseudoCircle_unmeasuredAxis_not_zero`、`forbiddenSupportPQFinset`、`forbiddenSupportQRFinset`、`squareFreeRepairTargetSupport`、`squareFreeHittingSet_q_hits_forbiddenSupports`、`squareFreeHittingSet_pr_hits_forbiddenSupports`、`squareFreeSourceWitnessRegime`、`squareFreeSourceDelta`、`squareFreeFiniteAlexanderDual`、`squareFreeRepairTargetAlexanderDualNonface`、`squareFree_repairSupport_notMemAlexanderDual_iff_hitsForbidden`、`squareFree_singletonQ_hitsForbidden`、`squareFree_pairPR_hitsForbidden`、`squareFree_singletonQ_minimalHitting`、`squareFree_pairPR_minimalHitting`、`squareFreeRepairRegime`、`squareFreeAlexanderDual`、`squareFreeMinimalRepairHittingSets`、`squareFreeAlexanderDualRepairBridge`、`squareFreeRepairExamplePackage`、`squareFree_singletonQ_minimalRepairHittingSet`、`squareFree_pairPR_minimalRepairHittingSet`、`tinyFiniteMeasurementRegime`、`finiteComputabilityExamplePackage`、`finiteComputabilityExample_verified`、`refactorInvarianceExamplePackage`、`lowDegreeRealComplex`、`lowDegreeRealComplex_laplacian_eq_zero`、`lowDegreeRealComplex_all_harmonic`、`lowDegreeRealHodgeDecomposition`、`lowDegreeRealComplex_kernel_equiv_harmonicCohomology`、`lowDegreeRealHarmonicDebtMinimality`、`lowDegreeRealEssentialRepairLowerBound`、`lowDegreeHodgePackage`、`lowDegreeHarmonicDebtPackage`、`supportTransferExamplePackage`、`measurementPacketExampleSynthesis`、`lowDegreeSelectedHodgeTheoremPackage`、`lowDegreePeriodStokesAccounting`、`lowDegreePeriodStokesTheoremPackage`、`lowDegreeTopologicalDebtNerve`、`lowDegreeTopologicalDebtComplex`、`lowDegreeTopologicalDebtTheoremPackage`、`gagaComparisonExamplePackage`、`RefactorInvarianceFiniteExample`、`CellularHodgeFiniteExample`、`SupportLocalizedTransferFiniteExample`、`MeasurementPacketGAGAFiniteExample`、`measurementPacketGAGAExample_certifiedCandidateSeparated`、`PartVIIIFiniteExampleSuite`、`PartVIIIFiniteExampleSuite.CoversR11`、`partVIIIFiniteExampleSuite_complete` を追加する。R11(b) は `{p,q}` / `{q,r}` forbidden supports と `{q}` / `{p,r}` repair supports で finite hitting-set minimality を証明し、`not_mem_finiteAlexanderDual_iff_hitsForbidden` 由来の nonface-to-hitting theorem を theorem 5.2 package の vertex/witness 対応と repair 対応に使う。R11(e) Hodge fixture は `EuclideanSpace ℝ (Fin 3)` の zero differential complex で `L=0`、全 cochain harmonic、`0 + x + 0 = x` の real decomposition、kernel/cohomology-harmonic equivalence、harmonic debt residual 最小性、repair lower-bound inequality を証明し、legacy `FiniteHodgeDecompositionData` の Prop はこの real theorem から読む。GAGA fixture は `lowDegreeSelectedHodgeTheoremPackage` により certified Hodge/harmonic field を実 `FiniteHodgeDecomposition` package に接続し、Period/Stokes は selected extension accounting additive theorem、topological debt は selected finite nerve cochain complex の debt-capacity theorem に接続する。 | finite examples は selected fixture surface であり、外部 extraction、actual repair operation semantics、一般 Alexander duality、一般 Hodge theory、一般 transfer necessary condition、外部 GAGA fidelity は主張しない。 |
...
| `VIII.定義5.1 / 定理5.2 / 定義5.4 / 定理候補5.5 / R3` Square-Free Repair / Alexander Dual / Discrete Morse | `defined only` / `proved theorem package construction` / `finite Alexander dual bridge` / `theorem candidate statement-only` |
...
| `VIII.定義8.1-8.3 / 定理8.5 / 定理8.6 / 系8.7 / 定義8.8 / 定義8.9 / 定理候補8.10 / R6` Cellular Laplacian / Hodge / Harmonic Debt | `defined only` / `proved theorem package construction` / `real finite inner-product bridge` / `theorem candidate statement-only` |
...
| `VIII.R11 finite examples` pseudo-circle measurement, square-free hitting set, finite computability, refactor invariance, cellular Hodge, support transfer, packet / GAGA separation | `proved selected finite examples` / `finite Alexander dual fixture fired` / `Hodge fixture fired on real 3D complex` |
...
AC 再判定: PRD-R R0 では AC15 の Hodge 側を
`real finite inner-product bridge` / `Hodge implemented` として読み、
AC16 の theorem 5.2 側は PRD-3 Stanley-Reisner theorem reuse と
finite Alexander dual / minimal hitting-set bridge で実装済みとして読む。
AC9 / AC10 / AC17 は `statement-only` として集中表で明示する。根拠宣言と
再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。
...
| `IX.§3 / R2` TemporalSite / TemporalCover / TempCoeff_A | `defined only` / `proved accessor` / `proved incidence differential equation` / `proved selected cohomology-level PRD-4 bridge`. `TemporalSite` が selected finite trace regime と selected finite-poset AAT site regime を束ね、`TemporalSite.Point` を trace/context pair として読み、`IncidenceLeg` / `idLeg` / `compLeg` で selected product-incidence leg を保持する。`TemporalCover` が trace arrow と architecture context order witness を持ち、`TemporalCoverToSiteCover`、`TemporalCechBridge`、`FinitePosetTemporalCechBridge` が temporal cover を PRD-4 cover-relative Čech complex と PRD-2 finite-poset comparison package に接続する。`TemporalCoefficient` / `TempCoeff_A` が selected coefficient profile、obstruction sheaf、pointwise additive temporal fiber、incidence leg restriction map、identity / composition law、obstruction sheaf section comparisonを保持する。IX-3 では `FiberZeroCochain` / `FiberIncidenceOneCochain` と product-site incidence differential `δ c f = restrict f (c q) - c p`、identity leg 上の zero theorem `incidenceDifferential_id`、product incidence complex package `ProductIncidenceComplex`、PRD-4 bridge package `ProductIncidencePRD4Comparison`、cohomology-level inverse accessors を追加した。`EvolutionPart9` が finite poset × two-point trace singleton coefficient の `unitFinitePosetTemporalCechComparisonData` / `unitFinitePosetTemporalCechBridge` / `unitProductIncidencePRD4Comparison` と cohomology-level inverse theorem を発火する。 | Temporal site は selected finite product/incidence surface であり、一般 product-site theory や外部 runtime time を主張しない。任意 finite poset × 任意 trace の general cohomology theorem は主張しない。 |
| `IX.§3 / R3` StateTransitionPresheaf / TemporalLaw | `defined only` / `proved accessor` / `proved constructor equation`. `StateTransitionPresheaf` が selected temporal point 上の state carrier、local transition category surface、context restriction、selected trace transport、identity / composition functoriality、context/trace commutative squareを保持する。`StateTransitionSheaf` が temporal cover 上の selected descent predicate と overlap compatibility predicate を保持する。`TemporalLawKind` と `TemporalLaw` が selected `St_A` と `Tr_E` に相対化された law vocabulary、incidence witness、state equation、transition predicate、descent predicateを保持する。#3100 IX-4 では `TemporalLaw.ofStateEquation` と kind 別 canonical constructors、`ofStateEquation_kind` / `ofStateEquation_holdsStateEquation`、finite example の `temporalLaw_kind` / `temporalLaw_stateEquation` を追加した。 | 一般 sheafification、外部 runtime trace、実時間、任意 future path forecast は主張しない。 |
...
| `IX.§4 / R5` ReplayDescentData / Temporal Descent Criterion | `defined only` / `proved theorem package` / `selected realization bridge`. `ReplayDescentData` が temporal cover と selected trace arrow 上の local replay map、degree-one mismatch cochain、law support certificate を保持し、`ReplayDescentData.mismatch` が R4 `TemporalMismatch` へ読む。`ReplayMismatchCocycle` が zero-differential witness を保持し、`EffectiveTemporalAdjustment` が zero-cochain correction と adjusted mismatch equation `m(adjust(c,r)) = m(r) - d c` を保持する。`TemporalDescentRealization` が selected temporal class vanishing と `adjustment.adjustedData` compatibility から actual `r.GlobalReplayTransition` を構成する bridge を保持し、`TemporalDescentCriterion.ofRealization` がその bridge から theorem 4.2 package を構成する。`TemporalDescentCriterion.temporal_descent_criterion` は mismatch cocycle、selected temporal class、`temporalClass.cohomologyClass = r.zeroMismatchClass`、effective adjustment、`adjustment.adjustedData` の compatibility から `Nonempty r.GlobalReplayTransition` を返す。 | Temporal Descent Criterion は片方向であり、global replay から class vanishing への逆向きは主張しない。realization は selected adjusted replay compatibility に相対化される。 |
...
| `IX.定義7.1 / 定理候補7.2 / R9` Force / Force Integrability Obstruction Candidate | `defined only` / `selected integration interface` / `inhabitable candidate data` / `statement-only candidate`. `Force` が selected incidence leg に沿う temporal state morphism と source/target state witness を保持する。`IntegrableForce` は `ForceIntegrationData` の `Nonempty` として R5 `ReplayDescentData`、`TemporalDescentCriterion`、selected global replay transition、force source/target と replay base source/target の一致、global replay が selected source state を selected target state へ送る証明、selected `TemporalLaw.stateEquation` による force respect 証明を読む。`ForceMismatchClass` が force 由来の degree-one temporal mismatch class、trace product site fixed 境界、mismatch construction soundness 境界を保持する。`ForceIntegrabilityObstructionCandidateData` は証明済み非可積分性を要求せず、selected nonzero marker、coefficient exactness、witness coverage、temporal descent detecting、local-to-global descent control を inhabitable な selected data として保持する。`ForceIntegrabilityObstructionCandidate` は #3091 で literal に inhabitable な selected statement package となり、`candidate_statement_recorded` は非可積分性 implication を証明せず statement-only 境界を記録する。 | theorem candidate は future proof obligation として残し、一般 force integrability theorem は証明しない。未選択 transition、未構成 state space、任意 future path forecast は主張しない。 |
| `IX.R10 finite temporal examples` | `proved examples` / `selected finite fixtures` / `nondegenerate replay fixture fired` / `force integration fixture fired` / `selected nonzero backed by concrete mismatch` / `PRD-10 circle bridge`. `Formal/AG/Examples/EvolutionPart9.lean` が two-step trace、finite temporal site/state presheaf、singleton temporal coefficient / Čech bridge、zero replay descent fixture、pseudo-circle nonzero replay mismatch、finite dissipation stopping、non-lawful terminal boundary、harmonic Lyapunov reading、force integration / mismatch class / obstruction candidate data fixture を持つ。`zeroReplayAdjustedCompatible` が adjusted replay の両 selected chart で `high -> mid` を読む compatibility を固定し、`zeroReplayTemporalDescentRealization` と `zeroReplayTemporalDescentCriterion` が R5 の theorem-4.2 package から global replay transition を読む。IX-5 では `ReplayDescentNonzeroExample` の selected nonzero marker を `True` token から `pseudoCircleMismatch edge ≠ 0` に置換し、`replay_selectedConcreteClassNonzero` が concrete `ZMod 2` mismatch の非零性を読む。`replay_nonzero_and_prd10_circle_coverRelativeH1_nonzero` は replay nonzero fixture と PRD-10 circle / Part IV cover-relative H1 nonzero theorem を同じ finite pseudo-circle nonzero surface に束ねる。`toyForceIntegrationData` と `toy_force_integrable` が theorem-4.2 replay data と `TemporalLaw.stateEquation` を selected force integration data として読み、`toyForceMismatchClass`、`force_candidate_selected_nonzero`、`force_candidate_selected_nonzero_backed_by_concrete`、`force_candidate_data_inhabited`、`force_candidate_package_inhabited` が R9 の `ForceMismatchClass` surface と concrete `ZMod 2` value に backed された inhabitable selected nonzero marker data / candidate package を接続し、`finite_temporal_examples_verified` が R10(a)-(g) の aggregate witness を束ねる。 | selected finite fixtures に限る。一般 temporal semantics、nonzero replay からの global failure、一般 force integrability theorem、未選択 transition、任意 future path forecast は主張しない。 |
...
| `ReplayDescentData` / `Temporal Descent Criterion` | `defined only` / `proved theorem package` / `selected realization bridge` |
...
| `Force` / `IntegrableForce` / `ForceMismatchClass` | `defined only` / `selected integration interface` / `proved accessor` |
| `ForceIntegrabilityObstructionCandidateData` | `inhabitable candidate data` / `proved accessor` |
| `ForceIntegrabilityObstructionCandidate` | `inhabitable statement-only candidate package` / `future proof obligation` |
| `Part IX finite examples` | `proved examples` / `selected finite fixtures` / `force integration fixture fired` |
...
AC 再判定: PRD-R AC17 では AC13 theorem 4.2 側を
`proved selected realization bridge` / `proved finite fixture` として読む。
PRD-R AC18 / AC5 IX-2 では `IntegrableForce` を R5
`ReplayDescentData` / `TemporalDescentCriterion` / selected global replay
transition に接続された selected integration interface として読み、
force candidate 側は concrete obstruction value に backed された
`inhabitable candidate data` と、future proof obligation に残る
`statement-only theorem candidate` に分離して読む。
根拠宣言と再判定理由は末尾の「PRD-R R0 AC 再判定」集中表に置く。

## AAT Algebraic-Geometric Part X Semantic Repair Descent (SAGA) Lean status

Files: `Formal/AG/SemanticRepair.lean`, `Formal/AG/SemanticRepair/Bootstrap.lean`,
...
| `X.§4` Additive Cech H1 and true-sheaf boundary relation | `defined only` / `proved` / `packaged (assumption-relative)`. `SemanticRepairCoverCechData`、`SemanticRepairAdditiveCechH1Data`、`SemanticRepairCoverH1BoundaryRelationAbelianData`、`SemanticRepairCoverH1BoundaryRelationAdditiveData`、`TrueSheafConditionCertificate`、`semanticRepairAdditiveH1Zero_iff_boundary`、`globalRepairCoherent_iff_additiveH1Zero` が selected bounded cover data 上の additive H1 zero と global semantic repair coherence の同値を証明する。 | true-sheaf certificate は cover membership と ambient sheaf condition を保持する明示 data であり、certificate-free に生成しない。 |
| `X.§6` Atom-generated site / cover-relative Čech bridge | `proved` / `packaged (assumption-relative)`. `atomGeneratedCoverage_generates_AATGrothendieckTopology`、`selectedAATSiteTopology_eq_atomGeneratedGrothendieckTopology`、`SemanticRepairCoverRelativeCoverBridge`、`SemanticRepairCover.toCoverRelativeCechCover`、zero-simplex incidence no-go / constructive boundary、`coverNerve_typedComponent_adequacy`、`aatSheafCondition_coverMembership_descent_effectiveGluing` を持つ。 | cover-relative Čech cover への bridge は selected zero-simplex incidence と selected cover membership に相対化され、bare cover bridge だけから incidence は作らない。 |
| `X.定義7.1 / 定理7.2 / 定理7.4` H1 comparison and nonzero transfer | `defined only` / `proved` / `packaged (assumption-relative)`. `SemanticRepairAdditiveH1Surface` は finite-free semantic additive H1 surface として定義され、`SemanticRepairCoverRelativeH1Comparison` と `SemanticRepairCoverRelativeCochainRealization.toH1Comparison` から `semanticRepairAdditiveH1_equiv_coverRelativeH1`、`semanticRepairAdditiveH1Zero_iff_coverRelativeH1Zero`、`semanticRepairAdditiveH1Nonzero_of_coverRelativeH1Nonzero`、`coverRelativeH1Nonzero_of_semanticRepairAdditiveH1Nonzero`、`semanticRepairAdditiveH1_coverRelativeH1_comparison_package` を証明する。 | comparison structure は degree-wise equivalence と differential compatibility だけを保持し、zero conclusion、global coherence、descent、law-grounded conclusion を field として持たない。nonzero transfer は theorem 7.2 zero-predicate equivalence の対偶として読む。 |
| `X.定理7.3` Grounded Global Gluing | `proved` / `packaged (assumption-relative)`. `SemanticRepairCoverH1BoundaryRelationAdditiveData.toAdditiveH1Surface` と `boundedAdditiveH1Zero_iff_surfaceH1Zero` で §4 bounded additive H1 と theorem-7.2 finite-free H1 surface を接続し、`SemanticRepairGroundedGlobalGluingPackage` / `trueSheafBoundaryRelationAdditive_coverRelativeH1Zero_effectiveGluing_package` が sheaf condition、descent、unique global section、H1 comparison package、`GlobalSemanticRepairCoherent ↔ CoverRelativeResidualH1Zero`、bounded additive H1 zero equivalence、later obstruction vanishings を返す。 | theorem は selected true-sheaf certificate、selected gluing data、selected cochain comparisonを前提にする。full sheaf cohomology comparison や refinement/naturality は主張しない。 |
| `X.定理7.5` Generated End-to-End SAGA packet | `proved` / `packaged (assumption-relative)`. `SemanticRepairGeneratedLawDependentConclusions` が law-equation grounding 由来の結論1〜3を保持し、`SemanticRepairGeneratedLawIndependentConclusions` が theorem 7.3 由来の結論4〜10を保持する。`SemanticRepairGeneratedEndToEndSAGAPacket` と `lawEquation_constructs_groundedComparisonPacket` は `LawEquationDefectSource.lawEquation_grounding_packet` と theorem 7.3 package を合成し、law 依存境界を statement 構造で分離する。`lawEquation_constructs_groundedComparisonPacket_fromRealization` は cochain realization から comparison を生成し、dependent-pair packet として返す。 | law-equation grounding は degree-zero conclusion group だけに依存し、descent / H1 / global coherence / higher vanishing は theorem 7.3 側の selected certificate と generated comparison から出る。Cycle 332 の巨大 route は本体へそのまま移植しない。 |
| `X.定理8.1 / 8.2 / 8.4 / 8.5` Boundary theorems | `proved` / `packaged (assumption-relative)`. `GeneratedSourceC0PointwiseZero`、`GeneratedSourceC0CechZero`、`GeneratedSourceC0ZeroPackage`、`displayedRequiredLawsHoldOn_constructs_generatedSourceC0_zeroPackage` が law fulfillment から degree-zero zero readings を構成する。`generatedSAGA_lawIndependentConclusions_withoutLawPremise` は law premise なしで theorem 7.5 の law-independent conclusion group を構成する。`SemanticRepairTypedComparisonTarget` / `SemanticRepairTypedZeroComparison` と `SemanticRepairRefinementZeroComparison` は comparison / refinement を explicit data とし、empty target、`typedComparisonTarget_carrier_nonuniformity_punit_zmod2`、coarse-zero/fine-nonzero の no-go を証明する。 | §8 theorem は zero preservation や law-independent conclusion を explicit selected data から読む。typed target、refinement comparison、cover-relative H1 comparison、cochain realization を無条件に構成しない。意味8.3 の conormal coefficient open question は R11/AC19 の future proof obligation 登録対象として残す。 |
| `X.例9.1` Lawful Firing Instance | `proved lawful firing instance over generated F2 coefficient`. `Formal/AG/Examples/SemanticRepairPart10.lean` が legacy singleton quotient presheaf の `quotientIsSheaf` を残しつつ、#3102 の完了根拠として `generatedF2QuotientPresheaf`、`finiteSiteTopology_cover_eq_top`、`generatedF2QuotientIsSheaf`、`generatedF2QuotientObstructionSheaf`、`generatedF2QuotientPresheaf_nontrivial` を追加する。`lawfulObject_noCycleLaw`、`defectSource.DisplayedRequiredLawsHoldOn`、`generatedF2TrueSheafCertificate`、`generatedF2H1Realization` から `lawfulFiring_generatedF2_endToEndPacket_fromRealization` が theorem 7.5 の generated-comparison route を発火する。`AxiomAudit.semanticRepairExample91GeneratedF2EndToEndFromRealization` が audit entrypoint で同じ theorem を elaboration する。 | 例9.1 は lawful zero firing witness であり、circle nerve 非零 class / transfer packet は例9.2 の別 witness で扱う。#3102 の completion witness は `ZMod 2` 係数の generated quotient route であり、legacy `PUnit` singleton route は完了根拠として扱わない。一般 quotient sheaf comparison は explicit data なしに構成しない。 |
...
| `X.例9.1` Lawful Firing Instance | `proved lawful firing instance over generated F2 coefficient` |
...
PRD-R R0 では、旧 PRD 1-9 の acceptance criteria を
[`lean_ag_peer_review_inventory.md`](lean_ag_peer_review_inventory.md) の三分化語彙で再判定する。
ここでの再判定は PRD 本文を書き換えず、査読時に `proved` と
`packaged (assumption-relative)` / `statement-only` を混同しないための台帳記録である。

| PRD | AC item | AC 再判定 | R0 status | 根拠宣言・根拠行 |
| --- | --- | --- | --- | --- |
| PRD-1 | AC11 | 旧 `ThreeReadingAgreementAssumptions` 版は explicit package field 由来。PRD-R R2 で required-law concrete counterpart が証明済み。 | `proved` / `packaged (assumption-relative)` | `Formal/AG/Atom/ThreeReading.lean`, `concreteThreeReadingAgreement`, `ThreeReadingAgreementAssumptions.*`; 詳細は `lean_ag_peer_review_inventory.md` I-1。 |
| PRD-1 | AC13 | 旧 `exists_ofComponents` は selected components を束ねる theorem package。PRD-R R2 で `AtomTowerRealization` と HEq-free counterpart が証明済み。 | `proved` / `packaged (assumption-relative)` | `Formal/AG/Atom/AATCore.lean`, `exists_ofAxiomRealization_noHEq`, `AATCorePackage.*`; 詳細は I-2。 |
| PRD-2 | AC5 | PRD-R R3 で seed-side support / axis coverage、required witness support、seed-overlap boundary visibility から closed admissible cover と `U`-adequacy を構成する additive counterpart を証明済み。旧 `WitnessClosureCover` 版は Research-compatible packaged surface として残る。 | `proved` / `packaged (assumption-relative)` | `Formal/AG/Site/Adequate.lean`, `SeedWitnessClosureCover.toAATCoverageFamily_admissible`, `SeedWitnessClosureCover.uAdequate`, `witnessClosureCover_uAdequate`; 詳細は II-1。 |
| PRD-2 | AC11 | finite model examples は selected finite fixtures に相対化される。PRD-R R3 II-5 で singleton fixture に加えて two-patch + overlap finite site firing、unit descent、selected sheafification gap、nonzero Boolean Čech differential calculation を追加済み。 | `proved` / `packaged (assumption-relative)` | `Formal/AG/Examples/FiniteModel.lean`, `siteSingletonCover_uAdequate`, `twoPatchCover_uAdequate`, `twoPatchUnit_descent`, `twoPatchSheafificationGap`, `twoPatchSeparatedCochain_differential_nonzero`; 詳細は II-5。 |
| PRD-3 | AC12 | Nullstellensatz candidate は statement surface に留まり、証明済み theorem ではない。 | `statement-only` | `Formal/AG/LawAlgebra/Nullstellensatz.lean`, `ArchitectureNullstellensatzCandidate`; 詳細は III-5。 |
| PRD-3 | AC18 | 定理11.1 Lawfulness-Ideal Correspondence は selected witness / coverage package に相対化される。 | `packaged (assumption-relative)` | `Formal/AG/LawAlgebra/Correspondence.lean`; 詳細は III-1。 |
| PRD-4 | AC3 / AC4 | Čech complex / finite-poset comparison は selected complex data と explicit d^2 field に相対化される。 | `packaged (assumption-relative)` | `Formal/AG/Cohomology/CechComplex.lean`, `Formal/AG/Cohomology/FinitePosetComparison.lean`; 詳細は IV-1。 |
| PRD-4 | AC11 / AC13 | flatness criterion and pseudo-circle H1 example are package/fixture-relative. R5 IV-4 で additive `H^1` vanishing と finite `C⁰` adjustment equivalence を追加済み。 | `proved under explicit finite C0 adjustment data` / `packaged (assumption-relative)` | `Formal/AG/Cohomology/FlatnessCriterion.lean`, `Formal/AG/Cohomology/FiniteExamples.lean`, `CoverRelativeH1NonzeroWitness`; 詳細は IV-2 / IV-4。 |
| PRD-5 | AC5 / AC6 | finite free / Taylor resolution exactness contains package assumptions to be hardened. | `packaged (assumption-relative)` | `Formal/AG/Derived/TaylorResolution.lean`, `Formal/AG/Derived/FreeResolution.lean`; 詳細は V-3 / V-4。 |
