...
計算可能なのは、選ばれた site、cover、coefficient、witness family、standard / selected ideal data、
...
```text
M =
  architecture object A_M
  architectural equation system E_M
  architecture signature Sig_M
  coverage requirements R_M
  context-overlap package Ov_M : ContextOverlapPullback(ArchCtx(A_M))
  finite AAT site X_M := Site_AAT(A_M,E_M,Sig_M,R_M,Ov_M)
  selected (E_M,R_M,Ov_M)-adequate cover U_M
  coefficient ring or field k_M
  coefficient effectiveness data EffCoeff_M
  coefficient sheaf Ob_M
  law display U := U_{E_M}
  standard obstruction ideal sheaf I_Ob^{E_M} generated from E_M
  selected witness coordinates Coord_M
  selected measurement ideal sheaf I_M^sel generated by Coord_M
...
`X_M`、`U_M`、`I_Ob^{E_M}` は同じ `E_M`、`Sig_M`、`R_M`、`Ov_M` から得る。
`I_M^sel` は有限測定に選ばれた coordinate だけから生成される auxiliary ideal であり、
標準 obstruction ideal `I_Ob^{E_M}` と定義上は同一でない。標準 lawfulness または標準 obstruction ideal
について結論する定理では、
...
をその regime で証明するか、明示的な comparison hypothesis として置く。
...
the selected measurement ideal I_M^sel, and every standard obstruction ideal used by a verdict,
  are finitely generated.
selected Tor computations use finite free, Koszul, Taylor, Scarf, or monomial resolutions.
```

この regime では、Čech cochain group、differential、`M` が選んだ ideal、selected Tor object は
有限データとして表せる。
...
the chosen cover is (E_M,R_M,Ov_M)-adequate for selected equation coordinates, witnesses, and axes.
...
square-free selected measurement ideal I_M^sel
...
を対応させ、selected measurement ideal を

```text
I_M^sel = < x_S | S in Forb_U >
```

と置く。
これは `Coord_M` 上の有限測定対象である。`I_M^sel = I_Ob^{E_M}` が証明された regime に限り、
この ideal を標準 obstruction ideal の square-free presentation として読む。
...
`I_M^sel` が finite square-free witness regime から来るとする。
...
law compatibility:
  rho^{-1} I_Ob^{E_Y} -> I_Ob^{E_X}

selected measurement compatibility:
  rho^{-1} I_Y^sel -> I_X^sel
...
standard obstruction ideals and selected measurement ideals pull back isomorphically.
...
したがって、`||h(g)|| > 0` なら、site、coefficient sheaf、profile の standard / selected ideal data を
変えない local adjustment だけでは
