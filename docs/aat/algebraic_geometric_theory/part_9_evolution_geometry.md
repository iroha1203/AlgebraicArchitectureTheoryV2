...
固定された trace category、state transition presheaf、temporal coefficient、law universe、
measurement profile の中で、時間方向の law と obstruction を読む。
...
`TempCoeff_A` は、state や transition の差が abelian coefficient として読める selected regime を
...
`ob(F)` の構成、coefficient exactness、trace product site、
