...
| `degree-zero equation contribution` | certified bounded inference | displayed equation fulfillment の Čech 複体への寄与が、次数 0 の pointwise 消滅であることを読む主張。site、cover nerve、係数、displayed residual の構成は fulfillment に依存せず、`H^1` comparison やその消滅は別の comparison data なしには従わない。 |
