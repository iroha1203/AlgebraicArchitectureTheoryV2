...
r : CoreRead(At)
  including
    ...
    LawUniverse U_r
    finite detector code, Boolean circuit semantics, and soundness
    invariant / signature reading
    operation reading

J : CoverageTopology for U_r
k : coefficient ring
```
...
と書くことがある。
source、doctrine、object / law / operation / signature reading、`J`、`k` のいずれかが変われば、
...
selected restriction-stable local circuit family
...
が extraction、composition、object formation、law role、signed-query matching と Boolean acceptance の同値、
signature、actual configuration operation を保存するとき、Part I の relative core construction は
...
required roleを保存するclosed-equational law universeのinclusionを固定する。
...
すなわち、各 test object `T` と section `s : T -> X` に predicate `HoldsOn_L(T,s)` を与え、
base change に対して安定であることを要求する。Part I の object-level `L(A)` とは、
selected architecture point `a_A` における comparison

```text
L(A) iff HoldsOn_L(T_A,a_A)
```
...
によって表現される。
ここで「表現される」とは、任意の section `s : T -> X` に対して
...
が成り立つ `LawIdealExact(L)` が与えられていることを意味する。
`s^*_{ideal} I_L` は `s^#(I_L)` が `O_T` の中で生成する extension ideal sheaf であり、
module pullback ではない。soundness と completeness はこの同値の各向きとして別々に読む。
...
として読む。
required closed-equational law ごとの exactness の下で、ideal-theoretic lawful locus は
...
| 語 | status | 読み方 |
| --- | --- | --- |
| `Atomizes_D / Atomize_D` | mathematical definition / Lean 未実装 | `Atomize_D(s).mem(a) iff Extracts_D(s,a)` で family を定義し、`Atomizes_D(s,F)` をその membership characterization とする。canonicality と A8 uniqueness は field ではなく extensionality theorem である。 |
| `Core_At : CoreRead(At) -> ObjectAlgebra(At)` | mathematical construction / Lean 要求未達 | admissible reading の rule を評価して operation-closed object family と indexed operation / circuit family を構成する。axiom system `S` は package で carrier を認証し、`CoreRead` の phantom parameter にはしない。 |
| `ConfigurationHom / Operation` | mathematical definition / Lean 未実装 | operation は atom map と family membership、relation、identification の transport を持つ actual configuration homomorphism である。現行の arbitrary `Prop`-valued map field は A7 の証明ではない。 |
| `CoreExact_At(phi)` | local theorem candidate / Lean 未証明 | `SignedExactCoreReadingHom` から object-algebra homomorphism を導き、identity と composition を保存する。matching / acceptance の同値により negative query も transport する。 |
| `PositiveCoreReadingHom` | mathematical definition / Lean 未実装 | atomic truth の一方向保存から positive circuit subfamily だけを運ぶ。signed circuit 全体、coarsening、projection の exact comparison とは区別する。 |
| `circuit soundness` | defined predicate / Lean 未実装 | finite signed-query datum `Q` の matching と finite-template detector code の Boolean acceptance から semantic law failure を導く条件。detector code は object / law-failure proof を格納できない。 |
| `required circuit completeness` | certified assumption / Lean 未実装 | 各 required semantic law failure が少なくとも一つの finite circuit presentation を持つこと。core generation 自体の必須条件ではない。 |
| `Circ_U^loc` | selected restriction data / Lean 未実装 | restriction map、identity、composition、各local object上のPart I circuit familyへのinclusion、matching / Boolean acceptance preservationを持つ。global signed circuitの無条件なrestrictionとは区別する。 |
| `circuit realization faithfulness` | certified assumption / Lean 未実装 | local circuit が対応する local point / evaluation 上で非零の witness-ideal generator に写ること。coefficient への map は natural `rho : I_L -> Ob_U` を経由し、`kappa(c)=rho(x_real(c))` を満たす。 |
...
| `zero-reflecting aggregation` | defined predicate | `omega_U(A)=0 iff forall required i in U, omega_i(A)=0` を保証する集約条件。selected law 全体の集約は `omega_U^all` と区別する。 |
| `GeometricLawReading` | mathematical definition / Lean 未実装 | `HoldsOn_i(T,s)` を base change-stable subfunctor predicate として与え、selected architecture point 上で Part I の `L_i(A)` と comparison 同値を持つ。 |
| `SemanticLawful / IdealLawful` | defined predicates + local theorem candidate / Lean 未証明 | required law ごとの `ClosedEquational_U(i)` と `LawIdealExact_U(i)` の下で、section predicate と extension ideal vanishing を同値にする。valuation / axis exactness は別の仮定である。 |
...
required lawをただ一つ、`Viol_L(W)`にselected witness `v`を一つ取り、
そのviolation coordinateを`coord_{L,W}(v) = 2`とする。
...
商はideal membership classを非自明に分離する。
