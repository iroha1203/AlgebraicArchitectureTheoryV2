...
```text
O_raw^U : ArchCtx(A)^op -> CommAlg_k
O_X^U   = (O_raw^U)^+
```
...
重要なのは、`O_raw^U(W)` の段階では selected law witness ideal を zero にしていないことである。
...
と読める。しかし、一般には cancellation を避けるため、`I_L(W)` の vanishing を primary encoding とする。
...
を使ってよい。
しかし、標数や符号付き係数によって witness の和が cancellation する可能性があるため、
...
第III部では、obstruction は selected law witness ideal の和として読む。

```text
obstruction
  =
sum of selected law witness ideals
```
...
したがって、`Spec_AAT` の点は AAT 固有の新しい点ではなく、通常の prime ideal に
selected AAT reading metadata を付けて読んだものである。
...
したがって、`Spec_AAT(O_X^U(W), D_W^U)` は無条件に raw moduli functor を表現するのではない。
...
これは scheme を弱めた代替物ではなく、affine chart、lawful locus、derived intersection を載せる
基礎階である。
...
として読める。
underlying chart は通常の affine scheme `Spec(O_X^U(W))` であり、AAT 固有の data は decoration である。
...
この定義により、`ArchitectureScheme` は単なる ringed site の別名ではない。
decorated `Spec_AAT` chart が、underlying scheme 側では open immersion と cocycle condition で貼れ、
...
これは「すべての architecture が絶対的に正しい」という定理ではない。
selected witness ideals が sound and complete に law failure を表現するなら、
