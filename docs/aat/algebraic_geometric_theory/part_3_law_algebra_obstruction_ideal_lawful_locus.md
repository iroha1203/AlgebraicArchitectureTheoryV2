...
J_struct(W)
  =
ideal generated by structural relation polynomials in FreeCommAlg(Coord_X(W)).
```
...
O_raw^U(W)
  =
  FreeCommAlg(Coord_X(W)) / J_struct(W)
```
...
点 `p in Spec_AAT(O_X^U(W))` または section `s` に沿って law `L` が成立するとは、
...
locally ringed architecture geometry である。
...
として読める。さらに chart overlap は定義 9.2 の chart compatibility を満たす。
...
`Spec_AAT` chart が scheme-compatible に貼れる場合の強い対象である。
