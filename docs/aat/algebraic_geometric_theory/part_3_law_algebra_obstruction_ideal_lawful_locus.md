...
witness coverage、E-generated scheme realization は、それぞれ failure の finite detection、generator の非零評価、
...
### 定義 5.2A Equation Fulfillment on Scheme Charts

selected ringed / scheme regime では、context と scheme chart の対応を先に固定する。
...
`chi` は context morphism `j : W' -> W` を open immersion `X_{W'} -> X_W` へ送り、
identity と composition を保存する。chart family `{X_W}` は `X` を jointly cover する。
sheafification unit と chart identification の合成を

```text
eta_W : O_E(W) -> Gamma(X_W,O_X^U)
```

と書く。`eta` は `O_E -> O_X^U` の presheaf morphism の component なので restriction と可換する。

scheme-theoretic section `s : T -> X` に対して
...
と置く。section evaluation は自由な写像として供給せず、sheafification unit と chart pullback の合成
...
ev_{s,W}
  := Gamma(s_W^#) compose eta_W
```

で構成する。`j : W' -> W` が誘導する `T_{W'} -> T_W` の restriction を `res^T_j` と書けば、
naturality により

```text
ev_{s,W'} compose res_j
  =
res^T_j compose ev_{s,W}
```

が成り立つ。

section `s` が選ぶ architecture reading を `A_s` とする。任意の `f : T' -> T` に対して
`A_{s compose f}` を `A_s` の pullback reading と同一視し、その指定が identity と composition に
整合するものを base-change-stable section reading と呼ぶ。section 上の residual は
...
であり、fulfillment と lawfulness を次で定義する。
...
base change `f : T' -> T` が誘導する `T'_W -> T_W` を `f_W` と書くと、section reading の
base-change stability と sheaf morphism の composition から

```text
epsilon_{W,i,a}(s compose f)
  =
f_W^#(epsilon_{W,i,a}(s))
```

が成り立つ。環準同型 `f_W^#` は零を零へ送るため、section fulfillment は base change で保存される。

selected architecture point `a_A : T_A -> X` と第I部の object reading を比較するときは、各 `W` に

```text
kappa_{A,W} : Gamma((T_A)_W,O_{T_A}) ≅ O_E(W)
```

を構成し、`kappa_{A,W}(epsilon_{W,i,a}(a_A)) = epsilon_{W,A,i,a}` と restriction naturality を証明する。
この point-realization theorem から
...
が従う。`kappa_{A,W}` は定義5.2Bの architecture-evaluation representing equivalence の component から
構成し、その component を持たない object にこの同値を主張しない。同値そのものを architecture point の
field として供給しない。

### 定義 5.2B E-Generated Scheme Realization

symbolic coordinate と object-dependent residual の scheme 上の接続は generator 水準で固定する。
上の chart / evaluation construction に対し、residual representability property を

```text
ResidualRepresentable_E(X)
  iff
forall T, s : T -> X, W, i, a,
  ev_{s,W}(nu_{W,i,a})
    =
  ev_{s,W}(epsilon_{W,A_s,i,a})
```

と定義する。これは vanishing の同値や zero-locus factorization を格納せず、universal symbolic
coordinate が architecture section で object residual に specialize することだけを述べる。

producer の source functor はまず各 context chart について `E` から定義する。可換環 `B` に対し

```text
ArchEvalPoint_{E,W}(B)
  :=
  { base-change-stable architecture reading A_B,
    ring map e_{B,W} : O_E(W) -> B }

c_{W,i,a}(A_B,e_{B,W})
  :=
  e_{B,W}(nu_{W,i,a})
    - e_{B,W}(epsilon_{W,A_B,i,a})

EqPoint_{E,W}(B)
  :=
  { p in ArchEvalPoint_{E,W}(B)
    | c_{W,i,a}(p) = 0 for all i,a }
```

環準同型 `B -> B'` は `A_B` と `e_{B,W}` を base change する。context morphism `j : W' -> W` には
`res_j` による chart transition を対応させ、その identity / composition と `epsilon` の restriction
compatibility を使って chartwise functors を接続する。`EqPoint_{E,W}` は、`E` の二つの coordinate family の
difference が生成する realization ideal の zero subfunctor であり、lawful locus の vanishing や witness ideal
membership は source functor に含まれない。

realization ideal `<nu-epsilon>` は `X_E` 上で symbolic coordinate の specialization を確定する。
witness ideal `<nu>` は、その `X_E` の内部で equation fulfillment を切り出す。前者を零にしても `nu` 自身は
零にならないので、realization construction と lawful closed subscheme construction は別の段階である。

selected representable scheme regime では、standard constructor は `ArchEvalPoint_{E,W}` の representing
chart と、各 function `p |-> e_p(epsilon_{W,A_p,i,a})` の regularity theorem を構成する。
これにより difference coordinates `c_{W,i,a}` は regular function になる。その生成 ideal が定める
closed equalizer chart `X_{E,W}` を取り、
context transitions に沿って glue して `X_E` を構成する。universal ring map を
chart section `eta_W` とし、section `s : T -> X_E` による pullback を定義5.2Aの `ev_{s,W}` とする。
したがって `A_s` と `ev_{s,W}` は representing equivalence から読み出され、
`residualRepresentable_E` は universal equality の componentwise pullback として証明される。
別の evaluation family は残らない。

`E`-generated scheme realization とは、standard equation-scheme constructor が `E`、その observable
presheaf、`nu` が生成する ideals から `X_E`、chart functor `chi_E`、sheafification unit `eta_E`、
base-change-stable section reading を構成し、さらに named producer theorem

```text
J_i^E(W)
  := < eta_W(nu_{W,i,a}) | a in At >
     subset Gamma(X_W,O_X^U)

residualRepresentable_E
  : ResidualRepresentable_E(X_E)

residualRegular_E(W,i,a):
  p |-> e_p(epsilon_{W,A_p,i,a})
  is regular on the representing chart of ArchEvalPoint_{E,W}

witnessIdealLocalizes_E(i,j):
  Gamma(X_{W'},O_X^U) tensor_{Gamma(X_W,O_X^U)} J_i^E(W)
    ≅ J_i^E(W')
  for j : W' -> W

witnessIdealChart_E(i,W):
  𝓘_i^E restricted to X_W
    ≅ the ideal sheaf associated to J_i^E(W)

witnessIdealQuasiCoherent_E(i):
  𝓘_i^E is a quasi-coherent ideal sheaf on X_E
```

を証明した出力をいう。`residualRegular_E` は concrete residual definition の algebraic dependence を
証明する producer theorem であり、free truth predicate ではない。`residualRepresentable_E` は
standard constructor の結果であり、独立の geometric truth predicate、別供給の coordinate family、
manual bridge を primary input にしない。
`witnessIdealLocalizes_E` は chart restriction と localization の比較から証明し、
`witnessIdealChart_E` は sheafification image と `eta_W(nu)` の生成 ideal の一致を生成元上で証明する。
`witnessIdealQuasiCoherent_E` はこれらの chartwise data を glue して得る。
selected scheme regime でこれらの producer theorem を構成できない場合、equation fulfillment と
ideal vanishing の対応は未証明のままであり、次の定理を適用しない。

ringed / scheme regime で `s : T -> X_E` と ideal sheaf `I subset O_{X_E}` に対し、

```text
s^*_{ideal} I
```

は `s^#(I)` が `O_T` の中で生成する extension ideal sheaf を表す。module pullback とは区別する。

### 定理 5.2C Generated Section-Ideal Comparison [Certified bounded inference]

`E`-generated scheme realization と、その producer theorems `residualRegular_E`、`residualRepresentable_E`、
`witnessIdealLocalizes_E`、`witnessIdealChart_E`、`witnessIdealQuasiCoherent_E` を固定する。
任意の equation index `i` と section `s : T -> X_E` について
...
が成り立つ。また、`𝓘_i^E` が定める closed subfunctor を `V(𝓘_i^E)` と書けば

```text
EquationHolds_E(i;s)
  iff
s factors through V(𝓘_i^E)
```
...
`𝓘_i^E` は `eta_W(nu_{W,i,a})` の sheaf-image が生成する ideal sheaf であり、chart family は
jointly covering なので、extension ideal の零性は全 local generator の評価が零であることと同値である。

```text
s^*_{ideal} 𝓘_i^E = 0
  iff
forall W,a, ev_{s,W}(nu_{W,i,a}) = 0
```

producer theorem の generator equality により右辺は
...
と同値であり、これは `EquationHolds_E(i;s)` の定義である。closed subfunctor への factorization は
extension ideal の零性に関する標準的な zero-locus universal property から従う。
対応同値は premise に置かれず、sheafified generator、section evaluation、producer theorem、
ideal generation の順に証明される。
...
で定義する。`E`-generated scheme realization では、equation fulfillment との同値を
定理 5.2C の generator-level proof から得る。
...
Closed equational law:
  E-generated scheme realization の下で、equation residual の消滅と
  witness ideal I_i^E の vanishing の対応として読む law。
...
別の概念であり、この段階では `IdealLawful_U(s)` と同一視しない。両者の同値は定理11.1で、
standard constructor が生成する `residualRepresentable_E` と定理 5.2C から得られる。
...
equation system `E` から定義5.2Bの standard constructor が `E`-generated scheme realization
`X_E` と generator / localization producer theorems を構成したとする。さらに、required indexed sum と
extension ideal の可換性、および selected ringed / scheme regime で
...
が成り立つとする。このとき、architecture section `s : T -> X_E` について次は同値である。
...
s factors through Flat_U(X_E) = V(I_Ob^U)
```
...
定理 5.2C により、これは law-by-law に extension ideal vanishing と同値である。
...
generator-level realization theorem を、その axis の standard constructor から別途構成する。
equation system 全体の index に対して同じ producer theorem を使うと
...
### 原則 11.2 Scheme Comparison Has Producer Provenance

定理11.1の同値は、standard constructor が生成した chart functor、sheafification unit、section evaluation、
base-change-stable section reading、`residualRegular_E`、`residualRepresentable_E`、`witnessIdealLocalizes_E`、
`witnessIdealChart_E`、`witnessIdealQuasiCoherent_E`、extension ideal と indexed sum の可換性に相対化される。
cover から計算する場合は、`E`-adequacy と ideal-sheaf descent を明示する。

residual vanishing と witness ideal vanishing の同値そのものを predicate、certificate field、manual bridge
として受け取らない。必要な material input は generator equality の producer theorem まで分解し、
その theorem が `E` と generated witness ideals からどの standard constructor により得られたかを記録する。
producer theorem が未構成の regime では定理11.1を完了根拠に数えない。
...
cover `𝒰 = {W_u -> W_base}_{u in I}` に対する cover-indexed displayed equation source では、
`D := I` とし、source の local context `W_u` を cover chart そのものに取る。各 chart の
architecture object、required equation index、support Atom はこの同じ source に格納する。
別の skeleton や二つ目の chart-indexed equation choice は置かない。
...
residual restriction naturality:
  context morphism g : Z -> W_q に対し
  d_{q|Z} := epsilon_{Z,A_q,i_q,a_q}
  と置くと、
  res_g^Q(interpret(q)) = [res_g(d_q)] = [d_{q|Z}]
  が成り立つ。
...
最初の restriction 結論を residual restriction naturality、最後の結論を zero restriction evaluator と呼ぶ。
どちらも入力の仮定ではなく、定理の結論である。
...
residual restriction naturality は、`epsilon` の restriction compatibility

```text
res_g(epsilon_{W_q,A_q,i_q,a_q})
  =
epsilon_{Z,A_q,i_q,a_q}
```

と、`nu` の restriction compatibility から商へ降りた `res_g^Q` の代表元計算を使う。
...
最後の射は quotient map が零を零へ送ることによる。`i_q` が required であることは
`I_{i_q}^E` が `I_Ob^E` の summand であることと、detector が required equation lawfulness を読むことに使う。
vanishing 自体には ideal membership の迂回を使わない。overlap 上の等式や descent は消費しない。

faithfulness は商環の一般論である。

zero restriction evaluator は、各 source の residual restriction naturality と vanishing により、
共通細分上の両辺が零 class になることから従う。
...
その interpretation class を生成する。`nu` の restriction compatibility と商の普遍性が quotient restriction を、
`epsilon` の restriction compatibility が residual restriction naturality を与える。fulfillment はその自然な class を
零へ送り、zero restriction evaluator を与える。required role は selected index を obstruction ideal の summand と
required lawfulness に接続する。各 component が証明のどこで使われるかをこの順序で区別する。
...
the standard scheme producer derives its correspondence with ideal vanishing.
