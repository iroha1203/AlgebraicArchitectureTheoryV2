...
と書く。空の pullback は積から除き、その値は定理 1.1 の
empty-overlap normalization で固定する。以下の比較 core 自体には有限性は要らないが、
有限 cover は中心定理、finite witness、実行可能な realization を同じ記号で
扱うために固定する。
第II部 命題 4.2 の finite-meet poset model ではすべての射が monomorphism なので、
この仮定は自動的に満たされる。一般の `ArchCtx(A)` では定理の明示仮定である。

本部では入力、構成、帰結を次のように区別する。

| 種類 | 内容 |
| --- | --- |
| selected | monomorphic AAT cover、empty-overlap normalization、semantic atom system、repair support、local repair relation、SAGA presentation exactness、local repair atlas、equation system `E`、local equation-lift atlas、Atom/equation interpretation、local-state interpretation `β` |
...
8. 積から除いた各 empty cover intersection `V` に対する
   ...
   これを empty-overlap normalization と呼ぶ。
...
### 補題 2.1A Monomorphic-cover normalization

第IV部 定義 3.2 の全 ordered tuple を使う Čech complex を
`\widetilde C^\bullet(𝒰,F)` と書く。`𝒰` を monomorphic AAT cover とし、
積から除いた empty intersection `V` では `F(V)=0` とする。
その differential の kernel と image を
`\widetilde Z^1(𝒰,F)`、`\widetilde B^1(𝒰,F)` と書き、商を
`\check H^1_{\mathrm{ord}}(𝒰,F)` と書く。
このとき increasing indices への restriction は

```math
\widetilde Z^1(\mathcal U,F)
  \xrightarrow{\sim}
Z^1(\mathcal U,F),
\qquad
\widetilde B^1(\mathcal U,F)
  \xrightarrow{\sim}
B^1(\mathcal U,F)
```
...
```math
\check H^1_{\mathrm{ord}}(\mathcal U,F)
  \xrightarrow{\sim}
...
さらに presheaf of sets `P` が除外した empty intersection 上で subsingleton なら、
族 `(p_i)` が全 ordered overlap 上で matching family であることと、
各 nonempty `i<j` 上で一致することは同値である。

#### 証明

`u_i` が monomorphism なので、diagonal

```math
U_i\xrightarrow{\sim}U_i\times_W U_i
```

は同型であり、二つの projection はこの同型の下で一致する。
全 ordered complex の `1`-cocycle `c` に cocycle equation を
tuple `(i,i,i)` で適用すると `c_{ii}=0` を得る。
tuple `(i,j,i)` で適用すると、canonical symmetry
`U_{ji}\cong U_{ij}` の下で

```math
c_{ji}=-c_{ij}
```

を得る。相異なる三添字の permutation に対する cocycle equation は、
この反対称性と fiber-product symmetry により、その添字を昇順に並べた equation の
符号付き像である。添字が重複する equation は `c_{ii}=0` と
`c_{ji}=-c_{ij}` に帰着する。

したがって ordered `1`-cocycle は `i<j` 成分だけで一意に決まり、
increasing-index cocycle は、diagonal 成分を零、逆順成分を負として一意に延長できる。
`0`-cochain group は両 complex で同じであり、

```math
(\delta^0a)_{ii}=0,\qquad
(\delta^0a)_{ji}=-(\delta^0a)_{ij}
```

なので coboundary も同じ対応を満たす。よって `Z^1`、`B^1`、その商がそれぞれ同型である。

local-state family についても、self-overlap 上の二つの restriction は monomorphism により
同じであり、逆順 overlap の equality は symmetry によって `i<j` の equality と同値である。
empty overlap 上の equality は subsingleton 性から一意である。よって最後の同値を得る。∎
...
補題 2.1A により、定理 1.1 の monomorphic AAT cover では第IV部の
全 ordered tuple を使う cover-relative Čech `H¹` と同一視できる。
...
monomorphism と empty-overlap normalization は、補題 2.1A による第IV部の
ordered Čech model との同一視、および次節の actual gluing で使用する。
...
3. `𝒰` がその topology に属する monomorphic AAT cover である。
4. 積から除いた empty intersection `V` 上で `P_sem(V)` は subsingleton である。
...
`p_i^{corr}=(-a_i)+p_i` と置く。この family は各 nonempty `i<j` overlap 上で一致する。
補題 2.1A により、monomorphism が self-overlap と逆順 overlap の equality を与え、
empty-overlap normalization が除外した empty overlap の equality を与える。
したがって corrected family は全 ordered overlap 上の matching family である。
...
を持ち、nondegenerate triple overlap を持たない monomorphic 4-cycle cover を取る。
