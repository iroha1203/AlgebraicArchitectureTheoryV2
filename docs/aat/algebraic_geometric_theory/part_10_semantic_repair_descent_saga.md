...
`P_E` も選ばれた topology 上の sheaf であるとする。定理 6.5 により
...
定理 4.3 と定理 5.4 の torsor structure は、selected local states から
...
最も強い既存資産である。その役割を declaration 単位で固定する。

| G-07 declaration | 本部での扱い | 数学的役割 |
| --- | --- | --- |
...
G-07 の Lean route は `LawGeneratedLargeCoefficientCech`、
sheafified short exact sequence、local-lift difference、sheaf amalgamationから構成される。
`LawGeneratedLargeCoefficientCech` は generic な `Formal.AG.Cohomology.CochainComparison` を
使うが、`SemanticRepairCechGrounding`、`SagaComparison`、`H1Comparison` の
SAGA-specific route を import しない。したがって本部で改訂した一般 SAGA comparison を
premise としていない。
本部はその proof pattern を一般化し、定理 9.1 で G-07 を特殊例として再読する。
...
## 11. Paper claim、Lean status、結論

### 11.1 Claim/status table

| claim | 数学本文 | Lean status |
| --- | --- | --- |
| semantic presentation `M_sem` と independently generated complex | 本部 定義 3.1–3.4、命題 3.3 | `Formal/AG` では未証明。Issue #3757 |
| semantic mismatch からの cocycle と choice independence | 本部 定義 4.2、補題 4.3、定理 4.4 | generic statement は `Formal/AG` では未証明。G-07 の `localLiftDifferenceFor_cocycle`、`connectingClass_choice_independent` は `unported (Research-proved)` |
| `Q_E` と restriction の equation generation | 第III部 定理 11.4、本部 定義 5.1 | `Formal/AG/LawAlgebra/LawEquation.lean` の `obstructionQuotientRestrict`、`obstructionQuotientCoefficient`、`obstructionQuotientPresheaf`、`generatedCoefficient_package`、`interpret` |
| primary presentation から `M_sem≃Q_E` | 本部 定理 6.3、系 6.7 | `Formal/AG` では未証明。Issue #3757 |
| cochain commutation と `H¹` comparison | 本部 定理 7.2、7.4 | `Formal/AG/Cohomology/CochainComparison.lean` は supplied cochain equivalence からの商上輸送を形式化済み。SAGA exact presentation から comparison を構成する段は Issue #3757 |
| residual class correspondence | 本部 定理 7.5 | `Formal/AG` では未証明。Issue #3757 |
| zero class と actual global repair | 本部 定理 8.2 | G-07 の `connectingClassFor_isZero_iff_nonempty_globalLift` と `connectingClass_isZero_iff_nonempty_semanticFirstOrderRepair` は `unported (Research-proved)` |
| G-07 conormal package | 本部 定理 9.1 | `lawGeneratedConormalFirstOrderDescent_package` は `ResearchLean` で証明済み。本体への蒸留は Issue #3757 |
| circle zero/nonzero witness | 本部 例 10.2、付録 B.9 | `conormalH1_zero_nonzero_pair` を含む G-07 Boolean-circle witness は `unported (Research-proved)` |

`lake build` の成功、declaration 名の存在、status 表の更新は、
未証明の general SAGA statement の代替にしない。

### 11.2 Abstract で使用する数学 claim

プレプリント初稿の Abstract では、数学 claim を次の強さで固定できる。
...
Lean については表 11.1 の status を独立に示す。一般 SAGA theorem 全体を
...
これが SAGA 中心定理の paper-level statement である。
