...
この gap は、lawful loci の非横断性から生じる構造的現象として現れうる。

## 9. No Monotone Repair Guarantee

### 原則 9.1 No Monotone Repair Guarantee
...
しかし、その repair comparison profile が `U`-axis の comparison だけを持ち、
`V`-axis についての monotonicity rule を含まないとする。

```text
no V-axis monotonicity certificate is assumed.
```

この profile の内部では、`U`-axis の obstruction 減少だけから、
`V`-axis の obstruction が非増加であるとは主張しない。
...
したがって、少なくともこの monomial regime では、
`U`-axis の obstruction 減少は `V`-axis の obstruction 非増加を含意しない。
...
selected derived conflict class is paired nontrivially with the repair direction.
```
...
これらは、単なる実装上の失敗としてではなく、lawful loci 間の derived non-transversality として
読める場合がある。
ただし、そのためには対象 law universe、conflict support、repair direction、transfer pairing を固定する。
...
と読む。
選ばれた conflict class の support を
...
と書く。
repair path または direction が support-localized であるとは、
その image、specialization、または first-order support が `Supp(kappa_{U,V})` と
選ばれた係数 regime の中で非自明に交わることをいう。
support-localized でない repair については、ambient `LawConflict_1(U,V) != 0` だけから
transfer residue を結論しない。
...
v is support-localized for kappa_{U,V}, or the pairing is otherwise justified on v.
