...
lawfulness は、定義の段階では obstruction absence と同一視しない。まず semantic lawfulness を置き、
...
ここで定義するのは architecture object 上の predicate である。scheme-theoretic section
`s : T -> X` 上の law predicate は第III部の geometric law reading が与え、selected
architecture point における comparison 同値によって `L_i(A)` と接続する。

optional law と derived law を含む selected law 全体を満たす場合は、区別して
...
と書く。これにより、required law による normative locus と、selected law 全体による
full locus を同じ law universe の上で比較できる。

semantic reading を明示する場合は

```text
SemanticLawful_U(A) := Lawful_U(A)
FullySemanticLawful_U(A) := FullyLawful_U(A)
```

と書く。semantic law predicate と、後に導入する circuit、valuation、ideal の vanishing は
この段階では別の概念である。
...
Law `L` に対する semantic obstruction は、`L(A)` の failure proposition である。

```text
SemanticOb_L(A) := not L(A)
```
...
列挙された有限 query だけであり、object 全体の law predicate ではない。

各 law `L` の circuit reading は、有限個の exact signed-query template からなる
...
Accepts_L(Q) := eval(code_L,Q) = true
```

`DetectorCode` の constructor は finite query datum とその有限選言しか持たず、
architecture object、law predicate、law failure proof を格納できない。
...
CircuitSound(L)
  iff
forall A Q,
  Matches(Q,A) -> Accepts_L(Q) -> not L(A)
```

からなる。`L` に対する obstruction circuit は、finite circuit datum と
検出の witness の組

```text
O = (Q, h_match, h_detect)
h_match  : Matches(Q,A)
h_detect : Accepts_L(Q)
```

である。law failure は `O` の field として格納せず、`CircuitSound(L)` を適用して導く。
...
semantic failure は detector code ではなく `CircuitSound(L)` の証明のみに現れる。

### 定義 8.2A Law-Indexed Obstruction Circuit Family

law universe `U` の各 law reading が `CircuitSound(L_i)` を備えるとする。
architecture object `A`、law index `i in U` に対して、`L_i` の finite
...
Circ_U^req(A)
  =
family { Circ_U(A,i) | i is required in U }
```

と書く。`Circ_U(A,i)` は law と object に添字付けられた finite presentation の
型である。soundness により、law が `A` 上で成立する場合、その fiber は空である。
...
law universe `U` の failure locus を次で定義する。

```text
Fail_U
  =
{ (A,i) | i in U and not L_i(A) }
```
...
TotCirc_U
  =
{ (A,i,O) | O in Circ_U(A,i) }
```

とする。各 circuit の matching witness、detection witness、`CircuitSound(L_i)` により、canonical
incidence map

```text
pi_fail : TotCirc_U -> Fail_U
pi_fail(A,i,O) = (A,i)
```
...
CircuitComplete(U)
  iff
forall (A,i) in Fail_U,
  Circ_U(A,i) is nonempty.
...
```text
pi_fail^req : TotCirc_U^req -> Fail_U^req

RequiredCircuitComplete(U)
  iff
pi_fail^req is surjective
```

と書く。`CircuitComplete(U)` は `RequiredCircuitComplete(U)` を含意するが、後者は
optional law の finite detectability を要求しない。`RequiredCircuitComplete(U)` は
required failure が有限 signed-query presentation を持つという追加の有限表示可能性条件であり、
circuit soundness や Atom 公理系だけからは従わない。

`Circ_U(A,i)` は architecture object 全体上の global signed-query family である。
...
Lawful_U(A)
  ->
forall required i in U,
  Circ_U(A,i) is empty.
```

さらに `RequiredCircuitComplete(U)` の下では逆も成り立つ。
...
law universe `U` と selected index support `J subset Index(U)` に対する集約

```text
Agg_{U,J} : (forall i in J, Value_U) -> Value_U
```

は zero-reflecting であると仮定する。

```text
Agg_{U,J}((v_i)_{i in J}) = 0
  iff
forall i in J, v_i = 0
```
...
```text
omega_{U,J}(A) := Agg_{U,J}((omega_i(A))_{i in J})

omega_U(A) := omega_{U,required(U)}(A)
omega_U^all(A) := omega_{U,Index(U)}(A)
```

と書く。添字を省略した `omega_U` は required law support 上の valuation であり、
`omega_U^all` は optional law と derived law を含む selected law 全体の valuation である。
個々の `omega_i` を circuit family から構成する場合、circuit realization と measurement の
...
```text
forall required i in U, omega_i is sound.
forall required i in U, omega_i is complete.
Agg_{U,required(U)} is zero-reflecting.
```
...
```text
Lawful_U(A)
  iff forall required i in U, L_i(A)
  iff forall required i in U, omega_i(A) = 0
  iff omega_U(A) = 0
```
...
同じ仮定を selected law 全体に課す場合は、full locus に対して

```text
FullyLawful_U(A) iff omega_U^all(A) = 0
```
...
operation の型、`Inv(A)` は `A` 上の invariant、`Law` は law universe、
`Circ(A,L)` は `A` 上の `L`-obstruction circuit の型、`Sig(A)` は
`A` の signature である。
...
configuration 成分が `C` に等しい ambient architecture object へ送る object-formation rule、`R_law` は law family、
law role、finite detector code、Boolean acceptance とその soundness、`R_inv` は selected invariant、
...
admissibility は、`Atomize_D(s)` の有限性、各 reading の型整合、A7 に従う
operation の Atom-origin preservation、`R_law` の circuit soundness を含む。
...
R_law
  -> LawUniverse U_r with sound circuit semantics

(Obj_r,U_r)
  -> law-indexed circuit family Circ_{U_r}(A,-)
     for A in Obj_r
...
この構成は実在する circuit を一つ選ぶのではなく、各 `(A,L)` に対する
`Circ(A,L)` を型族として構成する。同様に operation は `Op(A,B)` として構成され、
特定の非恒等 operation の存在は concrete model の性質として述べる。

固定された `source` と `extraction doctrine` に対する Atom family の一意性は A8 による。
full core の比較は、object、law、signature、operation reading まで固定した同値または
compatible change of reading に相対化される。

したがって、AAT は Atom 公理系と core reading の上で architecture object、law、
...
phi_L   : Index(U_r) -> Index(U_r')
phi_Q   : FiniteCircuitDatum_r -> FiniteCircuitDatum_r'
phi_Op  : Op_r(A,B) -> Op_r'(phi_Obj(A),phi_Obj(B))

required_r(i) iff required_r'(phi_L(i))
L_i(A) iff L'_{phi_L(i)}(phi_Obj(A))
Matches_r(Q,A) iff Matches_r'(phi_Q(Q),phi_Obj(A))
Accepts_r(i,Q) iff Accepts_r'(phi_L(i),phi_Q(Q))
```
...
object algebra の homomorphism は、object map、operation map、law index map、
circuit map、invariant / signature transport とそれらの可換性からなる。
