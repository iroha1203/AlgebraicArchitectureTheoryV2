...
source s + Atom axiom system S + core reading r
  ...
  -> AATSite(A_s^r, E_r, J_E)
  -> ArchitectureGeometry X_s^{r,J}
```

`X_s^{r,J}` は、extraction doctrine、object reading、architectural equation system、
operation / signature reading、coverage topology に相対化された、局所-大域的な architecture geometry である。
...
coverage topology `J_E` に対して、
architecture geometry を次のデータとして置く。

```text
X_s^{r,J}
  =
  (A_s^r, ArchCtx(A_s^r), E_r, J_{E_r})
```
...
J_{E_r}
  = E_r の required coordinate requirements から生成される coverage topology
```
...
equation system `E` に対し、context `W` で読まれる coverage requirements を
次の有限または局所有限データとして固定する。
...
coverage family が `E`-admissible であるとは、各成分が patch または指定された overlap の
restriction image で共同して覆われ、各 morphism が `nu` と `epsilon` の restriction compatibility を
保つことをいう。何を共同して覆うかは `Req_E(W)` に列挙され、説明のない追加条件は
coverage package に置かない。
...
equation system `E` に対して、まず `ArchCtx(A)` 上の admissible cover を定義し、
...
`J_E^{adm}` は、各 context `W` に対して `E`-admissible coverage family の族を割り当てる。
...
`{ W_i -> W }` が `J_E^{adm}(W)` に属するとは、次を満たすことである。

```text
Atom support coverage:
  Req_E(W) の required Atom support が W_i によって共同して覆われる。

Equation-coordinate coverage:
  各 required pair (i,a) が patch または指定された overlap で読め、
  nu_{W,i,a} と epsilon_{W,A,i,a} の restriction が定義どおり一致する。

Violation-witness coverage:
  selected violation witness support が patch または overlap に現れる。

Signature axis coverage:
  Req_E(W) の required axes が cover 全体で読まれる。

Interaction-overlap coverage:
  Req_E(W) に選ばれた mixed-axis、feature/core、semantic/runtime、
...
J_E
  =
the Grothendieck topology generated by J_E^{adm}.
```
...
以後、AAT site の sheaf はこの generated topology `J_E` に関して読む。

### 定義 7.2 E-Adequate Cover

generated topology `J_E` の cover のうち、equation lawfulness や cohomological flatness の定理で
使える cover を `E`-adequate cover と呼ぶ。

```text
𝒰 = { W_i -> W } is E-adequate
```

とは、`𝒰` が `J_E` の cover であり、かつ定理で要求される selected data について次を満たすことである。

```text
required Atom support is covered.
required equation-coordinate pairs are readable on patches or overlaps.
selected violation witnesses appear on patches or overlaps.
required signature axes are readable on the cover.
selected interaction witnesses are visible on overlaps.
restriction maps preserve nu, epsilon, and the generated witness ideals.
```

`J_E` は sheaf theory のための topology であり、`E`-adequate は equation / witness / axis を
読むための追加 predicate である。

### 補題 7.2A Equation-Closure Cover [Certified bounded inference]

required equation-coordinate family と selected witness family が局所有限であり、各 support が context category の中で
表現可能で、有限交差または必要な overlap が存在すると仮定する。
さらに、選ばれた reading について次を仮定する。

```text
restriction maps preserve nu and epsilon exactly.
restriction maps carry generated witness ideals into the corresponding local ideals.
...
--------------------------------
equation-closure cover is E-adequate
```
...
AAT site は、architecture context category、architectural equation system、
AAT Grothendieck topology を一つの package として保持する。

### 定義 8.1 AAT Site

architecture object `A_S^V`、equation system `E`、coverage topology `J_E` に対して、
AAT site を次で定義する。
...
underlying site は `(ArchCtx(A), J_E)` であり、`E` はその上の AAT equation decoration である。
AAT の局所性、coverage、projection、unmeasured axis、gluing は、この package によって制御される。
