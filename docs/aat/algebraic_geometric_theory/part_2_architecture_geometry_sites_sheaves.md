...
`Sig_r` は `r` の signature reading `R_sig` の値であり、`r` とは別に選ぶ入力ではない。
...
operation / signature reading、coverage requirements、selected pullback overlap、coverage topology に相対化された、局所-大域的な architecture geometry である。

## 2. Architecture Geometry

Architecture geometry は、architecture object に context category、equation system、signature、
coverage requirements、selected pullback overlap、そこから生成される coverage topology を載せた対象である。

### 定義 2.1 Architecture Geometry

source `s`、Atom axiom system `S`、core reading `r : CoreRead(At)`、architectural equation system `E_r`、
signature reading `Sig_r`、coverage requirements `R_r`、
`Ov_r : ContextOverlapPullback(ArchCtx(A_s^r))` に対して、
...
Sig_r, R_r, Ov_r
  = r から得る signature、それに依存する coverage requirements、typed context-overlap package
...
equation system、coverage requirements、selected pullback overlap、coverage topology が異なれば、
...
### 定義 4.3A Context Overlap Pullback Package

context category `C = ArchCtx(A)` 上の context-overlap package

```text
Ov : ContextOverlapPullback(C)
```
...
に対して、overlap context `W_L x_W W_R`、二つの projection、可換 square、
およびその square の pullback universal property を選ぶデータである。選択は identity と
composition に整合し、context restriction `W' -> W` に沿う引き戻しは、引き戻した cospan の
選択済み overlap と canonical isomorphism で一致する。

coverage requirement `R` の interaction-overlap request は、base context `W`、二つの legs、
その overlap 上で読む interaction support を持つ typed request とする。`Ov` が `R` を実現するとは、
各 request の二つの legs に対して `Ov` の選んだ pullback が存在し、その restriction image 上で
指定された interaction support が読めることをいう。
...
equation system `E` と architecture signature `Sig` を固定する。coverage requirement package
...
Req_R(W)
  ...
  (required Atom support,
   ...
   selected interaction-overlap requests)
```

さらに、

```text
Ov : ContextOverlapPullback(ArchCtx(A))
```

を選び、`Req_R(W)` の各 interaction-overlap request を実現すると仮定する。したがって依存順は
`E`、`Sig`、`R`、`Ov` であり、`Ov` は未型付けの overlap marker ではない。
...
Interaction-overlap coverage:
  ...
  authority/effect interaction request が、Ov の指定する pullback overlap 上で見える。
...
Req_R(W).selectedInteractionOverlapRequests are visible on the pullback overlaps selected by Ov.
...
`Req_R` の required Atom support、required equation-coordinate support、selected witness support が
局所有限であり、各 support が context category の中で表現可能で、有限交差または必要な overlap が
存在すると仮定する。
...
selected interaction witnesses remain visible on the pullback overlaps selected by Ov.
```

このとき、required Atom support、required coordinate support、witness support、指定された pullback overlaps を
含むように cover を閉じて得られる equation-closure cover は `(E,R,Ov)`-adequate である。

```text
local finite required Atom, equation, and witness supports
+ representable pullback overlaps selected by Ov
...
J_{E,R,Ov} is generated by finite equation-closure covers.
...
coverage requirements、context-overlap pullback package、そこから生成される AAT Grothendieck topology を
一つの package として保持する。
...
`Ov : ContextOverlapPullback(ArchCtx(A_S^V))` に対して、
AAT site を次で定義する。
