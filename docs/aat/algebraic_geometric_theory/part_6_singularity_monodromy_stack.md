...
第V部の derived law geometry は、lawful loci の交差に残る latent conflict を扱った。
...
これらは、derived architecture geometry の中で、obstruction の集中、経路作用、descent failure を読む三つの面である。
...
stratum は、Atom、law、obstruction、signature、coverage に相対化された architecture geometry の部分である。
...
cohomological grading には、`H^1(T_{S/U})` が first obstruction classes を含む convention を採用する。

### 原則 3.3 Tangent Is Not Operation List

tangent complex は、操作の一覧ではない。
...
も十分条件として使える。
これらは、同じ regime で smoothness を証明するための criterion である。
...
を仮定すると、`S` の近傍に実際の selected lifting failure が存在する。
...
そうではなく、lawful deformation が詰まり、repair direction が高次 obstruction に阻まれる場所である。
...
cotangent complex が滑らかでない場合も、滑らかな lifting が存在しない可能性がある。
そのため、`S` は repair や refactor が詰まりやすい singularity candidate である。

### 定理 6.2 Square-Zero Lifting Obstruction [Certified bounded inference]
...
operation category `Op_U(X)` に対して、selected operation homotopy を生成する関係族を
...
またはその selected finite-dimensional reading を対応させる。
operation path
...
へ降りるための必要十分条件は、各 homotopy generator 2-cell `sigma` について、
...
逆に quotient group へ因子化するなら、各関係式は単位元へ送られるため、すべての `mu_x(sigma)` は零になる。
...
このとき、固定された `K_H(X,U)`、transport、axis exactness の下で、`sigma` の境界 loop は、
selected axis `x` に関して恒等 transport を持つ filling としては読めない。
...
なら、`gamma` は hidden architecture debt を持つ。
...
groupoid として扱うのは、refactor equivalence が同型とその合成を持つからである。
...
この違いを保持するため、AAT は stack 的な対象へ進む。
...
すべての設計原則、semantic property、未選択 behavior を含むものではない。
...
gerbe が abelian sheaf `A` によって banded される場合は、
...
そうでない場合、`[gerbe_U(X)]` は cohomology group の元ではなく、
...
である。
したがって、local decompositions は存在しても、それらは global canonical decomposition へ貼り合わない。
...
第VI部では、第V部の derived obstruction が、architecture geometry の中でどのように現れるかを定式化した。
...
これらは、singularity、monodromy、stack として現れる architecture geometry の構造である。
