...
6. topology changeとcoefficient changeのactual `Sheaf.H` comparison、
   affine Tor flat-base-changeのfixed signatureを
...
| --- | --- | --- |
| `Formal.AG.ReadingFunctoriality.Core` | `Formal.AG.Atom.AATCore`、`Formal.AG.Site.Geometry`、`Formal.AG.LawAlgebra.StructureSheaf` | `ReadingCore`、exact / positive core change、`ObjectAlgebraHom` |
...
第VIII部 定理候補9.2のgeneral ringed-site statementはこのPRDの対象外であり、
上のaffine clauseだけを完了対象とする。`Measurement.FlatBaseChangeCandidate`は
comparison typeと結論をfieldに持つため、証拠として使用しない。
...
| `SemanticLawEquationWitnessIdealCore` / source bridge | 本文由来 / part3 dependency | source global equationsとlaw-index provenance | source `B`を使ってglobal equationsを生成し、finite modelで`coefficientBridge_valid`を証明。target rawのbridgeへ同じ`G`を再利用しない |
...
| part3 inclusion validity | 本文由来 |同一`raw` / `X`上のselected law strengthening | merged primitive law/atom mapとvalidityを直接再利用 |
| ideal inclusion / closed immersion / semantic monotonicity | 放電済み | part3の結論 | part3 theoremを直接参照し、new fieldにしない |
...
| affine Tor base-change iso | 放電済み | 第VIII部9.2 affine clause | `CategoryTheory.Tor`とflat tensor exactnessから構成し、finite firingではsource witnessのcanonical unit像をこのisoでtargetへ送る |
...
| law | part3 `weakToStrong`、`weakToStrong_valid`、`weak_ideal_lt_strong`、`weakToStrongMap_not_isIso`、`weakToStrongAllMap_not_isIso` | `coordinateBrokenInclusion_not_valid` | required / all-selected双方のstrict ideal comparisonとnon-iso closed immersion |
...
| Appendix A.2.1 law inclusion | SD2 | 同一standard geometry内のselected closed-law strengtheningについてpart3のactual ideal / closed geometry mapを直接再利用 |
...
| 第VIII部 定理候補9.2 | SD7 | affine Tor clauseをactual Mathlib Tor isoとして証明。general ringed-site clauseは対象外 |
...
- part3の同一`raw` / `X` / law-index条件を一般law-universe inclusionへ読み替えない。
...
- final part3 implementation signatureをSD2と再突合する。
...
- part3のrequired / all-selected ideal geometryとrequired / full all-law semantic comparisonを
  direct importする。
- duplicate `LawExtension`やcomparison mapを作らない。
- statement contractとfinite firingでpart3宣言を直接参照する。
...
- `mathlibTorFlatBaseChangeIso`を任意次数で証明する。
...
- [ ] AC9: nonidentity exact exampleとpositive-only negative exampleが発火する。
- [ ] AC10: law laneが同じ`S`、`k`、`HasSheafify`、`raw`、`X`を共有するpart3 declarationを直接使用し、新しいwrapper / duplicate mapを作らない。
...
- [ ] AC34: affine `mathlibTorFlatBaseChangeIso`が任意次数で証明され、finite theoremへの差し替えがない。
...
- [ ] AC38: `Int → Polynomial Int` modelでproper ideal、nonidentity scheme map、source Tor₁ witnessのgeneric base-change iso像、nonzero linear Čech class、nonzero actual `Sheaf.H` class mapが同じcoefficient changeから発火する。`finiteCechCoefficientCompatible`とsource / target Leray proofを具体modelで放電し、final actual H isoを構成する。
...
- law laneをpart3 declarationのwrapper / alias / repackageだけで水増しする。
- part3のsame-site mapを異なるlaw universe間のcomparisonと扱う。
...
- part3 merged signatureがSD2のsame-site contractと一致しない。
...
- affine Tor base-change isoをSD7のsignatureで証明できない。
...
- SD2 implementationにduplicate comparison definitionがなく、part3 direct importとdirect theorem useがある。
...
| dependency evidence | 対象main commit、part1 / part2のmerged core・site・standard scheme宣言、part3 final merged declaration inventoryとsame-site signature、Mathlib module-sheaf / actual-H scratch、finite modelのnamed `HasSheafify` instance chain |
...
| Tor / Čech evidence | arbitrary-degree affine Tor iso、linear Hn iso、cochain / differential formula、nonzero firing |
...
| negative evidence | positive-only mismatch、broken face、non-Leray non-evidence、non-flat map、broken relation、candidate-field非依存 |
