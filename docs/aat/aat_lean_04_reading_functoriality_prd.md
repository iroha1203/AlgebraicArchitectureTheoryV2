...
theorem semanticCoreIdealSheaf_baseChange_provenance
    (G : SemanticLawEquationWitnessIdealCore S)
    (B : SemanticLawEquationSchemeBridge raw G)
...
@[reassoc] theorem lawfulClosedSubschemeBaseChangeMap_immersion
    ...
    [S.topology.HasSheafCompose
      (f.coefficientExtension :
        AATCommAlgCat.{u, v} k ⥤ AATCommAlgCat.{u, v} k')] :
    lawfulClosedSubschemeBaseChangeMap raw X G B hB f ≫
...
@[reassoc] theorem allLawfulClosedSubschemeBaseChangeMap_immersion
    ...
    [S.topology.HasSheafCompose
      (f.coefficientExtension :
        AATCommAlgCat.{u, v} k ⥤ AATCommAlgCat.{u, v} k')] :
    allLawfulClosedSubschemeBaseChangeMap raw X G B hB f ≫
...
`semanticCoreIdealSheaf_baseChange_provenance`はPart 3の
`semanticCoreIdealSheaf_realized raw X G B hB`を直接proof-useし、source realizationと
required / all-selected ideal comap equalityを同時に返す。後続のideal equalityはこの定理の
projection、lawful closed geometry mapはそのequalityから構成し、`SemanticCoreIdealSheafRealized`を
caller-supplied premiseとして受け取らない。
...
| `SemanticLawEquationWitnessIdealCore` / source bridge / `IsSemanticLawEquationSchemeBridge` | 本文由来 / `ClosedEquationalGeometry` dependency | source global equations、law-index provenance、restriction naturality、canonical chart presentation | source `B`からglobal equationsを生成し、`hB`を`semanticCoreIdealSheaf_realized`とbase-change provenance theoremでproof-useする。finite modelでは`coefficientBridge_valid`で放電し、target rawのbridgeへ同じ`G`を再利用しない |
| `SemanticCoreIdealSheafRealized` | 放電済み | existing semantic coreのactual chart ideal-sheaf realization | Part 3の`semanticCoreIdealSheaf_realized raw X G B hB`から導出し、base-change provenance theoremの第1成分として返す。input fieldや追加premiseとして受け取らない |
...
| structural / witness ideal extension equality | 放電済み | ideal comparison | `hB`から導くsource realization、generator map、restriction、`IdealSheafData.comap`を`semanticCoreIdealSheaf_baseChange_provenance`で同時に証明 |
| lawful-locus mapとtriangle | 放電済み | scheme comparison | provenance付きideal equalityから生成し、全existing-core APIで`hB`をproof-useする |
...
| coefficient geometry | `intPolynomialFlatChange`、`coefficientExtension_hasSheafCompose`、`coefficientSectionSpecBaseChangeIso_fires`、`coefficientSemanticCore_realized`、`properIdeal_baseChange`、`lawfulLocus_baseChange_fires` | `intZModTwo_not_flat`、`brokenRelationChange_not_rawBaseChange` | site-dependent sheaf compatibility、source existing-core realization、proper ideal、nonidentity scheme mapを同じ係数変更で放電 |
...
noncomputable def coefficientSectionSpecBaseChangeIso_fires :
    LawAlgebra.architectureChartSpec
      ...
      pullback
        ...
        (AlgebraicGeometry.Scheme.Spec.map intPolynomialFlatChange.hom.op) :=
...
theorem lawfulLocus_baseChange_fires :
    ...
    ¬ IsIso
      (LawAlgebra.lawfulClosedSubschemeBaseChangeMap
        coefficientRaw coefficientScheme coefficientSemanticCore
        coefficientBridge coefficientBridge_valid intPolynomialFlatChange)
...
- `hB : IsSemanticLawEquationSchemeBridge raw G B`からPart 3の
  `semanticCoreIdealSheaf_realized`を導出し、source realizationとrequired / all-selected ideal sheafの
  comap equalityを`semanticCoreIdealSheaf_baseChange_provenance`で同時に証明する。
- provenance付きideal equalityからlawful closed geometry mapとambient triangleを構成する。
...
- [ ] AC31: `baseChangedSemanticCoreGlobalEquation`がsource global equationsをactual `baseChangeMap.appTop`で送り、`baseChangeOfSemanticCore`がtarget geometric reading、witness coordinates、restriction compatibilityを同じpullbackから生成する。同じ`G`を異なるcoefficient rawのbridgeへ再利用しない。`semanticCoreIdealSheaf_baseChange_provenance`は`hB`を受け、Part 3の`semanticCoreIdealSheaf_realized`をproof-useしてsource realizationを返す。
- [ ] AC32: required / all-selected ideal sheafの`IdealSheafData.comap` equalityが`semanticCoreIdealSheaf_baseChange_provenance`のprojectionとして証明され、`hB`を省略したexisting-core completion declarationがない。
- [ ] AC33: required / all-selected双方のlawful closed subscheme comparisonとambient triangleが`hB`とprovenance付きideal comparisonから構成される。
...
- `hB`からPart 3のsource ideal-sheaf realizationを導出し、generator / restrictionと同じproof chainで
  ideal comap equalityへ接続できない。
...
- coefficient routeが`RawAmbientRestrictionSystem.baseChange`、
  ...
  Part 3の`semanticCoreIdealSheaf_realized`、`semanticCoreIdealSheaf_baseChange_provenance`、
  `baseChangedSemanticCoreGlobalEquation`、`IdealSheafData.comap`、
  `lawfulClosedSubschemeBaseChangeMap`を同じprovenanceで使用する。
...
| coefficient evidence | `HasSheafCompose`のfinite放電、canonical raw-presheaf / sheafified-section comparison、actual pullback scheme、valid source bridge、source ideal-sheaf realization、source global equationのactual pullback、target reading、provenance付きideal comap、lawful map、unit / compositor compatibility |
