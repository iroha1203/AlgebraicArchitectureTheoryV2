...
- 数学的正典:
  - 第I部 原則10.6 `Change of Core Reading`
  - 第II部 `5.3 Refinement`、`5.4 Base Change`、定義7.1〜7.2
  - 第III部のstandard architecture scheme、ideal restriction、closed subscheme
  - 第IV部 定義2.2、原則2.3、原則2.5、定義3.1〜4.1
  - 第VIII部 定義9.1、定理候補9.2のaffine clause
  - Appendix A.2〜A.2.1
...
5. Mathlibの次のactual APIを対象main commit上でfocused checkし、tracking Issueへ
   ...
   - `Module.FinitePresentation`とprincipal ideal quotientのfinite-presentation instance route
...
6. topology changeとcoefficient changeのactual `Sheaf.H` comparison、
   affine Tor flat-base-changeのfixed signature、flat extensionが無限積と可換しない
...
| `Formal.AG.ReadingFunctoriality.Coefficient` | `Formal.AG.ReadingFunctoriality.Core`、`Formal.AG.ReadingFunctoriality.Coverage`、`Formal.AG.LawAlgebra.ClosedEquationalGeometry`、`Formal.AG.Derived.Intersection`、`Mathlib.Algebra.Category.ModuleCat.ChangeOfRings`、`Mathlib.Algebra.Category.ModuleCat.Descent`、`Mathlib.Algebra.Category.ModuleCat.Sheaf`、`Mathlib.Algebra.Module.FinitePresentation`、`Mathlib.Algebra.Module.TransferInstance`、`Mathlib.CategoryTheory.Sites.PreservesSheafification`、`Mathlib.AlgebraicGeometry.Pullbacks`、`Mathlib.AlgebraicGeometry.IdealSheaf.Functorial`、`Mathlib.RingTheory.RingHom.Flat` | closed-equational geometry宣言のdirect reuse、raw / scheme / ideal / Tor / linear Čech scalar extension / actual sheaf H coefficient change |
| `Formal.AG.ReadingFunctoriality.FiniteExamples` | 上記3module、既存finite model module | SD9のpositive / negative firing |
| `Formal.AG.ReadingFunctoriality` | 上記4moduleのみ | public aggregate |
| `Formal.AG.StatementContractsReadingFunctoriality` | public aggregateとfinite examples | SD0〜SD9のexact signature突合だけ |

`Formal/AG/ReadingFunctoriality.lean`を`Formal/AG.lean`からimportする。
`Formal/AG/StatementContracts.lean`と`Formal/AG/AxiomAudit.lean`はnew public theoremを
直接参照する。`Formal/AG`から`research/lean/ResearchLean`をimportしない。
...
noncomputable def baseChangeUnderlyingIso
    ...
    (X.baseChange raw f).underlying ≅
      pullback (X.coefficientStructureMap raw)
        (AlgebraicGeometry.Scheme.Spec.map f.hom.op)
...
`StandardArchitectureScheme.baseChange`はactual affine pullback、atlas、overlap、
reading decorationから構成する。caller-supplied Scheme morphismを受け取らない。
...
noncomputable def mathlibTorFlatBaseChangeIso
    ...
    [Module.FinitePresentation R (R ⧸ J)]
    (n : Nat) :
...
affine Tor theoremは任意次数で固定し、finite版への差し替えを許さない。
第VIII部 定理候補9.2が要求するfinite presentationは二つのquotient moduleの
typeclassとして明示する。coefficient compatibilityは`f.hom`による
`moduleScalarExtension`、support pullbackは`I.map f.hom` / `J.map f.hom`をtargetに
直接使うことでcanonicalに固定し、別のcompatibility certificateやsupport equalityを
入力しない。general ringed-site statementはこのPRDの対象外であり、この仮定列を持つ
affine clauseだけを完了対象とする。`Measurement.FlatBaseChangeCandidate`はcomparison typeと
結論をfieldに持つため、証拠として使用しない。
...
| `SemanticLawEquationWitnessIdealCore` / source bridge | 本文由来 / `ClosedEquationalGeometry` dependency | source global equationsとlaw-index provenance | source `B`を使ってglobal equationsを生成し、finite modelで`coefficientBridge_valid`を証明。target rawのbridgeへ同じ`G`を再利用しない |
...
| changed raw / scheme / reading | 放電済み | coefficient changeの出力 | raw / schemeは`baseChange`、readingはsource global equationsの`baseChangeMap.appTop`像から生成。field禁止 |
| structural / witness ideal extension equality | 放電済み | ideal comparison | generator map、restriction、`IdealSheafData.comap`から証明 |
| lawful-locus mapとtriangle | 放電済み | scheme comparison | ideal equalityから生成 |
| `Module.FinitePresentation R (R ⧸ I)` / `Module.FinitePresentation R (R ⧸ J)` | 本文由来 | 第VIII部9.2 affine clauseのfinite presentation | generic theoremで明示し、finite firingのprincipal ideal quotientではfinite presentation instanceをprimitive presentationから構成する |
| affine coefficient compatibility / support pullback | 放電済み | 第VIII部9.2 affine clause | coefficient changeを`f.hom`の`moduleScalarExtension`、pullback idealsを`I.map f.hom` / `J.map f.hom`として固定し、別certificateを受けない |
| affine Tor base-change iso | 放電済み | 第VIII部9.2 affine clause | 上記finite presentation、`CategoryTheory.Tor`、flat tensor exactnessから構成し、finite firingではsource witnessのcanonical unit像をこのisoでtargetへ送る |
...
| positive core | `positiveCoreChange`、`positiveBase_target_reachable`、`positiveCircuit_transport` | `negativeCircuit_not_transportable` | actual target algebra objectへpositive queryは運び、negative queryを含む回路は運べない |
...
| coefficient geometry | `intPolynomialFlatChange`、`properIdeal_baseChange`、`lawfulLocus_baseChange_fires` | `intZModTwo_not_flat`、`brokenRelationChange_not_rawBaseChange` | proper idealとnonidentity scheme map |
...
noncomputable instance properIdealQuotientFinitePresentation :
    Module.FinitePresentation Int (Int ⧸ properIdeal)
...
| 第VIII部 定理候補9.2 | SD7 | 二つのquotient moduleのfinite presentationを明示し、coefficient extensionを`f.hom`、support pullbackを`I.map f.hom` / `J.map f.hom`へ固定したaffine Tor clauseをactual Mathlib Tor isoとして証明。general ringed-site clauseは対象外 |
...
flat coefficient change
  -> canonical raw and standard scheme change
...
8. flat coefficient changeからraw、standard scheme、closed-equational readingがcanonicalに生成される。
9. required / all-selected idealとlawful closed geometryのbase-change comparisonが構成され、
...
- build、axiom scan、declaration existence、docs同期だけを数学claimの完了根拠にしない。
...
- sheafification、affine pullback、atlas、overlapから`StandardArchitectureScheme.baseChange`を構成する。
...
- required / all-selected ideal sheafのcomap equalityを証明する。
- lawful closed geometry mapとambient triangleを構成する。
- 二つのquotient moduleの`Module.FinitePresentation`を明示し、`mathlibTorFlatBaseChangeIso`を任意次数で証明する。
- coefficient compatibilityとsupport pullbackを`moduleScalarExtension f`、`I.map f.hom` / `J.map f.hom`のproof-useで放電する。
...
- live runtime sourceだけを同期し、archive文書を更新対象にしない。
...
- [ ] AC9: nonidentity exact example、positive-only negative example、`Invariant.transportedAlong_refl`、`Invariant.function_predicate_not_transportedAlong`が発火する。
...
- [ ] AC27: `FlatCoefficientChange`のprimitive dataがring homと`RingHom.Flat`だけである。
- [ ] AC28: `raw.baseChange`がcoordinate、relation、restrictionをcoefficient mapから構成する。
- [ ] AC29: standard scheme base changeとmapがactual affine pullback / decoration / atlas / overlapから構成され、interpretation equationと全chart pullbackが検査される。
- [ ] AC30: scheme base-change mapのidentity / compositionがunit / compositor isoを介したactual morphism equalityで証明される。
...
- [ ] AC32: required / all-selected ideal sheafの`IdealSheafData.comap` equalityが証明される。
- [ ] AC33: required / all-selected双方のlawful closed subscheme comparisonとambient triangleがideal comparisonから構成される。
- [ ] AC34: affine `mathlibTorFlatBaseChangeIso`が二つのquotient moduleの`Module.FinitePresentation`、`f.hom`によるcoefficient extension、`I.map f.hom` / `J.map f.hom`によるsupport pullbackの下で任意次数について証明され、finite theoremへの差し替えがない。
...
- [ ] AC38: `Int → Polynomial Int` modelでproper ideal、quotientのfinite-presentation instance、nonidentity scheme map、source Tor₁ witnessのgeneric base-change iso像、nonzero linear Čech class、nonzero actual `Sheaf.H` class mapが同じcoefficient changeから発火する。`finiteCechCoefficientCompatible`とsource / target Leray proofを具体modelで放電し、final actual H isoを構成する。さらに`Int → Rat`と`Nat`添字のinfinite duplicated coverでdegree-zero canonical componentがisoでないことと`infiniteProductCech_not_compatible`を証明し、compatibility predicateの不成立を直接示す。
...
- build、declaration existence、axiom scan、docs同期だけをcompletion evidenceにする。
...
- raw coefficient changeからstandard schemeをcanonicalにbase changeできず、arbitrary target schemeが必要になる。
- source global equationsのactual scheme pullbackからtarget closed-equational readingを構成できない。
- ideal comap equalityをgenerator / restrictionから証明できない。
- finite-presentation instanceを含むaffine Tor base-change isoをSD7のsignatureで証明できない。
...
- coefficient routeが`RawAmbientRestrictionSystem.baseChange`、
  `StandardArchitectureScheme.baseChange`、`baseChangedSemanticCoreGlobalEquation`、
  `IdealSheafData.comap`を同じprovenanceで使用する。
...
| dependency evidence | 対象main commit、`AATCorePackage` / `SelectedGeometryReading.toAATSite` / `StandardArchitectureScheme`のmerged宣言、`ClosedEquationalGeometry` final merged declaration inventoryとsame-site signature、Mathlib module-sheaf / actual-H scratch、finite modelのnamed `HasSheafify` instance chain |
...
| core evidence | exact / positive map、reachable construction、id / comp、negative-query差 |
...
| coefficient evidence | canonical raw / scheme、source global equationのactual pullback、target reading、ideal comap、lawful map、unit / compositor compatibility |
| Tor / Čech evidence | quotient finite presentation、canonical coefficient / support pullback、arbitrary-degree affine Tor iso、linear Hn iso、cochain / differential formula、nonzero firing |
