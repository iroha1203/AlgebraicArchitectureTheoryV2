...
@[simp] theorem ArchitectureAffineAtlas.pairToBase_eq_right
    {X : AlgebraicGeometry.Scheme}
    {D : AATReadingDecoration raw X}
    (atlas : ArchitectureAffineAtlas raw X D)
    (i j : atlas.Index) :
...
@[simp] theorem ArchitectureAffineAtlas.tripleToMiddle_eq_second
    {X : AlgebraicGeometry.Scheme}
    {D : AATReadingDecoration raw X}
    (atlas : ArchitectureAffineAtlas raw X D)
    (i j l : atlas.Index) :
...
`pairToBase_eq_*`と`tripleToMiddle_eq_*`はcontext categoryのsubsingleton homで証明する。
...
raw configuration functorは、現行`RawAffinePresentation`内の証明を同一
`RawAmbientRestrictionSystem`に直接紐付ける。
...
def LocalConfiguration
  ...
  { a : (raw.coordFamily W).CoordX → R //
    ∀ r : (raw.relationFamily W).Relation,
      MvPolynomial.aeval a ((raw.relationFamily W).polynomial r) = 0 }
...
`localConfigurationRepresentability`は現行
`RawAffinePresentation.rawQuotientRepresentability`のproof coreを抽出して構成する。
既存の`RawAffinePresentation` public APIはこのgeneric theoremの系として同じsignatureを
維持し、独立な二つ目のuniversal-property proofは書かない。
...
| `[HasSheafify S.topology (AATCommAlgCat k)]` | 本文由来の構成可能性条件 / finite modelでは放電済み | 第III部 第1節 / 定義2.1 / 定義9.1。一般APIではtypeclass parameter、finite modelでは`FiniteExamples.RingedSite.FiniteModel.hasSheafify`が放電する。 |
...
| `StandardArchitectureScheme.atlasValid` / `overlapsValid` | 上記predicateのproof package | `ofPresentation`では明示入力、completion evidenceとなる二constructorではprimitive dataから放電する。新しい結論相当fieldは追加しない。 |
...
| `RawAmbientRestrictionSystem.LocalConfiguration` / `localConfigurationRepresentability` | 本文由来 / 新premiseなし | 第III部 定義8.2〜定理8.3。同じ`raw.coordFamily W` / `raw.relationFamily W`から定義し、現行raw quotient universal-property proofを抽出して再利用する。 |
| `SheafifiedChartPresentation.canonical_isIso` | 本文由来のmaterial condition / finite modelで放電済み | 第III部 仮定8.4の強いcanonical-unit-isomorphism case。任意のcomparisonは受け取らず、`comparison`はcanonical unitの逆射から導出する。finite modelは`canonical_component_isIso`で放電する。 |
...
| 第III部 定義8.2〜定理8.3 | 既存`rawQuotientRepresentability`を維持 | raw typed configuration representabilityは再実装しない。 |
...
- `RawAmbientRestrictionSystem.LocalConfiguration`を同じ`raw.coordFamily W` /
  `raw.relationFamily W`から定義する。
- 現行`RawAffinePresentation.rawQuotientRepresentability`のproof coreを
  `localConfigurationRepresentability`へ抽出し、既存public APIをその系として維持する。
...
- [ ] AC22: `LocalConfiguration`が同じ`raw.coordFamily W` / `raw.relationFamily W`から定義され、`localConfigurationRepresentability`が現行raw quotient universal-property proofの抽出であり、既存`RawAffinePresentation` APIがその系として維持される。
...
- [ ] AC24: 新主経路に自由な`Prop` slot、`Prop + holds`、結論相当certificate、未使用material premise、definitionally unrelated componentの再包装がなく、`uncoveredAtlas_not_valid`がatlas validityの非恒真性を発火させる。
- [ ] AC25: 新definitionにconstructor/destructor/ext/characterization/comparison APIがあり、主要利用者がdefinition unfoldに依存しない。
- [ ] AC26: target declarationsとreference modelsがstandard axiomsのみを使用し、statement contractsとaxiom auditがgreenである。
- [ ] AC27: Lean theorem indexとproof-obligation台帳が、reading-decorated coreとfull law-equational geometryを区別して同期される。
- [ ] AC28: `docs/aat/algebraic_geometric_theory/**`に変更がなく、Research reverse importがない。
- [ ] AC29: `math-lean-review`の4本の独立査読がすべて`No major findings`である。
