# 古典 AAT / downstream Lean API 索引

この文書は、`Formal/Arch`、finite static structural core、Atom-generated kernel、
SFT / ArchSig / tooling bridge に存在する主要な Lean 定義・定理の分割索引である。
分割全体の入口は [Lean 定義・定理索引](lean_theorem_index.md) を参照する。

`proof_obligations_classical_aat.md` は古典 AAT / downstream 側の未解決課題と
実証仮説の詳細台帳として扱い、この文書は同じ範囲の実装済み Lean API を確認する
詳細索引として扱う。
...
[証明義務と実証仮説](proof_obligations_classical_aat.md) で管理する。
...
各節の Non-conclusions と `proof_obligations_classical_aat.md` の coverage / exactness assumptions を
併せて確認する。
...
この索引と `proof_obligations_classical_aat.md` に分離する。
...
は、この節と `proof_obligations_classical_aat.md` 側の対応を、第7章の統合・命名・索引 status
...
Auxiliary bridge files: `Formal/Arch/Evolution/SFTArtifactBoundaryBridge.lean`,
`Formal/Arch/Evolution/SFTAATArtifactBoundaryExamples.lean`,
...
`Formal/Arch/Examples/AtomGeneratedMoleculeExamples.lean`,
...
これらは [証明義務と実証仮説](proof_obligations_classical_aat.md)、tooling boundary、または empirical protocol 側で扱う。
