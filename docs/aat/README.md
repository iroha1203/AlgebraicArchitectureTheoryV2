...
1. [代数幾何的 AAT 数学本文](algebraic_geometric_theory/README.md)
   ...
   - 第I〜X部の現行 declaration mapping は
     [AG Lean API索引](lean_theorem_index_ag_aat.md)で確認する。
   - Part X theorem-7.5の固定signatureは
     [恒久statement contract](statement_contracts/semantic_repair_part_x.md)で確認する。
   - 完了根拠の降格・発火判定は
     [AG Lean status overlay](lean_ag_status_overlay.md)で確認する。
