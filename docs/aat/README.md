...
1. [代数幾何的 AAT 数学本文](algebraic_geometric_theory/README.md)
   ...
   - Lean 形式化との対応があるものは、`Formal/` のLean sourceで直接確認する。
   - Part X theorem-7.5の固定signatureは
     `Formal/AG/StatementContractsSemanticRepairPart10.lean`で確認する。
2. [AAT / Lean 編集ガイドライン](guideline.md)
3. [Lean 品質基準](lean_quality_standard.md)
   - mathlib 型 statement review 基準と target statement 固定手続きの正本。
4. [AAT Core / obstruction split statement contract](statement_contracts/aat_core_obstruction_split.md)
   - 定理10.5の無条件coreとlaw-failure相対のobstructed extensionについて、Lean schemaとtarget signatureを固定する。
...
現在 Lean に存在する定義・定理は `Formal/` のLean sourceで確認する。
未解決課題とResearchの進行状態は、数学本文を変更せずGitHub Issues・GOAL・PRDで管理する。
