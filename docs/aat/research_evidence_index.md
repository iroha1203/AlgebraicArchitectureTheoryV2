...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `ResponseImageSequence.protectedResponseComponent` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ResponseImageSequence.lean` | - | actual `labeledResponse`を有限積の普遍性で束ねたprotected responseについて、各積射影が対応するactual labeled responseを回収することを証明 | なし | G-aat-quality-surface-08 Cycle 18 | unported | #3600 |
| `ResponseImageSequence.responseImageShortComplex_shortExact` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ResponseImageSequence.lean` | - | actual protected-response kernel上のtarget response `s_t`から`K = kernel(s_t)`と`R = Abelian.image(s_t)`を構成し、canonical列`0 → K → K_P → R → 0`がShortExactであることを証明 | なし | G-aat-quality-surface-08 Cycle 18 | unported | #3600 |
| `ResponseImageSequence.chartProtectedKernelSectionsEquiv_subtype` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ResponseImageSequence.lean` | - | selected chart上のprotected-kernel sectionsと既存`chartProtectedResponseMap.ker`のcanonical linear equivalenceをproduct/kernel保存から構成し、actual kernel inclusionとの可換性を証明 | なし | G-aat-quality-surface-08 Cycle 18 | unported | #3600 |
| `ResponseImageSequence.chartTargetOnProtectedKernel_compatibility` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ResponseImageSequence.lean` | - | 上記kernel同値の下でactual restricted target responseのchart componentが既存`chartTargetOnProtectedKernel`に一致することを証明 | なし | G-aat-quality-surface-08 Cycle 18 | unported | #3600 |
| `ChartGeometricFiber.chartModuleToAllowedOperationStalk_isLocalized` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | actual chart-section moduleからactual restricted allowed-operation sheaf stalkへのcanonical mapがlocalizationであることをCycle 16のtilde比較から証明 | なし | G-aat-quality-surface-08 Cycle 17 | unported | #3595 |
| `ChartGeometricFiber.chartSectionFiberEquivGeometricFiber` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | section tensor residue-field fiberとactual sheaf stalkのresidue-field base changeをlocalizationの普遍性からcanonical linear equivalenceで接続 | なし | G-aat-quality-surface-08 Cycle 17 | unported | #3595 |
| `ChartGeometricFiber.chartGeometricFiberResponse_comp_sectionFiberEquiv` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | actual restricted responseのstalk/base-changeとsection tensor-fiber responseがcanonical fiber比較の下で一致することを証明 | なし | G-aat-quality-surface-08 Cycle 17 | unported | #3595 |
| `ChartGeometricFiber.chartGeometricFiberCircuitLocus_eq_chartCircuitLocus` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | actual geometric-fiber responseで独立に定義したcircuit locusとsection tensor-fiber circuit locusの集合等式をresponse可換性とdual linear equivalenceから証明 | なし | G-aat-quality-surface-08 Cycle 17 | unported | #3595 |
| `ChartGeometricFiber.chartGeometricFiberCircuitLocus_eq_support` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | actual geometric-fiber circuit locusとactual chart response-cokernel supportの集合等式を証明 | operation moduleのprojectivity、protected responseのsplit constant-rank data、target labelのprotected labels非所属 | G-aat-quality-surface-08 Cycle 17 | unported | #3595 |
| `ChartGeometricFiber.lawfulChartOpenOnSpace_isAffineOpen` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | selected lawful-space chartのimageをactual principal basic openへ同定し、open immersionからaffine性を証明 | なし | G-aat-quality-surface-08 Cycle 16 | unported | #3575 |
| `ChartGeometricFiber.chartAllowedOperationOnSpec` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | actual selected chartのaffine spectrum上にactual `allowedOperationSheaf`のrestrictionを固定 | なし | G-aat-quality-surface-08 Cycle 16 | unported | #3575 |
| `ChartGeometricFiber.allowedOperationChartTildeIso` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | actual chart-section moduleの`tilde`とactual restricted allowed-operation `Abelian.image`をcounit naturalityとcanonical image比較から同型化 | なし | G-aat-quality-surface-08 Cycle 16 | unported | #3575 |
| `ChartGeometricFiber.chartAllowedOperation_fromTildeΓ_isIso` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/ChartGeometricFiber.lean` | - | constructed selected-chart tilde比較からactual allowed-operation restrictionの`fromTildeΓ` counitがisomorphismであることを導出 | なし | G-aat-quality-surface-08 Cycle 16 | unported | #3575 |
| `AffineImageExactness.restrictMap_mono` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/AffineImageExactness.lean` | - | open immersionに沿うactual module-sheaf restrictionがmonomorphismを保つことをpresheaf componentのinjectivityから証明 | なし | G-aat-quality-surface-08 Cycle 15 | unported | #3577 |
| `AffineImageExactness.tildeMap_mono` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/AffineImageExactness.lean` | - | affine `tilde`がmonomorphismを保つことを各primeでのmodule localizationのinjectivityから証明 | なし | G-aat-quality-surface-08 Cycle 15 | unported | #3577 |
| `AffineImageExactness.restrictAbelianImageIso` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/AffineImageExactness.lean` | - | open restriction後のabelian imageをrestricted morphismのabelian imageとcanonical strong-epi/mono factorizationで同型化 | なし | G-aat-quality-surface-08 Cycle 15 | unported | #3577 |
| `AffineImageExactness.tildeAbelianImageIso` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/AffineImageExactness.lean` | - | module-category imageの`tilde`とinduced sheaf morphismのabelian imageをcanonical strong-epi/mono factorizationで同型化 | なし | G-aat-quality-surface-08 Cycle 15 | unported | #3577 |
