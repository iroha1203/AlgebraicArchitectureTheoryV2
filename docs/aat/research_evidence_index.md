...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `LinearRepair.exists_supportMinimalCircuit_of_no_normalizedRepair_of_target_ne_zero` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/LinearRepair.lean` | - | 有限protected response familyに対しactual repair vectorの存在とtarget responseのprotected span非所属を同値化し、zero targetをsingleton loopとして分離し、nonzero targetのrepair failureからtargetとprotected labelを含む有限support-minimal linear dependenceを構成 | なし | G-aat-quality-surface-08 Cycle 12 / PR #3324 | unported | #3282 |
