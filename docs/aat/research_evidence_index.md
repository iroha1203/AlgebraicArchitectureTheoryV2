...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `LawGeneratedLabeledConormal.requiredGeneratorConormal_span_top` | `AG/QualitySurface/IntrinsicLawResponseCircuitDescent/LabeledConormalGeneration.lean` | - | required `(lawIndex, atom)`の既存`violationWitness`がobstruction idealを生成することからambient・typed chart/overlapのfull `I/I²` conormal generationを導き、全labeled response零とfull response零の同値を証明 | なし | G-aat-quality-surface-08 Cycle 10 / PR pending | unported | #3282 |
