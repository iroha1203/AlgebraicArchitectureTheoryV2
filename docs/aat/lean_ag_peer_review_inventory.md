...
Issue as its tracking record. Only PRD-R B-level items or explicitly scoped
non-claims are recorded as `宣言`, `statement-only`, or future proof-obligation
boundaries; A-level items are not downgraded by cost.
