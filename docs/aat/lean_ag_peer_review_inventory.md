...
| IV-7 | `BoundaryResidue.lean` / `PeriodStokes.lean` | 部分昇格(差し戻し #3100) | `TwoChartGloballyUFlat D b` を `∃ s, b.b_U = d0 s ∧ d0 s = 0` に強め、単なる `∃ s, d0 s = 0` ではなく selected boundary mismatch に相対化した。`TwoChartBoundaryResolved` は `b_U = d0 s` の解消 predicate として残し、global U-flatness はその zero-boundary 強化として読む。`HolonomyKernelExactAtBoundary`、`DeltaKillsTwoChartBoundaries`、`boundaryResolved_of_boundaryHolonomy_zero_of_kernelExact` / `boundaryHolonomy_zero_of_boundaryResolved_of_deltaKillsBoundaries` / `not_boundaryResolved_of_boundaryHolonomy_nonzero_of_deltaKillsBoundaries` と `AxiomAudit.boundaryResidueTwoChartBoundaryResolved*` により、holonomy completeness/soundness 側を `BoundaryResidueHypotheses` の free field ではなく `ker(deltaH1) <= im(d0)` / `deltaH1 ∘ d0 = 0` の concrete exactness predicate から読む route に昇格した。13.2 は `IntervalBasisStokes.finiteIntervalStokes_basis` / `finiteIntervalConnectingStokes_basis` で one-simplex の basis 計算を certificate field なしに証明し、`AxiomAudit.finiteIntervalStokesBasis` / `finiteIntervalConnectingStokesBasis` で監査する。任意 cover の自動 Stokes 構成と derived Mayer-Vietoris triangle は未主張。 |
...
| VI-1 | Part VI toy models | 実装済み | #3099 で `Bool` / `Fin 2` ベースの concrete nondegenerate data instance を追加し、singular boundary、operation square、transport zero/nonzero、refactor Galois、decomposition gerbe の5 toy model firing theorem を発火済み。 |
...
| IX-3 | temporal product cohomology | 実装済み(#3100追補) | `TemporalCoefficient.FiberZeroCochain` / `FiberIncidenceOneCochain` / `incidenceDifferential` / `incidenceDifferential_id`、`ProductIncidenceComplex`、`ProductIncidencePRD4Comparison` に加え、`FinitePosetTemporalCechBridge.cohomology_to_from` / `cohomology_from_to` と product wrapper の `prd4_cohomology_to_from` / `prd4_cohomology_from_to` を追加。Part IX two-step example で `unitFinitePosetTemporalCechComparisonData`、`unitFinitePosetTemporalCechBridge`、`unitProductIncidencePRD4Comparison`、cohomology-level inverse theorem を発火する。さらに singleton-only でない evidence として `EvolutionPart9.twoPatchProductCech_differential_nonzero` が Part II two-patch finite-poset Čech fixture の非ゼロ degree-one differential を Part IX surface で読む。 |
...
| PRD-6 | AC20 | five finite toy models are selected finite witness packages. R7 で transport descent の空型 package は zero/nonzero case へ分割済み。#3139 review follow-up で operation-square filling は `mu axis = 0`、transport factorization は Bool quotient transport の equality/factorization predicate、refactor Galois arrow は state-equality-carrying finite hom、decomposition gerbe obstruction は `gerbeClass = zero` の否定として発火し、`False` / `True` / `PUnit` だけの空 witness ではなくなった。 | `proved example theorem` / `selected finite witness package` | `Formal/AG/Examples/SingularityMonodromyStackPart6.lean`, `toySquareFillingProblem`, `toyPresentedPi`, `toyRefactorGroupoid`, `toyNoCanonicalDecompositionData` |
...
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedEndToEndFromGeneratedInputs` | `AAT.AG.SemanticRepair.lawEquation_constructs_groundedComparisonPacket_fromGeneratedInputs` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `SemanticRepairLawEquationGeneratedInputs` grounding route | `Formal/AG/LawAlgebra/LawEquation.lean`, `Formal/AG/SemanticRepair/SagaComparison.lean`, `Formal/AG/SemanticRepair/Boundary.lean` | `LawEquationDefectSource`, `DisplayedRequiredLawsHoldOn`, `GeneratedRestrictionEvaluator`, `displayedRequiredLawsHoldOn_constructs_interpret_eq_zero`, `displayedRequiredLawsHoldOn_constructs_restrictionEvaluator`, `interpret_eq_zero_iff_defect_mem_obstructionIdeal`, `interpret_ne_zero_of_defect_notMem_obstructionIdeal`, `obstructionQuotientPresheaf`, `lawEquation_grounding_packet`, `SemanticRepairGeneratedEndToEndInputs`, `SemanticRepairNativeGeneratedEndToEndInputs`, `lawEquation_constructs_groundedComparisonPacket`, `lawEquation_constructs_groundedComparisonPacket_fromGeneratedInputs`, `lawEquation_constructs_groundedComparisonPacket_fromNativeGeneratedInputs`, `displayedRequiredLawsHoldOn_constructs_generatedSourceC0_zeroPackage` | law equation は degree-zero contribution と generated quotient coefficient surface を供給し、H1 / descent / global coherence は theorem 7.3 側の selected certificate と generated-input record に任せる。native generated route は係数を `G.obstructionQuotientPresheaf` に固定し、任意 `F` の再包装では theorem 7.5 の監査根拠にしない。 |
| `SemanticRepairLawEquationEndToEndInstance` | `Formal/AG/Examples/SemanticRepairPart10.lean`, `Formal/AG/AxiomAudit.lean` | `generatedF2QuotientPresheaf_nontrivial`, `generatedF2QuotientIsSheaf`, `generatedF2H1Realization`, `generatedF2EndToEndInputs`, `generatedLawQuotientPresheaf`, `generatedLawQuotientIsSheaf`, `generatedLawQuotientObstructionSheaf`, `generatedLawH1Realization`, `generatedLawNativeEndToEndInputs`, `lawfulFiring_generatedF2_endToEndPacket_fromGeneratedInputs`, `lawfulFiring_generatedLawQuotient_endToEndPacket_fromNativeGeneratedInputs`, `AxiomAudit.semanticRepairExample91GeneratedF2EndToEndFromGeneratedInputs`, `AxiomAudit.semanticRepairExample91GeneratedLawQuotientEndToEndFromNativeGeneratedInputs`, `lawfulFiring_generatedSourceC0Zero`, `circleNerve_nonzeroClassTransfer_packet` | 例9.1 lawful zero firing は #3102 の completion witness として native `lawEquationCore.obstructionQuotientPresheaf` route で theorem 7.5 を発火する。generated `F₂` route は非 `PUnit` coefficient の comparison witness として残し、legacy `PUnit` singleton route は完了根拠にしない。例9.2 nonzero circle witness は supplied circle-nerve simplicial data に相対化する。 |
...
| IV-7 | 部分昇格(差し戻し) | concrete `TwoChartGloballyUFlat` / `TwoChartBoundaryResolved` 実述語、2チャート soundness、holonomy kernel exactness からの boundary resolution、13.2 one-simplex basis 計算は実証明済み。任意 cover の自動 Stokes 構成と derived Mayer-Vietoris triangle は未主張。#3086 の docs-only closure では完了扱いしない。 | #3100 |
...
| V-3, V-4 | 部分達成 / 差し戻し | V-4 は `SelectedFiniteFreeResolution.exact` を Mathlib `Function.Exact` predicate に置換済み。V-3 は 2生成 presentation の face cardinality / pair multidegree / concrete `d₂,d₁` / regular-pair exactness / `coker(d₁) ≃ₗ[A] A/(a,b)` bridge に加え、具体係数環 `(ZMod 2)[x₀,x₁]` の `(X 0, X 1)` で nondegenerate regular-pair exact complex を証明済み。Taylor complex の一般完全性は未完了。#3086 の docs-only closure では完了扱いしない。 | #3100 |
...
| IX-3, IX-4, IX-5 | 実装済み | IX-3 は fiber-valued incidence differential、product-incidence complex package、finite poset × two-point trace の PRD-4 cohomology-level comparison instance を発火し、追加で `twoPatchProductCech_differential_nonzero` により two-patch finite-poset 非ゼロ differential を Part IX surface へ接続した。IX-4 kind 別 canonical constructor、IX-5 の `selectedConcreteClassNonzero := True` 解消と PRD-10 circle instance bridge も実装済み。 | #3100 |
