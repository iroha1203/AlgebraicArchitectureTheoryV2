...
| IV-7 | `BoundaryResidue.lean` / `PeriodStokes.lean` | 部分昇格(差し戻し #3100) | `TwoChartGloballyUFlat`、`exists_boundary_agreement`、`of_boundary_agreement` を追加し、`BoundaryResidueHypotheses.globallyUFlat` と soundness/completeness の入出力を concrete two-chart degree-zero cochain の boundary agreement `d0 s = 0` へ置換済み。さらに `TwoChartConnectingHomomorphism.boundaryHolonomy_zero_of_twoChartBoundaryAgreement` / `boundaryHolonomy_zero_of_boundaryMismatch_zero`、`BoundaryResidueGolden.zero_boundaryAgreement_forces_zero_holonomy`、`AxiomAudit.boundaryResidueTwoChartBoundaryAgreementSoundness` により、2チャート boundary agreement から `Hol_U = 0` への theorem-level soundness route を実証明した。13.2 は `IntervalBasisStokes.finiteIntervalStokes_basis` / `finiteIntervalConnectingStokes_basis` で one-simplex の basis 計算を certificate field なしに証明し、`AxiomAudit.finiteIntervalStokesBasis` / `finiteIntervalConnectingStokesBasis` で監査する。holonomy completeness 側の package 脱却と任意 cover の自動 Stokes 構成は未完了。 |
...
| IV-7 | 部分昇格(差し戻し) | concrete `TwoChartGloballyUFlat` 実述語と `BoundaryResidueHypotheses.globallyUFlat` への置換、2チャート soundness、13.2 one-simplex basis 計算は実証明済み。holonomy completeness 側の package 脱却と任意 cover の自動 Stokes 構成は未完了。#3086 の docs-only closure では完了扱いしない。 | #3100 |
