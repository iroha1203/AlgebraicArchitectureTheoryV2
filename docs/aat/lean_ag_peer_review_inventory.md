...
| IV-7 | `BoundaryResidue.lean` / `PeriodStokes.lean` | 部分昇格(差し戻し #3100) | `TwoChartGloballyUFlat`、`TwoChartBoundaryResolved`、`HolonomyKernelExactAtBoundary`、`DeltaKillsTwoChartBoundaries` を追加し、Boundary Residue の boundary mismatch 解消を concrete two-chart equation `b_U = d0 s` として読めるようにした。`TwoChartConnectingHomomorphism.boundaryHolonomy_zero_of_twoChartBoundaryAgreement` / `boundaryHolonomy_zero_of_boundaryMismatch_zero`、`BoundaryResidueGolden.zero_boundaryAgreement_forces_zero_holonomy`、`AxiomAudit.boundaryResidueTwoChartBoundaryAgreementSoundness` により、2チャート boundary agreement から `Hol_U = 0` への theorem-level soundness route を実証明した。さらに `boundaryResolved_of_boundaryHolonomy_zero_of_kernelExact` / `boundaryHolonomy_zero_of_boundaryResolved_of_deltaKillsBoundaries` / `not_boundaryResolved_of_boundaryHolonomy_nonzero_of_deltaKillsBoundaries` と `AxiomAudit.boundaryResidueTwoChartBoundaryResolved*` により、holonomy completeness/soundness 側を `BoundaryResidueHypotheses` の free field ではなく `ker(deltaH1) <= im(d0)` / `deltaH1 ∘ d0 = 0` の concrete exactness predicate から読む route に昇格した。13.2 は `IntervalBasisStokes.finiteIntervalStokes_basis` / `finiteIntervalConnectingStokes_basis` で one-simplex の basis 計算を certificate field なしに証明し、`AxiomAudit.finiteIntervalStokesBasis` / `finiteIntervalConnectingStokesBasis` で監査する。任意 cover の自動 Stokes 構成と derived Mayer-Vietoris triangle は未主張。 |
...
| IX-3, IX-4, IX-5 | 実装済み | IX-3 は fiber-valued incidence differential、product-incidence complex package、finite poset × two-point trace の PRD-4 cohomology-level comparison full instance を発火済み。IX-4 kind 別 canonical constructor、IX-5 の `selectedConcreteClassNonzero := True` 解消と PRD-10 circle instance bridge も実装済み。 | #3100 |
