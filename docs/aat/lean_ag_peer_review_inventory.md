...
| IV-7 | `BoundaryResidue.lean` / `PeriodStokes.lean` | 部分昇格(差し戻し #3100) | `TwoChartGloballyUFlat`、`exists_boundary_agreement`、`of_boundary_agreement` を追加し、global U-flatness を concrete two-chart degree-zero cochain の boundary agreement `d0 s = 0` として読む実述語を導入。`BoundaryResidueHypotheses.globally_UFlat` への完全置換、2チャート soundness、13.2 基底計算は未完了。 |
| IV-8 | `ObstructionSheaf.lean` Type-valued sheaf | 実質化 | AddCommGrpCat constructor 側を正とし既存 surface は凍結。 |
| V-1 | `Counterexample.lean` Tor1 nonzero | 未実施(差し戻し #3098) | selected principal kernel quotient / Tor bridge data に相対化された surface はあるが、concrete instance と certificate-free Tor₁ 実証明は未完了。 |
| V-2 | `Intersection.lean` Tor0 bridge | 未実施(差し戻し #3098) | Tor0 bridge は package field に残る。canonical `LawConflictPackage` instance と実証明 route は未完了。 |
| V-3 | `TaylorResolution.lean` | 部分昇格(差し戻し #3100) | `TwoGeneratorPresentation`、`face_card_le_two`、`selected_pair_multidegree_eq_union`、`TwoGeneratorTaylorResolutionPackage`、`resolvesQuotient_from_selectedExactBridge` を追加し、2生成 presentation の Taylor face cardinality と selected exactness bridge から quotient-resolution を読む assumption-relative theorem surface を導入。Taylor complex の自動構成・完全性の一般証明は未完了。 |
...
| VIII-5 | `Packet.lean` / `GAGA.lean` | 部分昇格 / 差し戻し #3100 | `SelectedFiniteHodgeTheoremPackage` と `AATGAGACertifiedFields.finiteHodgeTheoremPackage` に加え、`SelectedPeriodStokesTheoremPackage`、`SelectedTopologicalDebtTheoremPackage`、`SelectedDerivedConflictTheoremPackage` と certified field slots / accessors を追加。Period/Stokes・topological debt は arbitrary conclusion slot ではなく selected measurement-profile artifact の `Measured_M` witness から `InScope` を読む package として保持する。具体的 Stokes / cover-nerve theorem への完全接続は未完了。 |
...
| IX-3 | temporal product cohomology | 部分昇格 / 差し戻し #3100 | `TemporalCoefficient.FiberZeroCochain` / `FiberIncidenceOneCochain` / `incidenceDifferential` / `incidenceDifferential_id` に加え、`ProductIncidenceComplex` と `ProductIncidencePRD4Comparison` を追加。Part IX two-step example で `unitTemporalProductIncidenceComplex` / `unitTemporalProductIncidence_d0_eq` / `unitTemporalProductIncidence_d0_id` を発火済み。finite poset x two-point trace の cohomology-level comparison full instance は未完了。 |
...
| `AAT.AG.AxiomAudit.semanticRepairTheorem75GeneratedEndToEndFromRealization` | `AAT.AG.SemanticRepair.lawEquation_constructs_groundedComparisonPacket_fromRealization` | `propext`, `Classical.choice`, `Quot.sound` |
...
| `SemanticRepairLawEquationRealization` grounding route | `Formal/AG/LawAlgebra/LawEquation.lean`, `Formal/AG/SemanticRepair/SagaComparison.lean`, `Formal/AG/SemanticRepair/Boundary.lean` | `LawEquationDefectSource`, `DisplayedRequiredLawsHoldOn`, `GeneratedRestrictionEvaluator`, `displayedRequiredLawsHoldOn_constructs_interpret_eq_zero`, `displayedRequiredLawsHoldOn_constructs_restrictionEvaluator`, `interpret_eq_zero_iff_defect_mem_obstructionIdeal`, `interpret_ne_zero_of_defect_notMem_obstructionIdeal`, `lawEquation_grounding_packet`, `lawEquation_constructs_groundedComparisonPacket`, `lawEquation_constructs_groundedComparisonPacket_fromRealization`, `displayedRequiredLawsHoldOn_constructs_generatedSourceC0_zeroPackage` | law equation は degree-zero contribution を供給し、H1 / descent / global coherence は theorem 7.3 側の selected certificate と generated comparison に任せる。 |
| `SemanticRepairLawEquationEndToEndInstance` | `Formal/AG/Examples/SemanticRepairPart10.lean` | `lawfulFiring_endToEndPacket`, `h1Realization`, `lawfulFiring_endToEndPacket_fromRealization`, `lawfulFiring_generatedSourceC0Zero`, `circleNerve_nonzeroClassTransfer_packet` | 例9.1 lawful zero firing と例9.2 nonzero circle witness を分離する。例9.2 は supplied circle-nerve simplicial data に相対化する。 |
...
| IV-7 | 部分昇格(差し戻し) | concrete `TwoChartGloballyUFlat` 実述語は追加済み。`BoundaryResidueHypotheses.globally_UFlat` への完全置換、2チャート soundness、13.2 基底計算は未完了。#3086 の docs-only closure では完了扱いしない。 | #3100 |
...
| V-1, V-2 | 未実施(差し戻し) | `Example56LawConflictPackageFiring` は explicit package / principal-kernel data に相対化され、concrete instance と certificate-free Tor₁ / Tor₀ 実証明は未完了。 | #3098 |
| V-3, V-4 | 部分達成 / 差し戻し | V-4 は `SelectedFiniteFreeResolution.exact` を Mathlib `Function.Exact` predicate に置換済み。V-3 は 2生成 presentation の face cardinality / pair multidegree / selected exactness route を追加済みだが、Taylor complex の自動構成・完全性の一般証明は未完了。#3086 の docs-only closure では完了扱いしない。 | #3100 |
...
| VIII-3, VIII-4, VIII-5 | 部分昇格 / 差し戻し | VIII-3 finite/Fintype 実述語化と finite carrier lemma、VIII-4 Derived `LawConflictPackage` 接続、VIII-5 certified fields の measurement-profile artifact package と low-degree example firing は実装済み。Period/Stokes・topological debt の具体定理接続は未完了。 | #3100 |
...
| IX-3, IX-4, IX-5 | 部分昇格 / 差し戻し | IX-3 fiber-valued incidence differential と product-incidence complex package、IX-4 kind 別 canonical constructor、IX-5 の `selectedConcreteClassNonzero := True` 解消と PRD-10 circle instance bridge は実装済み。IX-3 の finite poset x two-point trace cohomology-level comparison full instance は未完了。 | #3100 |
...
| V-1/V-2 | package-relative surface / 差し戻し | `Formal/AG/Examples/DerivedPart5.lean` has `Example56LawConflictPackageFiring`, tying the example-5.6 principal kernel quotient calculation for `I_U=<xy>` / `I_V=<xz>` to a selected `LawConflictPackage` for the same ideals. This is still explicit package / principal-kernel data relative; concrete instance construction and certificate-free Tor0/Tor1 discharge are tracked in #3098. |
