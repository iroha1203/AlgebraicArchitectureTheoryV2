...
- 適用する: `lean-verified` 判定(research-loop G3)、
  `research_evidence_index.md` への登録、スパイン宣言、
...
[機械] タグ項目の CI 配線は後続の執行層 PRD で実装する。対象:

| 検査 | 規則 | 実装候補 |
| --- | --- | --- |
| 未使用 premise | §1.2 | mathlib `unusedArguments` リンター |
| docstring 欠落 | §3.2 | mathlib `docBlame` リンター |
| def / theorem 区別 | §3.4 | mathlib `defLemma` リンター相当 |
| 構文的恒真 statement | §1(空虚化) | mathlib `synTaut` リンター |
| 未使用 have / suffices | §1.2 | mathlib `unusedHavesSuffices` リンター |
| namespace 重複・不能 instance | §3 | `dupNamespace`, `impossibleInstance` |
| statement contract | §5.2 | 固定 signature の `example` 契約を CI で elaborate |
| 公理検査 | §4.1 | AxiomAudit(既存)の CI 実行 |
| import 方向(Research → 本体のみ) | refutation-checklist §6 | 既存 rg scan に加え `assert_not_exists` の Lean 内執行を検討 |
| 新規 premise の diff 検出 | §1.1 | changed declarations の premise 一覧を diff する script |

執行層 PRD の Acceptance Criteria はこの表を素材に書く。CI 未配線の
検査は、それまでレビュー(math-lean-review / 研究ループ品質監査)が
同じ規則で判定する。
