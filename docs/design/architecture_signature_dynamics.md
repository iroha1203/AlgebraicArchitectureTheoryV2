...
### Gap-oriented dynamics metrics

AAT + Dynamics で特に重要な差分は、現在の endpoint / axis-wise / measured な見え方と、
path / cross-axis / future distribution / unmeasured axis の危険性のずれである。

```text
locally safe
  does not imply globally safe trajectory

endpoint safe
  does not imply path safe

axis-wise safe
  does not imply cross-axis globally safe

same observed signature
  does not imply same future operation field

measured zero
  does not imply unmeasured zero
```

この節の指標は、既存の静的解析や endpoint diff では落ちやすい情報を、AAT の
`ArchitectureEvolution`、`SignatureTrajectory`、`FiniteOperationKernel`、
measurement boundary の上で扱うための候補である。Lean theorem として狙う場合は、
まず小さな finite counterexample / bounded theorem package に相対化し、tooling では
同じ形を report artifact として測定する。

| 指標 | 定義方向 | claim level | 意味 |
| --- | --- | --- | --- |
| `TransientExcursionDebt` | `max_t bad(σ_t) - max(bad(σ_0), bad(σ_T))` | Lean counterexample / tooling | endpoint では相殺されたが、途中で bad region を通過した量。 |
| `ForceCancellationRatio` | `1 - norm(sum Δσ_t) / sum(norm Δσ_t)` | Lean for additive axes / tooling | net delta が小さい一方で内部 churn が大きい window を検出する。 |
| `OperationCommutatorCurvature` | `d(σ(b ∘ a X), σ(a ∘ b X))` | Lean / tooling | operation の順序非可換性を signature 空間の曲率として読む。 |
| `CrossAxisClosureGap` | axis-wise safe だが cross-axis invariant が壊れた割合 | Lean counterexample / tooling | 多軸 signature を単なる独立カウンタではなく関係付き観測として扱う。 |
| `SupportRiskMass` | finite support 内で safe-preservation precondition を満たさない operation の重み | tooling / empirical | accepted history が安全でも、future support に unsafe operation が残る危険を測る。 |
| `SignatureAliasingRisk` | `O.observe X = O.observe Y` だが `kernel.support X ≠ kernel.support Y` となる state pair の割合 | Lean counterexample / tooling | 同じ current signature でも future operation field が違う状況を測る。 |
| `ObservabilityExpansionShock` | 新規 axis 追加で `unmeasured` から `measured nonzero` になった required axis 数・重み | Lean counterexample / tooling | 悪化ではなく観測可能化として扱うべき signal を分離する。 |
| `PathCompressionLoss` | endpoint-only report に圧縮したとき失われる excursion / commutator / rejected-force 情報 | Lean counterexample / tooling | 大きな PR や squashed history が内部 trajectory risk を隠す度合い。 |
| `EscapeCostLowerBound` | obstruction measure と 1 step decrease bound から得る最短 repair path 下界 | Lean theorem / search tooling | violation count ではなく、bad region から脱出する幾何的難度を測る。 |

これらはすべて「品質の単一スコア」ではない。むしろ、どの観測境界で安全に見え、
どの観測境界を広げると危険が現れるかを診断する multi-axis / trajectory-first な
指標である。
...
### Phase D7: gap theorem / non-conclusion counterexample package

AAT + Dynamics の最初の追加 theorem package は、強い正方向 theorem ではなく、
「局所・endpoint・axis-wise・measured に安全に見えることから、path・cross-axis・future
distribution・unmeasured axis の安全性は従わない」ことを示す bounded counterexample
package として切る。

この package は、AAT が endpoint diff や単一スコアではなく、path artifact、
operation algebra、future operation support、measurement boundary を第一級に扱う理由を
Lean 側で固定する。

| theorem / package 候補 | Lean での最小形 | 読み |
| --- | --- | --- |
| `ZeroNetForceNonZeroExcursion` | `EndpointSignatureDelta = 0` かつ start / target が safe でも、`SignatureTrajectoryInSafeRegion` は従わない小例。 | net force が相殺されても trajectory risk は消えない。 |
| `AxisWiseSafeNotGlobalSafe` | `Sig = A × B` で各 projection は safe だが cross-axis invariant が壊れる小例。 | 多軸 signature は独立カウンタの束ではなく、軸間関係を持つ。 |
| `SameSignatureDifferentFuture` | `O.observe X = O.observe Y` だが `FiniteOperationKernel.support X ≠ support Y` となる小例。 | current signature は future operation field を完全には決定しない。 |
| `ObservabilityExpansionShock` | coarse observation では safe だが refined observation の hidden axis が nonzero になる小例。 | unmeasured を zero と読まない原則を dynamics 上の theorem / metric にする。 |
| `PathCompressionLoss` | endpoint-only observation が等しい二つの path で、trajectory safety または excursion が異なる小例。 | PR / window を endpoint diff に圧縮すると内部 risk が失われる。 |
| `OperationCommutatorCurvature` | `MergeOrderSensitivity` の 0/1 metric を距離付き signature 差へ一般化する。 | merge conflict ではなく operation algebra の非可換性として順序感度を読む。 |

最初に Lean 化する優先順は次である。

1. `ZeroNetForceNonZeroExcursion`
2. `AxisWiseSafeNotGlobalSafe`
3. `SameSignatureDifferentFuture`
4. `ObservabilityExpansionShock`
5. `OperationCommutatorCurvature`

最初の四つは小さな finite counterexample として実装しやすい。`OperationCommutatorCurvature` は
既存の `TwoStepObservationCommutative` / `MergeOrderSensitive` /
`MergeOrderSensitivity` を、距離付き signature domain へ一般化する API 設計として扱う。

この package の non-conclusions:

- endpoint safe から path safe を結論しない。
- axis-wise safe から cross-axis invariant preservation を結論しない。
- 同じ signature observation から同じ future operation distribution を結論しない。
- measurement axis を追加して見えた obstruction を、architecture degradation と即断しない。
- PR / window の endpoint diff だけから internal trajectory risk がないとは読まない。
