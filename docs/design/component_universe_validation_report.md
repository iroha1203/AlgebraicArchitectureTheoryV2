...
で、ArchSig output を入力にこの report を生成する
`archsig validate --input ...` を実装した。
...
v0 report は ArchSig output を入力にする。

- 入力 schema は `archsig-sig0-v0` を対象にする。
...
ArchSig v0 では、local Lean source file だけを `components` に列挙し、`Mathlib.*`
...
最初の実装は既存 ArchSig output だけを読む pure validation pass とする。source
file を再 scan せず、入力 JSON に含まれる `components`, `edges`, `signature`,
`metricStatus`, 将来の `warnings` field だけから report を作る。

追加実装は次の順で進める。

1. `archsig validate --input ...` で schema / duplicate / edge classification を
   検査する。
