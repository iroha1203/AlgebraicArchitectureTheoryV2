...
などの外部 import target は `edges` の target として残す場合がある。このため report は
universe mode を明示する。

| mode | 意味 | `ComponentUniverse` への読み替え |
| --- | --- | --- |
| `local-only` | `components` に列挙された local component だけを universe とする。外部 target を持つ edge は closure 対象から除外し、warning として数える。 | local projection 上の universe 候補。外部 edge を含む graph 全体の witness ではない。 |
| `closed-with-external` | edge endpoint に現れる外部 node も synthetic component として universe 候補に含める。 | graph closure の検査には使えるが、source file coverage とは別扱いにする。 |
...
| `edge-endpoint-resolved` | 各 edge の source / target が component または external node として分類できれば `pass`。 | coverage / closure check の前提を固定する。 |
| `edge-closure-local` | `local-only` mode で local endpoint の edge が components 内に閉じていれば `pass`。 | local projection の edge-closedness 候補を検査する。 |
| `external-edge-targets` | 外部 target があれば `warn`、なければ `pass`。 | full import graph の witness と混同しない。 |
