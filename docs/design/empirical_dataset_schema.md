...
に置き、`runtimeFanout`, `unprotectedRuntimePropagationRadius`,
`circuitBreakerCoverageRatio` は `analysisMetadata.runtimeMetrics` に保持する。
...
`runtimeGraphMeasured = true`, `runtimePairCount = 0`, `runtimePropagation = 0` を許す。
...
| H5 | `runtimePropagation`, `runtimeFanout`, `unprotectedRuntimePropagationRadius`, `circuitBreakerCoverageRatio` | incident scope, repair time, rollback, hotfix size | runtime graph と coverage policy の測定状態が明示済み |
