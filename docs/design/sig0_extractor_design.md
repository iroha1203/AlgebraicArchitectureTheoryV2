...
Issue #51 の最小 CLI 実装では policy file をまだ読まない。そのため
`boundaryViolationCount` と `abstractionViolationCount` は placeholder `0` を出すが、
同時に `metricStatus` で `measured: false` と理由を記録する。これは違反なしの
測定結果ではなく、欠損値として扱う。policy file を読む後続実装では
[boundary / abstraction policy v0 schema](boundary_abstraction_policy_schema.md)
に従い、unique dependency edge を counting unit として violation count を出す。
...
- boundary / abstraction policy file の最小 schema は
  [boundary / abstraction policy v0 schema](boundary_abstraction_policy_schema.md)
  で固定した。後続実装ではこの schema を読んで violation count を出す。
