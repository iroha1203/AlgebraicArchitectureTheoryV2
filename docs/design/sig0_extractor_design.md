...
policy file は v0 の対象外である。そのため `boundaryViolationCount` と
`abstractionViolationCount` は placeholder `0` を出すが、同時に `metricStatus` で
`measured: false` と理由を記録する。これは違反なしの測定結果ではなく、欠損値として
扱う。
