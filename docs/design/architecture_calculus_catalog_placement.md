...
| `merge` | contract / observation package | `ArchitectureOperationKind.merge`, `OperationProofObligation.merge` | merged boundary が外部 contract を保つための observation equivalence と projection exactness。 |
| `isolate` | runtime / semantic localization package | `ArchitectureOperationKind.isolate`, `OperationProofObligation.isolate`, `RuntimeInteractionProtected` | dangerous dependency、runtime path、semantic drift が selected boundary 内に閉じる localization theorem。 |
| `protect` | concrete graph identity kernel + runtime policy package | `FiniteArchGraph.protect`, `ConcreteGraphOperation.protect`, `RuntimeInteractionProtected` | timeout、fallback、queue、bulkhead、circuit breaker など policy-aware runtime protection の bounded theorem。 |
| `migrate` | runtime / semantic package | `ArchitectureOperationKind.migrate`, `OperationProofObligation.migrate` | staged migration path、compatibility window、old / new architecture observation equivalence。 |
...
| `contract` | contract / observation package | `LocalReplacementContract`, `ObservationFactorsThrough`, `OperationProofObligation.contract` | implicit dependency / semantic expectation を explicit contract へ写す soundness と non-completeness 境界。 |
