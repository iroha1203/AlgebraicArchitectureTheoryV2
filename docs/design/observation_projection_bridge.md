...
後続で `LocalReplacementContract` のような bundle を導入する場合は、次の statement を
追加候補にする。
...
docs 上の設計整理として確定する。Lean status は既存 bridge theorem が `proved`、
局所契約 bundle は `future proof obligation` である。
