...
残す。Issue #126 では、`runtimeFanout` を v1 extension axis へ追加せず、実証分析用の
...
protocol 上の説明変数は次の 4 種類に分ける。

- `runtimePropagation`: raw 0/1 `RuntimeDependencyGraph` 上の propagation radius。
