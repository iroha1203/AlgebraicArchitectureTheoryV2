...
bounded horizon に相対化する。finite trajectory、selected initial states、finite operation
scripts、selected region、bounded horizon、source refs が欠ける場合は `unmeasured` /
`unavailable` / `notComparable` として出し、missing evidence を measured zero と読まない。
これらが揃う場合も、bounded simulation は candidate、non-candidate、missing-evidence
classification を分けて保持し、global basin stability や global recurrence theorem にはしない。
