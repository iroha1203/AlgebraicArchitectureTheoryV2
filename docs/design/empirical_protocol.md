...
| H5 | exploratory | runtime exposure / blast radius が大きい領域では incident scope と障害修正コストが増える。 | `runtimePropagation` / `runtimeExposureRadius`, `runtimeBlastRadius`, `runtimeFanout`, `unprotectedRuntimePropagationRadius`, `unprotectedRuntimeBlastRadius`, `circuitBreakerCoverageRatio` | affected components, repair time, rollback, hotfix size |
...
### H5: runtime exposure / blast radius と障害修正コスト
...
- `runtimePropagation` / `runtimeExposureRadius`
- `runtimeBlastRadius`
- `runtimeFanout`
- policy-aware extension としての `unprotectedRuntimePropagationRadius`
- policy-aware extension としての `unprotectedRuntimeBlastRadius`
- `circuitBreakerCoverageRatio`
...
- `runtimePropagation` / `runtimeExposureRadius`: raw 0/1 runtime graph 上で
  `edge c d` means `c depends on d` の向きに沿う到達範囲であり、Lean 側の
  `runtimePropagationOfFinite` から来る。既存の `runtimePropagation` は exposure 側の
  legacy alias として読む。
- `runtimeBlastRadius`: raw 0/1 runtime graph を逆向きに読んだ到達範囲である。障害源
  component が壊れたとき、直接・間接にその component へ依存して影響を受け得る範囲を
  測る。v0 では Lean core field に入れず、dataset / analysis metadata 側の派生値にする。
...
- `unprotectedRuntimePropagationRadius`: Circuit Breaker coverage を反映した
  exposure 側の policy-aware extension であり、raw `runtimePropagation` を置き換えない。
- `unprotectedRuntimeBlastRadius`: Circuit Breaker coverage を反映した blast 側の
  policy-aware extension であり、incident scope の説明では root cause component が
  分かる場合に優先して使う。
...
- affected component が特定できる場合は、その component からの runtime fanout と
  exposure radius を component-level に記録する。
- root cause component が特定できる場合は、その component への reverse reachability から
  blast radius を component-level に記録する。root cause が不明な incident では
  blast radius を primary analysis に使わず、欠損理由を metadata に残す。
- affected component が複数ある場合は、測定できた exposure / fanout / blast 指標について
  最大値、中央値、incident root cause component の値を分けて残す。root cause が不明な場合、
  root cause component の blast 値は `null` とし、affected component 集計とは混同しない。
- 30 / 90 / 180 日の観測窓で、runtime 指標が高い component 周辺の incident scope と
  repair cost の分布を比較する。incident scope の主説明変数候補は blast radius とし、
  外部依存への露出や依存先障害への脆弱性を見る分析では exposure radius を使う。
...
- `runtimeBlastRadius = 0` は、測定済み runtime graph 上で reverse reachable component が
  ないことを意味する。root cause component が不明で blast radius を計算しない場合は
  `null` とし、0 に丸めない。
...
- runtime exposure / blast 指標を component-level で測ったか repository-level で測ったか
