...
`nilpotencyIndex`, `rho(A)`, `relationComplexity`, `empiricalChangeCost` は v0 extractor
の対象外である。`runtimePropagation` は `--runtime-edges` で runtime edge evidence が
与えられた場合だけ、0/1 `RuntimeDependencyGraph` projection から dataset 側の
optional axis として埋める。runtime evidence が未指定の場合は `runtimePropagation = null`
かつ `metricStatus.measured = false` の欠損値として扱い、risk 0 に丸めない。
`runtimePropagation` は exposure radius の既存互換名であり、`edge c d` means
`c depends on d` の向きに沿う到達範囲を表す。incident scope 用の
`runtimeBlastRadius` は reverse reachability 由来の analysis metric として分ける。
...
では、`tools/archsig/` に Rust 製の最小 CLI を置く。実行例は次である。
...
Issue #128 では `--runtime-edges` で `runtime-edge-evidence-v0` JSON を読み、
`source`, `target`, `label`, `failureMode`, `timeoutBudget`, `retryPolicy`,
`circuitBreakerCoverage`, `confidence`, `evidenceLocation` を `runtimeEdgeEvidence`
として保持する。projection rule は `runtime-edge-projection-v0` であり、runtime evidence
が 1 件以上ある local component pair に `kind = "runtime"` の 0/1 edge を立てる。
入力がない場合は `runtimeDependencyGraph` を省略し、dataset 変換時の
`runtimePropagation` は未評価の `null` とする。入力がある場合は空 edge set でも
測定済み graph として扱い、到達範囲が空なら `runtimePropagation = 0` になる。
この値は exposure 側の測定済み 0 であり、root cause component 不明の
`runtimeBlastRadius = null` とは区別する。
...
- boundary / abstraction policy file の最小 schema は
  [boundary / abstraction policy v0 schema](boundary_abstraction_policy_schema.md)
  で固定し、Issue #115 で ArchSig がこの schema を読んで violation count を
  出す実装を追加した。
- extractor output と `ComponentUniverse` の責務境界を検査する validation report の
  ...
  で固定し、Issue #116 で `archsig validate --input ...` として実装した。
...
- 複数 repository に対する signature 時系列と PR metadata を結合する empirical dataset
  ...
  Issue #117 で `archsig dataset --before ... --after ... --pr-metadata ...`
  として単一 PR record の生成を実装した。
- GitHub API の PR detail / files / reviews JSON から dataset input 用 PR metadata を
  生成する補助 CLI は Issue #146 で
  `archsig pr-metadata --pull-request ... --files ...` として実装した。
- runtime edge metadata から 0/1 `RuntimeDependencyGraph` へ落とす projection は
  Issue #128 で `--runtime-edges` と `runtimeDependencyGraph` output として実装した。
  Circuit Breaker coverage を障害伝播低減として読む policy-aware metric は Issue #127
  と Issue #163 で raw `runtimePropagation` とは別の
  `unprotectedRuntimePropagationRadius`, `unprotectedRuntimeBlastRadius`,
  `circuitBreakerCoverageRatio` として設計した。実装は後続 tooling の責務とする。
