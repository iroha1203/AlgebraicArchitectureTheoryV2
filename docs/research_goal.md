...
AI 駆動開発では、この問題がさらに前面に出る。
AI agent は短時間に多くの proposal を生成し、既存のレビュー感覚や暗黙知をすり抜ける
shortcut も作りやすい。
...
- AAT（Algebraic Architecture Theory / 代数的アーキテクチャ論）は、変更が何を保存し、何を破るかを問う。
- SFT（Software Field Theory / ソフトウェアの場の理論）は、変更が未来の進化をどう形づくるかを問う。
